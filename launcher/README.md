...
│
├── contracts/
│   └── panel-contracts.v2.json            NPC/KShop/Crafting 跨层命令能力、AS2 业务裁决、数值权威、交互策略与共享边界向量登记表
│
├── src/
...
- **Panel pending 清理**：NpcShop/Crafting 共用 `PanelPendingCallTracker<TContext>`，只收敛机械 correlation、Timer、去重与单终态；业务 write/reconcile 仍由各 Task 裁决，early / bus-only / normal 三条退出路径均显式 `Dispose()` 两个 Task
