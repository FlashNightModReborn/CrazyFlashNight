...
Codex 的装备装配以 `flashswf/arts/new/Codex专用素材/Codex专用素材.xfl` 及完整 XFL 目录为唯一编辑源，由 CS6 直接发布旁边的 `Codex专用素材.swf`；不再维护同名 FLA。此库按作者隔离，避免与现有作者共写同一源文件。导出锚点 `Codex专用素材` 经 `flashswf/arts/things-new.fla` 的共享库导入元件和第 4 帧实例挂载；主文件继续沿已有 `things-new` 导入入口使用内部 AS2 链接。新增库须发布库本体与 `things-new.swf`，不因此重编 main。本节合同适用于一切新增独立素材库（装备、敌人、载具等，先例含 `素材库-诺亚新敌人`）：新库以自身 XFL 目录为唯一编辑源并发布自身 SWF；需要主文件使用时一律按实施顺序第 4 步经 things-new 导入元件挂载。地图静态摆件不建独立运行时库，见下文「静态载具进入地图」。
...
犀牛 R09 的地图摆件以 `地图-军阀基地` XFL 内 `Codex/犀牛R09/` 为静态矢量真源，通过 [rhino-map-assets](../tools/rhino-map-assets/README.md) 向两张战斗背景派生最小闭包。外交基地保留25号犀牛，前线第三图使用22/23/28号犀牛，防御地带首图使用29号抬炮犀牛。猛虎G05交接包的G04最终稿以同一外交地图内 `Codex/猛虎G05/` 为静态真源，通过 [tigr-map-assets](../tools/tigr-map-assets/README.md) 接入外交基地一辆、前线第二图三辆及防御地带首图一辆（替换卡车）。磁暴坦克G08从归档E3水平SVG经 [tesla-map-assets](../tools/tesla-map-assets/README.md) 派生单帧原生矢量Graphic，在外交基地替换27号犀牛；外交三车为犀牛25、猛虎41、磁暴，当前世界X为600/1010/1420。外交现役倍率下，磁暴车体显示长为犀牛约1.126倍、猛虎约1.104倍；不得把犀牛全外框或已退出场景的倍率混入车体长度换算。载具均随已有背景烘焙，不新增独立运行时素材库、全局linkage或碰撞；恢复动态演出时必须重新处理背景烘焙/卸载与动态层生命周期。
...
战技视觉供体递归净化、运动复用、固定矩阵余迹与连续观感核对，见[Flash战技特效装配与动态验收经验](../docs/Flash战技特效装配与动态验收经验-2026-09-11.md)；其中保留滑动喷泉被人类否决的反例，不以静帧或工程通过代签动态验收。
...
Three.js 布局原型到 Blender 精修的坐标、可编辑源与渲染合批分工、固定光影及语义选择方法，见[三维选关场景创作与交接经验](../docs/三维选关场景创作与交接经验.md)。其中 BM-009、BM-010 来自黑铁会总部美术回包，保留案例验证范围；相对尺度沙盘不直接套用上文人形与武器标尺。布局采用事实、配置参数和源码分别回到文中链接的工单与生产入口，不以历史回包的“项目长期记忆”覆盖后续迭代。
