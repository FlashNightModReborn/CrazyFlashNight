...
最后一条命令会从受保护的单路径段 `runtime-build-v2/<release-id>` source tag 触发固定 cloud workflow，并验证 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 都精确绑定请求的 full commit；随后等待精确 run、安全解压并产出 `$cloud.candidateRoot` / `$cloud.proofPath`。unsigned job 交接 artifact 保留 1 天，失败诊断与 signed 结果保留 7 天；超期未 promotion 就重新 dispatch，不把 Actions artifact 当长期档案。request、队列/CAS、双故障域 quorum、receipt 与 `promote-runtime-bundle.ps1` 的完整步骤以 [runtime v2 发布列车](../docs/runtime-build-reproducibility.md) 为准。最近一次完整 Runtime Lane C 是 clean F `cb38600aae51f5019d09f87c33bd9e67d2b1f511` 的 11/11、scalar 566、831.271 秒；该 Lane 仍不能单独替代环境门或双 builder，而本轮 PlayerInfo 已另由 request `839C74FD…8FB2` 的 X509 + GitHub OIDC 与 promotion `-VerifyOnly` 闭合非部署 quorum。现役正式 release 仍是材料/商城/战备箱 request `9AC9E3C9…53771` 经 promotion commit `40675cae…3723a` 达到的 `standard_entry_verified`；PlayerInfo F 没有 promotion、标准入口或部署。功能回归证据统一维护在 [测试指南](../agentsDoc/testing-guide.md)，正式发布记录见 [材料、商城与战备箱关闭证据](../docs/evidence/material-kshop-battlebox-workbench-closure-2026-07-28.md)。任何时候都禁止手工换 manifest、伪造证明，或把单机 candidate 复制进根 runtime。
