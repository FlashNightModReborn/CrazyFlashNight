...
2026-07-29 的 B7 施工从 commit `c96f4c3d750561022b706c72a4d53050431e627d` 起步；当前 HEAD 与 `origin/main` 均为 `c6ca84714d134310793defda893772aac2c0d5ea`，其上工作树继续承载 source-ahead 加固。B7 及其后续源码虽已通过各自自动门，但尚未形成 immutable candidate 或正式发布身份。下文把它写成当前源码真相，不把起始锚点或 source-ahead 工作树与上一段已经 `standard_entry_verified` 的旧冻结身份混同。
...
Native HUD `MATERIALS → openMaterialUI → CraftingPanelService.openMaterialsPanel` 由 Host 先建立独立 material wait 并生成 opaque `openRequestId`；AS2 只在合法 token 时把它原样回显到 `panel_request` 顶层。Host 只准入 exact `{task:"panel_request",panel:"crafting",source:"nativehud_materials",initData:{view:"materials"},openRequestId}`，随后重建固定 `{mode:"runtime",view:"materials",source:"nativehud_materials",debug:false}`，Web 进入 `crafting-materials.js` 的只读材料视图。省略 token 的旧 AS2 envelope 仅在 Host 没有 armed material intent 且没有 material wait 时保留 ordinary open；显式畸形 token 在 AS2 零发送，pending 时的 missing nonce 拒绝但不消费 wait，nonce-bearing wrong/near-match 则按当前目标失败处理。该入口不再设置旧 material-only flag、不打开旧 `物品与技能相关界面`，发送或挂载失败时只明确报错，禁止保留 Flash fallback。目录复用 `ItemUtil.materialDict` 与现有材料注释，详情复用 `ItemObtainIndex`、`SynthesisIndex` 和敌人名称投影，分别呈现哪些怪物/关卡可获得以及材料会用于哪些配方/装备；Web 不扫描 XML、不推断掉率，也不新增平板电脑或巴别塔文档迁移范围。材料视图独立持久化 `cf7.itemgrid.mode.crafting-materials`，无偏好时默认紧凑 7 列图标格，完整模式恢复 2 列可读文本；切换须同步键盘列数并保持选择/焦点。顶栏共享帮助说明筛选、密度、来源/用途和方向键，不另造材料专用模态系统。
...
`PreparationNavigationV1` 当前默认 on 时，Native/legacy HUD 把原七项“游戏”行原子拆成“游戏”与“整备”：游戏固定为战队、平板、商城，整备 frozen tuple 固定为装备、战备箱、装备调制、技能、材料、情报；显式 off 或非法配置值才整体恢复旧 HUD。`MATERIALS` 不复用 Character Build session，而是在无 Web panel/Character authority 冲突时先建立 material wait，再发送带 Host opaque `openRequestId` 的 `openMaterialUI`；只有 AS2 顶层 exact echo 与 Host admission 同时成立，才以固定 runtime initData 打开 Web 材料目录。旧 `__legacyMaterialOnly` flag 与旧物品界面“材料”帧不再属于入口链，也不保留发送/挂载失败 fallback。发送失败或已有活动面板时 fail-closed 并给出可见提示，不能把旧 Flash UI 叠在 Web 会话上。

当前工作树已实现 B0–B7 的协议、mutation、已穿戴调制、默认入口、材料隔离、Materials/Intelligence exact forward handoff 与 H1 exact reverse return、关闭/存盘屏障，以及默认 on 的两套 HUD/Build menu/Host focus/Web fixed mapping 原子切换，并完成 Character Build C1 分类计数的 AS2 → Host → Web 只读纵切；Character Build ↔ Skills exact 返回、workbench nonce 与热重载/断线撤销的正式发布状态仍以本节开头的旧 release identity 为准。B5–B7/C1/H1 后续源码已有同树 focused Host/Web 自动证据；当前 exact candidate 仅用于 150% DPI 双宿主浏览器输入 smoke，严格仍是 `candidate_built / NOT_DEPLOYED`，没有执行 Character/B7/H1 业务旅程，也未完成游戏内 E2E、GUI 人工验收、promotion 或部署。现役英雄 smoke、完整退出/重启回读和默认 on/显式 off 两套 HUD 实图仍按角色构筑专题与 [testing-guide.md](testing-guide.md) 验收。
