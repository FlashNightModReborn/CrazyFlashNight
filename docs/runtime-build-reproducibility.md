...
**最后核对代码基线**：本轮发布以 `b072f97841ccb30e167c14495241ae64d9054e22` 为 pre-push upstream base；`release/map-loot-20260721-final` 已由 commit `6218f8b1d82efc57b77131616667fe45f3033297` 记录正式 promotion。该部署冻结 source commit `c60aab2386aee4516608397373ae4c59148c5f77`（tag `runtime-build-v2/20260721-map-loot-canary-v1`）、release tree `e1e6e059f148a640b3e37cbf1d912f297156d956` 与 request `FBA91942C57DC3E6662AB77AC88EEAE3CA2AFA3C82BF5FFEA6BC68CFD6C31AE4`；唯一 canary 已在后续正式入口实机达到 `standard_entry_verified`。Flash publish-only 产物与 fresh AS2 trace 仍是独立证据层，不能替代 Core proof、quorum、promotion 或正式入口领域 smoke。

## 当前迁移状态

runtime v2 的工具、schema、队列、本地 X509 证明、GitHub OIDC/Sigstore 证明、promotion 与 CI 状态机已经完成正式闭环；**仓库当前受控部署已是 manifest/consensus v2**。当前 consensus 的 immutable source tag 为 `runtime-build-v2/20260721-map-loot-canary-v1`，request 为 `FBA91942C57DC3E6662AB77AC88EEAE3CA2AFA3C82BF5FFEA6BC68CFD6C31AE4`，source commit / release tree 分别为 `c60aab2386aee4516608397373ae4c59148c5f77` / `e1e6e059f148a640b3e37cbf1d912f297156d956`；artifact source `FCAF63616D73EE54912CC71332B38E318E9F087B18DC8BA19C923A065820F92A`、producer recipe `75723146B70547AD1B87F54A19A806A356501290D1507CBD43952ECB9C5906EE` 与 toolchain lock `CA5BCE58D3BBE7C77D37454B89651F4FB3F7D3F678007299BB641D5E8CE2A17F` 形成 build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`，payload closure 为 `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`，manifest SHA-256 为 `C834553F0A422FC42A4F2EC0CE0B9AE85E6A84EA7E31781CAA8BDF1ACCF8944B`。production policy `18ACDBBA77EDED4F25E898ACD695131CB718064ABC3E1930A56B8BC590A199A7` 的 receipt SHA-256 为 `55C7256E98393A01F87E51D880E37FEF2078379748D8BFCA8772AA112AAD2C4D`；`builder-local-b` / `physical-host-b` 与 GitHub OIDC / `github-hosted-windows` 的双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier 全部满足后，于 `2026-07-21T16:39:47.2329893Z` 完成 promotion。
...
本轮发布以 `b072f97841ccb30e167c14495241ae64d9054e22` 为 upstream base；当前正式部署对应 source commit `c60aab2386aee4516608397373ae4c59148c5f77`、build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`、payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C` 与 Core SHA-256 `0F58BF864B8DE9C7FCEA098D7E1EEA1996BDE38D85D87E844B047B53F5247232`，已包含本次 loot Host。2026-07-20 20:30:38 合法 nested loot open 后 Router 返回 `unsupported`，只代表当时仍加载 equipment-inspector 的 B643/FA1 旧正式 Core，是历史诊断而非当前部署状态。因此“native 源码可先以 `source-ahead` 进入 main”“从隔离目录启动 diagnostic Core”和“正式游戏加载已 promotion 的 Core”仍是三件不同的事。root EXE、`runtime/**`、manifest、builder registry 或 consensus 若随 source-only 提交变化，事后 Audit 必须转入 strict 并在无匹配 promotion 时失败。
...
tracked registry 继续保留 `builder-local-a` / `physical-host-a` 与 `builder-local-b` / `physical-host-b` 两张公钥；本次已部署 consensus 实际采用 `builder-local-b` 的 keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5` / `physical-host-b` 和 GitHub OIDC builder identity `394B7E898FD83FB60FA0DCD04223BC9A791346080CCAC5838C8F4D22B52A99C8` / `github-hosted-windows`，`builder-local-a` 未参与本次 quorum。任一单独本地票都仍不构成 quorum，也不授权单机 promotion。

当前换机账户仍不持有 `builder-local-a` 的历史不可导出私钥，也没有复制该密钥。维护者确认本机不是 `physical-host-a` 后，已按 enrollment 流程创建 `builder-local-b` / `physical-host-b` 的 3072-bit CurrentUser 不可导出 RSA key：keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`、thumbprint `141A0B12F18A1C25C2BF4A32B3C279F81C44D007`、epoch `1`；公钥 entry 已进入 `c60aab2386aee4516608397373ae4c59148c5f77` 的 immutable tree，私钥只留在本机证书库，并已实际签发本次 local proof。不同故障域的第二票已由 GitHub hosted OIDC/Sigstore 提供，双 faultDomain quorum 已满足；dirty staged/unstaged/untracked 工作树仍不能冒充 source commit。
