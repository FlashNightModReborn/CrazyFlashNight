...
## 2026-09-20 黑铁会总部集成：人类验收候选

本轮用户授权将 R8-I1-H1 集成到项目，并明确实际游戏验收交给人类。状态为 Web 源码和本地资产候选完成；没有改 C#/AS2 协议、重编 SWF、替换 runtime 或提交推送。不将下述浏览器 mock Host 检查称为真实游戏验收。

后续E1镜头/环境候选：默认47°俯视、偏正面10°；总览提供原镜头与周边环境对比。连续地面、外街与低层后勤/生活体块独立生成，不修改总部核心GLB或入口世界坐标。两种镜头分别派生标签位置；故障回退使用旧镜头截图及其匹配锚点。检查与人类观察点仍在[总部候选说明](黑铁会总部游戏集成候选-2026-09-20.md)，未作实际游戏人验通过结论。

E2曾依用户反馈默认采用原镜头放大12%，保留E1正面与原镜头对比；增加可关闭的外部断续圆环、低界石与候礼小坪太极地纹。以既有敌人对白“圆满方正”为视觉依据，纹样及天圆地方解释属于设计提案。道路直穿、世界锚点与两级祭仪不变；该地纹方案随后被E3替代。

E3进一步依用户反馈替代E2地纹：撤去完整圆环、界石、太极和纹样开关；使用不闭合的西侧旧街、前方支巷、坡脚检修路与转向/错落房屋暗示外围秩序。放大的原镜头、主门/侧门直达与内部布局保留。E1/E2仅作过程记录，当前人验候选为E3。

E4依据测试群截图改进选关UI：总部地图短标签按堂口方向排列，同时约束连接线与其他标签/连线的交叉；正式全称仍用于无障碍、资料与请求。总部和废城统一采用带深色底边的加粗连接线，键盘焦点联动建筑高亮。用户认可整体模型方向，UI改进仍交给人类验收；当前人验候选为E4，模型沿用E3。

收口：用户在E4交付后明确回复“可行，提交推送”，本轮采用E4界面与E3环境并完成授权源码交付。该整体认可不补填尚未执行的真实进关/返回和目标GPU专项记录；检查范围与后续回归点见[集成说明](黑铁会总部游戏集成候选-2026-09-20.md)。本次提交不修改正式runtime部署字节。

- 采用包 `黑铁会总部-R8-I1-H1-素材与经验-20260920.zip` 的 SHA-256 为 `5f6096e4807a35d37e96a80a81e3ba60598bf9f507a38efa24e0f18116afaee9`。主体/选择 GLB 字节保留；原生编辑源为包内 `工程/model/full/blackiron-hq-r8-i1.blend`，不把合批 GLB 当作唯一编辑源。
- 运行资源在 `launcher/web/assets/stage-diorama/blackiron-hq/`；`tools/import-blackiron-diorama.py <采用包.zip>` 确定性导入，`--check` 校验18项输入/派生文件及9MB范围。实际清单与体积只读其manifest。共用现有Three r180核心，新增随包后期模块。
- 场景管理器按选关页加载废城或总部。总部关闭/离页释放场景，取消未完成载入；迟到结果按generation废弃。废城保持原重试路径与游览行为。镜头预设按场景隔离，未修改玩家存档。
- 总部保留固定烘焙夜景、柔光和独立选择范围。总览/特写/游览共用单一Canvas及现役相机合同，普通关卡复用简报/情报/难度/出发；外交走原有直接跳转。回包的演示状态和按钮程序不进入正式选关。
- DOM顺序改为边缘→翅虎→火凤→黑龙→总堂；不沿用将翅虎外围编号01、总部边缘编号02的演示序号。入口ID、世界锚点、玩法数据不变；40px命中区使用有界屏幕偏移避免邻近点重叠，标签与命中区共同检查。
- 总部加载失败/上下文丢失时显示同镜头二维总览，保留原入口与决定面板，提供重试。没有另造常态/遇袭模型或增加第12个入口。

已执行原废城模型/交互/30次缓存开关、双栏聚焦、游览输入回归；新增总部布局检查与可复用完整集成测试。用户选择由人类做最终游戏验收，因此不把完整总部宿主旅程或30次总部开关标记为本轮已通过。使用说明、精确已跑命令及观察点见[总部候选说明](黑铁会总部游戏集成候选-2026-09-20.md)。

<a id="2026-09-09-废城固定镜头首版当前发布范围"></a>
## 2026-09-09 废城固定镜头首版（历史发布范围）
...
- 快捷选关：悬停卡片是否遮挡当前入口、难度按钮能否少移动直接出战，地点名称是否始终位于卡片下方。
- 聚焦详情：建筑特写能否对应关卡、图片与简报是否易读，奖励图标和敌人头像的提示是否好找；二维地图也应完整保留详情和出战。
- 游览取景：鼠标旋转与 WASD / 方向键同时操作是否自然，结束游览能否回到原视角；开发工具和预设文本区能否收起且不干扰预览。
- 通关情报：未有记录时隐藏新增情报；完成真实战斗并正常保存、重启后，已完成难度和情报应保留。旧存档缺少历史记录需要再完成一次，不从已有任务进度推断。

反馈优先附地图/关卡、难度、触发操作和截图；涉及流畅度时说明设备及是否正在游览。上述现场反馈独立于自动构建和部署完整性结论。
