/*武器表现效果要求：
1、武器主体作为冷兵器使用，装备入冷兵器槽，收纳时默认切换动画到收纳形态
2、在长枪没有装备时强制注册，通过动画切换实现枪械形态改变
3、在冷兵器模式下武器自体具备三种形态（钝器、长矛、镰刀，各自对应短、长、中距离），不同形态下使用不同的攻击动画（接口），且伤害类型、基础数据、刀口（位置和大小）根据形态发生改变
4、通过战技或其他方式实现强化状态（同样通过动画来实现形态转换），强化状态下攻击具有特效或数据增强，且武器切换不影响状态，一定时间或攻击次数后强制进入冷却
5、防止穿帮，武器卸下或是切换到其他武器时战技强制结束
6、战技启动时再次按下战技键触发二段战技，并强制结束战技*/
...
_root.装备生命周期函数.死者之手初始化 = function(反射对象, 参数对象) 
{
   ...
   {
      子弹威力 = split_temp / 100 * 自机.刀属性.power;
   }
   else
   {
   ...
   if(_root.兵器使用检测(自机))
   {
      if (_root.兵器攻击检测(自机)) 
      {
         if(超载许可)
         {
            ...
            {
               var 刀口 = 刀.刀口位置3;
               MuzzleWorldShoot.populate(刀口, 自机, 反射对象.子弹属性);
