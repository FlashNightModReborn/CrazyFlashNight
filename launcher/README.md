...
> **PlayerInfo 当前施工边界**：historical v1 继续只作旧字节的非部署审计；首列 main-space v2 q3 也因 qualification 漏挂 `PlayerInfoRasterPlan.cs` 使 production policy 仅 22/23 而退役，未形成 cloud/quorum/preflight。修复后的 F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` 已由全新 r2 tag/request 完成双 builder、双 faultDomain、23/23 policy 与不可复用的 `-VerifyOnly` preflight，自动门和 Kimi K1 均已闭合；`-VerifyOnly` 单独仍只到 `candidate_built`，随后 exact isolated candidate 已以 `p50` fixture 实际启动并保持旧 Flash HUD untouched。维护者已在真实游戏 composite 接受同屏旧 HUD 未改、z-order/遮挡、球体可见、HP glow、MP 文字与蓝条对齐、crop/透明、鼠标透传、动效和整体审美，观测环境为 144 DPI / dpr 1.5；B0 当前为 `b0_accepted`，oracle 为 `oracle_frozen_for_b0`。Launcher 生命周期仍严格为 `candidate_executed / source-ahead / NOT_DEPLOYED`；fixture surface 仍不接真实 `pi_*`、不隐藏或修改旧 Flash HUD，也未证明额外物理显示器切换、跨 renderer pixel parity、业务 E2E、promotion、标准入口或部署。精确 identity/closure/Core、PID、日志 marker 与人类 receipt 见 [runtime v2 深层文档](../docs/runtime-build-reproducibility.md)和[PlayerInfo 工具证据](../tools/player-info-hud/README.md)，验收范围见 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)与[验证矩阵](../agentsDoc/testing-guide.md)。
