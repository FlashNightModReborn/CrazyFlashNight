...
| 标签 | 含义 | 证据基础 | 使用方式 |
|------|------|---------|---------|
| `[T1a]` | **硬规则 · 零前提** | benchmark + BytecodeProbe 双重确认，替换无语义风险 | 新写代码直接遵循；存量代码可批量替换（仅限写法层面的等价变换） |
...
### 2a. 零前提（新代码直接遵循，存量可批量替换）

> 每条均有 benchmark + BytecodeProbe 双重确认，属于写法层面的等价变换，无语义风险。
...
- 编写性能敏感代码时，**先查 §2a T1a 硬规则**——新代码直接遵循，存量代码可批量替换
