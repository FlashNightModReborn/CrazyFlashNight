...
(function() {
    var 分界等级点数表:Array = [0, 150, 350, 600, 950, 1450, 2050, 2850];
    var 分界表长度:Number = 分界等级点数表.length;
    for (var lv:Number = 0; lv <= 技能点数最大预计算等级; lv++) {
        var 索引:Number = (lv / 10) >> 0;
        if (索引 >= 分界表长度) 索引 = 分界表长度 - 1;
        var 点数:Number = 分界等级点数表[索引];
        if (索引 < 分界表长度 - 1) {
            点数 += ((lv % 10) * (分界等级点数表[索引 + 1] - 分界等级点数表[索引]) / 10) >> 0;
        } else if (lv > 70) {
            点数 += (lv - 70) * 100;
        }
        技能点数查找表[lv] = 点数;
...
_root.主角函数.初始化可用技能 = function() {
    ...
    // 直接查表获取技能点数
    var 技能点总数:Number = 技能点数表[等级] || 技能点数表[_root.技能点数最大预计算等级];
