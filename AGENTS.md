...
**状态边界**：本列车把斗兽标定从工作簿原始来源摄取、参数闭包、`contamination` Schema/fixture、timeout 口径和真正 Schema 实例校验，收束到可冻结、可撤销、可持久恢复的 Gate F 星期级批处理。v7 计划含 58 个 scheduled candidate，另将 `B12` 隔离为 quarantine，共 198 个 10–25 run shard / 3,285 个机器 run；三份 fresh soak 各为 10 个代表样本 + 10 个派生阵营复核样本，`C9` 仍保留 `10 + 20 + 25` run 与 side-swap 全量复核。
...
提交后正式 admission 已在 exact formal runtime 上完成 20/20 finished、0 timeout/error/recovery、正常 shutdown 和受保护存档不变，目标死亡派生实际观察 red 8 / blue 8；`arena-calibration.soak-admission.v2` 将原始 manifest/result/report SHA-256、双向派生最低计数与案例模板闭合，准入 hash 为 `sha256:60c5662d89b5b8f0e53cbe288ea1461ef32a933aff654dd4171dadf8fac396a0`。v7 随后完成三份 20-run fresh soak：第一份 20/20 finished，第二、三份各为 19 finished + `E10` 一向 1 timeout，三份均 0 error/recovery、正式 runtime、存档不变并正常 shutdown；旧 controller 让 `soak` 沿用全局 `maximumTimeoutRate=0.05`，错误地把恰好 1/20 timeout 写成 completed。该 window 已 revoke，v7 的 60 行只作控制缺口与 E10 不稳定证据，不开放剩余 195 shard。现役修复要求任何 `soak` error/timeout 计数非零都 fail closed；下一 campaign 必须以新鲜正式双向 probe 替换 E10、重建 admission/plan 并重跑三份 soak。promotion/admission 不代签 Gate F low-touch、星期级数值样本或业务 `standard_entry_verified`，Audio H2 仍保持独立门。
