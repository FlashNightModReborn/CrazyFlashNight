...
**最后核对代码基线**：commit `62cdd19d10`（2026-07-12）。
...
- **主线准入**：普通文档/美术/策划/AS2/Web 合作者保留现有客户端直推；`Crazyfs`、`Flash-Night` 与未知新 collaborator 必须 PR + `verify-staged-bundle`，并只用 merge commit 保留 v2 source ancestor。纯文档/内容 PR 不设 CODEOWNER，trust-root/native PR 必须由另一 native owner 审批。账号期望状态以 `config/build/main-branch-admission.v1.json` 为准，native 黑名单以 `config/build/native-change-gate.v1.json` 为准；快速通道必须从固定 GitHub workflow 的 `push/main` 绿灯锚累计判定，锚不可用时 required check 保持失败并等待 API/锚恢复后重跑，不得用单次本地 strict 结果建立可继承绿灯；不得为消除误报而给 DLL/EXE 或 runtime 信任链开路径例外，完整边界见 [contribution-workflow.md](docs/contribution-workflow.md)
