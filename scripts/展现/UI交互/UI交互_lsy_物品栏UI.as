...
_root.物品图标UI函数.装备栏 = new Object();
_root.物品图标UI函数.药剂栏 = new Object();



//图标初始化
_root.物品图标UI函数.inventoryInit = function(_collection,_index){
	this.addValue = _root.物品图标UI函数.inventoryAddValue;
	//
...
_root.物品图标UI函数.inventoryAddValue = function(_value){
...
//UI层面的移动操作
_root.物品图标UI函数.清空物品格 = function(物品格){
	物品格.collection.remove(物品格.index);
	物品格.gotoAndStop("刷新");
}

_root.物品图标UI函数.移动物品格 = function(物品格1,物品格2):Boolean{
...
_root.物品图标UI函数.移动物品至玩家装备栏 = function(物品格,装备格,index):Boolean{
	...
	var result = _root.物品图标UI函数.移动物品格(物品格,装备格);
...
_root.物品图标UI函数.移动物品至药剂栏 = function(物品格,药剂格,index):Boolean{
	var 进度条 = _root.玩家信息界面.快捷药剂界面["进度条"+ index];
	if(!进度条.冷却) return;
	var result = _root.物品图标UI函数.移动物品格(物品格,药剂格);
	return result;
}
...
_root.物品图标UI函数.创建背包图标 = function(){
	...
	for (var i = 0; i < 总格数; i++)
	{
		...
		物品图标.init(背包,i);
		物品图标.RollOver = _root.物品图标UI函数.背包.RollOver;
		物品图标.Press = _root.物品图标UI函数.背包.Press;
		物品图标.Release = _root.物品图标UI函数.背包.Release;
		物品图标.locked = false;
		物品栏界面.背包图标列表[i] = 物品图标;
	}
	...
	for (var i = 0; i < 装备栏位列表.length; i++){
		...
		物品图标.init = _root.物品图标UI函数.inventoryInit;
		物品图标.init(装备栏,装备类型);
		物品图标.RollOver = _root.物品图标UI函数.装备栏.RollOver;
		物品图标.Press = _root.物品图标UI函数.装备栏.Press;
		物品图标.locked = false;
	}
}

_root.物品图标UI函数.删除背包图标 = function(){
...
//背包事件
_root.物品图标UI函数.背包.RollOver = function(){
...
_root.物品图标UI函数.背包.Release = function(){
	...
	if(_root.物品栏界面.hitTest(x, y)){
		...
		if(itemData.type == "武器" || itemData.type == "防具"){
			...
			if(目标元件.area.hitTest(x, y)){
				_root.物品图标UI函数.移动物品至玩家装备栏(this,目标元件,itemData.use);
				return;
			}
			//对手枪2进行额外判定
			if(itemData.use == "手枪" && _root.物品栏界面.手枪2.area.hitTest(x, y)){
				_root.物品图标UI函数.移动物品至玩家装备栏(this,_root.物品栏界面.手枪2,"手枪2");
				return;
			}
		}
		...
		for (var i=0; i<背包图标列表.length; i++)
		{
			...
			if(目标元件.area.hitTest(x, y) && 目标元件 !== this){
				_root.物品图标UI函数.移动物品格(this,目标元件);
				return;
			}
		}
		return;
	...
	if (itemData.use == "药剂" && _root.玩家信息界面.快捷药剂界面.hitTest(x, y)){
		for (var i=0; i<4; i++){
			...
			if(目标元件.area.hitTest(x, y)){
				_root.物品图标UI函数.移动物品至药剂栏(this,目标元件,i);
				// _root.玩家信息界面.快捷药剂界面.gotoAndPlay("刷新");
				return;
			}
		}
	...
	/*
	if (_root.购买物品界面._visible && _root.购买物品界面.hitTest(x, y) && _root.物品栏界面.hitTest(x, y) && !_root.物品栏界面.窗体area.hitTest(x, y))
	{
		if (!isNaN(tmp_sz[5]))
		{
			var 售卖倍率 = 0.25;
...
//装备栏事件
_root.物品图标UI函数.装备栏.RollOver = function(){
	_root.物品图标注释(this.name,this.value);
	if (!this.locked) this.互动提示.gotoAndPlay("卸下");
}

_root.物品图标UI函数.装备栏.Press = function(){
	_root.注释结束();
	if (this.locked) return;
	...
	var result = _root.物品图标UI函数.移动物品格(this,目标元件);
	if(!result) return;
	_root[this.index] = "";
	_root.刷新人物装扮(_root.控制目标);
}
...
//初始化药剂栏图标
_root.物品图标UI函数.初始化药剂栏图标 = function(){
	...
	for (var i = 0; i < 4; i++){
		...
		物品图标.init = _root.物品图标UI函数.inventoryInit;
		物品图标.init(药剂栏,i);
		物品图标.RollOver = _root.物品图标UI函数.药剂栏.RollOver;
		物品图标.Press = _root.物品图标UI函数.药剂栏.Press;
		物品图标.locked = false;
...
//药剂栏事件
_root.物品图标UI函数.药剂栏.RollOver = function(){
	_root.物品图标注释(this.name,this.value);
	if (!this.locked) {
...
_root.物品图标UI函数.药剂栏.Press = function(){
	_root.注释结束();
	if (this.locked) return;
	var 进度条 = _root.玩家信息界面.快捷药剂界面["进度条"+this.index];
	if(!进度条.冷却) return;
