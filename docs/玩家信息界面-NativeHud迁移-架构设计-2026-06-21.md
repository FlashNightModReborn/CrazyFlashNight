...
**最后核对代码基线**：commit `b31eebb49a`（2026-07-11，代码树）；战技与 12 槽快捷技能输入 AS2 化另核对 2026-07-12 工作树及 Flash 发布产物。本文 §2/§3 的初始断言来自 13-agent 代码级盘点 + 4 视角对抗审计 workflow，后续施工记录按实际 `.as`/`.xml` 持续回写。
...
**留给 B 接手时补的接口缺口（记此备查，不在本主线修）**：① 不把 `快捷技能栏N`/`快捷物品栏N` 注册成 inventory-domain owned 容器；按双栏 doc §4.3 的裁决，技能 equip 用 `containerId=快捷技能栏 + targetSlot` 进入技能管理域，药剂 equip 用 `containerId=药剂栏 + targetSlot` 写 `DrugInventory`，`快捷物品栏N` 只是 HUD 镜像，Web 不得直写；② §4.1 `equip/unequip` 补 payload schema + "装到几号槽"槽寻址；③ 药剂 unequip 补冷却门控 + 耗尽清栏语义（无叉）；④ equip/unequip 活体副作用（`gameworld[控制目标].被动技能` + `获取键值()`）必须留 AS2 重裁决，Web/C# 不可漏；⑤ `技能图标` 多份同源 symbol 且 equip 写逻辑已分叉，**迁移语义以主 FLA 新结构为准**（12 槽 + 被动联动 + `玩家信息界面.快捷技能界面` 路径）。`asset_source_map.xml` 当前把裸 `技能图标` 指到 `flashswf/arts/things.swf` 旧结构，主 FLA 本体也导出同一 linkage，运行态命中仍需真机/反编译确认；若确认命中旧结构，视为 linkage/发布产物治理问题，先消解 duplicate linkage，不倒推采用旧 1-6 槽语义。
