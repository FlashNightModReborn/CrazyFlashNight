...
**最后核对代码基线**：main-space v2 实现先冻结于 `7e718a1…e1a`，并由 merge commit `40853287e7ed04714d68935c0002f8ad6d8aea05` 纳入当时上游 `3df3bf3…`；上游差量与 PlayerInfo source/asset/视觉输入无交集。clean `40853287…` 的 B0-05 为 32/32（run `c42d755d…`，报告 738,872 B / SHA `3C738AC2…1C514`），B0-06 为 48/48（run `ed61d727…`，报告 653,904 B / SHA `182C7DC9…CA8E1`），完整 Launcher 为 1,747 pass + 3 opt-in skip / 1,750。正式 `visual-evidence.v2` 冻结两份 C# `36 files / 35 PNG / 1,261,624 B / DD58CA7C…6661E`、两份 Web `12 files / 11 PNG / 221,781 B / 9D21D823…17A7B`、两份 direct `66 PNG / 3,114,450 B / F05AE7F7…50ABF` 与 viewer `651,451 B / 2F71A3ED…01BC2`；MP 四字段均 unique `bestDx=0` 且对应 anchor edge `dx=0`。真实 1024×576 main-stage Flash v2 candidate 的四项 human review 仍为空，所以不是 `oracle_frozen`，自动包也不声明 parity。最终 main-space v2 Kimi 审计已对 clean `884fbf43…` 以同一 session 的 `kimi-code/k3 + high` 完成，165/165 request 命中，`K1_AUTOMATED_GATE=PASS`、P0/P1=0；它不代签人类项目。新 v2 tag/request/双 builder quorum/preflight、Computer Use 实际初始化和人类验收仍待完成；严格状态为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`。真实 `pi_*`、promotion、部署与现役 runtime 均未改变；精确状态以 [专项 ADR](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md) §12 为准。

**2026-07-30 build-train 纠错（覆盖上段“新 tag 尚待形成”的旧时点）**：首列受保护 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v2` 已永久指向 `40853287…`，q3 本地 X509 producer/票有效；但 production policy 只通过 22/23，唯一失败是隔离 qualification csproj 漏挂 `PlayerInfoRasterPlan.cs`，所以该列车因 policy 门失败退役，未启动 cloud/quorum/preflight。修复不改 PlayerInfo runtime 源码、SVG、视觉或 B0-05/06 性能闭包；它只补齐 qualification 编译闭包并把 cloud ref 切到全新 `runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2`。修复后的 production contract、renderer 12/12 + 78/78 + 8/8、正负合同 8/8 与 Launcher 1747+3/1750 已通过，最终 F2 仍须在全新 q4/w4 fresh 重算并完成 production receipt/quorum `-VerifyOnly`。
...
- 2026-07-30 的视觉基座 B0（自动报告 UTC 日期为 2026-07-29）保持两条正交状态。Oracle 轨已有 11-case、1024×576 main-stage、exported-symbol extraction、wrapper=false 的真实 Flash v2 candidate，且 tooling strict identity 通过；但四个 manifest human check（状态/层/crop/审美）与独立来源身份现场复核均为空，所以只是 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`。工程轨在 clean `40853287…` 已取得 B0-05 32/32、B0-06 48/48、full 1747+3/1750 和确定的 C#/Web/direct A/B；B0-05 继续是 8 logical layer / 10 owned PArgb payload、16 MiB whole-batch cache、8-field key 与 `split_required`，B0-06 继续是 allowlist fixture-only split surface，不注册现有 NativeHud union、不接真实 `pi_*`、不隐藏或修改旧 Flash HUD。HP 使用两次 `sigma=1` source-over blur（alpha 255 + 128）加白色 core，横线 active，只有 hp light-overlay 延后；MP current 右锚为 `74.55`，maximum runtime effective 左锚为 `80.95`，四字段自动横向配准均为 `bestDx=0 / anchorEdgeDx=0`。v1 结果只作历史；v2 的最终 Kimi `k3/high` 已 PASS，P0/P1=0，新 tag/request/quorum/preflight 与人签仍未闭合，总体为 `visual_correction_in_progress; human_exit_blocked`。FFDec/headless Edge/自动 HWND/Kimi PASS/未初始化的 Computer Use skill 都不能代签真实 DWM、游戏 composite、z-order/occlusion、鼠标透传、运行时 SWF 身份或审美。promotion/标准入口/部署不属于 B0 Exit；fixture-only 复刻不推翻 §4 的 state-first 运行态接入顺序。
...
| 5 | C# PlayerInfoWidget 只读 | B0-05 已完成 raster/cache 并因近全屏 union 接受独立 split surface；B0-06 已实现不接业务权威的 HP/MP fixture 纵切与独立 click-through surface，surface 独占 animation model，widget 只读消费接口，Resume 仅在有效布局与 raster request 建立后完成。历史 v1 证据只证明旧字节。当前 main-space v2 已在 clean `40853287…` 取得 B0-05 32/32、B0-06 48/48、全量 1,747+3/1,750 及正式 C#/Web/direct A/B 自动 closure，最终 Kimi `k3/high` 也已 PASS；仍为 `source-ahead / NOT_DEPLOYED`，新 tag/request/双 builder quorum/preflight 与人类验收待完成。B0 accepted 后仍按阶段2→3的 state-first 链路接真实 UiData，程序化复刻虚拟帧缓动 |
| 6 | 双轨对比 | 不变；按 §2.4 验收表 |
...
- **建议**：historical v1 记录仍只证明旧字节。当前 main-space v2 在 clean `40853287…` 已完成 qualification/full 与双份 C#/Web/direct/viewer 自动 closure，最终 `kimi-code/k3` + 当前最高 `high` 的完整异构审计也已在 clean `884fbf43…` PASS；仍为 `source-ahead / NOT_DEPLOYED`。下一步是创建新 v2 tag/request、完成双 builder/双 faultDomain quorum 与不可复用 preflight，之后仍由人类签收 Flash oracle、可见 DWM/游戏 composite、关键比例/平滑、混合 DPI、运行时 SWF 身份、真实手点与审美。自动包/Kimi 没有 parity verdict，全部通过也只证明视觉基座与自动 closeout；B0 accepted 后再沿阶段2→3建立真实 state/UiData，并做 HP/MP 双轨对比，不能把 fixture 结论外推为运行态接入。
...
**文档治理**：本文为玩家信息 HUD 迁移的纲领 canonical doc；SVG/B0 专项文档只承载视觉渲染子域，避免形成第二份业务权威。B0-04 已同步 strict canonical validator 与 Web/FFDec/Flash diagnostic 入口；B0-03b 已同步生产包、embedded resource、runtime identity/policy/notice 与 `renderer_qualified / NOT_DEPLOYED` 边界；B0-05 已同步 raster/cache、`synthetic_fixed_bounds` 与 `split_required`。B0-06 的 fixture visual-state/widget/compositor/path glyph atlas、prepared DIB split surface、PanelHost 生命周期、strict convergence qualification与 A/B capture/comparator 索引都已进入 current docs；Web static-only 与 direct metrics 包含 scope 差的边界保持显式。historical v1 的 K1/build 记录不批准当前 v2；current v2 的 clean `40853287…` 已闭合 32/32、48/48、1747+3/1750 与 visual-evidence.v2 自动 closure，clean `884fbf43…` 的最终 Kimi `k3/high` 也已 PASS，但新 tag/request/双 builder quorum/preflight 与人类验收仍待完成。本轮仍未声称 B0-04 人工视觉、真实 `pi_*`、可见 DWM/游戏交互、parity、promotion 或部署。全部当前门禁闭合后再同步视觉接受结论并运行 doc governance。后续按实际改动回写本文 §5 路线与 §6 风险，行号锚点随重构刷新。
