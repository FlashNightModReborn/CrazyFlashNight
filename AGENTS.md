...
- **Launcher 二进制发布**：只准从通过 `tools/check-runtime-build-env.ps1` 的机器运行 `launcher/build.ps1`；根 bootstrap、`runtime/` Core/native DLL 与 manifest 必须同轮原子提交，提交前跑 `tools/verify-runtime-bundle.ps1 -Staged`；基线与换机流程见 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)
