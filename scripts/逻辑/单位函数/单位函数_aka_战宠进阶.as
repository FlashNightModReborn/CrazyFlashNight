...
_root.战宠进阶函数.冲腿龙息 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.冲腿龙息 && this.当前宠物属性.冲腿龙息.次数 >= this.进阶方案.冲腿龙息.次数上限){
...
_root.战宠进阶函数.晶能者 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.晶能者 && this.当前宠物属性.晶能者.次数 >= this.进阶方案.晶能者.次数上限){
			return 基础介绍 + "<br>当前已领悟该招式。"
		}
...
_root.战宠进阶函数.复仇者 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.复仇者 && this.当前宠物属性.复仇者.次数 >= this.进阶方案.复仇者.次数上限){
			return 基础介绍 + "<br>当前已领悟该招式。"
		}
...
_root.战宠进阶函数.涅槃重生 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.涅槃重生 && this.当前宠物属性.涅槃重生.次数 >= this.进阶方案.涅槃重生.次数上限){
...
_root.战宠进阶函数.追踪飞弹 = {
	...
	描述:function(){
		if(this.当前宠物信息[1] < this.进阶方案.追踪飞弹.解锁等级){
			return this.进阶方案.追踪飞弹.解锁等级 + "级以上可以升级飞弹，解锁招式追踪效果"
		}
		if(this.当前宠物属性.追踪飞弹){
			return "飞弹升级已完成"
		}
		return "点击可升级飞弹，提高攻击并解锁新招式，需要消耗"+ this.进阶方案.追踪飞弹.消耗金币+"金币";
	},
	详情页:true,
	执行按钮文字:"升级飞弹",
	背景:"默认",
	详情页描述:function(){
		var 基础介绍 = "对铁血战士进行升级，提高攻击力、生命值与能量抗性，并解锁飞弹的追踪效果。"
		if(this.当前宠物信息[1] < this.进阶方案.追踪飞弹.解锁等级){
			return 基础介绍 + "<br>"+this.进阶方案.追踪飞弹.解锁等级+"级以上可以升级飞弹。"
		}
		if(this.当前宠物属性.追踪飞弹 && this.当前宠物属性.追踪飞弹.次数 >= this.进阶方案.追踪飞弹.次数上限){
...
_root.战宠进阶函数.驯鹰者 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.驯鹰者 && this.当前宠物属性.驯鹰者.次数 >= this.进阶方案.驯鹰者.次数上限){
...
_root.战宠进阶函数.美洲狮 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物信息[1] < this.进阶方案.美洲狮.解锁等级){
			return 基础介绍 + "<br>"+this.进阶方案.美洲狮.解锁等级+"级以上可以进阶。"
		}
		if(this.当前宠物属性.美洲狮 && this.当前宠物属性.美洲狮.次数 >= this.进阶方案.美洲狮.次数上限){
			return 基础介绍 + "<br>当前已完成进阶。"
		}
		return 基础介绍 + "<br>进阶需要消耗"+ this.进阶方案.美洲狮.消耗金币+"金币。";
	},
	条件:function(){
		if( this.当前宠物信息[1] < this.进阶方案.美洲狮.解锁等级){
			this.失败提示 = "";
...
_root.战宠进阶函数.追猎 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.追猎 && this.当前宠物属性.追猎.次数 >= this.进阶方案.追猎.次数上限){
			return 基础介绍 + "<br>当前已完成进阶。"
		}
