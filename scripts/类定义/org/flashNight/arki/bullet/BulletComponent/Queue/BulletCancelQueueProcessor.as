...
class org.flashNight.arki.bullet.BulletComponent.Queue.BulletCancelQueueProcessor {
    ...
    /**
     * 区域为主轴的双扫描线处理（性能优化版）
     * 当消弹区域少、子弹多时，以区域为主轴扫描效率更高
     *
     * @param bullets 排序后的子弹数组引用
     * @param bulletLeftKeys 子弹左边界数组
     * @param bulletRightKeys 子弹右边界数组
     * @param bulletCount 子弹数量
     * @return 处理结果对象，包含被消弹/反弹的子弹标记
     */
    public static function processByAreaFirst(bullets:Array, bulletLeftKeys:Array,
                                             bulletRightKeys:Array, bulletCount:Number):Object {
        if (!_soaPrepared || _soaLen == 0) return null;

        // 结果标记数组（与子弹数组对应）
        var cancelFlags:Array = new Array(bulletCount);
        var bounceFlags:Array = new Array(bulletCount);

        // 初始化标记
        for (var i:Number = 0; i < bulletCount; i++) {
            cancelFlags[i] = false;
            bounceFlags[i] = false;
        }
        ...
        // 缓存数组引用，减少属性查找
        var idx:Array = _sortIdx;
        var xMinArr:Array = _xMin;
        var xMaxArr:Array = _xMax;
        var yMinArr:Array = _yMin;
        var yMaxArr:Array = _yMax;
        var shootZArr:Array = _shootZ;
        var zRangeArr:Array = _zRange;
        var dirCodeArr:Array = _dirCode;
        var isEnemyArr:Array = _isEnemy;
        var isBounceArr:Array = _isBounce;
        var isPowerfulArr:Array = _isPowerful;
        var shooterArr:Array = _shooter;

        // 遍历每个消弹区域（区域为主轴）
        for (var iArea:Number = 0; iArea < _soaLen; iArea++) {
            var areaIdx:Number = idx[iArea];

            // 区域边界
            var areaXMin:Number = xMinArr[areaIdx];
            var areaXMax:Number = xMaxArr[areaIdx];
            var areaYMin:Number = yMinArr[areaIdx];
            var areaYMax:Number = yMaxArr[areaIdx];

            // 区域属性缓存
            var areaShootZ:Number = shootZArr[areaIdx];
            var areaZRange:Number = zRangeArr[areaIdx];
            var areaDirCode:Number = dirCodeArr[areaIdx];
            var areaIsEnemy:Boolean = isEnemyArr[areaIdx];
            var areaIsBounce:Boolean = isBounceArr[areaIdx];
            var areaIsPowerful:Boolean = isPowerfulArr[areaIdx];
            var areaShooter:String = shooterArr[areaIdx];

            // 二分查找起始位置（第一个左边界 >= areaXMin 的子弹）
            var startIdx:Number = 0;
            var endIdx:Number = bulletCount - 1;
            while (startIdx < endIdx) {
                var mid:Number = Math.floor((startIdx + endIdx) / 2);
                if (bulletLeftKeys[mid] < areaXMin) {
                    startIdx = mid + 1;
                } else {
                    endIdx = mid;
                }
            ...
            // 从起始位置开始扫描子弹
            for (var j:Number = startIdx; j < bulletCount && bulletLeftKeys[j] <= areaXMax; j++) {
                // 跳过已处理的子弹
                if (cancelFlags[j] || bounceFlags[j]) continue;

                var bullet:MovieClip = bullets[j];

                // 跳过近战或静止的子弹
                if ((bullet.flags & FLAG_MELEE) != 0 || bullet.xmov == 0) continue;

                // 右边界快速排除
                if (bulletRightKeys[j] < areaXMin) continue;

                // 敌我过滤
                if (areaIsEnemy == bullet.是否为敌人) continue;

                // 方向过滤
                if (areaDirCode != 0) {
                    var bulletDir:Number = (bullet.xmov > 0) ? 1 : -1;
                    if (bulletDir != areaDirCode) continue;
                }

                // Z轴判定
                var zOffset:Number = bullet.Z轴坐标 - areaShootZ;
                if (zOffset > areaZRange || zOffset < -areaZRange) continue;

                // 精确位置判定（点在矩形内）
                var bx:Number = bullet._x;
                var by:Number = bullet._y;
                if (bx < areaXMin || bx > areaXMax || by < areaYMin || by > areaYMax) continue;
                ...
                if (areaIsBounce) {
                    ...
                    var sh:MovieClip = _root.gameworld[areaShooter];
                    if (sh) {
                        bullet.是否为敌人 = sh.是否为敌人;
                        bullet.发射者名 = areaShooter;
                    }
                    ...
                    bullet.击中地图 = true;
                    // 强力消弹的标记
                    if (areaIsPowerful && (bullet.flags & FLAG_PIERCE)) {
                        bullet.强力消弹 = true;
                    }
                }
