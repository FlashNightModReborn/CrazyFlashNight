...
- **封印收押**：中心 Z 读 `宿主._y`（宿主是 attachMovie 的子弹元件，没有 `Z轴坐标`），单位读 `Z轴坐标`；站立时 `Z轴坐标 === _y` 所以等价，**宿主/施术者腾空时判定会飘**
- 元件原点被摆在锁定目标脚下 +90（更靠下）、视觉中心仍在目标脚下 → Z 判定改为**单向**：`dz = 中心Z - 单位.Z轴坐标 ∈ [0, 200]`，比中心更靠下的不收；X 仍走椭圆 `dx²/300² + dz²/200² ≤ 1`
- **Z 判定现状**：上半径 200（`范围半径Z`）、下方容差 40（`下方容差Z`）、X ±300 按所在半椭圆收窄；放逐 = 并集（带 `魔法抗性.凡俗` 标签收押当帧即逐，其余攒强度达标才逐）
