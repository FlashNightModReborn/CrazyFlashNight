...
- `LightningRenderer` 退化为“向后兼容代理层”，核心能力迁移到：
...
5) **多风格 renderer 的”读取 config + intensity 缩放 + LOD 开关”样板代码**
   - 现状：每个 renderer 都有 20-30 行完全相同的 `(config != null && !isNaN(config.xxx)) ? config.xxx : DEFAULT_XXX` 模板。
   - 建议：轻量 `RayVfxManager.prepareRenderContext(arc, defaults)` 返回预处理好的参数对象（尽量不引入大量对象分配；可复用池中对象）。

6) **`TeslaRayConfig` 中 `applyPreset()` / `fromXML()` 逐字段条件赋值**
   - `applyPreset()`（L562-602）40 行逐字段 `if (preset.xxx != undefined)` 与 `fromXML()`（L429-543）120 行几乎同构。
   - 建议：改为字段映射表 `FIELD_MAP` 驱动，新增字段只改一处。
...
2) **LOD=2 时“冻结重绘”**  
   - 现在 LOD 只影响分支/条纹等复杂度，但仍每帧 `mc.clear()+重绘`。  
   - 可选：LOD 2 的 arc 只在首帧绘制一次，后续只做淡出（减少 CPU/绘制压力）。

3) **`processRayBullets()` 的临时分配进一步池化（按压测决定）**  
   - pierce/fork 的 `hits` 数组与命中对象结构体可复用（类似 RayVfxManager 的 pool）。

4) **`drawCircle()` 的 trig 预计算（低优先级）**  
   - 当前每次画圆都会 8 段 `cos/sin`；可预存单位圆控制点，按半径缩放复用。
...
- [x] 将 `processRayBullets()` 拆出 2 个私有热点函数 ✅
  ...
  - `settleRayHit(bullet, shooter, hitTarget, hitX, hitY, dmgMult, tEntry, finalResult, flags, meleeMask, Dodge, Damage, FX)` — 闪避→伤害→事件→显示→特效结算管线，消除 4 处 copy-paste
...
### Iteration 3（性能闭环与可调参，视压测，3～7 天）

- [ ] LOD 与帧耗时闭环（结合 `PerformanceScheduler` 或性能日志）
- [ ] LOD=2 冻结重绘（可配置开关，便于 A/B）
