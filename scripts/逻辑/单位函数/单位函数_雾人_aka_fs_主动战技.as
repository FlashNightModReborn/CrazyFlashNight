 ...
 * 法阵 / 宿主的 Y 轴下移量（像素）。
 *
 * 元件的美术重心画在原点**上方**：把原点摆到参考点（目标脚下 / 施术者脚下）
 * 再往下 50，看到的法阵才正好压在参考点上。所以凡是"摆在某某脚边"的地方，
 * 统一写成 `参考点._y + 法阵Y偏移` —— 包括**没有敌人时的身前兜底位**，
 * 否则法阵（和它落成的封印宿主）会跟施术者错开一截、不在一条线上。
 *
 * ⚠ 元件里贴着敌人脚下的跟随同样要带这个偏移；
 *   `SealDomain` 的 Z 判定之所以上下不对称（见 范围半径Z / 下方容差Z 注释）根源也在这里。
 */
_root.主动战技函数.长枪.天启大封印.法阵Y偏移 = 50;
 ...
 * SealDomain 的判定范围（X 左右各 300；Z 只认比中心更小的一侧、纵深 200）。
 *
 ...
 *   想要视觉余量取 65（+2%）或 68（+7%）。
 *
 * ⚠ 判定现在是 600 × 200（宽高比 3.00），素材是 2.00 —— Z 方向视觉比判定**偏胖**
 *   （罩住的范围看起来比实际能封到的更厚）。要让 Z 也贴合，得在元件里把椭圆
 *   压到 300 : 200 的高度，不要反过来把判定改回双向。
 *
