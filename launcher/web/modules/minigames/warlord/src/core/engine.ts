...
function executeMoveOrAttack(
  ...
  if (!isBattle) {
    for (const pieceId of actualPieceIds) movePieceInPlace(state, pieceId, command.targetNodeId);
    addGameEvent(state, {
      type: 'move',
  ...
  if (result.winner === 'attacker') {
    for (const pieceId of actualPieceIds) {
      if (state.pieces[pieceId]) movePieceInPlace(state, pieceId, command.targetNodeId);
    }
...
export function applyCommand(state: GameState, command: GameCommand): CommandResult {
  ...
  if (!validation.ok) return { ok: false, state, error: validation.error ?? '命令非法。' };
