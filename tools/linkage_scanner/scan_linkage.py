...
def scan_xfl(xfl_dir):
    ...
    for xml_file in glob.glob(os.path.join(lib_dir, '**', '*.xml'), recursive=True):
        ...
        if m:
            sym_name, lid = m.group(1), m.group(2)
...
def write_xml():
    ...
    # Separate conflicts (multiple distinct SWF sources) for easy visual identification
    conflicts = {}  # lid -> [(swf, sym_name), ...]
    clean = {}      # lid -> (swf, sym_name)
    for lid in sorted(results.keys()):
        entries = sorted(results[lid])  # sorted tuples of (swf_rel, sym_name)
        # Distinct SWF sources (same SWF with different symbol names is not a conflict)
    ...
    for lid, (src, sym) in sorted(clean.items()):
