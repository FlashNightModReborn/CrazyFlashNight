...
**最后核对代码基线**：release source commit `0b71d91987bee27f399bedc90a2c648a8bcbf44e`（2026-09-03；tag `runtime-build-v2/20260903-reward-root-o1-v1`、tree `68ae915ad90ab96dc31c6371b37a1469be11a934`、request `A0DA1D0F5E3E91EF9A53BC8D81419E5917016B860EB49AC655DA4D83EDAF7367`；deployment push / Audit 待本轮收尾）。
...
正式 runtime 已于 `2026-09-03T00:56:05.591629Z` 原子 promotion，并通过 signed consensus、GitHub proof replay 与正式根 bootstrap `--verify-only`；deployment commit、push 与首次 post-promotion Audit 仍须在本轮完成后才可写为已部署。

**状态边界**：W/B0 窗口生命周期根治保持 `HUMAN_ACCEPTANCE_PASSED / promoted`，部署后未重跑 W 正式业务旅程，故不称 W 专项 `standard_entry_verified`。A/A1 Reward 根事务与 S/O1 观测已完成自动门和修复后 exact candidate 人工复验：H-A 覆盖 partial、跨重启、主动重开、整理后全领取，H-S 取得 13 个同实例样本并合法落到 `no_outstanding_operation`；第二列车随后独立取得本地 X509 + GitHub Hosted 双故障域共识并完成原子 promotion，当前准确状态为 `HUMAN_ACCEPTANCE_PASSED / promoted / DEPLOYMENT_PUSH_PENDING / FORMAL_BUSINESS_REVALIDATION_PENDING`，不称 A/S 专项 `standard_entry_verified`。完整状态见 [止血治理 ADR](docs/AS2-WebPanel止血治理-窗口生命周期与Reward根事务-ADR-2026-09-01.md)。
