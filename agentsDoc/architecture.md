...
- **当前失败/重入边界**：第一份 S `90985ae…705644` / run `cdf7…59f64` 是静音失败；S2 `0360972…2f950` / run `5cf1…f7ec3` 是真实非静音 capture 后 artifact/wire canonical mismatch；S3 `176b2a0…c800a` / run `a481…c2c7` 是 extensionless SFX ID 命中 unknown-ID。S4 `2f28e48…893f9` / tree `d4f286a…79781` 已由 clean A/B/live producer 重现 candidate `E2CF5C…6B686 / 75D305…B4F11`；run `65b22a3…d8d98` 在 `bgm_playback` begin 后因 prepare 未创建 capture root 而失败，run `50598a6…e30bc` 则被旧 crossfade replay 对相隔约 3.26ms 的同帧 cached read 假红。后者的诊断复核还发现短 SFX 在 `sfx_playback`/mix 的 post-stimulus snapshot 已结束而 peak=0，以及循环 Opus cursor `18240→14880/24000` 被原始单调断言误判。五个 runId 与全部 capture 均作废、没有 E1/H2，始终 `NOT_DEPLOYED`。当前版本以 exclusive prepare、qualified long SFX、loop-aware mix 与 bounded cached-read crossfade 修复这些确定问题；它与最终 dependency/doc closure 首次同树落盘的 clean implementation commit 定义为下一 Source S，commit/tree 只能由提交后的 Git object 解析，仍须重新双 clean repro、新 candidate 与新 runId。最终 qualification dependency 已稳定为 `568` entries / `142053` B / closure `4FC516AA6807EA30F5B94BCE9445698E2A4692B7E62C7A1FA1AAE02525D802DD` / manifest SHA-256 `40E51C802005A8D08D9FEB0048887A37CE2E4426BC261D01F3CEA85A8A741A86`；focused runner/materializer/operator/observer/assembler/updater 为 `28/8/12/23/5/5`。当前 source 状态与 promotion 边界见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)；H2 尚未取得，因此严格为 **H2 blocked / NOT_DEPLOYED**。
