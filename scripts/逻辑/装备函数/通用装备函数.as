...
_root.装备生命周期函数.初期特效周期 = function(反射对象, 参数对象) 
{
    ...
    if (_root.兵器攻击检测(自机)) 
    {
        if (_root.成功率(反射对象.成功率)) 
        {
            ...
            MuzzleWorldShoot.populate(刀口, 自机, 反射对象.子弹属性, 反射对象.xOffset, 反射对象.yOffset, 反射对象.身高修正比);
...
_root.装备生命周期函数.通用变形初始化 = function(reflector:Object, paramObj:Object) {
    ...
    // 同步初始帧
    var target:MovieClip = WeaponAnimationTarget.resolve(reflector);
    ...
    ToggleStateRestorer.restore(reflector, p);
};
...
_root.装备生命周期函数.通用变形周期 = function(reflector:Object, paramObj:Object) {
    // 移除异常检测
    EquipmentTick.cleanup(reflector);
...
_root.装备生命周期函数.通用特效刀口初始化 = function (reflector:Object, paramObj:Object) {
    ...
    reflector.func = EffectFunctionResolver.byString(paramObj.func, "_root.刀口触发特效.黑铁的剑特效");
