...
- **共享佣兵 / 主角纸娃娃预览（2026-08-09）**：`mercSnapshot` / `mercHireList` 与 Arena preview 的佣兵摘要下发 `id/gender/height/face/hair/equips`；`merc-portrait-renderer.js` 暴露共享 `MercPortraits`，用 `assets/dressup/manifest.json` 的 battle rig `空手站立` 重建 Team 卡片/右栏和 Arena 卡片/对手行胸像。Arena 全单位目录 v2 还会把 `units.json` 中 `主角-*` 单位的 `gender/face/hairstyle` 与装备字段派生成只读 dressup actor，使 Pig/Boy/King 等按各自纸娃娃外观显示，不再把 3 个主角模板身份误记为怪物头像缺口。若旧链路或开发夹具下发原始编号，先用 manifest 的 `appearance.faceById/hairById` 归一化；缺 `face/hair` 时降级为性别默认脸型与空发型。快照使用稳定外观 key 缓存、同 key 并发合并和 DOM request token fence，加载失败显示剪影；Team/Arena 卡片统一消费 `card + 112px + cover` 胸像口径。共享快照队列默认最多同时运行 4 个 renderer，Arena 长目录再用可见区 + 144px 邻域挂载，不能随目录条目数一次创建全部 Canvas。头像说明必须绑定 `PanelTooltip` scope 并在 DOM 替换前释放，禁止用原生 `title` 白色气泡绕过项目样式和生命周期。这条只读展示链不回写佣兵、单位数据或战斗权威。Team 培养页仍只保留一个 live canvas，绘制身体外观并与性格特质左右分栏；manifest item 的 `helmet` 继续控制头盔压发。
