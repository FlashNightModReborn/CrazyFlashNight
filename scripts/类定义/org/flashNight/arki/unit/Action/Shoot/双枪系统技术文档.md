...
私有变量:
  ...
  - _bothEmpty: Boolean       // [v2.0新增] 两把枪是否都空弹（缓存值）
```

#### 关键方法

##### `updateState(): Void`
**位置**: WeaponStateManager.as:63

**功能**: 刷新所有状态变量

**执行时机**:
- 构造函数初始化时
- **[v2.0变更]** 仅在需要判断换弹策略时才刷新（延迟更新优化）
- 每次换弹后
...
_bothEmpty = _mainIsEmpty && _subIsEmpty;  // [v2.0新增] 缓存计算结果
```

##### `shouldReloadMainFirst(isMainHandReloadable, isSubHandReloadable): Boolean`
**位置**: WeaponStateManager.as:123
...
##### `canFinishMainHandReload(remainingMainMag, remainingSubMag, hasImpactChain): Boolean`
**位置**: WeaponStateManager.as:174
**[v2.0变更]** 新增 hasImpactChain 参数
...
```actionscript
// 无可用弹匣：直接结束
if (remainingSubMag == 0) return true;

// [v2.0新增] 半换弹仅在"冲击连携启用 + 双枪异枪"时生效
...
##### `canFinishSubHandReload(remainingMainMag, remainingSubMag, hasImpactChain): Boolean`
**位置**: WeaponStateManager.as:197
**[v2.0变更]** 新增 hasImpactChain 参数

**功能**: 判断副手换弹后是否可以结束换弹流程（逻辑与主手对称）

---

#### [v2.0新增] 决策API

##### `shouldAutoReloadOnEmpty(isHeroControlled, hasImpactChain, currentHandMag): Boolean`
**位置**: WeaponStateManager.as:244

**功能**: 判断当前手空弹时是否应该触发自动换弹

**设计目的**: 封装冲击连携被动技能对换弹策略的影响，避免业务逻辑分散

**规则**:
| 控制类型 | 冲击连携 | 武器类型 | 换弹条件 |
|---------|---------|---------|---------|
| AI控制 | - | - | 有弹匣就换弹 |
| 玩家控制 | 启用 | 异枪 | 单手空即可换弹（半换弹） |
| 玩家控制 | 启用 | 同枪 | 两把都空才换弹 |
| 玩家控制 | 未启用 | - | 两把都空才换弹 |
...
##### `decideReloadHand(hasImpactChain, target): Number`
**位置**: WeaponStateManager.as:274
...
**设计目的**: 封装优先级判断和弹匣库存校验，确保不会出现"无弹匣也能换弹"的问题

**规则**:
| 模式 | 判断逻辑 |
|------|---------|
| 半换弹（冲击连携+异枪） | 使用 shouldReloadMainFirst 优先级判断 |
...
public function decideReloadHand(hasImpactChain:Boolean, target:Object):Number {
    ...
    if (halfReloadEnabled) {
        ...
        if (!_subIsFull) {
            ...
            if (isSubReloadable) return 2; // 副手换弹
        }
        if (!_mainIsFull && isMainReloadable) return 1;
    } else {
        // 非半换弹：优先换空弹的那只手
        if (_mainIsEmpty && !_mainIsFull) {
            isMainReloadable = !!(ItemUtil.singleContain(target.主手使用弹匣名称, 1));
            if (isMainReloadable) return 1;
        }
        if (_subIsEmpty && !_subIsFull) {
            isSubReloadable = !!(ItemUtil.singleContain(target.副手使用弹匣名称, 1));
            if (isSubReloadable) return 2;
        }
        // 都没空但未满，检查补弹
        if (!_mainIsFull) {
            isMainReloadable = !!(ItemUtil.singleContain(target.主手使用弹匣名称, 1));
...
4. 玩家控制时的决策 [v2.0变更: 使用 decideReloadHand API]
   if (rootRef.控制目标 === parentRef._name) {
       // 检查冲击连携被动是否启用
...
**v2.0变更说明**:
- 移除了直接调用 `ItemUtil.singleContain` 的逻辑
- 移除了 `shouldReloadMainFirst` 和 `shouldReloadSub` 的直接调用
- 统一使用 `stateManager.decideReloadHand()` 进行决策
- 决策API内部封装了冲击连携对半换弹的影响
...
##### `calculateWeaponPower(parentRef, weaponType, basePower): Number`
**位置**: ShootInitCore.as:324
**[v2.0重构]** 威力计算采用"加法收集 → 最终乘算"模式

**功能**: 计算武器的最终威力，统一处理被动技能倍率和额外加成

**计算公式**:
```
finalPower = basePower * (1 + totalMultiplier) + flatBonus
```

**倍率收集规则**:
| 加成来源 | 武器类型 | 倍率计算 | 固定加成 |
|---------|---------|---------|---------|
| 枪械攻击被动 | 长枪 | 0.5 + 等级 * 0.03 | +30 |
| 枪械攻击被动 | 手枪/手枪2 | 等级 * 0.015 | +20 |
| 额外攻击加成 | 长枪 | 长枪额外攻击加成倍率 | - |
| 额外攻击加成 | 手枪/手枪2 | 短枪额外攻击加成倍率 | - |
| 冲击连携被动 | 手枪（仅主手） | 0.10 + (等级-1) * 0.0111 | - |

**示例计算**:
```actionscript
// 假设：basePower=100, 枪械攻击等级=10, 冲击连携等级=5（主手手枪）
totalMultiplier = 0.015 * 10          // 枪械攻击: 15%
                + 0.10 + 4 * 0.0111   // 冲击连携: 约14.4%
                = 0.294
flatBonus = 20
finalPower = 100 * (1 + 0.294) + 20 = 149.4
```

##### `calculateWeaponImpact(parentRef, weaponType, baseImpact, weaponTypeTag): Number`
**位置**: ShootInitCore.as:386
**[v2.0重构]** 冲击加成范围调整
...
**[v2.0变更] 加成范围调整**:
| 加成类型 | 旧版适用范围 | 新版适用范围 | 加成值 |
|---------|-------------|-------------|--------|
| 基础冲击加成 | 手枪2 + 长枪 | 手枪2 + **霰弹长枪** | 20%→50% |
| 霰弹枪额外加成 | 霰弹枪 | 霰弹枪 | 固定50% |

**设计理由**: 避免长枪整体数值膨胀，仅对霰弹长枪进行补弱

##### `generateBulletProps(parentRef, weaponType, weaponData, extraParams): Object`
**位置**: ShootInitCore.as:428
...
计算属性:
  - 子弹威力 = calculateWeaponPower(parentRef, weaponType, basePower)
  - 击倒率 = calculateWeaponImpact(parentRef, weaponType, baseImpact, weaponTypeTag)
...
> **[v2.0新增章节]** 本章节详细描述冲击连携被动技能对双枪系统的影响

### 技能概述

冲击连携是一个专为双枪玩法设计的被动技能，通过以下机制增强双枪的战斗体验：

| 机制 | 描述 | 影响模块 |
|------|------|---------|
| **半换弹** | 单手空弹时即可触发换弹，无需等待两把都空 | WeaponStateManager, ShootInitCore |
| **火力加成** | 主手（手枪）获得10%-20%火力加成 | ShootInitCore.calculateWeaponPower |
| **冲击加成** | 手枪2和霰弹长枪获得冲击力提升 | ShootInitCore.calculateWeaponImpact |

### 启用条件

```actionscript
var hasImpactChain:Boolean = Boolean(
    parentRef.被动技能 &&
    parentRef.被动技能.冲击连携 &&
    parentRef.被动技能.冲击连携.启用
);
```
...
| 场景 | 无冲击连携 | 有冲击连携（同枪） | 有冲击连携（异枪） |
|------|-----------|------------------|------------------|
| 主手空、副手有弹 | 继续用副手，不换弹 | 继续用副手，不换弹 | **立即触发换弹** |
| 副手空、主手有弹 | 继续用主手，不换弹 | 继续用主手，不换弹 | **立即触发换弹** |
| 两手都空 | 触发换弹 | 触发换弹 | 触发换弹 |

#### 设计理由

1. **同枪不启用半换弹**: 同一种枪共享弹匣类型，频繁换弹会导致手感割裂
2. **异枪启用半换弹**: 不同枪独立管理弹匣，半换弹提供更流畅的战斗节奏
...
**注意**: 仅对 `weaponType == "手枪"` 生效，手枪2不享受此加成

### 冲击加成公式

冲击连携为特定武器提供冲击力加成（降低击倒率）：
...
- 手枪2霰弹枪，10级冲击连携
- 基础击倒率 = 100
- totalImpactBonus = 0.50（基础）+ 0.50（霰弹）= 1.0
- finalImpact = 100 / 2.0 = 50
- 冲击力提升 100%

### 换弹结束判断的影响

启用冲击连携时，换弹结束的判断逻辑会发生变化：

| 场景 | 无冲击连携 | 有冲击连携（异枪） |
|------|-----------|------------------|
| 主手换完、副手未空 | 继续换副手至满 | **直接结束**（避免补满未空的枪） |
| 副手换完、主手未空 | 继续换主手至满 | **直接结束** |

**设计理由**: 半换弹的目的是快速恢复战斗力，而非追求两手都满弹

### 代码集成点

| 集成位置 | 调用方式 | 功能 |
|---------|---------|------|
| ShootInitCore._createHandShootFunction | `stateManager.shouldAutoReloadOnEmpty()` | 判断是否触发自动换弹 |
| ReloadManager.createDualGunReloadStartFunction | `stateManager.decideReloadHand()` | 决定换哪只手 |
| ReloadManager.createHandReloadFunction | `stateManager.canFinishXXXHandReload(hasImpactChain)` | 判断是否结束换弹 |
| ShootInitCore.calculateWeaponPower | 直接检查被动技能 | 计算火力加成 |
| ShootInitCore.calculateWeaponImpact | 直接检查被动技能 | 计算冲击加成 |
...
#### [v2.0新增] 半换弹决策表（冲击连携 + 异枪）

| 主手状态 | 副手状态 | 主手弹匣 | 副手弹匣 | 换弹决策 | 换弹结束条件 |
|---------|---------|---------|---------|---------|-------------|
| 空 | 有弹 | 有 | - | 换主手 | 主手换完即结束 |
| 有弹 | 空 | - | 有 | 换副手 | 副手换完即结束 |
| 空 | 空 | 有 | 有 | 主手优先 | 仅换空弹的那把 |
| 未满 | 满 | 有 | - | 换主手 | 主手换完即结束 |
| 满 | 未满 | - | 有 | 换副手 | 副手换完即结束 |

**v2.0变更说明**:
- 半换弹模式下，单手空弹即触发换弹
- 换弹结束判断简化：只换空弹/未满的那把，不追求两手都满

#### 同种武器特殊规则
```actionscript
// WeaponStateManager.as:105
if (_isSameWeapon && !_mainIsEmpty && !_subIsEmpty) {
    return false;  // 两手都有子弹，不换弹
}

// 同种武器只有在任一手打空时才触发换弹
// [v2.0] 即使启用冲击连携，同枪也不启用半换弹（避免手感割裂）
```
...
| 2.0 | 2025-12-12 | 冲击连携被动技能集成，重构换弹决策逻辑 |

#### v2.0 详细变更记录

**WeaponStateManager.as**:
- 新增 `_bothEmpty` 缓存变量，优化性能
- 新增决策API：`shouldAutoReloadOnEmpty()`、`decideReloadHand()`
- 修改 `canFinishMainHandReload()` 和 `canFinishSubHandReload()` 签名，新增 `hasImpactChain` 参数
- 支持半换弹机制（冲击连携 + 异枪场景）

**ShootInitCore.as**:
- 重构 `calculateWeaponPower()` 威力计算，采用"加法收集 → 最终乘算"模式
- 修改 `calculateWeaponImpact()` 冲击加成范围，仅对手枪2和霰弹长枪生效
- 优化射击函数中的状态更新时机（延迟更新）
- 新增冲击连携被动的火力加成（主手手枪 10%-20%）

**ReloadManager.as**:
- 重构 `createDualGunReloadStartFunction()` 决策流程，使用 `decideReloadHand()` API
- 换弹结束判断支持 `hasImpactChain` 参数

**相关提交**:
- `bf735513c` - 开始进行性能优化与行为回归
- `febd98d69` - 重新调整了冲击连携的技能设计定位，更改实现以符合预期
- `f462f3eac` - 调整注释不同步的地方
