...
_root.装备生命周期函数.炎魔斩new初始化 = function(反射对象, 参数对象) {
    ...
    // 4. 从 item.value 恢复形态状态
    var wv:Object = 反射对象.自机[反射对象.装备类型].value;
    var _rv = _root.装备生命周期函数.读取持久值;
    d.chainsaw化   = _rv(wv, "链锯化", false);
    d.weaponValue  = wv;
    wv.链锯化      = d.chainsaw化;
    // 载入子弹属性与战技
    ...
    var Anim = {
        ...
        handleTransform: function() {
            ...
            if (d.currentFrame < endF) {
                ...
                if (d.weaponValue) d.weaponValue.链锯化 = d.chainsaw化;
                // 跳至新形态起始帧
                var nf = d.chainsaw化
    ...
    反射对象.wasTransformKeyDown = _root.按键输入检测(自机, _root.武器变形键) ? true : false;

    PlacementVisual.hookVisualUpdate(自机, "刀_引用", 反射对象, _root.装备生命周期函数.炎魔斩new视觉更新);
};
...
_root.装备生命周期函数.炎魔斩new周期 = function(反射对象, 参数对象) {
    ...
    if (d.isWeaponActive && !d.isTransforming) {
        if (KeyEdgeTrigger.onRise(反射对象, 自机, _root.武器变形键, "wasTransformKeyDown")) {
            d.toggleShape();
        }
    }
    ...
    // 更新画面
    _root.装备生命周期函数.炎魔斩new视觉更新(反射对象);
};

_root.装备生命周期函数.炎魔斩new视觉更新 = function(反射对象) {
    var 刀 = 反射对象.自机.刀_引用;
    刀.动画.gotoAndStop(反射对象.fsm.data.currentFrame);
};
