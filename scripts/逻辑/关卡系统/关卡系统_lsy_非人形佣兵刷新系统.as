...
// 内部实现：生成可雇用敌人（数据已就绪）
_root.生成游戏世界可雇用敌人内部 = function(添加佣兵函数, 机率, 是否门口, frameFlag)
{
	...
	for (var 迭代器 = 0; 迭代器 < 场上佣兵总人数; 迭代器++)
	{
		...
		_root.帧计时器.添加单次任务(function(是否门口, 添加佣兵函数, fFlag) {
		...
		}, _root.随机整数(1, 场上佣兵总人数 * 2) * 1000, 是否门口, 添加佣兵函数, frameFlag);
	}
};
...
_root.计算可雇用敌人价格 = function(佣兵对象){
	...
	// 当前售价含已购涨价（_root.获取宠物当前售价 = 基础价 + IncreasePrice×已购次数）：买宠越多，可雇用价越贵，
	// 与商城购买价同步上涨（抑制堆同质化战宠的设计意图）。IncreasePrice 仍作公式权重单独参与。
	var 当前售价 = _root.获取宠物当前售价(战宠编号);
	var 基础价格 = 当前售价 * 0.75 + 宠物数据.IncreasePrice * 0.25; //按售价和涨价决定基础价格
	var 等级增幅 = 25 * 佣兵对象.等级 * 佣兵对象.等级 + 0.04 * 当前售价 * 佣兵对象.等级; //随等级二次增长
