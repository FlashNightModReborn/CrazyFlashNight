...
本文提出了一种面向遗留Flash/AS2渲染器的闭环性能调度器，该系统面临三重约束：(i)级间增益高度非均匀（包含近死区级别）的四级离散执行器；(ii)严格运行时约束（微秒级每帧预算、零内存分配）；(iii)未建模的时变黑箱被控对象。本文证明，当PID控制器以帧计数为时间增量（30–120帧）驱动时，会退化为带偏置的比例阈值生成器——积分项在一步内饱和并夹紧，微分项衰减为适度的阻尼——因此**切换动态由量化器的确认式迟滞机制主导，而非PID精细调参**。这是本文的核心发现：在粗粒度执行器系统中，低振荡调度主要由确认机制决定，而PID的精细调参重要性较低。
