...
**状态边界**：本列车把斗兽标定从工作簿原始来源摄取、参数闭包、`contamination` Schema/fixture、timeout 口径和真正 Schema 实例校验，收束到可冻结、可撤销、可持久恢复的 Gate F 星期级批处理。当前 tracked draft 含 58 个 scheduled candidate，另将 `B12` 隔离为 quarantine，共 198 个 10–25 run shard / 3,255 个机器 run；首批三份 10-run fresh soak 固定为 `B2 / B9 / G2`，`C9` 仍保留 `10 + 20 + 25` run 与 side-swap 全量复核。
fresh 门包括 `node tools/arena-calibration/run-checks.js` 的 40 个 Schema / 64 项检查、`node tools/test-agent-entry-contract.js`、计划/manifest/decision hash 与 workbook hash + cell provenance、runtime identity、受保护存档和低注意力 fail-closed 契约。production policy `39/39`、worktree/index 33-file bundle、signed consensus、原子 promotion、正式根 bootstrap `--verify-only` 与 post-promotion Audit 均通过。
准确状态仅为斗兽 Gate F 所需正式 runtime `promoted`：正式 plan 尚未 `freeze/arm`，三份 soak 与其余 195 个 shard 均未执行，因此本列车尚无新的斗兽数值样本、真人 PVE 或业务 `standard_entry_verified`；promotion 也不代签 Gate F low-touch/星期级通过。Audio H2 仍保持独立门。
