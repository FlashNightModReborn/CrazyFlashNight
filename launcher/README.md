...
本节只描述 H1 activation commit `c5ca5dfa9718e8a7714038e929a64081b7fd0026` 之后的 current source。第一份 S `90985aecc9a3ce9a0bcd1ad11dde2b7158705644` / tree `beaaab2622bb9d3bfaa44485d63e274de14224a3` 的静音 A6 attempt 已失败；S2 `036097289bac2b62304b0cfc23605246d4c2f950` / tree `d6052412c3ddcfdaefe7825b9b0775c5fd1ab12c` 又在真实非静音 capture 后暴露 operator artifact/wire canonical mismatch；comparator 修复后的 S3 `176b2a0d8a7e21ffa281d9a8ed8c1991827c800a` / tree `221901c46317d16b55b3b3d3e75b0ad950f82c82` 在第三次 run 的 BGM capture 后暴露 extensionless SFX fixture ID 与 production full-filename key 失配。本节所在版本与 full-basename/dense-overlap AS2/C#/JS test/dependency/doc closure 首次同树落盘的 clean implementation commit 定义为新的 Source S，commit/tree 不在自身文档 blob 中预填，而由提交后的 Git object 解析；fresh source 门为 runner `28/28`、materializer `4/4`、operator `9/9`、observer `20/20`、C# stimulus `16/16`、Audio namespace `201/201`、updater `5/5`；Launcher canonical full 为 `3529 passed / 0 failed / 3 existing explicit skipped / 3532 total`、SDK resolver `7/7`、parallel collections off。Audio fresh runId `5666cb1002684540b003f7cc1141dd01` 为 `185/185`、Compiler `0/0`、32K retry `0`，TestLoader SWF `438436` B；publish-only asLoader SWF `1064688` B / SHA-256 `C52F52CEE90E57D79F6ACBE6464DD9472034F1A49D0DD958297953B94266063E` / blob `91188da3684523bd0ccf4dd2b4c2231e1be5b818` / `9748` functions / max `46025` B，Compiler `0/0`，FFDec 已确认 current SWF 含 `hasIdenticalStrings`。dependency 为 `568` 项 / `142053` bytes / closure `3655FEC5…EC7B81` / manifest `A26D5282…C0730E`。该 exact S 仍须重新执行双 clean repro、构建新 candidate 并从 case 1 重跑 A6。当前继续为 **H2 blocked / NOT_DEPLOYED**，未进入 runtime promotion；不能把源码、单元测试、本地 DLL、失败 run capture 或旧 candidate 外推为玩家正式入口已经升级。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
- stimulus 是与 observer 分离的 CurrentUserOnly pipe：`cf7-audio-v2-qualification-stimulus-{pid}-{runId}`。Host 只把 strict grammar 的命令投递为 `audioV2QualificationStimulus`；每条 dispatch 都在同一 ready generation 内串行、幂等重发 `arm` 后才发送命令，避免进程启动时的首次 arm 早于 AS2 handler 而丢失，generation 在 rearm 与命令之间变化则 fail-closed。Flash 端仍通过生产 `AudioQualificationStimulus → AudioBridge → XMLSocket → AudioTask → AudioCoordinator/native` 执行；响应中的 `sent=true` 只证明 exact-generation socket delivery，不证明 AS2 ingress、native started 或端点可听。前 10 case 顺序固定为 BGM 播放/seek/crossfade、Vorbis/AAC-MP4/Opus、SFX、dense overlap、BGM+SFX mix、gain zero/default-max；dense case 的唯一合法 grammar 是在同一 immediate S2 batch 内把同一个 full-filename linkage ID 重复 6 次，六个 unique ID 必须在 AS2/C#/JS 三层 fail-closed，因为 native 4-voice/30ms throttle 是 per catalog entry 而非全局池。observer 必须静态绑定 per-entry voice cap `4`，再把 AS2 batch、稳定 runtime tuple 与 counter delta `1 <= played <= 4`、`throttled >= 1`、`played + throttled = 6`、其他 outcome 零增长一起验证；不得把 H1 擅自收紧为 exact `1/5`。Opus 后在 case 外把 BGM gain 置 0，dense overlap 后在 case 外恢复为 1，末尾再次恢复为 1，避免 SFX capture 被循环 BGM 污染并保证 mix 前状态确定。
- `write-qualification-toolchain.ps1` 先复用 pinned runtime build environment gate，生成 assembler 接受的 canonical toolchain JSON，精确绑定 Node/MSVC/.NET/PowerShell/cmd/vcvars 路径与 SHA；显式 `CF7_NODE_EXE` 无效时不得回退 PATH。`materialize-qualification-fixtures.js` 只把 tracked、hash-bound 输入复制到 ignored run root；其 SFX `linkageId` 必须逐字等于 source path 的 full basename（包含 `.wav`），与 production `Path.GetFileName()` catalog key 一致，extensionless stem 禁止进入 plan。`qualification-operator.js` 组合 observer、stimulus client 与 capture tool，自动执行前 10 case。capture tool 在 `IAudioClient.Start` 后原子写 `ReadySignalPath`，operator 必须等到该 token 才发送刺激；observer/stimulus pipe 使用无空白 compact canonical JSON，而 capture configuration 必须精确是 recursive key-sort、two-space indent、terminal LF、UTF-8 no-BOM tracked artifact，operator 用独立 comparator 接受该格式并拒绝 compact wire、CRLF、乱序或 runtime tuple tamper。configuration 绑定 endpoint、backend/device digest 与 runtime tuple，runtime `f32` 和输出 WAV `pcm_s16le` 是两个不同合同，不能互相代证。`list-playback-endpoints.ps1` 只读枚举当前 render endpoint，operator 不替人切换默认设备、路由或睡眠机器。
...
- **失败 attempt 台账**：第一份 S 的 candidate identity `1A675D3F200557E41E2821E74CE31309107051FE0EAEC79D9A402E7DFA146CEB` / closure `D5232F5688598118196EE09945E6FE2C4AD2C739AFB247A985BD6C920DA96A1F` 以 runId `cdf7c0efbd6745d29ddb467808259f64` 进入 case 1 `bgm_playback`，因无 AS2 ingress、source `playing=true` 但 cursor/meter 为 0 而静音 fail-close。S2 的 A/B/live producer 一致得到 identity `0044438A14045F53DA88588DA96BCFF3E90AE740BA3A728234A2FE5403FAF2FA`、closure `32FD1E5B1035A369416F6A2AA837DEA455A884B80DC3C02A3107373D367A71EF` 与 runtime manifest SHA-256 `ED2DFB302DD7A659D9636530D3D92C3CDB857210AD1D9C9F49B7D356A49BEC98`；runId `5cf1375c53f5de5123d16b1b750f7ec3` 生成 384044 B / SHA-256 `00606ADAF21B326A4AFA8B9741CF1AA80F5D55A1F3FF5EEB9A79CB2AFD884ECA` 的真实非静音 WASAPI WAV 和 944 B / SHA-256 `9EF166E3DBDE8685AF4535E3964F028F44F581F4D0A0E0ED23DAD5411F8359CC` 的 configuration，却因 operator 混用 artifact/wire canonical 格式而 fail-close。comparator 修复后的 S3 `176b2a0d8a7e21ffa281d9a8ed8c1991827c800a` / tree `221901c46317d16b55b3b3d3e75b0ad950f82c82` 再由 A/B/live producer 重现同一 identity/closure/manifest；runId `a481b0989d1aed80e06064d2da79c2c7` 的 BGM configuration `944` B / SHA-256 `C5A64BB58E85F99EB764AF82519A7F0073368C2E3AC01078BCD9DA84933755B3` 与真实非静音 WAV `384044` B / SHA-256 `0DD3D93D49D3CF8821BED45CD211BACF01B6C3F987E6C32D0B8F128A5DFF2042` 已生成，但 case 7 `sfx_playback` 的 plan ID 缺 `.wav`；AS2 ingress 正常、runtime tuple 稳定，native `unknownId 0→1 / played=0 / meter=0`，因此 fail-close。第三份 candidate 由窗口关闭按钮正常退出且 30 秒内 PID/根端口文件消失，shadow/canonical save 与 baseline 逐字节一致；没有 candidate shutdown log，不外推 `shutdown_complete`。三个 candidate 始终 `NOT_DEPLOYED`，没有 E1/H2；三个 runId 与全部 capture 永久作废。后续验证必须绑定 full-basename/dense-overlap 修复后新 S 的双 clean repro、新 candidate 与新 runId。
