...
六阶段 `initial / before_first_start / after_commit / before_restart / after_readback / final` 必须绑定同一个 current-tree production closure。当前精确口径为 240 个 source role：

- Web 执行源 76：入口页 1、JavaScript 46、CSS 29；JavaScript 同时覆盖启动图、lazy registry、Crafting lazy 模块与 organizer 依赖，CSS 同时覆盖页面 link 与递归 `@import`；
...
浏览器子门的显式 module manifest 为 373 项：bootstrap/journal/helper、module/resource inventory、共享资源闭包、280 个 Node/runtime 文件、86 个 required/optional Web 资源和当前 Edge 文件。每次运行必须实际观察全部 56 个 required 资源；30 个 dress-up 帧只允许是 manifest 预绑定集合的运行时子集。当前实跑观察到 69 个资源、605 次 occurrence；唯一允许失败为一次 `/favicon.ico` 与三次显式缺图探针，其他漏读、额外请求、字节漂移、未闭合请求或浏览器路径漂移均失败。三种 viewport 各自必须精确通过 baseline `120`、coverage `15`、fault `8`、identity `10` 项，父门逐组核对唯一名称和完整结果，不只核对总数。
