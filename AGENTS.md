...
- **Flash 三层目标**：不要把“改了 `.as`”直接等价为编主文件。默认频率 / 优先级是 `asLoader`（业务逻辑注入，最高频）→ `TestLoader`（闭环调试 / 测试）→ `main`（少用，只在主 FLA / 资产 / linkage / 主时间轴有必要刷新时，多见于 UI 迁移）。选择 `-Target main` 必须能说明触及主文件层；普通 AS2 逻辑优先验证 `asLoader` / `TestLoader`
...
- **Automation / Build / Verification**：先读 [automation/README.md](automation/README.md) + [testing-guide.md](agentsDoc/testing-guide.md)；按需补 [FlashCS6自动化编译.md](scripts/FlashCS6自动化编译.md)（有编译 / smoke 验证 SWF 需求时即补读）、[launcher/README.md](launcher/README.md)、[cfn-cli.sh](tools/cfn-cli.sh)
