...
_root.敌人函数.行走 = function() {
    var self = this;
    if (self.右行 || self.左行 || self.上行 || self.下行) {
        ...
        var 初始目标状态 = self.攻击模式 + "跑";
        var 最终策略 = (self.状态 === 初始目标状态) ? 移动策略.跑 : 移动策略.走;
        var 最终状态 = self.攻击模式 + 最终策略.状态后缀;
...
_root.初始化敌人模板 = function() {
    ...
    if (敌人属性 == null) {
        敌人属性 = _root.敌人属性表["默认"];
        // _root.发布消息(this.兵种, "警告：敌人未加载兵种信息，使用默认属性。");
    }
