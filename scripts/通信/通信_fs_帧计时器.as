...
_root.帧计时器.更新目标缓存 = function(自机:Object, 更新间隔:Number, 请求类型:String, 自机状态键:String) {
    ...
    for (var 待选目标:String in 游戏世界) {
        ...
        if (!nameIndex[名称]) {
            ...
            // 检查位置是否需要更新
            var currentObj:Object = data[currentIndex];
            if (currentObj.aabbCollider.right == newRight) continue;
            
            // 移除旧位置并重新插入
            data.splice(currentIndex, 1);
            low = 0;
            high = data.length; // 数组长度已减1
            while (low < high) {
                mid = (low + high) >>> 1;
                if (data[mid].aabbCollider.right < newRight) {
                    low = mid + 1;
