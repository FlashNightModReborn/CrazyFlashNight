var JukeboxHarnessQA = (function() {
    ...
    function runSuite(api, host, onlyCase) {
        var cases = [
            ...
            ['availability-keyboard-guard', 'keyboard activation and roving skip non-available tracks', function() {
                return openWithCatalog(api, host, availabilityFixture()).then(function() {
                    var before = playMessageCount(host);
                    var probing = findTrack('Probing Track');
                    probing.focus();
                    key(probing, 'Enter');
                    api.assertEqual(playMessageCount(host), before, 'disabled keyboard activation emits no play');

                    probing.setAttribute('data-availability', 'available');
                    probing.setAttribute('aria-disabled', 'false');
                    key(probing, 'Enter');
                    api.assertEqual(playMessageCount(host), before, 'forged keyboard state cannot bypass catalog check');

                    var available = findTrack('Available Track');
                    available.focus();
                    key(available, 'ArrowDown');
                    api.assertEqual(document.activeElement, available, 'roving navigation skips all disabled rows');
                    key(available, 'Enter');
                    api.assertEqual(playMessageCount(host), before + 1, 'available keyboard activation emits exactly one play');
                    return 'availability keyboard guard ok';
                });
            }],
