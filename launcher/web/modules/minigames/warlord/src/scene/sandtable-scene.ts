...
export interface SandtableOptions {
  ...
  // 悬停信息钩子：info 为 null 表示悬停离开；anchor 是 canvas 本地 CSS 像素坐标
  onHoverInfo?(
...
function liftLightness(hex: number, factor: number): number {
...
function cssColor(hex: number, scale: number): string {
  const color = new THREE.Color(hex);
  const channel = (value: number): number => Math.max(0, Math.min(255, Math.round(value * scale * 255)));
  return `rgb(${channel(color.r)}, ${channel(color.g)}, ${channel(color.b)})`;
}

// 正交相机下的全屏背景：上深下暖、近地平线微亮的垂直渐变，色值全部从主题令牌派生
function makeBackdropTexture(theme: MapTheme): any {
...
function makeEnvironmentTexture(theme: MapTheme): any {
  const canvas = document.createElement('canvas');
  canvas.width = 64;
  canvas.height = 32;
  const context = canvas.getContext('2d');
...
function makeRouteMarkTexture(): any {
  const canvas = document.createElement('canvas');
  canvas.width = 64;
  canvas.height = 64;
  const context = canvas.getContext('2d');
...
export class SandtableScene {
  ...
  private sunLight: any | null = null;
  private environmentTarget: any | null = null;
  ...
  private buildGroundGrid(bounds: WorldBounds): void {
    const step = 1.25;
    ...
    const pushVertex = (x: number, z: number, color: any): void => {
  ...
  private buildContourLines(bounds: WorldBounds): void {
    ...
    const heightAt = (ix: number, iz: number): number => heights[iz * (countX + 1) + ix]!;
    const positions: number[] = [];
    const pushSegment = (p: readonly [number, number], q: readonly [number, number], level: number): void => {
    ...
    for (const level of levels) {
      for (let iz = 0; iz < countZ; iz += 1) {
        for (let ix = 0; ix < countX; ix += 1) {
          ...
          const top: readonly [number, number] = [x0 + stepX * ((level - h0) / (h1 - h0)), z0];
          const right: readonly [number, number] = [x0 + stepX, z0 + stepZ * ((level - h1) / (h2 - h1))];
          const bottom: readonly [number, number] = [x0 + stepX * ((level - h3) / (h2 - h3)), z0 + stepZ];
          const left: readonly [number, number] = [x0, z0 + stepZ * ((level - h0) / (h3 - h0))];
          const centerHigh = (h0 + h1 + h2 + h3) / 4 > level;
          if (mask === 1 || mask === 14) pushSegment(bottom, left, level);
          else if (mask === 2 || mask === 13) pushSegment(right, bottom, level);
          else if (mask === 3 || mask === 12) pushSegment(right, left, level);
          else if (mask === 4 || mask === 11) pushSegment(top, right, level);
          else if (mask === 6 || mask === 9) pushSegment(top, bottom, level);
          else if (mask === 7 || mask === 8) pushSegment(top, left, level);
          else if (mask === 5) {
            // 鞍点按中心均值消解歧义
            if (centerHigh) { pushSegment(top, left, level); pushSegment(right, bottom, level); }
            else { pushSegment(top, right, level); pushSegment(bottom, left, level); }
          } else if (mask === 10) {
            if (centerHigh) { pushSegment(top, right, level); pushSegment(bottom, left, level); }
            else { pushSegment(top, left, level); pushSegment(right, bottom, level); }
          }
        }
      }
    }
  ...
  private createNodeLandmark(kind: NodeKind, material: any, accentMaterial: any, signalMaterial: any): any {
  ...
  private emitHoverInfo(clientX: number, clientY: number): void {
    if (!this.options.onHoverInfo) return;
    const rect = this.renderer.domElement.getBoundingClientRect();
    const anchor = { x: clientX - rect.left, y: clientY - rect.top };
    if (this.hoveredPieceId) this.options.onHoverInfo({ kind: 'piece', pieceId: this.hoveredPieceId }, anchor);
    else if (this.hoveredNodeId) this.options.onHoverInfo({ kind: 'node', nodeId: this.hoveredNodeId }, anchor);
    else this.options.onHoverInfo(null, anchor);
  }
  ...
  private cameraIntentView(): CameraView {
    const tween = this.cameraTween;
    return tween
  ...
  private renderFrame(time: number, animate: boolean): void {
    ...
    if (this.cameraTween) {
      ...
      if (tween.startedAt === null) tween.startedAt = time;
    ...
    if (this.animationStartedAt === null) this.animationStartedAt = time;
    ...
    for (const [pieceId, visual] of this.pieceVisuals) {
      if (visual.current.distanceToSquared(visual.target) > 0.0001) {
        ...
        moving = moveStep < 1;
        if (moveStep >= 1) visual.current.copy(visual.target);
      }
