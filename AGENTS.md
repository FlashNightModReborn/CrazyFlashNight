...
**最后核对代码基线**：release source commit `dfa156c8cd7e860c1e5795c7987525b76829635f`（2026-09-07；tag `runtime-build-v2/20260907-map-workbench-phase2-v2`）。地图内容创作工作台第二阶段已完成双 signer / 双故障域共识、40/40 production policy 与原子 promotion，当前为 `promoted`。维护者已确认作者流程修复可行；部署后未自动重跑新地点入口、NPC 剧情迁移等完整游戏旅程，不称业务 `standard_entry_verified`。正式身份和远端审计见 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)，实施与边界见[两阶段记录](docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)。上游焦点持续录制与撤退结算修复一并保留。
