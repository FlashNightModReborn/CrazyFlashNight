...
_root.装备生命周期函数.剑圣手甲初始化 = function(ref:Object, param:Object) {
    ...
    // 无进阶：不挂载腕刃，以 ready_static 参与完整套装
    if (!tier) {
    ...
    // 左下臂引用真正就位后同步腕刃；不在 tick 中抢写纸娃娃坐标。
    if (!_root.装备生命周期函数.剑圣套装登记就位消费者(
        ref, "左下臂_引用", _root.装备生命周期函数.剑圣手甲视觉更新)) {
        return org.flashNight.arki.unit.UnitComponent.Initializer.SetEffectController.FAILURE;
    }
    ...
    if (!_root.装备生命周期函数.剑圣手甲初始化Buff系统(ref)) {
        return org.flashNight.arki.unit.UnitComponent.Initializer.SetEffectController.FAILURE;
    }
 ...
 * @param {Object} ref 生命周期反射对象
 */
_root.装备生命周期函数.剑圣手甲初始化Buff系统 = function(ref:Object):Boolean {
    var target:MovieClip = ref.自机;
    ...
    // 无加成则不创建 buff
    if (baseValue <= 0 && burstValue <= 0) return true;
    ...
    var controllerId:String = target.buffManager.addBuff(controllerMeta, "剑圣腕刃控制器");
    if (!controllerId || !_root.装备生命周期函数.剑圣套装登记Buff(ref, target.buffManager, controllerId)) return false;

    // ========== 2. 立即应用常驻态效果 ==========
    _root.装备生命周期函数.剑圣手甲切换到常驻态(ref);
    if (!_root.装备生命周期函数.剑圣套装登记Buff(ref, target.buffManager, "剑圣腕刃增强")) return false;
    return true;
};
...
_root.装备生命周期函数.剑圣手甲渲染更新 = function(ref:Object) {
    ...
    if (!_root.装备生命周期函数.剑圣套装读取就位基向量(
        ref, "左下臂_引用", container, p0, pX, pY)) return;
    weapon._x = p0.x;
    weapon._y = p0.y;
