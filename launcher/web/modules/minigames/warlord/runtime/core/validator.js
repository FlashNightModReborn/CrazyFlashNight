...
function validateMove(state, command) {
    ...
    if (state.activeFactionId !== command.factionId)
        return fail('当前不是该阵营的行动时机。');
    if (command.originNodeId === command.targetNodeId)
        return fail('起点与目标节点不能相同。');
    if (!nodeIsAdjacent(command.originNodeId, command.targetNodeId))
        return fail('目标节点与起点不相邻。');
    if (command.pieceIds.length === 0)
        return fail('至少选择一枚棋子。');
    if (new Set(command.pieceIds).size !== command.pieceIds.length)
        return fail('棋子列表包含重复项。');
    for (const pieceId of command.pieceIds) {
        const piece = state.pieces[pieceId];
        if (!piece || piece.hp <= 0)
            return fail(`棋子 ${pieceId} 不存在或已阵亡。`);
        if (piece.factionId !== command.factionId)
            return fail(`棋子 ${pieceId} 不属于当前阵营。`);
        if (piece.nodeId !== command.originNodeId)
            return fail(`棋子 ${pieceId} 不在起点节点。`);
    }
    ...
    if (actualPieceIds.length === 0)
        return fail(`${node.displayName}已达到驻军容量。`);
    ...
    switch (command.type) {
        ...
        case 'ENQUEUE_PRODUCTION': {
            ...
            if (cardState.level < definition.deploymentLevel)
                return fail(`卡牌需要 Lv.${definition.deploymentLevel} 才能生产。`);
            if (faction.gold < definition.productionCost)
                return fail(`金币不足，需要 ${definition.productionCost}G。`);
        ...
        case 'CANCEL_PRODUCTION': {
            ...
            if (faction.populationReserved < order.populationCost)
                return fail('生产预留人口状态异常，不能撤销。');
            return { ok: true };
        }
        case 'COMMIT_PLANNING': {
            if (state.phase !== 'SETTLEMENT_PLANNING')
                return fail('当前不是结算规划阶段。');
            if (state.factions[command.factionId].planningCommitted)
                return fail('该阵营已经提交规划。');
            return { ok: true };
        }
    }
