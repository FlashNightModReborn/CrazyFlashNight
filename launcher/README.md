...
> **最后核对架构与工作流**：CF7 Agent Runtime / Wings F8 release source `6f3d50a52413c747b05b74be88d6ee46650f4597` 已由 immutable tag `runtime-build-v2/20260731-agent-runtime-wings-f8-v1`、release tree `253e57f6d20a90fef6addfa744d0487d88f00dfb`、request `A9B33601805709DBB5EAE6DAF312C2B7B0B502096FDD3BDCEA9CBE26D8B1299C`、本地 X509 + GitHub OIDC/Sigstore 双故障域共识完成正式 promotion。无 candidate id 的 pure-MCP 正式入口 run `20260731T040942Z` 完成 Launcher/NativeHud/WebOverlay WGC、Flash metadata-only fail-closed、structured 帮助面板和 supported shutdown，故该明确窄纵切达到 `standard_entry_verified`；这不代表物理双屏、“13/13”、Flash pixels/input、Hair、Wings 完整产品、业务写、持久化或维护者目视签收。本文只维护 Launcher 的稳定入口、协议与验证规则；逐次证据见 [F8 人工验收与正式发布记录](../docs/evidence/cf7-agent-runtime-f8-manual-acceptance-2026-07-31.md)，通用判定见 [验证矩阵](../agentsDoc/testing-guide.md)。
> **PlayerInfo 当前边界**：F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / r2 自身仍是历史 non-deployment train；其实现字节被当前 `6f3d50a52413…` release 包含并进入 formal runtime。B0 仍为 `b0_accepted`，oracle 仍为 `oracle_frozen_for_b0`；本轮 formal smoke 没有启用 PlayerInfo fixture、观察真实 `pi_*` 或完成 PlayerInfo-specific standard-entry E2E，不能从总体 runtime 的 `standard_entry_verified` 外推 PlayerInfo 专项验收。历史 identity、可见验收与边界继续见 [runtime v2 深层文档](../docs/runtime-build-reproducibility.md)、[PlayerInfo 工具证据](../tools/player-info-hud/README.md)和 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)。
...
### CF7 Agent Runtime / Wings Network（F7 历史 + F8 当前正式发布）
