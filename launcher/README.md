...
> **最后核对代码基线**：commit `e9aaf0a7a6`（2026-07-16）+ 当前装备调制迁移工作树。装备调制已完成 AS2 单一权威服务、Host `equipment_tuning` domain、Web workbench tuning view、旧 renderer 服务适配和受控 agent 直达；battlebox 收纳箱固定提供 Web 调制反馈入口，不保留独立开关或 capability 分支，但旧 AS2 调制子入口在人工验收前继续进入服务化原生 renderer，Host 拒绝残留 legacy redirect。专用克隆槽已取得正式 opener → active workbench → 首个权威 snapshot 的真机证据。Skill 独立 Web 面板的既有 S0–S5 状态与观察债继续按技能迁移文档维护。
...
- **.NET 10 SDK**（10.0.300 patch band，对齐 `global.json` 的 `version: 10.0.300` + `rollForward: latestPatch`）
  - user-scope 装法（无需 admin）：
    ...
    ```
    安装后 `%LOCALAPPDATA%\Microsoft\dotnet\dotnet.exe` 即可用。`build.ps1` / `run_tests.ps1` 共用 `resolve-dotnet.ps1`，按根目录 `global.json` 的 feature band 在 `%LOCALAPPDATA%`、`%USERPROFILE%\.dotnet` 与系统 PATH 中选择兼容 host，不要求也不修改系统 PATH
...
`status` 返回 `launchState`、`revealPerformed`、`socketConnected`、`readyForRuntimeAutomation`、`runtimeReadyBlockedBy`、`readyForArenaCalibration`、`readyBlockedBy`、`save`、`saveRuntime`、只读 `activePanel` 和内嵌 `arenaCalibration`。通用 ready 只要求 `launchState=Ready`、Flash reveal、socket、安全 snapshot 决议，以及 AS2 `agent_runtime_status` 对同一 `attemptId/savePath` 的 loaded ack；arena ready 在此基础上再要求 arena status 可读。坏档停在 `save_decision_unsafe` / `runtime_save_not_loaded`，不得进入任何领域动作。`start` 默认 `requireFlashReveal:true`、`deferReveal:false`；slot 拒绝路径分隔符和保留字符，且默认不写 `lastPlayedSlot`。
...
目标固定默认为 `cf7_agent_equipment_tuning`；runner 永久拒绝 live target 和 `--fresh`，先备份/重建专用目标槽，再以 fresh log watermark、handoff、同 attempt runtime ack、active panel 与首个 snapshot 联合判定。它在 snapshot 后停止，`uiBusinessClicks=false`、`businessWritesAttempted=false`，因此证明正式入口和读链，不替代业务写入/重启回读专项。
...
| 装备调制切片 | `equipment-tuning-runtime.js` + `equipment-tuning-view.js`，嵌入 `workbench view=tuning` | 独立 `domain=equipment_tuning`；顶层为“强化度 / 交换 / 进阶 / 配件”，强化自动核算最终目标且以动态/永久上限分离避免 `+14` 幻象，交换通过 coordinator 只读 projection 在右栏展示同 use 权威候选且不改左栏筛选/面包屑；候选富注释保持 AS2 intro/desc 分段，workbench 禁止原生 `title` 竞争；配件按插件定义投影的档级/目录用途/定位建立共享树与面包屑，已安装件可原子替换或显式卸下，紧凑候选对齐 owned grid 的 `48/40/4px` 尺寸并以 25 项无滚动为最低画布门；全工作台使用 DLS 基调，强化石核心按密度与 reduced-motion 动静降级；顶栏 `?` 复用 workbench modal 且零业务消息。七类 wire 写全部由 AS2 snapshot/preview/opaque token commit 裁决；Host 维护实例租约、write epoch 与未知写结果精确对账，库存刷新完成前禁止并发转移/整理/关闭 |
...
| 合成工作台 | `crafting-runtime.js` + `crafting.js` + 共享 `item-filter.js`/工作台壳 | 12 类旧合成入口 → `crafting`；Flash 权威 preview/一次性 commit，支持批量份数上限、共享树筛选及合成↔战备箱本地子路由，返回后强制重新 snapshot/preview |
