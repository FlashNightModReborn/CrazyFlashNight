...
正式 v2 consensus 当前绑定 tag `runtime-build-v2/20260822-t800-managed-longgun-v1`、request `1643570AB50AE207B2D09AC55D6D1F374DD72990070B71423372B6F184497C8A`、source commit `d049daed44a537ef586f9e6254fcb824b98c4ee1`、build identity `0A4478EF7C9D18BCF560B6AB8F2EBA66C255930FCDC361F5FECC36469509A522` 与 payload closure `B18AB5EC8A2161621D55ED35C860C96248B9081772CC1DD4B911444815C2169C`。local signer 位于 `physical-host-b`，不可导出 keyId 为 `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`；GitHub Actions run `32577704317` 的 OIDC identity `D99418547862A8823FF5D42D71BFB2917A7789E515B282A2F2B4E613250C1D85` 提供 `github-hosted-windows` 第二票。production policy `FFD8546DF1FA6F8DA1CF93B7315D4DDEA9CD8BBA9F0A3D199B5A0D9A00172A44` 的 38/38 final receipt SHA-256 为 `4238A0DD7E35FDE00FB0BA912000EAE34F63F8E8A09C81AE2AEA564F2DF7E350`；promotion commit `a22490867ebd03b931c3066141c0954b02be25da` 已由 post-promotion audit run `32578381540` 确认为 `state=promoted`、`deploymentChanged=true`，严格 manifest/consensus 校验为 33 files、2 signers、2 faultDomains。维护者已在同 Core 字节隔离候选完成 T800 托管长枪、M134 与 XM214 可见业务验收；部署后无 candidate id 的正式入口也完成 `session.status` verified 与 trusted shutdown exit 0，但未调用三个武器业务命令，因此本列车只称 `HUMAN_ACCEPTANCE_PASSED / promoted`，不称 T800 专项 `standard_entry_verified`。cloud workflow 仍只允许 `Crazyfs` / `Flash-Night` 的固定 actor ID 首次 dispatch。
...
本列车还暴露过两类未在 B0 内根治的 portability/tooling debt。其一，首次隐藏 PowerShell worker 未显式初始化 UTF-8，中文 bundle path 被错误代码页解码并对同一 request fail-closed；保留该 `failure.json` 后，显式 UTF-8 wrapper 才成功，不能据此宣称正式 worker 已具备跨代码页可移植性。其二，当时 `launcher/scripts/dist/hit-number-bundle.js`、`launcher/web/modules/arena-custom-presets.js`、`launcher/web/modules/arena-unit-catalog.js`、`launcher/web/modules/arena-unit-param-presets.js` 只有 `text=auto`。2026-08-23 的独立 clean-clone 复核在 `core.autocrlf=true` 下精确复现：Git 状态干净但 `arena-custom-presets.js` 为 CRLF，raw worktree OID `9638a340400f38d65904d62142eae047c2b86e84` 不同于 index/clean-filter OID `78b7f822542db75b8ad17ec312ba3c9caf5b8854`，其 raw-byte `--check` 误报 stale；`Get-Cf7RuntimeV2WorktreeObjectIds` 与单文件对象 ID 均正确返回 clean-filter OID。当前 source-ahead 已为四文件显式声明 `text eol=lf`，并把真实物理 CRLF、Git-clean、batch/single/index OID 等价性纳入 `tools/test-runtime-build-v2.ps1`；fresh checkout 现为 LF 且同一 generator `--check` exit 0。该修复尚未形成新 request、builder closure 或 promotion，不改写 historical v1 F，也不表示正式 T800 runtime 已包含新属性字节；UTF-8 worker debt 仍须独立处理。
