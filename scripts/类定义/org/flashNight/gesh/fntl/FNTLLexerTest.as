...
class org.flashNight.gesh.fntl.FNTLLexerTest {
    ...
    private function getLexerTestCases():Array {
        ...
        // Test case 48: Extremely deeply nested tables
        var testCase48:Object = new Object();
        var nestedText48:String = '';
        for (var i:Number = 0; i < 1000; i++) {
            nestedText48 += '[';
        }
        for (var j:Number = 0; j < 1000; j++) {
            nestedText48 += 'key = "value"\n';
        }
        for (var k:Number = 0; k < 1000; k++) {
            nestedText48 += ']';
        }
        testCase48.text = nestedText48;
        testCase48.expected = null; // Depending on implementation, expected to fail or handle gracefully
        testCase48.description = "测试极度深度嵌套的表结构，检查是否存在无限循环或递归深度超限。";
        cases.push(testCase48);

        // Test case 49: Circular references (if supported)
        var testCase49:Object = new Object();
        testCase49.text = 'a = { b = { a = { } } }\n';
        testCase49.expected = null; // Expected to fail or handle circular references gracefully
        testCase49.description = "测试循环引用结构，检查解析器是否防止无限循环。";
        cases.push(testCase49);

        // Test case 50: Mixed nested arrays and tables
        var testCase50:Object = new Object();
        testCase50.text = 'mixed = [ { key1 = "value1", nested = [1, 2, 3] }, { key2 = "value2", nested = [4, 5, 6] } ]\n';
        var expected50:Object = {
            mixed: [
                { key1: "value1", nested: [1, 2, 3] },
                { key2: "value2", nested: [4, 5, 6] }
            ]
        };
        testCase50.expected = expected50;
        testCase50.description = "测试混合嵌套的数组和表结构，确保解析器正确处理。";
        cases.push(testCase50);


        // Test case 51: Large dataset
        ...
        largeText += ']\n';
        testCase51.text = largeText;
        var expected51:Object = {
            numbers: []
        };
        for (var j:Number = 0; j < 10000; j++) {
            expected51.numbers.push(j);
        }
        testCase51.expected = expected51;
        testCase51.description = "测试包含大量数据的数组，检查解析器的性能和稳定性。";
        cases.push(testCase51);

        // Test case 52: Irregular nesting without proper closures
        var testCase52:Object = new Object();
        testCase52.text = '[a.b.c = { key = "value" }\n'; // Missing closing bracket for [a.b.c]
        testCase52.expected = null; // Expected to fail due to improper nesting
        testCase52.description = "测试不规则的嵌套结构，检查解析器是否能正确报错。";
        cases.push(testCase52);

        // Test case 53: Extremely large empty array and object
        var testCase53:Object = new Object();
        var emptyArray:String = 'empty_large_array = [';
        for (var i:Number = 0; i < 10000; i++) {
            emptyArray += ' ';
        }
        emptyArray += ']\n';
        var emptyObject:String = 'empty_large_object = {';
        for (var j:Number = 0; j < 10000; j++) {
            emptyObject += ' ';
        }
        emptyObject += '}\n';
        testCase53.text = emptyArray + emptyObject;
        var expected53:Object = {
            empty_large_array: [],
            empty_large_object: {}
        };
        testCase53.expected = expected53;
        testCase53.description = "测试极限大小的空数组和对象，确保解析器稳定。";
        cases.push(testCase53);

        // Test case 54: Mixed multiple nesting types
        var testCase54:Object = new Object();
        testCase54.text = 'complex = [ { a = [1, 2, { b = "c" }], d = { e = [3, 4, 5] } }, { f = "g" } ]\n';
        var expected54:Object = {
            complex: [
                {
                    a: [1, 2, { b: "c" }],
                    d: { e: [3, 4, 5] }
        ...
        testCase54.expected = expected54;
        testCase54.description = "测试多种嵌套类型的混合，确保解析器正确处理。";
        cases.push(testCase54);

        // Test case 55: Asymmetrical nesting with extra closing brackets
        var testCase55:Object = new Object();
        testCase55.text = 'a = { b = { c = "d" } } } \n'; // Extra closing brace
        testCase55.expected = null; // Expected to fail due to extra closing brace
        testCase55.description = "测试不对称的嵌套结构，包含多余的闭合符号。";
        cases.push(testCase55);

        // Test case 56: Asymmetrical nesting with missing closing brackets
        var testCase56:Object = new Object();
        testCase56.text = 'a = { b = { c = "d" }\n'; // Missing closing brace for 'a'
        testCase56.expected = null; // Expected to fail due to missing closing brace
        testCase56.description = "测试不对称的嵌套结构，缺少闭合符号。";
        cases.push(testCase56);

        // Test case 57: Mixing different types of brackets
        var testCase57:Object = new Object();
        testCase57.text = 'mixed_brackets = { array = [1, 2, 3], table = { key = "value" } ]\n'; // Mismatched closing bracket
        testCase57.expected = null; // Expected to fail due to mismatched brackets
        testCase57.description = "测试混合使用不同类型的括号，包含括号类型不匹配的情况。";
        cases.push(testCase57);

        // Test case 58: Recursive structures
        var testCase58:Object = new Object();
        testCase58.text = 'recursive = { a = { b = { a = { b = { a = "end" } } } } }\n';
        var expected58:Object = {
            recursive: {
                a: {
                    b: {
                        a: {
        ...
        testCase58.expected = expected58;
        testCase58.description = "测试递归结构，确保解析器正确处理递归嵌套。";
        cases.push(testCase58);
