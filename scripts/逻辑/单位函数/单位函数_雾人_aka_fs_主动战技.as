...
_root.主动战技函数.长枪.突击者之眼 = {
    ...
    释放许可判定:function(自机){
        ...
        if(自机[shootCountName][自机["长枪"]] + 自机.突击者之眼数 > 自机[magazineCapName]) return false;
        if(自机.浮空 || 自机.倒地) return false;
        return true;
    },
    释放:function(自机){
...
_root.主动战技函数.兵器.EternalMaximumDrive = {
    初始化:null,
    释放许可判定:function(自机){
        return !自机.倒地;
    },
...
_root.主动战技函数.兵器.狂瀑扎 = {
    ...
    释放:function(自机){
		...
		脚部装备 == "黑犀鞋" ){
            ...
            自机.刀属性数组[13] += 自机.转换的冷兵器加成;
            _root.效果("紫金增幅",自机._x,自机._y + 30,100);
            _root.效果("刀虚影特效",自机._x,自机._y + 30,100);
...
_root.主动战技函数.空手.狂瀑顶 = {
    ...
    释放:function(自机){
		...
		脚部装备 == "黑犀鞋" ){
            ...
            自机.转换的冷兵器加成 = 0;
            _root.效果("紫金增幅",自机._x,自机._y + 30,100);
            _root.效果("拳虚影特效",自机._x,自机._y + 30,100);
