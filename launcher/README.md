...
按 commit `5b624757113c302ec756b0a8a6a809a2a8d862a5`（2026-07-23）时的源码树复核。只列追踪目录；`bin/` / `packages/` / `target/` / `node_modules/` / `obj/` 等构建产物和缓存均由 .gitignore 管理。后续若需复核本节，优先看 `git diff 5b624757113c302ec756b0a8a6a809a2a8d862a5..HEAD -- launcher/`。C# 文件级权威以 [CRAZYFLASHER7MercenaryEmpire.csproj](CRAZYFLASHER7MercenaryEmpire.csproj) / [tests/Launcher.Tests.csproj](tests/Launcher.Tests.csproj) 的 `Compile Include` 为准，README 仅保留职责树。
...
当前地图资源箱 Web-only consensus 采用 `builder-local-b`（keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`）/ `physical-host-b` 与 GitHub OIDC builder identity `66EEEDBA5D430735930D70053D9EF0A0F9D9561A5129683F6ED92A7180F582D4` / `github-hosted-windows`（run `29967356506`）；tracked registry 仍只含公钥。正式 bootstrap/runtime/manifest/consensus 已由唯一 promotion 入口切换到 request `F1F9493CF08DD88F26E1493FCACE306AC160866EA21440FC62698E5965A1AF04`、build identity `7C72B92B0C1CF57EB9BC0D3C1024D31657EE52E6B13D7BBF9FDB94FD5A6186DB` 与 payload closure `7E5EDCD4FEA80E1269C0B8BCC325D1FE0994EE8C7321F0F71CB9AF4B369C4A44`；此前商店与单-canary consensus 均只保留为历史记录。一次性 migration fuse 不得复用，后续发布不得降级 v1。
...
| Flash 实例 | Web 实现 | 协议 |
|-----------|---------|------|
| 基地理发店（迁移期，旧 UI fallback 保留） | `hairdresser-runtime.js` + `hairdresser.js` + `hairdresser.css` + 共享 `panel-runtime.js` / `dressup-doll-renderer.js` | 理发师 NPC 先发精确 `panel_request panel=hairdresser source=world_hairdresser`，发送失败才打开旧 `理发店界面`；独立 `hairdresser` domain 只允许 snapshot/commit，Web 本地预览，AS2 权威写发型并标 dirty，未知写只 fresh snapshot 对账且零重放 |
...
- **hairdresser**（基地理发店，F3 迁移期）: `web/modules/hairdresser-runtime.js` + `web/modules/hairdresser.js` + `web/css/hairdresser.css`；lazy 依赖固定为 `panel-runtime → asset-timeline → dressup-doll-renderer → hairdresser-runtime → hairdresser`，`panel-scale.js` 已由 overlay boot 加载，禁止在 lazy deps 中重复执行。基地理发师 NPC 只调用 `_root.gameCommands["openHairdresser"]()`，发送精确 `panel_request {panel:"hairdresser",source:"world_hairdresser"}`；命令缺失或发送失败时才走同一入口唯一 `if (!opened)` 的旧 `理发店界面` fallback。独立 domain 只有 snapshot/commit：AS2 从现役三数组保持 77 行原序与重复项，任一非零/非法价格整体拒绝；snapshot 下发权威 `gender/face/currentHair/catalog`。Web 选择只用共享 `DressupDollRenderer` 做 strict 脸型/发型、本地、非动画预览，光头只绘脸型；未知性别或缺素材只给可读降级，不猜性别，也不禁目录或 commit。commit 只传 hairIdentifier，无价格、货币、backend preview、token 或自动存盘；AS2 先重查免费目录和全部依赖，再依次写 root 发型、live actor、刷新装扮及 dirty mark。字段完整且 `currentHair` 精确匹配本次选择的成功回包立即复用正常 `requestClose()`，只产生一次 close；确定失败和未知写对账保持面板可操作，不加延时器或第二套完成状态。Host `HairdresserTask` 是 `PanelPendingCallTracker<TContext>` 的第三个真实消费者，不自建 pending/Timer/callId mux；未知写只以 fresh snapshot 判定 applied/not-applied，绝不重放。生产 close 会清 transport pending，若有在途写则保留 `needs_reconcile` 到重开对账，迟到 commit 不复活。release policy 已把 `css\hairdresser.css`、`modules\hairdresser-runtime.js`、`modules\hairdresser.js` 设为必需路径；packer 仍由既有 `css/**`、`modules/**` 规则纳入，无独立 include。当前自动门为 contract 4 domains/21 commands、变异 **62/62**、browser 三视口 **39/39**、shared runtime **6/6**、Launcher **1257/1257**、fresh Output Panel `HairdresserPanelServiceTest 25/25` 和两个精确 Flash 目标 publish/verify。首个绑定 `d49ed94d9461ffb5791888700645906413898694` 的隔离候选已证明真实 NPC/socket/WebView2 入口、权威写入、live actor 刷新、正常存盘及完全退出，但同时暴露并定位了“成功确认后未自动关闭”；该旧候选不能证明本轮修复。仍须以修复后的 clean immutable tree 重建候选，复核自动关闭并完成不重新播种的完整重启回读和最终维护者验收；旧 UI 与主 XFL 三个全局发型函数继续保留，不得称已部署或迁移完成。
