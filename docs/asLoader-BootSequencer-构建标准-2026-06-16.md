...
**状态**：契约与单帧实现均已落地；当前 29 个 live source 由唯一 `BOOT_SOURCES` 生成 6 个纯装配 stage，S0/S5/S9 业务归 `BootSequencer`。happy-path 真机 boot、S2→S10 全 10 事件 trace 等价门与 L1 `BootSequencerTest 58/58` / `BootstrapHandshakeTest 12/12` 是已有历史证据；当前源码树的 tracked L1 契约已更新为 `86/86 + 12/12`，须以 §5.3 新入口取得 fresh trace 后才能声称当前行为闭环。七边界只保留真 socket 10s 超时为低价值人工观察项（见 §5.3）。
...
- **L1 — TestLoader 单元测试**（`scripts/类定义/org/flashNight/boot/BootSequencerTest.as` + `scripts/类定义/org/flashNight/neur/Server/test/BootstrapHandshakeTest.as`）：现行 tracked 入口是 `powershell -ExecutionPolicy Bypass -File scripts/run-boot-sequencer-tests.ps1 -TimeoutSeconds 240`，由共享 focused runner 在仓库编译 mutex 内安装 template、消费唯一 runId block 的 fresh trace，再按 SHA-256 恢复本地 `scripts/TestLoader.as`。在 debug player 里 **mock `host`/`_root._bootstrap`/`_root.server`/`_root.__boot` staged 函数/`存档恢复等待中` 等**，直接 `new BootSequencer(host)` + 手动驱 `step()` 断言状态机；BootstrapHandshakeTest 额外守 `timeoutMs` 传给 sender。**deterministic、每次改 BootSequencer.as / bootstrap handshake 即回归**。当前 exact contract 为 `BootSequencerTest 86/86` + `BootstrapHandshakeTest 12/12`；`-SkipCompile` 只验 template/suite/BOM/scratch 事务，不是 Flash 编译或行为证据。2026-08-11 fresh runId `631b5a64fa7c41e682431ab6956818f2` 已取得 `86/86 + 12/12`、Compiler `0/0`、32K retry `0`，`scripts/flashlog.txt` SHA-256 为 `FB1E36D19F36611BDC33E3029FDCC48D657274E1FF97661A32776D5EF7ABBD6A`。覆盖：(b) socket 超时、(c) shim 缺失、握手失败、(a) 修复 gate 自旋→放行、(g) `run()` 幂等、S6 等待门/抽干顺序、S8 数据与装备配置收敛乱序、S9 材料目录与 item/enemy/legacy/mod/shop/kshop 权威 readiness/failure 及 `CRAFTING_OK`、S10 `HANDOFF_PLAY`，以及 prewarm hold 不再被 ServerManager 默认 20s callback timeout 抢先打断。
...
- ✅ **TestLoader 重编**：TransitionsTest 117 passed / 0 failed；BootSequencerTest **47 passed / 0 failed**（含新增 `(e/race)` 首层慢-list 4 断言 + `(e/ceiling)` 150 帧兜底 4 断言 + `(e/current)` malformed queue 4 断言；旧 36→47）；BootstrapHandshakeTest **12/12 passed**（含 sender timeoutMs=60000 断言）。S6 `sawPending`、`loaders.current` 防御与 handshake timeout 透传均有 L1 守卫。
- **全库 linkage scanner**：`tools/linkage_scanner/scan_linkage.py` 完整跑完（约 436.8s），不是死锁；当前全库仍有历史 live cross-SWF conflicts=98、orphans=36。asLoader XFL 三件套保持 CLEAN，但全库 linkage 不能声明干净。
- ✅ **编译 harness 加 `-Target` 开关**（`compile_test.ps1 -Target test|publish|<路径>` + `compile_action.jsfl` 读后删除 `scripts/compile_target.cfg` 一次性 file:/// URI；publish 自动启用 `-VerifySwf`）：免手动切 Flash 活动文档即可在 test/发布端切换。见 [FlashCS6自动化编译.md §3](../scripts/FlashCS6自动化编译.md) / [testing-guide §编译 smoke](../agentsDoc/testing-guide.md)。
