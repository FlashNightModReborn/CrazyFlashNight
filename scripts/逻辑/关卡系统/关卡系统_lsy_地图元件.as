...
_root.创建可拾取物 = function(物品名, 数量, X位置, Y位置, 是否飞出, parameterObject){
	...
	var pickupItem = gameworld.attachMovie("可拾取物2", "可拾取物" + _root.可拾取物计数, gameworld.getNextHighestDepth(), parameterObject);

	pickupItem.焦点高亮框.gotoAndPlay(_root.随机整数(1,59));
	
	if (!pickupItem.dispatcher) {
		pickupItem.dispatcher = new LifecycleEventDispatcher(pickupItem);
	}
    ...
    pickupItem.dispatcher.subscribeSingleGlobal("interactionKeyDown", pickUpFunc, pickupItem);
	pickupItem.dispatcher.subscribeSingleGlobal("interactionKeyUp", resetFunc, pickupItem);
