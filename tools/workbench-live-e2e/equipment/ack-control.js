...
function main(argv) {
  ...
  if (args.help) {
    console.log(JSON.stringify({ status: "HELP", usage:
      "node ack-control.js --run-dir <owned-run> --request-id <id> --transport <launcher_agent_runtime|codex_computer_use> --result <completed|unavailable|cancelled|failed> --provider-receipt <owned-provider-receipt.json> [--authorization-decision-id <id>]" }));
    return null;
  }
