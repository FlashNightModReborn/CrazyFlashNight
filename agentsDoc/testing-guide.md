...
**Browser harness**(直接打开)：`launcher/web/modules/minigames/{lockbox,pinalign,gobang}/dev/harness.html` / `launcher/web/modules/map/dev/harness.html`。
...
静态校验拦截:旧平铺 Lockbox 入口、旧版分游戏 session 命令名、旧共享结构 class 名。Jukebox panel（Phase 5）暂无自动 harness：改 `web/modules/panels/jukebox-panel.js` / `WebOverlayForm.HandleJukeboxMessage` / `MusicCatalog` / `JukeboxTitlebarWidget` 时人工走 [launcher/README.md](../launcher/README.md) "Phase 5 Jukebox panel 手测" 12 步。
