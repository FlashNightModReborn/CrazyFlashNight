...
正式根 bootstrap `--verify-only`、33-file bundle 与 signed consensus 已通过。首轮 `gate-f-week-full-v2` campaign 已运行三份正式 soak：前两份 20/20 finished，第三份 8 finished + B11/C12 原向 2 timeout；该失败暴露的是静态 soak 路由把候选自身长战随机性混入基础设施门，不是平台或存档污染。证据准入修正后的 `gate-f-week-full-v3` campaign 三份 fresh soak 为 30/30 finished、0 timeout/error/recovery、exact formal runtime、正常关闭且受保护存档不变；全量随后累计 8/198 completed shard、115 条 durable row、10 条 duplicate excluded、0 failed，并因旧进程监控偶发把 runner 内部 `arena-tools → run-checks.js → gate-fctl.js --check` 子进程误判为独立 producer 而安全暂停。修复后的 tracked plan 保持 58 个 scheduled candidate + `B12` quarantine、198 个 shard / 3,255 个 run，执行 campaign 提升为 `gate-f-week-full-v4`；v3 事实原样保留但不跨 plan hash 混计，v4 重新 freeze/arm/fresh soak 前不称 Gate F 通过、斗兽业务 `standard_entry_verified` 或新真人 PVE 证据。
...
node tools/arena-calibration/build-gate-f-week-plan.js `
  ...
  --plan-id gate-f-week-full-v2 --campaign-id gate-f-week-full-v4 `
