...
test('AC-27 每个伤亡只结算一次击杀方 1 倍、损失方 3 倍经验', () => {
  ...
  assert.equal(state.factions.red.xpPool, expected);
  assert.equal(state.factions.blue.xpPool, expected * 3);
  const afterFirst = { red: state.factions.red.xpPool, blue: state.factions.blue.xpPool };
  runSettlementAutoInPlace(state);
  assert.deepEqual({ red: state.factions.red.xpPool, blue: state.factions.blue.xpPool }, afterFirst);
});
...
test('AC-28 经验溢出保留；升级同步现存棋子并保持生命比例', () => {
  ...
  setPlanning(state);
  state.factions.red.xpPool = amount;
  state = applyOk(state, { type: 'ALLOCATE_XP', factionId: 'red', cardId: 14, amount });
  assert.equal(state.factions.red.cards[14].level, 3);
  assert.equal(state.factions.red.cards[14].xpIntoLevel, 10);
  ...
  assert.ok(synced);
  const newStats = getRuntimeStats(14, state.factions.red.cards[14]);
...
test('AC-29 普通卡必须按基础训练→强化药剂→超级血清顺序购买', () => {
  ...
  setPlanning(state);
  state.factions.red.cards[14].level = 50;
  state.factions.red.gold = 1000;
  ...
  assert.deepEqual(state.factions.red.cards[14].purchasedPromotions, ['基础训练']);
  state.factions.red.cards[14].promotedThisSettlement = false;
...
test('AC-30 精锐卡从源序列首项起步，不白送被省略阶段属性', () => {
  ...
  setPlanning(elite);
  elite.factions.red.cards[82].level = 50;
  elite.factions.red.gold = 1000;
  const beforeElite = getRuntimeStats(82, elite.factions.red.cards[82]);
  elite = applyOk(elite, { type: 'PURCHASE_PROMOTION', factionId: 'red', cardId: 82, promotionId: '强化药剂' });
  const afterElite = getRuntimeStats(82, elite.factions.red.cards[82]);
  ...
  assert.deepEqual(elite.factions.red.cards[82].purchasedPromotions, ['强化药剂']);

  let boss = makeState('ac-30-boss');
  setPlanning(boss);
  boss.factions.red.cards[85].level = 50;
  boss.factions.red.gold = 1000;
  const beforeBoss = getRuntimeStats(85, boss.factions.red.cards[85]);
  boss = applyOk(boss, { type: 'PURCHASE_PROMOTION', factionId: 'red', cardId: 85, promotionId: '超级血清' });
  const afterBoss = getRuntimeStats(85, boss.factions.red.cards[85]);
  assert.equal(afterBoss.maxHp - beforeBoss.maxHp, PROMOTIONS.超级血清.hp);
  assert.deepEqual(boss.factions.red.cards[85].purchasedPromotions, ['超级血清']);
});
...
test('AC-35 难度只改变 AI 聚合后向下取整的金币，不改变属性、AP、经验或决策算法', () => {
  ...
  assert.deepEqual([easy.factions.blue.gold, normal.factions.blue.gold, hard.factions.blue.gold, extreme.factions.blue.gold], [16, 20, 25, 30]);
  ...
  assert.deepEqual(
    { hp: normalBlue.maxHp, ap: normal.factions.blue.actionPoints, card: normal.factions.blue.cards[normalBlue.cardId] },
    { hp: hardBlue.maxHp, ap: hard.factions.blue.actionPoints, card: hard.factions.blue.cards[hardBlue.cardId] },
  );
  ...
  easy.map.nodes['South-Depot'].ownerFactionId = 'blue';
  easy.map.nodes['South-Depot'].activeFromRound = 1;
  const beforeIncome = easy.factions.blue.gold;
  runSettlementAutoInPlace(easy);
  assert.equal(easy.factions.blue.gold - beforeIncome, 15);

  hard.factions.blue.gold = normal.factions.blue.gold;
...
test('AI-VALIDATOR AI 行动与规划命令均由玩家同款 validator 接受', () => {
  ...
  const faction = initial.activeFactionId;
  assert.ok(faction);
  const run = runAiActionPhase(initial, faction);
  ...
  planning.factions.red.gold = 200;
  planning.factions.red.xpPool = 10_000;
...
test('AI-TIER-UPGRADE 核心职能齐全后为已解锁 T2 留预算，并在可支付时生产重装', () => {
  ...
  setPlanning(saving);
  saving.factions.red.cards[15].level = 10;
  saving.factions.red.cards[15].purchasedPromotions = ['基础训练'];
  saving.factions.red.gold = 59;
  ...
  setPlanning(buying);
  buying.factions.red.cards[15].level = 10;
  buying.factions.red.cards[15].purchasedPromotions = ['基础训练'];
  buying.factions.red.gold = 60;
