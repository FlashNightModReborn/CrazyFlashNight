...
**最后核对代码基线**：release source commit `0b71d91987bee27f399bedc90a2c648a8bcbf44e`（2026-09-03；tag `runtime-build-v2/20260903-reward-root-o1-v1`、tree `68ae915ad90ab96dc31c6371b37a1469be11a934`、request `A0DA1D0F5E3E91EF9A53BC8D81419E5917016B860EB49AC655DA4D83EDAF7367`、deployment `95ea7e8f7c862ea28f6b8c7c906173a701d71336`）。
本地 X509 `builder-local-c` / `physical-host-c`（keyId `CFB70E2D339ACB25E9B6C2873DF4F1AEEBA8EA75AD23B825724B27FCA70C0B86`）与 GitHub OIDC/Sigstore builder `A714D2836BD6EE928FD163BA46032555665E57F99D27DEAE151237CE0AD25CEA` / `github-hosted-windows`（cloud run `33701115609`）已对 identity `1E09139852A981BBDA4A99E65F4FCF5BDBDD71E44DDE39EE13FC5B71ABF29709`、closure `81DE32947F126EEC968280C4771C427140A82268CA042F91BE945F60A441F76B` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `AE0E56CFCA82E12DE54845636D36A3F2D5E9D9EFE94A7BFC30228B21E00F8E8B`，40/40 production receipt SHA-256 为 `BC6467ED923BE45E3341545A4DF35BD558B1F379A253E01DCE8D34E6A6A5C6C5`，manifest / consensus SHA-256 分别为 `BEE0507B2F55C04039A38C086998A2A74043EF9003AA22C66B29C26CE33E750C` / `86124784FE893AA474A12BB1DB2B9770C3BA6DF8365779C42C3057FD0B8C52E2`。
正式 runtime 已于 `2026-09-03T00:56:05.591629Z` 原子 promotion，并通过 signed consensus、GitHub proof replay 与正式根 bootstrap `--verify-only`；deployment 已推送，首次 post-promotion Audit run `33702253144` 通过并精确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：W/B0 窗口生命周期根治保持 `HUMAN_ACCEPTANCE_PASSED / promoted`，部署后未重跑 W 正式业务旅程，故不称 W 专项 `standard_entry_verified`。A/A1 Reward 根事务与 S/O1 观测已完成自动门和修复后 exact candidate 人工复验：H-A 覆盖 partial、跨重启、主动重开、整理后全领取，H-S 取得 13 个同实例样本并合法落到 `no_outstanding_operation`；第二列车随后独立取得本地 X509 + GitHub Hosted 双故障域共识、原子 promotion、部署推送与 post-promotion Audit，当前准确状态为 `HUMAN_ACCEPTANCE_PASSED / promoted / FORMAL_BUSINESS_REVALIDATION_PENDING`，不称 A/S 专项 `standard_entry_verified`。完整状态见 [止血治理 ADR](docs/AS2-WebPanel止血治理-窗口生命周期与Reward根事务-ADR-2026-09-01.md)。
