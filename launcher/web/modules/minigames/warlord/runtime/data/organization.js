...
export const DEMO_1_ORGANIZATION = validation.definition;
export function unitTemplateMetrics(cardId) {
    const definition = DEMO_1_ORGANIZATION.unitTemplates.find((entry) => entry.cardRef === cardId);
    if (!definition)
        throw new Error(`Missing organization unit template for card ${cardId}.`);
    return definition;
}
export function formationProfile(profileId) {
    const definition = DEMO_1_ORGANIZATION.formationProfiles.find((entry) => entry.id === profileId);
    if (!definition)
        throw new Error(`Unknown formation profile ${profileId}.`);
    return definition;
}
export function taskGroupTemplate(templateId) {
    const definition = DEMO_1_ORGANIZATION.taskGroupTemplates.find((entry) => entry.id === templateId);
    if (!definition)
        throw new Error(`Unknown task-group template ${templateId}.`);
    return definition;
}
