...
**2026-09-13 正式部署增量**（release source `e04f86be7b258d1eb4995c03a9216ea3ba0fc155`）：现场对白迁入 NativeHud。`native_dialogue` v1 以 requestId `nd:<seq>` / sceneId / revision 做有序快照；`nativeDialogueAction` 仅承载 Host→AS2 的 advance/close 意图，`dialogue_portrait_result` 是唯一允许的 Web 返回。AS2 保持台词顺序、完成与取消权威；关卡对白的可合并暂停租约与窗口暂停责任取逻辑或，普通 NPC 对白不额外暂停。动态人形沿现有 Web 纸娃娃出 768 PNG 后回 Host，静态为两倍尺寸、经过超采样的无损 WebP。面板遮挡与失焦只暂停呈现，全部解除后恢复；断连按 finish 清理自身暂停并消费一次后续事件，跳过未读台词的取舍待体验确认。输入要求真实前台、无面板并受修饰键/长按门控；Web 图像结果不构成剧情指令。当前 `promoted / FIELD_REVALIDATION_PENDING`，三项用户反馈已修、待复验；原版美术由 XFL→SVG 导出复用，按钮分态绘制、外部立绘保作者取景；纸娃娃固定作者窗口，内存与有界磁盘缓存绑定外观、表情及渲染版本；空关卡属性表取玩家权威装备，真实佣兵保持自身；协议与验收边界见[对话框迁移与高清立绘治理](../docs/对话框迁移与高清立绘治理-调研与施工准备-2026-09-12.md)。
...
外观初始装备仍只提交 identifier，但 AS2 snapshot 对每个 allowlist 物品用 `BaseItem.create(identifier,1)` 与 `InventoryPanelService.buildTooltipProjection` 生成严格六字段 `{identifier,name,iconName,itemType,introHTML,descHTML}`；Host 与 Web 独立严格验证。Web 以上装/下装/鞋三个固定槽加当前槽位共享候选池呈现：候选每槽只有 3–6 项，默认放大完整卡片直接给名称与类型；77 项发型默认紧凑方形图标格，两视图密度独立持久化并可各自切换；只复用 `Icons + PanelTooltip` 的图标、富注释与密度语义，不挂载 Inventory/Character Build 的 session、lease、拖放或提交状态机。发型继续消费理发店真实目录并保留 77 个源行、重复 source index 与非随机行为，以单发型槽加首帧缩略图池呈现；紧凑和完整模式都挂载全部 77 项并使用候选池内部自然滚动，完整卡片显示去掉冗余“发型-男式/女式-”前缀后的可辨识名称，原始名称仍保留在 ARIA 与项目注释中。绝对 source index、选中态与 roving focus 不因密度切换或滚动改变。`光头` 可无资源降级，其余缩略图直接读取既有 manifest，不新建 77 个 renderer，也不向玩家显示 raw identifier。脸型字段仍按性别固定并贯穿 snapshot/renderer/submit/AS2 exact 校验，但不显示成可配置项；身高只在左侧预览区编辑。难度全文与装备说明统一由 scoped `PanelTooltip` 展示，hover/focus 均零 RPC；浮层使用同一逻辑坐标系与有界尺寸，不能随宿主缩放二次放大或遮住主要操作区，候选注释锚定到表单列右缘的固定锚条，不随格内位置跳动、不遮挡纸娃娃预览。
...
同日返回失败恢复：新 Host 以 `stageOutcomeSync v2` 协商 `returnFailure` 投影，AS2 默认保留旧 Host 的严格 v1。`prepared` 后的明确失败保留原生“重试返回”，同 run/gameworld 重试复用冻结结算，详见[返回失败恢复](../docs/关卡结果与基地结算-CSharp-Web-ADR-2026-08-27.md#0d-2026-09-08-返回失败后的原生重试入口)。
...
NPC 的价格证明边界必须保持精确：wire 只传安全整数 `buyRatePermille`（千分比，当前合法域 `0..1000`），不传浮点倍率。catalog 行满足 `unitPrice = floor((basePrice × 1) × buyRatePermille / 1000)`；buy/tradePreview 的购买小计满足 `floor((basePrice × quantity) × frozen buyRatePermille / 1000)`，不得改写为 `unitPrice × quantity`。`balance/basePrice/unitPrice/line total/buyTotal/sellTotal` 是非负安全整数；`netDelta/projectedBalance` 是有符号安全整数，余额不足 preview 仍可用负 `projectedBalance + canCommit=false` 表达，只有 commit 后的 `balance` 必须非负。rate、quantity 以及每个乘法、加减中间值都必须处于 JavaScript/AS2 安全整数域，AS2 与 Host 使用同一 checked fixed-point 规则，非法、非整数、非有限值或中间值越界一律 fail closed。该字段替换保留同域 `v=1`，只允许 Host、asLoader、Web 与 harness 进入同一 immutable candidate 原子交付；不提供旧 `buyMultiplier` 双读或滚动混配。口才 Lv2 `17100×940/1000=16074`、Lv10 `2700×700/1000=1890` 是对旧 binary64 少 1 的有意纠偏，不得用兼容名义恢复。余额分别绑定冻结 snapshot 或 preview 的预计余额。catalog proof 只证明买入目录；出售 source 继续由 lease-bound Inventory 引用和 AS2 扫描裁决，batch/trade commit 使用各自冻结 preview，不得拿买入 catalog 冒充出售身份权威。任何复用 NPC catalog 的 AS2 consumer（包括材料详情与两种跨面板商店授权）都必须从同一个 service 读取当前 `getBuyRatePermille()`，验证后显式传给 `buildCatalog(shopId,buyRatePermille)`；不得硬编码 `1000`、把第二参数改回可选，或调用带 collection snapshot/lease 副作用的 `buildState` 代取目录。consumer 测试至少保留一条真实 NPC leaf 组合缝，宽松一参数 fake 不得作为通过依据。
...
战利品箱（`LootContainerService`）的 `slotLease` 自 2026-09-03 起与上述 inventory OCC 规则分开治理：它只绑定槽位内容身份（物品引用 + 签名指纹），不再绑定容器级 `mutationRevision`；同箱兄弟格领取后，未动格租约在后续 snapshot 中保持字面值稳定，这是 Web 批量部分成功证明（`loot-state.js` `_claimBatchProjection` 要求幸存格租约等于领取前冻结值）可证的前提。容器级并发失鲜由 `expectedContainerVersion`（对照 `getMutationRevision()`）与 `expectedAuthorityRevision` 各自承担，租约不再重复承担；被领取格内容变化仍轮换其租约，生命周期围栏（开启/挂起/恢复/终态/transport detach）仍全量失效全部租约。装备箱物化（`LootMaterializationPlanner`）与关卡结算奖励物化（`StageRunSession.materializeRewards`）同时接入情报持有上限分流（`ItemUtil.planInformationAcquire`，与敌人地面掉落同一语义）：已达 `maxvalue` 的零价剧情情报不生成、槽位留空，部分剩余容量按剩余量截断，非情报规则不受影响；结算侧 `remainingRewards` 只按实际生成物品计数，被截掉的掷骰计入 `rewardRollOmissions`。
...
2026-08-29 起，Equipment Tuning 把“调制后最终态”作为正常玩法写入不变量：背包装备仍可提前进阶并保留高于玩家等级的物品；`sourceKind:"loadout"` 的任一非 no-op preview/commit 必须用传入的 after value 重算完整有效数据，并要求其有限有效等级不高于 `_root.等级`，否则返回 `level_locked` 且装备、材料、revision 均零写。所有调制写还必须满足 after 的有效 `modslot` 为有限非负整数且已装配件数不超容量；`replace_mod` 必须先拆目标及冻结的一跳依赖，再用拆件后 probe 的有效数据校验候选，after projection 也必须从 after value 重算，不能沿用替换前容量。该门只约束新的调制写，不改变 SaveManager/Character Build 的存档加载、迁移或自动卸装行为；既有手改/测试污染存档原样保留，但不能再经正常调制流程制造新的超等级或超槽已穿戴状态。
...
NpcShop、Crafting、Hairdresser、Settings、Pet 与 Merc 的 Host transport lifecycle 统一组合内部 sealed `PanelPendingCallTracker<TContext>`。helper 只拥有 backend/Web callId correlation、pending/Timer、active/recent 去重、readiness/send、timeout、迟到/重复抑制和 clear/dispose drain；`TContext` 对它完全 opaque，command、payload、read/write、token、业务 verdict、`writeState` 与何种读可解除 reconcile 全部留在领域 Task。response、timeout、send-false、clear 与 dispose 都通过同一个原子 take 竞争唯一终态，callback 在 helper 锁外执行；`XmlSocketServer.TrySend(false)` 内部保守记为 `DeliveryUnknown`，各 Task 再映射自己的现役错误。Pet/Merc 另把同一个 active Team `panelInstanceId` 存入 pending，Host 路由必须匹配 exact owner，Web 只采用同时匹配当前 instance、pending callId/cmd/session 的 response；replacement 会先 clear 旧 pending，迟到或错命令响应不得写入新实例，缺少外层 `TeamPanelHost` 的旧 child 文档也不得本地 close。Hairdresser 对未知 commit 保留 `needs_reconcile`；Settings 的非 preview 写 timeout、投递未知或明示 ambiguous 回包均返回/锁存 `requiresReconcile=true`，Host 对锁存前发出的迟到 snapshot 继续明示 `requiresReconcile`，Web 只采用跨过当前 epoch 的 fresh snapshot，绝不自动重放或显示确定未执行。`ClearPending()` / shutdown dispose 不伪造 Web 回包；dispose 是 shutdown-only 终态，返回后拒绝新的请求入队，六个 Task 都在三条进程退出路径显式 dispose。Web 继续使用独立的 `PanelRequestMux` 做 browser generation/session 隔离；Settings 的生产 exact close observer 会 clear pending 并另发 `settingsPanelClosed` 恢复试听，断线遗漏则由 AS2 30 秒 expiry 兜底。Skill/Loot 的 execution lease 与恢复机制不接入本 helper。
...
`candidates` 请求现役 exact payload 必须携 `candidateScope:"compatible"|"backpack"`，响应按同一请求精确回显；缺失、非法或错回显都拒绝整份结果。selector 只允许三种封闭形状：exact `slotKey`、exact `drugSlot`，或两者都省略且 `candidateScope="backpack"`。第三种回显 exact `target:{kind:"backpack"}`，是 fresh open/未选槽时的无目标背包总览；无 selector 的 `compatible`、双 selector 或多余键均 fail closed。`compatible` 保持已选槽的精确候选语义；`backpack` 返回全部合法占用背包行。每个装备候选无论来自 `compatible` 还是 `backpack` 都必须携权威 `equipmentEligibility:{slots:[...],blockedReason:""|"level_locked"}`：`slots` 只能是 canonical 装备槽的有序子集，AS2 按实例有效 `use/level` 生成；Host 在两种 scope 下都独立重算槽位集合、顺序和阻断原因。无目标总览中，可装备行按 eligibility 启用，正有限数量的 `use=药剂` 行启用，其他合法行以 `disabled=true / blockedReason="incompatible_item"` 保留检视；未知/空 `use` 但实例 value 仍呈装备形状的损坏行必须进 diagnostics 并跳过。

Web 的乐观通道严格限定为**权威盖章后的只读重投影**：只保留一个最近成功条目；无目标 `target=backpack`、已选装备槽的 `compatible/backpack` 都按 `equipmentEligibility` 派生装备拖拽落点，合法药剂行则可投递到全部 8 个物理药剂槽，目标槽通过 `lane=slot%4` 共享冷却，仍由 Host/AS2 在写前最终裁决。浏览 scope 只决定候选集合，已选槽只作为 `compatible` 的筛选锚点和主 CTA 目标；拖拽落点由当前候选的权威白名单独立裁决，不能因处于 `compatible` 就钉死在锚点。只有玩家把行拖到一个高亮槽后才组成 exact target mutation；写成功、确定失败或对账收束都不得把实际 drop target 反写成新的浏览锚点，也不得把无目标背包总览自动切回 `compatible`。只有显式槽点击或 scope 切换能改变浏览上下文。不新增“选物品后点任意槽即穿戴”的快速模式，也不按物品自动猜槽。`compatible` 仍只把 exact `手枪/手枪2` 规范为 `equipment:手枪`，药剂因冷却随时间变化而不扩张成多 key LRU。cache key 必须绑定 exact `panelInstanceId`、session generation、loadout/drug revision、candidate scope 与 target family。新的 candidates/snapshot authority request 在发送前即重置缓存；revision/generation/实例变化、mutation/write、flush/finalize、调制切入、suspend/rebind/close、degraded/失败也都会清除条目。缓存命中不发送新 `candidates`、不生成新 lease，也绝不乐观写入。最终写仍复用现役 `equipEquipment/equipDrug`，携 exact drop target、原 source lease 与 revision，并由 Host/AS2 复验 effective data。
...
角色构筑的无目标背包总览同时是物品使用的唯一入口；战备箱/仓库 organizer 与装备/药剂兼容候选不出现使用动作。AS2 只在该总览行附加 `useAction:{command:"open"|"consume",label:"打开"|"服用",source:{physicalSlot,slotLease,itemName,backpackVersion}}` 和同级 `useBlockedReason`，不改写既有装备 source `{containerId:"背包",slot,expectedLease}`。Web 只允许显式按钮或选中行后按 Enter 使用，不把双击装备交互解释成使用。药剂直用按 0→3 选最低 ready lane：先找上下两个物理药剂格都为空的 lane；不存在时再找任一格已装备同名药剂的 ready lane；仍不存在则 `no_available_lane`。同名药剂占多个 lane 的并发冷却是显式玩法，暂停态允许使用，Host/Web 不推导 lane。

物品使用使用严格 `panel=workbench,domain=item_use` envelope，命令仅为 `open/consume/query/inboxSnapshot/cooldownSnapshot`。`open/consume` 绑定 `operationId`、Character session、背包物理槽/lease/物品名/version；写超时或投递未知后只允许同 `operationId` 的 `query`，绝不重放。`query` 的窄成功只返回 `found`、已提交 receipt 与 `inboxSummary`，不携 Loot authority；确认已提交后 Web 必须再显式读取 `inboxSnapshot`。`cooldownSnapshot` 是零 `operationId` 的只读命令，只接受四条按 lane 排序的 AS2 `ManualCooldownService` 帧权威；Web 只把样本投影为两组对应药剂格内的阴影并短轮询活动通道，不新增独立冷却行，不得用浏览器现实时间续算或把动画提升为裁决。礼包结果和读档恢复项进入 `_saveExt` 中持久、可追加、不可变批次的 `RewardInbox`（总容量 64 occurrences）；它与地图/关卡当前进程 Loot singleton 分离，但领取时复用顶层 Loot UI，并以 `sourceKind=reward_inbox` 的 exact authority 物化。Character Build 为角标预物化但尚未服务 Loot snapshot 的 authority，在后续礼包追加令 durable `authorityRevision` 变化时必须失效并重建；已经服务 snapshot 的面板身份保持稳定，新增批次留到关闭后再物化。

Reward Inbox v1 的单领与批领共用 durable root journal：wire `operationId` 即 `rootOperationId`，首个 child 副作用前必须持久化 `activeClaimRoot` 的有序 entry、cursor、applied count 与可重绑定 descriptor；terminal 后只保留一个 `claimRootTerminal`。重启 discovery 通过 `inboxSummary/rewardAuthority` 的 `recoverableRootOperationId/recoverableRootStatus/recoveryRequired/recoveryOnly` 建立 authority，即使 remaining 为零也能 exact query；`lootQuery` 必须显式携 root ID，响应以 `rootStatus/resultKind/result/appliedCount` 裁决并可幂等 acknowledgement。capacity terminal 的 blocked entry ID 集必须与 remaining ID 集精确相等，错误只允许容量类；已提交 child 造成 Reward authority 重投影时 retained slot lease 可以权威轮换，Host/Web 仍按 exact root tuple、applied/remaining delta 与物理槽占用证明结果，并保留 exact 容量原因和“整理背包”恢复动作。Host/Web 对 `reward_inbox` 禁止仅用 projection delta、freshness 或 write retry 猜未知结果。v1 nested future/malformed root 原样保留并只 quarantine Reward lane，不升级为整档拒绝。精确 `CF7_REWARD_ROOT_ADMISSION=0` 只关闭 Host/Web 新 root admission；完整兼容回滚还必须配套经验证的 AS2 admission-disabled SWF，既有 discovery/query/quarantine 继续可用。

Character Build → Reward Inbox 是 Host-owned 受控交接。只有 Web 完成 Character finalize 后发送 exact close reason `navigate_reward_inbox`，Host 才可锁存最近一次完整 `inboxSnapshot/open` authority；普通 close、只刷新 inbox、foreign/stale instance 均不得开 Loot。Host 必须等 Character binding、visual 与 pause lease 全部释放，再由 `LootPanelCoordinator` 打开同一份缓存 authority；Web 不直接调用 `Panels.open`，也不把持久 inbox 降格为地图 Loot 的恢复分支。Reward Inbox 领取完毕或玩家主动关闭时不使用 PanelHost `returnTo`、不复活已退休 Character session；仅 `sourceKind=reward_inbox` 的 strict terminal/suspended proof 可先锁存 exact replacement，再经现役装备入口的 nonce-bound AS2 preflight 请求一份全新的 Character Build panel/session。回包 tuple、nonce 与 Loot baseline 全匹配时，`PanelHost.TryReplacePanelExact` 在同一 Web/native surface 内提交目标并消费匹配的 tracked lease，`loot-panel` 在此期间保留 DOM，旧 Loot coordinator 只 detach、不释放共享 pause；任一步失败或超时才撤销 replacement、走原 exact close/unpause，并由 `BindingSettled` 从 idle 基线单次重试。shutdown、竞争面板、重复 callback 均 fail-closed。
...
36 键表以逻辑 ID 和默认顺序为权威，在四个快捷药剂键前固定增加“药剂组切换键”，默认为 `6` / keycode `54`。携键表的 snapshot/apply 保持 settings 外层 v1，但必须精确携 `keySchemaVersion:2`。旧 33/35 键存档按 ID 保留已改值并追加缺项；若 `54` 已被旧动作占用，必须保留原绑定，按冻结 fallback 顺序给新动作选择空闲合法键并向 Web 投影一次迁移提示；不得以重复键进入运行时双路由。非法物理键回落默认，无关历史重复键或保留键仍允许先投影到 Web 供逐项修复，但 apply 必须全表唯一、拒绝 Esc/F1–F12 且不自动交换。显示标签不是身份：AS2 遇到未就绪翻译、空值或字面 `undefined/null` 必须回退逻辑 ID，Host 对成功 snapshot 中的占位标签 fail-closed，Web 仍以逻辑 ID 作最后显示兜底。键位页在 `1024×576` 基准画布上采用左右两列分区板和组内 2/3 列紧凑网格，36 项必须同屏可达且不依赖内容区纵向滚动；密度来自缩短标题留白、行距和“文字末端到按键控件”的空隙，不得再靠不可读小字换空间。当前基准门要求键名与按键值均至少 `12 CSS px`，逐行实测文字—控件最大间距不超过 `9px`。`KeyManager` 的订阅身份是逻辑键名，刷新只重建逻辑名到当前物理键的投影；KeyDown/Up、长按、双击、重复与组合键均须在改键后继续跟随，读档后必须调用刷新。性能等级协议只接受 `0|1`；历史 `2|3` 在 snapshot 时归一为 `1` 并置 migration latch，只有 durable save 才清除。

游戏设置和键位使用草稿、显式“应用并保存”。apply 携 `expectedRevision + keySchemaVersion:2`，AS2 重新校验 14 项设置与 36 键，先应用运行态再 `flushNow()`；应用成功但落盘失败返回 `applied=true/durable=false` 并只允许幂等重试保存。开始应用后的异常返回 `apply_ambiguous + requiresReconcile`，Web 只拉 fresh snapshot，不重放写。音量滑条即时 preview，另有现役界面音效试听；恢复租约必须先于任一音量 setter 挂起，若第二个 setter 或试听抛错则立即尝试双项回滚，回滚仍失败时保留首次基线并重挂 30 秒租约，供 timer、cancel 或 exact close 重试，不能留下无恢复机会的半应用音量。Host 偏好沿用 `UserPrefs` 并逐项即时保存/失败回滚；Web 偏好只聚合各消费者既有 localStorage key，不新造第二份配置，长驻模块在下一次 open 重读。点歌器的三项 Flash 权威规则与现有 Web 点歌器主题统一归入“本机与 Web”页；前者仍随底部 apply/save，后者仍沿用原 localStorage key。

打击伤害数字是独立的机器全局 Host 偏好，不进入上述 14 项 AS2 snapshot/save。Web “本机与 Web”页直接配置 `hitNumberMode=off|balanced|total|classic|detail` 与 `hitNumberWorldRowLimit=0..1000`，逐项经 Host 白名单即时落盘；空数字输入非法，非零值限制全局攻击行，有限 `detail` 还对每目标只保留最新六行，`0` 才明确解除两层产品上限并保持真正无限制。Host 保存失败必须回滚内存值与控件。配置成功后 `FrameTask` 立即用新 generation 重排，Host 只向 AS2 发布 `H0/H1` 来源开关；AS2 不知道行为模式或世界行上限。旧 `_root.是否打击数字特效`、`_root.同屏打击数字特效上限`、Flash 随机散开/对象池 renderer 与 V8 HitNumber reducer 均已退役，不设 fallback；`classic` 是 C# 对旧 Flash 散射/14 帧动效的兼容复刻，不恢复第二套 renderer。有限上限只裁剪世界攻击行，所有模式仍保留舞台外 100px 余量剔除和自然寿命；`detail` 按每发攻击 Burst 原子展示逐段且有限档不拆攻击，布局按实际数字和属性标签边界扩格/缩列，不截断逐段事实；`balanced` 以目标锚定摘要保留同目标最近三次 Burst，并复用总伤的最新段来源色→Burst 贡献主体色、属性来源色、固定效果色、emoji、贡献比例与精确吸血/护盾；四模式共用一份 11 色语义，逐段项不做聚合色插值，贡献可见度与模式标签密度分离。`total` 按目标维护 1.25 秒静默窗口，保留旧总伤的颜色过渡、按贡献状态、精确吸血/护盾、渐进总伤/hit、MISS 忽略、淡出续击恢复与最新目标置顶语义，并补齐伤害类型/粉碎独立文字槽和非 MISS 零伤来源色；旧二段冲击脉冲按平衡字号封顶。balanced/detail 也按最新受击目标最后绘制。精确历史与世界表现分离：不得占用 `Alt` 或任何战斗键，暂停态设置面板通过 Host-local `hit_number_ledger` 分页读取固定 32,768 段环形账本；Host 只在请求时物化 JSON，并显式报告溢出与最老 Burst 可能不完整。`off` 发布 `H0` 后不再接收新段，既有账本保留到 reset 或容量覆盖。
...
强制控制只保留“尝试复活”和“返回基地”，并置于默认“游戏”页首屏：前者单击后委托 `StageRunSession.requestReviveLocal` 的共享扣币与 exact-hero respawn 事务，不直接改 HP、不重开旧 Flash 复活弹窗、不绕过 `DisableResurrection`；后者单击后立即调用现役返回基地流程并关闭面板。两条 wire payload 都是 exact `{v:1}`，不接受旧 `confirmed` 字段；作弊执行仍保留独立二次确认。安全退出已有其他入口接管，不在这里重复。
...
- blackmarket O1 观测只在 `CF7_BLACKMARKET_SOFTLOCK_OBSERVATION=1` 时启用。Web 以 exact panel/document/sequence 每 10 秒发送只读 heartbeat；Host 再绑定当前 panel instance、Host document generation 与 socket generation，按生命周期最多记录 64 条，并用独立 observation-only call 获取 AS2 `businessOwner/pauseOwner/sceneOwner/frameSequence`。close/rebind/pagehide/socket retirement 会清理探针；该通道不得增加 retry、deadline、watchdog、自动 close、状态写或修复行为。
