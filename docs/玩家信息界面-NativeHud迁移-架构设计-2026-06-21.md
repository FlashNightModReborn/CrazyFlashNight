...
**最后核对代码基线**：main-space v2 实现由 merge commit `40853287e7ed04714d68935c0002f8ad6d8aea05` 纳入主线，evidence/docs 后继为 `884fbf436f21c5e27ed3370c445cbae941349496`；qualification 闭包修复后的最终 F2 为 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd`，不改变 PlayerInfo runtime 源码、SVG 或视觉输入。B0-05 32/32、B0-06 48/48、Launcher 1,747 pass + 3 opt-in skip / 1,750、visual-evidence.v2 与 Kimi K1 均已闭合；F2/r2 也已完成双 builder、双 faultDomain、两侧 23/23 policy 与不可复用的 `-VerifyOnly` preflight。严格总体状态为 `awaiting_human_acceptance; human_exit_blocked`；仍仅 `candidate_built / source-ahead / NOT_DEPLOYED`，没有 candidate execution、E2E、promotion、标准入口或部署。真实 `pi_*` 与旧 Flash HUD 均未改变；视觉门以 [专项 ADR](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md) §12 为准，构建精确证据见 [runtime v2 深层文档](runtime-build-reproducibility.md)。

**2026-07-30 build-train 纠错**：首列受保护 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v2` 继续永久指向 `40853287…`；q3 本地 X509 producer/票有效，但 production policy 仅 22/23，唯一失败是隔离 qualification csproj 漏挂 `PlayerInfoRasterPlan.cs`，所以该列车因 policy 门失败退役，未启动 cloud/quorum/preflight。修复后的 F2 由全新 `runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2` 与新 request 完成双 builder、23/23 policy 及 `-VerifyOnly` 闭环；它只关闭机器可复现门，不表示重跑 historical v1 Runtime Lane C，也不执行或授权 promotion/部署。
...
| 5 | C# PlayerInfoWidget 只读 | B0-05 已完成 raster/cache 并因近全屏 union 接受独立 split surface；B0-06 已实现不接业务权威的 HP/MP fixture 纵切与独立 click-through surface，surface 独占 animation model，widget 只读消费接口，Resume 仅在有效布局与 raster request 建立后完成。历史 v1 证据只证明旧字节。当前 main-space v2 已取得 B0-05 32/32、B0-06 48/48、全量 1,747+3/1,750、正式 C#/Web/direct A/B、Kimi K1 与 F2/r2 可复现闭环；严格仍为 `candidate_built / source-ahead / NOT_DEPLOYED`，只待真实 Flash/桌面与人类验收。B0 accepted 后仍按阶段2→3的 state-first 链路接真实 UiData，程序化复刻虚拟帧缓动 |
| 6 | 双轨对比 | 不变；按 §2.4 验收表 |
...
- **建议**：historical v1 记录仍只证明旧字节。当前 main-space v2 已完成 qualification/full、双份 C#/Web/direct/viewer 自动 closure、最终 Kimi `k3/high` 异构审计，以及 F2/r2 双 builder、双 faultDomain、23/23 policy 与不可复用 preflight；仍只到 `candidate_built / source-ahead / NOT_DEPLOYED`。下一步只由人类签收 Flash oracle、可见 DWM/游戏 composite、关键比例/平滑、混合 DPI、运行时 SWF/IDE Output 身份、真实手点与审美。自动包/Kimi 没有 parity verdict，全部通过也只证明视觉基座与自动 closeout；B0 accepted 后再沿阶段2→3建立真实 state/UiData，并做 HP/MP 双轨对比，不能把 fixture 结论外推为运行态接入。
