...
**文档角色**：游戏系统 canonical 索引。**最后核对代码基线**：HEAD / `origin/main` `2d19cd6681a0219749a58501271b6f1bd23cc28f`（2026-07-21 断电恢复后复核）+ 当前尚未晋级的本地 S0/S1/S2 dirty 工作树。
...
- **发布层**：当前 HEAD / `origin/main` 为 `2d19cd6681`，正式 runtime v2 consensus 仍沿用既有 build identity/payload closure；promoted Core 不含本地 S0/S1/S2 Host。源码可以在部署闭包不变时以 `source-ahead` fast-forward 进入 main，但本地资源箱仍没有 immutable Git-tree request、production policy receipt、有效 local X509 proof、不同 signer + `faultDomain` quorum、strict verifier 或 promotion；当前 diagnostic candidate 与历史 candidate 都不是 release evidence。
