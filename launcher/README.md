...
> **最后核对架构与工作流**：source commit `f01f4b121a4ceebd7dae051f14bb511c5ae3f1cb` 已由 immutable tag `runtime-build-v2/20260730-workbench-no-as2-fallback-v1`、release tree `3dfe2dcdb25b26f22a2c36b400524a3b6aca0e28`、request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B`、本地 X509 + GitHub OIDC/Sigstore 双故障域共识完成正式 promotion。exact candidate 与无参正式入口 attempt `8baf52bbcceb452da32da641e58d2922` 都完成 production Equipment Tuning opener → exact workbench instance → 同实例首个权威 snapshot → supported application shutdown 的只读纵切，故 Launcher 生命周期在该明确范围内为 `standard_entry_verified`；GUI 人工验收已通过，但不代表角色构筑/双栏工作台完整业务、写入、普通关闭、持久化或各子功能专项 standard-entry 验收。本文只维护 Launcher 的稳定入口、协议与验证规则；逐次证据见 [本轮 no-AS2-fallback runtime 发布](../docs/evidence/workbench-no-as2-fallback-runtime-release-2026-07-30.md)，通用判定见 [验证矩阵](../agentsDoc/testing-guide.md)。
> **PlayerInfo 当前边界**：F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / r2 自身仍是历史 non-deployment train；其实现字节被当前 `f01f4b121a4c…` release 包含并进入 formal runtime。B0 仍为 `b0_accepted`，oracle 仍为 `oracle_frozen_for_b0`；本轮 formal smoke 没有启用 PlayerInfo fixture、观察真实 `pi_*` 或完成 PlayerInfo-specific standard-entry E2E，不能从总体 runtime 的 `standard_entry_verified` 外推 PlayerInfo 专项验收。历史 identity、可见验收与边界继续见 [runtime v2 深层文档](../docs/runtime-build-reproducibility.md)、[PlayerInfo 工具证据](../tools/player-info-hud/README.md)和 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)。
> **当前退役 cut（2026-07-30）**：旧 AS2 工作台 fallback、Skill 无 token 兼容与主文件 legacy UI 资产可达性已经移除，GUI 人工验收已通过；当前正式 runtime 绑定 build identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465`、payload closure `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF` 与 Core `565C1F9710421E6B6CC5CB6DDA05DE36B8F1B22B3D7A7CA19617F9786C7D8A4B`，严格状态为 `standard_entry_verified`。首轮同身份标准入口 attempt `bfb6bfe515f14af58f9b4096c8c539dd` 因 reveal watchdog 比真实 title receipt 早约 8 ms 而 fail-closed；后续 clean attempt `8baf52bbcceb452da32da641e58d2922` 取得 fresh handoff、真实 title receipt、单次 enter、首个权威 snapshot 与 supported shutdown，失败记录继续保留。
...
> **当前冻结发布证据**：source `f01f4b121a4ceebd7dae051f14bb511c5ae3f1cb` 的正式 runtime 绑定 Core SHA-256 `565C1F9710421E6B6CC5CB6DDA05DE36B8F1B22B3D7A7CA19617F9786C7D8A4B`、build identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465` 与 payload closure `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF`；immutable tag `runtime-build-v2/20260730-workbench-no-as2-fallback-v1`、request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B` 和 GitHub run `30538330223` 形成双故障域共识。无参标准入口 attempt `8baf52bbcceb452da32da641e58d2922` 取得 fresh handoff、真实 title receipt、单次 enter 与同实例首个权威调制 snapshot 后 supported shutdown；未发送 preview/commit，也未验证普通 panel close、持久化或重启回读。
> **CF7 Agent Runtime / Wings F7 历史 source freeze（2026-07-31）**：一期实现与协议源曾冻结在 C1 `dd84230a1d262c6478591cae2d11051b7a8aa7b1`，仍是 `1.0` 预发布合同；ADR 文件名保留首次冻结日 `2026-07-30` 以避免 canonical 路径/链接漂移。documentation-only D1 只引用其父 C1，不能在自身内容中自引用 D1 commit hash。破坏 wire 的变更必须连同 schema、生成物、实现、fixture、harness 与文档原子升级。exact C1 candidate 已通过 production policy **26/26**，只达到 `candidate_built / NOT_DEPLOYED`；当时无前台会话的严格入口按凭据门失败关闭且无 completion evidence，故未达到 `candidate_executed`、`e2e_verified` 或 `promoted`。这组记录只保留为 F7 历史证据。
> **CF7 Agent Runtime / Wings F8 当前冻结与验收（2026-07-31）**：当前 source `53caabc90941826ddacf626f536b0f473adbf049` / tree `5ac63ec05fbbc9b89aa14f7f0b5ab25698f9742d` 的 immutable candidate `c-0f4c92f237ab-98ebd18146-20260731t022411220z-20da007a` 绑定 build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C`、payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0` 与 Core SHA-256 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD`。operator 纯 MCP 操作链在不使用 Codex Computer Use、browser/Chrome、legacy privileged HTTP 或任何 `input.*` method 的前提下完成真实 Launcher / NativeHud / WebOverlay WGC 与 `panel.open` 帮助面板可见性；stdin 收束后由 trusted wrapper 通过同一 Agent Runtime 协议执行受信 shutdown。像素只在内存中校验 SHA-256，不写 PNG、不请求 persistence/export。验收报告为 `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.json`，协议 transcript 为 `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.jsonl`；严格状态是 `e2e_verified / NOT_DEPLOYED`，没有 promotion 或 standard-entry verification，正式 runtime 继续保持上面的既有已 promotion 身份。完整边界见 [一期范围冻结 ADR](../docs/CF7-Agent-Runtime与Wings-Network一期-范围冻结-ADR-2026-07-30.md)。
> **当前正式 runtime 边界（历史已 promotion source）**：审阅/F0 锚点 `c96f4c3d750561022b706c72a4d53050431e627d` 之后的 A2b、A3、B1–B7、C1–C3、E、G1–G5、H1–H6、双宿主浏览器策略、Native 右侧条件槽，以及旧 AS2 工作台 fallback / Skill 无 token 兼容 / main legacy UI 可达边的退役，已包含在 `f01f4b121a4c…` 冻结 tree 与正式 runtime。它不等于上条 F7 C1 source baseline；自动门和总体 promotion 也不等于所有功能都完成该 release identity 下的专项实机验收，正式入口只覆盖上述 Equipment Tuning opener/snapshot 窄纵切。
...
当前 consensus 采用本地 X509 keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5` / `physical-host-b` 与 GitHub OIDC builder identity `7FFC35F8EDAD03FA477FC9D6BDF86C0FCA8A5CCB591F0CEB56577C0B85BDE9E7` / `github-hosted-windows`（run `30538330223`）；tracked registry 仍只含公钥。正式 bootstrap/runtime/manifest/consensus 已由唯一 promotion 入口切换到 request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B`、build identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465` 与 payload closure `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF`；此前材料/商城/战备箱、角色构筑 ↔ Skills、地图资源箱、理发店、商店与单-canary consensus 均只保留为历史记录。一次性 migration fuse 不得复用，后续发布不得降级 v1。
...
### CF7 Agent Runtime / Wings Network（F7 历史 + F8 当前冻结）
...
F8 当前 fresh gates 为 SDK resolver **7/7**（精确 `.NET SDK 10.0.300`）、Launcher **2724 passed + 3 explicit opt-in skipped / 2727 total，0 fail**、Node client **37/37**、TrustedRunner **57/57**、production policy **26/26**；evidence 绑定 source `53caabc90941826ddacf626f536b0f473adbf049`、tree `5ac63ec05fbbc9b89aa14f7f0b5ab25698f9742d`、candidate `c-0f4c92f237ab-98ebd18146-20260731t022411220z-20da007a`、build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C`、payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0`、Core SHA-256 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD`、report `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.json` 与 transcript `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.jsonl`。该纯 MCP operator chain 未调用 Computer Use/browser/legacy privileged HTTP/`input.*`，也未持久化像素；它只把这一个 exact identity 推进到 `e2e_verified / NOT_DEPLOYED`。没有 v2 promotion 与无参标准入口复核，正式 runtime 保持不变。
