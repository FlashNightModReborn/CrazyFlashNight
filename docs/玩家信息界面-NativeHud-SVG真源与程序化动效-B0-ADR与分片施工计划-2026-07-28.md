...
依赖关系：B0-01A、B0-02、B0-03a 在 B0-00 后可并行；B0-01B 等 B0-01A，但可与 B0-02 并行。B0-04 明确依赖 B0-01A + B0-02 后方可开始；其 `canonical_asset_candidate_validated` 结构入口足以启动 B0-03b，也足以在 B0-03b 后启动 B0-05。B0-01B 的 accepted oracle/人工签收只阻断 B0-04 视觉 Exit、B0-06 parity 结论和最终 B0 接受，不阻断 B0-03b/05 的无关结构施工。B0-06 已闭合 MP fill 独立 mask fragment、HP 固定轮廓/旋转渐变、path glyph atlas、独立 surface、正式 qualification 与双份视觉确定性；其最终退出仍依赖先形成 evidence/docs-only containing commit，再由独立最小配置切片冻结后续 source commit/tag 并完成双 builder/quorum，以及 B0-04 人工 oracle Exit、真实游戏 composite/手点、Kimi K1 和最终人类视觉/UI签收。
...
- [ ] 两类正式 builder 可 locked restore，并对同一后续 source-freeze commit/tag 形成一致 build identity / payload closure；该最小配置切片必须显式更新 `runtime-github-builder.v2.json` 的 PlayerInfo 专用 `sourceRef`，重跑 policy/request/admission，并确认 artifact source/build identity 未因纯 policy 变更被误改。最终以同一 production promotion 验证链的 `-VerifyOnly` 报告留证，不执行 promotion/deployment，也不把报告回灌为发布输入。
...
| B0-06 | `qualification_final_tree_passed; automated_visual_ready; human_exit_pending` | 已实现单表驱动的 11-case `PlayerInfoFixtureInput`、surface 独占的 visual state/41-tick HP 式动画、getter-only widget seam、MP 双 fragment、HP 固定轮廓/旋转渐变、Aero/LCD Std path glyph atlas、source-red HP Glow/MP no-Glow、typed effect policy、窄 composition recipe、恒 false hit-test widget、带可靠 resume pending 的独立 click-through `PlayerInfoSplitSurface`、PanelHost companion 与 Program allowlist opt-in；代码未发起旧 Flash HUD 修改/隐藏。clean `bf8dd2c…8479` 的 `runtime-qualification.json` 为 47/47，run `a6aadeb6…57a7b`、647,234 B、SHA `65628682…9CD74`；focused 为 136 pass + 3 opt-in skip / 139，clean official full 为 1,737 pass + 3 opt-in skip / 1,740。`visual-evidence.json` 已准备好冻结 C# 36-file、Web 12-file、direct 66-PNG 及 supplemental 33-PNG 双跑闭包，并机器声明 Web static-only scope；Runtime Lane C 11/11 exit 0、scalar 566，guardrails `scripts=3 / unsafeCandidateCases=3` | evidence/docs-only containing commit、旧 Flash 同屏可见性、B0-04 oracle 人签、真实游戏 composite、真实手点、审美、最终 Kimi K1 PASS 与后续 source-freeze commit/tag 的双 builder/quorum 尚未完成；promotion/标准入口/部署不属于 B0 Exit，预期仍为 `NOT_DEPLOYED`；不声称 `fixture_static_parity` 或 `fixture_full_parity` | 先冻结 evidence/docs-only 后继提交；再以独立最小配置切片冻结 PlayerInfo source commit/tag、完成双 builder/quorum `-VerifyOnly` 与 Kimi 最终增量复审，最后交人工 |
| B0-07 | `k1_revision_applied; final_delta_reaudit_pending; human_review_pending` | §9.5 已完成文档建设 k3/max 审计；§9.6 已完成最终实现包完整审计，3 项 Kimi P1 与 2 项独立复核问题已修复于 `bf8dd2c…8479`；Computer Use 已恢复并确认 Flash 当前打开 `TestLoader.xfl` | K1 最终增量 PASS、全 Exit Gate、运行时 SWF 身份复核、游戏 composite/真实手点/审美与人类接受；自动桌面控制仍不能代签 | 提交当前 evidence/docs-only 候选；随后冻结最终 source commit/tag、取得双 builder/quorum，再让 k3/max 覆盖最终差量与结构化证据，最后人工终审 |
...
> Oracle 轨保留 B0-01B 第七轮 candidate，等待人类签署来源、状态、层隔离、crop 与审美 review receipt；不得用 FFDec/Web/Flash-Web 指标代签。工程轨的 clean `bf8dd2c…8479` 已通过 47/47 formal qualification，并以 `runtime-qualification.json` / `visual-evidence.json` 准备冻结 C#/Web/direct/supplemental A/B；fixture state/widget、MP fragment、HP 固定轮廓/旋转渐变、path glyph atlas 与独立 click-through surface 不注册进现有 NativeHud union，代码也不隐藏/修改旧 Flash HUD。主工作树已在 `8c703c…4958` 纳入 `origin/main@c96f4c…627d`，ahead/behind=`21/0`。下一步先提交当前 evidence/docs-only 候选；再以独立最小配置切片把 `runtime-github-builder.v2.json` 更新到 `runtime-build-v2/20260729-player-info-nativehud-b0-v1`，重跑 policy/request/admission、确认 artifact source/build identity 未因纯 policy 变更漂移，并冻结最终 source commit/tag。两个正式 builder 对该同一源形成 quorum `-VerifyOnly` 报告后，再让 Kimi `k3 + max` 覆盖最终差量、处置和结构化证据到明确 PASS；最后利用已恢复的 Computer Use 辅助检查 TestLoader/运行器，但旧 HUD 同屏、游戏 composite、手点与审美仍由人类签收。B0 不要求 promotion、标准入口或部署；双 builder、一致 K1 与人签未完成前 Exit 仍不勾，任何自动结果也不得冒称 parity 或代替人签。每完成一片都在同提交回写 §12。
