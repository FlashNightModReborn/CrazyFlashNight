...
C# WinForms 守护进程，承担游戏启动全链：WebView2 / Steam / Flash trust 三道 fail-closed 启动门槛 → GuardianForm 单窗体（承载 BootstrapPanel 引导页）→ GameLaunchFlow 状态机（含 `ready` 触发的 silent prewarm）→ Flash Player SA 嵌入 + V8 脚本总线 + HTTP / XMLSocket 通信 + 启动前存档决议（Protocol 2）。

> **新接手阅读顺序**：本节 → [架构概览](#架构概览)（启动时序 + 运行态面板栈）→ [Bootstrap 前端与协议](#bootstrap-前端与协议)（cmd 表 + reveal gate + config_set）→ [存档权威迁移 (Protocol 2)](#存档权威迁移-protocol-2)。其他章节（音频 / 性能调度 / GPU / UI 迁移 / 面板系统）基本保持原样。
...
Program.Run(args)
   ...
   │   └─ Steam 进程 + SteamAPI AppID → 未正版则拒启
   │      reason: steam_not_running / not_owned / dll_missing / dll_load_failed / ...
   │
   ...
   ├─ UserPrefs 加载（%LOCALAPPDATA%/CF7FlashNight/launcher_user_prefs.json）
   ├─ PortAllocator → XmlSocketServer.Start + HttpApiServer.Start
   ├─ TaskRegistry.RegisterAll(router, ...)
   ├─ hotkey_guard.exe 子进程拉起 (可选, 不存在则跳过)
   │
   ├─ GuardianForm 构造（单 Form，子栈：_logBar + _flashPanel + BootstrapPanel）
   │   ├─ 正常模式：bootstrapWebDir ≠ null → BootstrapPanel 创建，加载 bootstrap.html
   │   │   _flashPanel 初始 Visible=false（BootstrapPanel 覆盖在上方）
   │   └─ bus-only 模式：BootstrapPanel 不创建，_flashPanel 直接 Visible=true
   │
   ├─ BootstrapPanel 的 WebView2 就绪 → JS 发 {cmd:'ready'}
   │   └─ LifecycleCommandHandler.HandleReady → GameLaunchFlow.Prewarm()
   │      └─ 冷启动 silent prewarm：Idle → Spawning（后台静默拉起 Flash）
   │         session-level latch（_prewarmTriggered）只跑一次
   │         45 s deadline 未消费 → 主动 Reset 回 Idle
   │
   └─ GameLaunchFlow 状态机（两路进入）：
        ┌──────────────────────────────────────────────────────┐
        │  路径 A: silent prewarm (ready 触发)                  │
        │    Idle → Spawning → WaitingConnect →                │
        │    WaitingHandshake → Embedding →                    │
        │    WaitingGameReady → Ready（silent；不 reveal）      │
        │    停在 Ready 等用户选槽，bootstrap_handshake 其时发出 │
        │                                                      │
        │  路径 B: 用户点击确认（start_game / rebuild）          │
        │    prewarm 已 Ready  → 直通 reveal 流程               │
        │    prewarm 未开始/被 Reset → Idle → Spawning → … 同 A │
        └──────────────────────────────────────────────────────┘

        Ready 抵达后 reveal gate（两个独立 wait flag）：
          ├─ deferReveal:true      → 等 JS 发 {cmd:'reveal_ok'}（片头播完/跳过）
          ├─ requireFlashReveal:true → 等 Flash 发 bootstrap_reveal_ready（封面帧 81）
          └─ 两 flag 清空 → DoPerformReveal():
                ① readyWiring（toast/web/inputShield/notch.SetReady）
                ② BootstrapPanel.SetPanelVisible(false)
                ③ FlashHostPanel.Visible = true  ← panel swap
                ④ hotkey_guard.exe 起
                ⑤ 同时 PostToWeb {cmd:'flash_ready'}（跳过按钮改就绪样式）
```
...
按实际 `ls` 重生成（2026-04-19）。只列追踪目录；`bin/` / `packages/` / `target/` / `node_modules/` / `obj/` 等构建产物和缓存均由 .gitignore 管理。
...
│
├── src/
│   ├── Program.cs                         入口：WebView2/Steam/Trust 三门槛 → 总线 → GuardianForm 构造 → GameLaunchFlow
│   │
│   ├── Config/
│   │   ├── AppConfig.cs                   config.toml 解析（flashPlayerPath/swfPath/gpuSharpening/sharpness）
│   │   ├── UserPrefs.cs                   用户级偏好持久化（lastPlayedSlot/introEnabled/sfxEnabled/ambientEnabled/uiFontScale）
...
│   │   ├── BootstrapPanel.cs              启动引导面板（WebView2 Control，承载 bootstrap.html；reveal 时 SetPanelVisible 隐藏）
...
│   ├── SanityTests.cs                     基建冒烟
│   ├── Save/                              SaveMigratorTests / SolResolverTests（fixture 驱动）
│   ├── Bus/                               MessageRouterTests（锁定当前观察行为）
│   └── Fixtures/SaveMigrator/             ~60 个 .json（对齐 AS2 SaveManagerTest.as 覆盖面）
│
├── docs/
...
| 子目录 | 覆盖面 | 规模 |
|--------|--------|------|
| `Save/SaveMigratorTests.cs` | undefined→2.6→2.7→3.0 迁移链 + MergeTopLevelKeys + ValidateResolvedSnapshot 28 项结构校验 | ~60 用例（镜像 AS2 `SaveManagerTest.as`） |
| `Save/SolResolverTests.cs` | 决议矩阵 16 分支（tombstone / Rust parse 失败 / v3.0 / v2.7 迁移 / pre-2.7 严格 vs 宽松时间戳比较）+ ISolParser / IArchiveTaskForResolver 窄接口注入 | ~12 用例 |
| `Bus/MessageRouterTests.cs` | 协议分发 + callId wrap + error 构造，**锁定当前观察行为**（异常冒泡 / respond 无去重） | ~12 用例 |
| `SanityTests.cs` | 基建冒烟 | 1 |
| `Fixtures/SaveMigrator/` | JSON fixture（UTF-8 无 BOM，与 AS2 源码 UTF-8 BOM 相反） | ~60 |
...
Steam + Trust 两步组成反盗版链的 launcher 侧兜底。
...
路径：`%LOCALAPPDATA%\CF7FlashNight\launcher_user_prefs.json`。
首次启动会从项目根 `launcher_user_prefs.json` 一次性迁移（legacy 位置），之后不再动 repo 根。
...
前端（[web/modules/about.js](web/modules/about.js), [web/bootstrap-main.js](web/bootstrap-main.js)）通过 **`config_set` 协议**读写这些字段（见 Bootstrap 前端与协议节）。`list_resp` 每次都会把这 5 个字段以及 slots 一并推回，前端启动时从一次握手即可恢复全部偏好。
...
`bootstrap-main.js` 启动时按以下顺序联动：
1. `modules/audio.js` 先加载，暴露 `window.BootstrapAudio`（UI 音效 + ambient hum）
2. `config/version.js` 同步填充 `#version-number / #version-tail / #version-channel`
3. WebView2 就绪 → `send({cmd:'ready'})` → C# 侧 `Prewarm()` 静默拉 Flash
4. 再发 `{cmd:'list'}` 取槽位 + 偏好（走同一个 `list_resp`）
...
两 flag 都清空后，`DoPerformReveal()` 执行 panel swap（BootstrapPanel 隐藏、FlashHostPanel 显示）并发 `{cmd:'flash_ready'}` 给前端改跳过按钮样式。
...
| GameState | `start_game` | 点击确认启动该 slot（附 `deferReveal?` / `requireFlashReveal?`），自动写 `lastPlayedSlot` | 非 Idle 态 `StartGame` 内部直接 return 不回包，前端只能从 state 推送观察 |
...
| Archive query | `list` | 列所有槽位 → `list_resp`（slots + **5 个 userPrefs 字段**） | 任意 |
| Archive query | `load` | 读取 slot 的 shadow JSON → `load_resp`（遇 tombstone 报错） | 任意 |
| Archive query | `load_raw` | 绕过 tombstone 读原始 JSON（editor 用）→ `load_raw_resp` | 任意 |
...
`GameLaunchFlow.OnStateChanged` 每次状态跳转推单条消息：

```json
{ "type": "bootstrap", "cmd": "state", "state": "<State>", "msg": "<free-form>" }
```

其他推送：`flash_ready`（reveal 时）、`config_set_resp`、各种 `*_resp`、`error`。没有 `reason` 字段，前端靠 `msg` 解析（如 `"flash_exited"` / `"handshake_timeout"`）。
