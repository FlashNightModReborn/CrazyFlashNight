...
=== AUTOMATION TEST ===
Time: Sat Mar 21 16:36:50 GMT+0800 2026
[DepthManagerTest] 
[DepthManagerTest] ==================================================
[DepthManagerTest]  DepthManager (Twip Trick) 测试套件 v2
[DepthManagerTest] ==================================================
[DepthManagerTest] 正在设置测试环境...
[DepthManagerTest] 测试环境设置完成，创建了 256 个测试影片剪辑
[DepthManagerTest] 
...
所有监听器已移除：onUnload，已恢复原生处理器。
目标对象的所有事件已清理，已释放 watch 拦截器。
[DepthManagerTest] √ 测试通过: 生产容量 N=256 可行
监听器已移除：HID9
所有监听器已移除：onUnload，已恢复原生处理器。
目标对象的所有事件已清理，已释放 watch 拦截器。
...
[DepthManagerTest] ==================================================
[DepthManagerTest] 预生成随机数据: 512000 个 (种子=42, 可复现)
...
[DepthManagerTest] 原生 swapDepths: 总 899ms / 2000 迭代 = 450 μs/帧
[DepthManagerTest] 测试 DepthManager 性能（稳态，2000 迭代）...
监听器已移除：HID32
所有监听器已移除：onUnload，已恢复原生处理器。
目标对象的所有事件已清理，已释放 watch 拦截器。
[DepthManagerTest] DepthManager(逐实体): 总 2005ms / 2000 迭代 = 1003 μs/帧（稳态）
[DepthManagerTest] 测试 DepthManager 批量性能（稳态，2000 迭代）...
监听器已移除：HID35
所有监听器已移除：onUnload，已恢复原生处理器。
目标对象的所有事件已清理，已释放 watch 拦截器。
[DepthManagerTest] DepthManager(批量): 总 1324ms / 2000 迭代 = 662 μs/帧（稳态）
[DepthManagerTest] ────── 逐实体 updateDepth（2000 迭代，256 实体） ──────
[DepthManagerTest]   裸 swapDepths : 450 μs/帧（理论下界，无 lookup/clamp/注册/stale 检测）
[DepthManagerTest]   DepthManager  : 1003 μs/帧
[DepthManagerTest]   倍率          : 2.23x
[DepthManagerTest]   管理层开销    : 123%
[DepthManagerTest]   旧方案基线    : 1358 μs/帧（WAVL 树）→ 当前 1x 加速
[DepthManagerTest] ────── 批量 updateDepthBatch（2000 迭代，256 实体） ──────
[DepthManagerTest]   裸 swapDepths : 450 μs/帧（理论下界，无 lookup/clamp/注册/stale 检测）
[DepthManagerTest]   DepthManager  : 662 μs/帧
[DepthManagerTest]   倍率          : 1.47x
[DepthManagerTest]   管理层开销    : 47.3%
[DepthManagerTest]   旧方案基线    : 1358 μs/帧（WAVL 树）→ 当前 2x 加速
[DepthManagerTest] 
[DepthManagerTest] ==================================================
[DepthManagerTest]  测试总结
[DepthManagerTest] ==================================================
[DepthManagerTest] 功能测试:
[DepthManagerTest] - 总测试数: 30
[DepthManagerTest] - 通过: 30
