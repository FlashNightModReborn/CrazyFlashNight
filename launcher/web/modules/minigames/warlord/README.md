...
外层 `×` 和无内部状态可消费的 Esc 只发送携 active `panelInstanceId` 的 exact Host close；Bridge 投递成功不等于关闭确认。确认缺失时恢复同实例重试权，不本地 retire；generation、instance 与 Host active owner 共同保证迟到 A 不能关闭 replacement B。AS2 handoff 已 prepared 后，关闭按钮与 Esc 等待 Host 完成 exact close，避免 Web 先销毁实例、通用 pause 尚未释放而竞技场已启动。Esc 仍不越过当前交互层，由内向外依次消费：规则配置 → 解除进攻二次确认 → 战斗播放（未播完先跳到结尾，已播完关闭播放窗）→ 清除棋子编组。
...
- 全图/定位/按钮与 `+/-`、方向键/WASD 单步、`0/Home` 等程序性相机移动带 240ms ease-out 运镜过渡，过渡期间 HUD 读数连续刷新、`atFit` 仅在落位后为真；新输入（拖拽/滚轮/新目标）立即打断旧过渡并以其时实际位置为起点。拖拽平移与滚轮缩放保持 1:1 即时跟手不做插值；`prefers-reduced-motion` 下全部程序性移动直接跳变；
...
- 蓝方 AI 回合改为按节奏重放：先在行动前状态跑出整回合命令，再以 ≥460ms 间隔逐条经公开 `applyCommand` 投影中间态（与棋子补间对齐，不再齐飞）；多场战斗排队依次以 4× 播放，播完停留一拍自动关闭并继续，最后一并结束行动。as2 产品模式的 AI 派发间隔同样放宽到 450ms；
