...
- loot feed（左下物品获得播报）：AS2 `_root.发布战利品消息`（`scripts/引擎/引擎_鸡蛋_lsy_物品系统.as`，埋点 `PickUpManager` 拾取、通关奖励界面、`FinishTask` 任务交付、`LootClaimCommitCoordinator` 战利品箱提交）经 `{"task":"loot","payload":{kind,name,count,source,icon}}` fire-and-forget 到 `LootFeedTask` → NativeHud `LootFeedWidget`（左下血条上方锚底堆叠、内容宽度胶囊）；native 单渲染端，无 web fallback，与地图战利品箱的 `loot_response` 回包是两个域。`launcher/web/icons/manifest.json` 自此同时是 NativeHud 图标源（`LootIconCatalog` 为其首个运行时消费者）。P2 击杀播报复用同一 feed：与 killStats 共用 `enemyKilled` 事件链收口（`BulletQueueProcessor` 子弹/攻击体致死发布 → `EnemyKilledEventComponent.onEnemyKilled` 更新击杀统计并调 `_root.发布击杀播报`，语义为"我方（主角/佣兵）击杀"），按 `getUnitTypeKey` 发射 `kind:"kill"`，显示名取 `_root.敌人属性表` displayname，头像取 `launcher/web/assets/enemy-portraits` manifest（键=`敌人-*` 类型键，214/214 覆盖），默认 variant SVG 经 Svg.Skia 光栅化（SKSvg 生命周期与安全配置对齐 PlayerInfoStrictSvg；`<filter>` 元素解析前剥离）；后续关卡结算击杀数展示消费同一 `killStats`/事件链
- 集成测试入口：`--bus-only`
