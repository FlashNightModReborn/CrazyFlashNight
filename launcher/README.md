...
> **2026-08-14 Audio 发布列车当前态**：远端 `main` 仍停在兼容热修 `ed2f2f4d…8ebec`，因此现役 `runtime/**` 与旧 AS2/Web 路径保持可用。本分支已按 R4 H1/H4 恢复 Audio v2 的 AS2/SWF、Jukebox、格式路径与资格化 source closure，并实现 15 秒目标/30 秒硬上限、最终同 generation 双 snapshot PCM 证明和 E3 request-link promotion gate；这些字节须作为同一个新 Source 冻结并通过 H2、E3 与正式双构建后，才会与新 runtime 一起进入 `main`。当前仍为 **H2 blocked / NOT_DEPLOYED**。
...
> **最后核对架构与工作流**：跨层回包键契约修复正式发布 source commit `a87be26f1d6e80c0ea8883bbc10e46c895c35799`、release tree `32e5b9be67de220eb297e6c568c90546b283c471`、request `44A8D5461BE38E0807A7BE1DFEDAD4F5065F7981C590460CAEBAB4153E577A5E`，已由本地 X509 `physical-host-a` 与 GitHub OIDC/Sigstore `github-hosted-windows` 完成双故障域 v2 promotion；正式 runtime 绑定 identity `C510ED0C27E78F3FE2552AFA37C3E3B2E673CEC54B8B5AC4E69D516D72BBD8BC`、closure `844B898C7B74633DF7392298286E3A354D24F344B2A02DEF9C6E5545DC1ACF81`，正式入口复验 `runtimeMode=formal_runtime` 且 identity/closure 与 promotion 一致。本轮未跑无 candidate id 的 Agent Runtime MCP 窄纵切，因此不声称该最新列车达到 `standard_entry_verified`。P4/P5 的 pure-MCP 正式入口报告 `tmp/manual-agent-acceptance/workbench-v2/formal-workbench-20260808T083940Z.json`、商城旧档与竞技场业务 E2E 只保留为历史 release 证据，不得代证当前列车，也不外推为物理双屏、物理设备输入、任意业务写或玩家目视签收。Audio Platform v2 当前以 P4 `755c88eef2f0fb92f247d3a381dc91638cdf8e77` 与 H4 `0792adf201802f1369a91b2fd3e4561a25c279b9` 为已接受合同底座；它尚未取得 H2、E3 或 promotion，严格为 **H2 blocked / NOT_DEPLOYED**，不得并入上述正式 runtime 结论。此前 A1–A6 与 F8 发布只保留为历史分层；通用判定见 [验证矩阵](../agentsDoc/testing-guide.md)。
...
### 原生音频平台 v2（R4 working source，NOT_DEPLOYED）

R4 当前实现把已接受合同落到 observer/runner 与 promotion preflight，并恢复与 Audio v2 runtime 配套的 AS2/SWF、Jukebox 和音频路径。`sleep_resume` 的 `<=15s` 是质量目标，`>15s && <=30s` 结构化记录 `targetMiss=true` 但仍 bounded，`>30s` 才 fail-closed；恢复后 PCM 只比较最终同 generation、同 physical tuple 的两份显式 snapshot。E2/H2 后可创建 request 与启动双 builder；request 后立即形成 E3，缺失或绑定漂移在 VerifyOnly 与正式 promotion transaction 写前拒绝。该实现随本次精确 Source commit 冻结，提交前不预填自身 commit/tree；H2 与部署状态不因此改变。

S10/S11 的双 clean repro、candidate 与十个失败/诊断 run 只保留为历史，不能代签 R4 Source、A6、E1/H2 或正式发布。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
**2026-08-14 材料/配方人类验收纠正**：`a5-0814-h3` 虽然形成了机器 acceptance，但真人在配方页完成合成后证明无“返回材料”路径，关闭后游戏 UI 也失去交互。因此 h3 只保留为历史机器证据，不再是 current `e2e_verified`。现役 source 在同 crafting owner 内提供一跳“← 返回材料”，以 stable `materialName` 重读并重选，不建立通用 panel history；PanelHost 在 shield/backdrop/HUD/Notch/Toast/ESC/cursor 全部收尾后再执行一次 `panel_close:settled:<panel>` Flash 焦点恢复。材料用途按钮现为“前往合成”；每张配方卡以单行三段固定产物、23px 静态材料格带与右侧动作列，数量角标/tooltip 留在格内，当前材料另有描边，避免按钮或材料跨入相邻卡片；敌人/关卡与商店说明只在来源区顶部显示一次，卡片内改用简短“基础掉落 / 关卡基准”值。current focused `--material-recipe-only` 为三视口各 `22/22`，`--material-shop-only` 为三视口各 `11/11`，Crafting runtime 为 `30/30`；旧文中“查看配方”“不提供返回材料”和 `21/21` current 表述均已失效。fresh candidate 人类复验前状态为 `A5_REOPENED / NOT_DEPLOYED / RELEASE_HOLD`，不启动云端共识或 DLL 部署。

本轮 fresh Flash 证据为 run `66a55e40661a4294aaeb4f2c117c860c`：Crafting `128/128`、Synthesis `13/13`、compiler `0/0`；publish 产物 `scripts/asLoader.swf` 为 `1,071,740` bytes、SHA-256 `5D4A7170E3B85E7AF96BB9ED845E3C9D89D6844FE81D93BEC58EAA2C65AAAC03`。该证据仍只证明 current AS2/SWF，不代签 fresh candidate 人类交互。

**2026-08-14 当前覆盖（取代上段旧计数与焦点推断）**：材料 v2 的 `navigationAccess` 由 AS2 现役基建投影并在执行时重验；自行车及以上解锁“前往商店”，摩托车及以上解锁“前往合成”，越野车兼容两者，锁定态分别显示“需自行车 / 需摩托车”。generator 仍将 `data/infrastructure/infrastructure.xml` 的 67 个需求 occurrence 去重为 21 个 identity，并 exact-check catalog 的 `system:infrastructure_upgrade / 基建升级` 引用；含该用途的详情现条件携带只读 `infrastructureUses[]`，按已发现基建展示项目、`Level[]` 物理等级、需求角标、缺口及完成/当前/后续状态。选中材料的总持有量只在详情标题显示一次；逐级 `owned` 只参与缺口校验与无障碍说明，不再重复进入视觉文案。未发现项目不泄露；等级不信任 XML `id`；Web 用专用动态卡替换重复标签，但仍没有“前往基建”按钮、新导航协议或旧 Flash 基建 owner 跳转。退出后游戏 UI 不可交互的根因不是 Flash focus，而是 telemetry 后迟到的全屏 `interactiveRect` 留下透明 InputShield mask；现由 telemetry enter/exit 重置 capture/hit rect/mask、期间拒收 interactive rect，并在 PanelHost 隐藏 overlay 前 best-effort 发送 instance-bound `panel_cmd close`。此前 fresh Flash / candidate 数值只保留为动态卡合同加入前的历史证据；新合同完成 focused 门与 fresh candidate 真人复验前保持 `A5_REOPENED / NOT_DEPLOYED / RELEASE_HOLD`，不启动云端共识、promotion 或 DLL 部署。
