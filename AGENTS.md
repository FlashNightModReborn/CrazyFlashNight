...
**最后核对代码基线**：release source commit `248cca7be212219655319c666304407b0568e658`（2026-08-25；implementation `7e9c053512`、合并 AS2 publish `cac7b7a475`、tag `runtime-build-v2/20260825-warlord-as2-battle-v2`、tree `2bbf0eaa19f10461d5feb9cc6c0e26a1a2f9845a`、request `4F57D8E7BA7499021E4A36823BFB27A42BDAE043CA9483BF46B34F42AB09456D`、deployment `126c10a3d5bf46b66a973e8b68ad27c00ef9257d`）。
本地 X509 `builder-local-b` / `physical-host-b`（keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`）与 GitHub OIDC/Sigstore builder `8273B61EB030A519118FF275A39F73351D5413DCB13A230D1E74ED6F6C318ED6` / `github-hosted-windows`（cloud run `32821144648`）已对 identity `6482B71F2811065A5537F5CDCB0E338D44C23C2BECDC13987D9924B289EC8C59`、closure `4B46C47478DAC64E038F574F2636AFDFF9E0E6EA16F807171FF5F31E21237062` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `CA5E2FF757FB4636D033E0A97026C6B919C7E11BC2E08C35088A962B9B8A4E85`，38/38 production receipt SHA-256 为 `95AA0D3E0570EAE9712DADF1EB237EE81D8EBB205A199F0BCB4B7FDD02942DF2`；deployment commit `126c10a3d5bf46b66a973e8b68ad27c00ef9257d` 与首次 post-promotion audit run `32822831143` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车把军阀战术演习的 3D 沙盘、编组移动、生产调度、战宠卡牌目录投影、真实 AS2 战斗 handoff/receipt 与专用战后恢复路由接入正式 Launcher；JS resolver 只留 fixture，产品 AS2 不可用时 fail closed。战旗的战宠经济标定固定 `observe_only / settlementPolicy=none / writesPlayerState=false`，不扣玩家金币/K 点、不写战宠或存档。
fresh 门包括 Warlord Node `75/75`、Edge/CDP `16/16`、共享 Minigame `57/57`、Launcher 全量 `4099 pass + 3 explicit opt-in skip / 4102 total`、Flash CS6 Compiler `0/0`；合并源重新发布的 `scripts/asLoader.swf` 为 1,141,507 bytes / SHA-256 `690C1C871FFF915BCEFC17158146B79F805D70A69793014095343019F0454539`。production policy `38/38`、worktree/index 33-file bundle、signed consensus、正式根 bootstrap `--verify-only`、远端 Audit 与无 candidate selector 的正式身份/生命周期 smoke 均通过。
维护者已在同功能隔离候选上确认沙盘体验与 AS2 战斗结束自动返回有效；部署后的 formal smoke 只覆盖 fresh reveal、两次同 lifecycle `session.status=verified`、精确 identity/closure、strict shutdown、Flash/Guardian exit 0 与零进程残留，没有重跑军阀业务。因此准确状态为 `HUMAN_ACCEPTANCE_PASSED / promoted`，不称军阀专项 `standard_entry_verified`；二次交战、最终回基地、玩家存档/经济无污染及 Audio H2 仍保持独立门。
