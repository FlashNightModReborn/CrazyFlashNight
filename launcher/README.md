...
#   - 注入 CSS 隐藏 web 端 #safe-exit-panel 避免双重渲染
# 性能收益（当前）：仅 panel 打开期 α blend 下降；idle 仍是 Phase 0 baseline。
# 完整 idle SW_HIDE WebView2 推迟到 Phase 4 收尾后（剩余 combo/quest-notice/jukebox-titlebar/map-hud widget 迁完）。
useNativeHud = false
```
...
- WebOverlay 注入 CSS 隐藏 web 端 `#notch` / `#toast-container` / `#top-right-tools` / `#safe-exit-panel` 避免与 C# 渲染重叠；notch/toast 消息（AddNotice/SetStatusItem/AddMessage）始终走 fallback (NotchOverlay/ToastOverlay) 而不是 web ExecScript
- **NativeHud widget 已迁**（[Hud/](src/Guardian/Hud/)）：[TopRightToolsWidget](src/Guardian/Hud/TopRightToolsWidget.cs)（5 键：GAMESETTINGS/SETTINGS/PAUSE/HELP/SAFEEXIT，监听 UiData "p"/"s"），[NotchToolbarWidget](src/Guardian/Hud/NotchToolbarWidget.cs)（5 键：PETS/MERCS/TABLET/WAREHOUSE/SHOP，WAREHOUSE 跟 q>13 条件可见，恢复 kshop 入口），[CurrencyWidget](src/Guardian/Hud/CurrencyWidget.cs)（金币 + KP，含 600ms ease-out 滚动 + 1200ms delta 浮动），[SafeExitPanelWidget](src/Guardian/Hud/SafeExitPanelWidget.cs)（UiData "sv" 状态机：sv:1 显示「存盘中…」状态条，sv:2 加 取消/退出 按钮，取消纯本地 `_dismissed=true` 不通知 AS2，退出走 router `EXIT_CONFIRM` → ForceExit；s:0 复位）
...
- SAFEEXIT 二次确认：router 仅 `SendGameCommand("safeExit")` 触发存盘，C# [SafeExitPanelWidget](src/Guardian/Hud/SafeExitPanelWidget.cs) 监听 UiData "sv" 自动切换面板状态；移除原 PostToWeb `safe_exit_show` 路径（web 端 Bridge 监听器在 idle SW_HIDE 后会失效）
