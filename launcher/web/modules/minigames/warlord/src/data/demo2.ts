...
export type Demo2HomeKey = 'player' | 'pact-a' | 'independent' | 'pact-b';

export type Demo2SectorId =
  | 'sector.player-home'
  | 'sector.pact-a-home'
  | 'sector.independent-home'
  | 'sector.pact-b-home'
  | 'sector.player-arm'
  | 'sector.pact-a-arm'
  | 'sector.independent-arm'
  | 'sector.pact-b-arm'
  | 'sector.central-industry';

export interface Demo2SectorCatalogEntry {
  readonly id: Demo2SectorId;
  readonly displayName: string;
  readonly role: 'home' | 'contested-arm' | 'central-ring';
  readonly nodeRefs: readonly string[];
}

interface HomeSpec {
  readonly key: Demo2HomeKey;
...
const HOME_SPECS: readonly HomeSpec[] = Object.freeze([
...
export const DEMO_2_COMMANDER_CARD_BY_FACTION: Readonly<Record<string, number>> = Object.freeze({
  player: 83,
  'boss-pact-a': 111,
...
const HOME_NODE_ROLES: readonly HomeNodeRole[] = Object.freeze([
...
const HOME_EDGE_PAIRS: readonly (readonly [number, number])[] = Object.freeze([
  [1, 2], [1, 3],
  [2, 4], [2, 5], [3, 5], [3, 6],
  [4, 5], [5, 6],
  [4, 7], [5, 7], [5, 8], [6, 8],
...
const CENTRAL_POSITIONS: readonly (readonly [number, number])[] = Object.freeze([
  [536, 397],
  [600, 375],
  [664, 397],
  [690, 450],
...
function homeNodeId(key: Demo2HomeKey, ordinal: number): string {
  return 'd2-' + key + '-' + twoDigit(ordinal);
}

function armNodeId(key: Demo2HomeKey, ordinal: number): string {
  return 'd2-arm-' + key + '-' + twoDigit(ordinal);
}

function centralNodeId(ordinal: number): string {
  return 'd2-central-' + twoDigit(ordinal);
}

function pointTowardCenter(
  spec: HomeSpec,
  progress: number,
  lateral: number,
...
function homeNodeAuthoring(role: HomeNodeRole): MapNodeAuthoringV1 {
...
function homeNodeRule(
  spec: HomeSpec,
  role: HomeNodeRole,
): ScenarioNodeRuleAuthoringV1 {
  const values: readonly (readonly [number, number, number, number])[] = [
    [6, 5, 6, 2],
    [4, 2, 8, 2],
    [4, 1, 8, 3],
    [3, 8, 3, 1],
  ...
  if (!value) throw new Error('Missing Demo 2 home-node economy values.');
  return {
...
function edgeBuilder(): {
  readonly edges: MapEdgeAuthoringV1[];
  readonly add: (a: string, b: string) => void;
} {
...
]))) as Readonly<Record<Demo2HomeKey, readonly string[]>>;
...
export const DEMO_2_CENTRAL_NODE_IDS: readonly string[] = Object.freeze(
  Array.from({ length: 8 }, (_, index) => centralNodeId(index + 1)),
);

const mapNodes: MapNodeAuthoringV1[] = [];
const presentationNodes: Array<{ nodeRef: string; x: number; y: number; visualAnchor?: string }> = [];
const nodeRules: ScenarioNodeRuleAuthoringV1[] = [];
...
for (let ordinal = 1; ordinal <= 8; ordinal += 1) {
  ...
  if (!point) throw new Error('Missing Demo 2 central-ring position.');
...
const armToCenter: Readonly<Record<Demo2HomeKey, readonly [number, number]>> = Object.freeze({
  player: [8, 1],
  'pact-a': [2, 3],
  independent: [4, 5],
  'pact-b': [6, 7],
...
} satisfies MapDefinitionAuthoringV1;
...
} satisfies MapPresentationDefinitionV1;

export const DEMO_2_SECTORS: readonly Demo2SectorCatalogEntry[] = Object.freeze([
  ...HOME_SPECS.map((spec) => Object.freeze({
    id: ('sector.' + spec.key + '-home') as Demo2SectorId,
    displayName: spec.displayName + '本土战区',
    role: 'home' as const,
    nodeRefs: DEMO_2_HOME_NODE_IDS[spec.key],
  })),
  ...HOME_SPECS.map((spec) => Object.freeze({
    id: ('sector.' + spec.key + '-arm') as Demo2SectorId,
    displayName: spec.displayName + '争夺臂',
    role: 'contested-arm' as const,
    nodeRefs: DEMO_2_ARM_NODE_IDS[spec.key],
  })),
  Object.freeze({
    id: 'sector.central-industry' as const,
    displayName: '中央高价值工业环',
    role: 'central-ring' as const,
...
} satisfies WarlordScenarioAuthoringV1;
 ...
 */
export const DEMO_2_ALLIED_PACT_FACTIONS: readonly [string, string] = Object.freeze([
  'boss-pact-a',
  'boss-pact-b',
]);
