...
**文档角色**：游戏系统 canonical 索引。**最后核对代码基线**：上游 `origin/main` `b072f97841ccb30e167c14495241ae64d9054e22`（2026-07-21 发布前复核）+ 当前 `release/map-loot-20260721-final` 未 promotion 发布候选树。
...
- **中央裁决**：`BoxInteractionArbiter` 只接受六个资源箱 preset；每个 world 只有一个全局互动监听，一次输入按 `(dx²+dz², registrationOrder)` 选择至多一个箱。`SceneInteractionManager.currentMC` 对箱候选独占，非箱 `InteractionHandler` 与地面拾取仍保留既有共消费。对已识别箱，arbiter 注册失败必须使整个 initialize 回滚，绝不降级为逐 target legacy `subscribeGlobal`；A03F 固定证明零 per-target listener / pickup / 初始化 marker / 残留 record。`unregister` 仅停用、重注册复用稳定顺序；目标 unload 用 `forget` 删除已知记录，world 销毁再整体 cleanup。
- **旧行为保留**：`row == 0 || col == 0` 的直投箱仍走原爆落处理；中央裁决只解决多箱 fan-out，不把所有箱强制改成 Lockbox，也不改变投影召唤器 / 地面拾取语义。
- **S1/S2 生产选择器**：只接受目标自身严格三字段 `chestRolloutId + lootFlowProfile=web-loot-v1 + unlockPolicy=skip`，并要求网格 preset、正整数容量、唯一 id、合法掉落规则且不与 S0 marker 共存。当前真实 XML 只有 `夺取材料.xml | SubStage 5 | Instance 0 | loot-canary-material-5-0-v1` 一处 rollout；部分字段、错类型、direct 误标或坏掉落配置都 fail closed，无 marker 仍走 legacy。
...
- **源码层**：S0 AS2 bridge / production callback、Host `DevLockboxS0Runtime` / tracked queue、production overlay 的默认休眠 bootstrap / actual-wire 保持已接；两道 S0 门仍默认关闭。S1/S2 又已接 AS2 `LootContainerService` 与地图 / 回调 / strict 7/8/9-key query/recovery proof，Host `LootPanelCoordinator/LootTask`（含 admission fence、bind/close/pause retry、exact proof settlement 与 socket generation ordering）、Web `loot` 四模块 / lazy registry / CSS，以及一处真实 XML rollout；这是源码状态，不是部署状态。
- **自动层**：S0 历史门保持可追溯；当前 S0 static wiring **454/454**。Rock Park 4 个 `TEMP` 夹具已精确移除，stage audit **41/41**，当前真实 XML 为 **28 declarations / 27 nodes / 26 identities**、Rock Park **active 1 / commented 8**、rollout **1**；`摇滚公园.xml` 为 **27,345 bytes** / SHA-256 `C184AD15F1EAE24B31EF9BD552890F95153431C3FFEA80A0CC0760682AB990D5` / Git blob `43018f3b5886bc89b1881c7bb04801d4bd45f5fb`。S1/S2 source-tree 已通过 map wiring **243/243**、Host 相关定向 **108/108**、AgentControl **29/29**、Launcher/Host clean full **1220/1220**、Host focus 定向 **9/9**、共享 focus integration **12/12**、XmlSocket 脱敏 **10/10**、Web loot **46/46 + 62/62 + 6/6**、save multi-instance **44/44**。fresh CS6 map-loot runId `dbcc28a3d99444c38bde38029ff4bd61` 为 Service **159/159** + Planner **7/7** = **166/166**，failed **0**、Compiler Errors **0/0**、BOM **227/227**；最终 publish-only `scripts/asLoader.swf` 为 **1,041,903 bytes** / SHA-256 `AD799970A8A2AFD0F9A402C704934F08985A3144FA608A7564E17BC4899C815E` / Git blob `3dd59f73f9fc71128da99c2eb03796290df1e010`；mtime **2026-07-21 17:12:59 +08**；17:13:03 的 `compiler_errors/compile_output` 为 **0 错误 / 0 警告**、`[compile] done`。publish-only 不生成行为 trace，不能替代该 fresh TestLoader run。
- **候选层**：`c-fca19ba526c4-08846e81b3-20260721t060547571z-978633c6` 只是嵌入式 organizer 与 attempt-bound agent 标题帧门之前的历史 candidate；其 build identity `FCA19BA526C4120B240B594FDD29786261A318F805A0ACD8001D48A842B60F24`、payload closure `B2AF3BEFCBA055FC68F3D49A9A450B6D946F20DD5A8E28FEAF0EC8B32BFAC4CD`、Core SHA `231388D757201454655E95876834155E17ED118476C0B5806A96E2A0A6492780` 曾达到 `candidate_executed / NOT_DEPLOYED`，但不能代表当前 source tree。当前精确隔离 candidate `tmp/runtime-candidates/v2/c-2a0cddb077b7-08846e81b3-20260721t100014612z-f32a40a3` 的 native build identity 为 `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`、payload closure 为 `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`、Core SHA-256 为 `0F58BF864B8DE9C7FCEA098D7E1EEA1996BDE38D85D87E844B047B53F5247232`；它已精确启动并完成单 canary 资源箱领域闭环，状态为 `e2e_verified / NOT_DEPLOYED`，但没有 promotion 或正式部署证据。该 candidate 的 25-file native bundle 不含 `launcher/web`，运行时从当前 `projectRoot/launcher/web` 读取外部 Web，因此相同 native identity 不绑定本轮 Web 修复；Web 证据须另以源码 SHA-256 与当前 WebView2 实机链绑定。候选随后优雅 shutdown，Guardian PID **14820** 与 HTTP/XMLSocket **1192/1924** 均退出，Flash CS6 PID **20444** 保留。
- **实机宿主层**：新 candidate 已载入专用槽 `cf7_agent_loot_target_full_v1`；attempt `62f04fa249c44d94ba3171f619e61754` 严格按 start 后日志水位→fresh handoff→真实 `[LaunchFlow] bootstrap_reveal_ready: Flash reveal cleared`（watchdog **0**）→唯一一次 `_root.agentEnterResolvedSave()`→同包 `s:1|ga:<exact attempt>`→Host exact receipt 执行，最终 `readyForRuntimeAutomation=true`、`blockers=[]` 且 `saveRuntime` exact loaded；人类同时确认正常标题帧唤起的 NativeHud 可见且指令有效。满背包混合箱随后在同一 `panelInstanceId=panel.loot.8a1227b8229f4eef99ac052395e4e12b` 完成：抗生素并入已有堆叠、黑暗吉他因满背包保留→同面板 organizer→真实二次确认丢弃整格牛肉罐头 **×16**（背包 50→49）→fresh `ACTIVE` snapshot 返回战利品且无 close/rebind→领取黑暗吉他→terminal close/unpause 自动回游戏。普通关闭回归又在 **18:37:08** 首开非空箱，**18:37:18** 单次 close 后 PanelHost 以 `instanceDigest=83e7e242b7fc / reason=suspended_already_closed` 关闭，**18:37:19** unpause；**18:37:21** 同锚点 fresh reopen，人类目视强化石 **×2 + ×3** 内容一致且无旧 AS2 UI；**18:37:29** 两次 claim 与一次 terminal close 以 `instanceDigest=6ef2f7f9fcb2 / reason=terminal_already_closed` 收束并 unpause。由此当前单 canary 的 organizer、普通 suspend/reopen 与一次点击 modal 均完成实机闭环，可写为 `e2e_verified / NOT_DEPLOYED`。历史 attempt `b9bb7f9a800f479cba04542820fa2748` 与冷启动 `7560a368fdb64dc5b7e2d2e19377add7` 只继续作为旧交互的经济写/持久化证据。
- **发布层**：当前上游基线 `origin/main` 为 `b072f97841ccb30e167c14495241ae64d9054e22`，正式 runtime v2 consensus 仍沿用既有 build identity/payload closure；promoted Core 不含本地 S0/S1/S2 Host。源码可以在部署闭包不变时以 `source-ahead` fast-forward 进入 main，当前资源箱发布候选仍没有 immutable Git-tree request、production policy receipt、有效 local X509 proof、不同 signer + `faultDomain` quorum、strict verifier 或 promotion；当前 diagnostic candidate 与历史 candidate 都不是 release evidence。
- **部署层**：正式 Core 仍不含本地资源箱 Host，root bootstrap / `runtime/` 没有本轮 promotion；不得把当前 `e2e_verified / NOT_DEPLOYED`、历史 `candidate_executed / NOT_DEPLOYED`、source-tree 自动门、外部 Web 哈希或 publish-only Flash 产物写成部署。当前剩余门是完整 S1/S2 资产与 S0 exact fixture / 全局 arbiter 人工验收，以及正式 immutable request/receipt/quorum/strict verifier/promotion/standard-entry 验证。冻结边界见 [S0 ADR](../docs/地图资源箱-S0无奖励编排-ADR-2026-07-17.md)、[S1/S2 ADR](../docs/地图资源箱-S1S2真实战利品容器与Web双栏-ADR-2026-07-18.md) 与 [专题治理文档](../docs/地图资源箱-Web战利品工作台与开锁流程-前期调研-2026-07-17.md)。
