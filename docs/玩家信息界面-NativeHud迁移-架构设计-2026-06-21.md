...
**最后核对代码基线**：commit `bf8dd2c…8479` 的 47/47、旧 `runtime-qualification.json` / `visual-evidence.json`、source-freeze F `cb38600…f511`、不可变 v1 tag、双 builder quorum、`-VerifyOnly` preflight 与旧 Kimi PASS 均为历史 v1 证据，只证明旧字节且始终 `NOT_DEPLOYED`。当前 main-space v2 已排除 child wrapper，修正 main-visible bounds、HP 两遍 Glow/横线与 MP 排版；MP current 保持 source-derived 右锚 `74.55`，maximum 在 XFL authored `80.65` 上应用仅供 C# path rasterizer 匹配 Flash standard-text coverage 的 `+0.30` local phase correction，effective anchor 为 `80.95`。B0-05 当前合同为 8 logical layer / 10 owned PArgb payload、8-field key，并在每 viewport 20 个 acceptance 样本前固定运行 16 轮同路径 round-robin excluded warmup。真实 1024×576 main-stage Flash v2 candidate 已捕获且 strict tooling identity 通过，但四项 human review 仍为空。fresh B0-05/B0-06/full、正式 C#/Web/direct A/B、新 source-freeze/tag/request/双 builder quorum/preflight 与最终 k3/max 均待完成；`config/build/runtime-github-builder.v2.json` 只预留全新 `...-b0-v2` sourceRef，tag 尚未创建。严格状态为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`。真实 `pi_*`、promotion、部署与现役 runtime 均未改变；精确状态以 [专项 ADR](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md) §12 为准。
...
- 2026-07-29 的视觉基座 B0 保持两条正交状态。Oracle 轨已有 11-case、1024×576 main-stage、exported-symbol extraction、wrapper=false 的真实 Flash v2 candidate，且 tooling strict identity 通过；但四个 manifest human check（状态/层/crop/审美）与独立来源身份现场复核均为空，所以只是 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`。工程轨保留 B0-03b 的 renderer-qualified 架构、8 SVG + manifest 与 `NOT_DEPLOYED` 边界；B0-05 为 8 logical layer / 10 owned PArgb payload、16 MiB whole-batch cache、8-field key与 `split_required`，当前 v2 runner 采用固定 16 轮同路径 excluded warmup + 每 viewport 20 acceptance。B0-06 仍是 allowlist fixture-only split surface，不注册现有 NativeHud union、不接真实 `pi_*`、不隐藏或修改旧 Flash HUD；HP 使用两次 `sigma=1` source-over blur（alpha 255 + 128）加白色 core，横线已 active，只有 hp light-overlay 延后；MP current 右锚为 `74.55`，maximum runtime effective 左锚为 `80.95`。v1 的 47/47、A/B、F/quorum/preflight/Kimi 只作历史；v2 formal、视觉 A/B、新 F/tag/request/quorum/preflight/Kimi 与人签未闭合，总体为 `visual_correction_in_progress; human_exit_blocked`。FFDec/headless Edge/自动 HWND 不能代签真实 DWM、游戏 composite、z-order/occlusion、鼠标透传、运行时 SWF 身份或审美。promotion/标准入口/部署不属于 B0 Exit；fixture-only 复刻不推翻 §4 的 state-first 运行态接入顺序。
...
| 5 | C# PlayerInfoWidget 只读 | B0-05 已完成 raster/cache 并因近全屏 union 接受独立 split surface；B0-06 已实现不接业务权威的 HP/MP fixture 纵切与独立 click-through surface，surface 独占 animation model，widget 只读消费接口，Resume 仅在有效布局与 raster request 建立后完成。历史 v1 实现基线 `bf8dd2c…8479` 曾取得 formal qualification 47/47、focused 136+3/139、全量 1,737+3/1,740、视觉确定性、F source-freeze/tag、双 builder 非部署 quorum 与 Kimi K1 PASS；这些只证明旧 v1 字节。当前 main-space v2 仍为 `source-ahead / NOT_DEPLOYED`，fresh formal/full/视觉、新 F/tag/request/双 builder quorum/preflight、最终 Kimi 与人类验收均待完成。B0 accepted 后仍按阶段2→3的 state-first 链路接真实 UiData，程序化复刻虚拟帧缓动 |
| 6 | 双轨对比 | 不变；按 §2.4 验收表 |
...
| 资源管线 | ★★★★☆ fixture 基座完成、main-space v2 重验中 | 8 个 HP/MP SVG + runtime manifest 已作为 9 项 embedded resource；`Svg.Skia 5.1.1` / `SkiaSharp 3.119.4`、strict facade、notice、runtime identity 与 production contract 已接线，B0-03b 保持 `renderer_qualified / NOT_DEPLOYED`。B0-05 是 content-height/576、8 logical layer / 10 owned PArgb payload、8-field key、16 MiB whole-batch cache与 `split_required`；B0-06 是 11-case fixture-only split surface、9 个文字布局、两个 active effect（动态文字/HP Glow、HP 横线）与唯一 deferred hp light-overlay。HP 以 source-red `sigma=1` 的 alpha 255 + 128 两次 blur近似 strength 1.5；MP current 右锚为 `74.55`，maximum runtime effective 左锚为 `80.95`。历史 v1 47/47 与 A/B 不批准当前 v2；真实 main-stage Flash v2 candidate 已捕获但待人审，fresh formal/视觉 A/B/Kimi 与可见 DWM/游戏 composite/手点/审美均未闭合 |
...
- **建议**：HP/MP fixture-only B0 的 formal qualification、双份 visual closure、F source-freeze/tag、双正式 builder 非部署 quorum 与 Kimi `k3 + max` 最终增量 K1 PASS 均为 historical v1 记录，只证明旧 v1 字节。当前 main-space v2 仍为 `source-ahead / NOT_DEPLOYED`；下一步须先完成 fresh qualification/visual、新 F/tag/request/双 builder quorum/preflight 与最终 `k3 + max`，再由人类签收 Flash oracle、可见 DWM/游戏 composite、关键比例/平滑、混合 DPI、运行时 SWF 身份、真实手点与审美。它们全部通过也只证明视觉基座。B0 accepted 后再沿阶段2→3建立真实 state/UiData，并做 HP/MP 双轨对比，不能把 fixture 结论外推为运行态接入。
...
**文档治理**：本文为玩家信息 HUD 迁移的纲领 canonical doc；SVG/B0 专项文档只承载视觉渲染子域，避免形成第二份业务权威。B0-04 已同步 strict canonical validator 与 Web/FFDec/Flash diagnostic 入口；B0-03b 已同步生产包、embedded resource、runtime identity/policy/notice 与 `renderer_qualified / NOT_DEPLOYED` 边界；B0-05 已同步 raster/cache、`synthetic_fixed_bounds` 与 `split_required`。B0-06 曾按 historical v1 实现基线 `bf8dd2c…8479` 的 fixture visual-state/widget/compositor/path glyph atlas、HP source-red Glow/MP no-Glow、surface 单一 model ownership、可靠 Resume pending、typed active/deferred effect policy、独立 split surface、PanelHost 生命周期、strict convergence qualification、机器报告与 A/B capture 索引，以及旧 F source-freeze/tag、双 builder `-VerifyOnly` quorum 与 `NOT_DEPLOYED` 边界，同步 `launcher/README.md`、testing guide 与本路线；Web static-only 与 direct metrics 包含 scope 差的边界也已显式回写。historical v1 的最终 K1 曾在 clean G 上取得 PASS、P0/P1=0，并由同一 docs-only 后继同步自动 Gate；这些记录不批准当前 main-space v2。当前 v2 仍为 `source-ahead / NOT_DEPLOYED`，fresh formal/full/视觉、新 F/tag/request/双 builder quorum/preflight、最终 Kimi 与人类验收均待完成；本轮仍未声称 B0-04 人工视觉、真实 `pi_*`、可见 DWM/游戏交互、parity、promotion 或部署。全部当前门禁闭合后再同步视觉接受结论并运行 doc governance。后续按实际改动回写本文 §5 路线与 §6 风险，行号锚点随重构刷新。
