...
- WebOverlay 注入 CSS 隐藏 web 端 `#notch` / `#toast-container` / `#top-right-tools` / `#safe-exit-panel` / `#quest-notice-bar` / `#combo-status` / `#jukebox-panel` / `#map-hud` 避免与 C# 渲染重叠；notch/toast 消息（AddNotice/SetStatusItem/AddMessage）始终走 fallback (NotchOverlay/ToastOverlay) 而不是 web ExecScript
...
- 完整 idle SW_HIDE 推迟到 Phase 4 收尾：当前 web 端仍承载 jukebox-titlebar / map-hud 等常驻 HUD（safe-exit-panel / quest-notice / combo 已 C# 化）。`DoFullIdleSuspend` 代码已就位，等剩余 widget 都迁完后启用
