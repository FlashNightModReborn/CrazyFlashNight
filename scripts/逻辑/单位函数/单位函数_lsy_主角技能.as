...
_root.技能函数.气动波攻击 = function()
{
	...
	if(_parent.被动技能.内力爆发 && _parent.被动技能.内力爆发.启用){
		if(_parent.mp >= _parent.被动技能.内力爆发.技能等级 * 2){
			_parent.mp -= _parent.被动技能.内力爆发.技能等级 * 2;
			子弹.子弹威力 += (_parent.内力 * 0.2 + _parent.被动技能.内力爆发.技能等级 * 2) * _parent.被动技能.内力爆发.技能等级;
			if(_parent.被动技能.内力爆发.技能等级 >= 5){
				子弹.伤害类型 = "魔法";
				子弹.魔法伤害属性 = "冲";
			}
		}
...
_root.技能函数.龟派气功攻击 = function(当前攻击点, 段数类型, 霰弹值)
{
	...
	if(循环次数 > 0){
		...
		if(_parent.被动技能.内力爆发 && _parent.被动技能.内力爆发.启用){
			子弹.子弹威力 += 循环百分比消耗 * 2 * _parent.被动技能.内力爆发.技能等级;
		}
	}
	if(_parent.hp攻击加成){
		子弹.子弹威力 += _parent.hp攻击加成;
	...
	if(段数类型 == 1){
		...
		if(_parent.被动技能.内力爆发 && _parent.被动技能.内力爆发.启用 && _parent.被动技能.内力爆发.技能等级 >= 10){
			子弹.伤害类型 = "魔法";
			子弹.魔法伤害属性 = undefined;
		}
	}
