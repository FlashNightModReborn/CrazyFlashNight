...
- Character Build 的拆分边界固定为：`character-build-session-contract.js` 只做 wire shape，`character-build-transport.js` 只做 loadout mux framing，`character-build-candidate-channel.js` 只做单条权威只读缓存、候选读取及 stale recovery，`character-build-loadout-presenter.js` 只做 11+4 槽 tooltip/角标回注与已装备注释（槽位网格渲染已迁 `loadout-picker-slot-grid.js`），候选范围/选择/恢复/pointer drag 交互层已抽为共享 `loadout-picker/`（pane 状态机 `loadout-picker-candidate-pane.js`、五态 `loadout-picker-candidate-state.js`、拖拽接线 `loadout-picker-candidate-drag.js`、落点策略 `loadout-picker-drop-policy.js`、操作栏动词注入 `loadout-picker-action-view.js`；ports 默认值保留 character-build-* 类名与文案，佣兵换装二期经 `merc/merc-loadout-*.js` 适配层消费同一 picker，会话/写通道/revision 仍各面板私有），`character-build-candidate-eligibility.js` 只做 universal 背包行的纯资格重投影。facade/session/view 继续拥有组合与生命周期，不得把这些叶模块扩成第二套领域状态机或多 key 缓存框架。
