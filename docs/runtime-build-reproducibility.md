...
**最后核对代码基线**：CF7 Agent Runtime / Wings F8 release source commit `6f3d50a52413c747b05b74be88d6ee46650f4597`（tag `runtime-build-v2/20260731-agent-runtime-wings-f8-v1`）、release tree `253e57f6d20a90fef6addfa744d0487d88f00dfb` 与 request `A9B33601805709DBB5EAE6DAF312C2B7B0B502096FDD3BDCEA9CBE26D8B1299C` 已完成正式 promotion。无 candidate id 的 `automation/start.ps1` 正式入口通过项目自有 Agent Runtime MCP 完成单屏 Launcher/NativeHud/WebOverlay WGC、Flash metadata-only fail-closed、structured 帮助面板与 supported shutdown 的窄纵切，严格达到 `standard_entry_verified`。该状态不代表物理双屏、“13/13”、Flash pixels/input、Hair、Wings 完整产品、业务写、持久化或维护者目视签收，完整边界见 [F8 人工验收与正式发布记录](evidence/cf7-agent-runtime-f8-manual-acceptance-2026-07-31.md)。

## 当前迁移状态

runtime v2 的工具、schema、队列、本地 X509 证明、GitHub OIDC/Sigstore 证明、promotion 与 CI 状态机已经完成正式闭环；**仓库当前受控部署已是 manifest/consensus v2**。当前 consensus 的 artifact source `98000EBA762749C28583579271362FBED9E2E5FFE202FFF5FB0B0D8028456893`、producer recipe `B97998EA7246D6AB667902BCBBD7994DFA5F658A37CFA427D2EEEABA6924DE28` 与 toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD`，在 policy `33A18135EEB9094F305C63ADA887600230C18AD49FA0AE151E469A4F68DDC09F` 下形成 build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C`；payload closure 为 `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0`，manifest SHA-256 为 `A0A7CA0C992149DC2F9A4D5DDC60C2CAAC2EDC8BBC7907B0CF8EE4C7D5E694E4`，promotion 后 Windows worktree consensus bytes 的 SHA-256 为 `8E6292039E0BD01C7F1C0ADD1AE28DF840D676D4A8FB959337BBDAB07D285F26`（不是 Git clean-filter 后 LF blob 的字节哈希）。production policy 26/26 receipt SHA-256 为 `DA46C6E99CB02A268099ACB709C80B60E8B2D821134E1D32C9C84CB288EEC38C`；本地 X509 signer / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC builder `2E05B5577CC8D2895DC56FC8778A05D09C440302832A05DD4AE551589C9AE918` / `github-hosted-windows`（run `30602046108`）的双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier 全部满足后，于 `2026-07-31T03:56:21.4933374Z` 完成 promotion。

当前 request 使用 ACL 收紧的 `C:\qf8` 与显式 UTF-8 本地 worker，并且只 dispatch 一次 GitHub run `30602046108`。helper 一次瞬时 GitHub API EOF 后按同一 run id fail-closed resume；artifact 首轮传输超时也只恢复同一成功 run 的可续传下载，没有重新 dispatch。最终本地与云端有效证明精确绑定同一 source tag/commit/tree、build identity 与 payload closure。上一 `f01f4b…` no-AS2-fallback request 与更早 portability 观察均只保留为历史。
...
当前正式部署对应 source commit `6f3d50a52413c747b05b74be88d6ee46650f4597`、build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C`、payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0`、Core EXE SHA-256 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD` 与 Core DLL SHA-256 `0CEA0C64C037090ADAB4E9C38294075E58F1D298615DD447677D0D6725A9271E`，包含 F8 Agent Runtime 与此前正式能力。无 candidate id 的正式根入口以 `formal_runtime` 启动并绑定上述 consensus；`tmp/manual-agent-acceptance/formal-f8/agent-runtime-help-20260731T040942Z.json` 取得 `acceptancePassed=true`、57 次 pure-MCP call、帮助面板可见保持 12009 ms、可信 shutdown 与存档不变，`formal-residue-comparison.json` 为 `noResidualDelta=true`，故该窄纵切严格达到 `standard_entry_verified`。native identity / closure 只覆盖正式 runtime 文件闭包；Web/Flash 字节与本轮行为另由冻结 source/release tree、production policy、报告和实际窗口绑定。机器证据要求人类另行观察可见停留，当前没有记录维护者目视签收；本轮也没有物理双屏、“13/13”、Flash pixels/input、Hair/Wings 完整产品、业务写、持久化或重启回读，不能把总体状态外推成这些专项验收。

**PlayerInfo main-space v2 当前状态**：下面保留的旧 F/tag/request/quorum/47-Gate/Kimi 数值均为历史专项证据。qualification 闭包修复后的 F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / r2 列车自身仍是 non-deployment train；其实现字节随后被当前 `6f3d50a…` release 包含并进入 formal runtime。B0 仍为 `b0_accepted`，oracle 仍为 `oracle_frozen_for_b0`；本轮 formal smoke 虽观察 NativeHud WGC，却没有启用 PlayerInfo fixture、观察真实 `pi_*` 或执行 PlayerInfo-specific standard-entry E2E，因此不能从总体 runtime 发布状态外推 PlayerInfo 专项验收。
