...
- 外交地图入口按原版绿色点直达；副本/委托任务入口通过 `openWebDungeon` 重定向 Web `tasks` 面板副本 tab
...
- 外交地图直达不能弹二次难度选择；副本任务必须重定向 Web `tasks`，不能误当普通关卡进入
...
- 锁定关卡不可进
- 挑战模式只显示/允许地狱
- 长文本 hover 不露 `<BR>` / `<FONT>`；任务节点 hover 时红色标记与黄色脉冲不消失，局部卡片难度按钮无需先打开固定检查器即可直接点击
