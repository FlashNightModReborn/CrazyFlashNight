...
**斗兽标定生产目录（2026-09-02，已正式部署）**：`gate-f-week-full-v9` 已闭合 195 个 completed shard / 3,230 条 selected durable row，33 个默认生产 profile 兼容 candidate 去重为 55 个 exact roster；`G2` 两组按真人 PVE 固定为 `Lv10 / arena-2`，`B11/C11/C7` context-only，21 个 timeout candidate provisional，`D10/B12` quarantine。生产目录只接收 Host session-scoped id，不写 faction `benchLevel`。合并后 exact bundle `sha256:6ed3c7461330657f4770abc454b9c6ccdf280c5c2190f347d798c19fa5f21b43` 已在 revision `06e554e762d3bae83ccd20c264c78e7fa74ec92c` 上取得 `APPLIED_VERIFIED`；applied SHA-256 为 `sha256:f9bfb2d2496da9648b5bbe4227c3fb0e5828c3456b16b9115a76456eb98d41e5`，receipt hash 为 `sha256:8f6b152a3289c72e3d20bd8cb723f3d7dcec37a85a747e159280fe4ea6d56a68`，Host 7/7、Arena browser 36/36 与文档治理通过。随后本节所述目录已由当前正式列车完成双构建、39/39 policy、promotion、部署推送与 post-promotion Audit；尚未执行部署后的 Arena 正式入口业务复验，因此不称业务 `standard_entry_verified`。
...
测试员采集返回基地焦点问题时，双击根目录 [焦点诊断启动.cmd](../焦点诊断启动.cmd)。它通过正式 `start.ps1` 入口启动，在本轮进程开启有界观察，正常退出后把日志与版本身份打包至 `logs/focus-diagnostic/*.zip`；途中可双击 [收集焦点诊断日志.cmd](../收集焦点诊断日志.cmd)。观察上限从进程启动起算 30 分钟，普通启动默认关闭。保留一次失灵及原有恢复办法即可，详见[焦点观察契约](../docs/焦点管理-诊断与卡顿排查-2026-05-24.md#910-2026-09-05物理手势到-as2-结果的有界观察)。
