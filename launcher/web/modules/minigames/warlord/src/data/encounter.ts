...
} as const satisfies EncounterDefinitionAuthoringV1;
...
export const DEMO_1_ENCOUNTER: EncounterDefinitionV1 = validation.definition;
export const DEMO_1_ENCOUNTER_BINDING: EncounterDefinitionBindingV1 = Object.freeze({
  definition: DEMO_1_ENCOUNTER,
  configDigest: ENCOUNTER_CONFIG_DIGEST,
});

export function encounterProfile(
  profileId: EncounterProfileId | string,
): EncounterProfileDefinitionV1 {
  const profile = DEMO_1_ENCOUNTER.profiles.find((candidate) => candidate.id === profileId);
  if (!profile) throw new Error(`Unknown encounter profile ${profileId}.`);
  return profile;
}
