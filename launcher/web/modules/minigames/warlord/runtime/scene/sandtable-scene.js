...
export class SandtableScene {
    ...
    buildTerrain(bounds) {
        ...
        const grid = new THREE.GridHelper(gridSize, Math.min(36, Math.max(16, Math.ceil(gridSize / 1.25))), this.theme.gridCenter, this.theme.gridLine);
    ...
    createNodeLandmark(kind, material, signalMaterial) {
    ...
    onPointerDown = (event) => {
        if (this.disposed || (event.button !== 0 && event.button !== 1))
            return;
