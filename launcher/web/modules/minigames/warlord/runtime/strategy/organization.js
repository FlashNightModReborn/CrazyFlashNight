...
const ARENA_FORMATIONS = new Set(['line', 'column', 'wedge', 'shield', 'grid']);
const SLOT_ROLES = new Set(['vanguard', 'flank', 'fire_support', 'reserve']);
...
function rejectUnexpectedFields(value, allowed, path, issues) {
...
export function validateOrganizationDefinition(input) {
    const issues = [];
    if (!isRecord(input)) {
    ...
    const formationFirstPath = new Map();
    for (const [index, value] of normalizeOneOrMany(input.formationProfiles).entries()) {
        ...
        const arenaFormationRef = typeof value.arenaFormationRef === 'string'
            && ARENA_FORMATIONS.has(value.arenaFormationRef)
            ? value.arenaFormationRef
            : null;
        ...
        for (const [roleIndex, role] of normalizeOneOrMany(value.slotRoles).entries()) {
            if (typeof role !== 'string' || !SLOT_ROLES.has(role)) {
    ...
    const taskGroupFirstPath = new Map();
    ...
    const defaultFormationRef = readId(input.defaultFormationProfileRef, '$.defaultFormationProfileRef', 'formation-profile-reference', issues);
    ...
    const definition = Object.freeze({
        ...
        id: id,
        rulesVersion,
        defaultFormationProfileRef: defaultFormationRef,
