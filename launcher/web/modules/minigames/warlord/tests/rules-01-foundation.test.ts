...
import { CARD_IDS, PRODUCTION_CARD_IDS, getCardDefinition } from '../src/data/cards.js';
import { nodeConfig } from '../src/data/map.js';
import { applyOk, faction, makeState, makeUnit, moveCommand, recursivelySwapFactions, setPlanning } from './helpers.js';
...
test('AC-03 八张普通兵卡与三张唯一指挥官卡的身份语义唯一', () => {
  assert.equal(CARD_IDS.length, 11);
  assert.equal(PRODUCTION_CARD_IDS.length, 8);
  const definitions = CARD_IDS.map(getCardDefinition);
  assert.equal(new Set(definitions.map((card) => card.cardId)).size, 11);
  assert.equal(new Set(definitions.map((card) => card.unitTypeId)).size, 11);
  assert.equal(new Set(definitions.map((card) => card.identifier)).size, 11);
  assert.deepEqual(CARD_IDS.filter((cardId) => getCardDefinition(cardId).tags.includes('commander')), [111, 112, 113]);
...
test('AC-05B 唯一指挥官卡不能绕过 Commander ledger 进入普通排产', () => {
  const state = makeState('ac-05b');
  setPlanning(state);
  faction(state, 'red').gold = 999;
  faction(state, 'red').cards[111].level = 50;
  ...
  assert.equal(result.reasonCode, 'commander_state_invalid');
});
...
test('AC-06 标准开局六个己方节点激活稳定，人口 11、AP 4 且存在首次占领路径', () => {
  ...
  for (const [nodeId, factionId] of initialNodes) {
    assert.equal(requireNode(state, nodeId).activeFromRound, 1);
    assert.ok(isNodeStable(state, nodeId, factionId));
  }
  assert.equal(faction(state, 'red').populationCap, 11);
  assert.equal(faction(state, 'blue').populationCap, 11);
  assert.equal(faction(state, 'red').actionPoints, 4);
  assert.equal(faction(state, 'blue').actionPoints, 4);
...
test('AC-07 状态与地图只有金币；南部军需稳定激活时提供 6G', () => {
  ...
  assert.equal(nodeConfig('South-Depot').goldIncome, 6);
  assert.equal(nodeConfig('South-Depot').population, 0);
  assert.equal(nodeConfig('South-Depot').productionSlots, 0);
  ...
  requireNode(state, 'South-Depot').ownerFactionId = 'red';
  requireNode(state, 'South-Depot').activeFromRound = 1;
...
test('AC-08 新部署棋子当轮不贡献 AP，但可以消耗既有公共 AP', () => {
  ...
  assert.equal(faction(state, 'red').apGeneratedThisRound, 4);
  // Every round is player-first; the freshly deployed piece can spend the already-frozen base pool.
  ...
  assert.equal(faction(state, 'red').actionPoints, 3);
});
