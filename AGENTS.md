...
**最后核对代码基线**：release source commit `b2385ee83ebd2511e501bc2b2fdd6131310aa663`（2026-08-29；implementation `306a3cb1e8140a0f3c86a09e8b7874ef50b9be8a`、tag `runtime-build-v2/20260829-stage-nav-revive-v1`、tree `6a410bd5e80c3d37f23f3554e5f5f86b09d54da7`、request `C64342535E6D8CEBD88D2D359A118C75CEC967071EA00A22CEA4783A769C69E9`、deployment `95dec8e1c770187c57c98be6256ece6260fbed79`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `387EC78270435A3DDF2687648CD0064864C561BC7419DBD48F8C74664F27F298` / `github-hosted-windows`（cloud run `33220240979`）已对 identity `47001CF2926D0E47AF235AF3672BBEE984F69162570167029118FE61791A5B69`、closure `24C95A76AEF22CC40A3EE38F61EB916DF6AD664AF4F5006462452BDD80ED0296` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `B03C2F9453CEA7F29A986421BC7182761DEC59E80ADC1726B628B59690BB984A`，39/39 production receipt SHA-256 为 `3D356FFA7B4D50F500DB69E86E66266AE33FFDFE659177434A85BEC712831772`；deployment commit `95dec8e1c770187c57c98be6256ece6260fbed79` 与首次 post-promotion audit run `33221250923` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车收束了地图绕过关卡生命周期、选关回包跨淡出丢失、死亡复活后普通技能门未恢复、NativeHud 手势/焦点竞争和复活按钮文案越界。维护者已在与正式 Core 逐字节相同的隔离候选上确认原功能矩阵与布局修正版均有效；production policy `39/39`、worktree/index 33-file bundle、signed consensus、原子 promotion、正式根 bootstrap `--verify-only` 与 post-promotion Audit 均通过。
准确状态为关卡转场与复活修复 `HUMAN_ACCEPTANCE_PASSED / promoted`。部署后只执行了根 bootstrap 完整性验证，没有从无 candidate 的正式入口重跑选关、死亡复活、返回基地或存档旅程，因此不称本功能业务 `standard_entry_verified`。斗兽 Gate F campaign 与 Audio H2 继续保持各自独立门，不能由本次通用 runtime promotion 代签。
