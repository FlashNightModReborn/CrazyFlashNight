...
- **capture / evidence boundary**：loopback capture 必须先由 capture process 在 `IAudioClient.Start` 后写 `ReadySignalPath`，再允许 stimulus；runtime tuple/`f32` 与 endpoint WAV `pcm_s16le` 分别绑定。observer/stimulus pipe 使用无空白 compact canonical UTF-8 JSON；capture configuration 是递归 key-sort、two-space indent、terminal LF、UTF-8 no-BOM 的 tracked artifact，operator 必须用独立 comparator 校验，禁止拿 pipe wire canonical bytes 代验 artifact。toolchain writer 用 pinned runtime environment gate 生成 canonical Node/MSVC/.NET/PowerShell/cmd/vcvars 描述，隔离 runner 只接收 hash-bound absolute `CF7_NODE_EXE`，不继承 PATH；materializer 的 SFX `linkageId` 必须与 production catalog 一样使用包含 `.wav` 的 full basename，禁止 extensionless stem，operator/client 和 endpoint lister 只准备输入或执行资格化。assembler 只生成 9 组配置/输入和 `HUMAN_REQUIRED` drafts，固定 `promotionAuthorized=false`，不生成 E1、H2 或 pass。crossfade 自动证据只证明非零 fade request、相关 `started`、source 替换及相邻 ≤500ms 的连续非静音 meter，不证明 dual-slot gain envelope。
- **当前失败/重入边界**：第一份 S `90985ae…705644` / tree `beaaab2…24a3` 的 candidate `1A675D…6CEB / D5232F…6A1F` 在 run `cdf7…59f64` 的 case 1 因无 AS2 ingress、source playing 但 cursor/meter 为 0 而静音 fail-close。S2 `0360972…2f950` / tree `d605241…ab12c` 的 run `5cf1…f7ec3` 取得真实非静音 WASAPI capture，却因 operator 混用 artifact/wire canonical 格式在配置读取处 fail-close。comparator 修复后的 S3 `176b2a0…c800a` / tree `221901c…2c82` 已由 A/B/live producer 重现 `004443…F2FA / 32FD1E…A71EF`、runtime manifest `ED2DFB…BEC98`；run `a481…c2c7` 的 BGM capture 成功，但 case 7 `sfx_playback` 使用 extensionless plan ID，AS2 ingress 正常且 runtime tuple 稳定，native `unknownId 0→1 / played=0 / meter=0`，暴露 materializer stem 与 production full-filename catalog key 失配。三个 candidate 均已退出，无 E1/H2，三个 runId 与 capture 均作废且始终 `NOT_DEPLOYED`；第三次 candidate 的 shadow/canonical save 与 baseline 逐字节一致。本段所在版本与 full-basename/dense-overlap 回归、dependency/doc closure 首次同树落盘的 clean implementation commit 定义为新 S，commit/tree 由提交后的 Git object 解析；该 exact S 仍必须重新双 clean repro、构建新 candidate 并从新 runId 重跑完整 A6。当前 fresh source 门为 runner `28/28`、materializer `4/4`、operator `9/9`、observer `20/20`、C# stimulus `16/16`、Audio namespace `201/201`、updater `5/5`；Launcher canonical full 为 `3529 passed / 0 failed / 3 existing explicit skipped / 3532 total`，SDK resolver `7/7`，parallel collections off。Audio fresh runId `5666cb1002684540b003f7cc1141dd01` 为 `185/185`、Compiler `0/0`、32K retry `0`，TestLoader SWF `438436` B；publish-only `scripts/asLoader.swf` 为 `1064688` B / SHA-256 `C52F52CEE90E57D79F6ACBE6464DD9472034F1A49D0DD958297953B94266063E` / blob `91188da3684523bd0ccf4dd2b4c2231e1be5b818` / `9748` functions / max `46025` B，Compiler `0/0`。qualification dependency 为 `568` 项 / `142053` bytes / closure `3655FEC5DA57619EA38F71C6DE19C2A4648AD0DAF515ED0875BB66DAB4EC7B81` / manifest SHA-256 `A26D5282F6F40122437DF6AD090503976AE1F9D356B3E7C80D6267A0E6C0730E`，双 `--check` 已通过。当前 source 状态与 promotion 边界见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)；Launcher 细节以 [launcher/README.md](../launcher/README.md) 为准。H2 尚未取得，因此严格为 **H2 blocked / NOT_DEPLOYED**。
