...
import type { HelpAnchor } from './help-profile.js';

export const PLAYER_TEXT_SCHEMA_VERSION = 'warlord.player-text-catalog.v1' as const;
export const PLAYER_TERMINOLOGY_VERSION = 'warlord.player-terminology.zh-cn.v1' as const;

export interface PlayerReasonText {
  reason: string;
  nextStep: string;
...
export interface EncounterDistanceText {
...
} satisfies Record<EncounterDistanceBand, Readonly<Omit<EncounterDistanceText, 'assistiveText'>>>);

type ReasonFormatter = (params: ValidationReasonParams) => Omit<PlayerReasonText, 'assistiveText'>;

function numberParam(params: ValidationReasonParams, key: string, fallback = 0): number {
  const value = params[key];
  return typeof value === 'number' && Number.isFinite(value) ? value : fallback;
}

function stringParam(params: ValidationReasonParams, key: string, fallback: string): string {
  const value = params[key];
...
} satisfies Record<ValidationReasonCode, ReasonFormatter>;

export const ALL_VALIDATION_REASON_CODES = Object.freeze(
  Object.keys(REASON_FORMATTERS) as ValidationReasonCode[],
);

export function playerTextForReason(
  reasonCode: ValidationReasonCode | undefined,
  params: ValidationReasonParams = {},
): PlayerReasonText {
  const resolved = reasonCode
    ? REASON_FORMATTERS[reasonCode](params)
    : { reason: '当前操作暂时不可执行。', nextStep: '请返回上一步重新选择。', helpAnchor: 'overview' as const };
  return { ...resolved, assistiveText: `${resolved.reason} ${resolved.nextStep}` };
}

export function playerReasonSummary(
  reasonCode: ValidationReasonCode | undefined,
  params: ValidationReasonParams = {},
): string {
...
export function playerEncounterDistanceText(
  distanceBand: EncounterDistanceBand,
): EncounterDistanceText {
  const copy = ENCOUNTER_DISTANCE_TEXT[distanceBand];
  return Object.freeze({ ...copy, assistiveText: `${copy.label} · ${copy.impact}` });
}

export function playerFactionName(factionId: FactionId): string {
  return factionId === 'red' ? '我方' : '敌方';
}

export function playerOwnerName(factionId: FactionId | null): string {
  return factionId === null ? '无主' : playerFactionName(factionId);
}

export function playerBehaviorName(behaviorId: string): string {
...
export function playerPowerTierName(powerTier: string): string {
...
export function formatMilitaryFunds(value: number): string {
  return `军费 ${value}`;
}

export function formatActionPoints(value: number): string {
  return `行动点 ${value}`;
}

export function formatStrategicRound(value: number): string {
  return `第 ${value} 回合`;
}
