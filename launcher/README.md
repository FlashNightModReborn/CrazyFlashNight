...
> **PlayerInfo 当前施工边界**：historical v1 的 31/31、47/47、视觉 A/B、旧 tag/quorum/preflight 与 Kimi PASS 只批准旧字节，始终 `NOT_DEPLOYED`。main-space v2 首列实现位于 clean merge `40853287e7ed04714d68935c0002f8ad6d8aea05`；其 B0-05 为 32/32（run `c42d755d…`，报告 738,872 B / SHA `3C738AC2…1C514`），B0-06 为 48/48（run `ed61d727…`，报告 653,904 B / SHA `182C7DC9…CA8E1`），完整 Launcher 为 1,747 pass + 3 opt-in skip / 1,750。正式 `visual-evidence.v2` 的两份 C# 各 `36 files / 35 PNG / 1,261,624 B / DD58CA7C…6661E`，两份 Web 各 `12 files / 11 PNG / 221,781 B / 9D21D823…17A7B`，两份 direct 各 `66 PNG / 3,114,450 B / F05AE7F7…50ABF`，viewer 为 `651,451 B / 2F71A3ED…01BC2`；MP label/current/maximum/percent 都是 unique `bestDx=0` 且对应 anchor edge `dx=0`。真实 Flash v2 candidate 已按 1024×576 main-stage、exported-symbol extraction、wrapper=false 捕获 11 case且 strict tooling identity 通过，但四项 human review 为空，只能记 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`；自动包也没有 parity verdict。最终 Kimi 已在 clean `884fbf43…` 上完成 165/165 个 `kimi-code/k3 + high` request，`K1_AUTOMATED_GATE=PASS`、P0/P1=0。首列 v2 tag/q3 本地 X509 票有效，但 production policy 仅 22/23，唯一失败是隔离 qualification 项目漏挂 `PlayerInfoRasterPlan.cs`；该列车已因 policy 门退役，未启动 cloud/quorum/preflight。修复后 production contract、renderer 12/12 + 78/78 + 8/8、合同 8/8 与 Launcher 1747+3/1750 均通过；最终 F2 仍须以全新 `…-b0-v2-r2`、q4/w4 fresh builder 和 `-VerifyOnly` 收口。Computer Use 实际初始化与人类签收也仍待执行，因此总体状态仍为 `visual_correction_in_progress; human_exit_blocked`，并非“只剩人签”。fixture surface 继续只接 allowlist、不注册现有 NativeHud union、不接 UiData/真实 `pi_*`、不隐藏或修改旧 Flash HUD。workspace/terminal unrestricted 与 skill 已暴露都不代表 Apps 控制后端已初始化；FFDec/headless Playwright/Edge/自动 HWND 不能代签真实桌面、游戏 composite、鼠标透传、审美或人类接受。没有 promotion、release-state 或标准入口部署变更。exact runner、Kimi 证据与生命周期边界见 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)和[验证矩阵](../agentsDoc/testing-guide.md)。
