...
- [ ] v1 两类正式 builder/F/quorum 事实仍按历史保留。main-space v2 源码与上游已冻结到 clean merge commit `40853287e7ed04714d68935c0002f8ad6d8aea05`，但新 tag/request、双 signer/双 faultDomain 一致 build identity/payload/Core 与不可复用 promotion preflight 尚未形成。cloud config 已预留全新单段 source ref `refs/tags/runtime-build-v2/20260729-player-info-nativehud-b0-v2`，但 tag 当前不存在；它只能在最终 build source 上创建一次，旧 v1 tag 永不移动/复用。新报告仍须明确零 runtime/release-state/promotion/deployment 变更；旧 `7687B561…7A9F` / `AF2E9E57…724B` / `A45471CF…B652` 不得代签。
...
| B0-06 | `main_space_v2_automated_qualification_passed; builder_human_pending` | clean merge `40853287…` 的 formal run `ed61d7275fdd4a96a73f762c5c2a4d71` 为 48/48、**653,904 B**、SHA `182C7DC9…CA8E1`；clean full 为 1747 pass + 3 opt-in skip / 1750。两份 C# 36-file、Web 12-file、direct 66-PNG 与 viewer 自动包均确定；MP 四字段 unique `bestDx=0` 且 anchor edge `dx=0`。prepared top-down PArgb DIB/DC、真实 ULW transaction 与 lifecycle/source/binary/renderer closure 均受报告约束；§9.7 当前 K1 已 PASS | 新 tag/request/双 builder quorum/preflight、Computer Use 实际初始化、Flash oracle/真实桌面与全部人工项仍待完成；自动图像/Kimi 只作 diagnostic/audit，不批准 parity/部署 | 先闭合新 build，再做 Computer Use/真实游戏/IDE 身份和人签；新版本不得复用或移动 v1 tag |
...
> v1 工程轨曾冻结 F=`cb38600…f511` 与不可变 tag，并由 clean F Lane、两个正式 builder、production receipt、`-VerifyOnly` preflight 和 clean-G K1 证明旧字节；这些事实保持历史有效，但不得外推到 main-space v2，也不得移动/删除 v1 tag。当前 v2 source F=`40853287…` 与 evidence/docs G=`884fbf43…` 已形成；clean B0-05 32/32、B0-06 48/48、full 1747+3/1750 以及两份 C#/Web/direct/viewer 自动包均已形成，MP 四字段都是 unique `bestDx=0` + anchor edge `dx=0`。§9.7 的同一 Kimi session 已在 `k3/high` 完整审计到 PASS，P0/P1=0；它不代签人类项目。真实 11-case Flash candidate 仍只有 strict tooling identity，human review 四项为空，所有自动图像都只是 diagnostic。恢复时先 fetch 并集中整合无交集上游，再按 F 创建全新 v2 tag/request，完成双 builder/双 faultDomain quorum 与不可复用 preflight。随后初始化 Computer Use/Browser 实际控制，并核验旧 Flash 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实桌面手点、运行时 SWF/IDE Output 身份与审美人签。当前总体状态仍为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`；B0 不要求 promotion、标准入口或部署，build/人签全部完成前不得写成 parity、`oracle_frozen`、`awaiting_human_acceptance` 或 `b0_accepted`。每完成一片都在同一提交回写 §12。
