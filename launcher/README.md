...
| 5    | `dotnet publish ... -c Release -r win-x64 --self-contained false -p:DebugType=embedded -o $publishDir` — 出 FDD apphost (`Core.exe`) + 17 个 DLL + `Core.deps.json` + `Core.runtimeconfig.json`，~37MB 散落形态 |
| 6    | 拷贝产物到 `projectRoot\runtime\`：(a) `publishDir/*` 除 `*.xml`（含 Core.exe 全套） (b) `bootstrap.exe` → projectRoot 改名 `CRAZYFLASHER7MercenaryEmpire.exe`（用户面入口） (c) `sol_parser.dll` + `miniaudio.dll` from `bin/Release`；拷贝走 `Copy-IfDifferent`（SHA256 比对，相同跳过 → 可复现构建、不污染 git）+ 定向清理过期产物；**硬断言 `CRAZYFLASHER7MercenaryEmpire.exe` + `Core.exe` + `Core.dll` + `sol_parser.dll` + `miniaudio.dll` 全部落盘**（任一缺失 exit 1）；bundled runtime installer（`tools/dotnet-runtime/windowsdesktop-runtime-10.*-win-x64.exe` glob）缺失现为 **fail-fast exit 1**（旧版只 WARN） |
| 6f   | 编译产物优化护栏 — `dotnet run tools/assert-optimized.cs -- runtime\...Core.dll` 用 PEReader 读 `DebuggableAttribute`，断言发布的 managed assembly 是 optimized（非 Debug）build；Debug DLL 漏进 `runtime\` 是历史性能回归根因，命中即 exit 1 |
...
- **Stage Select manifest / audit / harness**：`node tools/export-stage-select-manifest.js --summary`、`node tools/audit-stage-select-layout.js --json`、`node tools/audit-diplomacy-stage-select-links.js --json`、browser harness `web/modules/stage-select/dev/harness.html?qa=1`（in-page `dev/qa-suite.js`，约 33 用例；无 Node CLI runner）
