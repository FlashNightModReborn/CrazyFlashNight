...
import { auditEncounterState, frozenStateHasAnyEncounterFields, } from '../core/encounter.js';
...
import { auditOrganizationState, createOrganizationRuntimeState, registerOrganizationMemberInPlace, } from '../core/organization.js';
...
export const AS2_BATTLE_RECEIPT_SCHEMA = 'warlord.as2-battle-receipt.v2';
...
function projectStateForAs2BattleHandoff(value) {
...
function assertFrozenEncounterState(state, label) {
    const raw = state;
...
function normalizedFrozenState(value) {
    const state = clone(value);
    const raw = state;
...
export async function buildAs2BattleEnvelope(input) {
    ...
    if (requestBytes > AS2_BATTLE_REQUEST_SOFT_LIMIT_BYTES) {
        throw new Error(`AS2 战斗请求为 ${requestBytes} 字节，超过 Web 侧 ${AS2_BATTLE_REQUEST_SOFT_LIMIT_BYTES} 字节软上限。`);
    }
    return {
        type: 'panel',
        panel: 'warlord',
...
function battleSnapshot(state, pieceId, encounterUnit) {
    ...
    if (!cardState)
        throw new Error(`冻结战略态缺少 ${piece.factionId}/${piece.cardId} 的兵种状态。`);
...
function normalizeUnits(raw, expectedIds, expectedFactionId, state, label) {
    ...
    for (const candidate of raw) {
        ...
        const cardState = requireFaction(state, piece.factionId).cards[piece.cardId];
        if (!cardState)
            throw new Error(`${label}单位缺少兵种状态：${pieceId}。`);
...
function applyAcceptedReceipt(request, receipt, frozenState) {
    ...
    if (defenderIds.length === 0)
        throw new Error('恢复时冻结战略态缺少防守单位。');
    ...
    if (winner === 'attacker') {
        movePiecesInPlace(state, attackerIds.filter((pieceId) => state.pieces[pieceId] !== undefined), command.targetNodeId);
    }
