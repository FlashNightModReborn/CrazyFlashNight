...
**最后核对代码基线**：玩家物资事务与双向播报 release source commit `cf2fbd7b07fde5e008675fa1e6bda725bdfb96dd`（tag `runtime-build-v2/20260822-player-asset-transactions-v2`）、release tree `08b17b9ca00a437dc6fe4dbf0116dcbccbaa680d`、request `DD030AC6C7D780FBECC827D11828BD4959191AF182E46921CB480E3B08D4B8FD` 已由本地 X509 `builder-local-b` / `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `32548053397`）完成 v2 双 signer/双故障域 promotion；正式 runtime 绑定 identity `6CD9AEB99B3B5A3DF470F1CDDD99DF11CF7F706394F9FD3AF6D5594ABC03BC78`、closure `60EA3AACDBFAAE178866317448FCEA0BEC0D3AA73E808AD4952ACF695571F62D`、Core DLL SHA-256 `E0A7B31C70DA564ECCA43B8B58DDA2F6443553112DD95B6EE05182FB94F78474`，38/38 production receipt SHA-256 `4A5FC71CE10EBBBE4BDD487F34A0EA3A7FC9D1939E815544CE50F15A719E5546`；promotion commit `b7f075e53c31c5d4b67e02dcda1978695321d33d` 与 post-promotion audit run `32548688437` 均已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本轮玩家物资事务已完成维护者对同 Core 字节隔离候选的人工验收，确认双向播报、换弹 loss 聚合与同物品买卖全过程有效；Launcher 4014 pass + 3 explicit opt-in skip / 4017 total、focused Loot/TaskRegistry 196/196、PlayerAssetTransaction 61/61、受影响 SWF fresh publish 与 Compiler 0/0 均通过。正式 runtime 已 `promoted`，但部署后未重跑无 candidate-id 的业务标准入口，因此只称 `HUMAN_ACCEPTANCE_PASSED / promoted`，不称 `standard_entry_verified`，也不外推为真实玩家存档重启或全游戏 E2E。上一设置专项的单屏 `settings_camera_preview` 窄纵切仍保持 `standard_entry_verified`；字体 Gate E 仍保持 14 asset / 14 face / 28 role / 9 preset 与 `GATE_E_COMPLETE / promoted`，Audio H2 继续作为专项 `pending`。
