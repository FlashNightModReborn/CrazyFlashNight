...
**最后核对代码基线**：commit `9118eb5097ab073d26a9806138f9fabf28e3ca79` 已由 tag `runtime-build-v2/20260730-workbench-character-build-v1`、request `F5992FE5AFA3B74024CACEFCA1BACD311C1A3EE7C50CF3D08145A3E49BC211BC` 完成 v2 promotion；正式 runtime 绑定 identity `EB60E241929B5F88110C4EAE218DFD98569AE657F2B765179DBF644F0EEE0255`、closure `889FC7A800CFE738EAA99992CD6C5689AA65ECFEF3F406A617B3A1A344F4520B` 与 Core `9DE1C5249EA5827AB8CE7C19CAE0CAE8724809BE2BC7DE4F600AD2F7AB78F336`。标准入口 attempt `9539e5f3f6d44b7daf487d8985465972` 达到 `standard_entry_verified`，但范围仅为 production `EQUIPMENT_TUNING` opener → exact workbench instance → 同实例首个权威 snapshot → supported application shutdown；此前 `c4faf14460…` 的双向 GUI 与普通关闭证据仍是独立历史身份，不能拼接扩张本次验收范围。

2026-07-29 的 B7 施工从 commit `c96f4c3d750561022b706c72a4d53050431e627d` 起步，后续实现现已进入 `9118eb5097…` 冻结源码与配对正式 runtime。下文保留的 `c6ca847…` / source-ahead / `NOT_DEPLOYED` 记述是 promotion 前施工与自动门历史；当前发布事实必须同时附带上段只读 snapshot 验收范围，不能把“字节已部署”改写成 Character、Materials、Intelligence、PlayerInfo 或写入旅程已实机验收。
...
滚动部署只为 Skills opener 保留一条临时单向兼容：新 AS2 在 `skillPanelOpen` 参数对象存在但缺少 `openRequestId` 时，仍可为尚未更新的旧 Host 发出不带 nonce 的 `nativehud/manage`；显式畸形 token 继续零发送。旧 Host 可按既有路径打开“我的技能”，而新 Host 永远发送 nonce，且其 typed capability 对无 nonce 回包继续 fail-closed，因此这条分支不能被 Character 交接、教师请求或 foreign source 借用。它明确不适用于上述 workbench tuple；workbench 必须配对 candidate。`9118eb…` 已满足正式 Host 与 `asLoader.swf` 配对部署的退役触发条件，但 exact source 仍保留该分支；必须在后续独立 source/release 删除，并继续保留“旧 Host 可开 manage / 新 Host 拒绝无 nonce / trainer 不得携 nonce”三类回归，不能把本次 deployment 写成兼容已移除。
