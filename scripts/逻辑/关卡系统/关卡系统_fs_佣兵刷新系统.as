...
// 注意：原 loadMoreByExpression 按表达式分段调 N 次回调。当前唯一用例 1 段，
// 改成 refreshPool 后回调只调 1 次（无外部调用方，简化无观测变化）。
...
// SaveManager + DOMDocument 用 _root.载入新佣兵库数据(0,0,0,0,0) 作"触发完整重载"。
// 前 3 个 numeric 参数从 Step 5 起就没意义，5 参签名保留兼容外部调用方。
