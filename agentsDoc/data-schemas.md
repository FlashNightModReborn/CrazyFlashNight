...
| `MapPanelLoader` | `data/map/map_panel.xml` | WebView 地图面板的 groups/hotspots/avatar_visibility（启动失败硬错、不做 fallback；Catalog 的 canonical 白名单精确校验在代码侧）。`task_npcs/aliases` 已迁出本文件，真相源 = launcher/web/modules/map-panel-data.js 的 staticAvatars+dynamicAvatars，AS2 通过 `DataQueryTask("task_npc_registry")` 启动期拉取 |
...
<map_panel>
  ...
  <!-- task_npcs / aliases 已迁出本文件，见下方 ## task_npc_registry.json schema -->
  <avatar_visibility>
...
**硬约束**（`MapPanelCatalog.applyFromXml` 在加载时强制校验；失败 → trace + **先无条件 reset 所有表为安全零值、isLoaded 复位为 false** + 返回 false。同时 `MapPanelLoader.load` 对"结构上无效但非 null"的 XML 会调用 `clearCache()`，防止基类缓存污染后续 reload）：
...
- hotspot.group 必须命中 groups 里声明的 id
- task_npcs 段的校验已迁到派生脚本：name 不重复（大小写折叠）+ hotspot 命中 Catalog + alias.canonical 命中 npc，详见 `tools/derive-task-npc-registry.js`
...
- 新增任务 NPC 只需在 `launcher/web/modules/map-panel-data.js` 的 staticAvatars/dynamicAvatars 加 entry，build.ps1 Step 1b 自动派生 `data/map/task_npc_registry.json`；不再需要回写 XML 或更新 AS2 REQUIRED 列表
