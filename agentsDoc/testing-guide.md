...
| Launcher Web / Task Panel | `node tools/run-tasks-harness.js --qa`（task-ui1~12，Edge headless 跑筛选/列表/排序/详情缓存/富物品 tooltip/空态自断言） | 改 AS2 `taskSnapshot/taskDetail/tasksTooltip` 或 C# `TaskTask`/`WebOverlayForm` tooltip 路由时追加 `launcher/tests/run_tests.ps1` + `scripts/compile_test.ps1`；视觉/动效复核 `node tools/run-tasks-harness.js --shot=<png> --query="view=list&filter=副本&detail=6"`；正式 runtime 富 tooltip/筛选/动效手感需游戏内端到端手测 |
