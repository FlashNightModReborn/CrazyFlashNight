...
当前证据已覆盖自动门和精确候选实机。`node tools/test-agent-entry-contract.js` 锁定 helper 与正常入口的同包 `s:1|ga:<attemptId>`、notifier fail-closed 和 `gotoAndStop("读盘")`；装备和斗兽 runner 的 `--check` 覆盖 post-watermark 真实 title-frame marker、watchdog 拒绝、`title_frame_not_observed`、fresh handoff、exact slot/attempt、single enter 与 attempt-bound `gameEnteredObserved`。Launcher source-tree 全量 **1220/1220**，其中 AgentControl 定向 **29/29**；装备 runner `--check` 为 **34 checks**。隔离 candidate `tmp/runtime-candidates/v2/c-2a0cddb077b7-08846e81b3-20260721t100014612z-f32a40a3`（build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`、payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`、Core SHA-256 `0F58BF864B8DE9C7FCEA098D7E1EEA1996BDE38D85D87E844B047B53F5247232`）已精确启动并载入专用槽 `cf7_agent_loot_target_full_v1`。attempt `62f04fa249c44d94ba3171f619e61754` 严格取得 start 前 `/logs` 水位、start 后 fresh handoff、真实 `[LaunchFlow] bootstrap_reveal_ready: Flash reveal cleared`（watchdog **0**）、唯一一次 helper、同包 `s:1|ga:<exact attempt>`、Host `gameEnteredAttemptId==expectedAttemptId` 与最终 `readyForRuntimeAutomation=true` / `blockers=[]` / `saveRuntime` exact loaded。人类确认 NativeHud 可见且其指令有效，Host status 同时显示 `activePanel=stage-select`。同轮完成正常 NativeHud 入口与同实例 organizer 主链（满背包混合箱→真实确认丢弃牛肉罐头 ×16→fresh ACTIVE 返回且无 close/rebind→继续领取→terminal close/unpause），并完成普通主页单次 X→suspended→同锚点强化石 **×2 + ×3** 内容不变重开且无旧 AS2 UI→两次 claim→terminal close/unpause。该 candidate 的 25-file native bundle 不含 `launcher/web`，WebView2 从 `projectRoot/launcher/web` 读取工作树，因此 native identity/closure 不能绑定本轮 modal/Web 修复；外部 Web 哈希见测试指南。本轮单 canary 资源箱切片达到 `e2e_verified / NOT_DEPLOYED`；候选已优雅 shutdown，Guardian PID **14820** 与 HTTP/XMLSocket **1192/1924** 均退出，Flash CS6 PID **20444** 保留。promotion 与 standard-entry 仍未完成。
