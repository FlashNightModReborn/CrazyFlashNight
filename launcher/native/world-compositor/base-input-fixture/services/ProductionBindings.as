...
// SOURCE: scripts/逻辑/单位函数/单位函数_fs_aka_玩家模板迁移.as
...
/*防止被自动格式折叠
   ...
   {技能名:"觉醒霸体", 点数:150, 冷却:20, 消耗:60, 限制:30, 距离min:0, 距离max:400, 类型:"增益", 功能:"霸体"},
...
_root.刷新人物装扮 = function(目标) {
    ...
    // 修复：不进入"刷新装扮"帧（避免播放到frame 500导致hp归零）
    // 直接在脚本中完成"刷新装扮"帧的意图：清除格斗架势
    目标人物.格斗架势 = false;

    // 跳回安全帧：容器态必须回到"容器"，普通态回到对应状态
...
人形怪技能表.push({技能名: "觉醒霸体", 点数: 150, 冷却: 120, 消耗: 60, 限制: 30, 距离min: 0, 距离max: 400, 类型: "增益", 功能: "霸体"});
...
_root.主角函数.初始化可用技能 = function() {
    ...
    // 兼容分支：装备生命周期（如九命猫妖初始化）在本函数之前已提前注册 已学技能表 → 跳过随机生成，
    // 防止提前注册的技能表被随机覆盖。原随机业务对未提前注册的单位完全不变。
    if (this.已学技能表 instanceof Array && this.已学技能表.length > 0) {
        return;
    }
...
_root.主角函数.死亡检测 = function() {
    ...
    if (用户ID != undefined && 已删除 != true) {
        ...
        if (是佣兵) {
            ...
            // 旧 Flash 佣兵信息界面 已退役不实例化（佣兵管理迁 web 战队页）；此处刷新调用为死 no-op，删除。
...
DrugInputService.installRootBridge(_root);
...
_root.主角函数.释放主动战技 = function() {
    ...
    // SP 消耗：目前唯一吃 SP 的战技/技能入口。原子释放战技自行预检与提交，框架不代扣。
    var 消耗sp:Number = 当前战技.消耗sp > 0 ? Number(当前战技.消耗sp) : 0;
...
_root.主角函数.装载主动战技 = function(战技信息, 攻击模式) {
    ...
    // SP（技能点数）消耗：战技数据里可写 <sp>n</sp>，缺省 0 = 不吃 SP。
    // 目前只有「天启大封印」使用；余额校验与扣除统一在 释放主动战技 里处理。
    var sp消耗信息 = 战技信息.sp == undefined ? "0" : String(战技信息.sp);
    当前战技.消耗sp = sp消耗信息 > 0 ? Number(sp消耗信息) : 0;
    // 当前战技.伤害参数 = 战技信息.damage > 0 ? Number(战技信息.damage) : 0;
    var 战技等级信息 = Number(战技信息.level);
    当前战技.技能等级 = (!isNaN(战技等级信息) && 战技等级信息 > 0 && 战技等级信息 <= 10) ? 战技等级信息 : 1;
...
_root.计算AI参数 = function(p:Object):Void {
    ...
    // 技能 commit 帧数：帧计数锁的最小执行窗口（主保护靠 self.状态=="技能" 动画锁）
    // 高勇气→更长窗口（坚持完成动作），基准比 attack 高一档
    p.skillCommitFrames   = 8 + Math.round(p.勇气 * 8);     // 8~16帧
...
// SOURCE: scripts/逻辑/单位函数/单位函数_fs_装备生命周期配置.as
...
// SOURCE: scripts/逻辑/单位函数/单位函数_fs_装备引用配置.as
...
// SOURCE: scripts/逻辑/单位函数/单位函数_lsy_主角行走状态机.as
