...
>
> **代码基线**：编译基建研究检查点 `fd6e3e6e424d69319e867a5d0dd31887f38df664`；2026-07-19 角色构筑投影补充另核对至 `f06021a911d49824a018fa48071dc5b99c90c548` 及当前工作树。
>
...
当前不建议开工。原因不是前景差，而是项目中的业务 authority 与 UI 迁移仍在快速演进；编译器只有在多个稳定模块需要双端实现时才会摊薄成本。2026-07-19 新增的“角色构筑属性投影”是一个真实且高收益的候选样本，但其输入 schema 和装备迁移协议尚未稳定，单一样本也不足以越过 §11 的开工门槛。现在适合冻结问题、子集、样本、路线和启动门槛，等业务稳定窗口再复核。
...
| ClearScript V8 / TS | 当前只保留搓招 `GameInput` DFA 窄域；由 Flash 帧消息驱动 | 不是通用业务运行时，也没有存档/物品提交 authority；伤害数字已迁至 C# |
...
- 所有访问经过一个 `_lock`；
- 只加载历史兼容文件名 `dist/hit-number-bundle.js`，其现役内容仅为 `GameInput`；
- API 面只有 `GameInput`，旧 `HitNumber` namespace 已删除；
- [`FrameTask.cs`](../launcher/src/Tasks/FrameTask.cs) 只把输入 payload 交给 V8；camera/hit 段由独立 C# reducer 处理；
...
| 角色构筑属性投影 | `PlayerInfoProvider` 当前读取 live hero；Web 已有纸娃娃，装备槽迁移计划采用批次活体刷新 | **拆 adapter 后中高** | base/loadout → build stats 是强双端样本；MovieClip、临时 buff、穿脱提交和存档禁止进入 portable core |
| Stage Select 整体 | Web 布局 + AS2 snapshot/enter + Host 生命周期 | **拆分后才可移植** | 只共享解锁/选择纯规则，不能“编译整个面板” |
| 伤害数字 overlay | C# reducer/layout/painter 专属，AS2 只发段事件 | **不适用** | 已无 TS/AS2 双端需求，不应为了展示 compiler 而反向生成 |
...
### 4.1 角色构筑投影：短期批次刷新，长期同源纯核

装备槽、旧物品 Hub「个人信息」详情与 Web 纸娃娃的联合迁移，为本基建新增了一个明确业务场景。当前 `PlayerInfoProvider.populatePlayerInfo` 通过 `TargetCacheManager.findHero()` 直接读取 live `MovieClip`；而 `_root.刷新人物装扮` 还会清理射击状态并重新初始化单位。因此短期路线不在每次候选变化时计算完整 live 数值，而把页面拆成互斥的 `edit/stats`：编辑态只做 Web 纸娃娃与物品级差值，已确认装备/调制提交只置 dirty；切入数值态或关闭前才按 revision 最多刷新一次活体。该路线无需等待编译基建。

长期可移植的目标不是 `PlayerInfoProvider` 整类，而是拆成三层：

```text
AS2 CharacterBuildInputAdapter
  live hero / 存档 / 装备容器 → versioned CharacterBuildInput
                                  ↓
                     Portable CharacterProjectionKernel
                         ├─ JS：Web hover/select 实时属性预览
                         └─ AS2：权威复核、fallback 与差分 oracle
                                  ↓
AS2 Loadout/EquipmentTuning services：验证、提交、dirty、存档与活体副作用
```

portable 输入可包含基础人物属性、规范化装备记录、强化/进阶/插件、永久技能/被动修正和明确的规则参数；输出只包含“脱战构筑属性”，例如最大 HP/MP、防御、抗性、重量/速度与武器威力。当前 HP/MP、临时 buff、战斗 MovieClip、动画/射击状态、容器引用、opaque token、穿脱合法性、存档写和活体刷新都留在 adapter/authority 层。旧个人信息里的称号、杀敌数、身高体重等非构筑字段继续由 AS2 snapshot 直出，不必为了“全 TS 化”强塞进 kernel。

“两边公用”指**同一版本化源、数据契约和语义 IR 生成两个 backend**，不表示 Web/V8 持有可变游戏状态。Web 接收只读 `CharacterBuildInput` 后可以在 hover 频率本地计算，但确认装备时仍只发送稳定 item/slot intent，AS2 必须重新验证并提交。协议至少携 `loadoutRevision/schemaVersion/rulesHash/dataHash`；任一版本不匹配时，Web 禁止声称精确实时数值，降级为纸娃娃预览、物品级差值与最近一次 AS2 结算快照。

该样本只有在 `CharacterBuildInput`、loadout/tuning authority 和数值定义稳定至少一个发布周期后才可进入 R1 corpus。它可以与解锁谓词、Objective、DFA、LCG/排序等共同满足 3–5 个真实样本门槛；不能单独成为立即启动编译器的理由。
...
- 选关解锁谓词、Objective、DFA、一个排序/空间 kernel、一个 LCG step；角色构筑输入 schema 若已稳定一个发布周期，再加入一组脱战属性投影，不稳定则继续留在 adapter 研究；
...
角色构筑属性投影增强了这条路线的业务理由，但不改变上述裁决：短期装备页先用 edit/stats 结算屏障和批次活体刷新交付；长期只有在 schema 稳定、版本握手和差分矩阵成立后，才让同源 TS 纯核承担 Web 实时预览与 AS2 复核。
