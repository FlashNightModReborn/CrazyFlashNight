...
最后一条命令会从受保护的单路径段 `runtime-build-v2/<release-id>` source tag 触发固定 cloud workflow，并验证 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 都精确绑定请求的 full commit；随后等待精确 run、安全解压并产出 `$cloud.candidateRoot` / `$cloud.proofPath`。unsigned job 交接 artifact 保留 1 天，失败诊断与 signed 结果保留 7 天；超期未 promotion 就重新 dispatch，不把 Actions artifact 当长期档案。request、队列/CAS、双故障域 quorum、receipt 与 `promote-runtime-bundle.ps1` 的完整步骤以 [runtime v2 发布列车](../docs/runtime-build-reproducibility.md) 为准。在 detached clean commit `0215c03b4594221f02e993bf8d58dd1bc669e815` 上，最近一次完整 Runtime Lane C 复跑为 **11/11 个入口 exit 0**；十个 scalar 套件合计 **566**，guardrails 另报告 `scripts=3 / unsafeCandidateCases=3`。该 Lane 未包含环境 bootstrap `-VerifyOnly`，也不能替代真实双 builder 的 promotion `-VerifyOnly` preflight；这些只证明 runtime/admission 守门回归，本身不产生 candidate identity、runtime proof 或 promotion。本轮材料/商城/战备箱 release 由 request `9AC9E3C9A1BC5162F17881DDB37A6F23C8AAAA9830A808CAF4648EDE60753771` 将 source `7d869c5eadf224953b2c61ec7a6ceee2ca03055f` 冻结到 tag `runtime-build-v2/20260728-material-shop-storage-workbench-v1`，local X509 与 GitHub run `30364763726` 对同一 build identity `3F7887ADF9041DFEDA74EEA26040A3628919A725B04D322FCE667791425F7B44` / payload closure `A9B142A417CF2AC45EA19845144569391FCDEE8DF4235F5279A1EC5F0F9022F2` 完成双故障域 quorum，经 promotion commit `40675cae8264a5330904731b9dd8871a2263723a` 后由 exact candidate 与无参正式入口 GUI 达到 `standard_entry_verified`。功能回归证据统一维护在 [测试指南](../agentsDoc/testing-guide.md)，完整发布记录见 [材料、商城与战备箱关闭证据](../docs/evidence/material-kshop-battlebox-workbench-closure-2026-07-28.md)。任何时候都禁止手工换 manifest、伪造证明，或把单机 candidate 复制进根 runtime。
