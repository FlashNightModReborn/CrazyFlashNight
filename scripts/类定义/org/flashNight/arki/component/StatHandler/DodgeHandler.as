...
class org.flashNight.arki.component.StatHandler.DodgeHandler
{
    ...
    public static function calculateDodgeState(targetMC:MovieClip, dodgeResult:Boolean, bulletMC:MovieClip):String
    {
        ...
        var lazyDodge:Number = targetMC.懒闪避 > 0 ? targetMC.懒闪避 : 0;
        if (targetMC.__titaniumType61) lazyDodge = Math.max(lazyDodge, targetMC.__titaniumType61.getLazyDodge());
        if (lazyDodge > 0)
        {
            ...
            if (lazyMiss(targetMC, calcDamage, lazyDodge))
            {
