...
当前 consensus 精确绑定 request `13EB5AC5F2413D7C1547D5E0486C5ACF8183C0145DE02785F8B1932FE5FE2FEC` 与 release tree `72656dde8da92ee5863f3fc760db8e9f20e07ead`。artifact source 为 `15B50A16AF44E14D5E77CFA0939C3FE33E0E5E7716D72989E6857E4E29337150`、producer recipe `93A007D1BD03C54F3CC257F1E4610354BBBB83C97B635AC28C2B5D393C75868A`、toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD`、policy `FD7736B3CA81F510A8057BE90D0FAD34C2152BEF44F171AE2DCE04FF2AA46083`；build identity `C23D09677F191D6CCF5666E64E64F8CE664738E6F85AB46FDE916CED4F667708`、payload closure `FF6CD703624ECA341C8B52AAB726DA7DEDF3914FBD2306F0712A3BE5D3183C6E`、manifest SHA-256 `7780119AA8E6412EE1529395B1014D2889F502F769ABFF275E198EA9FB0303F4`、promotion 后 Windows worktree consensus bytes SHA-256 `429999BA0603AD9832508D1F84C8CA74AF4F7291FD777F01B9F5C2FB902C57C8`。production policy receipt SHA-256 为 `019D6EC6E41EE7F34E318507A977DE48FB299EA44C116D4A03FBCB08C8AE9172`；本地 X509 signer `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5` / `physical-host-b` 与 GitHub OIDC builder `DB44DFAF3706D707AB5CFDE0309D0396D855F6F00A8AB5DF1C009E04C4E75AB3` / `github-hosted-windows`（run `31928663322`）满足双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier，于 `2026-08-16T05:40:09.2990021Z` 完成 promotion。

当前正式部署的根 EXE SHA-256 为 `55EABB5C3280FDFEE8302843D969C6B6354808F1AF4DC24DB5E2397AA80499D8`（未变），Core DLL SHA-256 为 `6543352D859EFCB4361EB5C90AC8D61AA3A9955D770D958C4596745073287165`，`miniaudio.dll` SHA-256 为 `AC3A8FC3BE306D595918F392B44E298D95B71221311920E892C4BE2A6E3AD20A`（未变）。部署后 33-file bundle、signed consensus 与正式根入口 `--verify-only` 分别通过；post-promotion audit run `31930140127` 精确报告 base `9df1fb8fda…`、head `72f988d4e6…`、`state=promoted`。本轮自动证据覆盖污染夹具、Host/Web 严格采用与拒绝、AS2 fresh NPC suite、发布身份和部署完整性；测试员反馈称事故存档是当时版本从零新建，但该存档现已丢失且测试员机器与开发机物理隔离，因此不能把“仅旧档结构可触发”或任何污染夹具认定为原事故唯一根因，也不声称真实存档业务 E2E、玩家目视签收或 `standard_entry_verified`。下行仍以“当前”描述旧 source / identity 的段落均按历史列车输入阅读，不覆盖本段。
...
promotion 重新验证 request、精确 `request.releaseTreeOid` worktree、receipt、candidate 与所有证明，要求至少两个不同 signer identity + faultDomain 且五项共同产物字段（前三域、build identity、payload closure）全等。产品专项的听感、截图、H1/H2/E3 或其他人工验收证据不进入这个通用事务，也不能由 promotion 反向推导。随后脚本才可在 `tmp/runtime-promotions/` 组装 next/previous，事务替换正式 runtime、bootstrap 与 `config/build/runtime-release-consensus.json`。v2 consensus 内嵌 policy receipt 与全部签名/Provenance proof；正式安装完成后同步、有界等待 full-install bootstrap `--verify-only` 并检查真实 exit code，两个 verify 模式同时出现会按 CLI 误用拒绝。任何失败或 120 秒超时都进入自动回滚，previous 保留供人工恢复。
