...
- **建议**：historical v1 记录仍只证明旧字节。当前 main-space v2 在 clean `40853287…` 已完成 qualification/full 与双份 C#/Web/direct/viewer 自动 closure，仍为 `source-ahead / NOT_DEPLOYED`；下一步是创建新 v2 tag/request、完成双 builder/双 faultDomain quorum 与不可复用 preflight，再以 `kimi-code/k3` + 当前最高 `high` 做完整异构审计。之后仍由人类签收 Flash oracle、可见 DWM/游戏 composite、关键比例/平滑、混合 DPI、运行时 SWF 身份、真实手点与审美。自动包没有 parity verdict，全部通过也只证明视觉基座；B0 accepted 后再沿阶段2→3建立真实 state/UiData，并做 HP/MP 双轨对比，不能把 fixture 结论外推为运行态接入。
