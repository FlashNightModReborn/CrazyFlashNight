...
**当前正式发布（2026-08-15）**：面板回包闭环与最小通用构建门 release source `704d836f42b70f788db8b5b2151c8ab3dcf77792`（tag `runtime-build-v2/20260815-panel-wire-minimal-gate-v1`）、release tree `c4581f1ed445840ce6d648a842c65b113509c20c` 与 request `CC8C3E614F3B38CA75F2868BE6DDE6A49891EFA8E9EED7E8B23E291DBBA62306` 已完成正式 promotion。本地 X509 `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC builder `B876A8927E422955A71DE393A78C2D10135FFBE9348E7071F9F5D5B42D91C39F` / `github-hosted-windows`（run `31886952236`）对 identity `BB77F46E18C253AD66AF9DA6E89E9E254D7798FC7FAFEC2D520A8E7D3D72595E` / closure `25DD44F37E3936D11D5A281200F72FE68A1833A288B678A0520F64A4CBC49660` 达成双 signer、双 faultDomain 共识；正式 Core SHA-256 为 `58EC1703DF19C455EF06B711FB616DBF52A8DA72ABCAF307AC22727EA051C4BA`，promotion 提交 `019cf8d0d253adc23578f8d2ad0042067866dec2` 与 post-promotion audit run `31888427406` 均已推送/通过。通用 promotion 已移除由持久 Audio H1 manifest 推导 H2 阻断及 emergency bypass；Audio qualification corpus 也不再进入通用 policyHash/Native Audit 触发面，真正影响 DLL 的 Audio source/recipe/toolchain 仍在构建身份闭包内。Audio H2 继续作为 Audio 专项 `e2e_verified` / `standard_entry_verified` 的 `pending` 证据，不限制通用 promotion。本轮跳过重复的 promotion `-VerifyOnly` 预演，只执行一次 production policy 与一次带原子写入/回滚的正式 promotion；根入口 `--verify-only` 为 `exit 0`，但没有执行面板业务专项正式旅程，故只称 `promoted`。下行旧正式发布段落均按历史列车输入阅读，不覆盖本段。
...
当前 consensus 精确绑定 request `CC8C3E614F3B38CA75F2868BE6DDE6A49891EFA8E9EED7E8B23E291DBBA62306` 与 release tree `c4581f1ed445840ce6d648a842c65b113509c20c`。artifact source 为 `3448C76B3E19B4417308BC1F5014C5413FF32214628DA5205AE8194BCBCFE16A`、producer recipe `93A007D1BD03C54F3CC257F1E4610354BBBB83C97B635AC28C2B5D393C75868A`、toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD`、policy `0607061370D07EF253D4517EE61AF1450D5D78D3283D68AC378BC0F83042E1B1`；build identity `BB77F46E18C253AD66AF9DA6E89E9E254D7798FC7FAFEC2D520A8E7D3D72595E`、payload closure `25DD44F37E3936D11D5A281200F72FE68A1833A288B678A0520F64A4CBC49660`、manifest SHA-256 `3675385E37200DEF3EEB8CC5C9D233324E5CFE56DEA52917088A7F4E5BDDAEBC`、promotion 后 Windows worktree consensus bytes SHA-256 `E03E8DB8922098D912503CA14B78427F0C87B27109E7AC01D16BB3CB8E1F9139`。production policy receipt SHA-256 为 `B580E43ABE8ED6F3A68E007973E107DC94D1C782797484BD6F1B8FA2E3301D20`；本地 X509 signer `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3` / `physical-host-a` 与 GitHub OIDC builder `B876A8927E422955A71DE393A78C2D10135FFBE9348E7071F9F5D5B42D91C39F` / `github-hosted-windows`（run `31886952236`）满足双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier，于 `2026-08-15T13:43:23.2534205Z` 完成 promotion。

当前正式部署的根 EXE SHA-256 为 `55EABB5C3280FDFEE8302843D969C6B6354808F1AF4DC24DB5E2397AA80499D8`，Core DLL SHA-256 为 `58EC1703DF19C455EF06B711FB616DBF52A8DA72ABCAF307AC22727EA051C4BA`，`miniaudio.dll` SHA-256 为 `AC3A8FC3BE306D595918F392B44E298D95B71221311920E892C4BE2A6E3AD20A`。部署后 33-file bundle、signed consensus 与正式根入口 `--verify-only` 分别通过；post-promotion audit run `31888427406` 精确报告 base `704d836f42…`、head `019cf8d0d2…`、`state=promoted`。本轮未完成面板或 Audio 专项正式旅程，因此不声称对应 `standard_entry_verified`，也不外推任何业务写或玩家目视签收。下行仍以“当前”描述旧 source / identity 的段落均按历史列车输入阅读，不覆盖本段。
