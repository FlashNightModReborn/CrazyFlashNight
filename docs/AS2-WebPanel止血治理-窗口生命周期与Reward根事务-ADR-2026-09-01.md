...
- **状态**：`W_HUMAN_ACCEPTANCE_PASSED / PROMOTED / FORMAL_BUSINESS_REVALIDATION_PENDING；A/S_HUMAN_ACCEPTANCE_PASSED / PROMOTED / FORMAL_BUSINESS_REVALIDATION_PENDING`
- **决策日期**：2026-09-01
...
- **调度约束**：维护者已于 2026-09-02 在 exact candidate 上通过 H-W，W 的 immutable request、双故障域共识、promotion、部署与推送均已完成。A/A1 与 S/O1 的第二次真人窗口已于 2026-09-03 分别通过 H-A 与 H-S；第二列车随后完成源码提交、immutable request、本地与 GitHub Hosted 双故障域共识、原子 promotion、deployment push 与 post-promotion Audit。发布链仍不得把候选人工证据写成正式入口复验。
- **当前实现状态**：W/B0 代码、focused/full Launcher 自动门、隔离候选执行、H-W、40/40 production policy、双 signer / 双 faultDomain 共识、严格 preflight、原子 promotion、worktree/index 正式根复验与 bootstrap `--verify-only` 均已完成；deployment 已推送，首次 post-promotion Audit run `33645182028` 通过并精确输出 `state=promoted`、`deploymentChanged=true`。部署后没有重跑正式入口 W 业务旅程，因此只称 `HUMAN_ACCEPTANCE_PASSED / promoted`，不称 W 专项 `standard_entry_verified`。A/A1 与 S/O1 的实现、自动门、修复后隔离候选与 H-A/H-S 均已完成；A 覆盖跨重启不重不丢、主动重开、整理后领取全部 remaining，O1 取得 13 个同实例连续样本并合法落到 `no_outstanding_operation`。正式构建另由本地 X509 与 GitHub Hosted 对同一 33-file closure 达成共识并 promotion；deployment `95ea7e8f7c862ea28f6b8c7c906173a701d71336` 已推送，Audit run `33702253144` 通过并精确输出 `state=promoted`、`deploymentChanged=true`。A/S 当前为 `HUMAN_ACCEPTANCE_PASSED / promoted / FORMAL_BUSINESS_REVALIDATION_PENDING`。
