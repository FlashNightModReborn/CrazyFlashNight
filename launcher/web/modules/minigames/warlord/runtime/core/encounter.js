import { DEMO_1_ENCOUNTER_BINDING, } from '../data/encounter.js';
...
export function createEncounterRuntimeState(binding = DEMO_1_ENCOUNTER_BINDING) {
    return {
...
export function auditEncounterState(state) {
    const issues = [];
    let runtimeBundle;
    try {
    ...
    const profileById = new Map(encounterBinding.definition.profiles.map((profile) => [profile.id, profile]));
    for (const nodeId of expectedNodeIds) {
        const node = state.map.nodes[nodeId];
        if (!node)
            continue;
        const rawNode = node;
        const presentCount = NODE_ENCOUNTER_FIELDS.filter((field) => (Object.prototype.hasOwnProperty.call(rawNode, field))).length;
...
export function frozenStateHasAnyEncounterFields(value) {
    const rawState = value;
    if (rawState.encounter !== undefined)
        return true;
    return Object.values(value.map.nodes).some((node) => {
        const rawNode = node;
        return NODE_ENCOUNTER_FIELDS.some((field) => (Object.prototype.hasOwnProperty.call(rawNode, field)));
    });
}
