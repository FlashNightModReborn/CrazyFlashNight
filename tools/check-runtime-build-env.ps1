...
if (Test-Path -LiteralPath $vswhere -PathType Leaf) {
    foreach ($candidate in @(& $vswhere -products '*' -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath)) {
        ...
        if (Test-Path -LiteralPath $candidateTools) { $vsRoot = $candidate; $vcToolsDir = $candidateTools; break }
    }
}
if (-not $vcToolsDir) {
    Add-Mismatch "MSVC toolset missing version=$($lock.msvc.toolsVersion)"
} else {
