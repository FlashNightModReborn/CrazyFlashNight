...
_root.战宠进阶函数.强化药剂 = {
	...
	详情页描述:function(){
		...
		if(this.当前宠物属性.基础训练 && this.当前宠物属性.基础训练.次数 >= this.进阶方案.强化药剂.次数上限){
			return 基础介绍 + "<br>当前已注射强化药剂，注射强化药剂可获得的属性提升达到上限。"
		}
...
_root.战宠进阶函数.超级血清 = {
	...
	描述:function(){
		...
		if(this.当前宠物属性.基础训练 && this.当前宠物属性.基础训练.次数 >= this.进阶方案.超级血清.次数上限){
			return "已注射超级血清。注射超级血清可获得的属性提升达到上限"
		}
	...
	执行按钮文字:"注射血清",
	详情页描述:function(){
		var 基础介绍 = "近期的实验表明，如果要通过这种方式制造超级战士，循序渐进的培养方案是最稳妥的。我们不能寄希望于速成，因为人体（以及其他动物的身体）必须适应药物带来的变化，这需要时间。此外，药物试验需要与之匹配的工业产能，这必然需要资金投入。从客户手中获取改造资金已是必然选择，该领域的商业化推广已是箭在弦上。";
		if((this.进阶方案.基础训练 || this.进阶方案.强化药剂) && (!this.当前宠物属性.基础训练 || this.当前宠物属性.基础训练.次数 < this.进阶方案.超级血清.次数条件)){
			return 基础介绍 + "<br>当前体质不足以承受超级血清，请先进行训练。";
		}
		if(this.当前宠物信息[1] < this.进阶方案.超级血清.解锁等级){
			return 基础介绍 + "<br>五十级以上可以注射超级血清。"
		}
		if(this.当前宠物属性.基础训练 && this.当前宠物属性.基础训练.次数 >= this.进阶方案.超级血清.次数上限){
			return 基础介绍 + "<br>当前已注射超级血清，注射超级血清可获得的属性提升达到上限。"
		}
