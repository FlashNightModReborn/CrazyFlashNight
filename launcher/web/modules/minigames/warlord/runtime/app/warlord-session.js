...
import { commandElementForMember, commandElementMetrics, commandElementsAtNode, nodeDeploymentSize, selectionOrganizationMetrics, } from '../core/organization.js';
...
import { mountPortraits, normalizePlayerAvatarPortrait, } from '../assets/portrait-texture-source.js';
import { SandtableScene, } from '../scene/sandtable-scene.js';
import { MAP_THEMES, normalizeMapTheme, } from '../scene/map-theme.js';
import { StageOuterSessionAuthority, } from '../stage/session-authority.js';
import { parseStageOuterBinding } from '../stage/outer-authority.js';
import { buildResumeAppliedReceipt, canEmitStageGameOver, } from '../stage/resume-apply.js';
import { DisposableBag, GenerationFence, isEditableKeyboardTarget } from './lifecycle.js';
import { buildLargeMapSectorIndex, deriveLargeMapAlerts, searchLargeMapNodes, } from './large-map-navigation.js';
...
import { closeHelpUi, createHelpUiState, helpProfileForScenario, isHelpAnchor, openHelpUi, } from './help-profile.js';
import { formatActionPoints, formatMilitaryFunds, formatStrategicRound, playerBehaviorName, playerEncounterDistanceText, playerOwnerName, playerPowerTierName, playerReasonSummary, playerTextForReason, } from './player-text-catalog.js';
import { buildActionPreviews, factionLabel, nextPromotionFor, ownerLabel, PHASE_LABEL, projectBattleUnitPresentation, projectBattleVisual, projectNodes, } from './presenter.js';
...
const FORMATION_SLOT_LABEL = {
    vanguard: '前卫',
    flank: '侧翼',
    fire_support: '火力',
    reserve: '预备',
};
function formationEffect(profileId) {
...
function playerEventMessage(event, game) {
...
function battleUnitName(record, pieceId) {
    if (!pieceId)
        return '一支部队';
...
function playerBattleEventMessage(record, event) {
...
function as2ResumeIdentity(value) {
...
export class WarlordSession {
    ...
    selectedNodeId;
    selectedPieceIds = [];
    splitMemberSelections = new Map();
    productionNodeId;
    selectedSlotId;
    productionControlMode = 'auto';
    // notice 通过 setter 同步可见 toast 的序号；错误/阻断类用 setNotice(..., 'error') 区分色调
    noticeText = '点击己方部队或按住多选键框选，再点击高亮据点直接下令。';
    noticeTone = 'info';
    toastSerial = 0;
    toastFadeTimer = null;
    toastHideTimer = null;
    armedTargetNodeId = null;
    attackArmTimer = null;
    // fixture 模式 AI 回合重放队列：公开 applyCommand 逐条投影中间态
    aiReplay = null;
    aiCameraLease = null;
    // 首次引导（仅会话内记忆，不落盘）：发过一轮命令 / 完成过一轮行动后不再提示
    coachCommandIssued = false;
    coachDone = false;
    coachSkipped = false;
    helpState = createHelpUiState();
    helpReturnFocus = null;
    helpFocusTarget = null;
    playback = null;
    automationTimer = null;
    portraitGeneration = new GenerationFence();
    stageCloseGeneration = new GenerationFence();
    stageAutoCloseTimer = null;
    ...
    nodePageIndex = 0;
    largeMapSectorId = 'all';
    largeMapSearchQuery = '';
    largeMapToolsExpanded = false;
    rosterCollapsed = false;
    sceneError = null;
    configOpen = false;
    disposed = false;
    panelCloseQuiesced = false;
    seedDraft;
    presetDraft;
    difficultyDraft;
    ...
    constructor(root, initData) {
        ...
        const preferredStart = this.game.preset === 'all-units'
            ? Object.keys(playerFaction.productionQueues)[1]
    ...
    restartScene() {
        ...
        if (!this.playbackRecord && !this.handoffPending && !this.panelCloseQuiesced)
            this.startScene();
    }
    /**
     ...
     */
    releaseSceneForBattleBoundary(preserveAiFollow = false) {
    ...
    clearStageAutoClose() {
        if (this.stageAutoCloseTimer !== null)
            window.clearTimeout(this.stageAutoCloseTimer);
    ...
    scheduleStageExactClose() {
        if (this.disposed || this.stageAutoCloseTimer !== null)
            return;
    ...
    cancelAiActionCamera(token, resumeAutomation = false) {
        if (this.aiCameraLease?.token !== token)
            return;
        this.scene?.cancelActionFollow(token);
        this.aiCameraLease.carry = null;
        this.aiCameraLease = null;
        if (resumeAutomation && !this.disposed)
            this.scheduleAutomation();
    }
    holdAiActionCameraForContinuation(token) {
        ...
        void (completion ?? Promise.resolve(false)).then((settled) => {
            if (this.disposed || this.aiCameraLease?.token !== token)
                return;
            if (!settled) {
                this.aiCameraLease = null;
    ...
    returnAiActionCamera(token) {
        const pending = this.aiCameraLease;
        if (!pending || pending.token !== token || pending.mode === 'returning')
            return;
    ...
    closeBattlePlayback() {
        ...
        if (pending)
            this.holdAiActionCameraForContinuation(pending.token);
    }
    recoverFromDefinitiveAs2BattleFailure(command, playerNotice, aiNotice) {
    ...
    async beginAs2Battle(command) {
        ...
        if (!bridgeSend) {
            this.recoverFromDefinitiveAs2BattleFailure(command, '战斗场景暂时无法打开；战略态未改变，请重新选择部队后再试。', '战斗场景暂时无法打开；本次进攻未写入战略态，敌方行动已结束，演习继续。');
            return;
        }
        ...
        try {
            ...
            if (!identity)
                throw new Error('生成的 AS2 战斗身份非法。');
            ...
            if (this.pendingBattlePrepared)
                return;
            if (delivered !== true) {
                this.recoverFromDefinitiveAs2BattleFailure(command, '战斗场景未能接收本次进攻；战略态未改变，请重新选择部队后再试。', '战斗场景未能接收本次进攻；本次进攻未写入战略态，敌方行动已结束，演习继续。');
                return;
            }
        }
        catch {
            ...
            if (this.pendingBattlePrepared)
                return;
            this.recoverFromDefinitiveAs2BattleFailure(command, '战斗场景准备失败；战略态未改变，请重新选择部队后再试。', '战斗场景准备失败；本次进攻未写入战略态，敌方行动已结束，演习继续。');
        }
    }
    sendResumeApplied(resume, status) {
        if (!this.stageAuthority.isStageMode)
            return true;
        const receipt = buildResumeAppliedReceipt(resume, this.init.stageOuterBinding, status);
        if (!receipt || !this.init.resumeAppliedSend)
            return false;
        try {
    ...
    async consumeAs2Resume(resume) {
        ...
        if (this.disposed || this.resumeCommitSlot !== slot
            || this.stageAuthority.contextIdentity !== stageContextIdentity)
            return;
        this.handoffPending = false;
        this.pendingBattleCallId = null;
        this.pendingBattleCommand = null;
        ...
        this.pendingBattlePrepared = false;
        this.clearAuthorityAckTimer();
        if (result.ok || !result.resultUnknown)
            this.authoritySessionId = identity.sessionId;
        if (!result.ok || !result.state || !result.battleRecord) {
            ...
            if (slot.ackStatus !== null && !slot.ackSent) {
                this.authorityBlocked = true;
                this.authorityReturnOnly = false;
                this.setNotice('战斗恢复状态已经读取，但 Host 尚未确认应用；当前战局保持冻结，请关闭面板后从任务提示恢复同一战局。', 'error');
                this.render();
                return;
            }
        ...
        if (!slot.ackSent) {
            this.authorityBlocked = true;
            this.setNotice('战斗结果已经载入，但 Host 尚未确认恢复完成；当前战局保持冻结，请关闭面板后从任务提示恢复同一战局。', 'error');
            this.render();
            return;
        }
        this.authorityBlocked = false;
    ...
    handleHostResponse(response) {
        ...
        this.recoverFromDefinitiveAs2BattleFailure(command, '战斗场景拒绝了本次进攻；战略态未改变，请重新选择部队后再试。', '战斗场景拒绝了本次进攻；本次进攻未写入战略态，敌方行动已结束，演习继续。');
        return true;
    }
    scheduleAutomation(resumeAfterCamera = false) {
        ...
        if (this.disposed || this.panelCloseQuiesced || this.helpState.open
            ...
            || this.stageAuthority.blocksGameplay)
            return;
        if (this.aiCameraLease?.blocking)
            return;
        ...
        if (this.playback && record) {
            ...
            if (this.playback.index >= total && this.aiReplay
                ...
                this.automationTimer = window.setTimeout(() => {
                    if (this.disposed || !this.playback)
                        return;
        ...
        if (this.aiReplay) {
            ...
            this.automationTimer = window.setTimeout(() => this.stepAiReplay(), resumeAfterCamera ? 0 : AI_REPLAY_INTERVAL_MS);
            return;
        }
        ...
        if ((this.game.phase === 'FIRST_FACTION_ACTION' || this.game.phase === 'SECOND_FACTION_ACTION')
            ...
            if (this.init.battleAuthority === 'as2') {
                this.automationTimer = window.setTimeout(() => {
                    ...
                    if (!succeeded) {
                        if (cameraToken !== null)
                            this.cancelAiActionCamera(cameraToken, true);
                        return;
                    }
                    if (isBattle) {
                        if (cameraToken !== null)
            ...
            this.automationTimer = window.setTimeout(() => {
                if (this.disposed || this.aiReplay)
                    return;
                this.beginAiReplay();
            }, 180);
            return;
    ...
    openBattle(record, settled = false) {
    ...
    completeMemberSelection(memberIds) {
    ...
    setPieceChecked(pieceId, checkedState) {
        const piece = this.game.pieces[pieceId];
        const element = commandElementForMember(this.game, pieceId);
        if (!piece || !element || piece.factionId !== this.playerId || !this.canSelectPieces())
            return;
        this.disarmAttack();
        if (!checkedState) {
            const removed = new Set(element.memberIds);
    ...
    setSplitMemberChecked(elementId, memberId, checkedState) {
        const element = this.game.organization.commandElements[elementId];
        if (!element || element.kind !== 'task_group' || !element.memberIds.includes(memberId))
            return;
    ...
    setCommandElementFormation(elementId, profileId) {
        const element = this.game.organization.commandElements[elementId];
        if (!element || element.formationProfileId === profileId)
            return;
        const profile = formationProfile(profileId);
        this.dispatch({
            type: 'SET_FORMATION',
    ...
    armAttack(nodeId, targetName) {
        ...
        this.attackArmTimer = window.setTimeout(() => {
            ...
            if (this.disposed || this.armedTargetNodeId !== nodeId)
                return;
    ...
    clearAttackArmTimer() {
        if (this.attackArmTimer !== null)
            window.clearTimeout(this.attackArmTimer);
        this.attackArmTimer = null;
    }
    ...
    startNewGame() {
        ...
        this.selectedNodeId = this.game.preset === 'all-units'
            ? Object.keys(playerFaction.productionQueues)[1] ?? playerFaction.commandPostNodeId
            : playerFaction.commandPostNodeId;
        this.selectedPieceIds = [];
    ...
    onInput = (event) => {
        ...
        if (target?.dataset.field === 'large-map-search')
            this.largeMapSearchQuery = target.value;
    };
    onChange = (event) => {
        const target = event.target;
        if (!target)
        ...
        if (target.dataset.field === 'task-group-member') {
            ...
            if (elementId)
                this.setSplitMemberChecked(elementId, target.value, target.checked);
        }
        ...
        if (target.dataset.field === 'large-map-sector' && this.game.scenarioId === 'warlord_demo_02_v1') {
            ...
            this.nodePageIndex = 0;
            const firstNodeId = sector?.nodeIds[0];
            if (firstNodeId) {
                this.selectedPieceIds = [];
    ...
    onClick = (event) => {
        ...
        if (target.getAttribute('aria-disabled') === 'true') {
            ...
            const reasonCode = target.dataset.reasonCode;
        ...
        if (action === 'toggle-large-map-tools' && this.game.scenarioId === 'warlord_demo_02_v1') {
            ...
            if (app)
                app.dataset.largeMapToolsExpanded = String(this.largeMapToolsExpanded);
            this.renderLargeMapTools();
            this.renderNodes();
            queueMicrotask(() => {
                if (this.disposed)
                    return;
                this.root.querySelector('[data-action="toggle-large-map-tools"]')
        ...
        if (action === 'toggle-roster') {
            ...
            queueMicrotask(() => {
                if (this.disposed)
                    return;
                this.root.querySelector('[data-action="toggle-roster"]')
                    ?.focus({ preventScroll: true });
            });
            return;
        }
        ...
        if (action === 'large-map-search' && this.game.scenarioId === 'warlord_demo_02_v1') {
            ...
            const search = searchLargeMapNodes(this.largeMapNodeSummaries(sectorIndex.sectorByNodeId), sectors, this.largeMapSearchQuery, 20);
            ...
            this.nodeNavigatorMode = 'all';
            this.nodePageIndex = nodePageIndexFor(sectorIndex.nodeIdsBySectorId[first.sectorId] ?? [], first.nodeId);
            this.navigateLargeMapNode(first.nodeId, search.totalMatches > 1
                ? `找到 ${search.totalMatches} 个匹配项，已定位第一个：${first.displayName}。`
                : `已定位${first.displayName}。`);
            return;
        }
        if (action === 'select-all-at-node')
            this.selectAllAtNode(this.selectedNodeId);
        if (action === 'merge-task-group')
            this.mergeSelectedTaskGroup();
        if (action === 'split-task-group' && target.dataset.element)
            this.splitTaskGroup(target.dataset.element);
        ...
        if (action === 'production') {
            const cardId = Number(target.dataset.card);
            const choice = resolveProductionChoice(this.game, this.playerId, cardId, this.productionControlMode, this.productionNodeId, this.selectedSlotId);
        ...
        if (action === 'commit-planning')
            this.dispatch({ type: 'COMMIT_PLANNING', factionId: this.playerId });
    ...
    forwardCameraKey(key, shiftKey) {
        ...
        if (normalized === '+' || normalized === '=')
            ...
            this.scene.zoomBy(0.8);
        else if (normalized === '0' || normalized === 'home')
            this.scene.fitToMap();
        else if (normalized === 'arrowleft' || normalized === 'a')
            this.scene.panBy(-step, 0);
        else if (normalized === 'arrowright' || normalized === 'd')
            this.scene.panBy(step, 0);
        else if (normalized === 'arrowup' || normalized === 'w')
            this.scene.panBy(0, -step);
    ...
    requestClose(reason = 'escape') {
        ...
        if (this.largeMapToolsExpanded) {
            ...
            const app = this.root.querySelector('.warlord-app');
            if (app)
                app.dataset.largeMapToolsExpanded = 'false';
            this.renderLargeMapTools();
            this.renderNodes();
            queueMicrotask(() => {
                if (this.disposed)
     ...
     */
    quiesceForPanelClose() {
        if (this.disposed || this.panelCloseQuiesced)
            return;
        this.panelCloseQuiesced = true;
        this.clearAutomation();
        this.releaseSceneForBattleBoundary(this.aiCameraLease !== null);
        if (this.aiCameraLease && !this.aiCameraLease.carry)
            this.aiCameraLease = null;
        this.root.dataset.sceneLifecycle = 'released_for_panel_close';
    }
    resumeAfterPanelCloseTimeout() {
        if (this.disposed || !this.panelCloseQuiesced)
            return;
    ...
    maybeEmitStageTerminal() {
        ...
        const emitted = canEmitStageGameOver(this.authorityBlocked, this.game.phase, this.game.result !== null) && this.game.result
            ? this.stageAuthority.emitGameOver(this.game.result.winner, this.playerId, this.game.result.winningVictoryGroupId, this.playerFaction().victoryGroupId)
            : null;
    ...
    render() {
        ...
        if (sceneMustBeReleased)
            this.releaseSceneForBattleBoundary();
        else if (!this.handoffPending && !this.scene && !this.sceneError)
            this.startScene();
        const app = this.root.querySelector('.warlord-app');
        if (app) {
            ...
            app.dataset.rosterCollapsed = String(this.rosterCollapsed);
            app.dataset.largeMapToolsExpanded = String(this.game.scenarioId === 'warlord_demo_02_v1' && this.largeMapToolsExpanded);
    ...
    renderCommandBar() {
        ...
        if (theater)
            theater.textContent = `${MAP_THEMES[this.mapTheme].theaterLabel} · 战术指挥台`;
        ...
        if (round)
            round.innerHTML = `<b>${escapeHtml(formatStrategicRound(this.game.strategicRound))}<small> / 共 24 回合</small></b><span>${phaseLabel} · ${this.game.activeFactionId ? `${factionLabel(this.game.activeFactionId, this.game)}行动` : '统一处理'}</span>`;
        if (factions)
            factions.innerHTML = this.game.turnOrder.map((factionId) => {
                const faction = requireFaction(this.game, factionId);
    ...
    renderCommandElementCard(element, canSelect) {
      ...
      <span><b>${escapeHtml(elementName)}</b>${commanderBadge}<small>${escapeHtml(summary)}</small><small class="warlord-element-metrics">行动消耗 ${metrics.commandLoad} · 规模 ${metrics.deploymentSize} · 战斗负载 ${metrics.encounterCost}</small>${element.kind === 'singleton' ? `<i${hpBarClass(representative.hp, representative.maxHp)}><em style="width:${hpPercent(representative.hp, representative.maxHp)}%"></em></i>` : ''}</span>
        ...
        if (element.kind !== 'task_group' || element.factionId !== this.playerId)
            return card;
        const template = DEMO_1_ORGANIZATION.taskGroupTemplates.find((entry) => entry.id === element.taskGroupTemplateId);
        const allowedProfiles = DEMO_1_ORGANIZATION.formationProfiles.filter((candidate) => (!template || template.formationProfileRefs.includes(candidate.id)));
    ...
    largeMapNodeSummaries(sectorByNodeId) {
    ...
    navigateLargeMapNode(nodeId, notice) {
        if (!this.game.map.nodes[nodeId])
            return;
        this.selectedPieceIds = [];
        this.inspectNode(nodeId);
    ...
    renderLargeMapTools() {
        const region = this.root.querySelector('[data-region="large-map"]');
        if (!region)
            return;
        ...
        const commandPostThreatened = adjacentNodeIds(this.game, playerFaction.commandPostNodeId).some((nodeId) => (nodeOccupyingFactions(this.game, nodeId).some((factionId) => (relationBetween(this.game, this.playerId, factionId) === 'hostile'))));
        ...
        for (const [nodeId, slots] of Object.entries(playerFaction.productionQueues)) {
            if (!slots?.some((slot) => slot.orders.some((order) => order.status === 'waiting_deployment')))
                continue;
        ...
        const alertButtons = alerts.alerts.map((alert) => (`<button data-action="navigate-node" data-node="${escapeHtml(alert.nodeId)}" title="${escapeHtml(`${alert.detail} ${alert.nextStep}`)}"><b>${escapeHtml(alert.title)}</b><span>${escapeHtml(requireNode(this.game, alert.nodeId).displayName)}</span></button>`)).join('');
    ...
    renderCards() {
        ...
        region.innerHTML = `<div class="warlord-roster-label"><button class="warlord-roster-toggle" data-action="toggle-roster" aria-controls="warlord-roster-cards" aria-expanded="${!this.rosterCollapsed}" aria-label="${this.rosterCollapsed ? '展开兵种蓝图与生产操作' : '收起兵种蓝图，让沙盘获得更多空间'}"><b>兵种蓝图</b><span aria-hidden="true">${this.rosterCollapsed ? '展开兵牌' : '收起兵牌'}</span></button><span>结算升级 / 排产</span>${planningPhase ? '' : '<small class="warlord-roster-hint">统一结算阶段开放排产 / 升阶</small>'}</div><div id="warlord-roster-cards" class="warlord-card-track"${this.rosterCollapsed ? ' hidden' : ''}>${PRODUCTION_CARD_IDS.map((cardId) => {
            ...
            const production = resolveProductionChoice(this.game, this.playerId, cardId, this.productionControlMode, this.productionNodeId, this.selectedSlotId);
    ...
    renderPlanning() {
        ...
        if (this.game.phase === 'SETTLEMENT_PLANNING') {
            ...
            const playerCommander = Object.values(this.game.commanders).find((commander) => (commander.factionId === this.playerId && commander.role === 'player_avatar'));
    ...
    renderBattle() {
      ...
      <header><span><b>${escapeHtml(this.game.map.nodes[record.nodeId]?.displayName ?? '目标据点')}</b><small>${escapeHtml(formatStrategicRound(record.strategicRound))}的战斗</small></span><strong>${playback.index}/${record.result.eventLog.length}</strong></header>
    ...
    closeHelp() {
        ...
        this.scheduleAutomation();
        queueMicrotask(() => {
            if (!this.disposed && returnFocus?.isConnected)
                returnFocus.focus({ preventScroll: true });
        });
    }
    renderHelp() {
        const region = this.root.querySelector('[data-region="help"]');
        if (!region)
            return;
        if (!this.helpState.open) {
            region.hidden = true;
      ...
      <div class="warlord-help-body">
        ...
        if (focusTarget) {
            queueMicrotask(() => {
                if (this.disposed || !this.helpState.open)
                    return;
                const target = focusTarget === 'close'
                    ? region.querySelector('[data-action="close-help"]')
                    : region.querySelector('[data-help-current]');
                target?.focus({ preventScroll: true });
            });
        }
    }
    trapHelpFocus(event) {
        if (!this.helpState.open || event.key !== 'Tab')
            return false;
        const dialog = this.root.querySelector('.warlord-help-dialog');
        if (!dialog)
            return false;
    ...
    renderCameraHud() {
      ...
      <div class="warlord-camera-controls">
        ...
        if (position)
            position.textContent = `横向偏移 ${this.cameraSnapshot.centerX.toFixed(1)} · 纵向偏移 ${this.cameraSnapshot.centerZ.toFixed(1)} · ${this.cameraSnapshot.nodeCount || Object.keys(this.game.map.nodes).length} 个据点`;
    ...
    renderToast() {
        const toast = this.root.querySelector('[data-region="toast"]');
        if (!toast)
            return;
        if (toast.dataset.serial === String(this.toastSerial))
            return;
    ...
    clearToastTimers() {
        if (this.toastFadeTimer !== null)
            window.clearTimeout(this.toastFadeTimer);
        this.toastFadeTimer = null;
    ...
    renderAuthorityBanner() {
        const region = this.root.querySelector('[data-region="authority"]');
        if (!region)
            return;
        if (this.stageAuthority.status === 'blocked') {
            region.hidden = false;
    ...
    renderCoach() {
        const region = this.root.querySelector('[data-region="coach"]');
        if (!region)
            return;
        ...
        if (!this.coachDone && !this.coachSkipped && playerAction && !this.playback && !this.handoffPending
            ...
            if (this.coachCommandIssued)
                tip = '③ 点击“结束我方行动”收束本回合';
            else if (this.selectedPieceIds.length === 0)
                tip = '① 点击己方棋子建立编组';
        }
        if (!tip) {
            region.hidden = true;
    ...
    renderHoverChip(info, anchor) {
        const chip = this.root.querySelector('[data-region="hover"]');
        if (!chip)
            return;
        if (!info) {
            chip.hidden = true;
        ...
        chip.hidden = false;
        const stage = this.root.querySelector('.warlord-map-stage');
        if (stage) {
