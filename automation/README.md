...
**文档角色**：启动与运行自动化入口。  
**当前正式列车（2026-08-25，军阀战术演习 AS2 战斗）**：release source `248cca7be212219655319c666304407b0568e658`、tag `runtime-build-v2/20260825-warlord-as2-battle-v2`、release tree `2bbf0eaa19f10461d5feb9cc6c0e26a1a2f9845a` 与 request `4F57D8E7BA7499021E4A36823BFB27A42BDAE043CA9483BF46B34F42AB09456D` 已由本地 X509 `builder-local-b` / `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `32821144648`）对 identity `6482B71F2811065A5537F5CDCB0E338D44C23C2BECDC13987D9924B289EC8C59`、closure `4B46C47478DAC64E038F574F2636AFDFF9E0E6EA16F807171FF5F31E21237062` 达成双 signer / 双 faultDomain 共识。38/38 final receipt SHA-256 为 `95AA0D3E0570EAE9712DADF1EB237EE81D8EBB205A199F0BCB4B7FDD02942DF2`；deployment commit `126c10a3d5bf46b66a973e8b68ad27c00ef9257d` 与 post-promotion audit run `32822831143` 均已推送/通过。
无 candidate selector 的 formal runtime attempt `37181289fc744636844c4af38ee49107` 取得 fresh reveal、两次同 lifecycle `session.status=verified`、精确 identity/closure、strict shutdown、Flash/Guardian exit 0 与零进程残留。该 smoke 只证明正式身份和生命周期；军阀沙盘与 AS2 战后返回的维护者验收发生在 promotion 前的隔离候选，因此当前是 `HUMAN_ACCEPTANCE_PASSED / promoted`，不称军阀专项 `standard_entry_verified`。不可变 v1 tag/request 在本地 policy 暴露派生物与事务门问题后停止，未 dispatch 云端、未 promotion，由 v2 request 原子 supersede。
...
正式 v2 consensus 当前绑定 tag `runtime-build-v2/20260825-warlord-as2-battle-v2`、request `4F57D8E7BA7499021E4A36823BFB27A42BDAE043CA9483BF46B34F42AB09456D`、source commit `248cca7be212219655319c666304407b0568e658`、release tree `2bbf0eaa19f10461d5feb9cc6c0e26a1a2f9845a`。
build identity 为 `6482B71F2811065A5537F5CDCB0E338D44C23C2BECDC13987D9924B289EC8C59`，payload closure 为 `4B46C47478DAC64E038F574F2636AFDFF9E0E6EA16F807171FF5F31E21237062`。local signer 位于 `physical-host-b`，不可导出 keyId 为 `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`；GitHub Actions run `32821144648` 的 OIDC identity `8273B61EB030A519118FF275A39F73351D5413DCB13A230D1E74ED6F6C318ED6` 提供 `github-hosted-windows` 第二票。
production policy `62CC0DC1F5096C990D3D3ECD4E9DD11B195A25102D44DC783636C55B535366A8` 的 38/38 final receipt SHA-256 为 `95AA0D3E0570EAE9712DADF1EB237EE81D8EBB205A199F0BCB4B7FDD02942DF2`；deployment commit `126c10a3d5bf46b66a973e8b68ad27c00ef9257d` 已由首次 post-promotion audit run `32822831143` 确认为 `state=promoted`、`deploymentChanged=true`，严格 manifest/consensus 为 33 files、2 signers、2 faultDomains。
无 candidate selector 的正式入口取得 fresh reveal、前后两次 `session.status=verified`、同 lifecycle、strict shutdown receipt、Flash/Guardian exit 0 与零进程残留。该 smoke 只证明正式身份和生命周期；维护者在 promotion 前确认的军阀沙盘与战后返回不等于部署后军阀业务标准入口，也不代签其他业务专项 `standard_entry_verified`。cloud workflow 仍只允许 `Crazyfs` / `Flash-Night` 的固定 actor ID 首次 dispatch。
...
2026-08-25 限时关卡 v1 preflight 又暴露材料与 ShopPortraits sidecar 的两个 raw-byte 输入仍是 `text=auto`；该 v1 在 37/39 policy 后停止且未触发云构建或 promotion。现已把 `EquipmentTuningService.as`、`bake-dialogue-portraits.py` 与缺失的 ShopPortraits receipt 固定为 LF，并将完整材料/ShopPortraits raw-byte 集合纳入 versioned EOL 回归（181/181）；正式列车必须改用新的 immutable tag/request。
