...
**最后核对代码基线**：commit `bf9be8c43b223b84f487464a7e6aa9eb8211630b` 已由 tag `runtime-build-v2/20260808-p4-p5-source-complete-v1`、release tree `44de7f66421a908723f196718e42867dce601f30`、request `424D9CC1975CF099A705C828E354215EF3F568B7198B6B19C13DB019CB25163C` 完成 v2 promotion；正式 runtime 绑定 identity `D6AC04EB7EBE290819B41E2FE17DA7583F154FCB4D46BA258D17EF9866B74350`、closure `91C8AC7469EBF2788C9BDAF8AB13D8F3C02B59588D6B770F0DA6CAD1604DDF5B` 与 Core DLL `D5EDBA1277BB65D6741E9446415F2555D3D94BFD78723852E621F7366FB8616D`。商城旧存档只读投影保持原档/clone 字节不变，竞技场 P5 以 `arena_config.xml` 派生 C# 目录并由 AS2 最终裁决；两者均有同身份真实 E2E。无 candidate id 的 pure-MCP 正式入口达到 `standard_entry_verified`，但 Help smoke 不代证商城/竞技场领域旅程，也不外推为物理输入或玩家目视签收。
...
历史 exact 跳转及材料商店导航证据为 Crafting 三视口各 baseline `150/150` + current `15/15` + owner `8/8` + identity `10/10` + v1 `6/6` + session-lock `9/9` + recipe-jump `21/21` + material-shop `11/11`，Crafting runtime `30/30`，Panel runtime `40/40`。一跳返回的 current focused browser 场景在 1024×576 为 `22/22`；全量 harness 本轮被既有 Inspector RAF 计数的 3 项 baseline 波动挡住，尚不是完整全绿。这些仍只证明 mock-browser/Node closure，不代签真实 WebView2→Flash、游戏内 E2E、candidate、promotion 或部署。
