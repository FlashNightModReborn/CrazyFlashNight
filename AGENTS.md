...
**状态边界**：本列车把斗兽标定从工作簿原始来源摄取、参数闭包、`contamination` Schema/fixture、timeout 口径和真正 Schema 实例校验，收束到可冻结、可撤销、可持久恢复的 Gate F 星期级批处理。tracked v9 计划含 58 个 scheduled candidate，另将 `B12` 隔离为 quarantine，共 198 个 10–25 run shard / 3,285 个机器 run；三份 fresh soak 各为 10 个代表样本 + 10 个派生阵营复核样本，`C9` 仍保留 `10 + 20 + 25` run 与 side-swap 全量复核。
...
提交后正式 admission 已在 exact formal runtime 上完成 20/20 finished、0 timeout/error/recovery、正常 shutdown 和受保护存档不变，目标死亡派生实际观察 red 8 / blue 8；`arena-calibration.soak-admission.v2` 将原始 manifest/result/report SHA-256、双向派生最低计数与案例模板闭合。v7 的三份 20-run fresh soak 因 `E10` 两向各出现一次 timeout 暴露旧 controller 的 5% 口径缺口，60 行与 window 均已隔离；现役修复要求任一 `soak` error/timeout 非零即 fail closed。v8 frozen plan `sha256:d9c029815d18e3960dbe9050f90403875c3fdb76e2e9bf159ee9c2cc2f0fbed5` 上，前两份各 20/20 finished，第三份为 19 finished + `B2` original 1 timeout；三份均 0 error/recovery、exact runtime、存档不变、正常 shutdown，派生 red/blue 分别为 `7/6`、`4/4`、`7/5`。严格门正确暂停并 revoke，v8 的 60 行仅作 B2 随机卡住诊断，剩余 195 shard 未开放；B2 仍保留在自身数值分片。替代普通参数代表 D2 的旧 55/55 只作选探针依据，当前正式 exact-policy 原/换边各 5 的新鲜 probe 为 10/10 finished、0 timeout/error/recovery、最大 423 帧、存档与 shutdown 闭合。新 admission group 为 `D2/C7/G2/F3/C11`，hash `sha256:9034715cffb240f9f1d42b6eb6b31064cfb5147dd0b879d67cc3b164eb83f4a2`；v9 tracked plan 已重建，尚未 clean freeze/arm 或执行三份 fresh soak。promotion/admission 不代签 Gate F low-touch、星期级数值样本或业务 `standard_entry_verified`，Audio H2 仍保持独立门。
