...
> **当前施工树边界**：本轮从 commit `c96f4c3d750561022b706c72a4d53050431e627d` 的审阅/F0 债务锚点起步，当前 HEAD 与 `origin/main` 均为 `c6ca84714d134310793defda893772aac2c0d5ea`，其上工作树继续承载 source-ahead 加固；A2b、A3、B1–B7、C1–C3、E、G1–G5、H1–H6、双宿主浏览器策略与 Native 右侧条件槽已取得各自专项自动证据，但这些后续源码未进入上述冻结 runtime。阶段性本地构建至多是 `candidate_built`，脏工作树发布策略随后拒绝继续，不能称为 `promoted`、`deployed` 或 `standard_entry_verified`；后续源码变化也会使阶段性 candidate identity 失效，本文不把它登记为最终身份。
...
当前 HEAD `c6ca84714d134310793defda893772aac2c0d5ea` + source-ahead 工作树的最新汇总覆盖并取代“当前树=1720/1729/831”的旧口径：Host H1 定向历史门为 **253/253**，Launcher 全量最终 fresh 为 **1902 pass + 3 explicit opt-in skip / 1905**、0 fail；首次全量曾暴露既有 SkillTask 20ms wall-clock flake，隔离 5 次均绿后仅把测试 timeout 调至 250ms，确定性 timeout 负例仍在。B0-05/B0-06 formal 与 B0-06 visual capture 本轮保持 explicit opt-in skip；只完成 qualification contract `3/3`、runner parser `0 error` 与失败 envelope/affinity AST 纯内存门。上游 F2 的 `48/48 / b0_accepted` 仅属其精确 base closure；当前右槽 workload 断言使其不能冒充本工作树 formal qualification；Crafting 三视口 **119/119**、Intelligence **30/30**、Equipment Tuning 三视口各 **115/115**（model **61/61**、runtime **25/25**、confirmation **4/4**、interaction **7/7**、source-marker leaf **2/2**）、Character standalone 三视口各 **218/218**、workbench **867/867 + 12/12 + 18/18**、inventory modules **31/31**、dressup **222/222**、preparation leaf **10/10**、KShop **135/135**、item-grid matrix **20/20**、ratchet **66/66**，default/release-tree strict UI audit 均为 **0 error / 0 warning**。合并树最终 Flash focused run 分别为 Tuning `a6f2a1abb3dd44189e9cd1ab4583490b`（**55/55 + 144/144**）、Character `a5d8512263894ded9540aedec991c643`（**539/539**）与 Item Panels `49b52f058b4046bdba5dcafffd6bb50f`（**28/28 + 46/46 + 144/144 + 36/36**）；三轮均为 fresh trace/output/errors、Compiler **0/0**、32K retry **0**。随后只对 `scripts/asLoader/asLoader.xfl` 执行 `-Target publish`，产物为 **1,065,911 bytes**、SHA-256 `BAB9E46D140739F753262264DB326039432552783E87CC184DE8C423AD084CAB`、9,723 functions、最大 46,025B；没有编 main，publish 不出 fresh trace，行为仍由上述 TestLoader 证明。上段 1720、上游 PlayerInfo 1740 与后文各批次数字仍保留为历史/局部快照，不得冒充最终整合树；这些源码/自动门也不等于 candidate、游戏 E2E 或部署。
