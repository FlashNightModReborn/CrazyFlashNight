...
> **最后核对代码基线**：本轮以 `b072f97841ccb30e167c14495241ae64d9054e22` 为 pre-push upstream base；`release/map-loot-20260721-final` 冻结 source commit `c60aab2386aee4516608397373ae4c59148c5f77`（tag `runtime-build-v2/20260721-map-loot-canary-v1`），并由 commit `6218f8b1d82efc57b77131616667fe45f3033297` 记录正式 promotion。当前纳入 Runtime 可复现构建、地图资源箱、attempt-bound Agent 入口、同面板 Loot organizer，以及最小 `equipment_tuning` commit→inventory refresh capability 竞态修复与确定性 harness；唯一 canary 已达到 `standard_entry_verified`，其余装备调制条目保留为上游基线。高变动功能的真机结论与未闭合 Gate 以对应专题章节和 [验证矩阵](../agentsDoc/testing-guide.md) 为准，未重跑计数必须标为上游基线或待复验。
...
│   │   │   ├── SafeExitPanelWidget.cs      安全退出单行二次确认（覆盖右侧条件状态槽）
...
`builder-local-a` / `physical-host-a` 与 GitHub hosted OIDC/Sigstore 的组合是首次 v2 双故障域共识，作为历史记录保留。当前地图资源箱 consensus 实际采用 `builder-local-b`（keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`）/ `physical-host-b` 与 GitHub OIDC builder identity `394B7E898FD83FB60FA0DCD04223BC9A791346080CCAC5838C8F4D22B52A99C8` / `github-hosted-windows`；tracked registry 仍只含公钥。正式 bootstrap/runtime/manifest/consensus 已由唯一 promotion 入口切换到 request `FBA91942C57DC3E6662AB77AC88EEAE3CA2AFA3C82BF5FFEA6BC68CFD6C31AE4`、build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107` 与 payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`。一次性 permanent migration fuse 已完成历史使命，后续发布不得复用该例外或降级 v1。
...
- **Native HUD 默认组成**：[NotchWidget](src/Guardian/Hud/NotchWidget.cs) 接管 web `#notch`（金币/KP、FPS/光照、时钟、游戏/辅助/系统三行 hover toolbar、通知栈和展开图表；[AudioHudState](src/Guardian/Hud/AudioHudState.cs) 以最高 10Hz 采样 BGM L/R peak，并把低透明度包络画在 FPS 折线背景；辅助行含点歌机/地图开关/修改器/帮助）。“系统 → 其他”不再渲染 16 项长列，而以“控制 / 测试 / 工具”三级分类页显示当前分类；任一叶子命令执行或刘海开始收起时同时关闭子页。[ToastWidget](src/Guardian/Hud/ToastWidget.cs) 承载 toast，[RightContextWidget](src/Guardian/Hud/RightContextWidget.cs) 承载 252px 六入口行、32px 条件状态槽和可选 `compact/expanded` 地图预览，[SafeExitPanelWidget](src/Guardian/Hud/SafeExitPanelWidget.cs) 以同锚点单行覆盖条件状态槽并取得最高绘制/命中优先级，[ComboWidget](src/Guardian/Hud/ComboWidget.cs) 承载搓招输入与命中动画。地图 shared renderer 复用 [MapHudWidget](src/Guardian/Hud/MapHudWidget.cs) 的 WebP alpha silhouette/blocks fallback；地图图像不在启动期全量解码，而在 `mh` 切换后只异步预热当前 outline 工作集。
...
- SAFEEXIT 二次确认：router 先调 `OnSafeExitArm`（→ `SafeExitPanelWidget.Arm()`，必须）再 `SendGameCommand("safeExit")` 触发存盘；C# widget 在右侧第二行的同一个 252×32 状态槽内显示 Saving，`sv:2` 后同一行切为状态/取消/退出三段。Done 态无操作 5 秒后按安全取消语义自动收起，不执行退出；悬停“取消”或“退出游戏”时暂停倒计时，移开后继续。它注册在 RightContext 之后，绘制与命中均覆盖任务提示；普通自动存盘没有 Arm 不会显示。
...
`useNativeHud=true` 的视觉/功能 gate 以当前收敛布局为准：刘海与右侧动作行共享居中 `32px` 顶行，展开时“游戏 / 辅助 / 系统”三行工具区不得超过几何安全宽度，所有绘制硬裁切在刘海矩形内；“系统 → 其他”的“控制 / 测试 / 工具”分类页必须随刘海收起，不能残留独立长菜单。常驻组件使用黑底、直角发丝外框、与外框分离的内侧明暗压边和技能槽式角标；hover/pressed/active/danger 主要通过底色、文字和角标表达，不得把整圈外框加粗或提成纯白，也不得回退为圆角/切角外壳、大圆角半透明卡片或随常见分辨率膨胀成 2px 粗框。展开/收起动画的 HWND placement 与 bitmap resize 只允许端点级变化，透明 CompositeBounds 必须 click-through。右侧动作行在所有状态固定为 `right:48px / width:252px / 3×50px+3×34px`；第二行只在任务通知或 SafeExit 出现时占用，地图无通知时直接贴在动作行下方，不再附带整条“地图预览”header。SafeExit Done 态无操作 5 秒自动收起，悬停确认按钮时暂停，自动收起不得触发退出。地图 `compact=64px / expanded=112px`，按钮和卡片均一击打开完整 Web 地图；卡片 `＋/－` 只切 `compact ↔ expanded`，刘海“辅助 → 地图开关”只切显示/关闭，切换后必须显示中文 toast。恢复 `auto` 走设置/偏好写入。人工覆盖：1024×576、1600×900、1920×1080 与 4:3 letterbox；100%/125%/150%/175% 下发丝框、角标和文字基线清晰；刘海折叠/展开、其他分类页生命周期、四项辅助入口、BGM 包络、六路动作、任务可交付、SafeExit、地图尺寸/可见性职责、combo/toast、panel 开关后 idle。性能回归仍需确认 idle WebView2 `SW_HIDE`，并对比 `nativeHud.boundsSource / repaintSource / commit` 计数与 p95。
...
当前证据已覆盖自动门、精确隔离 candidate E2E 与正式标准入口。`node tools/test-agent-entry-contract.js` 锁定 helper 与正常入口的同包 `s:1|ga:<attemptId>`、notifier fail-closed 和 `gotoAndStop("读盘")`；装备和斗兽 runner 的 `--check` 覆盖 post-watermark 真实 title-frame marker、watchdog 拒绝、`title_frame_not_observed`、fresh handoff、exact slot/attempt、single enter 与 attempt-bound `gameEnteredObserved`。Launcher source-tree 全量 **1220/1220**，其中 AgentControl 定向 **29/29**；装备 runner `--check` 为 **34 checks**。隔离 candidate `tmp/runtime-candidates/v2/c-2a0cddb077b7-08846e81b3-20260721t100014612z-f32a40a3`（build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`、payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`、Core SHA-256 `0F58BF864B8DE9C7FCEA098D7E1EEA1996BDE38D85D87E844B047B53F5247232`）曾在 attempt `62f04fa249c44d94ba3171f619e61754` 完成 corrected Agent entry、正常 NativeHud、同实例 organizer、普通 suspend/same-anchor 内容不变 reopen/final claim，因此当时达到 `e2e_verified / NOT_DEPLOYED`；其 native bundle 不含 `launcher/web`，外部 Web 字节仍由测试指南所列源码哈希与实机日志独立绑定。随后相同 build identity / payload closure 已经 request `FBA91942C57DC3E6662AB77AC88EEAE3CA2AFA3C82BF5FFEA6BC68CFD6C31AE4` 的双故障域 quorum 与 promotion；prewarm-held formal attempt `4eae1360cedd413fa3175db6a8997158` 取得 fresh title marker、exact attempt receipt，并完成真实 loot 两次 claim、terminal close/unpause 与存盘，故唯一 canary 已达到 `standard_entry_verified`。此前一次 formal 冷启动因 reveal watchdog 先于真实 title receipt 而返回 `title_frame_not_observed`，保留为换机 portability 历史观察，不覆盖后续成功结论。
...
| 装备调制切片 | `equipment-tuning-runtime.js` + `equipment-tuning-view.js` + `equipment-inspector.js`，嵌入 `workbench view=tuning` | 独立 `domain=equipment_tuning`；顶层为“强化度 / 交换 / 进阶 / 配件”，强化自动核算最终目标且以动态/永久上限分离避免 `+14` 幻象，交换通过 coordinator 只读 projection 在右栏展示同 use 权威候选且不改左栏筛选/面包屑；snapshot 额外提供 Host 强校验的权威男女分支，顶部当前装备图标与已选交换目标可打开共享 185% 交互检视，材料/进阶/配件/候选目录不扩容；候选富注释保持 AS2 intro/desc 分段，workbench 禁止原生 `title` 竞争；配件按插件定义投影的档级/目录用途/定位建立共享树与面包屑并由 tuning host 呈现 DLS 晶体索引风格，已安装件可原子替换或通过专用按钮卸下；顶栏安全/快速偏好仅本地持久化，快速仍经 AS2 preview/token 且只对无连带单件操作自动提交，紧凑候选对齐 owned grid 的 `48/40/4px` 尺寸并以 25 项无滚动为最低画布门；全工作台使用 DLS 基调，强化石核心按密度与 reduced-motion 动静降级；顶栏 `?` 复用 workbench modal 且零业务消息。七类 wire 写全部由 AS2 snapshot/preview/opaque token commit 裁决；Host 维护实例租约、write epoch 与未知写结果精确对账，库存刷新完成前禁止并发转移/整理/关闭 |
...
| 合成工作台 | `crafting-runtime.js` + `crafting.js` + `crafting-inspector.js` 薄适配 + 共享 `equipment-inspector.js`/`dressup-doll-renderer.js`/`item-filter.js`/工作台壳 | 12 类旧合成入口 → `crafting`；Flash 权威 preview/一次性 commit，并以权威 `actionType`/`gender` 驱动普通武器完整 skin、双刀/疾影双 holder、防具性别聚焦或其他当前图标的只读特写 modal；支持批量份数上限、共享树筛选及合成↔战备箱本地子路由，返回后强制重新 snapshot/preview |
| 技能管理 | `skills-runtime.js` + `skills.js` + 独立 `skills` domain | 刘海屏独立「技能」入口与教师入口统一打开 `skills`；教师页可安全 rebind 到管理页；浏览、初学、升级、12 槽装备/卸下/排序和纯被动启用均由 AS2 权威服务裁决，展示复用完整/紧凑、三组首击生效且可组合的 Skill facet 和共享 pointer drag，但不进入 inventory/equipment domain |
...
装备调制不设独立玩家放量开关，也不下发可制造调试/游玩差异的 `tuningAvailable` capability。Native HUD `WAREHOUSE` 的 `profile=battlebox view=storage` 固定显示背包装备卡片级“调制”动作，作为 Web 人工反馈入口；AS2 旧调制子入口在验收前固定留在已服务化 renderer，不发送 Web 请求，且仍调用同一 `EquipmentTuningService`。TaskRegistry 与 Router 对旧 SWF 可能残留的 `source=legacy_equipment_tuning` 请求 fail-closed 返回 `migration_paused`，不允许误开面板；宿舍 `profile=warehouse` 不提供调制。`EQUIP_UI` 始终打开旧装备 Hub，禁止用本切片整体改道。agent source 走同一正式 workbench/profile 语义，但另受专用槽、attempt、runtime-ready 与正式 opener 约束。
...
- **loot**（地图资源箱 S1/S2 单 canary）：`web/modules/loot/loot-runtime.js`、`loot-state.js`、`loot-view.js`、`loot-organizer.js`、`loot-panel.js` 复用 `transfer-pair` 双栏和 owned inventory primitive；lazy deps 固定先加载 panel runtime/lifecycle/focus/workbench primitives/components，再加载 `inventory-ui`、`inventory-runtime` 与五个 loot 模块。AS2 是奖励/容器唯一权威，Host 只做 strict shape、binding、串行、panel 生命周期与断线证明，Web 只发意图。claim-all 冻结有界 `physicalSlot` 队列并串行；claim success 必须同时证明 `authorityRevision` 精确 `+1`、`remainingCount` 精确 `-1`、本次 operation 与 exact requested slot empty，close success 必须同时证明 revision 精确 `+1`、本次 operation 与 close 意图对应的 expected state / remaining。capacity response 只有 strict raw failure shape、exact current binding 与完整 known prestate 同时成立，且已知 revision/remaining/lastApplied/`closeLease`/`containerVersion`/requested slot/lease 全部不变时才是零写证明；错误名本身不是证明，Host 只在该 exact prestate 下保留已知 `closeLease/containerVersion`。Host/Web 各自维护独立 unknown-freshness watermark；只有 matching pending call/identity 中未证明的更高 revision才推进该 unknown operation 的水位且不覆盖已知 authority，其他 call/identity 不得抬高水位，后续 claim/close/query proof 不得回退；普通七键 query 不增加 `reconcileAfterCallId` 等 wire 字段。ordinary `target_full / inventory_full` 现在进入**同一个 tracked `panel=loot`、同一个 `panelInstanceId`** 内的嵌入式背包—战备箱整理子页；不发 `lootClose`、不 suspend、不返回 AS2，也不另开 `workbench` panel，`LootPanelCoordinator`、全局 pause 和 AS2 `LOOT_ACTIVE` 均保持。该子页向 `InventoryTask` 发送严格七键 envelope，仅允许 `domain=inventory` 的 `snapshot/autoTransfer/discard`；Host 要求 `panel=loot` 与 exact active instance capability，`autoTransfer` 只准背包↔战备箱、`discard` 只准背包，瞬态 loot container 从不进入 owned transfer。所有成功、本地拒绝、timeout 与发送失败回包都回显原 `panel/panelInstanceId`，Web mux 也逐项核验；AS2 消息无法注入这条 Host 活跃实例能力。inventory 写完成、未知结果对账收口且 fresh loot ACTIVE snapshot 成功后才返回战利品页；fresh snapshot 失败时留在整理页，不重放写。整理页打开时 X/Esc/backdrop/子页 × 首次只返回战利品页；战利品主页面对非空箱的 X/Esc/backdrop 仍 `close(abandon=false) → LOOT_SUSPENDED`，只有确认“放弃剩余”才 `ABANDONED`。初次 open transport failure 是 legacy renderer 唯一入口，而且 renderer 为 claim-only：禁用 drag/swap/discard、玩家→loot 与特殊资产直写，每次领取仍调用 `LootContainerService.claimLegacyRecoverySlot`；reopen 的 pre/post-accept 故障都只回 suspended/terminal，绝不 legacy。场景切换与 restart cleanup 必须在销毁 world 前通过 `LootContainerService.expireScene`；`removeGameWorld/dispose/reset` 及根清理返回 Boolean，pending 时完整保留场景并只排一个下一帧重试。`cleanupForRestart():Boolean` 必须在任何 manager dispose 前先做 loot preflight，失败时保持整个 runtime 原样；场景淡出也须先停止，成功后才继续播放。真实 open 仍是 nested `panel_request` wrapper；普通 query/recovery/proof query 为 strict 7/8/9-key，proof 字段不进入 Web；socket generation、proof ledger、admission fence 与日志脱敏边界不变。应用重激活使用 generation/single-flight/200ms/前台复核的 WebView2 `MoveFocus`，外部前台不抢焦。当前 source-tree Launcher/Host 全量为 **1220/1220**，其中 AgentControl 定向 **29/29**；map wiring **243/243**、stage audit **41/41**、S0 wiring **454/454**、Host focus **9/9**、共享 focus integration **12/12**、XmlSocket **10/10**、Web loot **46/46 + 62/62 + 6/6**、workbench **0/0** / atlas **48/48** / item-grid **14/14**、save multi-instance **44/44**。fresh CS6 runId `dbcc28a3d99444c38bde38029ff4bd61` 为 Service **159/159** + Planner **7/7** = **166/166**、failed **0**、Compiler Errors **0/0**、BOM **227/227**；publish-only SWF 不能替代 fresh trace。旧隔离 candidate `c-fca19ba526c4-08846e81b3-20260721t060547571z-978633c6` 仅保留为嵌入式 organizer 与 corrected Agent entry 之前的历史。本轮 promotion 前的隔离 candidate `tmp/runtime-candidates/v2/c-2a0cddb077b7-08846e81b3-20260721t100014612z-f32a40a3`（build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`、payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`、Core SHA-256 `0F58BF864B8DE9C7FCEA098D7E1EEA1996BDE38D85D87E844B047B53F5247232`）已载入 `cf7_agent_loot_target_full_v1`，并在 attempt `62f04fa249c44d94ba3171f619e61754` 上完成 start 前日志水位→start→fresh handoff + 真实 marker（watchdog **0**）→唯一 helper→同包 exact receipt→Host exact attempt→runtime ready；人类确认 NativeHud 可见且指令有效，Host `activePanel=stage-select`。同轮又在 exact `panelInstanceId=panel.loot.8a1227b8229f4eef99ac052395e4e12b` 真人完成满背包混合箱→抗生素合并、黑暗吉他保留→organizer→真实确认丢弃牛肉罐头 ×16（50→49）→fresh ACTIVE 返回且零 close/rebind→领取黑暗吉他→terminal close/unpause 回游戏。真实点击最初暴露 shared modal 自身被 `SecondaryPage` underlay 设为 `inert + aria-hidden`；现已让 `secondaryUnderlay()` 排除 `.workbench-modal-layer`，modal FocusScope 只 inert shell 的其他直接子节点，并以 browser **62/62**、focus integration **12/12** 与一次点击实机过门。candidate 的 25-file native bundle 不含 `launcher/web`，故 native identity/closure 不绑定这些外部 Web 字节；精确 SHA-256 见测试指南。随后普通主页于 **18:37:08** 首开，**18:37:18** 单次 close 后以 `instanceDigest=83e7e242b7fc / reason=suspended_already_closed` 关闭并 unpause；**18:37:21** 同锚点 fresh reopen，人类目视强化石 **×2 + ×3** 内容一致且无旧 AS2 UI；**18:37:29** 两次 claim 与一次 terminal close 以 `instanceDigest=6ef2f7f9fcb2 / reason=terminal_already_closed` 收束并 unpause。故该隔离运行当时达到 `e2e_verified / NOT_DEPLOYED`；旧 attempt 的 suspend/原生整理/reopen/consume 与冷启动 SOL 回读只作历史证据。候选已优雅 shutdown，Guardian PID **14820** 与 HTTP/XMLSocket **1192/1924** 均退出，Flash CS6 PID **20444** 保留。随后相同 build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107` / payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C` 已通过 request `FBA91942C57DC3E6662AB77AC88EEAE3CA2AFA3C82BF5FFEA6BC68CFD6C31AE4` 的 production receipt、`builder-local-b` + GitHub OIDC 双故障域 quorum 与 promotion；prewarm-held formal attempt `4eae1360cedd413fa3175db6a8997158` 取得 fresh title marker、exact attempt receipt，并完成真实 loot 两次 claim、terminal close/unpause 与存盘，因此唯一 canary 已达到 `standard_entry_verified`。此前一次 formal 冷启动的 `title_frame_not_observed` 仅保留为 reveal watchdog portability 历史观察。Rock Park 4 个 `TEMP` 夹具已恢复，stage audit **41/41**、真实 XML **28/27/26**、Rock Park **active 1 / commented 8**、rollout **1**；完整 S1/S2 资产及 S0 exact fixture/全局 arbiter 人工门仍未完成，因此仍不得称整体迁移完成，但不再把唯一 canary 表述为未部署。
...
- `panels-lazy-registry.js` + `lazy-loader.js`: 面板懒注册表（id → deps[]）+ 按需 `<script>` 注入，首次 `Panels.open(id)` 才加载对应模块（kshop/workbench/npcshop/crafting/skills/help/jukebox/dressup/map/stage-select/intelligence/arena/team/lockbox/pinalign/gobang/tasks/cutscene-test）
- `tooltip.js`: 复合 hover + anchored 两种模式，AS2 HTML 转换；触发物到浮层保留短过桥时间，长说明支持浮层滚轮、触发物 `PageUp/PageDown` 与 `Esc`，确定性生命周期立即关闭；商城、情报、任务、佣兵、竞技场 runtime tooltip 共用，图标通过 `PanelTooltip.dynamicIconHtml` 接入动态图/分层图播放链
