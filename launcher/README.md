...
本节只描述 H1 activation commit `c5ca5dfa9718e8a7714038e929a64081b7fd0026` 之后的 current source。历史五次 failed attempt 后，S6 `681e328…216e` / tree `387dd12…d683` / candidate `E2CF5C…6B686 / 75D305…B4F11` 又产生两次 operationally invalid/aborted run 与一次 default-device switch crash-invalidated run；八个 runId/capture 均不可复用。S7 Source matrix A fresh 已通过：Node `28/8/12/24/5/5`、native `57`、managed `42/42`、shipped `12`、AS2 static `185`、Launcher full `3543 passed + 3 opt-in skipped / 3546`（resolver `7/7`、exit 0）及生成 closure 已稳定。本节所在版本与最终 source/generated/doc closure 首次同树落盘的 clean implementation commit 定义为下一 Source S；其 commit/tree 只能从提交后的 Git object 解析，并仍须双 clean repro、新 candidate、全新 runId 与完整 A6。当前严格 **H2 blocked / NOT_DEPLOYED**，未进入 runtime promotion；不能把源码、单测、诊断 capture 或旧 candidate 外推为正式入口已升级。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
- 该 pipe **不是音频控制面**：它只写 marker、在 coordinator owner 队列读取 snapshot、读取 journal，绝不直接驱动 native。BGM/SFX 动作仍经生产 AS2 → `AudioTask` → facade/coordinator 链；journal 将 AS2 ingress、coordinator completion/recovery 与 qualification snapshot 分源记录。snapshot 只暴露 backend、设备 digest/name、session/ready/device generation、BGM source、BGM/SFX raw meter 和全部 SFX counters，不暴露 raw endpoint ID；engine session sample format（当前 `f32`）也不等同于 endpoint capture 文件格式（例如 `pcm_s16le`）。`sleep_resume` 必须至少有 1 个 recovery episode；同 episode 的多个 `Recovering` 样本合并，每个顺序 episode 都独立要求在 15 秒内闭合为 `Ready`，最终 post 必须晚于最后 `Ready`（或就是同一事件）且 tuple 相等，零 episode 或任一中间段漂移都 fail-closed。内部 meter 非零仍不能代签物理端点可听。
...
- **失败 attempt 台账**：历史五次 failed attempt 之后，S6 又产生 run `fe73…3766`（TWS 电量耗尽）与 `8320…d1c6`（自动 lane 早于存档选择/Flash prewarm exit）两次 operationally invalid/aborted；run `82a4…9870` 的前十 case 与三份 TWS 非静音诊断 capture 已完成，但 Realtek→TWS 切换令 Core 在 `miniaudio.dll` 以 `0xc0000005` 崩溃，case 无 end/capture/post，整个 run/capture 作废。根因是 miniaudio auto-reroute 与 CF7 owner rebuild 并发 mutation WASAPI graph；S7 已改为 notify-only/owner-only 与上述 bounded recovery，Source matrix A/生成 closure 已稳定。合计八个 runId 均不可复用、无 E1/H2、始终 `NOT_DEPLOYED`；完整 WER/dump、capture 与旧五次细节只在 [Audio v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md) 维护。仍须冻结下一 Source S、双 clean repro、新 candidate 与全新 runId。

**A5 source inventory**：生成器的 shipped-audio 清单共 827 项，精确分为 795 个 `tracked` + 32 个 `ignored_source`；manifest 为 `843455` B / SHA-256 `60CA5E3E6B42842965FBD5A2A8313827D0C2E7AB0CAE4E16D9E8E3B0986B6A9C`。qualification dependency 为 `568` entries / closure `D499F95801801D94CEC50EB6EE43F051BE39BFC976686DD060CA9890D4E2EB3D` / manifest `142054` B / SHA-256 `FAB7B9BA652B91D0114D7AD0A2BE0467E99EABC7CD084480B8376F15A84FA80F`。content sniff 确认的 11 个 AAC 资产已从错误的 `.wav` 名称重命名为 `.m4a`；这些清单是 source truth，不把“被扫描到”偷换成“已可播放”。详细投放说明见 [`../sounds/README.md`](../sounds/README.md)。
