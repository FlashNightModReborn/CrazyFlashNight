         ...
         if(超载许可)
         {
            ...
            if(_root.成功率(反射对象.成功率))
            {
               ...
               if (刀口 && 刀口._parent)
               {
                  ...
                  if(目标 != null)
                  {
                     var 目标瞄准点:Object = UnitUtil.getAimPoint(目标);
                     if (目标瞄准点 == null)
                     {
                        目标瞄准点 = {x:目标._x, y:目标._y - UnitUtil.calculateCenterOffset(目标)};
                     }
                     var 目标碰撞平面Y:Number = 目标瞄准点.y + (射线发射Z - 目标.Z轴坐标);
                     var 射线ZY比例:Number = (目标.Z轴坐标 != 0 && 目标碰撞平面Y != 0) ? 目标.Z轴坐标 / 目标碰撞平面Y : 1;
                     var 最小速度分量:Number = 0.001;
                     var dx:Number = 目标瞄准点.x - 坐标.x;
                     var dy:Number = 目标碰撞平面Y - 坐标.y;
                     if (dx < 最小速度分量 && dx > -最小速度分量)
                     {
                        dx = dx >= 0 ? 最小速度分量 : -最小速度分量;
                     }
                     if (dy < 最小速度分量 && dy > -最小速度分量)
                     {
                        dy = dy >= 0 ? 最小速度分量 : -最小速度分量;
                     }
                     反射对象.射线子弹属性.速度X = dx;
                     反射对象.射线子弹属性.速度Y = dy;
                     反射对象.射线子弹属性.ZY比例 = 射线ZY比例;
