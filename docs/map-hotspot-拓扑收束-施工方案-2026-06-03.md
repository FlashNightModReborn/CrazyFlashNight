...
> **心智模型变更（评审点 6，需广播给协作者）**：收束后改 hotspot 拓扑**不再需要 AS2 重编译**，但**仍需跑 `build.ps1` 刷新派生文件**——"改 JS 即生效"只对纯 Web 渲染层成立，拓扑层有 build 派生这一步。

> **map-panel-data.js 实际派生出 3 个文件（2026-06-03 核实，均已纳入 build.ps1 自动重生成）**：
> | 派生文件 | build 步骤 | 消费方 | 影响 |
> |---|---|---|---|
> | `data/map/map_catalog.json` | Step 1c | AS2 MapPanelCatalog（DataQueryService）| 导航权威 |
> | `data/map/task_npc_registry.json` | Step 1b | AS2 MapTaskNpcRegistry | 任务红点 |
> | `launcher/data/map_hud_data.json` | **Step 1d（本次新增）** | C# MapHudDataCatalog → RightContextWidget | NativeHud 小地图 context 渲染 |
>
> **历史坑（subway "小地图UI关闭" 实测真因）**：map_hud_data 此前只在 Step 7b 做"存在性校验"、不重生成 → 加 hotspot 只改 JS 时它陈旧（旧文件仍在、过校验），运行期 `[RightContextWidget] map hotspot not in catalog: <id>` → 小地图 context 空白。2026-06-03 经 `logs/launcher.log` 定位；已对齐 1b/1c 改为每 build 重生成。
> 注：`export-maphud-data.js` 每次都写盘（非 idempotent skip），有轻微 git mtime 噪声；与 1b/1c 的 skip-if-unchanged 不一致，可选后续统一。
