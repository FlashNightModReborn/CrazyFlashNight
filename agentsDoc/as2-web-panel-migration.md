...
静态门全量审计所有真实网格声明、item catalog 闭包、难度 `CaseSwitch`、概率/总数和容量边界，并解析地图元件 `DOMDocument.xml` 的真实 Include closure：六个 canonical 箱体 linkage 必须唯一、统一初始化与开启回调、破碎拓扑符合资产语义，两个带旧直投脚本的顶层孤儿 symbol 必须保持未 Include；不再把易漂移的时间轴帧号当准入条件。`最小数量/最大数量` 任一缺省由 planner 私有归一为 `1/1`，不批量改生产 XML。当前树已通过 stage **55**、wiring **180**、Launcher **1141/1141**，fresh TestLoader 每个完整块为 Arbiter **13/13 cases / 53 assertions**、Service **129/129**、Planner **7/7**、Compiler Errors **0/0**；`asLoader.swf` 与地图元件 SWF 也已取得独立发布和哈希证据，因此达到 `compiled`。旧单-canary 的 candidate、promotion 与 standard-entry 仍只作历史记录，不能继承给当前 source；当前剩余门是绑定最终提交的 candidate、代表性 E2E、正式发布与标准入口复核。
