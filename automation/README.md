...
Audio Platform v2 R4 不改变 start/operator 命令面。`sleep_resume` 收尾需在末 closing `Ready` 后取得两份显式 snapshot，由 observer 在同一 final generation/physical tuple 的同一路 bus 上证明 frame 前进与非静音。正常 H2 合规路径仍要求有效 E2/H2 后才创建 source tag/request 与正式 builders，并在 request 后形成 evidence-only E3 `h2-request-link.json`；2026-08-15 的 owner-emergency promotion 明示 bypass E1/H2/E3，不得反向解释为这些证据已存在。两条路径都不放宽 immutable request、双 signer / 双 faultDomain、production policy、strict verifier、原子替换与 rollback。
...
正式 v2 consensus 当前绑定 tag `runtime-build-v2/20260731-agent-runtime-wings-f8-v1`、request `A9B33601805709DBB5EAE6DAF312C2B7B0B502096FDD3BDCEA9CBE26D8B1299C`、source commit `6f3d50a52413c747b05b74be88d6ee46650f4597`、build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C` 与 payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0`。local signer 位于 `physical-host-a`，不可导出 keyId 为 `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`；GitHub Actions run `30602046108` 的 OIDC identity `2E05B5577CC8D2895DC56FC8778A05D09C440302832A05DD4AE551589C9AE918` 提供 `github-hosted-windows` 第二票。production policy `33A18135EEB9094F305C63ADA887600230C18AD49FA0AE151E469A4F68DDC09F` 的 26/26 final receipt SHA-256 为 `DA46C6E99CB02A268099ACB709C80B60E8B2D821134E1D32C9C84CB288EEC38C`；promotion 于 `2026-07-31T03:56:21.4933374Z` 成功，严格 manifest/consensus 校验为 33 files、2 signers、2 faultDomains。cloud workflow 仍只允许 `Crazyfs` / `Flash-Night` 的固定 actor ID 首次 dispatch。
