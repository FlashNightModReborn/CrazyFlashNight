import { DAMAGE_RANDOM_MAX, DAMAGE_RANDOM_MIN, DAMAGE_SCALE, HIT_MAX, HIT_MIN, MAX_BATTLE_ROUNDS, } from '../data/config.js';
import { clamp } from '../core/math.js';
import { DeterministicRng, deterministicTie } from '../core/rng.js';
function initialNextAttackRound(behaviorId) {
    switch (behaviorId) {
        case 'ammo': return 2;
...
function nextInterval(behaviorId) {
    switch (behaviorId) {
        case 'sniper':
        case 'ammo':
            return 2;
...
function createCombatant(unit, side) {
    return {
        ...unit,
        tags: [...unit.tags],
        side,
...
function displayOrder(units) {
    return [...units].sort((a, b) => (a.formationRank - b.formationRank
        || hpRatio(b) - hpRatio(a)
        || a.pieceId.localeCompare(b.pieceId)));
}
function normalTargets(enemies, count) {
    const candidates = alive(enemies);
    const picked = [];
    while (picked.length < count && candidates.length > 0) {
        candidates.sort((a, b) => (a.formationRank - b.formationRank
            || hpRatio(a) - hpRatio(b)
            || a.pieceId.localeCompare(b.pieceId)));
...
function sniperTargets(enemies, count) {
    const candidates = alive(enemies);
...
function targetCount(behaviorId) {
    return behaviorId === 'sniper' || behaviorId === 'ammo' ? 2 : 1;
}
function selectTargets(actor, enemies) {
    if (actor.behaviorId === 'sniper')
        return sniperTargets(enemies, targetCount(actor.behaviorId));
    return normalTargets(enemies, targetCount(actor.behaviorId));
}
function tagMultiplier(actor, target) {
    if (actor.behaviorId === 'assault' && target.tags.includes('human'))
        return 1.5;
    if (actor.behaviorId === 'sniper' && target.tags.includes('elite'))
        return 1.5;
    return 1;
}
function rollDamage(actor, target, rng, nodeDefenseBonus) {
    const hitChance = clamp(0.9 + (actor.speed - target.speed) / 200, HIT_MIN, HIT_MAX);
    const roll = rng.next();
    const multiplier = tagMultiplier(actor, target);
    if (roll > hitChance)
        return { hit: false, hitChance, roll, damage: 0, multiplier };
    const defenseBonus = target.side === 'defender' ? nodeDefenseBonus : 0;
    const effectiveDefense = target.defense * (1 + defenseBonus);
    const damage = Math.max(1, Math.floor(actor.attack
        * DAMAGE_SCALE
...
function immutableEvents(events) {
...
export function resolveBattle(input) {
    ...
    const sideUnits = (side) => side === 'attacker' ? attackers : defenders;
    const enemyUnits = (side) => side === 'attacker' ? defenders : attackers;
    const finish = (winner, reason, battleRounds) => {
        push({
    ...
    for (let battleRound = 1; battleRound <= maxRounds; battleRound += 1) {
        ...
        if (battleRound === 1) {
            ...
            if (allDead(attackers) && allDead(defenders))
                return finish('defender', 'mutual_wipe', battleRound);
            if (allDead(defenders))
                return finish('attacker', 'wiped', battleRound);
            if (allDead(attackers))
                return finish('defender', 'wiped', battleRound);
        ...
        const actionOrder = alive(allUnits).sort((a, b) => (b.speed - a.speed
        ...
        for (const actor of actionOrder) {
            if (actor.dead || actor.hp <= 0)
                continue;
            if (actor.nextAttackRound > battleRound)
                continue;
            if (battleRound === 1 && actor.behaviorId === 'sniper')
                continue;
