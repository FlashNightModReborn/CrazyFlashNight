...
**最后核对代码基线**：commit `e979c2e8ba64aa2d9ec24aa579d8654c365540a8`（2026-08-27，库存显式批量转移真批量事务及封闭审计映射；`HUMAN_ACCEPTANCE_PASSED / NOT_DEPLOYED`）。Web 以一次 `autoTransferBatch` 提交最多 50 个有序 lease，Host 严格重建请求与 full/partial 回包，AS2 一次扫描目标、每容器一次 revision/index rebuild、一次窗口组重投影；旧 `autoTransfer` 只保留给单件 `Ctrl+单击`。fresh Shared TestLoader 为 EquipmentInventory `28/28` + Inventory `170/170`、Compiler `0/0`、32K retry `0`；同进程 50 件 fixture 为旧逐项约 `673 ms`、新 batch 约 `20 ms`。维护者随后在 exact isolated candidate 中确认大量物资即时完成转移；Host 日志分别以单一 call 完成 10 件（约 423 ms）和 31 件（约 277 ms），无 reconcile/stale/timeout/rollback/重复派发，退出时 shadow save/confirm 成功且 Flash code 0。候选日志把新命令投影为 `other` 的词表缺项已由本 commit 补为 exact `autoTransferBatch → inventoryAutoTransferBatch`；该纯审计修复不改变已验收业务语义，正式 runtime 仍待双故障域共识与 promotion。
