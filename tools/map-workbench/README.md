...
第二阶段已于 2026-09-07 完成双构建共识和原子 promotion：页面／地点／NPC／任务端点／素材与同一 C# 地图域进入正式 runtime。身份／共识／部署状态以[运行库记录](../../docs/runtime-build-reproducibility.md)及[两阶段记录](../../docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)为准。作者流程确认与 promotion 不代签全部真实游戏旅程。

正式游戏入口：普通启动器 → **其他 → 工具 → 地图工作台**。根目录 `地图工作台测试启动.cmd` 仍是后续源码开发用的隔离候选入口，不等于正式入口；正常使用本次发布无需再运行该开发构建。不要把单独更新的 v2 定义／asLoader 配给旧版本 Core。

游戏内帮助的应用／排错章节仍保留隔离候选验收时的测试 CMD 说明。正式版应用后请完整退出，再从普通启动器重开；不需要为了查看已应用内容改走开发构建。
...
5. 对照两套方案、保存前内容和原因树，检查所有受影响文件后应用；完整重启启动器再看游戏效果。删除被引用对象前会列出任务、驻点和表现。撤回只恢复本批字节，遇后续独立修改或仍被共享引用的新增图片会拒绝覆盖／删除。
