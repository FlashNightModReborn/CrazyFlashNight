...
**最后核对代码基线**：release source commit `c666c3bbaa18d505c289f488094d098361fe432d`（2026-09-04；tag `runtime-build-v2/20260904-reward-save-storm-v2`、request `23AD3915889502F63F198E98603FA9436775F9D0D1A02EACBE019B650273278D`、deployment `81396275e24ee31182a24bd50c60c977111cc067`）。
本地 X509 `builder-local-c` / `physical-host-c`（keyId `CFB70E2D339ACB25E9B6C2873DF4F1AEEBA8EA75AD23B825724B27FCA70C0B86`）与 GitHub OIDC/Sigstore builder `AD099B7E03564E2BDEE20378EB26FC37DA9B2B238C146F874B42BB975ED3411F` / `github-hosted-windows`（cloud run `33820443529`）已对 identity `6AEDAFF36B75BA0C810B03F5A7B2241F3866A74F2DBF60BF2B549C17B4D1100E`、closure `825F5B2F214A2C64C93FB66D81D38A37151B8D703522BF85CE5500188A669362` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `23FD3DA709ABE22A7EA0F3CE6FFBFBB135821A8CF2B2B5AC636F6605B611570A`，40/40 production receipt SHA-256 为 `7339FD925F14A7E5D307D7E16A77780831754E1E0FD5C233AFFF1B313320985C`，manifest / consensus SHA-256 分别为 `5E7482F913002BAEA0D0C89525A9787DAE401E5E548514E5F7259AF5D3EF720B` / `1B77203DCCD3459ACC839C24976C329C4DE3AA38679A39441BEB5E4D34218203`。
正式 runtime 已原子 promotion，并通过 signed consensus、GitHub proof replay 与正式根 bootstrap `--verify-only`；deployment 已推送，post-promotion Audit run `33821697381` 通过并精确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：W/B0 窗口生命周期根治与 A/A1 Reward 根事务 + S/O1 观测均保持 `HUMAN_ACCEPTANCE_PASSED / promoted`，部署后未从正式入口重跑各自业务旅程，不称对应专项 `standard_entry_verified`。loot 车道观测列车（堕落城深处奖励数据修复与主线 39 解锁、开箱拒开 toast、O1 tuple 扩展 `lootLane` / `rewardInboxActive`）已完成自动门、双故障域共识、原子 promotion、部署推送与 post-promotion Audit，当前准确状态为 `promoted / FIELD_REVALIDATION_PENDING`。Reward 存盘风暴止血列车（durable-cut 桥接 2N+1→N+1、pending 混合投影三层封死、语义 fault-cut、F5 清理、K 店 durable finality、材料字典 provenance 重绑定）已完成自动门、双故障域共识、原子 promotion、部署推送与 post-promotion Audit，当前准确状态为 `promoted / FIELD_REVALIDATION_PENDING`：部署后尚未从无 candidate 正式入口重跑收件箱连领、连续开箱与 K 店 checkout/claim 业务旅程，不称 `e2e_verified` 或业务 `standard_entry_verified`，由测试员现场反馈收束；该列车自动门证据与后续路线（R1 分层 19 步 / R3a / R3b）见 [存盘风暴止血 ADR](docs/存盘风暴止血-Reward相邻durable-cut桥接与存盘次数回归门-ADR-2026-09-03.md)。完整状态见 [止血治理 ADR](docs/AS2-WebPanel止血治理-窗口生命周期与Reward根事务-ADR-2026-09-01.md) 与 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)。
