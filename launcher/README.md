...
| 6    | 拷贝产物到 projectRoot：(a) `publishDir/*` 除 `*.xml`（含 Core.exe 全套） (b) `bootstrap.exe` → projectRoot 改名 `CRAZYFLASHER7MercenaryEmpire.exe`（用户面入口） (c) `sol_parser.dll` + `miniaudio.dll` from `bin/Release`；**硬断言 `CRAZYFLASHER7MercenaryEmpire.exe` + `Core.exe` + `Core.dll` + `sol_parser.dll` + `miniaudio.dll` 全部落盘**（任一缺失 exit 1）；`tools/dotnet-runtime/*.exe` 缺失给 WARN（bootstrap 装运行时需要） |
