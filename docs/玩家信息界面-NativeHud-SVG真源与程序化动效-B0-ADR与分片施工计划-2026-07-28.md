...
**当前裁决状态**：总体仍为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`；build/Kimi 自动门与人类签收都尚未闭合，绝不表示“只剩人签”。main-space v2 已在 clean merge `40853287…` 通过 B0-05 32/32、B0-06 48/48 与完整 Launcher 1747+3/1750；tracked `visual-evidence.v2` 已冻结 C# A/B `36 files / 35 PNG / 1,261,624 B / DD58CA7C…6661E`、Web A/B `12 files / 11 PNG / 221,781 B / 9D21D823…17A7B`、direct A/B `66 PNG / 3,114,450 B / F05AE7F7…50ABF` 及 viewer `651,451 B / 2F71A3ED…01BC2`，MP 四字段均为 unique `bestDx=0` 且对应 anchor edge `dx=0`。child authoring wrapper 已排除，Flash/C#/Web 均以 1024×576 main content viewport 为基准；MP current 右锚 `74.55`、maximum authored `80.65` + C# local phase `0.30`、HP Glow/横线均已纳入上述 clean closure。真实 Flash v2 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 的 tooling strict identity 通过，但 4 项 human review 仍为空，所以只到 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`；自动图像明确不声明 parity。新 v2 tag/request/双 builder quorum/preflight、最终 `kimi-code/k3 + high`、Computer Use 实际初始化与人类验收尚未完成。旧 F/G/quorum/K1 只作 v1 historical；正式 runtime、真实 `pi_*`、promotion 与部署均未改变。
...
- v2 B0-06 自动资格与视觉诊断已完成：clean `40853287…` 的 run `ed61d727…` 为 48/48，完整 Launcher 为 1747+3/1750，tracked visual index 已冻结两份 C#/Web/direct/viewer。独立 split surface、fixture widget、程序化 HP/MP、MP 双 fragment、path glyph atlas、prepared DIB 与 PanelHost 生命周期均进入 exact closure；最终 Kimi 与人类视觉/桌面门仍未完成。
...
- 总体视觉交付严格状态：`visual_correction_in_progress; human_exit_blocked`；其中资产/实现子状态是 `main_space_v2_automated_qualification_passed; build_kimi_human_pending`，oracle 子状态是 `standalone_child_diagnostic_captured; main_rsl_equivalent_candidate_captured; awaiting_human_review`。不得把结构化验证、tracked/ignored diagnostic、FFDec/Web 指标、旧 Kimi PASS 或错 placement 的 Flash candidate 写成 `fixture_static_parity`、`fixture_full_parity`、`oracle_frozen`、`runtime_integrated`，更不得写成已部署。
...
- [x] `launcher/README.md`、testing guide、工具 README、总体迁移 ADR 与本台账已同步 clean `40853287…` 的 qualification/full/视觉 closure 及当前 Apps 边界；旧 F/G docs-only 事实继续只作 v1 history，Kimi/build/human 仍明确 pending。
- [ ] main-space v2 的 evidence/docs-only 后继 commit 尚未形成；三份 tracked B0-05/B0-06/visual v2 snapshot 与本轮 canonical docs 已在工作树备妥，但还须由独立后继提交承载，并复核 clean-tree、doc governance 与 `git diff --check`。最终 K1 身份只能在 Kimi 完整审计后另行回写；旧 `81168e…c549` 只承载 v1。
...
- [ ] §9.5 文档级 K0 与 §9.6 v1 K1 事实保留；main-space v2 的完整 source/asset/evidence/doc 增量尚未以 `kimi-code/k3`、当前最高可用思考强度 `high` 重新审计。必须记录新 CLI/session/prompt/input/output 身份与逐条 disposition；不得复用 clean G 的旧 PASS，也不得因耗时降级或切换模型。
...
| B0-06 | `main_space_v2_automated_qualification_passed; build_kimi_human_pending` | clean merge `40853287…` 的 formal run `ed61d7275fdd4a96a73f762c5c2a4d71` 为 48/48、**653,904 B**、SHA `182C7DC9…CA8E1`；clean full 为 1747 pass + 3 opt-in skip / 1750。两份 C# 36-file、Web 12-file、direct 66-PNG 与 viewer 自动包均确定；MP 四字段 unique `bestDx=0` 且 anchor edge `dx=0`。prepared top-down PArgb DIB/DC、真实 ULW transaction 与 lifecycle/source/binary/renderer closure 均受报告约束 | 新 tag/request/双 builder quorum/preflight、Kimi `k3/high`、Computer Use 实际初始化、Flash oracle/真实桌面与全部人工项仍待完成；自动图像只作 diagnostic，不批准 parity/部署 | 先闭合新 build 与强制 Kimi，再做 Computer Use/真实游戏/IDE 身份和人签；新版本不得复用或移动 v1 tag |
| B0-07 | `v1_k1_historical; corrected_delta_audit_required` | v1 Kimi k3/max 审计 113/113 与 P0/P1=0 继续作为旧实现历史；其输入未包含 r4 placement 纠错与人类反馈 | 对纠正后的完整增量以 `kimi-code/k3` 重新执行一轮最高可用思考强度异构对抗，并完成运行时 SWF、new main-RSL oracle、游戏 composite、关键比例/平滑、混合 DPI、真实手点、审美与人类明确接受 | 自动重门与新 K1 完成后再交人类终审；自动桌面/FFDec/Edge/Kimi 均不能代签 |
...
> v1 工程轨曾冻结 F=`cb38600…f511` 与不可变 tag，并由 clean F Lane、两个正式 builder、production receipt、`-VerifyOnly` preflight 和 clean-G K1 证明旧字节；这些事实保持历史有效，但不得外推到 main-space v2，也不得移动/删除 v1 tag。当前 v2 implementation/source 已在 `40853287…` 纳入上游；clean B0-05 32/32、B0-06 48/48、full 1747+3/1750 以及两份 C#/Web/direct/viewer 自动包均已形成，MP 四字段都是 unique `bestDx=0` + anchor edge `dx=0`。真实 11-case Flash candidate 仍只有 strict tooling identity，human review 四项为空，所有自动图像都只是 diagnostic。恢复时先确认本轮 canonical docs/evidence 后继提交与远端祖先关系，再创建全新 v2 tag/request，完成双 builder/双 faultDomain quorum 与不可复用 preflight；随后以 `kimi-code/k3` + 当前最高可用 `high` 完整审计，不得降级或切换模型。Computer Use/Browser 当前仅 skill 已暴露、尚未初始化，之后仍要核验旧 Flash 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实桌面手点、运行时 SWF/IDE Output 身份与审美人签。当前总体状态仍为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`；B0 不要求 promotion、标准入口或部署，build/Kimi/人签全部完成前不得写成 parity、`oracle_frozen`、`awaiting_human_acceptance` 或 `b0_accepted`。每完成一片都在同一提交回写 §12。
