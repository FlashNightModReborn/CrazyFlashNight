...
**PlayerInfo main-space v2 当前状态**：下面保留的 F/tag/request/quorum/47-Gate/Kimi 数值均为历史 v1 记录，不批准纠正后的 v2 字节。v2 尚待 fresh source-freeze 与完整双 builder `-VerifyOnly` 闭环；`config/build/runtime-github-builder.v2.json` 已预留全新一次性 `refs/tags/runtime-build-v2/20260729-player-info-nativehud-b0-v2`，该 tag 当前尚未创建，旧 v1 tag 永不移动或复用。v2 tag 一旦创建就不可移动；只有后续 Kimi disposition 导致已冻结的 F/source-policy 输入字节变化时，才必须改用新的 tag/request。source-freeze 后仅追加 evidence/docs-only disposition 不递归重冻。v2 仍严格为 `source-ahead / NOT_DEPLOYED`，本轮不 promotion、不改正式 runtime 或 release state。
