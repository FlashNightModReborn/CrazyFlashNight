 ...
 *
 * 依赖：
 * - 引擎_fs_路由基础.as（状态切换作业机制、容器初始化对象构建、屏蔽旧man卸载、绑定容器结束写状态、同帧跳转保护）
 *
 * 容器约定：
...
_root.空手攻击路由.__job_跨容器跳转 = function(u:MovieClip):Void {
    ...
    _root.路由基础.绑定容器结束写状态(man, u, _root.路由基础.SMALL_END_BAREHAND);
...
_root.空手攻击路由.跨容器标签跳转 = function(unit:MovieClip, containerActionName:String, targetLabel:String):Void {
    ...
    var job:Object = _root.路由基础.创建状态切换作业(
        unit,
        _root.路由基础.LABEL_CONTAINER,
...
_root.空手攻击路由.载入后跳转空手攻击容器 = function(container:MovieClip, unit:MovieClip):MovieClip {
    ...
    _root.路由基础.绑定容器结束写状态(man, unit, _root.路由基础.SMALL_END_BAREHAND);
