...
本节只描述 H1 activation commit `c5ca5dfa9718e8a7714038e929a64081b7fd0026` 之后的 current source。上一冻结 S `90985aecc9a3ce9a0bcd1ad11dde2b7158705644` / tree `beaaab2622bb9d3bfaa44485d63e274de14224a3` 的 isolated A6 attempt 已失败并作废；本节所在版本与完整修复 closure 首次同树落盘的 clean implementation commit 定义为新的 Source S，commit/tree 不在自身文档 blob 中预填，而由提交后的 Git object 解析。该 exact S 仍须重新执行双 clean repro、构建新 candidate 并从 case 1 重跑 A6。当前继续为 **H2 blocked / NOT_DEPLOYED**，未进入 runtime promotion；不能把源码、单元测试、本地 DLL 或旧 candidate 外推为玩家正式入口已经升级。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
- **失败 attempt 台账**：旧 S 的 candidate identity `1A675D3F200557E41E2821E74CE31309107051FE0EAEC79D9A402E7DFA146CEB` / closure `D5232F5688598118196EE09945E6FE2C4AD2C739AFB247A985BD6C920DA96A1F` 以 runId `cdf7c0efbd6745d29ddb467808259f64` 进入 case 1 `bgm_playback`，但未观察到 AS2 ingress；native source 投影 `playing=true`，cursor 与 meter 仍为 0，operator 因静音严格 fail-close。candidate 已优雅停止，始终 `NOT_DEPLOYED`；没有 E1/H2，该 runId 永久作废。任何修复后的验证必须绑定新 S 的新双 clean repro、新 candidate 与新 runId。
