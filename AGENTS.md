...
**最后核对代码基线**：release source commit `d10d587e79bb84e04b8bb58146ee799129ef1c39`（2026-09-03；tag `runtime-build-v2/20260903-loot-lane-observation-v1`、tree `912210f10c65dff72c71bd9f6da39b548747cab9`、request `22D549D3F7042FAB0A54D396D82311B7E0987DFB13A202432F3F783F912BE943`、deployment `793c8b960235e30c94f7d1cd8dfe56f7dd5cdb20`）。
本地 X509 `builder-local-c` / `physical-host-c`（keyId `CFB70E2D339ACB25E9B6C2873DF4F1AEEBA8EA75AD23B825724B27FCA70C0B86`）与 GitHub OIDC/Sigstore builder `CC063AE7B662E883F4B83B166144C873375F7497FE071BD62EADB622C60053C2` / `github-hosted-windows`（cloud run `33717586170`）已对 identity `CE754EDD6BB0E24DC98F347A6C3F26FC1836417C54A0A8D10C1F145F2CDF358F`、closure `DD530373E6FD1BFFC21ABCDFBBD78A1A76ED93DF3E5C4EB75A85513209C6725C` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `9C7055B4FD1E655A6FC217E0AFA141C81F998B030C4B82CDC6F70426CE9213EE`，40/40 production receipt SHA-256 为 `6A2558D754D5DE3BE3580F9E69B4A8C730AECC1B4E5930B8BF6262913D80528A`，manifest / consensus SHA-256 分别为 `847545E68D32AE85F599190E68725799A1597055BF968E825D5CF8DC6B3524BD` / `F541B95FF0A843E77C209FF00793E0E47C9EC85F9BEBF31E80AE812401F2DF6B`。
正式 runtime 已于 `2026-09-03T05:21:14.4751506Z` 原子 promotion，并通过 signed consensus、GitHub proof replay 与正式根 bootstrap `--verify-only`；deployment 已推送，首次 post-promotion Audit run `33718671517` 通过并精确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：W/B0 窗口生命周期根治与 A/A1 Reward 根事务 + S/O1 观测均保持 `HUMAN_ACCEPTANCE_PASSED / promoted`，部署后未从正式入口重跑各自业务旅程，不称对应专项 `standard_entry_verified`。loot 车道观测列车（堕落城深处奖励数据修复与主线 39 解锁、开箱拒开 toast、O1 tuple 扩展 `lootLane` / `rewardInboxActive`）已完成自动门、本地 X509 + GitHub Hosted 双故障域共识、原子 promotion、部署推送与 post-promotion Audit，当前准确状态为 `promoted / FIELD_REVALIDATION_PENDING`：部署后尚未从无 candidate 正式入口重跑主线 39 交付、拒开 toast 与连续开箱回归，不称 `e2e_verified` 或业务 `standard_entry_verified`，由测试员现场反馈收束。完整状态见 [止血治理 ADR](docs/AS2-WebPanel止血治理-窗口生命周期与Reward根事务-ADR-2026-09-01.md) 与 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)。
