...
**当前裁决状态**：总体为 `awaiting_human_acceptance; human_exit_blocked`。main-space v2 已在 clean merge `40853287…` 通过 B0-05 32/32、B0-06 48/48 与完整 Launcher 1747+3/1750；tracked `visual-evidence.v2` 已冻结 C# A/B `36 files / 35 PNG / 1,261,624 B / DD58CA7C…6661E`、Web A/B `12 files / 11 PNG / 221,781 B / 9D21D823…17A7B`、direct A/B `66 PNG / 3,114,450 B / F05AE7F7…50ABF` 及 viewer `651,451 B / 2F71A3ED…01BC2`，MP 四字段均为 unique `bestDx=0` 且对应 anchor edge `dx=0`。child authoring wrapper 已排除，Flash/C#/Web 均以 1024×576 main content viewport 为基准；MP current 右锚 `74.55`、maximum authored `80.65` + C# local phase `0.30`、HP Glow/横线均已纳入上述 clean closure。真实 Flash v2 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 的 tooling strict identity 通过，但 4 项 human review 仍为空，所以只到 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`；自动图像明确不声明 parity。最终 `kimi-code/k3 + high` 已对 clean evidence/docs commit `884fbf43…` 完成同一会话的完整只读审计，165/165 个请求模型/effort 全部命中，结论 `APPROVE_AUTOMATED_CLOSEOUT / K1_AUTOMATED_GATE=PASS`、P0/P1=0。F2/r2 的全新 request、本地 X509 + GitHub OIDC 双 builder、两份 23/23 production receipt、双 signer/双 faultDomain quorum 与不可复用 `-VerifyOnly` preflight 也已闭合；严格生命周期仍只有 `candidate_built / NOT_DEPLOYED`。本次 Computer Use 原生管道初始化两次均因 native pipe 不存在失败，Browser 初始化后 `browsers=[]`；这只是当前控制后端不可用，不降低自动闭环，也不能代签可见桌面。现在只剩 Flash oracle 来源/crop、真实 DWM/游戏 composite、鼠标透传、关键比例/平滑、混合 DPI、运行时 SWF/IDE Output 身份、审美与人类明确接受。旧 F/G/quorum/K1 只作 v1 historical；正式 runtime、真实 `pi_*`、promotion 与部署均未改变。
...
- 已完成历史诊断与 v2 candidate capture、未取得 accepted main oracle：B0-01B 第七轮 v1 candidate 已绑定 fresh TestLoader、实际 Debug Player、精确 child、11-case raw/crop 与安全恢复，但它直接加载整个 child 文档，包含 `ty=3` wrapper；状态降为 `standalone_child_diagnostic_captured`。当前 v2 tooling 以 `main_rsl_exported_symbol_equivalent` / wrapper-excluded / 1024×576 合同完成真实 11-case capture，tool strict identity 为 `head_index_clean_filter_equal`，且 clean `40853287…` 的 direct A/B 已重新消费该 candidate；但 human review 四项仍为空，当前只能写 `candidate_captured; awaiting_human_review`，不得写成 `oracle_frozen`。
- 已完成自动结构与视觉诊断：B0-04 的 8 个 canonical SVG、runtime manifest、repo-only provenance、strict validator 与 Web、FFDec/Web 诊断已闭合自动结构门；current asset 为 99,576 B / revision `d67ab427…92ba1` / manifest SHA `6F0AAA45…FEF826`。tracked `visual-evidence.v2` 已冻结 placement-correct C#/Web/direct A/B，但 Web 仍只捕获 canonical static SVG，不含 C# 程序化文字/Glow；自动指标没有 parity verdict。一次性 converter 原型按 stop-loss 已移出权威工具树且不进入 build，视觉接受仍等待 Flash oracle 与人工复核。
...
- v2 B0-06 自动资格与视觉诊断已完成：clean `40853287…` 的 run `ed61d727…` 为 48/48，完整 Launcher 为 1747+3/1750，tracked visual index 已冻结两份 C#/Web/direct/viewer。独立 split surface、fixture widget、程序化 HP/MP、MP 双 fragment、path glyph atlas、prepared DIB 与 PanelHost 生命周期均进入 exact closure；最终 Kimi 已按 §9.7 PASS，人类视觉/桌面门仍未完成。
...
- 总体视觉交付严格状态：`awaiting_human_acceptance; human_exit_blocked`；其中资产/实现子状态是 `main_space_v2_automated_closeout_passed; human_pending`，oracle 子状态是 `standalone_child_diagnostic_captured; main_rsl_equivalent_candidate_captured; awaiting_human_review`。不得把结构化验证、tracked/ignored diagnostic、FFDec/Web 指标、Kimi PASS 或错 placement 的 Flash candidate 写成 `fixture_static_parity`、`fixture_full_parity`、`oracle_frozen`、`runtime_integrated`，更不得写成已部署。
...
**Oracle 轨**：第七轮 Flash candidate 仅保留为 standalone child 历史诊断，不能再进入 accepted review。v2 TestLoader/runner 从精确 child 中隔离 exported-symbol instance，排除 `ty=3` wrapper，以 identity root + main `ty=512` 生成 1024×576 `main_rsl_exported_symbol_equivalent` capture；manifest/receipt 均为 v2，历史 v1 fail-closed。真实 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 已完成 11 case，tool strict identity 通过，并被 clean `40853287…` 的两份 direct comparator 重新消费；下一步是完成四个 manifest check（状态、层隔离、crop、审美）及独立来源身份现场复核，形成 review receipt。闭合前只能保持 `candidate_captured; awaiting_human_review`，不得写成 `oracle_frozen`。

**工程轨**：B0-05 的 raster/cache/PArgb 与 `split_required` 架构裁决仍有效。main-space v2 已修正 child-stage 非裁剪语义、HP/MP placement、MP 文字锚点/底字、HP Glow 与横线效果，并把 C#/Web/Flash capture/comparator 统一到 main content viewport。clean `40853287…` 的 B0-05 32/32、B0-06 48/48、full 1747+3/1750、两份独立 C#/Web/direct 与诊断 viewer 已形成 final source-bound automatic evidence；最终 `kimi-code/k3 + high` 和 F2/r2 的双 builder/quorum/不可复用 preflight 也已闭合。下一步直接进入真实桌面、oracle/game composite、真实手点、关键比例与平滑、混合 DPI、运行时 SWF/IDE Output 身份与审美签收。
...
tools/player-info-hud/
  ...
  evidence/
    ...
    b0-06/
      ...
      visual-evidence.json                # current final-tree v2 automated diagnostic index; human review remains pending
...
FFDec binary reference 另冻结 23 files / 22 PNG：PNG-only 为 245,587 B，30,433 B report 计入后完整目录为 276,020 B / closure `AEC84002…E59`，reference revision `sha256:a8369211…79c0`；FFDec↔Web A/B 的 33-PNG 子闭包均为 698,799 B / `BC2A83D1…9E08A`，standalone-child Flash-candidate↔Web A/B 的 33-PNG 子闭包均为 661,944 B / `6141D387…C185`。这些都是 placement 纠错前的 supplemental history，各 A/B report 因输入路径不同而保留不同哈希，不能写成“34/34 全文件相同”。current v2 index 顶层 `final_tree_diagnostic_awaiting_human_review` 与 comparator 的 `diagnostic_awaiting_human_review` 只表示 clean final-tree 自动闭包已冻结且仍待人审；它们没有接受阈值，也不表示 Flash oracle 已接受。FFDec、headless Web、C# offscreen capture 与 placement-correct Flash candidate 都仍只是诊断；人类必须查看透明 crop、真实游戏 composite、关键比例/平滑、旧 HUD 同屏、可见 DWM/z-order/occlusion、混合 DPI 与手工鼠标透传。原始图片当前仍在 ignored `tmp/`，人类签收及后续约定打包完成前禁止清理；Git 中的索引与哈希不能代替可观看原件。
...
过度设计审计结论是保留 strict SVG 双层 fail-closed、8-field subpixel key、whole-batch atomic LRU、single-worker latest-wins、独立 split surface、path glyph atlas、双层 B0-05/06 gate 与 embedded-resource closure；它们各有当前风险/测量锚。通用 XFL→SVG converter、通用效果图和独立 schema 目录已正确停止扩张。视觉上不建议把生产叠层改为 Difference/Invert：这些只适合 viewer 诊断，游戏仍保持 source-over。自动图只排除了 MP 整体几何错位；HP halo 强度/色调、`%` 字形、真实 DWM composite、z-order/occlusion、鼠标透传、关键比例/平滑、混合 DPI、运行时 SWF/IDE Output 身份和整体审美继续由人类终审。
...
- [x] `launcher/README.md`、testing guide、工具 README、runtime deep doc、automation README、总体迁移 ADR 与本台账已同步 clean `40853287…` 的 qualification/full/视觉 closure、F2/r2 自动闭环及当前控制能力边界；旧 F/G docs-only 事实继续只作 v1 history，当前只剩 desktop/oracle/human Gate。
...
| B0-01A | completed (`placement_closure_frozen`) | r4：16-file exact closure、HP/MP/shared=10/5/2、17 authored/18 expanded edge、14 timeline、0 BitmapFill/DOMBitmapInstance；Git-canonical digest `6f4bf9…4368`；index/clean-filter/anchor/source-binary verifier 全绿；另独立证明 standalone root `ty=3`/HP-MP `5.65/-1.3` 与 main RSL root `ty=0`/HP-MP `2.65/-4.3` | placement-correct TestLoader/player capture 的 accepted 人工复核归 B0-01B；真实 v2 candidate 已捕获但尚未签收，不声称 `oracle_frozen` | `40853287…` 已重跑 C#/Web/direct 自动链；B0-04 结构保留、视觉 Gate 等人工 01B |
| B0-01B | `standalone_child_diagnostic_captured; main_rsl_equivalent_candidate_captured; awaiting_human_review` | 第七轮 v1 candidate 保留为 wrapper 错位历史。v2 template/runner/protocol 已升级为 1024×576 main stage、exported-symbol extraction、wrapper=false、main `ty=512`、manifest/receipt v2 与 `CLEAR`；真实 run `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 完成 11 case，capture tooling 在 `fe56b2b9…` 为 `head_index_clean_filter_equal` strict identity；`40853287…` 的两份 direct comparator 已重新消费并核验该 candidate/placement/provenance 合同 | 四个 manifest human check（状态/层/crop/审美）与独立来源身份现场复核均为空，accepted oracle 未取得；ignored candidate 与自动 comparator 不能升级为 `oracle_frozen` | 提交人工来源/crop/状态/层/审美复核，保留旧 wrapper candidate 仅作错位诊断 |
...
| B0-04 | `canonical_asset_candidate_validated; awaiting_human_review` | `40853287…` 已冻结 8 SVG / **99,576 B**，revision `d67ab427…92ba1`、manifest SHA `6F0AAA45…FEF826`；strict/full、Web A/B、placement-correct C#/direct 自动链已通过。旧 **99,564 B** / revision/hash 与 standalone Flash/Web closure 只作 v1 history | Bevel/singular-gradient/MP variants/HP gradient rotation、透明 crop 与整体审美仍待人工；不声称 parity | 保留当前 exact closure；随 B0-01B 一并人签 |
...
| B0-06 | `main_space_v2_automated_closeout_passed; human_pending` | clean merge `40853287…` 的 formal run `ed61d7275fdd4a96a73f762c5c2a4d71` 为 48/48、**653,904 B**、SHA `182C7DC9…CA8E1`；clean full 为 1747 pass + 3 opt-in skip / 1750。两份 C# 36-file、Web 12-file、direct 66-PNG 与 viewer 自动包均确定；MP 四字段 unique `bestDx=0` 且 anchor edge `dx=0`。§9.7 当前 K1 已 PASS。首列 v2 q3 本地 producer/票有效但 policy 22/23，唯一 qualification compile-closure 缺口随后以 F2/r2 fresh 关闭：tag `…-b0-v2-r2`、request `6B5E9BD…1460D`、local/cloud 23/23、identity `DAE8A71C…8ED1`、33-file closure `051FA840…D2C`、双 faultDomain 与 `preflight-passed` 全部一致 | Flash oracle/真实桌面、可见 DWM/game composite、鼠标透传、关键比例/平滑、混合 DPI、SWF/IDE 身份和全部人工项仍待完成；当前 Computer Use/Browser backend 不可用。自动图像/Kimi/VerifyOnly 只作 diagnostic/audit，不批准 parity、执行、E2E 或部署 | 直接进入真实游戏/IDE 身份与人类视觉/UI签收；旧 tag/q3/CAS/票/失败 receipt 仅保留审计且不复用 |
| B0-07 | `main_space_v2_k1_passed; human_pending` | §9.7 同一 Kimi session `session_cbe7e035-bcdd-475a-9147-5f021ff5700d` 在 `k3/high` 下完成；165/165 request、5/5 config update 全部命中，P0/P1=0，裁决 `APPROVE_AUTOMATED_CLOSEOUT`；23 个唯一 PNG 已看，MP 不建议整组平移或改生产 blend mode | 运行时 SWF、new main-RSL oracle、游戏 composite、关键比例/平滑、混合 DPI、真实手点、HP halo/色调、`%` 字形、审美与人类明确接受 | 自动 build Gate 已闭合，进入人类终审；Computer Use/FFDec/Edge/Kimi 均不能代签 |

表中 B0-03b 行的“两正式 builder 未验证”仅描述其早期 renderer-qualified candidate `c-c571…` 自身，不能外推到后续 F；v1 source-freeze 的正式 builder/quorum 曾由旧 B0-06 行与 preflight 报告闭合。反过来，F/F2 的 proof 也不重写早期 candidate 的 payload/Core 身份。F2/r2 proof 已关闭当前 main-space v2 builder/quorum Exit，但不关闭 Flash oracle、真实桌面、人类视觉/UI或部署状态。
...
> v1 工程轨曾冻结 F=`cb38600…f511` 与不可变 tag，并由 clean F Lane、两个正式 builder、production receipt、`-VerifyOnly` preflight 和 clean-G K1 证明旧字节；这些事实保持历史有效，但不得外推到 main-space v2，也不得移动/删除 v1 tag。main-space v2 runtime/visual implementation F=`40853287…`、G=`884fbf43…`、B0-05 32/32、B0-06 48/48、full 1747+3/1750、自动视觉包与 Kimi `k3/high` PASS 均已形成。首列 tag `…-b0-v2` 与 q3 本地票有效但 policy 22/23，因 qualification compile closure 漏项退役；F2=`891d9b08…57ecd` 的 r2 tag/q4/w4 已 fresh 取得 local/cloud 23/23、双 signer/双 faultDomain quorum 与不可复用 `preflight-passed`，严格仍为 `candidate_built / NOT_DEPLOYED`，禁止复用 q3。真实 11-case Flash candidate 仍只有 strict tooling identity，human review 四项为空，所有自动图像都只是 diagnostic；本轮 Computer Use native pipe 与 Browser backend 实测不可用。恢复时直接从现场/人类轨继续：核验旧 Flash 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实桌面手点、运行时 SWF/IDE Output 身份与审美人签。当前总体状态为 `awaiting_human_acceptance; human_exit_blocked`；B0 不要求 promotion、标准入口或部署，在人签前不得写成 parity、`oracle_frozen` 或 `b0_accepted`。每完成一片都在同一提交回写 §12。
