...
$productionBeginContracts = @(
    ...
    @{ Name = 'pet'; Path = $petPath; ExpectedBeginCount = 7; RequiredTokens = @(
        ...
        'catch (deleteSceneRefreshError)', 'catch (deleteHeroLookupError)',
        'catch (deleteSceneReloadError)',
        'catch (worldNpcCleanupError)', 'catch (worldSceneRefreshError)',
        'catch (worldSceneReloadError)',
        'catch (rebuildError)',
        'deleteSceneCleared = _root.删除场景宠物() === true;',
        'worldSceneCleared = _root.删除场景宠物() === true;',
