...
export interface QuickBattleStats {
  ...
  averageAttackerRemainingHp: number;
  averageDefenderRemainingHp: number;
}

function findMatchup(state: GameState, selectedNodeId: NodeId): {
  attackerNode: NodeId;
  defenderNode: NodeId;
...
export function runQuickBattle(state: GameState, selectedNodeId: NodeId, runs = 100): QuickBattleStats {
  ...
  const defenderUnits = piecesAtNode(state, matchup.defenderNode, matchup.defenderFaction)
    .map((piece) => snapshot(state, piece.pieceId));
  if (attackerUnits.length === 0 || defenderUnits.length === 0) throw new Error('快测编队为空。');
