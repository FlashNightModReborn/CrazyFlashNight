...
//   - 关卡 Enemy 里出现的 兵种 全量保留；is_hostile 只作为标签，不再决定是否进入元战队。
//   - 人形模板仍标记为 humanoid，并只从自动势力采样池剔除；teams[]/定制预设保留真实关卡组合。
...
if (!CHECK_ONLY) {
    fs.writeFileSync(OUT_PATH, catalogText, 'utf8');
    ...
    const expected = [[OUT_PATH, catalogText], [WEB_MODULE_PATH, webModule]];
    let stale = false;
    for (const pair of expected) {
        if (!fs.existsSync(pair[0])) {
            console.error('[--check] 缺少生成物: ' + path.relative(ROOT, pair[0]));
            stale = true;
            continue;
        }
        const actual = fs.readFileSync(pair[0], 'utf8').replace(/\r\n/g, '\n');
        if (actual !== pair[1]) {
            console.error('[--check] 生成物已过期: ' + path.relative(ROOT, pair[0]));
            stale = true;
        }
    }
    if (stale) process.exit(1);
    console.log('\n[--check] 两个生成物均与关卡/兵种/佣兵真源一致');
}
