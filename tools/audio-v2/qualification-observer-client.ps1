...
param(
    ...
    [ValidatePattern('^cf7-audio-v2-qualification-[1-9][0-9]{0,9}-[0-9a-f]{32}$')]
    [string]$PipeName,
...
public static class Cf7AudioV2NamedPipeNative
{
...
try {
    if ($RequestFromStdin) {
        ...
        do {
            ...
            if ($read -gt 0) {
                ...
                if ($memory.Length -gt 65536) {
                    throw 'qualification observer request is outside the byte bound'
                }
            }
        } while ($read -gt 0)
        ...
        if ([Convert]::ToBase64String($requestBytes) -cne $RequestBase64) {
            throw 'qualification observer request base64 is not canonical'
        }
    }
    ...
    if ($request.IndexOf("`r") -ge 0 -or $request.IndexOf("`n") -ge 0) {
        throw 'qualification observer request must be one NDJSON record'
    }
    ...
    if (-not [Cf7AudioV2NamedPipeNative]::GetNamedPipeServerProcessId($handle, [ref]$serverPid)) {
    ...
    if ([int64]$serverPid -ne [int64]$ExpectedServerPid) {
        throw "qualification observer pipe server PID mismatch: expected $ExpectedServerPid, actual $serverPid"
    }
    ...
    if (-not $readTask.Wait($TimeoutMilliseconds)) {
        throw 'qualification observer response timed out'
    }
    $response = $readTask.Result
    if ($null -eq $response) {
        throw 'qualification observer pipe closed without a response'
    }
    if ($response.IndexOf("`r") -ge 0 -or $response.IndexOf("`n") -ge 0) {
        throw 'qualification observer response must be one NDJSON record'
    }
    $responseBytes = $utf8.GetBytes($response)
    if ($responseBytes.Length -lt 2 -or $responseBytes.Length -gt (16 * 1024 * 1024)) {
        throw 'qualification observer response is outside the byte bound'
    }
    ...
    [Console]::Error.WriteLine(
        'audio-v2 qualification observer client failed: ' +
