...
**当前阶段**：Stage 2 Step 1 测试入口真实进关闭环。
...
- Step 1：`stageSelectSnapshot` / `stageSelectEnter` 只服务刘海屏“选关测试”，按真实解锁校验后进关
- Step 2：替换原 Flash 选关界面入口，补齐打开时机、关闭回退、当前 frame label 同步；施工包见 `docs/选关界面-AS2入口替换交接.md`
  - AS2 新增 `openWebStageSelect`，通过 `panel_request` 请求 `stage-select`
  - C# `TaskRegistry` / `LauncherCommandRouter` 支持 `panel_request stage-select` 与 `frameLabel` 初始化
  - Web runtime 模式隐藏 fixture/dev 控件，`return` 类 nav 关闭 panel
  - 先替换 `基地门口` 单入口，再扩展车库、地下 2 层、停机坪、联合大学
  - 保留旧 Flash `关卡地图MC` 与 `切换场景("", "关卡地图", ...)` fallback
- 后续：`stage_jump_frame` 接管页内跳转；外交地图按钮继续触发原 Flash 委托任务界面
