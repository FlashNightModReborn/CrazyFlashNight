...
**最后核对代码基线**：B0-06 实现及 `runtime-qualification.json` / `visual-evidence.json` 仍精确绑定 commit `bf8dd2c410267855c8ea12f25a594042b3158479` 和各自的 sourceTrace/input closure；formal run `a6aadeb6e3264577bb9da8fe90857a7b` 为 47/47，不能把后续 source-freeze 外推为重跑该报告。最终 source-freeze F 为 commit `cb38600aae51f5019d09f87c33bd9e67d2b1f511`、tree `c74cc66f445921f5fcda1da04a2de0f013fef8ef`，由 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v1` 冻结；clean detached F 的 Runtime Lane C 为 11/11 exit 0、scalar 566。request `839C74FD1DF61ACC1DA580041F6FA71CA13A84DF1F43A55237BF9BEEF8648FB2` 已取得本地 X509 与 GitHub Hosted OIDC/Sigstore 两正式 proof，在 `physical-host-b` / `github-hosted-windows` 两故障域对同一 build identity/payload closure 形成 quorum；`runtime-promotion-preflight.v2.json` 为 `preflight-passed`，且 `-VerifyOnly` 未改变 runtime 或 release state、未 promotion/deploy、不可复用为 promotion 输入，故严格状态仍为 `NOT_DEPLOYED`。B0-04 的资产/证据身份、B0-03b 的早期 candidate/production Gate、B0-05 的 raster/topology 与上述收尾事实统一以 [专项 ADR](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md) §12 和结构化报告为稳定锚。Kimi `k3 + max` 最终增量已在 clean G 上取得 `K1_AUTOMATED_GATE=PASS`、P0/P1=0；真实 `pi_*`、accepted Flash oracle、可见桌面 DWM/游戏 z-order 与 occlusion、真实手点、关键比例/平滑、混合 DPI、运行时 SWF 身份、审美与人签仍未闭合，故 B0 总体最多为 `awaiting_human_acceptance`。上游其他功能的正式验收不得外推为本 HUD 纵切。
...
- 2026-07-29 的视觉基座 B0 保持两条正交状态：B0-04 仍为 `canonical_asset_candidate_validated; awaiting_human_review`——第七轮真实 Flash Player 的 11-case candidate 尚无人签收来源/层/crop/审美，仓内 Web/FFDec/Flash 诊断的确定性也不表示跨 renderer 像素一致；工程轨的 B0-03b 已把 `Svg.Skia 5.1.1` / `SkiaSharp 3.119.4`、共享 strict facade、8 SVG + manifest embedded resource、notice、runtime identity 与 production contract 接入生产构建链，并以专项 ADR §12 的 fresh v2 candidate 达到 `renderer_qualified / NOT_DEPLOYED`。B0-05 又完成 physical raster/PArgb/16 MiB atomic-batch cache，并由 `synthetic_fixed_bounds` 预检接受 `split_required`。B0-06 据此实现 11-case fixture visual state/animation、MP 双 fragment、HP 固定轮廓/旋转渐变、source-bound path glyph atlas、独立 click-through layered surface 与 PanelHost lifecycle；HP 三处动态文字使用源 SWF 的红色 Glow，MP 不加 Glow，Skia `sigma=1` / alpha `220` 明确只是 single-pass 近似。该 surface 只由 allowlist 环境变量显式启用，不注册进现有 NativeHud union，也不隐藏或修改旧 Flash HUD。当前 formal run 完成 3000 visible + 3000 idle，并在第 3 个 excluded warmup group 收敛后通过独立 100-cycle acceptance，最终 47/47；C# 36-file A/B、Web 12-file A/B 与 comparison 66-PNG A/B 也各自确定。Web capture 明确只含 canonical static SVG，不含 C# 程序化动态文字/Glow；C#↔Web direct metrics 则有意包含这一层范围差。source-freeze F `cb38600aae51f5019d09f87c33bd9e67d2b1f511` 已由不可变 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v1` 冻结；本地 X509 与 GitHub Hosted OIDC/Sigstore 两故障域对同一 build identity/payload closure 形成 quorum，`runtime-promotion-preflight.v2.json` 为 `preflight-passed`。该报告仅为 `-VerifyOnly`：未改变 runtime 或 release state，未 promotion/deploy，也不可作为 promotion 输入，故仍为 `NOT_DEPLOYED`。精确身份见专项 ADR 的 `runtime-qualification.json`、`visual-evidence.json` 与 preflight 报告；Kimi 最终增量已取得 `K1_AUTOMATED_GATE=PASS`、P0/P1=0。formal HWND/ULW runner 的 owner 在屏外，真实可见 DWM composition、游戏 z-order/occlusion、真实 `pi_*`、accepted Flash oracle、游戏 composite/手点/关键比例/平滑/混合 DPI/运行时 SWF 身份/审美仍未完成，因此总体为 `awaiting_human_acceptance`。较早同日会话成功使用 Computer Use 的截图保留为历史 evidence；当前 workspace/terminal unrestricted，但 Apps 后端独立，Computer Use native pipe 初始化报 `os error 2`，Browser 初始化后 `list=[]`。只读 `Get-Process` 仅见 `Flash.exe` 标题 `TestLoader.xfl`，本次未见独立 player/Test Movie；它不证明活动文档或运行时 SWF 身份。仓内 FFDec 与 headless Playwright/Edge harness 可辅助诊断，但不能代签真实桌面检查。promotion/标准入口/部署不属于 B0 Exit；B0 的 fixture-only 复刻不推翻 §4 的 state-first 运行态接入顺序。
...
下表以阶段0 的 UI MovieClip 隐藏职责为基线，并在“处置”列回写 2026-07-15 现状。标为“已解除”的职责已迁到 AS2 服务；未解除项仍禁止随 HUD 整体搬走。

| # | 隐藏逻辑 | 证据位置 | 后果（若随 HUD 移除而不补偿） | 处置 |
|---|---|---|---|---|
| 1 | **12 槽快捷技能输入、槽描述符与逐槽冷却已脱 HUD**：`QuickSkillInputService` 按 1→12 读取 live 键位，从 `SkillLoadoutService.getSlotDescriptor()` 取装备态，并以 `ManualCooldownService.quickSkillKey(1..12)` 判定/启动冷却；旧 `进度条N` 只在存在时绑定为 renderer，缺失不阻塞释放 | QuickSkillInputService.as；SkillLoadoutService.as；ManualCooldownService.as；通信_fs_帧计时器.as | HUD 不再是快捷技能输入、装备或冷却权威 | **已解除该停止线**；S6 前保留旧 renderer/fallback 作观察对照 |
| 1a | **战技输入与共享冷却已脱 HUD**：`WeaponSkillInputService` 持有锁存与 release 编排，`ManualCooldownService.WEAPON_SKILL_KEY` 持有共享冷却；旧战技进度条只接受投影。普通战技与副武器 F/R 仲裁仍经 `UnitActionIntentService` 保持既有语义 | UnitActionIntentService.as；WeaponSkillInputService.as；ManualCooldownService.as；通信_fs_帧计时器.as | 删除旧进度条不再让普通战技 fail-closed | **已解除该停止线**；保留 renderer 回归直到 HUD 退场门 |
| 1b | **4 槽药剂输入与库存扣减已脱 HUD**：`DrugInputService` 读取 live 键位与权威药剂栏，调用 `_root.使用药剂`，经 `ManualCooldownService.drugKey(0..3)` 启动冷却，再 `addValue(-1)` 并在耗尽时清快捷槽镜像 | DrugInputService.as；ManualCooldownService.as；通信_fs_帧计时器.as | 药剂控制器不再是输入或扣库存权威 | **已解除输入/扣减停止线**；药剂拖放 hitTest 仍见 #6 |
| 2 | **手动玩家冷却权威已迁 AS2 服务**：`ManualCooldownService` 独占 `weapon:shared + quick:1..12 + drug:0..3` 共 17 路状态并沿 CooldownWheel 调度；旧 `Symbol 1791` 的字段和 `冷却开始()` 只做代理/投影 | ManualCooldownService.as；ManualCooldownServiceTest.as；Symbol 1791.xml | 旧进度条缺失或重绑不再导致无冷却连发或无法施放 | **已解除**；50/50 fresh trace 覆盖生命周期、renderer 重绑与暂停推进 |
...
| 4 | **技能装备/卸下/排序/被动写已服务化**：`SkillLoadoutService` 原子维护 `主角技能表`、`快捷技能栏1..12`、被动缓存、动态 CD/MP 投影和 `dirtyMark`；旧叉按钮只调用 `legacySkillUnequip()` 委托 | SkillLoadoutService.as；SkillPanelService.as；快捷技能界面.xml | HUD 不再持有技能写权威；旧按钮仍可作 fallback | **已解除 Skill 侧停止线**；S6 观察门前不删除旧入口 |
...
- **留 AS2（领域服务层）**：`WeaponSkillInputService` / `QuickSkillInputService` / `DrugInputService`、`ManualCooldownService`、`SkillLoadoutService` / `SkillPanelService` 持有输入、冷却和技能管理权威；这些职责不迁入 C#。旧 HUD 仅保留可选 renderer，以及尚未替换的药剂拖放 hitTest 等显示列表能力。
- **最终形态 = 双轨后收敛**：当前是 C# 只读显示目标 + AS2 服务权威 + XFL 兼容投影/命中壳。阶段8 可先隐藏可见图层，但只有药剂拖放等剩余 MC 能力替代、Skill S6 观察门通过后，才能停止实例化对应旧壳；AS2 服务本身继续存活。
...
- **C1 — `PlayerInfoState` 显式分 `cur`/`target`**：把 onEnterFrame 缓动（§2.1 精确 `step` 式，不用“30/1/20%”口号替代）纳入 renderer 契约。只发终值的话 C# 无从复刻平滑序列，阶段6 对比恒红。经验条特例：不缓动、按 `remain` 直跳；当前 `.frame=100` 影子字段写入不进入契约，除非 B2 真机证明它有独立可见效果。
...
- **C3 — 阶段1 不只是"时间轴脚本外置"，必须同时把 §2.2 的输入/冷却/技能写从 MC 时间轴剥离成 AS2 类**。2026-07-15 已完成：战技、12 槽快捷技能与 4 槽药剂分别进入 `WeaponSkillInputService` / `QuickSkillInputService` / `DrugInputService`，17 路冷却进入 `ManualCooldownService`，技能装备/卸下/排序/被动写进入 `SkillLoadoutService`。旧进度条仅为可选 renderer，旧 Skill 入口只委托服务；阶段7/8 的剩余停止线已缩小到药剂拖放等显式 MC 能力和观察门。
...
阶段0 事实：装/卸的**落点/控件物理上就是 HUD 槽 MC**（药剂=`快捷药剂界面.hitTest`+`药剂图标列表[i].area.hitTest`；技能=`快捷技能界面.hitTest`+`快捷技能栏i.hitTest`；技能卸载为 12 个叉 button）。当前 Skill 入口已全部委托 `SkillLoadoutService`，Web 面板也不需要 HUD hitTest；旧技能 MC 只作 fallback/renderer。药剂拖放落点仍保留该物理耦合，因此不能从 Skill 已解耦倒推出整个玩家信息 symbol 可删除。
...
| 7-8 facade / 隐藏可见层 | Skill 侧不再要求 HUD hitTest/叉；药剂侧仍要求 `_visible=0` 的真 MC 与 C# 可见槽坐标对齐，未替代前不可 facade→Object / 停 placement |
| 9 彻底删壳 / 移除 §2.2#6 停止线 | Skill S1/S3 已完成，但旧技能 UI 退役仍受 S6 观察门约束；药剂槽仍依赖物品/药剂管理切片。两者门均闭合后 HUD 槽才可只剩新 renderer |
...
**接口闭环状态**：① 技能已使用 `skillKey + slot + expectedRevision` 的独立 `skills` 协议，且不注册为 inventory-domain 容器；被动重算、动态 CD/MP、EasyMode 重复装备语义与坏档重复行 fail-closed 均已落地。② 药剂仍应以独立容器写处理装备，必须保留冷却门控、耗尽自动清栏与 4 槽权威。③ Skill S6 清理前仍须完成观察样本和旧 UI 退役审计；这不是重新开放 Skill/装备域耦合的理由。
...
| 1 | AS2 外部脚本化（仅 bootstrap） | **核心停止线已完成**：战技/快捷技能/药剂输入、17 路冷却与 Skill 装备写均已进入 AS2 服务；旧进度条只作 renderer。剩余为药剂拖放等独立 MC 能力与退役观察门 |
...
| 5 | C# PlayerInfoWidget 只读 | B0-05 已完成 raster/cache 并因近全屏 union 接受独立 split surface；B0-06 已实现不接业务权威的 HP/MP fixture 纵切与独立 click-through surface，surface 独占 animation model，widget 只读消费接口，Resume 仅在有效布局与 raster request 建立后完成。当前实现基线 formal qualification 47/47、focused 136+3/139、全量 1,737+3/1,740 与视觉确定性已通过；F source-freeze/tag、双 builder 非部署 quorum 与 Kimi K1 PASS 也已闭合，但 Web 仅覆盖 static SVG，真实可见 DWM/游戏交互、accepted Flash oracle、关键比例/平滑/混合 DPI/运行时 SWF 身份与人审尚未退出。B0 accepted 后仍按阶段2→3的 state-first 链路接真实 UiData，程序化复刻虚拟帧缓动 |
| 6 | 双轨对比 | 不变；按 §2.4 验收表 |
| 7 | facade 化 | **前提**：§2.2 逻辑已剥离（C3）；facade 仅承显示层 + 保留 §6 列出的 MC 能力 |
| 8 | 隐藏 AS2 可见 UI | **改写**：隐藏可见图层、**保活隐形逻辑壳**（不能停止实例化整个 symbol） |
| 9 | 清理旧依赖 | 不变；长期收敛 161 处扫描命中（其中约 157 处为可执行调用） |
...
- **当日边界**：该切片当时未迁移 `Symbol 1791` 冷却状态机、技能装备/卸载、图标 attachMovie/拖放 hitTest、药剂输入与库存扣减；其中冷却、Skill 写、药剂输入/扣减已于 2026-07-15 后续切片关闭，仅药剂拖放等 MC 能力仍开放。
...
| ~~R2~~ | ~~手动冷却权威仍在 UI~~ → **已解除**：`ManualCooldownService` 持有 17 路逻辑冷却，三条输入服务只读写该 authority，旧进度条只作 renderer | — | 50/50 fresh trace + LongGun/Drug 回归；继续禁止底层释放函数绕过服务自行判冷却 |
...
| R11 | **装备/卸载交互末端跨主线依赖只剩药剂侧**（§4.2）：Skill S1/S3 已接管技能写，旧叉仅委托；药剂拖放仍借 HUD hitTest，阶段8 提前 facade→Object/停 placement 会使其失效 | 中 | Skill 旧 UI 等 S6 观察门后退役；药剂删壳等物品/药剂管理切片，未完成前药剂命中壳仍用 `_visible=0` 真 MC + 坐标对齐 |
...
| 资源管线 | ★★★★☆ fixture 实现/自动资格完成、视觉未验收 | 8 个 HP/MP SVG（99,564 B）+ runtime manifest 已作为 9 项 embedded resource；`Svg.Skia 5.1.1` / `SkiaSharp 3.119.4`、共享 strict facade、notice、runtime identity 与 production contract 已接线；renderer-family 实际 DLL/native 采用递归路径 exact-closure 门。B0-03b 已达 `renderer_qualified / NOT_DEPLOYED`；B0-05 已闭合 content-height/576 physical scale、8-layer PArgb、16 MiB atomic-batch cache并裁决 `split_required`；B0-06 已实现 fixture widget/compositor/path glyph atlas 与独立 layered surface，并把 effect policy 收束为 B0 active `hp-mp-dynamic-text-and-glow`、B3 deferred `hp-horizontal-line-glow` / `hp-light-overlay` 和七个冻结文字布局；其中 HP 三处 Glow 取源红色，MP 无 Glow，`sigma=1` / alpha `220` 是 Skia 近似而非像素 parity 声明。当前 formal qualification 已 47/47，C#/Web A/B 与 comparison PNG 也有确定性闭包；Web static-only 不含程序化文字/Glow，direct C#↔Web 指标显式包含层范围差。跨 renderer 指标仍无接受阈值，accepted Flash oracle、可见 DWM/游戏 composite/手点/审美仍待人工 |
| AS2 玩家信息界面 | ★★★☆☆ | **权威逻辑已大幅脱壳**：战技/快捷技能/药剂输入、17 路冷却及 Skill 装备写均已服务化；旧 XFL 主要保留 renderer、药剂拖放命中和 161 处历史扫描命中（其中约 157 处为可执行调用） |

**难度分层**：
- **只读 C# 显示镜像（阶段3-6 核心）**：中等偏低、低风险（基础设施成熟）。
- **完整迁移（含 facade + 退场）**：**中等**；C3 与 R2 已关闭，剩余风险集中在 R1（弹药散点）、R4（显示列表能力）、R11（药剂命中）及真实观察门。
- **建议**：HP/MP fixture-only B0 的 formal qualification、双份 visual closure、F source-freeze/tag、双正式 builder 非部署 quorum 与 Kimi `k3 + max` 最终增量 K1 PASS 均已形成；下一步只由人类签收 Flash oracle、可见 DWM/游戏 composite、关键比例/平滑、混合 DPI、运行时 SWF 身份、真实手点与审美。它们全部通过也只证明视觉基座。B0 accepted 后再沿阶段2→3建立真实 state/UiData，并做 HP/MP 双轨对比，不能把 fixture 结论外推为运行态接入。
...
**文档治理**：本文为玩家信息 HUD 迁移的纲领 canonical doc；SVG/B0 专项文档只承载视觉渲染子域，避免形成第二份业务权威。B0-04 已同步 strict canonical validator 与 Web/FFDec/Flash diagnostic 入口；B0-03b 已同步生产包、embedded resource、runtime identity/policy/notice 与 `renderer_qualified / NOT_DEPLOYED` 边界；B0-05 已同步 raster/cache、`synthetic_fixed_bounds` 与 `split_required`。B0-06 已按实现基线 `bf8dd2c…8479` 的 fixture visual-state/widget/compositor/path glyph atlas、HP source-red Glow/MP no-Glow、surface 单一 model ownership、可靠 Resume pending、typed active/deferred effect policy、独立 split surface、PanelHost 生命周期、strict convergence qualification、最终机器报告与 A/B capture 索引，以及 F source-freeze/tag、双 builder `-VerifyOnly` quorum 与 `NOT_DEPLOYED` 边界，同步 `launcher/README.md`、testing guide 与本路线；Web static-only 与 direct metrics 包含 scope 差的边界也已显式回写。最终 K1 已在 clean G 上取得 PASS、P0/P1=0，并由同一 docs-only 后继同步自动 Gate；本轮仍未声称 B0-04 人工视觉、真实 `pi_*`、可见 DWM/游戏交互、parity、promotion 或部署。人类签收后再同步视觉接受结论并运行 doc governance。后续按实际改动回写本文 §5 路线与 §6 风险，行号锚点随重构刷新。
