...
本节只描述 H1 activation commit `c5ca5dfa9718e8a7714038e929a64081b7fd0026` 之后的 current source。历史五次 failed attempt、S6 两次 operationally invalid/aborted、一次 default-device switch crash-invalidated 与 S9 evidence-contract diagnostic invalidated 合计九个 runId/capture，均不可复用。S7 完整 Source matrix 只保留为实现前基线；R3 修正的完整 Source matrix 已闭合：observer `25/25`、runner `29/29`、assembler `5/5`、C# Diagnostics/Stimulus 各 `16/16`、Launcher `3547 passed + 3 opt-in skipped / 3550`，并通过 native/runtime、AS2 static、Jukebox 与发布控制面，qualification dependency closure 已稳定。本节所在版本与最终 source/generated/doc closure 首次同树落盘的 clean implementation commit 定义为新 Source S；其 commit/tree 只能从提交后的 Git object 解析，随后仍须双 clean repro、新 candidate、全新 runId 与完整 A6。当前严格 **H2 blocked / NOT_DEPLOYED**，未进入 runtime promotion；不能把源码、单测、诊断 capture 或旧 candidate 外推为正式入口已升级。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
- 该 pipe **不是音频控制面**：它只写 marker、在 coordinator owner 队列读取 snapshot、读取 journal，绝不直接驱动 native。BGM/SFX 动作仍经生产 AS2 → `AudioTask` → facade/coordinator 链；journal 将 AS2 ingress、coordinator completion/recovery 与 qualification snapshot 分源记录。snapshot 只暴露 backend、设备 digest/name、session/ready/device generation、BGM source、BGM/SFX raw meter 和全部 SFX counters，不暴露 raw endpoint ID；engine session sample format（当前 `f32`）也不等同于 endpoint capture 文件格式（例如 `pcm_s16le`）。`sleep_resume` 必须至少有 1 个 recovery episode；同 episode 的多个 `Recovering` 样本合并，每个顺序 episode 都用 `workingStateElapsed100ns` 差值要求 `<=150000000`，最终 post 必须晚于最后 `Ready`（或就是同一事件）且 tuple 相等，零 episode 或任一中间段漂移都 fail-closed。有限 SFX 两 case 以当前 generation dispatch 为 anchor：最后 pre-dispatch SFX `peakAbs <64`、至少一份 post-anchor snapshot 同时 frame 推进且 `peakAbs >=64`、final 总 frame 推进；terminal silence 合法。stale case 在设备变化前写 hash-bound `recovery_sfx_armed{result:armed,sent:false}`，严格锁定 `arm < first Recovering < old-generation batch < first Ready after that Recovering < post`；只允许 `staleGenerationDrops` 按 batch 增加，其他 counter 与 played 不变。内部 meter 非零仍不能代签物理端点可听。
...
- **失败 attempt 台账**：历史五次 failed attempt 之后，S6 又产生 run `fe73…3766`（TWS 电量耗尽）与 `8320…d1c6`（自动 lane 早于存档选择/Flash prewarm exit）两次 operationally invalid/aborted；run `82a4…9870` 的前十 case 与三份 TWS 非静音诊断 capture 已完成，但 Realtek→TWS 切换令 Core 在 `miniaudio.dll` 以 `0xc0000005` 崩溃，整个 run/capture 作废；S9 `b2370…a108` 又因旧有限 SFX/UTC/stale-counter evidence contract fail-close。R3 三项修正及完整 Source matrix 已完成，但合计九个 runId 均不可复用、无 E1/H2、始终 `NOT_DEPLOYED`；完整 WER/dump、capture 与旧 attempt 细节只在 [Audio v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md) 维护。本文当前版本形成新 Source S 后，仍须双 clean repro、新 candidate 与全新 runId。

**A5 source inventory**：生成器的 shipped-audio 清单共 827 项，精确分为 795 个 `tracked` + 32 个 `ignored_source`；manifest 为 `843455` B / SHA-256 `60CA5E3E6B42842965FBD5A2A8313827D0C2E7AB0CAE4E16D9E8E3B0986B6A9C`。qualification dependency 为 `568` entries / closure `5368D99011EAECE6DD64C220D90ACD5675C5C35A2C9C10E924751793481B1C22` / manifest `142054` B / SHA-256 `4F0EB431524AEC2332C426562F56167307E927EB2E281E8EFFC735206F89FDA2`。content sniff 确认的 11 个 AAC 资产已从错误的 `.wav` 名称重命名为 `.m4a`；这些清单是 source truth，不把“被扫描到”偷换成“已可播放”。详细投放说明见 [`../sounds/README.md`](../sounds/README.md)。
