 ...
 *   1. 在「落成那一刻的宿主坐标」上钉一个范围：以**视觉中心**（= 宿主 `_y` 向上
 *      `宿主Y偏移`，也就是被锁定目标的脚下）为圆心的椭圆 —— X 半径 300、Z 半径 80
 *      （详见 宿主Y偏移 / 范围半径Z 的注释）。
...
class org.flashNight.arki.skill.SealDomain {
    ...
    /**
     * 宿主原点相对「封印视觉中心」的下移量（像素）。
     *
     * 为了配合 Z 轴排序，光柱元件的原点被画在形状**下端**，而封印的视觉中心
     * （= 被锁定目标的脚下 / 法阵圆心）在原点上方这么远的地方。判定要用视觉中心，
     * 所以 `中心Z = 宿主._y - 宿主Y偏移`。
     *
    ...
    /**
     * Z 轴判定半径（像素）：**以视觉中心为圆心、上下对称**的椭圆短半轴。
     *
     * 圆心 = 宿主 `_y` 向上 `宿主Y偏移` 处（见该常量）。换算到宿主坐标上是
     * `单位.Z轴坐标 ∈ [宿主._y - 2×范围半径Z, 宿主._y]` —— 比宿主原点更靠下
     * （Z 更大）的单位一律收不到，与「法阵压在脚下、往上罩一层」的视觉一致。
     *
     ...
     * 「三蝶手稿」插件带来的封印强度倍率。
     *
     * 判定口径：**手部战技名 == 天启大封印** ⟺ 手套上装了三蝶手稿。
     * （三蝶手稿的 `<use>` 是 `刀,手部装备,长枪`，装到手套上时
     *  `EquipmentCalculator.calculateInPlace` 会把 `modifiers.skill` 覆盖进
     *  `itemData.skill`，`DressupInitializer.loadWeaponSkill` 再拿
     *  `target["手部装备数据"].skill` 去 `装载主动战技(…, "空手")`。）
     *
     * 这样判还顺带解决歧义：手稿装在刀 / 长枪上、手套另配时，手部战技不是
     * 天启大封印 → 不加成，符合"只有戴着手稿的那只手套才强化封印"的直觉。
     *
     *   手部装备 = 真猫妖手套 → 5.0
     *   手部装备 = 猫妖手套   → 2.0
     *   其余 / 取不到     → 1.0
     *
     * ⚠ 倍率挂在 状态 上（落成时快照一次），**不要**去改静态的 每秒强度 ——
     *   那是全局量，动了会让所有封印一起变。
     */
    private static function 取强度倍率(施术者:MovieClip):Number {
        if (施术者 == undefined) {
            return 1;
        }
        var 手部数据:Object = 施术者["手部装备数据"];
        if (手部数据 == undefined || 手部数据 == null || 手部数据.skill == undefined) {
            return 1;
        }
        if (String(手部数据.skill.skillname) != "天启大封印") {
            return 1;
        }
        var 装备名:String = String(手部数据.name == undefined ? "" : 手部数据.name);
        if (装备名 == "真猫妖手套") {
            return 5;
        }
        if (装备名 == "猫妖手套") {
    ...
    public static function 封(宿主:MovieClip):Object {
        ...
        var 状态:Object = {
            ...
            施术者: 宿主.施术者,
            中心X: 取数(宿主._x, 0),
            // 视觉中心：宿主原点画在形状下端，判定圆心要往上抬 宿主Y偏移
            中心Z: 取数(宿主._y, 0) - SealDomain.宿主Y偏移,
            已存续帧: 0,
            记录表: [],
            任务ID: null,
            强度倍率: 1,
        ...
        // 落成时快照一次强度倍率（三蝶手稿 + 猫妖手套，见 取强度倍率）：
        // 挂在本状态上而不是改全局 每秒强度，多个封印可各算各的。
        状态.强度倍率 = SealDomain.取强度倍率(状态.施术者);
    ...
    private static function 解除单位(记录:Object):Void {
        ...
        // 6) 补执行封印期间被拦下的最后一次状态请求（见 收押 步骤 1.5）：
        //    短路版只记录不执行，状态机停在收押那一刻；这里用还原后的真函数
        //    gotoAndStop 到最后请求的状态，让状态机与封印期间实际发生的事对齐
        //    （man 若中途被外部代码 play 过、播过代码结束帧，这一步还能把它
        //    拉回一个合法的状态帧，避免从坏位置继续播）。
        //    放在 恢复时间轴 之后：先 play 放起来，再 gotoAndStop 精确定位；
        //    若请求的状态与当前 状态 相同，真函数会自行 early-return，无副作用。
        if (单位.__封印最后状态 != undefined) {
            var 待恢复状态 = 单位.__封印最后状态;
            delete 单位.__封印最后状态;
            单位.状态改变(待恢复状态);
        }
    ...
    private static function 每帧(状态:Object):Void {
        ...
        for (var i:Number = 记录表.length - 1; i >= 0; i--) {
            ...
            记录.已存续帧++;
            // 强度倍率在 封() 里快照过（三蝶手稿 + 猫妖手套）；取不到时按 1 处理。
            记录.强度 += SealDomain.每秒强度 * 取数(状态.强度倍率, 1) / 率;
    ...
    private static function 补收(状态:Object):Void {
        ...
        for (var 名:String in 世界) {
            ...
            // 上下对称椭圆：圆心是**视觉中心**（宿主 _y 往上 宿主Y偏移），不是宿主原点。
            // dz > 0 = 单位比圆心更靠下。落到宿主坐标上，比宿主原点更靠下的单位
            // （Z > 宿主._y）落在椭圆之外，天然收不到。
    ...
    private static function 收押(状态:Object, 单位:MovieClip):Boolean {
        ...
        // 1.5) 状态改变 不用纯空函数，换成「记录 + 再停」版：
        //      封印期间它若被调用，说明有外部代码把时间轴 play 了回去（正常情况下
        //      man 是停着的，动画完毕 之类的末帧脚本不该有机会跑）。此时：
        //      ① 把停过的那些时间轴再 stop 一遍，防止 man 播过代码结束帧
        //        （动画完毕 走不通 → 继续往下播）导致动画错乱/消失；
        //      ② 把请求的状态记到单位身上（不改写 单位.状态，它保持收押那一刻的值），
        //        解除封印时由还原后的真函数补执行 —— 见 解除单位 步骤 6。
        if (记录.短路表["状态改变"] != undefined) {
            var 被封单位:MovieClip = 单位;
            var 封印记录:Object = 记录;
            被封单位.状态改变 = function(新状态名):Void {
                被封单位.__封印最后状态 = 新状态名;
                var 停过的:Array = 封印记录.记录表_停过的;
                if (停过的 != undefined) {
                    for (var k:Number = 0; k < 停过的.length; k++) {
                        var 节点 = 停过的[k];
