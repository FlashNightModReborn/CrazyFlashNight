...
Audio Platform v2 另有独立的 H1/H2 人类闸。H1 只授权 A1–A6 实现、最终 source 冻结与 unsigned isolated candidate；最终 source 必须重新执行两次 clean producer 并比较 identity、payload closure 与逐文件 rows，较早 A3 构建不能代签 A4/A5 后的 candidate。A6 的自动报告、真实 endpoint capture 与人工听感全部绑定同一 exact candidate 后，H2 才能授权正式 source tag/request、双 fault-domain 构建与 promotion。现役 `cf7-runtime-build-request.v2` 保持严格原 schema，不混入音频 H1/H2 字段；H2 receipt 与 request 的关联使用独立 evidence-only link，任何 candidate/source/证据漂移都重新阻断。
...
仅当仓库 owner 对本次 Audio v2 release 明确启用紧急 non-H2 路径时，`promote-runtime-bundle.ps1` 才接受必须成对出现的 `-AudioV2EmergencyOwnerAuthorizationPath <canonical-json>` 与 `-AcknowledgeAudioV2NonH2Compliant`。授权文件由 `validate-emergency-owner-authorization.js --emit` 生成，逐字冻结 owner 指令并绑定 request raw SHA/requestId、source commit/tree、四域 hash、build identity、最终 payload closure、`bypass=[E1,H2,E3]`、保留控制与到期时刻；promotion 在 full closure 已知后、VerifyOnly 报告首次写入前、正式事务首次写入前三次重算文件 hash 并完整验绑定。该窄旁路只跳过 Audio E1/H2/E3，不修改 request/consensus schema，不伪造 H2；immutable request、双 signer、双 faultDomain、production policy、strict v2 verifier、原子 promotion 与 rollback 仍全部保留。无这两个参数时，原三次 H2/E3 fail-close 语义不变；紧急路径的 worktree 必须精确 materialize `request.releaseTreeOid`，VerifyOnly 报告显式标记 `nonH2Compliant=true`。
