...
_root.装备生命周期函数.铁枪初始化 = function(ref, param) {
    ...
    // —— 初始帧
    data.currentFrame = data.unmaykr化
        ? CONFIG.FRAMES.UNMAYKR.start
        : CONFIG.FRAMES.BFG.start;

    // 4. 工具方法
    var Utils = {
        validateFrame: function(frame, cfg) {
        ...
        getFrameRegion: function(frame, cfg) {
            var f = cfg.FRAMES;
            if (frame >= f.BFG.start && frame <= f.BFG.end)         return "BFG";
            if (frame >= f.TRANSFORM.start && frame <= f.TRANSFORM.end) return "TRANSFORM";
            if (frame >= f.UNMAYKR.start && frame <= f.UNMAYKR.end) return "UNMAYKR";
            return "INVALID";
        },
    ...
    var AnimationStrategies = {
        handleTransformation: function(d) {
    ...
    if (data.isWeaponActive && !data.isTransforming) {
        if (_root.按键输入检测(自机, _root.武器变形键)) {
            _root.更新并执行时间间隔动作(
                ...
                data.config.TRANSFORM.interval,
                false,
                data
            );
        }
