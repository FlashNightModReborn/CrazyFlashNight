...
function validateMove(state: GameState, command: MoveOrAttackCommand): ValidationResult {
  ...
  if (state.activeFactionId !== command.factionId) return fail('active_faction_mismatch', '当前不是该阵营的行动时机。');
  ...
  if (command.originNodeId === command.targetNodeId) return fail('origin_equals_target', '起点与目标节点不能相同。');
  ...
  if (command.pieceIds.length === 0) return fail('selection_empty', '至少选择一枚棋子。');
  if (new Set(command.pieceIds).size !== command.pieceIds.length) return fail('selection_duplicate', '棋子列表包含重复项。');

  for (const pieceId of command.pieceIds) {
    const piece = state.pieces[pieceId];
    if (!piece || piece.hp <= 0) return fail('piece_unavailable', `棋子 ${pieceId} 不存在或已阵亡。`);
    if (piece.factionId !== command.factionId) return fail('piece_wrong_faction', `棋子 ${pieceId} 不属于当前阵营。`);
    if (piece.nodeId !== command.originNodeId) return fail('piece_wrong_origin', `棋子 ${pieceId} 不在起点节点。`);
  }
  ...
  if (foreign.length > 0 && allies.length > 0) return fail('mixed_garrison_state', '节点存在混合阵营驻军，状态非法。');
  if (foreign.length > 1) return fail('hostile_garrison_ambiguous', '目标据点存在多个战略阵营，无法进入二方交战。');
  ...
  const targetRelation = targetFactionId
    ...
    : null;
  if (targetRelation === 'allied') return fail('allied_destination_forbidden', '可以经过盟军据点，但不能在盟军驻地结束行动。');
  if (targetRelation === 'neutral') return fail('neutral_attack_forbidden', '中立阵营尚未进入敌对关系，不能发起攻击。');
  ...
  if (isBattle) {
    ...
    if (requestedMetrics.deploymentSize > node.attackWidth) {
      return fail(
        ...
        `进攻宽度为 ${node.attackWidth}，当前投入规模 ${requestedMetrics.deploymentSize}。`,
        { maximum: node.attackWidth, selected: requestedMetrics.deploymentSize },
      );
    }
    for (const pieceId of command.pieceIds) {
      const piece = state.pieces[pieceId];
    ...
    if (faction.actionPoints < requiredAp) {
      return fail(
        ...
        `公共 AP 不足，需要 ${requiredAp}。`,
        { required: requiredAp, available: faction.actionPoints },
      );
    }
    return {
      ok: true,
  ...
  if (actualPieceIds.length === 0) return fail(
    'garrison_capacity_full',
    `${node.displayName}已达到驻军容量。`,
    { nodeName: node.displayName, maximum: node.capacity },
  );
...
function validateReorganizationAuthority(
  state: GameState,
  command: { factionId: GameCommand['factionId']; nodeId: NodeId },
...
function validateMergeTaskGroup(
  state: GameState,
  command: MergeTaskGroupCommand,
): ValidationResult {
  ...
  const elements = command.commandElementIds.map((elementId) => (
    state.organization.commandElements[elementId]
  ));
  ...
  const template = DEMO_1_ORGANIZATION.taskGroupTemplates.find((entry) => (
    entry.id === command.taskGroupTemplateId
  ));
  if (!template) return fail('task_group_template_mismatch', '当前组合没有可用的编制模板。');
...
function validateSplitTaskGroup(
  state: GameState,
  command: SplitTaskGroupCommand,
): ValidationResult {
  const authority = validateReorganizationAuthority(state, command);
  if (authority) return authority;
...
function validateSetFormation(
  state: GameState,
  command: SetFormationCommand,
): ValidationResult {
  const authority = validateReorganizationAuthority(state, command);
  if (authority) return authority;
  const element = state.organization.commandElements[command.commandElementId];
  ...
  const profile = DEMO_1_ORGANIZATION.formationProfiles.find((entry) => (
    entry.id === command.formationProfileId
  ));
  if (!profile) return fail('formation_unknown', '所选阵型不存在。');
  if (element.taskGroupTemplateId) {
    const template = DEMO_1_ORGANIZATION.taskGroupTemplates.find((entry) => (
      entry.id === element.taskGroupTemplateId
    ));
    if (!template?.formationProfileRefs.some((profileId) => profileId === command.formationProfileId)) {
      return fail('formation_unknown', '所选阵型不适用于该任务编组。');
    }
...
export function validateCommand(state: GameState, command: GameCommand): ValidationResult {
  if (state.phase === 'GAME_OVER') return fail('game_over', '对局已经结束。');
  if (!state.factions[command.factionId]) return fail('faction_unknown', '命令引用了未知阵营。');
  if (isFactionDefeated(state, command.factionId)) return fail('faction_defeated', '该阵营已经退出战局。');
  switch (command.type) {
    case 'MOVE_OR_ATTACK':
    ...
    case 'END_ACTION':
      if (state.phase !== 'FIRST_FACTION_ACTION' && state.phase !== 'SECOND_FACTION_ACTION') return fail('action_phase_required', '当前不是行动阶段。');
      if (state.activeFactionId !== command.factionId) return fail('active_faction_mismatch', '当前不是该阵营的行动时机。');
      return { ok: true };
    case 'ALLOCATE_XP': {
      if (state.phase !== 'SETTLEMENT_PLANNING') return fail('planning_phase_required', '当前不是结算规划阶段。');
      const faction = requireFaction(state, command.factionId);
      if (faction.planningCommitted) return fail('planning_already_committed', '该阵营已经提交规划。');
      if (!Number.isInteger(command.amount) || command.amount <= 0) return fail('xp_amount_invalid', '经验分配量必须是正整数。');
      if (command.amount > faction.xpPool) return fail('xp_insufficient', '待分配经验不足。', { required: command.amount, available: faction.xpPool });
      if (!faction.cards[command.cardId]) return fail('card_unknown', '未知卡牌。');
      return { ok: true };
    }
    case 'PURCHASE_PROMOTION': {
      ...
      if (cardState.promotedThisSettlement) return fail('promotion_already_purchased_this_round', '每张卡每个战略结算最多升阶一次。');
      const expected = definition.allowedPromotions[cardState.purchasedPromotions.length];
      if (!expected) return fail('promotion_complete', '该卡牌没有后续可购买升阶。');
      if (expected !== command.promotionId) return fail('promotion_sequence_required', `必须按序购买 ${expected}。`, { promotionName: expected });
      const promotion = PROMOTIONS[command.promotionId];
      if (cardState.level < promotion.level) return fail('card_level_insufficient', `需要卡牌达到 Lv.${promotion.level}。`, { requiredLevel: promotion.level });
      if (faction.gold < promotion.cost) return fail('military_funds_insufficient', `金币不足，需要 ${promotion.cost}G。`, { required: promotion.cost, available: faction.gold });
      return { ok: true };
    }
    case 'ENQUEUE_PRODUCTION': {
      ...
      if (!node || node.productionSlots <= 0) return fail('production_node_invalid', '目标节点不是生产节点。');
      ...
      if (!slot) return fail('production_slot_missing', '生产槽不存在。');
      const definition = getCardDefinition(command.cardId);
      if (isCommanderCard(command.cardId)) {
        return fail('commander_state_invalid', '唯一指挥官只能通过指挥所重建，不能作为普通兵种排产。');
      }
      const cardState = faction.cards[command.cardId];
      if (cardState.level < definition.deploymentLevel) return fail('card_level_insufficient', `卡牌需要 Lv.${definition.deploymentLevel} 才能生产。`, { requiredLevel: definition.deploymentLevel });
      if (faction.gold < definition.productionCost) return fail('military_funds_insufficient', `金币不足，需要 ${definition.productionCost}G。`, { required: definition.productionCost, available: faction.gold });
    ...
    case 'CANCEL_PRODUCTION': {
      ...
      if (!order) return fail('production_order_missing', '生产订单不存在或已离开该槽。');
      ...
      if (faction.populationReserved < order.populationCost) return fail('production_reservation_invalid', '生产预留人口状态异常，不能撤销。');
      return { ok: true };
    }
    case 'COMMIT_PLANNING': {
      if (state.phase !== 'SETTLEMENT_PLANNING') return fail('planning_phase_required', '当前不是结算规划阶段。');
      if (requireFaction(state, command.factionId).planningCommitted) return fail('planning_already_committed', '该阵营已经提交规划。');
      return { ok: true };
    }
    case 'ENQUEUE_COMMANDER_PRODUCTION': {
      ...
      if (commander.status !== 'available') return fail('commander_state_invalid', '该指挥官已经在场或正在生产。');
      const faction = requireFaction(state, command.factionId);
    ...
    case 'REDEPLOY_PLAYER_AVATAR': {
      ...
      if (commander.status !== 'rear') return fail('commander_state_invalid', '玩家指挥官当前不在后方待部署。');
      const faction = requireFaction(state, command.factionId);
...
export function firstProductionSlotId(state: GameState, factionId: string, nodeId: NodeId): string | null {
