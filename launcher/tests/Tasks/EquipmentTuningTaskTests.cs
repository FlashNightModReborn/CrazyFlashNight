...
namespace Launcher.Tests.Tasks
{
    public sealed class EquipmentTuningTaskTests
    {
        ...
        [InlineData(true)]
        [InlineData(false)]
        public void DetachBeforeFirstSnapshot_SucceedsLocallyAndIsIdempotent(
            bool clientReady)
        {
            var sent = new List<JObject>();
            var web = new List<JObject>();
                ...
                }))
            {
                ...
                task.HandleWebRequest(
                    ...
                    Request(
                        "detach",
                        "tune.detach.before-first-snapshot.1"));

                Assert.Empty(sent);
                JObject first = Assert.Single(web);
                Assert.True((bool)first["success"]);
                Assert.Equal("detach", (string)first["cmd"]);
                Assert.Equal(
                    "workbench.instance.1",
                    (string)first["panelInstanceId"]);
                Assert.Equal(
                    "tuning.session.1",
                    (string)first["viewSessionId"]);
                Assert.Equal(0, (int)first["writeEpoch"]);
                Assert.Null(task.ActiveViewSessionId);
                Assert.True(task.CanClose);
                web.Clear();
                ...
                task.HandleWebRequest(
                    ...
                    Request(
                        "detach",
                        "tune.detach.before-first-snapshot.2"));

                Assert.Empty(sent);
                JObject repeated = Assert.Single(web);
                Assert.True((bool)repeated["success"]);
                Assert.Equal("detach", (string)repeated["cmd"]);
                Assert.Null(task.ActiveViewSessionId);
                Assert.True(task.CanClose);
            }
        }
        ...
        [Theory]
        [InlineData("install_mod", 1, 3)]
        [InlineData("replace_mod", 1, 3)]
        [InlineData("detach_all_mods", 3, 1)]
        public void DynamicModSlotCapacity_PreviewCommitAndPostStateAreAccepted(
            string operation,
            int beforeCapacity,
            int afterCapacity)
        {
                ...
                web))
            {
                task.HandleWebRequest(
                    ...
                    Request(
                        "snapshot",
                        "tune.dynamic-capacity.snapshot." + operation));
                JObject snapshotResponse = OperationSnapshotResponse(
                    Assert.Single(sent),
                    operation,
                    false);
                snapshotResponse["snapshot"]["equipment"][
                    "modSlotCapacity"] = beforeCapacity;
                task.HandleFlashResponse(snapshotResponse, null);
                Assert.True((bool)Assert.Single(web)["success"]);
                sent.Clear();
                web.Clear();
                ...
                task.HandleWebRequest(
                    ...
                    Request(
                        "preview",
                        "tune.dynamic-capacity.preview." + operation,
                        operation));
                JObject previewResponse = OperationPreviewResponse(
                    Assert.Single(sent),
                    operation,
                    false);
                SetSourceModSlotCapacities(
                    previewResponse,
                    beforeCapacity,
                    afterCapacity);
                task.HandleFlashResponse(previewResponse, null);
                JObject preview = Assert.Single(web);
                Assert.True((bool)preview["success"]);
                Assert.Equal(
                    afterCapacity,
                    (int)preview["after"]["source"]["equipment"][
                        "modSlotCapacity"]);
                Assert.Equal(1, task.PreviewBindingCount);
                sent.Clear();
                web.Clear();
                ...
                task.HandleWebRequest(
                    ...
                    Request(
                        "commit",
                        "tune.dynamic-capacity.commit." + operation));
                JObject commitResponse = OperationCommitResponse(
                    Assert.Single(sent),
                    operation,
                    false);
                SetSourceModSlotCapacities(
                    ...
                    beforeCapacity,
                    afterCapacity);
                SynchronizeCommittedSourceSurfaces(commitResponse);
                task.HandleFlashResponse(commitResponse, null);

                JObject committed = Assert.Single(web);
                Assert.True((bool)committed["success"]);
                Assert.Equal(
                    afterCapacity,
                    (int)committed["snapshot"]["equipment"][
                        "modSlotCapacity"]);
                Assert.Equal("idle", task.WriteState);
                Assert.Equal(0, task.PreviewBindingCount);
            }
        ...
        [Fact]
        public void DynamicModSlotCapacity_RejectsAfterModsBeyondCapacity()
        {
            var sent = new List<JObject>();
            var web = new List<JObject>();
            using (var task = NewTask(
                ...
                web))
            {
                task.HandleWebRequest(
                    ...
                    Request(
                        "snapshot",
                        "tune.dynamic-capacity-overflow.snapshot"));
                JObject snapshotResponse = OperationSnapshotResponse(
                    Assert.Single(sent),
                    "install_mod",
                    false);
                ...
                task.HandleWebRequest(
                    ...
                    Request(
                        "preview",
                        "tune.dynamic-capacity-overflow.preview",
                        "install_mod"));
                JObject response = OperationPreviewResponse(
                    Assert.Single(sent),
                    "install_mod",
                    false);
                SetSourceModSlotCapacities(response, 1, 0);
                task.HandleFlashResponse(response, null);
        ...
        private static void SetSourceModSlotCapacities(
            JObject response,
            int beforeCapacity,
            int afterCapacity)
        {
            response["before"]["source"]["equipment"][
                "modSlotCapacity"] = beforeCapacity;
            response["after"]["source"]["equipment"][
                "modSlotCapacity"] = afterCapacity;
        }
