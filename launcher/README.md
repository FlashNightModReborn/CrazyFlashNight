...
> **PlayerInfo 当前施工边界**：B0-03b 的 renderer candidate 与 B0-05/B0-06 的 31/31、47/47、视觉 A/B、v1 tag/quorum/preflight、旧 Kimi PASS 都是历史 v1 证据，只批准旧字节且始终 `NOT_DEPLOYED`。main-space v2 已排除 child wrapper，修正 main-visible tight bounds、HP Glow/横线与 MP 排版；MP current 保持 source-derived 右锚 `74.55`，maximum 在 XFL authored `80.65` 上应用 C# path-rasterizer `+0.30` local phase correction，effective anchor 为 `80.95`。真实 Flash v2 candidate 已按 1024×576 main-stage、exported-symbol extraction、wrapper=false 捕获 11 case且 strict tooling identity 通过，但四项 human review 仍为空，只能记 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`。中间 clean F 的 B0-05 32/32 已被后续 prepared-DIB closure 变化 supersede；当前 source-freeze 前 focused 为 180+3/183、full 为 1747+3/1750、dirty B0-06 为 48/48，但 final clean B0-05/B0-06/full、正式 C#/Web/direct A/B、新 source-freeze/tag/request/双 builder quorum/preflight 与最终 Kimi `k3 + highest-available-thinking` 仍待执行。总体状态严格为 `visual_correction_in_progress; human_exit_blocked`：自动门仍未闭合，且人类签收也是不可省略的 Exit Gate，并非“只剩人签”。fixture surface 仍只接 allowlist、不注册现有 NativeHud union、不接 UiData/真实 `pi_*`、不隐藏或修改旧 Flash HUD。workspace/terminal unrestricted 不代表 Apps backend 可用；FFDec 与 headless Playwright/Edge 只能诊断，不能代签真实桌面、游戏 composite、鼠标透传、审美或人类接受。自动结果也不代表 parity、promotion 或标准入口部署。exact runner 与生命周期边界见 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)和[验证矩阵](../agentsDoc/testing-guide.md)。
...
B0-06 自动入口为 `tools/player-info-hud/run-b0-06-runtime-qualification.ps1` 和 [`tools/player-info-hud/README.md`](../tools/player-info-hud/README.md) 的 C# visual capture / headless Edge direct-edge 流程。qualification 合同固定 3000 visible visual steps、3000 idle ticks，并在同一 STA/owner 上运行最多 5 个 100-cycle excluded warmup group；只有三类 handle 的全部 checkpoint 不高于各自组起点、endpoint delta 不为正且无正向趋势时，该组才严格收敛。必须连续两组各自满足该规则，首个合格 pair 后才立即运行新的独立 100-cycle acceptance group；五组内没有合格 pair 必须以 `diagnostic_after_warmup_cap` 失败，不能放宽原门。commit `bf8dd2c…8479` 的 47/47、run `a6aadeb6…7a7b`、旧 NativeHud/split p95 与 C#/Web/comparison A/B 都是历史 v1 qualification，只证明该旧 source/binary/renderer closure，不能批准 main-space v2。当前 v2 真实 Flash candidate 已捕获但待人审；fresh B0-06 formal、两份 C#/Web/direct A/B 与最终 `visual-evidence.json` 尚待 source freeze 后生成。visual manifest 固定为 `structural_capture_complete / fixture_only`，individual comparison 固定为 `diagnostic_awaiting_human_review` 且无接受阈值；Web scope 精确为 `canonical_static_svg_layers_only`，不含 C# 程序化文字/Glow，因此 C#↔Web 指标明确包含 layer-scope difference。这些 offscreen/headless 证据不证明 visible DWM、z-order、click-through 或审美；在 accepted Flash oracle、游戏 composite、真实手点和审美验收前，不得写 `fixture_static_parity`、`fixture_full_parity` 或已部署。

当前 prepared-DIB dirty worktree 预检 run `3b6e9aa8d0574a80a1f91bce82941513` 为 48/48、0 failure，surface/observer-commit/ULW-only p95=`3.3409/0.3052/0.3036 ms`，33-file sourceTrace 包含 DIB helper，environment 为 Normal/精确继承 affinity/server-GC=false；它只证明冻结前当前工作树在该次环境下通过。中间 clean F 的 B0-05 32/32 又已被后续 prepared-DIB closure 变化 supersede；两者都不能代替 final clean/source-bound B0-05/B0-06。
