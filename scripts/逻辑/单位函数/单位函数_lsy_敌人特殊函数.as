...
//
// 【重要说明 - 原始行为兼容性】
// 原始FLA代码中的增益逻辑依赖"基本属性"来控制增益是否生效及其上限。
// 单位需要在初始化时定义相应的"基本属性"，增益才能正常工作。
//
// 摇滚公园单位（萝莉/御姐/少女）在初始化敌人模板中定义了以下基本属性：
//   - 基本防御力: 已定义（来自敌人属性表）→ 防御力增益可生效
//
// 以下基本属性未定义，对应增益不会生效（这是原始行为）：
//   - 基本速度: 未定义 → 速度增益不生效
//   - 基本空手攻击力: 未定义 → 攻击力增益不生效
//   - 基本被击硬直度: 未定义 → 韧性增益不生效（已移除此增益以降低前期难度）
//
// 如需启用这些增益，需在单位初始化时记录对应的基本属性值。
...
_root.敌人函数.摇滚速度提升 = function(单位:MovieClip, 增量:Number, 上限倍率:Number):Void {
	if (!单位) return;
	// 前置条件：需要定义 基本速度 才能计算上限，否则提前退出（保持原始行为）
	if (isNaN(单位.基本速度)) return;

	if (isNaN(增量)) 增量 = 1;
	if (isNaN(上限倍率)) 上限倍率 = 2;

	var 速度上限:Number = 单位.基本速度 * 上限倍率;
 ...
 *
 * 【前置条件】
 * 需要单位定义 基本空手攻击力 属性，否则增益不生效（保持原始行为）
 *
 * @param 单位 目标单位（通常是 _parent）
...
_root.敌人函数.摇滚攻击提升 = function(单位:MovieClip, 百分比增量:Number, 上限倍率:Number):Void {
	if (!单位) return;
	// 前置条件：需要定义 基本空手攻击力 才能计算上限和增量，否则提前退出（保持原始行为）
	if (isNaN(单位.基本空手攻击力)) return;

	if (isNaN(百分比增量)) 百分比增量 = 0.1;
	if (isNaN(上限倍率)) 上限倍率 = 5;

	var 攻击上限:Number = 单位.基本空手攻击力 * 上限倍率;

	if (单位.空手攻击力 < 攻击上限) {
		单位.空手攻击力 += 单位.基本空手攻击力 * 百分比增量;
	}
...
_root.敌人函数.摇滚防御提升 = function(单位:MovieClip, 百分比增量:Number, 上限倍率:Number):Void {
	if (!单位) return;
	// 前置条件：需要定义 基本防御力 才能计算上限和增量，否则提前退出
	// 摇滚公园单位已定义此属性（来自敌人属性表），此检查通常会通过
	if (isNaN(单位.基本防御力)) return;

	if (isNaN(百分比增量)) 百分比增量 = 0.05;
	if (isNaN(上限倍率)) 上限倍率 = 10;

	var 防御上限:Number = 单位.基本防御力 * 上限倍率;

	if (单位.防御力 < 防御上限) {
		单位.防御力 += 单位.基本防御力 * 百分比增量;
	}
