...
(function() {
    ...
    var snapshot, draft, submitted, phase = 'closed', busy = false, composing = false, generation = 0, instance = '';
    ...
    function onOpen(host, data) {
        ...
        disposers.push(Controls.bindNameInput(byId('character-name'), {
            ...
            onChange:function(value) { if (editable()) { draft.characterName = value; refresh(); } }, onEnter:submit
        }));
        disposers.push(Controls.bindGender(element.querySelectorAll('input[name="surgery-gender"]'), function(value) {
            if (editable()) { draft.gender = value; refresh(); render(); }
        }));
        disposers.push(Controls.bindHeight(byId('height'), function(value) {
            if (editable()) { draft.height = value; refresh(); render(); }
        }));
    ...
    function refresh() {
        if (!element) return;
        element.setAttribute('data-state', busy ? 'busy' : phase === 'unknown' || phase === 'save_pending' ? 'reconcile' : phase === 'error' ? 'error' : 'ready');
        var locked = !editable();
    ...
    function receive(result, fromWrite) {
        ...
        if (state) {
            ...
            if (state.phase === 'applied') {
                ...
                requestClose();
            } else setStatus(state.phase === 'save_pending' ? errors.save_pending : '编辑和取消不扣费；确认后立即生效。');
            return;
        }
        ...
        setStatus(errors[code] || '暂时无法完成操作，请重试或重新打开整形。');
    }
    function submit() {
    ...
    function cleanup() {
        ...
        needsSnapshot = false; instance = ''; recoveryToken = '';
