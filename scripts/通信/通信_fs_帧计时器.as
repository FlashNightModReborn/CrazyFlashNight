 ...
 *
 * 示例：
 *   _root.帧计时器.手动设置性能等级(2); // 强制降至低画质
 */
_root.帧计时器.手动设置性能等级 = function(目标等级:Number):Void {
    // 规范化等级值
    ...
    if (this.性能等级 !== 目标等级) {
        ...
        // 重置PID状态，避免立即被自动调控覆盖
        // 但PID系统会在后续帧继续运行，实现智能回升
        this.PID.reset();
        this.awaitConfirmation = false;

        // 调试信息（可选）
        // trace("[性能调控] 手动设置等级: " + 目标等级);
    }
};
 ...
 *
 * 示例：
 *   _root.帧计时器.降低性能等级(1); // 从0档降到1档，或从1档降到2档
 */
_root.帧计时器.降低性能等级 = function(下降档数:Number):Void {
    下降档数 = 下降档数 || 1;
