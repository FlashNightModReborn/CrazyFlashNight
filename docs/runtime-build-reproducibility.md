...
**PlayerInfo main-space v2 当前状态**：下面保留的旧 F/tag/request/quorum/47-Gate/Kimi 数值均为 historical v1，不批准纠正后的 v2 字节。current v2 source F 已冻结为 clean merge `40853287e7ed04714d68935c0002f8ad6d8aea05`，evidence/docs 后继为 `884fbf436f21c5e27ed3370c445cbae941349496`；最终 Kimi `k3/high` 已对该后继完成 165/165 request 的完整只读审计并取得 `K1_AUTOMATED_GATE=PASS`、P0/P1=0。完整双 builder `-VerifyOnly` 闭环仍待 fresh 运行；`config/build/runtime-github-builder.v2.json` 已预留全新一次性 `refs/tags/runtime-build-v2/20260729-player-info-nativehud-b0-v2`，该 tag 当前尚未创建，旧 v1 tag 永不移动或复用。v2 tag 一旦创建就不可移动；只有已冻结的 F/source-policy 输入字节变化时才必须改用新的 tag/request；F 后仅追加 evidence/docs-only disposition，不递归重冻。v2 仍严格为 `source-ahead / NOT_DEPLOYED`，本轮不 promotion、不改正式 runtime 或 release state。
