...
const DISTANCE_BANDS = Object.freeze(['near', 'medium', 'far']);
const DISTANCE_BAND_SET = new Set(DISTANCE_BANDS);
...
function readSpawnDistance(value, path, issues) {
...
export function validateEncounterDefinition(input) {
    const issues = [];
    if (!isRecord(input)) {
    ...
    const profiles = [];
    const profileFirstPath = new Map();
    const bandFirstPath = new Map();
    for (const [index, value] of normalizeOneOrMany(input.profiles).entries()) {
        ...
        const distanceBand = typeof value.distanceBand === 'string'
            && DISTANCE_BAND_SET.has(value.distanceBand)
            ? value.distanceBand
            : null;
        ...
        const catalogEntry = profileId === null
            ? undefined
            : FROZEN_PROFILE_CATALOG[profileId];
