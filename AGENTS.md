...
**最后核对代码基线**：终结者 T800 托管长枪与射击核心 release source commit `d049daed44a537ef586f9e6254fcb824b98c4ee1`（tag `runtime-build-v2/20260822-t800-managed-longgun-v1`）、release tree `7d5e7916fce705eb3ed02971c5887feb5b1bde90`、request `1643570AB50AE207B2D09AC55D6D1F374DD72990070B71423372B6F184497C8A` 已由本地 X509 `builder-local-b` / `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `32577704317`）完成 v2 双 signer/双故障域 promotion；正式 runtime 绑定 identity `0A4478EF7C9D18BCF560B6AB8F2EBA66C255930FCDC361F5FECC36469509A522`、closure `B18AB5EC8A2161621D55ED35C860C96248B9081772CC1DD4B911444815C2169C`、Core DLL SHA-256 `420305DBE54F11A80BABE48BD111FEA960248D3E3430FB90F1B4829EA056DE40`，38/38 production receipt SHA-256 `4238A0DD7E35FDE00FB0BA912000EAE34F63F8E8A09C81AE2AEA564F2DF7E350`；promotion commit `a22490867ebd03b931c3066141c0954b02be25da` 与 post-promotion audit run `32578381540` 均已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本轮 T800 已由维护者在与正式 Core 逐字节相同的隔离候选上完成真实可见业务验收，确认托管枪交付/展示、M134 枪管旋转与 XM214 特效/射速爬升有效；Launcher 4021 pass + 3 explicit opt-in skip / 4024 total、focused Host 22/22、Team 三视口 T800 21/21、ManagedLongGunService 43/43、DressupReferenceManager 70/70、受影响 SWF fresh publish 与 Compiler 0/0 均通过。部署后无 candidate-id 的正式入口又取得 `runtimeMode=formal_runtime`、同 identity/closure、只读 `session.status` 与可信 shutdown exit 0，因此状态为 `HUMAN_ACCEPTANCE_PASSED / promoted`；正式入口没有重跑 `equip_weapon / withdraw_weapon / weapon_tooltip` 业务旅程，仍不称 T800 专项 `standard_entry_verified`，也不外推为全部长枪、真实玩家存档重启或全游戏 E2E。上一设置专项的单屏 `settings_camera_preview` 窄纵切仍保持 `standard_entry_verified`；字体 Gate E 仍保持 14 asset / 14 face / 28 role / 9 preset 与 `GATE_E_COMPLETE / promoted`，Audio H2 继续作为专项 `pending`。
