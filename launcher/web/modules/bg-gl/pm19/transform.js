export function allocateIndexArray(size) {
...
export function reversal(index, size) {
    return size - 1 - index;
}
export function isPermutation(permutation, size = permutation.length) {
...
export function commutesWithReversal(permutation, size = permutation.length) {
    if (permutation.length !== size) {
        return false;
    }
    for (let index = 0; index < size; index += 1) {
...
export function isLegalOrbitTransform(transform, size) {
...
export function reversalCentralizerSize(size) {
...
export function identityTransformInto(output) {
 ...
 */
export function composeTransformsInto(first, second, output) {
    const size = first.rowPermutation.length;
    if (first.columnPermutation.length !== size ||
    ...
    if (output.rowPermutation === first.rowPermutation ||
...
export function invertTransformInto(input, output) {
    const size = input.rowPermutation.length;
    if (input.columnPermutation.length !== size ||
        output.rowPermutation.length !== size ||
        output.columnPermutation.length !== size) {
        throw new RangeError("all transform arrays must have equal lengths");
    }
    if (output.rowPermutation === input.rowPermutation ||
