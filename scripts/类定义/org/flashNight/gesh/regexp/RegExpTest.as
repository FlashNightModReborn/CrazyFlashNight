...
class org.flashNight.gesh.regexp.RegExpTest {
    ...
    private static function runFeatureSupportTests():Void {
        ...
        assertTrue("测试9：/(ab(c|d))*/ 匹配 'abcdabcdabcc'", regex11.test("abcdabcdabcc") === true);
        
        // 测试10：量词 {0}
        var regex12:RegExp = new RegExp("a{0}", "");
        assertTrue("测试10：/a{0}/ 匹配 'abc'", regex12.test("abc") === true);
        
        // 测试11：量词 {n,m}，n > m
        try {
            var regex13:RegExp = new RegExp("a{3,1}", "");
            trace("正则表达式 /a{3,1}/ 解析成功（不符合预期）");
            assertFalse("测试11：/a{3,1}/ 匹配 'aaa'", regex13.test("aaa") === true);
        } catch (e:Error) {
            trace("正则表达式解析错误：" + e.message);
            assertFalse("测试11：/a{3,1}/ 匹配 'aaa' -> false 或处理错误", true); // 预期解析错误
        }
        
        // 测试12：匹配空字符串
        var regex14:RegExp = new RegExp("^$", "");
        assertTrue("测试12：/^$/ 匹配 ''", regex14.test("") === true);
        
        // 测试13：量词允许零次匹配
        var regex15:RegExp = new RegExp("a*", "");
        assertTrue("测试13：/a*/ 匹配 ''", regex15.test("") === true);
        
        // 测试14：任意字符匹配
        var regex16:RegExp = new RegExp("a.c", "");
        assertTrue("测试14：/a.c/ 匹配 'abc'", regex16.test("abc") === true);
        assertTrue("测试14：/a.c/ 匹配 'a c'", regex16.test("a c") === true);
        assertFalse("测试14：/a.c/ 匹配 'abbc'", regex16.test("abbc") === true);
        
        // 测试15：字符集与量词组合
        var regex19:RegExp = new RegExp("[abc]+", "");
        assertTrue("测试15：/[abc]+/ 匹配 'aaabbbcccabc'", regex19.test("aaabbbcccabc") === true);
        
        // 测试16：否定字符集与量词组合
        var regex20:RegExp = new RegExp("[^abc]+", "");
        assertTrue("测试16：/[^abc]+/ 匹配 'defg'", regex20.test("defg") === true);
        
        // 测试17：多个选择的组合
        var regex21:RegExp = new RegExp("a|b|c", "");
        assertTrue("测试17：/a|b|c/ 匹配 'b'", regex21.test("b") === true);
        assertFalse("测试17：/a|b|c/ 匹配 'd'", regex21.test("d") === true);
        
        // 测试18：量词嵌套
        var regex23:RegExp = new RegExp("(a+)+", "");
        assertTrue("测试18：/(a+)+/ 匹配 'aaa'", regex23.test("aaa") === true);
        
        // 测试19：无法匹配的情况
        var regex24:RegExp = new RegExp("a{4}", "");
        assertFalse("测试19：/a{4}/ 匹配 'aaa'", regex24.test("aaa") === true);
        ...
        assertTrue("测试20：/a{1000}/ 匹配 1000 个 'a'", regex25.test(longString) === true);
    ...
    private static function runComplexProjectTests():Void {
        ...
        assertTrue("测试21：numberRegExp 匹配 '123'", numberRegExp.test("123") === true);
        assertTrue("测试21：numberRegExp 匹配 '-123.45'", numberRegExp.test("-123.45") === true);
        assertTrue("测试21：numberRegExp 匹配 '1e10'", numberRegExp.test("1e10") === true);
        assertFalse("测试21：numberRegExp 匹配 '12a'", numberRegExp.test("12a") === true);
        
        // 测试22：日期时间验证
        var dateTimeRegExp:RegExp = new RegExp("^\\d{4}-\\d{2}-\\d{2}[Tt ][0-2]\\d:[0-5]\\d:[0-5]\\d(\\.\\d+)?([Zz]|([+-][0-2]\\d:[0-5]\\d))?$", "");
        assertTrue("测试22：dateTimeRegExp 匹配 '2024-10-09T08:30:00Z'", dateTimeRegExp.test("2024-10-09T08:30:00Z") === true);
        assertTrue("测试22：dateTimeRegExp 匹配 '2024-10-09 08:30:00+02:00'", dateTimeRegExp.test("2024-10-09 08:30:00+02:00") === true);
        assertFalse("测试22：dateTimeRegExp 匹配 '2024-13-40T25:61:61Z'", dateTimeRegExp.test("2024-13-40T25:61:61Z") === true);
        
        // 测试23：布尔值验证
        var booleanRegExp:RegExp = new RegExp("^(true|false)$", "i");
        assertTrue("测试23：booleanRegExp 匹配 'true'", booleanRegExp.test("true") === true);
        assertTrue("测试23：booleanRegExp 匹配 'FALSE'", booleanRegExp.test("FALSE") === true);
        assertFalse("测试23：booleanRegExp 匹配 'yes'", booleanRegExp.test("yes") === true);
        
        // 测试24：特殊浮点数验证
        var specialFloatRegExp:RegExp = new RegExp("^(nan|inf|-inf)$", "i");
        assertTrue("测试24：specialFloatRegExp 匹配 'NaN'", specialFloatRegExp.test("NaN") === true);
        assertTrue("测试24：specialFloatRegExp 匹配 'inf'", specialFloatRegExp.test("inf") === true);
        assertTrue("测试24：specialFloatRegExp 匹配 '-INF'", specialFloatRegExp.test("-INF") === true);
        assertFalse("测试24：specialFloatRegExp 匹配 '1.23'", specialFloatRegExp.test("1.23") === true);
        
        // 测试25：电子邮件验证
        var emailRegExp:RegExp = new RegExp("^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$", "");
        assertTrue("测试25：emailRegExp 匹配 'test@example.com'", emailRegExp.test("test@example.com") === true);
        assertFalse("测试25：emailRegExp 匹配 'invalid-email@'", emailRegExp.test("invalid-email@") === true);
        
        // 测试26：URL验证
        var urlRegExp:RegExp = new RegExp("^(https?:\\/\\/)?([\\w.-]+)\\.([a-z\\.]{2,6})([\\/\\w .-]*)*\\/?$", "i");
        assertTrue("测试26：urlRegExp 匹配 'https://www.example.com/path'", urlRegExp.test("https://www.example.com/path") === true);
        assertFalse("测试26：urlRegExp 匹配 'htp:/invalid-url'", urlRegExp.test("htp:/invalid-url") === true);
    ...
    private static function runPrototypeInjectionTests():Void {
        ...
        if (typeof(str.regexp_match) == "function") {  // 在调用之前检查函数是否存在
            ...
            assertTrue("测试3：regexp_match 返回数组长度为 2", matchResult != null && matchResult.length === 2);
            if (matchResult != null) {
                assertEquals("测试3：regexp_match 第1个匹配项", "123", matchResult[0]);
                assertEquals("测试3：regexp_match 第2个匹配项", "456", matchResult[1]);
            }
        } else {
        ...
        if (typeof(str.regexp_replace) == "function") {
            ...
            assertEquals("测试4：regexp_replace 结果", "This is # and #.", replaceResult);
        } else {
        ...
        if (typeof(str.regexp_search) == "function") {
            ...
            assertEquals("测试5：regexp_search 结果", 8, searchResult);
        } else {
        ...
        if (typeof(str.regexp_split) == "function") {
            ...
            assertTrue("测试6：regexp_split 返回数组长度为 3", splitResult != null && splitResult.length === 3);
            if (splitResult != null) {
                assertEquals("测试6：regexp_split 第1部分", "This is ", splitResult[0]);
                assertEquals("测试6：regexp_split 第2部分", " and ", splitResult[1]);
                assertEquals("测试6：regexp_split 第3部分", ".", splitResult[2]);
            }
        } else {
