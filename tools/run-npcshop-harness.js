...
function audit(){
  ...
  if(!['bag','material','intelligence'].every(v=>panel.includes("createOwnedView('"+v+"'")))throw new Error('right-view sibling contract missing');
  ...
  if(!panel.includes('autoClassificationPath')||!panel.includes('item.weaponType || item.actionType')||!panel.includes("mode:'manual'"))throw new Error('hierarchical automatic grouping or manual override missing');
