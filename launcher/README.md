...
> **最后核对代码基线**：commit `6a4bdd97ba`（2026-06-25）。
...
按 commit `6a4bdd97ba`（2026-06-25）时的源码树复核。只列追踪目录；`bin/` / `packages/` / `target/` / `node_modules/` / `obj/` 等构建产物和缓存均由 .gitignore 管理。后续若需复核本节，优先看 `git diff 6a4bdd97ba..HEAD -- launcher/`。C# 文件级权威以 [CRAZYFLASHER7MercenaryEmpire.csproj](CRAZYFLASHER7MercenaryEmpire.csproj) / [tests/Launcher.Tests.csproj](tests/Launcher.Tests.csproj) 的 `Compile Include` 为准，README 仅保留职责树。
...
[build.ps1](build.ps1) 的 `Step 7b`（脚本注释里标注为 6b，但运行时打印 `[Step 7b/7]`）会校验 `save_schema.json` 存在；缺失时报错并提示运行 `node tools/extract-save-schema.js`。当前 diff 视图主要靠 schema 里 inline 的 `default`，`save_schema.json` 是为后续 diff 范围扩展（不在 schema 白名单内的任意字段）准备的基线。
