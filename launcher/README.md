...
按 commit `c2118e295`（2026-04-20）时的源码树整理。只列追踪目录；`bin/` / `packages/` / `target/` / `node_modules/` / `obj/` 等构建产物和缓存均由 .gitignore 管理。后续若需复核本节，优先看 `git diff c2118e295..HEAD -- launcher/`。
...
│   │   ├── ISolParser.cs / ISolFileLocator.cs / IArchiveStateProbe.cs
...
当前 `Launcher.Tests.csproj` 只编译 4 个测试源码文件，没有额外的 `Fixtures/SaveMigrator/` 外部 fixture 目录；`SaveMigratorTests` 用的是代码内 helper 数据。

| 文件 | 覆盖面 | `[Fact]` 数量 |
|------|--------|---------------|
| `Save/SaveMigratorTests.cs` | undefined→2.6→2.7→3.0 迁移链 + MergeTopLevelKeys + ValidateResolvedSnapshot 结构校验 | 24 |
...
| Archive query | `list` | 列所有槽位 → `list_resp`（`slots` + 4 个固定偏好字段；`lastPlayedSlot` 仅非空时附带） | 任意 |
