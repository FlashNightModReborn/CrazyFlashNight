...
class org.flashNight.naki.DataStructures.AVLTreeTest {
    ...
    private function testAdd():Void {
        trace("\n测试 add 方法...");
        avlTree = new org.flashNight.naki.DataStructures.AVLTree(numberCompare);
        avlTree.add(10);
        avlTree.add(20);
        avlTree.add(5);
        avlTree.add(15);
        ...
        assert(avlTree.size() == 4, "添加元素后，size 应为4");
        assert(avlTree.contains(10), "AVLTree 应包含 10");
    ...
    private function testRemove():Void {
        trace("\n测试 remove 方法...");
        var removed:Boolean = avlTree.remove(20);
    ...
    private function testContains():Void {
        ...
        assert(avlTree.contains(5), "AVLTree 应包含 5");
        assert(avlTree.contains(15), "AVLTree 应包含 15");
        assert(!avlTree.contains(25), "AVLTree 不应包含 25");
    }

    private function testSize():Void {
        trace("\n测试 size 方法...");
        assert(avlTree.size() == 3, "当前 size 应为3");
        avlTree.add(25);
    ...
    private function testBuildFromArray():Void {
        ...
        var newTree:AVLTree = AVLTree.buildFromArray(arr, numberCompare);
    ...
    private function testChangeCompareFunctionAndResort():Void {
        ...
        avlTree = new AVLTree(numberCompare);
        ...
        avlTree.changeCompareFunctionAndResort(descCompare);

        var sortedDesc:Array = avlTree.toArray();
     ...
     * 测试AVL树特有属性
     */
    private function testAVLProperties():Void {
        trace("\n测试AVL树特有属性...");
    ...
    private function testPerformance():Void {
        ...
        for (var j:Number = 0; j < capacities.length; j++) {
            ...
            var largeTree:AVLTree = null;

            for (var i:Number = 0; i < iteration; i++) {
                // 1) 测试添加性能
                largeTree = new org.flashNight.naki.DataStructures.AVLTree(numberCompare);
                ...
                startTime = getTimer();
                var tempTree:AVLTree = AVLTree.buildFromArray(arr, numberCompare);
            ...
            assert(containsAll, "所有添加的元素都应存在于 AVLTree 中");
     ...
     * 3. 高度计算正确
     */
    private function validateAVLProperties(node:AVLNode):Boolean {
        if (node == null) {
            return true;
        }
        ...
        if (balance < -1 || balance > 1) {
            trace("AVL违规: 节点 " + node.value + " 平衡因子=" + balance + " (应在[-1,1]范围内)");
            return false;
        }

        // 验证高度计算正确
        var expectedHeight:Number = 1 + (leftHeight > rightHeight ? leftHeight : rightHeight);
        if (node.height != expectedHeight) {
            trace("AVL违规: 节点 " + node.value + " 高度=" + node.height + " (应为" + expectedHeight + ")");
            return false;
        }

        // 递归检查子树
        return validateAVLProperties(node.left) && validateAVLProperties(node.right);
    }
