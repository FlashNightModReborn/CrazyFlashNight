...
> **PlayerInfo 当前施工边界**：B0-03b 仅达到 `renderer_qualified / candidate_built / NOT_DEPLOYED`，B0-05 达到 `passed / synthetic_fixed_bounds / split_required`。2026-07-29 的 B0-06 已实现 allowlist opt-in 的 fixture-only `PlayerInfoWidget`、程序化 HP/MP 与独立 click-through split surface；它不注册进现有 NativeHud union、不接 UiData/真实 `pi_*`，也不自动隐藏或修改旧 Flash HUD。clean 实现基线 `bf8dd2c410267855c8ea12f25a594042b3158479` 的真实 HWND/ULW qualification 为 47/47，C# 36-file A/B、Web 12-file A/B 与 comparison 66-PNG A/B 也各自确定，证据为 [`runtime-qualification.json`](../tools/player-info-hud/evidence/b0-06/runtime-qualification.json) 与 [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json)。Computer Use 本轮已恢复初始化、窗口枚举、显式激活与截图，确认 Flash CS6 当前打开 `TestLoader.xfl`；遮挡态首抓会误取前台窗口，Flash UIA 也只暴露窗口/标题栏，且当前没有独立 Test Movie/SWF 窗口，所以它只能辅助核对 authoring 目标，不能证明运行时加载身份或代替真人验收。完整实现包 Kimi k3/max 首轮问题已修复，但最终增量 PASS、Flash candidate 人签、真实桌面 DWM/z-order、游戏 composite、手工鼠标透传与审美仍未闭合，因此自动结果不代表 parity、promotion 或标准入口部署。exact runner 与生命周期边界见 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)和[验证矩阵](../agentsDoc/testing-guide.md)。
