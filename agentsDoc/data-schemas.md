...
| `data/intelligence/` | 情报详情 legacy txt 文本；AS2 旧界面和 Launcher Web 情报测试面板共用 |
| `config/` | 系统配置 |
...
| `InformationDictionaryLoader` | `data/dictionaries/information_dictionary.xml` | 情报条目元数据；Launcher Web 阶段一由 C# `IntelligenceTask` 读取同一 XML，并按字典白名单读取 `data/intelligence/<itemName>.txt` |
...
Launcher Web 情报测试面板的阶段一实现不开放 WebView2 对 `data/` 或项目根的 fetch 权限，而是由 C# `IntelligenceTask` 精确命中字典项后读取固定目录文本，并校验最终 full path 仍在 `data/intelligence/` 下。Web 侧首选消费 `bundle` 全量包（全部情报元数据、分页正文、加密规则与 `iconName`），保留 `catalog` / `snapshot` 兼容路径；`iconName` 来自 `data/items/收集品_情报.xml` 的 `<icon>` 字段，最终图标仍通过 `launcher/web/icons/manifest.json` 解析。
