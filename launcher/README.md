...
│   ├── overlay.html                       DOM 结构 (Toast + Notch + 工具条 + 任务行 + 安全退出 + 帮助 + Jukebox)
...
│       └── help.js                        游戏帮助弹窗 (Markdown 加载 + tab 切换 + 缓存)
├── bin/Release/                           编译输出
├── packages/                              NuGet 包缓存
└── tools/
    └── nuget.exe                          NuGet CLI
...
| 帮助界面 (帮助界面.swf) | `#game-help-modal` Markdown 弹窗 | 纯 Web 侧 (XHR 加载 .md) |
