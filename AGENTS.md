...
**状态边界**：本列车把斗兽标定从工作簿原始来源摄取、参数闭包、`contamination` Schema/fixture、timeout 口径和真正 Schema 实例校验，收束到可冻结、可撤销、可持久恢复的 Gate F 星期级批处理。tracked v7 草案含 58 个 scheduled candidate，另将 `B12` 隔离为 quarantine，共 198 个 10–25 run shard / 3,285 个机器 run；三份 fresh soak 各为 10 个代表样本 + 10 个派生阵营复核样本，`C9` 仍保留 `10 + 20 + 25` run 与 side-swap 全量复核。
...
提交后正式 admission 已在 exact formal runtime 上完成 20/20 finished、0 timeout/error/recovery、正常 shutdown 和受保护存档不变，目标死亡派生实际观察 red 8 / blue 8；`arena-calibration.soak-admission.v2` 将原始 manifest/result/report SHA-256、双向派生最低计数与案例模板闭合，准入 hash 为 `sha256:60c5662d89b5b8f0e53cbe288ea1461ef32a933aff654dd4171dadf8fac396a0`。准确状态仍只是下一 cohort 已具备 clean freeze 条件，尚未 freeze/arm 或执行三份 fresh soak；promotion/admission 不代签 Gate F low-touch、星期级数值样本或业务 `standard_entry_verified`，Audio H2 仍保持独立门。
