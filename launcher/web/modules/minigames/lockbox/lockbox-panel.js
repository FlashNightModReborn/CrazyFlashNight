var LockboxPanel = (function() {
    ...
    function createDOM() {
        ...
        _el.innerHTML = [
            '<div class="minigame-header lockbox-header">',
                '<div>',
                    '<div class="minigame-kicker lockbox-kicker">// WEBVIEW PROTOTYPE //</div>',
                    '<div class="minigame-title lockbox-title">高安箱协议破解器</div>',
                '</div>',
                '<div class="minigame-header-right lockbox-header-right">',
                    '<button class="minigame-chrome-btn lockbox-chrome-btn" type="button" data-action="toggle-help">规则</button>',
                    '<button class="minigame-chrome-btn lockbox-chrome-btn" type="button" data-action="toggle-hud">调试</button>',
                    '<button class="minigame-chrome-btn lockbox-chrome-btn" type="button" data-action="toggle-mute">♪ 开</button>',
                    '<div class="minigame-phase-badge lockbox-phase-badge" id="lockbox-phase-badge">INIT</div>',
                    '<button class="minigame-close-btn lockbox-close-btn" type="button">×</button>',
                '</div>',
            '</div>',
            '<div class="lockbox-help-panel" id="lockbox-help-panel">',
                '<div class="lockbox-help-card">',
                    '<div class="lockbox-help-head">',
                        ...
                        '</div>',
                        '<button class="minigame-chrome-btn lockbox-chrome-btn" type="button" data-action="toggle-help">关闭</button>',
                    '</div>',
            ...
            '<div class="minigame-main lockbox-main">',
                ...
                '<div class="minigame-side-pane lockbox-side-pane">',
                    ...
                    '<section class="minigame-side-section lockbox-side-section">',
                        '<div class="minigame-side-title lockbox-side-title">目标序列</div>',
                        '<div id="lockbox-sequences"></div>',
                    '</section>',
                    '<section class="minigame-side-section lockbox-side-section lockbox-buffer-section">',
                        '<div class="minigame-side-title lockbox-side-title">Buffer</div>',
                        '<div id="lockbox-buffer"></div>',
                    '</section>',
                    '<section class="minigame-side-section lockbox-side-section lockbox-trace-section">',
                        '<div class="minigame-side-title lockbox-side-title">Trace</div>',
                    ...
                    '</section>',
                    '<section class="minigame-side-section lockbox-side-section lockbox-status-section">',
                        '<div class="minigame-side-title lockbox-side-title">运行状态</div>',
            ...
            '<div class="lockbox-hud">',
                ...
                '<div class="lockbox-hud-grid">',
                    '<div class="lockbox-hud-card">',
                        '<div class="minigame-side-title lockbox-side-title">Solver 指标</div>',
                        '<div id="lockbox-metrics"></div>',
                    '</div>',
                    '<div class="lockbox-hud-card">',
                        '<div class="minigame-side-title lockbox-side-title">运行指标</div>',
    ...
    function onOpen(el, initData) {
        ...
        loadPuzzle(data);
        startLoop();
        notifyHost('open', undefined, {
            mode: data.mode,
            requestedProfile: data.profile,
            requestedSource: data.source,
            familySeed: data.familySeed,
    ...
    function notifyHost(eventName, resultPayload, extraData) {
        ...
        var key;
        if (extraData) {
            for (key in extraData) payload[key] = extraData[key];
        }
        if (_state) {
            payload.profile = _state.config.id;
            payload.source = _state.request.source;
            payload.familySeed = _state.puzzle.familySeed;
            payload.variantIndex = _state.puzzle.variantIndex;
            payload.metrics = collectSessionMetrics();
        }
        if (resultPayload !== undefined) payload.result = resultPayload || null;
        sendHostSession(eventName, payload);
    }
