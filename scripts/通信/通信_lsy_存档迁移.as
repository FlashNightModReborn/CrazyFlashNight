...
_root.物品栏数据迁移 = function(){
   ...
   for(var i=0; i<_root.物品栏总数; i++){
      ...
      if(旧物品[2] == 1){
         if(use == "药剂"){
            if(_root.快捷物品栏1 == 旧物品[0]) 新物品栏.药剂栏[0] = 新物品;
            else if(_root.快捷物品栏2 == 旧物品[0]) 新物品栏.药剂栏[1] = 新物品;
            else if(_root.快捷物品栏3 == 旧物品[0]) 新物品栏.药剂栏[2] = 新物品;
            else if(_root.快捷物品栏4 == 旧物品[0]) 新物品栏.药剂栏[3] = 新物品;
         }else{
            新物品栏.装备栏[use] = 新物品;
         }
      }else if(use == "材料"){
   ...
   for(var i=0; i<1200; i++){
      var 旧物品 = 旧仓库[i];
      if(!旧物品 || 旧物品[0] == "空") continue;
      新物品栏.仓库[i] = {name:旧物品[0],value:旧物品[1]};
   }
   //迁移战备箱数据
   for(var i=0; i<400; i++){
      var 旧物品 = 旧仓库[i+1200];
      if(!旧物品 || 旧物品[0] == "空") continue;
      新物品栏.战备箱[i] = {name:旧物品[0],value:旧物品[1]};
   }
   //完成
   // _root.物品栏 = 物品栏;
   // _root.仓库栏 = null;
