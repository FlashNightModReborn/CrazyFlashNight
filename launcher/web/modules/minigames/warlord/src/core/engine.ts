...
import {
  enqueueCommanderProductionInPlace,
  handleCommanderCasualtyInPlace,
  redeployPlayerAvatarInPlace,
} from './commanders.js';
...
function battleSnapshot(state: GameState, pieceId: string): BattleUnitSnapshot {
...
function executeMoveOrAttack(
  ...
  for (const pieceResult of result.pieceResults) {
    ...
    if (piece) piece.hp = pieceResult.hpAfter;
  }
  ...
  if (result.winner === 'attacker') {
    movePiecesInPlace(
      state,
      actualPieceIds.filter((pieceId) => state.pieces[pieceId] !== undefined),
...
function executeAllocateXp(state: GameState, command: AllocateXpCommand): void {
  const faction = requireFaction(state, command.factionId);
  ...
  while (card.level < 50) {
    ...
    if (card.xpIntoLevel < required) break;
...
function executePromotion(state: GameState, command: PurchasePromotionCommand): void {
  const faction = requireFaction(state, command.factionId);
  const card = faction.cards[command.cardId];
...
function executeEnqueueProduction(state: GameState, command: EnqueueProductionCommand): void {
  const faction = requireFaction(state, command.factionId);
  ...
  if (!slot) throw new Error(`Validated production slot disappeared: ${command.slotId}`);
...
function executeCancelProduction(state: GameState, command: CancelProductionCommand): void {
  const faction = requireFaction(state, command.factionId);
  const slot = faction.productionQueues[command.nodeId]?.find((candidate) => candidate.slotId === command.slotId);
  if (!slot) throw new Error(`Validated production slot disappeared: ${command.slotId}`);
  const orderIndex = slot.orders.findIndex((candidate) => candidate.orderId === command.orderId);
  if (orderIndex < 0) throw new Error(`Validated production order disappeared: ${command.orderId}`);
  const [order] = slot.orders.splice(orderIndex, 1);
  if (!order) throw new Error(`Validated production order disappeared: ${command.orderId}`);
...
export function applyCommand(state: GameState, command: GameCommand): CommandResult {
  ...
  switch (command.type) {
    case 'MOVE_OR_ATTACK':
      battleId = executeMoveOrAttack(
        next,
        command,
        validation.actualPieceIds ?? command.pieceIds,
        validation.isBattle ?? false,
        validation.commandLoad ?? command.pieceIds.length,
        validation.defenderFactionId,
      );
      break;
    case 'MERGE_TASK_GROUP': {
      const element = mergeTaskGroupInPlace(
        next,
        command.commandElementIds,
        command.taskGroupTemplateId,
        command.formationProfileId,
      );
      addGameEvent(next, {
    ...
    case 'END_ACTION':
      ...
      if (next.result) break;
      {
        ...
        if (nextIndex !== null) {
          ...
          if (nextFactionId === undefined) throw new Error('Turn order cursor escaped its bounds.');
...
export function applyCommands(state: GameState, commands: GameCommand[]): CommandResult {
  let current = state;
  let lastBattleId: string | undefined;
  for (const command of commands) {
    const result = applyCommand(current, command);
    if (!result.ok) return result;
    current = result.state;
    if (result.battleId) lastBattleId = result.battleId;
  }
