...
> **最后核对架构与工作流**：双栏工作台 A1–A6 release source `730c6be781ddd22bfd7a59a2e7773acce892f105` 已由 immutable tag `runtime-build-v2/20260806-workbench-authority-a1-a6-v2`、release tree `1434e71d44fe41ad0fa426bc857085f1aff940dd`、request `D45D0DEF50E159B8A875DCA922D856BEC69D64A6B779B6B7B7082D68AC6B92CF`、本地 X509 `physical-host-b` + GitHub OIDC/Sigstore `github-hosted-windows` 双故障域共识完成正式 promotion。无 candidate id 的 pure-MCP 正式入口报告 `tmp/manual-agent-acceptance/workbench-v2/formal-workbench-20260806T133125Z.json` 完成 Launcher 与 allow-listed Help WebOverlay WGC、one-shot terminal receipt、supported shutdown、测试存档不变和无新增残留进程，故该明确窄纵切达到 `standard_entry_verified`；这不代表 Character/Equipment 业务 opener、A1–A6 写操作、物理拖拽、玩家目视签收或所有专项实机旅程。本文只维护 Launcher 的稳定入口、协议与验证规则；发布细节见 [双栏工作台治理 ADR](../docs/双栏工作台-权威数据与交互能力一致性治理-ADR-2026-08-01.md) §18，通用判定见 [验证矩阵](../agentsDoc/testing-guide.md)。
> **PlayerInfo 当前边界**：F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / r2 自身仍是历史 non-deployment train；其实现字节被当前 `730c6be781…` release 包含并进入 formal runtime。B0 仍为 `b0_accepted`，oracle 仍为 `oracle_frozen_for_b0`；本轮 formal smoke 没有启用 PlayerInfo fixture、观察真实 `pi_*` 或完成 PlayerInfo-specific standard-entry E2E，不能从总体 runtime 的 `standard_entry_verified` 外推 PlayerInfo 专项验收。历史 identity、可见验收与边界继续见 [runtime v2 深层文档](../docs/runtime-build-reproducibility.md)、[PlayerInfo 工具证据](../tools/player-info-hud/README.md)和 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)。
...
当前 consensus 采用本地 X509 keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3` / `physical-host-a` 与 GitHub OIDC builder identity `2E05B5577CC8D2895DC56FC8778A05D09C440302832A05DD4AE551589C9AE918` / `github-hosted-windows`（run `30602046108`）；tracked registry 仍只含公钥。正式 bootstrap/runtime/manifest/consensus 已由唯一 promotion 入口切换到 request `A9B33601805709DBB5EAE6DAF312C2B7B0B502096FDD3BDCEA9CBE26D8B1299C`、build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C` 与 payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0`；此前 no-AS2-fallback、材料/商城/战备箱、角色构筑 ↔ Skills、地图资源箱、理发店、商店与单-canary consensus 均只保留为历史记录。一次性 migration fuse 不得复用，后续发布不得降级 v1。
