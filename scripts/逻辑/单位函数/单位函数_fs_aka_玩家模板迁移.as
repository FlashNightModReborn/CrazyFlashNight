...
_root.刷新人物装扮 = function(目标) {
    ...
    if (攻击模式 != "空手" && 目标人物.状态.indexOf(旧攻击模式) != -1) {
        if (旧攻击模式 == "长枪" && !长枪)
            ...
            是否卸下装备 = true;
        else if (旧攻击模式 == "手枪2" && !手枪2)
            是否卸下装备 = true;
        else if (旧攻击模式 == "双枪" && (!手枪 || !手枪2))
            是否卸下装备 = true;
        else if (旧攻击模式 == "兵器" && !刀)
            是否卸下装备 = true;
        //
    ...
    var 缓存键:String = 头部装备 + "_" + 上装装备 + "_" + 手部装备 + "_" + 下装装备 + "_" + 脚部装备 + "_" + 颈部装备 + "_" + 长枪 + "_" + 手枪 + "_" + 手枪2 + "_" + 刀 + "_" + 头部强化等级 + "_" + 上装强化等级 + "_" + 手部强化等级 + "_" + 下装强化等级 + "_" + 脚部强化等级 + "_" + 颈部强化等级 + "_" + 长枪强化等级 + "_" + 手枪强化等级 + "_" + 手枪2强化等级 + "_" + 刀强化等级 + "_" + 手雷;
    //_root.发布调试消息(缓存键);
    ...
    for (var 迭代器 in 装备数据) {
        ...
        //初始化一下
        if (_temp武器加成前缀 != "") {
            目标人物[_temp武器加成前缀 + "伤害类型"] = "";
            目标人物[_temp武器加成前缀 + "魔法伤害属性"] = "";
        }
    ...
    if (目标人物.装备刀锋利度加成 && 目标人物.刀属性.power) {
        目标人物.刀属性.power += 目标人物.装备刀锋利度加成;
    }

    目标人物.裸装长枪威力 = 目标人物.长枪属性.power;
    目标人物.裸装手枪威力 = 目标人物.手枪属性.power;
    目标人物.裸装手枪2威力 = 目标人物.手枪属性.power;
    if (目标人物.装备枪械威力加成) {
        if (目标人物.长枪属性.power) {
            目标人物.长枪属性.power += 目标人物.装备枪械威力加成;
        }
        if (目标人物.手枪属性.power) {
            目标人物.手枪属性.power += 目标人物.装备枪械威力加成;
        }
        if (目标人物.手枪2属性.power) {
            目标人物.手枪2属性.power += 目标人物.装备枪械威力加成;
        }
    }
    ...
    if (_root.控制目标 === 目标人物._name) {
        //健身房加成
        if (isNaN(_root.全局健身HP加成))
            _root.全局健身HP加成 = 0;
        if (isNaN(_root.全局健身MP加成))
            _root.全局健身MP加成 = 0;
        if (isNaN(_root.全局健身空攻加成))
            _root.全局健身空攻加成 = 0;
        if (isNaN(_root.全局健身内力加成))
            _root.全局健身内力加成 = 0;
        if (isNaN(_root.全局健身防御加成))
            _root.全局健身防御加成 = 0;
        if (目标人物.hp == 目标人物.hp满血值)
            目标人物.hp += _root.全局健身HP加成;
        目标人物.hp满血值 += _root.全局健身HP加成;
        if (目标人物.mp == 目标人物.mp满血值)
            目标人物.mp += _root.全局健身MP加成;
