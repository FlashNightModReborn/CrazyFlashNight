...
- **改 / 增装备生命周期函数**：编译真源是 `scripts/asLoaderManifest/frame37.as`（f37_N chunk），**不是**旧 `装备函数列表.as`（已删）。用途索引 / API 快查 / 新增 7 步见 `scripts/逻辑/装备函数/README.md`；改完 `node tools/assemble-collapsed-frame.js` → CS6 重开 asLoader 重编 → `node tools/validate-equip-fn-coverage.js` 自检一致性。
