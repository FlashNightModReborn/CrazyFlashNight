...
| 1e   | 生成 `web/modules/tasks/task-catalog.json` 后运行 `node tools/test-derive-task-conditions.js`（23 个合成夹具覆盖 conditions 单调 AND-OR 不动点；任务奖励/需求 `icon` 从 `data/items/*.xml` 派生；失败 exit 1） |
