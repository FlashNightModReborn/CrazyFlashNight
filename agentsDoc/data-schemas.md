...
- **武器补弹参数** `<fillrate>`（武器 `<data>` 内，可选）：`auto` = 纵向联弹按本次实际射击间隔（含枪械师点按/连按修正）推导每帧补弹数，保证霰弹值在两次射击间隔内发射完毕（超高射速武器用，如 XM214）；正整数 = 显式每帧补弹数（小数向上取整）；**未配置 = 完全旧行为**（每帧补 1 发，射击链路不写不读任何新键）。
