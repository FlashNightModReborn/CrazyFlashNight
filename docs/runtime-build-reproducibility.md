...
**最后核对代码基线**：no-AS2-fallback 双栏工作台退役 cut 的 source commit `f01f4b121a4ceebd7dae051f14bb511c5ae3f1cb`（tag `runtime-build-v2/20260730-workbench-no-as2-fallback-v1`）、release tree `3dfe2dcdb25b26f22a2c36b400524a3b6aca0e28` 与 request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B` 已完成正式 promotion。exact isolated candidate 与无参 `automation/start.ps1` 正式入口 attempt `8baf52bbcceb452da32da641e58d2922` 均完成 production Equipment Tuning opener → exact workbench instance → 同实例首个权威 snapshot → supported application shutdown 的只读纵切，故 Launcher 发布生命周期在该明确范围内为 `standard_entry_verified`。GUI 人工验收已通过；该状态不代表 Character Build / Workbench 全业务、写入、普通关闭、持久化或各子功能专项 standard-entry 验收，完整边界见 [no-AS2-fallback runtime 发布记录](evidence/workbench-no-as2-fallback-runtime-release-2026-07-30.md)。

## 当前迁移状态

runtime v2 的工具、schema、队列、本地 X509 证明、GitHub OIDC/Sigstore 证明、promotion 与 CI 状态机已经完成正式闭环；**仓库当前受控部署已是 manifest/consensus v2**。当前 consensus 的 artifact source `D80540591DDB71CD1C935A1EE3DEC7657D64F43BA8CC81FB359E181A6CBAEB2F`、producer recipe `B97998EA7246D6AB667902BCBBD7994DFA5F658A37CFA427D2EEEABA6924DE28` 与 toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD` 形成 build identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465`；payload closure 为 `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF`，manifest SHA-256 为 `B921A734A5DE490E8DDD5E2E34BA90949D2E7CB444AF484B7AA1E9D955CB920D`。production policy `0755D2284266537033A4B862D830EE59E0202E7D9659D2C84F280CD8D606A105` 的 26/26 receipt SHA-256 为 `C9F225466717594F6675512841A194A94A29938A905B13ED28D093713322DDA8`；本地 X509 signer / `physical-host-b`（keyId `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`）与 GitHub OIDC builder `7FFC35F8EDAD03FA477FC9D6BDF86C0FCA8A5CCB591F0CEB56577C0B85BDE9E7` / `github-hosted-windows`（run `30538330223`）的双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier 全部满足后，于 `2026-07-30T11:42:21.7606896Z` 完成 promotion。

上一 `9118eb…` request 的隐藏本地 worker UTF-8 解码失败与 GitHub helper 瞬时 API EOF 只保留为历史 portability 观察，不属于当前 request。当前 `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B` 使用显式 UTF-8 本地 worker，并且只 dispatch 一次 GitHub run `30538330223`；本地与云端有效证明精确绑定同一 build identity / payload closure。
...
当前正式部署对应 source commit `f01f4b121a4ceebd7dae051f14bb511c5ae3f1cb`、build identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465`、payload closure `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF` 与 Core SHA-256 `565C1F9710421E6B6CC5CB6DDA05DE36B8F1B22B3D7A7CA19617F9786C7D8A4B`，包含 no-AS2-fallback 双栏工作台退役 cut 及此前正式能力。无参正式根入口以 `formal_runtime` 启动并绑定上述身份；首轮 attempt `bfb6bfe515f14af58f9b4096c8c539dd` 在 `20260730T114417Z` 因 reveal watchdog 于 `19:44:51.961` 先到而以 `title_frame_not_observed` fail-closed，真实 `bootstrap_reveal_ready` 于约 8 ms 后到达，失败报告和 supported shutdown 继续保留。随后全新 attempt `8baf52bbcceb452da32da641e58d2922`（`tmp/equipment-tuning/unattended/20260730T114654Z-cf7_agent_equipment_tuning/run-report.md`）取得 fresh handoff、真实 title receipt、单次 enter、production Equipment Tuning opener、同实例首个权威 snapshot 与 supported application shutdown，故严格状态为 `standard_entry_verified`。native identity / closure 只覆盖正式 runtime 文件闭包；Web/Flash 字节与该只读纵切另由冻结 source/release tree、production policy、GUI 人工验收和实际日志绑定。本轮没有业务 preview/commit、普通 panel close、持久化或重启回读，不能把总体状态外推成各功能专项验收。

**PlayerInfo main-space v2 当前状态**：下面保留的旧 F/tag/request/quorum/47-Gate/Kimi 数值均为历史专项证据。qualification 闭包修复后的 F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / r2 列车自身仍是 non-deployment train；其实现字节随后被当前 `f01f4b121a4c…` release 包含并进入 formal runtime。B0 仍为 `b0_accepted`，oracle 仍为 `oracle_frozen_for_b0`；本轮 formal smoke 没有启用 PlayerInfo fixture、观察真实 `pi_*` 或执行 PlayerInfo-specific standard-entry E2E，因此不能从总体 runtime 发布状态外推 PlayerInfo 专项验收。
