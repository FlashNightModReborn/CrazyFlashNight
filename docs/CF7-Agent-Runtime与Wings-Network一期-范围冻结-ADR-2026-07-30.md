...
- slot allow-list 固定为 `cf7_agent_equipment_tuning`、`cf7_agent_arena_calibration`、`cf7_agent_character_build`、`cf7_agent_loot_target_full_v1`，每次运行再绑定 exact save path/attempt/build identity；
- runner 独占 Guardian 生命周期并复用现役 fresh/rebuild/batch/recovery 与 readiness gate；`/console` 仍不得承担业务 preview/commit；
- runner 等待 Host 发布 observed credential 的 policy maximum 固定为 **30 秒**，始终以单调 `Stopwatch` 计时；它与 bootstrap request/credential lease 的 10 分钟最大生存期是两个独立上限。Node、PowerShell、CLI/client 都不能配置或延长这 30 秒；等待服从取消，超时固定为 `trusted_runner_credential_timeout`，所有失败都进入 exact-owned Guardian recovery；
...
- 正式 `promoted → standard_entry_verified` 仍需要 immutable request、local X509 与 GitHub hosted OIDC/Sigstore 双 signer/双 faultDomain；单机 candidate 不能替代。
