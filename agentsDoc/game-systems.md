...
**文档角色**：游戏系统 canonical 索引。**最后核对代码基线**：正式 promotion commit `6218f8b1d82efc57b77131616667fe45f3033297`（release tree `e1e6e059f148a640b3e37cbf1d912f297156d956`，2026-07-22 换机标准入口复核）。
...
- **部署层**：在 `C:\cf7-standard-entry-20260722\resources` 从无参 `automation/start.ps1` / 根 bootstrap 启动正式闭包，外层与 Core 自检均为 `manifest verification OK files=23`，实际 Core 进程命中正式 `runtime/`。槽位 `cf7_agent_loot_standard_entry_v3`、attempt `4eae1360cedd413fa3175db6a8997158` 依次取得 fresh handoff、真实 `bootstrap_reveal_ready`（watchdog **0**）、唯一一次 `_root.agentEnterResolvedSave()` 与 exact `s:1|ga:<attempt>` receipt，达到 `readyForRuntimeAutomation=true`、`blockers=[]`；随后单 canary 实箱完成 snapshot、两次 claim、terminal close、PanelHost idle、`generic_unpause delivered=true` 与人工存盘。最终存档 `collection.材料.强化石=209498`，相对基线净增 **35**，SHA-256 `CCDF9BFDD71E03CDD205090F958C187587E5C332BDAC327BC754B0CC9AB0DA6B` 且二次复核稳定。故获批的**单 S1/S2 canary 切片**现为 `standard_entry_verified`，可称该范围已正式部署；不得外推为完整 S1/S2 迁移或 S0 actual-feature 验收。完整 S1/S2 资产、S0 exact fixture 与全局 arbiter 人工门仍保留。冻结边界见 [S0 ADR](../docs/地图资源箱-S0无奖励编排-ADR-2026-07-17.md)、[S1/S2 ADR](../docs/地图资源箱-S1S2真实战利品容器与Web双栏-ADR-2026-07-18.md) 与 [专题治理文档](../docs/地图资源箱-Web战利品工作台与开锁流程-前期调研-2026-07-17.md)。
