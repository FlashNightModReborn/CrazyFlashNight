...
class org.flashNight.arki.item.equipment.PropertyOperators {
     ...
     *
     * 【字符串类型】前缀保留拼接（通用规则）：
     * - 适用于所有 "{前缀}-{后缀}" 格式的字符串（如联弹格式 "横向联弹-普通子弹"）
     * - 原值有连接符"-"时：保留前缀部分，替换后缀部分
     *   例："横向联弹-普通子弹" + "次级穿刺子弹" → "横向联弹-次级穿刺子弹"
     * - 原值无连接符时：直接使用新值（等同于覆盖）
     *   例："普通子弹" + "次级穿刺子弹" → "次级穿刺子弹"
     * - 新值有连接符时：完全覆盖（视为完整格式）
     *   例："横向联弹-普通子弹" + "纵向联弹-穿甲子弹" → "纵向联弹-穿甲子弹"
     * - 注意：无连接符的普通字符串行为与直接覆盖一致，无副作用
     *
     * 【对象类型】递归深度合并：
     * - 只更新指定的键，保留其他键不变
     *
     ...
     * 智能合并字符串，支持前缀保留拼接。
     *
     * 适用于任何 "{前缀}-{后缀}" 格式的字符串，典型应用：
     * - 联弹格式："{联弹类型}-{子弹类型}"，如 "横向联弹-普通子弹"
     * - 其他复合格式："{分类}-{具体类型}"
     *
     * 合并规则：
     * - 新值有连接符：直接使用新值（视为完整格式，完全覆盖）
