...
**文档角色**：启动与运行自动化入口。  
**当前正式列车（2026-08-29，装备调制最终态与 Web 反馈修复）**：release source `cda82e6e4128faf8664235e48b3ed0d84c14a553`、tag `runtime-build-v2/20260829-equipment-tuning-final-state-v2`、release tree `f52fe54c5e292856280343c74a0dd56cdbd2b3c8` 与 request `3A7030BF2E3177039DD012ED5B852AFD01171EA7690C22572DD7956115B6D426` 已由本地 X509 `builder-local-a` / `physical-host-a` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `33227123193`）对 identity `BEA826A1DD33CAB9548B19F328C8C16264EFDD27F2FAA407689B00EF679774B3`、closure `4860E38FD8DA98B7B9A7080D51E986EE1BC9CEB8FC4BB2EEE28B7D6647881556` 达成双 signer / 双 faultDomain 共识。39/39 final receipt SHA-256 为 `46ED9AF8CA18F6472705BBD9B5C66D08447E7D2930F21ED43C57FE5AA3516CF6`；deployment commit `43c8ad6ea23f677bcb2dd7d406e8024b831d93dc` 与 post-promotion audit run `33227550372` 均已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。
正式根 bootstrap `--verify-only`、33-file bundle 与 signed consensus 已通过。维护者已确认等级/容量最终态、不足材料反馈、四入口布局与强化度交换修正版有效，专项状态为 `HUMAN_ACCEPTANCE_PASSED / promoted`；部署后没有从无 candidate 的正式入口重跑装备调制或存档旅程，不称本功能业务 `standard_entry_verified`。首个 v1 request 因材料字典 provenance 过期在本地 38/39 停止，未 dispatch 云构建、未 promotion，现已由 v2 原子 supersede。

**上一正式 runtime 列车（2026-08-28，斗兽星期级全量标定 Gate F；领域 Gate 仍独立进行）**：release source `c64a5440e5506a3f1567143711f984d063e56505`、tag `runtime-build-v2/20260828-arena-calibration-gate-f-v3`、release tree `abe5cd0ef38fff3e596b4bfe4c84ea3c30e5ead8` 与 request `62E4E1537771F0638DB9204A950DD19598962F7E8BBE7D73E2916FAB519982FE` 已由本地 X509 `builder-local-b` / `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `33130111918`）对 identity `48E2ACEA81194C0D6C3A89226DEC2748192612B5514D3F3ADB8444FA4AF6C528`、closure `DA7E5BD135FF2407ED7CE459F521BEA95C9CB6F5CC63AA5291ABAC795DAF59F1` 达成双 signer / 双 faultDomain 共识。39/39 final receipt SHA-256 为 `AC2FE8CA89D3A88C70C383E4C3DF84639DDFA956761793A16D66780634D95738`；deployment commit `693baf7051d9e67be8930b309dc14eea65c0eab6` 与 post-promotion audit run `33130680653` 均已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。
...
**斗兽标定生产目录（2026-09-02，合并后已精确应用）**：`gate-f-week-full-v9` 已闭合 195 个 completed shard / 3,230 条 selected durable row，33 个默认生产 profile 兼容 candidate 去重为 55 个 exact roster；`G2` 两组按真人 PVE 固定为 `Lv10 / arena-2`，`B11/C11/C7` context-only，21 个 timeout candidate provisional，`D10/B12` quarantine。生产目录只接收 Host session-scoped id，不写 faction `benchLevel`。合并后 exact bundle `sha256:6ed3c7461330657f4770abc454b9c6ccdf280c5c2190f347d798c19fa5f21b43` 已在 revision `06e554e762d3bae83ccd20c264c78e7fa74ec92c` 上取得 `APPLIED_VERIFIED`；applied SHA-256 为 `sha256:f9bfb2d2496da9648b5bbe4227c3fb0e5828c3456b16b9115a76456eb98d41e5`，receipt hash 为 `sha256:8f6b152a3289c72e3d20bd8cb723f3d7dcec37a85a747e159280fe4ea6d56a68`，Host 7/7、Arena browser 36/36 与文档治理通过。当前仍是本地状态，未推送、未 promotion 或 `standard_entry_verified`。

**更早正式列车（2026-08-28，双药剂组与八槽共享冷却；历史）**：release source `b2bc05775c621616fe64be55354aebe21c63a2af`、tag `runtime-build-v2/20260827-dual-drug-banks-v1`、release tree `826f37292cb591782e3d3cc41145b9158a426032` 与 request `9FE68E5BEB945B969A2D4CDAA1D20B2D2838A4208DB8773EF7344918B0383658` 已由本地 X509 `builder-local-a` / `physical-host-a` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `33090620311`）对 identity `6C9CF4699C217CC65083038D3AA69B0D6640C4E8DF5A50D367D0628E366D379D`、closure `1D0C3A1272CD084ECC9532B0565E017E07A233C1511B4372C091FF081701252F` 达成双 signer / 双 faultDomain 共识。39/39 final receipt SHA-256 为 `361CAAEDD6322796631112882DB069C5B1C2F6599AF20685F9A20BB719BD6677`；deployment commit `6902b2b6ed067c4882e9a67267d055ce0db90b34` 与 post-promotion audit run `33092179946` 均已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。
