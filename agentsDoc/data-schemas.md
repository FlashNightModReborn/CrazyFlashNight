...
| `data/intelligence/` | 情报详情 legacy txt 文本；AS2 旧界面和 Launcher Web 情报面板共用 |
| `config/` | 系统配置 |
...
| `InformationDictionaryLoader` | `data/dictionaries/information_dictionary.xml` | 情报条目元数据；Launcher Web 情报面板由 C# `IntelligenceTask` 读取同一 XML，并按字典白名单读取 `data/intelligence/<itemName>.txt` |

> 完整列表见 `org/flashNight/gesh/xml/LoadXml/`。另有 `BaseStageXMLLoader`（按路径加载单个关卡 XML）和 `StageXMLLoader`（非单例，支持 CaseSwitch 条件值解析）。

### 情报字典与 legacy txt

`data/dictionaries/information_dictionary.xml` 维护情报条目的名称、排序、分页解锁值、加密等级与替换/截断规则；每个条目的正文固定读取 `data/intelligence/<Name>.txt`。txt 内使用 `@@@PageKey@@@` 分隔分页正文，`PageKey` 必须命中字典中的 `Information PageKey`。

Launcher Web 情报面板不开放 WebView2 对 `data/` 或项目根的 fetch 权限，而是由 C# `IntelligenceTask` 精确命中字典项后读取固定目录文本，并校验最终 full path 仍在 `data/intelligence/` 下。正式 runtime 入口通过 AS2 `intelligenceState` 只回每条情报收集值、解密等级和玩家名，C# 合并本地 catalog 后返回 `state` 小包；Web 点击目录项时再请求 `snapshot(itemName)`，锁定页正文保持空字符串，不一次性下发全部正文。Dev/harness 仍保留 `bundle` 全量包、`catalog` 与显式 `snapshot(value, decryptLevel, pcName)` 兼容路径；`iconName` 来自 `data/items/收集品_情报.xml` 的 `<icon>` 字段，最终图标仍通过 `launcher/web/icons/manifest.json` 解析。目录 hover 的物品注释由 AS2 `intelligenceTooltip` 复用 `TooltipComposer` 返回富文本，Web 只负责定位与 AS2 HTML 兼容转换。
