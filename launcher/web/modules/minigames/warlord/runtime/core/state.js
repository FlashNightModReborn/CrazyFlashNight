...
import { DEMO_1_RUNTIME } from '../data/map.js';
...
function runtimeNodeConfig(bundle, nodeId) {
    const config = bundle.nodeConfigs[nodeId];
    if (!config)
        throw new Error(`Scenario references unknown node ${nodeId}.`);
    return config;
}
function createSlots(bundle, nodeId) {
...
function createFaction(definition, bundle, difficulty, preset, victoryGroupId) {
    ...
    const ownHq = definition.headquartersNodeRef;
    const ownSupply = definition.supplyNodeRef;
    return {
        factionId,
...
function createMapNodes(bundle) {
    const entries = bundle.mapDefinition.nodes.map((definition) => {
        ...
        const ownerFactionId = authoredOwner ?? null;
        return [nodeId, {
                ...config,
                ownerFactionId,
...
function isCardId(value) {
    return CARD_IDS.includes(value);
}
function initialDeployments(bundle, preset) {
    return preset === 'all-units'
        ? bundle.scenario.initialState.allUnitsDeployments
        : bundle.scenario.initialState.standardDeployments;
}
function spawnScenarioPieces(state, preset, bundle) {
    for (const deployment of initialDeployments(bundle, preset)) {
        const factionId = deployment.factionRef;
        if (!state.factions[factionId])
            throw new Error(`Deployment uses unknown faction ${factionId}.`);
        const nodeId = deployment.nodeRef;
        for (const cardValue of deployment.cardIds) {
            if (!isCardId(cardValue))
                throw new Error(`Deployment uses unknown card ${cardValue}.`);
            createPieceInPlace(state, factionId, cardValue, nodeId, 1);
        }
    }
}
function bindScenarioCommandersInPlace(state) {
    for (const commander of Object.values(state.commanders)) {
        if (commander.status !== 'fielded')
            continue;
        ...
        const candidates = Object.values(state.pieces)
            .filter((piece) => (piece.factionId === commander.factionId
        ...
        if (candidates.length !== 1) {
            throw new Error(`Commander ${commander.commanderId} requires exactly one deployed piece at its authored node; found ${candidates.length}.`);
        }
        const candidate = candidates[0];
        if (!candidate)
            throw new Error(`Commander ${commander.commanderId} has no deployed piece.`);
...
export function createGame(options = {}) {
    ...
    const turnOrder = bundle.scenario.turnOrder.map((factionId) => factionId);
    const playerFactionId = bundle.scenario.playerFactionRef;
    ...
    const victoryGroupByFaction = new Map();
    for (const group of Object.values(victoryGroups)) {
        for (const factionId of group.factionIds)
            victoryGroupByFaction.set(factionId, group.victoryGroupId);
    }
    const factions = Object.fromEntries(bundle.scenario.factions.map((definition) => {
        const factionId = definition.id;
        return [factionId, createFaction(definition, bundle, difficulty, preset, victoryGroupByFaction.get(factionId) ?? factionId)];
    }));
