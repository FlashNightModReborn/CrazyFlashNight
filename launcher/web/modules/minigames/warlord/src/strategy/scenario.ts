import type {
  ...
  ScenarioId,
  ScenarioNodeControlDefinitionV1,
  ScenarioNodeRuleDefinitionV1,
...
function rejectUnexpectedFields(
  ...
  for (const key of Object.keys(source).sort()) {
    if (!allowed.has(key)) addIssue(issues, 'unexpected_field', `${path}.${key}`, { field: key });
  }
}
...
function parseDeployments(
  ...
  controlByNode: ReadonlyMap<string, string>,
  issues: StrategyValidationIssue[],
): ScenarioDeploymentDefinitionV1[] {
  const result: ScenarioDeploymentDefinitionV1[] = [];
  for (const [index, raw] of normalizeOneOrMany(value).entries()) {
    ...
    if (issues.length === issueStart && nodeRef !== null && factionRef !== null) {
      result.push(Object.freeze({
        nodeRef: nodeRef as NodeId,
        factionRef: factionRef as FactionId,
...
export function validateWarlordScenario(
  input: unknown,
  map: MapDefinition,
  ...
  const factions: ScenarioFactionDefinitionV1[] = [];
  const factionIds: string[] = [];
  const factionFirstPath = new Map<string, string>();
  for (const [index, raw] of normalizeOneOrMany(input.factions).entries()) {
    ...
    if (id !== null) {
      ...
      if (firstPath !== undefined) addIssue(issues, 'duplicate_id', `${path}.id`, { kind: 'faction', id, firstPath });
      else {
    ...
    if (
      ...
      factions.push(Object.freeze({
        ...
        displayName,
        controller,
        headquartersNodeRef: headquartersNodeRef as NodeId,
        supplyNodeRef: supplyNodeRef as NodeId,
      }));
    }
  }
  ...
  const nodeRules: ScenarioNodeRuleDefinitionV1[] = [];
  const nodeRuleIds: string[] = [];
  const nodeRuleFirstPath = new Map<string, string>();
  for (const [index, raw] of normalizeOneOrMany(input.nodeRules).entries()) {
    ...
    if (nodeRef !== null) {
      ...
      if (firstPath !== undefined) addIssue(issues, 'duplicate_id', `${path}.nodeRef`, { kind: 'node-rule', id: nodeRef, firstPath });
      else {
  ...
  const nodeControls: ScenarioNodeControlDefinitionV1[] = [];
  const controlByNode = new Map<string, string>();
  for (const [index, raw] of normalizeOneOrMany(initialState?.nodeControls).entries()) {
    ...
    if (issues.length === issueStart && nodeRef !== null && factionRef !== null) {
      ...
      nodeControls.push(Object.freeze({ nodeRef: nodeRef as NodeId, factionRef: factionRef as FactionId }));
    }
  }
  const standardDeployments = parseDeployments(
    initialState?.standardDeployments,
    '$.initialState.standardDeployments',
    knownNodeIds,
    knownFactionIds,
    controlByNode,
  ...
  const allUnitsDeployments = parseDeployments(
    initialState?.allUnitsDeployments,
    '$.initialState.allUnitsDeployments',
    knownNodeIds,
    knownFactionIds,
    controlByNode,
  ...
  const definition: WarlordScenarioDefinitionV1 = Object.freeze({
    schemaVersion: 1,
    id: scenarioId as ScenarioId,
    rulesVersion,
    mapRef: mapRef as MapId,
    mapPresentationRef: presentationRef,
    playerFactionRef: playerFactionRef as FactionId,
    turnOrder: Object.freeze(turnOrder as FactionId[]),
