...
**斗兽标定生产目录（2026-09-02，合并后重新封包）**：`gate-f-week-full-v9` 已闭合 195 个 completed shard / 3,230 条 selected durable row，33 个默认生产 profile 兼容 candidate 去重为 55 个 exact roster；`G2` 两组按真人 PVE 固定为 `Lv10 / arena-2`，`B11/C11/C7` context-only，21 个 timeout candidate provisional，`D10/B12` quarantine。生产目录只接收 Host session-scoped id，不写 faction `benchLevel`。合并前批准包 `sha256:fbda67edc9378bc77348900d4b097f614d59f4cb0ccb775bdf5b49c62686d3b5` 的 `APPLIED_VERIFIED` 回执绑定旧 revision；上游整合与 fresh asLoader 发布后必须重新生成并批准 exact bundle，旧回执不得代签最终 push、promotion 或 `standard_entry_verified`。
