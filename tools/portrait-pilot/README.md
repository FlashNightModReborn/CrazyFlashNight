...
若人工框选所绑定的精确 PNG 超过 Pillow 默认像素阈值，但仍位于 manifest 的 `maximumSourceFrameDimension²` 内，使用版本化 [`render-framing-guidance-large-frame-v1.py`](render-framing-guidance-large-frame-v1.py)；若同一行还实际命中已证明的 GIF 二值 alpha 表示差，改用 [`render-framing-guidance-large-frame-fidelity-v1.py`](render-framing-guidance-large-frame-fidelity-v1.py)。两者都必须 `render/check`，只允许 manifest 有界解码；后者还须精确绑定身份、角色、候选和 frame，不能提高全局 MAE。现行 `render-feature-fidelity-v2.py` 进一步把轴向钳到 16384、面积钳到 `min(maximumSourceFrameDimension², 240,000,000)`，并在基础 renderer 的 `.convert()` / `.load()` 前按图像头拒绝超限输入；禁止用 Pillow warning 后再完整解码的方式冒充预解码门。
...
生产写入分三层：`promote-team-portraits-v1.py check` 保留为 98 identity Team 子集门；`promote-enemy-portraits-v1.py promote --replace-existing` 是消费者无关的全量入口；`promote-arena-portrait-supplement-v1.py preflight/promote/check` 只允许在已验证、尚未增量的完整基础 manifest 上追加已冻结的 Arena 尾批，并在最终 closure 内用自包含压缩 archive 精确冻结该基础 manifest/receipt。两条写入口都先完整预验 staging，再按“内容寻址 subjects → receipt → manifest 单文件权威切换 → stale cleanup → shared check”发布；中断时当前 manifest 引用保持完整并可重跑收敛，不再移动 live 目录，也不宣称多文件事务原子。`build-enemy-portrait-evidence-pack-v1.py` 生成并自校验随仓 pack 与 raw sidecar：353 条显式 artifact path + 211 条 digest-bound selected-master 派生记录恰为 564 条，其中 560 条属于 `tmp/portrait-pilot` 生产证据、4 条是 tracked controller provenance；25 条无法由稳定 committed basis 无损重建的 JSON/PNG 以原始字节进入 sidecar。`test-evidence-only-full-build-v1.py` 在 base exact-fileset 清理后清空 pack/materialized cache、切换到 base-cleaned basis root，再构建 supplement，并要求进程内 promotion build/check 路径 564/564 全消费且不触达 live tmp；该测试显式不启动四个历史 verifier 子进程，因为父进程 audit hook 不能覆盖它们。四门历史 verifier 由真实 normal supplement promotion 与各自 standalone check 独立覆盖，不能把这项进程内回归外推成子进程 clean-checkout 证明。runtime checker 另要求 subjects 精确等于 runtime 引用与 17 条显式 `preservedEvidence`（12 条 orientation-only + 5 条 SVG reconstruction basis；后 5 条也被最终 supplement runtime 引用），而 evidence pack 自身的 `preservedSubjects` 只计前 12 条 orientation source。最终 disk/tracked 为 454 个 subject 文件，442 个唯一 runtime 引用与 preserved 声明的去重并集也恰为 454，extra/missing 都为 0；任何真正未声明的 stale/orphan 文件都会被删除或被 checker 拒绝。当前 `cf7.enemy-portrait-manifest.v1` 为 226 identity / 227 variant / 222 human-accepted variant / 2 pending / 1 excluded / 2 aliases，manifest digest `EFDBD928…06E5`、receipt digest `17FC0D9B…0EDF`；`check` 同时验证 444 个 SVG/PNG 绑定（442 个唯一文件）、Team 子集、JK 双变体、方向来源、alias target 与 supplemental closure `C70B03A8…8AD3`（base manifest `EED4D8DC…01D0`）。在没有任何 `tmp/portrait-pilot` provenance 文件的 clean checkout 中，设置 `CF7_PORTRAIT_EVIDENCE_ONLY=1` 后运行通用包与 supplement `check` 是强制 evidence-only 复验门，不能用现存 tmp 文件假绿。
...
增量后不得继续对 r210 运行 `--current`：它绑定的是增量前 controller/manifest，保留为 217 项历史基线；evidence/publisher controller 更新后，r220 也只保留为当时 222 项历史快照。当前方向门使用 `audit-portrait-orientation-propagation-v1.py` 的 supplemental-aware 全量重建；r221 绑定最终 production manifest 与当前三个 promotion controller，闭合 222/222、0 action/SVG/PNG mismatch、0 legacy，report digest `DDC843A4…0576`。
