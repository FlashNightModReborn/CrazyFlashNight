...
_root.技能函数.地震攻击 = function(Z轴攻击范围)
{
	...
	子弹.子弹散射度 = 0;
	子弹.子弹种类 = "近战子弹";
	子弹.子弹威力 = 8 * _parent.内力 + _parent.空手攻击力 * 1 + 3.5 * _parent.内力 * _parent.技能等级;
	if(_parent.hp攻击加成){
		子弹.子弹威力 += _parent.hp攻击加成;
	}
	if(_parent.mp攻击加成){
...
_root.技能函数.龟派气功护盾释放 = function(target:Object, 技能等级:Number):Boolean {
	...
	/**
	 * 迅驰腿护盾释放 - 在每段迅驰腿攻击动画时生成短周期护盾
	 *
	 * 参照龟派气功护盾的循环刷新模式：
	 * - 每段攻击开始时调用，自动替换上一段护盾
	 * - 护盾持续15帧（约0.5秒），覆盖单段攻击窗口
	 * - 容量随段数递增（后段盾更厚）
	 * - 强度基于内力，提供有限的单次伤害格挡
	 *
	 * @param target 目标单位（_parent）
	 ...
	 * @return Boolean 是否成功释放
	 */
_root.技能函数.迅驰腿护盾释放 = function(target:Object, 段数:Number):Boolean {
		// 移除上一段护盾（如果存在）
		if (target.迅驰腿护盾ID != undefined) {
			var 旧护盾ID:Number = target.迅驰腿护盾ID;
			target.shield.removeShieldById(旧护盾ID);
		}

		// === 参数计算 ===
		var 持续帧数:Number = 15;  // 约0.5秒
		var 基础容量:Number = target.hp满血值 * 0.03 + target.内力 * 0.5;
		var 护盾容量:Number = 基础容量 * (1 + 段数 * 0.3);  // 段数越高盾越厚
		var 护盾强度:Number = target.内力 * 0.3;  // 单次最大抵挡伤害

		var 当前护盾ID:Number = _root.护盾函数.添加临时护盾(target, 护盾容量, 护盾强度, 持续帧数, "迅驰腿护盾");

		// 记录护盾ID
		target.迅驰腿护盾ID = 当前护盾ID;
...
_root.技能函数.扭转乾坤恢复 = function(扭转乾坤护盾承伤量:Number, 技能等级:Number):Boolean {
	if(扭转乾坤护盾承伤量 && 技能等级 && _parent.hp > 0){
		...
		HealApplier.applyHpOverflow(_parent, hp恢复量, _parent.hp满血值);
	}
	return true;
}
...
/* ============================================================================
   王之财宝 —— 数值集中区（调用方：容器 技能容器-王之财宝 第 1 帧脚本）
   ----------------------------------------------------------------------------
   要调数值 / 倍率只改这块；容器里只留流程与控制。

   面板       装备面板计算：物品 use + data → 类型 + 面板
              刀 / 手枪 / 长枪 = data.power
              拳套（手部装备） = data.punch + data.defence + data.damage
                                 （拳套自身 data 的三项都算进面板，缺项按 0）
   各流派加成 施术者属性快照：空手攻击力 / 装备刀锋利度加成 / 装备枪械威力加成 / 伤害加成，
              口径同 PlayerInfoProvider.getUnarmedBonus / getMeleeBonus /
              getFirearmBonus / getDamageBonus
   子弹倍率   威力 = (面板 × 面板倍率 + 对应流派加成) × 类型整体倍率
                     × (1 + 等级 × 等级系数) + 伤害加成
              对应流派加成：拳套 = 空手攻击力
                            刀   = 装备刀锋利度加成 + 空手攻击力 × 0.2
                            枪械 = 装备枪械威力加成
              类型整体倍率：拳套 = 拳套整体倍率（1.2，乘在「面板 + 空手」整体上）
                            其它 = 1（不额外乘）
   ============================================================================ */

// ---- 可调倍率表 ----
_root.技能函数.王之财宝数值 = {
	面板倍率:     2,      // 武器面板整体倍率（见下方注）
	拳套整体倍率: 1.2,    // 拳套：(面板 + 空手攻击力) 整体 × 这个
	刀加成倍率:   1,      // 刀流派  ：(装备刀锋利度加成 + 空手攻击力 × 0.2) × 这个
	枪械加成倍率: 1,      // 枪械流派：装备枪械威力加成 × 这个
	等级系数:     0.5     // 技能等级系数：1 + 等级 × 这个
};

// ---- 数字兜底：undefined / 空串 / 非数 → 0（口径同容器 建武器池 里的 取数）----
_root.技能函数.王之财宝取数 = function(v):Number
{
	var n:Number = Number(v);
	return isNaN(n) ? 0 : n;
};

// ---- 装备面板计算：物品 use + 数据 → {类型, 面板}；不是武器 / 零威力返回 null ----
_root.技能函数.王之财宝取面板 = function(用途:String, 数据:Object):Object
{
	if (数据 == undefined) return null;
	var 数 = _root.技能函数.王之财宝取数;
	var 类型:String = "", 面板:Number = NaN;
	if (用途 == "刀")                                      { 类型 = "刀";   面板 = Number(数据.power); }
	else if (用途 == "手枪")                               { 类型 = "手枪"; 面板 = Number(数据.power); }
	else if (用途 == "长枪")                               { 类型 = "长枪"; 面板 = Number(数据.power); }
	else if (用途 == "手部装备" && Number(数据.punch) > 0)
	{
		// 拳套面板 = 拳击 + 防御 + 通用伤害（拳套自身 data 的三项，缺项按 0）
		类型 = "拳套";
		面板 = 数(数据.punch) + 数(数据.defence) + 数(数据.damage);
	}
	else return null;
	if (!(面板 > 0)) return null;                 // 图纸 / 装饰 / 零威力不进池
	return {类型: 类型, 面板: 面板};
};

// ---- 各流派加成快照：施术者 → {空手, 冷兵, 枪械, 伤加, 等级} ----
_root.技能函数.王之财宝取加成 = function(施术者:Object):Object
{
	var 空手:Number = Math.floor(施术者.空手攻击力);
	if (isNaN(空手)) 空手 = 0;
	var 冷兵:Number = 施术者.装备刀锋利度加成 ? 施术者.装备刀锋利度加成 : 0;
	var 枪械:Number = 施术者.装备枪械威力加成 ? 施术者.装备枪械威力加成 : 0;
	var 伤加:Number = Math.floor(施术者.伤害加成 ? 施术者.伤害加成 : 0);
	if (isNaN(伤加)) 伤加 = 0;
	var 等级:Number = (施术者.技能等级 > 0) ? 施术者.技能等级 : 1;
	return {空手: 空手, 冷兵: 冷兵, 枪械: 枪械, 伤加: 伤加, 等级: 等级};
};

// ---- 子弹倍率：一发的威力 ----
// 对应加成按装备类型分流：拳套 → 空手攻击力；
//                          刀   → 冷兵器 + 空手攻击力 × 0.2（刀额外吃两成空手）；
//                          其余（手枪 / 长枪）→ 枪械
// 类型整体倍率只对拳套生效（乘在「面板 + 空手」整体上），刀 / 枪械恒为 1
_root.技能函数.王之财宝算威力 = function(面板:Number, 类型:String, 空手:Number, 冷兵:Number, 枪械:Number, 等级:Number, 伤加:Number):Number
{
	var 配:Object = _root.技能函数.王之财宝数值;
	var 对应加成:Number = 0;
	var 整体倍率:Number = 1;
	if (类型 == "拳套")
	{
	...
	else                    对应加成 = (枪械 ? 枪械 : 0) * 配.枪械加成倍率;
	return (面板 * 配.面板倍率 + 对应加成) * 整体倍率 * (0.5 + (等级 ? 等级 : 1) * 配.等级系数);
};
