...
### 2.5 角色构筑会话、默认入口与 visual-retire 屏障（2026-07-27 工作树）
...
Native HUD 展开区“游戏”行固定七项：战队、平板、战备箱、情报、材料、技能、商城。`MATERIALS` 不复用 Character Build session，而是只在无 Web panel/Character authority 冲突时发 `openMaterialUI`；AS2 设置 `__legacyMaterialOnly=true` 后打开旧物品界面的“材料”帧，技能/个人信息/情报/物品栏四个顶层跳转都必须拒绝，关闭或场景切换先清 flag。发送失败或已有活动面板时 fail-closed 并给出可见提示，不能把旧 Flash UI 叠在 Web 会话上。

当前工作树已实现 B0–B4 的协议、mutation、已穿戴调制、默认入口、材料隔离和关闭/存盘屏障；该表述只说明源码与自动门范围，不代表 candidate 已执行、游戏内 E2E、GUI 人工验收、promotion 或部署。现役英雄 smoke、完整退出/重启回读和七按钮实图仍按角色构筑专题与 [testing-guide.md](testing-guide.md) 验收。
