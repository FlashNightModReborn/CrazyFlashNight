...
//
// 【BuffManager集成 - 保守乘法】
// 增益通过BuffManager的MULT_POSITIVE（正向保守乘法）实现：
//   - 同属性的MULT_POSITIVE buff只取最大值，有效控制数值膨胀
//   - 使用固定ID确保同来源buff替换而非叠加，优化性能
//   - "摇滚速度": 速度乘数buff
//   - "摇滚攻击": 攻击乘数buff
//   - "摇滚防御": 防御乘数buff
// 当前乘数存储在单位的 _摇滚XX乘数 属性中（从1开始，1表示无增益）
//
// 注：韧性增益已移除以降低前期难度
 ...
 *
 * 【效果】
 * 通过BuffManager的MULT_POSITIVE提升单位的移动速度
 *
 * 【实现】
 * 使用固定ID "摇滚速度" 的buff，每次调用累加乘数增量并替换旧buff
 * 当前乘数存储在 单位._摇滚速度乘数 属性中（从1开始）
 * 由于行走Y/跑X/跑Y通过SpeedDeriveInitializer从行走X速度派生，
 * 只需修改行走X速度即可自动影响所有速度
 *
 * 【数值膨胀控制】
 * MULT_POSITIVE语义：同属性的所有正向保守乘法buff只取最大值
 * 即使有多个来源的速度增益，也只有最强的一个生效
 *
 * @param 单位 目标单位（通常是 _parent）
 * @param 乘数增量 每次调用增加的乘数值，默认为0.05（即5%）
 * @param 上限倍率 乘数上限，默认为2（即最多2倍速度）
 */
_root.敌人函数.摇滚速度提升 = function(单位:MovieClip, 乘数增量:Number, 上限倍率:Number):Void {
	if (isNaN(乘数增量)) 乘数增量 = 0.05;
	if (isNaN(上限倍率)) 上限倍率 = 2;

	// 初始化乘数（从1开始，1表示无增益）并记录开始时间
	if (isNaN(单位._摇滚速度乘数)) {
		单位._摇滚速度乘数 = 1;
		单位._摇滚速度开始帧 = _root.帧计时器.当前帧数;
		单位._摇滚速度上次进度 = 0; // 记录上次播报的进度档位
		_root.服务器.发布服务器消息("摇滚速度开始", 单位._name + " 开始叠速度buff");
	}

	// 累加乘数但不超过上限
	if (单位._摇滚速度乘数 < 上限倍率) {
		var 旧乘数:Number = 单位._摇滚速度乘数;
		单位._摇滚速度乘数 = Math.min(单位._摇滚速度乘数 + 乘数增量, 上限倍率);

		// 计算进度（0-10档位，每档10%）
		var 当前进度:Number = Math.floor((单位._摇滚速度乘数 - 1) / (上限倍率 - 1) * 10);

		// 每十分之一进度播报一次
		if (当前进度 > 单位._摇滚速度上次进度) {
			单位._摇滚速度上次进度 = 当前进度;
			var 耗时帧:Number = _root.帧计时器.当前帧数 - 单位._摇滚速度开始帧;
			var 耗时秒:Number = Math.round(耗时帧 / 30 * 10) / 10;
			if (当前进度 >= 10) {
				_root.服务器.发布服务器消息("摇滚速度达到上限", 单位._name + " 达到" + 上限倍率 + "倍速度上限，耗时" + 耗时帧 + "帧(" + 耗时秒 + "秒)");
			} else {
				_root.服务器.发布服务器消息("摇滚速度进度", 单位._name + " 速度" + (当前进度 * 10) + "%，当前" + Math.round(单位._摇滚速度乘数 * 100) / 100 + "倍");
			}
		}

		// 创建并应用buff（同ID自动替换）
		// 使用MULT_POSITIVE确保同属性增益只取最大值
		// 只需修改行走X速度，其他速度通过SpeedDeriveInitializer自动派生
		var podBuff:PodBuff = new PodBuff("行走X速度", BuffCalculationType.MULT_POSITIVE, 单位._摇滚速度乘数);
 ...
 *
 * 【效果】
 * 通过BuffManager的MULT_POSITIVE提升单位的空手攻击力
 *
 * 【实现】
 * 使用固定ID "摇滚攻击" 的buff，每次调用累加乘数增量并替换旧buff
 * 当前乘数存储在 单位._摇滚攻击乘数 属性中（从1开始）
 *
 * 【数值膨胀控制】
 * MULT_POSITIVE语义：同属性的所有正向保守乘法buff只取最大值
 * 即使有多个来源的攻击增益，也只有最强的一个生效
 *
 * @param 单位 目标单位（通常是 _parent）
 * @param 乘数增量 每次调用增加的乘数值，默认为0.1（即10%）
 * @param 上限倍率 乘数上限，默认为5（即最多5倍攻击力）
 */
_root.敌人函数.摇滚攻击提升 = function(单位:MovieClip, 乘数增量:Number, 上限倍率:Number):Void {
	if (isNaN(乘数增量)) 乘数增量 = 0.1;
	if (isNaN(上限倍率)) 上限倍率 = 5;

	// 初始化乘数（从1开始，1表示无增益）并记录开始时间
	if (isNaN(单位._摇滚攻击乘数)) {
		单位._摇滚攻击乘数 = 1;
		单位._摇滚攻击开始帧 = _root.帧计时器.当前帧数;
		单位._摇滚攻击上次进度 = 0;
		_root.服务器.发布服务器消息("摇滚攻击开始", 单位._name + " 开始叠攻击buff");
	}

	// 累加乘数但不超过上限
	if (单位._摇滚攻击乘数 < 上限倍率) {
		单位._摇滚攻击乘数 = Math.min(单位._摇滚攻击乘数 + 乘数增量, 上限倍率);

		// 计算进度（0-10档位，每档10%）
		var 当前进度:Number = Math.floor((单位._摇滚攻击乘数 - 1) / (上限倍率 - 1) * 10);

		// 每十分之一进度播报一次
		if (当前进度 > 单位._摇滚攻击上次进度) {
			单位._摇滚攻击上次进度 = 当前进度;
			var 耗时帧:Number = _root.帧计时器.当前帧数 - 单位._摇滚攻击开始帧;
			var 耗时秒:Number = Math.round(耗时帧 / 30 * 10) / 10;
			if (当前进度 >= 10) {
				_root.服务器.发布服务器消息("摇滚攻击达到上限", 单位._name + " 达到" + 上限倍率 + "倍攻击上限，耗时" + 耗时帧 + "帧(" + 耗时秒 + "秒)");
			} else {
				_root.服务器.发布服务器消息("摇滚攻击进度", 单位._name + " 攻击" + (当前进度 * 10) + "%，当前" + Math.round(单位._摇滚攻击乘数 * 100) / 100 + "倍");
			}
		}

		// 创建并应用buff（同ID自动替换）
		// 使用MULT_POSITIVE确保同属性增益只取最大值
		var podBuff:PodBuff = new PodBuff("空手攻击力", BuffCalculationType.MULT_POSITIVE, 单位._摇滚攻击乘数);
		var metaBuff:MetaBuff = new MetaBuff([podBuff], [], 0);
 ...
 *
 * 【效果】
 * 通过BuffManager的MULT_POSITIVE提升单位的防御力
 *
 * 【实现】
 * 使用固定ID "摇滚防御" 的buff，每次调用累加乘数增量并替换旧buff
 * 当前乘数存储在 单位._摇滚防御乘数 属性中（从1开始）
 *
 * 【数值膨胀控制】
 * MULT_POSITIVE语义：同属性的所有正向保守乘法buff只取最大值
 * 即使有多个来源的防御增益，也只有最强的一个生效
 *
 * @param 单位 目标单位（通常是 _parent）
 * @param 乘数增量 每次调用增加的乘数值，默认为0.05（即5%）
 * @param 上限倍率 乘数上限，默认为10（即最多10倍防御力）
 */
_root.敌人函数.摇滚防御提升 = function(单位:MovieClip, 乘数增量:Number, 上限倍率:Number):Void {
	if (isNaN(乘数增量)) 乘数增量 = 0.05;
	...
	if (isNaN(单位._摇滚防御乘数)) {
		单位._摇滚防御乘数 = 1;
		单位._摇滚防御开始帧 = _root.帧计时器.当前帧数;
		单位._摇滚防御上次进度 = 0;
		_root.服务器.发布服务器消息("摇滚防御开始", 单位._name + " 开始叠防御buff");
	}

	// 累加乘数但不超过上限
	if (单位._摇滚防御乘数 < 上限倍率) {
		单位._摇滚防御乘数 = Math.min(单位._摇滚防御乘数 + 乘数增量, 上限倍率);

		// 计算进度（0-10档位，每档10%）
		var 当前进度:Number = Math.floor((单位._摇滚防御乘数 - 1) / (上限倍率 - 1) * 10);

		// 每十分之一进度播报一次
		if (当前进度 > 单位._摇滚防御上次进度) {
			单位._摇滚防御上次进度 = 当前进度;
			var 耗时帧:Number = _root.帧计时器.当前帧数 - 单位._摇滚防御开始帧;
			var 耗时秒:Number = Math.round(耗时帧 / 30 * 10) / 10;
			if (当前进度 >= 10) {
				_root.服务器.发布服务器消息("摇滚防御达到上限", 单位._name + " 达到" + 上限倍率 + "倍防御上限，耗时" + 耗时帧 + "帧(" + 耗时秒 + "秒)");
			} else {
				_root.服务器.发布服务器消息("摇滚防御进度", 单位._name + " 防御" + (当前进度 * 10) + "%，当前" + Math.round(单位._摇滚防御乘数 * 100) / 100 + "倍");
			}
		}

		// 创建并应用buff（同ID自动替换）
		// 使用MULT_POSITIVE确保同属性增益只取最大值
		var podBuff:PodBuff = new PodBuff("防御力", BuffCalculationType.MULT_POSITIVE, 单位._摇滚防御乘数);
		var metaBuff:MetaBuff = new MetaBuff([podBuff], [], 0);
 ...
 *
 * @param 单位 目标单位（通常是 _parent）
 * @param 攻击乘数增量 攻击力乘数增量，默认0.1（即10%）
 * @param 显示特效 是否显示特效，默认true
 */
_root.敌人函数.摇滚战斗增益 = function(单位:MovieClip, 攻击乘数增量:Number, 显示特效:Boolean):Void {
	if (!单位) return;
	if (isNaN(攻击乘数增量)) 攻击乘数增量 = 0.1;
	if (显示特效 == undefined) 显示特效 = true;

	// 速度提升（乘数增量0.05，上限2倍）
	_root.敌人函数.摇滚速度提升(单位, 0.05, 2);

	// 韧性提升 - 已移除以降低前期难度
	// _root.敌人函数.摇滚韧性提升(单位, 8, 0.2);

	// 攻击力提升
	_root.敌人函数.摇滚攻击提升(单位, 攻击乘数增量, 5);
 ...
 * @param 防御乘数增量 防御力乘数增量，默认0.05（即5%）
 * @param 攻击乘数增量 攻击力乘数增量，默认0.1（即10%）
 * @param 防御上限倍率 防御力乘数上限，默认10
 */
_root.敌人函数.摇滚灯光增幅 = function(单位:MovieClip, 触发概率:Number, 防御乘数增量:Number, 攻击乘数增量:Number, 防御上限倍率:Number):Void {
	if (!单位) return;
	if (isNaN(触发概率)) 触发概率 = 50;
	if (isNaN(防御乘数增量)) 防御乘数增量 = 0.05;
	if (isNaN(攻击乘数增量)) 攻击乘数增量 = 0.1;
	if (isNaN(防御上限倍率)) 防御上限倍率 = 10;

	if (random(100) < 触发概率) {
		_root.敌人函数.摇滚防御提升(单位, 防御乘数增量, 防御上限倍率);
		_root.敌人函数.摇滚攻击提升(单位, 攻击乘数增量, 5);
 ...
 * @param 防御乘数增量 防御力乘数增量，默认0.05（即5%）
 */
_root.敌人函数.吉他攻防增益 = function(单位:MovieClip, 攻击乘数增量:Number, 防御乘数增量:Number):Void {
	if (!单位) return;
	if (isNaN(攻击乘数增量)) 攻击乘数增量 = 0.1;
	if (isNaN(防御乘数增量)) 防御乘数增量 = 0.05;

	_root.敌人函数.摇滚攻击提升(单位, 攻击乘数增量, 5);
	_root.敌人函数.摇滚防御提升(单位, 防御乘数增量, 10);
	_root.效果("灯光增幅", 单位._x, 单位._y, 100);
};
 ...
 *
 * @param 单位 目标单位（通常是 _parent）
 * @param 乘数增量 速度乘数增量，默认0.005（即0.5%）
 */
_root.敌人函数.键盘速度增益 = function(单位:MovieClip, 乘数增量:Number):Void {
	if (!单位) return;
	if (isNaN(乘数增量)) 乘数增量 = 0.005;

	_root.敌人函数.摇滚速度提升(单位, 乘数增量, 2);
	_root.效果("天蓝增幅", 单位._x, 单位._y, 100);
};
