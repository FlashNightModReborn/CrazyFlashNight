...
_root.计算技能点数总和 = function(目标等级:Number):Number {
    ...
    if (索引 < 分界等级点数表.length - 1) {
        ...
        技能点数总和 += 额外技能点数;
    } else if (目标等级 > 70) { // 对于超过 70 级的情况，每升 1 级增加 100 点
        技能点数总和 += (目标等级 - 70) * 100;
    }
    return 技能点数总和;
};
...
_root.主角函数.初始化可用技能 = function() {
    ...
    // 从预处理的等级桶获取可用技能索引
    var 桶:Object = _root.主角函数.技能等级桶;
    var 查询等级:Number = 等级;
    if (查询等级 > _root.主角函数.技能等级桶最大等级) {
        查询等级 = _root.主角函数.技能等级桶最大等级;
    }
    var 基础可用索引:Array = 桶[查询等级];

    // 根据装备过滤技能
    var 可用技能表:Array = [];
    var 基础长度:Number = 基础可用索引.length;
    for (var i:Number = 0; i < 基础长度; i++) {
        var 技能索引:Number = 基础可用索引[i];
        var 技能类型:String = 技能表[技能索引].类型;
        if (!((无刀 && 技能类型 == "刀技") || (无枪 && 技能类型 == "火器"))) {
            可用技能表.push(技能索引);
        }
    }
