...
**Agent Runtime F8 current（2026-07-31）**：source commit `53caabc90941826ddacf626f536b0f473adbf049` / tree `5ac63ec05fbbc9b89aa14f7f0b5ab25698f9742d` 的 exact candidate `c-0f4c92f237ab-98ebd18146-20260731t022411220z-20da007a`（identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C` / closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0` / Core SHA-256 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD`）通过 production policy **26/26**（receipt `tmp/manual-agent-acceptance/runtime-policy-f8-final-53ca-20260731T102324.json`），并以纯 Agent Runtime MCP 完成可见 Launcher、NativeHud 与授权 WebOverlay 面板的人工验收，达到 `e2e_verified / NOT_DEPLOYED`。该轮没有使用 Codex Computer Use、browser、privileged legacy HTTP 或 `input.*`，没有落盘像素或 PNG；观察帧仅在进程内计算 SHA-256 后清零。证据为 report `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.json`、transcript `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.jsonl` 与 residue comparison `tmp/manual-agent-acceptance/residue-compare-f8-final-53ca-attempt1.json`。它只覆盖单显示器、Flash metadata-only 和授权面板纵切，未做 promotion 或标准入口验收；正式 runtime 与上段已 promotion 身份保持不变。
...
F8 current evidence 绑定 source `53caabc90941826ddacf626f536b0f473adbf049` / tree `5ac63ec05fbbc9b89aa14f7f0b5ab25698f9742d` 与 exact candidate `c-0f4c92f237ab-98ebd18146-20260731t022411220z-20da007a`（identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C` / closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0` / Core SHA-256 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD`）。人工验收只通过 MCP `initialize`、`tools/list` 与 `tools/call` 驱动 Agent Runtime：工具清单中不存在 `window.activate`，未调用 `input.*`，Flash 双空 mode 与 pixel `unsupported_for_surface` 均被实测；Launcher、NativeHud、WebOverlay 的 WGC 观察只在内存中散列，`panel.open` receipt 绑定 broker dispatch，最后由 trusted wrapper 走同一 Agent Runtime shutdown 协议关闭 exact owned runtime。全程未使用 Computer Use、browser 或 privileged legacy HTTP，未保存截图/PNG，结束后 residue comparison 为无新增残留。production policy receipt 为 `tmp/manual-agent-acceptance/runtime-policy-f8-final-53ca-20260731T102324.json`；report `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.json`、transcript `tmp/manual-agent-acceptance/agent-runtime-help-20260731T022753Z.jsonl`、residue comparison `tmp/manual-agent-acceptance/residue-compare-f8-final-53ca-attempt1.json`。该单屏纵切达到 `e2e_verified / NOT_DEPLOYED`，不等于 Flash 像素/原生输入、物理双屏、promotion 或 `standard_entry_verified`。
