...
│       ├── inventory-ui.js                页码/排序/树筛选/权威整理 + OwnedInventoryViewShell
...
**面板类型**：
- **kshop**（K 点商城 + 战备箱库存态）: 唯一支持入口为 Launcher `SHOP` → Web Panel；旧 Flash `shopMainMC` 已退役。采购态继续使用 catalog/cart 恢复影子与可靠写 gate；`kshop-views.js` 把目录、购物车、历史待领取和二级结算 DOM 从组合层拆开。新结算先发只读 `checkoutPreview`，由 AS2 重算目录、等级、K 点与 `ItemUtil.require` 容量并铸造单次 `checkoutToken`；`checkoutCommit` 复核后以 `ItemUtil.acquire` 整单直接交付到背包/材料/情报/药剂栏等实际落点，再扣 K 点、清空购物车并强制存盘。余额或容量不足整单不扣款，恰好等额允许购买。新购买不再进入 `_root.商城已购买物品`；旧存档队列继续以 purchased token + claim 单飞自然清空。commit 未知结果只做 `bulkQuery + inventory snapshot` 对账。`item-filter.js` 为 KShop/NPC/库存共用分类模型：宽目录使用单层 drilldown，只替换当前层而不累加祖先行；窄库存通过 `InventoryFilterControl` 的固定高度自绘 trigger 打开绝对定位 popover，隐藏 native select，popup 内只有一个纵向滚动所有者。权威 `filterSpec/filterFacets`、ItemCard 语义槽位、完整/紧凑密度和 `OwnedInventoryViewShell` 边界不变。公共视觉门现固定 **8 个正交样本 / 10 项 conformance**。自动门：Edge harness 70/70、NPC harness 54/54、item-filter model 12/12、视觉矩阵 10/10、Launcher xUnit 803/803、Flash `InventoryPanelServiceTest` 80/80 fresh trace、七阶段构建/125 项资产门；真机门见验证矩阵。
- **workbench**（独立物品工作台）: `web/modules/inventory-workbench.js` 按 Host 固化的严格 profile 重新配置同一 `InventoryCoordinator`：`battlebox`=背包 50 + 战备箱 40（Native HUD `WAREHOUSE` 默认；`CF7_WEB_INVENTORY_WORKBENCH=0` 可回退旧 Flash），`warehouse`=背包 50 + 真实仓库 50（宿舍 `openInventoryWorkbench` → `panel_request` 专用，24 页）。两种 profile 均不请求商城目录、不触发 `shopPanelOpen/Close`；图标 required-assets 门、全屏 anchor、tooltip、树状权威筛选、排序、lease 与事务实现完全共享。`warehouse` 首批开放 Ctrl+单击单件快速转移与“快速存入/快速取出”常驻模式：Web 最多缓存 24 个点击并顺序提交，AS2 `inventoryAutoTransfer` 在完整目标可访问范围执行 merge-first→首空位，不接受 Web 目标槽、不自动交换、目标满仓零写入；回包按当前 `windows` 重铸 lease 而不强制跳页，未发送项可再次点击取消，Esc 先退模式。未知 profile 在 C# 路由拒绝，XFL 不能传任意 containerId。当前全量门为 Edge harness 70/70、Launcher xUnit 803/803、Flash `InventoryPanelServiceTest` 80/80 fresh trace；宿舍场景合集独立 XFL 发布也须确认目标 SWF 刷新。真游戏仍需复核宿舍 Ctrl/常驻连续点击、满仓停队、不同剧情战备箱页数、真实物品转移及重启回读。
- **npcshop**（NPC 金币物品商店）: `web/modules/npcshop-runtime.js` + `web/modules/npcshop.js`；入口与交易权威边界保持不变。公共 `ItemFilter` 从 `majorType/use/actionType/weaponType` 建立互斥树，单 use 自动下钻后继续显示 subtype；目录改为单层 drilldown，返回按钮走 exact path，避免单节点父级被自动弹回。背包复用 inventory-domain Coordinator 权威 facets/filterSpec，并以固定 24px 自绘 trigger + popover 展示层级：不再出现 native select、横纵嵌套滚动条或随深度增长的 chrome。材料/情报仍保持 NPC 商店领域投影。帮助、结算等 `workbench-secondary-page` 从 48px 顶栏下沿完整覆盖双栏；紧凑待售格只保留角标。自动门：数据 35 NPC/833 商品、browser harness 54/54、视觉矩阵 10/10、Launcher xUnit 803/803、Flash `NpcShopPanelServiceTest` 37/37 与 `InventoryPanelServiceTest` 80/80 fresh trace。
