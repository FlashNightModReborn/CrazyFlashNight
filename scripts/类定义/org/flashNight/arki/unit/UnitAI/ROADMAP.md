...
**文档角色**：UnitAI 内部结构、离线仿真与长期演进路线。真实 Flash A/B、参数热调和真实关卡战斗切片的实验契约以 [佣兵 AI 真实战斗标定与关卡试炼](../../../../../../../docs/佣兵AI-真实战斗标定与关卡试炼-设计-2026-07-16.md) 为准。
...
测试层级:
  ...
  L3: 真实 Flash A/B（下一局生效的白名单参数注入 + 预编译 pipeline/profile 选择）
```

L3 明确不建设任意 AS2 源码/函数/class 热替换。结构性策略变更仍正常编译发布；同一 Flash 会话通过每局重建 actor/ActionArbiter 消费新参数，避免频繁重启并保持样本可复现。局中 live mutation 只供人工诊断，发生后该局不得进入统计。

---

## Phase 3: 量化评估 + A/B 测试 [设计已收敛，待分片施工]

### 3.1 A/B 测试基建

施工不再要求 Phase 2 全部结束后一次性启动。按独立切片推进：

1. 斗兽干净场支持固定人形佣兵构筑与 profile/pipeline 消融。
2. 先做策略消融，再对有贡献或疑似失调的模块做粗参数扫描，最后才做小规模组合。
3. DecisionTrace 保留诊断职责；批量统计另采结构化局内聚合，避免解析长日志字符串。
4. 候选通过竞技场同构 A/B 后，再进入真实关卡单 SubStage 战斗切片验证地形与波次泛化。
5. 完整“佣兵代理主角”不属于本 Phase，任务导航与交互语义另立项目。

实验冻结字段、分类口径、热调边界、遥测最小集和止损门统一见上述设计 ADR，本文不复制第二份真源。
...
- 2026-07-16: 接入佣兵 AI 真实战斗标定 ADR；L3 从笼统“游戏内热替换”收敛为下一局白名单参数注入，Phase 3 固定为消融→粗扫→组合→真实 SubStage 泛化验证
