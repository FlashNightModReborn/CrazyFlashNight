...
**最后核对代码基线**：release source commit `95019c7e63492d9cb88c010d1ee06376281c590b`（2026-08-29；tag `runtime-build-v2/20260829-frontdoor-authority-migration-v1`、tree `dba425955faeae279dd083a9ee97ffbb900bf07c`、request `86DC638511BFE275791BB2A2DEAC525F8B2CC51675DDD917C44F07F57C34E9DA`、deployment `fdb4fe30aa911bbac91d60d7c349c217fa71a78c`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `37246F2160E8D13B17A1A2D61B2598594A9484E8C81D02FBBC98FACE97669180` / `github-hosted-windows`（cloud run `33255138023`）已对 identity `DBA565EFC247492C9921C24D664BC896083366346551F3145C993397304C4F4E`、closure `B72DC4721E9062C3FE357259DCD41DB5DC2D74DA0CC524F781E89147FE872801` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `BA36F11CEEB156D5946144D67C665077AC2E072BAFFDF2E4A624892BC6FE0876`，39/39 production receipt SHA-256 为 `FD4C8F62B8E782C9BCB672CC519E1F11955FD33544DC60B2715F854E975EF945`；deployment commit `fdb4fe30aa911bbac91d60d7c349c217fa71a78c` 与首次 post-promotion audit run `33255727460` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车完成主时间轴 82–126 启动前门与角色创建收尾：C#/Web 承担启动、草稿、外观/装备/难度、Markdown 作者与版本记录及场景超时治理，AS2 保留最终一次权威提交与 frame 130 既有读盘网关；槽位显示名、旧 SOL/备份保护及普通读档继续遵循 fail-closed 迁移契约。维护者已确认最终隔离候选实际使用无问题；production policy `39/39`、worktree/index 33-file bundle、signed consensus、原子 promotion、正式根 bootstrap `--verify-only` 与 post-promotion Audit 均通过。
准确状态为启动前门与建角迁移 `HUMAN_ACCEPTANCE_PASSED / promoted`。部署后只执行了根 bootstrap 完整性验证，没有从无 candidate 的正式入口重跑普通读档、新建 durable 后重启读回、重建备份或旧 SOL 保护旅程，因此不称本功能业务 `standard_entry_verified`。斗兽 Gate F campaign 与 Audio H2 继续保持各自独立门，不能由本次通用 runtime promotion 代签。
