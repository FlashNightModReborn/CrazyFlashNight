...
**最后核对代码基线**：正式部署仍冻结在基地理发店 Web-only source commit `7237762fa9b5b89b5cb103c3d73e67ef46b08756`（tag `runtime-build-v2/20260724-p0f-hairdresser-web-only-v1`）、release tree `9a294bee63a621663fdfe72b950f18e777a39586` 与 request `DC44D51855EFDE2D7F3223CA3038008023B36A404BB32064511C962ABF824131`，并由 promotion commit `5c074eb4192d05df6fa58dc940301877a89a1d65` 记录正式部署闭包；发布协议与 B0-03b source-ahead renderer 资格化另核对到 commit `a560bc041ca86036bf27bb01ff2ec4d8ffb66a85` + 2026-07-28 同提交工作树。P0-F F3 关闭时的隔离候选严格为 `e2e_verified / NOT_DEPLOYED`；其后无参 `automation/start.ps1` 的首次 bootstrap attempt `0ebb4f1d1bd94585add33f20538e8ee1` 与重启 bootstrap attempt `fac214115f3743308234c0e995193aca` 验证正式 manifest 并以 `formal_runtime` 启动，维护者完成“蓝色头巾”提交、自动关闭、存盘、完整退出及重启回读，游戏角色与理发店权威快照一致，故当前正式冻结状态为 `standard_entry_verified`。

## 当前迁移状态

runtime v2 的工具、schema、队列、本地 X509 证明、GitHub OIDC/Sigstore 证明、promotion 与 CI 状态机已经完成正式闭环；**仓库当前受控部署已是 manifest/consensus v2**。当前 consensus 的 artifact source `CD8CC49F02C0E243FBF02CAF3DB7A610AD42316D7A742067FB77FC42B285F306`、producer recipe `75723146B70547AD1B87F54A19A806A356501290D1507CBD43952ECB9C5906EE` 与 toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD` 形成 build identity `3F6110809903BF28D08E90F92087A4333E5C604F0AC62994E945775690CD25C6`；payload closure 为 `C16ED23C85DCF0C2B2A321158F2269BED9D45ACC32BACCD428ED51AD4D49E107`，manifest SHA-256 为 `F1D5474E9CEA8EE1A2AC04C17F0359E281357D7B9062B62B4EEAC7EA170FE90B`。production policy `BA495554E0F136BE714464D5036F180C2BB25C607E09B50B9308A328BDE4801D` 的 21/21 receipt SHA-256 为 `2991C42B9D59831802E2CC07287B91CE6B1FC81E92923BDA9FC8EAABF792BA67`；`builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC builder `1E1DC9F870607E5E11583275C0CD877B76181D97E70DDEB0EF84D59CDDB7A669` / `github-hosted-windows`（run `30073314631`）的双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier 全部满足后，于 `2026-07-24T07:01:16.1689195Z` 完成 promotion。
...
当前正式部署对应 source commit `7237762fa9b5b89b5cb103c3d73e67ef46b08756`、build identity `3F6110809903BF28D08E90F92087A4333E5C604F0AC62994E945775690CD25C6`、payload closure `C16ED23C85DCF0C2B2A321158F2269BED9D45ACC32BACCD428ED51AD4D49E107` 与 Core SHA-256 `15E22D10B450A8616766D25F709D761C20B5A773BA60314680060348B7970EC2`，包含基地理发店 Web-only、fail-closed 入口及共享 Host/Web pending 生命周期收敛。正式根入口在 bootstrap attempt `0ebb4f1d1bd94585add33f20538e8ee1` 验证正式 manifest 并以 `formal_runtime` 启动；该 manifest 绑定上述 native 身份，随后业务日志完成权威 commit、自动关闭、重开 fresh snapshot、存盘与 Flash exit 0。完整重启 bootstrap attempt `fac214115f3743308234c0e995193aca` 再次验证同一 manifest、载入正式 runtime 并回读“蓝色头巾”，维护者同时确认游戏角色和面板当前值一致。native identity / closure 只覆盖正式 runtime 文件闭包；Hairdresser Web/Flash 字节及业务闭环另由冻结 source/release tree、production policy、正式入口日志与维护者目视共同绑定。因此 `standard_entry_verified` 是机器证据与人工视觉验收的组合结论，不由 consensus 单独推出。source-only、隔离 candidate 与已 promotion 的正式 Core 仍是三件不同的事；root EXE、`runtime/**`、manifest、builder registry 或 consensus 若变化，Audit 必须转入 strict 并在无匹配 promotion 时失败。
...
tracked registry 继续保留 `builder-local-a` / `physical-host-a` 与 `builder-local-b` / `physical-host-b` 两张公钥；本次 consensus 实际采用 `builder-local-a` 的 keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3` / `physical-host-a` 和 GitHub Actions run `30073314631` 的 OIDC builder identity `1E1DC9F870607E5E11583275C0CD877B76181D97E70DDEB0EF84D59CDDB7A669` / `github-hosted-windows`。任一单独本地票都仍不构成 quorum，也不授权单机 promotion。
