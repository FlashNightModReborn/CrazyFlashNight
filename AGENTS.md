...
**最后核对代码基线**：角色构筑手雷/药剂装入、卸下与获取堆叠语义统一，无目标背包总览，已穿戴强化度交换，仅持有配件默认筛选，以及背包候选调制期间切换已穿戴槽位修复，release source commit `60d672332d31bf7b33f75a9b42a4ac6454fb9ca8`（tag `runtime-build-v2/20260817-character-build-stacking-tuning-v2`）、release tree `52dfe8d392d105594570d20e2e77964f44c474e5`、request `5DF08056E7D8AC114D9F8D6BD7A2381CCCFECA4967BE21FA23C2BF404699A466` 已由本地 X509 `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `32022036259`）完成 v2 双 signer/双故障域 promotion；正式 runtime 绑定 identity `D7D97223C2A145237CAC9A3D9873E0CEFDF979D60FF2E45821C1CF37F2285636`、closure `7CA66AEE59AB4054B2F85B51E621D2AE2BCB4738CBEA92EEBF5FABD5B270E289`、Core SHA-256 `CF715C643FB89FCEB301789B3C8C006C5BA570C3FC09B04490AC4F6F25159303`，promotion 提交 `93449940ff863d7ec6fc2ab80a0540f178f7fa06` 与 post-promotion audit run `32025152299` 均已推送/通过，审计明确输出 `state=promoted`。领域工程证据为 Launcher 全量 3760 pass + 3 explicit opt-in skip、Character Build production controller 1095/1095 + 12/12 + 18/18、tuning browser 三视口各 137/137、strict UI audit 0 error / 0 warning、两轮 fresh Flash focused run 与 publish-only `scripts/asLoader.swf`（Compiler 0/0）；正式根 bootstrap `--verify-only` 为 exit 0。未执行真实玩家存档写后重启旅程，故本列车只称 `promoted`，不称 `e2e_verified` 或 `standard_entry_verified`。此前选关界面工作台 `f0c501fcdf74220a5cb6388f01692db7c96389d2`、NPC 商店 snapshot 数量污染防护 `9df1fb8fdadf1df10c6bcc9b0552ebcfcf16a834`、Web UI 音效语义/audioPrefs `b8565b3681e59dd977d71fb80b1493ffae8fac92`、tooltip/合成链修复 `29ab59fd7832d10d54badc3132a5153646c1fa3d`、面板回包最小门 `704d836f42b70f788db8b5b2151c8ab3dcf77792`、Audio Platform v2 emergency `48c312234ab189b16f9edd2f5f7b0b2e78fc1407`、装备调制属性对比 `167d929a4bc3a85d6d21cc7faa220644eefe01d7`、跨层回包键 `a87be26f1d6e80c0ea8883bbc10e46c895c35799`、竞技场 P4/P5、商城旧档、双栏工作台、F8 与 F7 C1 均只保留为历史分层证据；Audio H2 继续作为专项 `pending`，不进入通用 runtime promotion 门。
...
## Context Packs（按任务最小加载，最后核对 commit `60d672332d31bf7b33f75a9b42a4ac6454fb9ca8`）
