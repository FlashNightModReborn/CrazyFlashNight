...
**最后核对代码基线**：release source commit `b2bc05775c621616fe64be55354aebe21c63a2af`（2026-08-27；implementation `9a85f95e748a986faca8f9a846ab360e2c7900d2`、tag `runtime-build-v2/20260827-dual-drug-banks-v1`、tree `826f37292cb591782e3d3cc41145b9158a426032`、request `9FE68E5BEB945B969A2D4CDAA1D20B2D2838A4208DB8773EF7344918B0383658`、deployment `6902b2b6ed067c4882e9a67267d055ce0db90b34`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `C1BF6E8247E0D943EBD9480A4915504196BE5AB13BD90150E67D2460A06917FB` / `github-hosted-windows`（cloud run `33090620311`）已对 identity `6C9CF4699C217CC65083038D3AA69B0D6640C4E8DF5A50D367D0628E366D379D`、closure `1D0C3A1272CD084ECC9532B0565E017E07A233C1511B4372C091FF081701252F` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `D793647E666423EB73FE78F8E272CDF9A2E5792B9D694259639A6B35DD1624D0`，39/39 production receipt SHA-256 为 `361CAAEDD6322796631112882DB069C5B1C2F6599AF20685F9A20BB719BD6677`；deployment commit `6902b2b6ed067c4882e9a67267d055ce0db90b34` 与首次 post-promotion audit run `33092179946` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车把药剂背包扩为 `2 组 × 4 lane = 8` 个物理槽；`6` 只切换活动组，`7/8/9/0` 继续对应四条共享冷却，同列上下槽不得绕过怪物输出按四血药通道建立的平衡。`drug:switch` 使用独立 3000ms 冷却，成功切换同帧抑制用药并锁存四键到松开；活动组只属于进程内会话，读档、新建或删档清理才复位。Settings 36 键表、Character Build `drugLayout.v=2` 两排四列与 PlayerInfo `○ / × + 1 / 2` 双帧图标均已进入冻结 source。
fresh 门包括 player manual `486+57+55+14`、Character Build `674`、Equipment Tuning `78+170`、Host focused `266/266`、Launcher 全量 `4308 pass + 3 explicit opt-in skip / 4311 total`、Web Character Build `222/222×3`、生产 workbench `1110/1110×3 + 12/12 + 18/18`、Settings `18/18 + visual 116/116`，以及 fresh Compiler `0/0`、32K retry `0`、FFDec、PlayerInfo SWF SHA-256 `63FAD7E1DFCB28E2928FC6B7B7514D355ADDDA0286EFB936C0037FB948C2D764` 与 `scripts/asLoader.swf` SHA-256 `0CEBD6FAE41DE4CDF7DAB26926E0536968609F714432BE7E8403A52B43DF1415`。production policy `39/39`、worktree/index 33-file bundle、signed consensus、原子 promotion 与正式根 bootstrap `--verify-only` 均通过。
维护者已先后确认功能、切换手感及小分辨率 `○ / × + 1 / 2` 图标有效，因此双药剂组准确状态为 `HUMAN_ACCEPTANCE_PASSED / promoted`。无 candidate selector 的正式入口 run `2478a9f25873043318f2402f80105b52` 又以两次 verified status、同一 lifecycle、fresh handoff/reveal、可信 shutdown receipt、Guardian/Flash code 0、存档哈希不变且未触发强制清理完成 identity/lifecycle 窄纵切 `standard_entry_verified`；`businessJourneyExecuted=false`，没有从正式入口重跑八槽、切换、旧档迁移或重启读回，不称本功能业务 `standard_entry_verified`。Audio H2 仍保持独立门。
