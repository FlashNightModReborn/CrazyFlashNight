...
正式 v2 consensus 当前绑定 tag `runtime-build-v2/20260823-pet-lifecycle-transaction-panel-hardening-v2`、request `4A445F1F94DB33E017FAE14FCBC499982F80D305B6F9AD4CFD879864AEB6EF6D`、source commit `416c4441947d40bc27ae3854178e87e2c5cf1ac9`、release tree `71d5955735150e811407c742b3ed3d657c779ac2`。
build identity 为 `036502C3E92B0AC3CF5309E747F0CC1B4B4BCD468443B3BFB2F9F8DB183F3925`，payload closure 为 `9432302F90D34F5AF9282FBF8ACF173B191CCFC80B0D1AE1A7C2CD46AEEFD133`。local signer 位于 `physical-host-b`，不可导出 keyId 为 `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5`；GitHub Actions run `32644169141` 的 OIDC identity `67F6B646F5DF863E6398FB2F80F45A3E3EFC75CCD2C7C811341EF580F6851A59` 提供 `github-hosted-windows` 第二票。
production policy `1BEDD3042CBEB199ACD4126DEBAEFDE69EA039473B5DD79018FBA39EE0078325` 的 38/38 final receipt SHA-256 为 `936320F55201296D542EEA87414A64BB8B3AD0127A4B8013C7C1A039384BE287`；deployment commit `df060eac491c7251b261a3f5626960a8ea18911a` 已由首次 post-promotion audit run `32644777181` 确认为 `state=promoted`、`deploymentChanged=true`，严格 manifest/consensus 为 33 files、2 signers、2 faultDomains。
无 candidate selector 的正式入口还取得 fresh reveal、前后两次 `session.status=verified`、同 lifecycle、strict shutdown receipt、Flash/Guardian exit 0 与零进程残留。该 smoke 只证明正式身份和生命周期；维护者在 promotion 前确认的战宠休息/换图闭环不等于部署后业务标准入口，也不代签本列车其他业务专项 `standard_entry_verified`。cloud workflow 仍只允许 `Crazyfs` / `Flash-Night` 的固定 actor ID 首次 dispatch。
