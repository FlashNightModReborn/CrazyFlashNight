...
**最后核对代码基线**（全局技术栈评估的历史锚点）：commit `9f8f0c225`（2026-04-20）；它不是下文 §3 的 Agent Runtime source baseline。§3 保留 F7 C1 与 F8 协议冻结的历史锚点；当前 P4/P5 formal release source 为 `bf9be8c43b223b84f487464a7e6aa9eb8211630b` / tree `44de7f66421a908723f196718e42867dce601f30`，F8 implementation/release source `53caabc90941826ddacf626f536b0f473adbf049` / `6f3d50a52413c747b05b74be88d6ee46650f4597` 只保留为被当前列车继承的协议底座历史。
...
| 栈 / 组件 | 当前角色 | 收敛原则 |
|----------|----------|----------|
| TypeScript / ClearScript V8 | Launcher 内嵌 `GameInput` DFA 与构建工具链 | 只服务于 `launcher/`，不扩展成新的独立应用层；伤害数字 reducer 不回流 V8 |
...
| V8/Flash 同时维护伤害数字 renderer 或 fallback | 已退役；C# 是唯一 reducer/painter，AS2 只在 `H1` 时产出段事件 |
...
一期 ADR 文件名保留首次冻结日 `2026-07-30`，避免 canonical 路径与链接漂移。F7 documentation-only D1 只引用父 C1 `dd84230a1d262c6478591cae2d11051b7a8aa7b1`；它仍是历史记录，不覆盖 F8 implementation/release source `53caabc90941826ddacf626f536b0f473adbf049` / `6f3d50a52413c747b05b74be88d6ee46650f4597`，也不覆盖当前 P4/P5 formal release source `bf9be8c43b223b84f487464a7e6aa9eb8211630b`。
...
受信 runner 在每次完整/周期 surface refresh 重试 credential publication，以 single-flight 和 teardown barrier 隔离 dispose；Core 内部使用 caller 不可调、按 exact ordinal slot 固定的单调 credential-acquisition 上限：四个历史槽 30 秒，仅 A5 真实标题/固定进档栅栏 60 秒，独立于最长 10 分钟 bootstrap request/session。stdout 只承载 JSONL/MCP protocol 且无秘密；只有 adapter=0、strict receipt、exact child exit 0 且无 forced recovery 时，stderr 才输出一行 ≤16 KiB、只含 schema/runtime/process/Core hash/build identity/payload closure/PID/terminal receipt 的非秘密证据。
...
## 7. SteamOS 安装与更新器长期方向

**最后核对代码基线**：commit `bcab5ac3dbcc32c3665dad1411f6158cd751b5b0`（2026-09-07，本节路由与职责核对；实测每日包身份独立记录）。采用公共更新核心 + Windows / SteamOS 平台适配层，版本清单描述依赖，GUI / CLI / Agent 共用安装逻辑；保留每日包，运行时依赖按需下载并复用，不随每个游戏版本复制一套更新器。该方向待实现，不新增当前生产子栈或通用发布门。

Steam Deck 已完成指定历史包的 .NET / WebView2 安装与混合前门运行，Flash 连接仍阻塞；不能据此声称 SteamOS 可玩，也不据此启动游戏主体重写。实测摘录、中文文件名编码线索、WebView2 分发成本与后续阶段统一见 [Steam Deck 实机验证与更新器长期路线](SteamDeck实机验证与跨平台更新器长期路线-ADR-2026-09-07.md)。
