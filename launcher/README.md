...
Program.Run(args)
   ...
   │
   └─ GameLaunchFlow 状态机（两入口 + prewarm hold）：
        ┌──────────────────────────────────────────────────────┐
        │  A. ready 触发 silent prewarm (_pendingSlot = null):  │
        │     Idle → Spawning → WaitingConnect →               │
        │     WaitingHandshake                                 │
        │     └─ bootstrap_handshake 到达见 _pendingSlot==null:  │
        │        park respond callback → PrewarmHandshakeHeld   │
        │        Flash 已拉起, 但 snapshot 响应挂起等用户选槽     │
        │        (不会自行走到 Embedding / Ready)                │
        │                                                      │
        │  B. start_game / rebuild 进入 (消费 slot):             │
        │     PrewarmHandshakeHeld → flush held handshake resp  │
        │                          → Embedding → WaitingGameReady → Ready
        │     WaitingConnect / WaitingHandshake (握手未到)       │
        │                          → 存 _pendingSlot, 待 handshake 快路径
        │     Idle (未 prewarm 或已 Reset)                       │
        │                          → 冷启 A 路径; handshake 到时走快路径
        │     其他态 → 直接 return (已在启动中, 忽略重复请求)      │
        └──────────────────────────────────────────────────────┘
        ...
        Ready 抵达后 reveal gate (两个独立 wait flag):
          ...
          │     (Flash 帧 81 封面帧; 清 _revealWaitingFlash 时**立即** PostToWeb
          │      {cmd:'flash_ready'} 给前端改跳过按钮样式, 不等 JS reveal)
...
**IIFE 初始化后的消息顺序**：
1. 先 `send({cmd:'list'})` 取槽位 + 偏好（`list_resp` 一次带回 slots 数组 + 5 个 userPrefs 字段）
...
**`flash_ready` 时机**：`bootstrap_reveal_ready` 到达清掉 `_revealWaitingFlash` 的瞬间就广播 `{cmd:'flash_ready'}`，让跳过按钮切"进入游戏"态——哪怕此时 JS 侧片头还没播完、`_revealWaitingJs` 仍 true、panel swap 尚未执行。两 flag 都清空后 `DoPerformReveal()` 才真正做 panel swap（BootstrapPanel 隐藏、FlashHostPanel 显示 + readyWiring + hotkey guard 起）。
