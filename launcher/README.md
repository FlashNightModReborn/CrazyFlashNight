...
> **最后核对代码基线**：commit `11d2e9b245`（2026-07-10）；KShop Step 0–5a / Gate A2–A3 与 Web 游戏 UI 行为边界另核对本轮工作树实现。
...
│       ├── kshop-runtime.js               KShop request mux + save/checkout/claim 写协调 + purchased token
│       ├── inventory-runtime.js           range/window snapshot / owned write gate / transfer·discard·sortAndMerge Coordinator
│       ├── inventory-ui.js                库存窗口页码模型/矩阵快捷键 + 展示排序/权威整理控件
│       ├── kshop.js                       采购态 + 背包↔仓库当前页双模式工作台组合层
...
当前 `Launcher.Tests.csproj` 采用 SDK glob 自动纳入测试源码；2026-07-11 全量执行为 **760/760** 通过，其中 `ShopTaskTests` 覆盖 KShop Host 写 gate、成功载荷校验、对账与防重放，`InventoryTaskTests` 覆盖 lease-bound `tooltip`、三容器 source/target、`filterKey` 严格枚举与三容器 `sortAndMerge` 透传，`LauncherCommandRouterTests` 另守 `WAREHOUSE → workbench` 默认路由与 Flash 回退开关。`SaveMigratorTests` 继续使用代码内 helper 数据；外部 fixture 目前主要集中在 `Fixtures/MapHud/`。
...
| K点商城 / 仓库管理 (旧商城界面 MC 已退役) | `game-ui-behavior.*` + `workbench.js` + `kshop-runtime.js` + `inventory-runtime.js` + `inventory-ui.js` + `kshop.js` | `SHOP` → `kshop`；采购态与背包—真实仓库库存态，1200 槽按 50 分页（24 页矩阵）、displaySort、权威 sortAndMerge、discard 与 claim |
| 战备箱工作台 | `inventory-workbench.js` + 上述共享 inventory/workbench 组件 | 刘海 `WAREHOUSE` → `workbench`（背包—战备箱）；40 槽/页、剧情权威 0..6 页、双向 move/merge/swap，不进入商城生命周期；`CF7_WEB_INVENTORY_WORKBENCH=0` 回退旧 Flash `warehouse` |
...
**面板类型**：
- **kshop**（K 点商城 + Gate A3 完整仓库）: 唯一支持入口为 Launcher `SHOP` → Web Panel；旧 Flash `shopMainMC` 已退役。Gate A1/A2 已由用户验收，Shell/ViewHost/Grid/pointer 与 InteractionBroker/owned transfer 分别标 `gate-a1-accepted` / `gate-a2-accepted`；完整仓库为 `gate-a3-candidate`。顶栏切换采购态与库存态，并在共享 Shell 上显式切换 `data-workbench-skin=shop|inventory`：采购态继承地图面板的低亮黑底、荧光黄绿主线、琥珀信息层、切角与扫描线；库存态继承任务面板的拟物底图、石板控件、宋体标题和白灰功能层，同时保留原版背包的黑底白线框、黄色等级框与紧凑槽阵列。皮肤只覆盖视觉 token，不改变命中、布局和协议 DOM。玩家态另行收敛信息密度：顶栏只留标题、状态点、K 点、模式与关闭；双栏 marker/kicker、协议英语、槽号、`EMPTY/DRAG` 和重复物理范围均不再可见，异常与不可逆后果仍用玩家短文案表达。`inventory-ui.js` 独立承载页码状态派生、页码矩阵/方向键与全局首末/前后页快捷键、当前页展示排序、全局分类筛选、权威整理控件和 owned-slot 展示；它只通过注入回调请求窗口或发出整理意图，不读取 KShop 传输态、不执行 AS2 写入。`kshop.js` 只组合容器、权限、确认模态和结果提示，权威 `setWindow/setFilter/sortAndMerge` 继续归 `InventoryCoordinator`。库存态保留背包 50 槽，仓库按 50 槽窗口覆盖 1200 容量；分类由 AS2 扫描完整容器再按匹配项分页，保留真实物理槽位和 lease。分页保留前后页，并把当前页改为 4×6 的 24 页直接跳转矩阵；工具条只常驻当前页/总页数，物理范围下沉到 title/aria，支持 `PageUp/PageDown`、`Ctrl+Home/End` 与会话内 offset 记忆。背包与仓库均支持 displaySort、分类筛选、确认后的 AS2 权威 sortAndMerge，以及同/跨容器 whole-slot move/merge/swap、discard、claim 刷新。物品格优先玩家可见 `displayname`，数量与强化度用图标角标，装备满强化使用金框，升阶与三个插件槽按独立菱形/档级角色符号表达；occupied 槽 tooltip 按 lease 读取真实实例，选择或拖拽时抑制。每次打开只在新 `bulkQuery` 完整成功后开放商城写入。确定性总门（含独立战备箱工作台）为 Edge harness 54/54、Launcher xUnit 760/760、Flash `InventoryPanelServiceTest` 68/68 fresh trace；视觉用 `--visual=inventory|inventory-pages|battlebox|battlebox-pages|battlebox-real-icons`。真实存档仍须覆盖 InputShield、真实装备投影、24 页仓库整理与重启回读。
- **workbench**（刘海战备箱）: 正式模块为 `web/modules/inventory-workbench.js`，固定挂载背包 50 槽与战备箱 40 槽窗口，不请求商城目录。图标 manifest 已上移为 `Panels.init/open` 的全局 required-assets 门：Overlay 启动即预热，任何生产 Panel 都在其加载尝试完成后才进入 `create/onOpen`，工作台不再自带一套 `Icons.load()` 竞态。工作台的 `#panel-content` 明确 `inset:0`，1024×576 固定画布经 `PanelScale` 使用完整 anchor，不再继承通用 `4% 6%` 卡片内缩。AS2 snapshot 同时返回战备箱物理 `capacity=400` 与剧情 `accessibleCapacity=0..240`；Web 只按后者显示 0..6 页，右栏采用 5×8 图标柜，未解锁返回成功锁定空态。背包与战备箱共用显示排序、分类筛选和权威整理；战备箱整理只原子重写已解锁前缀，锁定保留区逐槽对象引用不变。`WAREHOUSE` 默认打开该 panel；`CF7_WEB_INVENTORY_WORKBENCH=0` 回退旧 Flash `warehouse`。close 为纯 Web cleanup + 通用 unpause，不发 `shopPanelClose`，也不重建 Flash 仓库图标。
