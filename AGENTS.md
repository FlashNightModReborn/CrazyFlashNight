...
- **Launcher 验收术语**：统一使用 `compiled → candidate_built → candidate_executed → e2e_verified → promoted → standard_entry_verified`；`launcher/build.ps1` 最多到 `candidate_built`，候选执行 / E2E 必须绑定实际进程路径、build identity 与 payload closure。只有 `promoted` 后再从 `automation/start.ps1` / 根 bootstrap 标准入口验证到同一正式身份，才可称“已部署 / 正式验收”；完整证据契约见 [testing-guide.md](agentsDoc/testing-guide.md)
