...
**当前裁决状态**：总体为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`；前一个词表示 source freeze 后的正式自动门仍未闭合，后一个词只表示人类签收也是必需条件，绝不表示“只剩人签”。main-space v2 代码/工具预检已经完成：child authoring wrapper 被排除，Flash/C#/Web 均以 1024×576 main content viewport 为基准，C# main-visible tight envelope 已修正为 `282×81@(0,474)`；MP current 保持 source-derived 右锚 `74.55`，maximum 在 authored 左锚 `80.65` 之上增加仅供 C# path rasterizer 匹配 Flash standard-text coverage 的 `+0.30` local phase correction，前景/装饰锚点成对一致；HP Glow/横线效果也已纠正。当前完整 PlayerInfo namespace + `PanelHostHudCompanionTests` + `NativeHudBoundsUnionTests` + `NativeHudTopologyProbeTests` focused 为 180 pass + 3 opt-in skip / 183，`PlayerInfoFrameCompositorTests` 为 9/9，C# opt-in capture 预检 1/1、Web main-space 预检 11/11；当前 source-freeze 前完整 Launcher 回归另为 1,747 pass + 3 opt-in skip / 1,750。中间 clean F 的 B0-05 曾取得 32/32，但随后 prepared-DIB closure 变更已使其失效；当前 prepared-DIB dirty worktree 的 B0-06 取得 48/48，只作冻结前性能诊断。真实 Flash v2 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 已按 1024×576 `main_stage`、`loaded_child_exported_symbol_instance`、wrapper=false 合同捕获 11 case；capture tooling 在 `fe56b2b9…` 取得 `head_index_clean_filter_equal` strict identity，但 4 项 human review 仍全部为空，所以状态只是 `candidate_captured; awaiting_human_review`，不是 `oracle_frozen`。以上仍是**整包 source freeze 前预检/诊断**：该 candidate 须在最终工具字节不变的前提下复验身份，final clean B0-05/B0-06 qualification/full、两份正式 C#/Web/direct A/B、v2 source-freeze/tag/request/双 builder quorum、最终 `k3 + highest-available-thinking` 审计与人类验收均未完成。旧 F/G/quorum/K1 只作 v1 historical；正式 runtime、真实 `pi_*`、promotion 与部署均未改变。
...
- 已完成历史诊断与 v2 candidate capture、未取得 accepted main oracle：B0-01B 第七轮 v1 candidate 已绑定 fresh TestLoader、实际 Debug Player、精确 child、11-case raw/crop 与安全恢复，但它直接加载整个 child 文档，包含 `ty=3` wrapper；状态降为 `standalone_child_diagnostic_captured`。当前 v2 tooling 以 `main_rsl_exported_symbol_equivalent` / wrapper-excluded / 1024×576 合同完成真实 11-case capture，tool strict identity 为 `head_index_clean_filter_equal`；但 human review 四项均为空，整包 source freeze 与正式视觉 A/B 也未完成，当前只能写 `candidate_captured; awaiting_human_review`，不得写成 `oracle_frozen`。
...
- v1 历史完成、v2 待重验：B0-05 的 typed runtime manifest、viewport-height/576 物理 scale、原子 8 logical-layer / 10 owned-PArgb-payload batch、单 worker latest-wins、16 MiB active+inactive whole-batch LRU、100 churn/3000 current-hit 与 `split_required` 裁决仍保留；但旧 qualification 绑定的是错 placement/旧 asset bytes。main-space v2 已把 raw canonical envelope 修正为 `285×81@(-3,474)`、main-visible envelope 修正为 `282×81@(0,474)`，并把 fresh-bake 预热合同改为同执行路径的固定 16 轮 round-robin excluded warmup（64 个样本）后再取每 viewport 20 个 acceptance 样本；fresh B0-05 formal qualification 尚未生成。
- v1 历史完成、v2 自动资格待重验：独立 split surface、fixture widget、程序化 HP/MP、MP 双 fragment、HP 固定轮廓/旋转渐变、path glyph atlas 与 PanelHost 生命周期结构仍保留。clean `bf8dd2c…8479` 的 47/47、旧 C#/Web/Flash 视觉包和 K1 PASS 只证明 v1。main-space v2 已完成 compositor/capture/comparator 代码预检与当前 180+3 focused/topology 复核；source-freeze 前 full 与 dirty B0-06 虽已通过，final clean qualification/full、正式三方视觉证据与最终 K1 仍未完成。
...
**Oracle 轨**：第七轮 Flash candidate 仅保留为 standalone child 历史诊断，不能再进入 accepted review。v2 TestLoader/runner 从精确 child 中隔离 exported-symbol instance，排除 `ty=3` wrapper，以 identity root + main `ty=512` 生成 1024×576 `main_rsl_exported_symbol_equivalent` capture；manifest/receipt 均为 v2，历史 v1 fail-closed。真实 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 已完成 11 case，tool strict identity 通过；下一步是在整包 source freeze 后复验 tooling 字节不变，并完成四个 manifest check（状态、层隔离、crop、审美）及独立来源身份现场复核，形成 review receipt。闭合前只能保持 `candidate_captured; awaiting_human_review`，不得写成 `oracle_frozen`。

**工程轨**：B0-05 的 raster/cache/PArgb 与 `split_required` 架构裁决仍有效。main-space v2 已修正 child-stage 非裁剪语义、HP/MP placement、MP 文字锚点/底字、HP Glow 与横线效果，并把 C#/Web/Flash capture/comparator 统一到 main content viewport。中间 clean F 的 B0-05 32/32 已被后续 prepared-DIB closure 变化 supersede；当前 prepared-DIB dirty worktree 的 B0-06 48/48 与 focused/full 通过都只是冻结前诊断，不是最终 source-bound evidence。下一步依次为 v2 source freeze、复验既有 Flash v2 candidate 身份、两份独立 C#/Web capture、direct comparator/诊断 viewer、final clean B0-05/B0-06/full tests、新 tag/request/双 builder quorum与最终 `k3 + highest-available-thinking`。这些自动门完成后仍须人工 oracle/game composite/真实手点/关键比例与平滑/混合 DPI/运行时 SWF 身份/审美签收。
...
tools/player-info-hud/
  ...
  evidence/
    ...
    b0-06/
      ...
      runtime-qualification.json          # current machine qualification; committed copy is historical v1 until v2 freeze
      visual-evidence.json                # current diagnostic index; committed copy is historical v1 until v2 freeze
...
- 本轮 v2 在正式重跑前先修正 B0-05 测量合同：固定执行 16 轮 round-robin excluded warmup，共 4 viewport × 16 = 64 个完整样本；它与 acceptance 使用同一 `Task.Run`、round-robin、fresh SVG parse+raster+PArgb `Bake` 路径，不允许自适应提前停或改变进程优先级。runner 在启动任何 dotnet host 前拒绝会改变 JIT/GC 路径的已登记 `DOTNET_*` / `COMPlus_*` 覆盖，并由报告与脚本双向核验 Normal priority、继承的 affinity mask、server-GC 状态；当前合同共 32 Gate。随后仍为每 viewport 20 个独立 acceptance 样本、nearest-rank p95 `≤100 ms`，所有 64 个 excluded 样本也写入报告供 runner 独立重算。parse/raster 计数是完成的 StrictSvg/Skia payload-slot 操作总数，不宣称 PArgb copy completion 或逐 logical-layer 归因；独立 PArgb copy 诊断只为每个逻辑层复制一个代表性 payload，`mp.fill` 两个 fragment 不在该诊断中。中间 clean F=`48b1f524…` 的报告 run `db0527d4…c7fb` 为 739,608 B、SHA-256 `924BBBA1C6AB00A81F3AA66461165C208AC605BC8960595F73989A01F2A672F2`，32/32、`repositoryDirty=false`、`closureDirty=false`、`recordedDecision=split_required`；但其后 prepared-DIB 修改触及该 37-file closure 内的 `LayeredWindowCommit.cs`、`OverlayBase.cs` 与 `LayeredWindowCommitTests.cs`，故它已被 supersede，final clean source freeze 必须重跑，不能写作 current qualification。
...
| Computer Use/Browser 控制后端不可用，或把 authoring 窗口标题/历史截图误当成已运行 SWF | workspace/terminal full access 与 Apps 控制面分开记录；`os error 2` / Browser `[]` 只作切换 ChatGPT App 前的历史诊断。App 已重新暴露 Computer Use/Browser skill，但须以本轮实际初始化结果为当前事实。`Get-Process` 只作进程/标题诊断；FFDec/无头 Edge 只作静态或 Web/SVG 诊断，实际 HWND/ULW 由专用 STA qualification 测，运行身份仍以 loader/player/SWF trace 为准，真实 composite、手点、遮挡与审美必须人签 |
...
- [ ] v1 两类正式 builder/F/quorum 事实仍按历史保留，但 main-space v2 尚未形成新的 source commit/tag/request、双 signer/双 faultDomain 一致 build identity/payload/Core 与不可复用 promotion preflight。cloud config 已预留全新单段 source ref `refs/tags/runtime-build-v2/20260729-player-info-nativehud-b0-v2`，但 tag 当前不存在；它只能在 final clean F 上创建一次，旧 v1 tag 永不移动/复用。新报告仍须明确零 runtime/release-state/promotion/deployment 变更；旧 `7687B561…7A9F` / `AF2E9E57…724B` / `A45471CF…B652` 不得代签。
...
- [ ] B0-05 raster/cache 合同保留，但 main-space v2 须重跑专用 qualification，重新证明 8-field batch key（含 `sourceToBitmapIdentity`）、8 logical layer / 10 owned PArgb payload、固定 16 轮同路径 excluded warmup、3000 current-key 的 parse/raster/publish delta 和新 source closure。
...
- [ ] fixture HP/MP 的 11-case main-space v2 正式视觉包尚未冻结。C# 1/1 与 Web 11/11 只是预检；仍须两份独立 C# capture、两份 Web capture、真实 Flash v2 candidate、两份 direct comparator、MP 四字段 unique `bestDx=0` + 对应 anchor edge `dx=0`、viewer 与最终 `visual-evidence.json`。旧 C# 36-file/Web 12-file/direct 66-PNG/FFDec/Flash closure 全属 v1 historical。
...
| B0-05 | `v1_historical_qualified; main_space_v2_requalification_pending; split_required` | raster/cache/PArgb/LRU 架构与 split 裁决保留；v1 31/31 报告继续作历史。v2 focused 已固定 raw envelope `285×81@(-3,474)`、visible tight `282×81@(0,474)` 与四 viewport bounds；runner 已纠正为 8 logical layer / 10 owned payload、8-field key、固定 16 轮同路径 excluded warmup与 32 Gate，并冻结 JIT/GC override、Normal priority、继承 affinity/server-GC；parse/raster 只作 aggregate payload-slot 计数，代表性 PArgb copy 不含两个 `mp.fill` fragment。中间 clean F 32/32 曾通过，但其 closure 后续被 prepared-DIB 修改，现已 supersede | final source closure 下的专用 qualification、性能/cache/dispose/topology structured report；v1、被 supersede 的中间 clean F 与 dirty 诊断均不批准最终 v2 | source-freeze 后运行 `run-b0-05-runtime-qualification.ps1`，回写新 run/hash/计数；不回并现有 NativeHud union |
| B0-06 | `v1_historical_qualified; main_space_v2_preflight_passed; formal_requalification_pending` | v1 的 47/47、旧视觉 A/B、F/quorum/preflight 继续证明旧字节。v2 已完成 placement、MP current 右锚、maximum `+0.30` local renderer phase correction、前景/装饰锚点关系、`00000`、HP two-pass Glow/横线 active，以及按真实 anchor edge 收紧的 main-space C#/Web/Flash tooling；split surface 已改为复用 prepared top-down PArgb DIB/DC，B0-06 sourceTrace 同时绑定该 helper，formal runner 复用 B0-05 的 exact JIT/GC override、Normal priority、非零继承 affinity 与 server-GC=false 合同；真实 Flash v2 candidate 已捕获但待人审；当前完整 PlayerInfo/PanelHost/union/topology focused 为 180+3，compositor 9/9，source-freeze 前 full 为 1747+3/1750；prepared-DIB dirty 诊断 48/48，但都不构成 final source-bound evidence；不可变 v1 tag 永不移动 | final clean full/48-Gate、两份 C#/Web/direct/viewer、新 source freeze/tag/request/双 builder quorum/preflight、既有 Flash v2 candidate 的最终身份复验，以及全部人工桌面项；旧结果不能批准新字节 | 先 source-freeze 并复验 Flash candidate/tooling identity，再依次重跑 qualification/视觉/full/builder；新版本不得复用或移动 v1 tag |
...
> v1 工程轨曾冻结 F=`cb38600…f511` 与不可变 tag，并由 clean F Lane、两个正式 builder、production receipt、`-VerifyOnly` preflight 和 clean-G K1 证明旧字节；这些事实保持历史有效，但不得外推到 main-space v2，也不得移动/删除 v1 tag。当前 v2 已完成源冻结前实现/工具预检：standalone child root 仍为 `ty=3`/HP-MP `5.65/-1.3`，main RSL exported-symbol root 为 `ty=0`/HP-MP `2.65/-4.3`；capture tooling 采用 `main_rsl_exported_symbol_equivalent`、wrapper=false、main `ty=512`、1024×576、manifest/receipt v2 与 `CLEAR`，真实 11-case Flash candidate 已捕获且 strict tooling identity 通过，但 human review 四项仍为空。恢复时先运行 B0-01A verifier 与 focused tests，形成包含 capture tooling/资产/compositor/comparator 的 source commit；再复验既有 candidate 的 `strictToolIdentity` 与 source closure，随后生成两份 C#/Web、两份 direct/viewer、final clean B0-05/B0-06/full、new tag/request/quorum/preflight 与完整 `k3 + highest-available-thinking` 审计，最后才交付旧 Flash 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实桌面手点、运行时 SWF/IDE Output 身份与审美人签。当前总体状态为 `visual_correction_in_progress; human_exit_blocked`；B0 不要求 promotion、标准入口或部署，全部自动门与人签完成前不得写成 parity、`awaiting_human_acceptance` 或 `b0_accepted`。每完成一片都在同一提交回写 §12。
