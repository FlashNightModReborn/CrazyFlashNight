...
**最后核对代码基线**：B0-06 实现冻结 commit `0215c03b4594221f02e993bf8d58dd1bc669e815`（2026-07-29）；该提交已包含此前对 `origin/main` `ef31b39069272304f5831b9028b786ce333aa62e` 的集中整合。收尾时重新 `fetch` 后确认 `origin/main` 仍是该基线祖先、ahead/behind=`18/0`，`git merge --ff-only origin/main` 为 no-op；客户端截图中的待拉取计数不是本工作树的实际 Git 状态。B0-00 审计入口的历史工作树只含“新增本文 + 同步总体迁移 ADR”两项文档改动；B0-01A r3 由 verifier 分别绑定 `HEAD`、index、clean-filter worktree 与 source-binary anchor；B0-01B r4-r11 固化恢复安全、实际 runner/parser 的 Git-canonical 自身份、注册 CS6 Debug Player、无引号相对启动、跨完整性级别 authoring 身份与 `Stage.align` requested/reported 语义。第七轮已从该 strict 基线形成真实 Flash candidate；B0-04 的资产/证据身份、B0-03b 的新鲜 v2 candidate/production Gate、B0-05 与 B0-06 的机器资格报告只以 §12 和结构化报告为稳定锚，过期临时产物不得回写，且任一 B0 candidate 都不构成正式 runtime。B0-06 最终自动证据绑定上述实现基线、32-file sourceTrace closure 与执行二进制；承载证据/文档的后继提交必须只改证据与文档，不能借此把未重跑的代码变更混入。

**当前裁决状态**：高层方案已接受；`Svg.Skia 5.1.1` 已由 HUD-SVG-005 接受并锁定为 B0 受控子集的生产 renderer。B0-01A 已以 Git-canonical r3 证据达到 `placement_closure_frozen`。B0-01B 第七轮由 CS6 15 秒生成 389,160 B fresh TestLoader SWF，并由注册 Debug Player `11,2,202,228` 实际加载 SHA-256 `450b1f…a615` 的玩家信息 child；11 个 1024×64 raw、11 个紧边 crop、4,458,323 B 精确 run block 与 manifest `dcc9e11a…bf472` 均已形成 candidate，事务精确恢复，但人工来源/层/crop/审美复核仍为空，所以尚未达到 `oracle_frozen`。B0-02 strict corpus 已扩至 12/12、78 项 fail-closed（其中 58 项值级 grammar），并对当前 8/8 canonical SVG 结构验证通过；其历史状态仍是 `isolated_qualification_passed`，`rendererQualified=false`，不得回写。B0-04 已产生 8 个 SVG（99,564 B）、runtime manifest 与 repo-only provenance；asset-set revision 为 `sha256:c8f58cb7…93871`，manifest SHA-256 为 `1006f90e…5630`。仓内 Playwright + Edge 的两次 11-case Web 输出闭包 A↔B 12/12 文件逐字节一致；FFDec/Web 与 Flash/Web 比较器的两次输出闭包分别 A↔B 34/34 文件逐字节一致。该确定性不表示跨 renderer 像素一致，两类比较均未设置接受阈值，也不替代 Flash Player 或人工验收。B0-03b 已以 fresh v2 candidate `c-c571959c1f2a-08846e81b3-20260728t130535410z-0b23fc7a` 闭合生产 PackageReference、共享 strict facade、9 项 embedded resource、actual renderer/deps closure、runtime identity/policy/notice、packer 与全量回归，达到 `renderer_qualified`。B0-05 又完成 typed manifest、物理 scale planner、8-layer atomic raster、PArgb bridge、单 worker latest-wins/16 MiB whole-batch LRU、可观察 dispose，以及 fixed-bounds topology 预检；结构化报告为 `passed / synthetic_fixed_bounds`。紧 PlayerInfo `282×46` 与右上 `252×32` 仍会桥接成 `982×564`、占 viewport 93.90%、面积放大 26.33×，故 HUD-SVG-010 裁决为 `split_required`。B0-06 已实现 fixture-only visual state/animation、程序化 HP/MP 合成、MP 双 fragment、HP 固定轮廓与旋转渐变、source-bound path glyph atlas、独立 click-through layered surface、PanelHost suspend/resume 与启动/关闭 drain；该 surface 只由 allowlist 环境开关显式启用，不注册进现有 `NativeHudOverlay`，旧 Flash HUD 未被隐藏或修改。最终实现基线的 formal qualification 为 `passed`、47/47 Gate，run `c53cd48cd1054f6e9b988d11f5f06c85`、647,221 B、SHA-256 `E41F8CFC…FBFAB`；严格 warmup 在第 3 个 100-cycle group 首次收敛，紧邻独立 100-cycle acceptance 的 process/GDI/USER delta=`-3/0/0`。两次 C# 36-file、Web 12-file 与 direct-edge 66-PNG 输出闭包分别逐字节一致，索引为 `visual-evidence.json`，状态仍只到 `final_tree_diagnostic_awaiting_human_review`。全量 Launcher 回归为 1,735 pass + 3 opt-in skip / 1,738，Runtime Lane C 为 11/11 exit 0、scalar 566，guardrails 另为 `scripts=3 / unsafeCandidateCases=3`。真实游戏 composite、真实手点透传与审美仍待人类。B0-03b candidate 严格为 `candidate_built / NOT_DEPLOYED`；同步纳入的上游正式 runtime 已推进为 build identity `3F7887AD…F7B44`、payload closure `A9B142A4…022F2` 与 Core `DADE2CC2…CCA1`，但该正式闭包不包含本 PlayerInfo source-ahead candidate。B0 Exit 仍需承载最终证据/文档的 closeout containing commit 的两正式 builder/quorum 与最终 Kimi k3/max 对抗，但不要求 promotion 或部署；真实 `pi_*` 也未由本片接入。
...
- 已完成自动资格、尚未人工退出：B0-06 独立 split surface、fixture widget、程序化 HP/MP、MP 双 fragment、HP 固定轮廓/旋转渐变、path glyph atlas、PanelHost 生命周期与 deterministic C# capture/三方 direct-edge 诊断工具。最终实现基线 formal qualification 已通过 47/47，C#/Web A/B 与 comparison PNG 闭包也由 [`runtime-qualification.json`](../tools/player-info-hud/evidence/b0-06/runtime-qualification.json) / [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json) 冻结；它们不声称 parity、真实 UiData、游戏 composite、手点或人工视觉结论。
...
- MP fill 在 bake 时按 manifest 的两个 clip binding 生成独立、不可变、由 batch 所有的 PArgb fragment；合成器按虚拟帧建立 mask。HP 把固定覆盖轮廓与 `hp-fill-gradient-0003` 坐标旋转分开，不能旋转轮廓。数字使用由 XFL/SWF 来源与生成器身份约束的 Aero/LCD Std path glyph atlas，不读取机器字体，不新增运行时字体依赖。effect policy 以 typed disposition 固定为一个 B0 `ImplementedActive` 层（动态文字与 Glow）和两个 `DeferredB3` HP 层；`PlayerInfoCompositionRecipe` 只冻结当前七个文字布局与 HP paint source，compositor 对 disposition/ID 漂移 fail-closed，不建立通用效果图。
...
- 当前 computer-use connector 在 2026-07-29 会话初始化时因 native pipe 缺失（Windows error 2）不可用。该环境故障不削弱 headless Edge/Playwright、实际 HWND/ULW qualification 或结构化 capture，但它也不能被自动化截图“补算”为真实手点、实际游戏 composite 或审美接受；后三项必须由人类完成。
...
- 最终实现基线报告 [`runtime-qualification.json`](../tools/player-info-hud/evidence/b0-06/runtime-qualification.json) 为 canonical LF、无 BOM，绑定 base commit `0215c03b4594221f02e993bf8d58dd1bc669e815`、32-file sourceTrace Git blob closure、exact SDK 10.0.300 与执行二进制；run `c53cd48cd1054f6e9b988d11f5f06c85`、647,221 B、SHA-256 `E41F8CFC6D72DDA96D16E0AF25533A52A2B5022E684282AFDF2B9613D75FBFAB`，47/47、0 failure。三个 warmup group 的 process/GDI/USER endpoint delta 依次为 `+37/0/+1`、`+1/0/+1`、`-8/0/0`，第三组首次满足完整 envelope；紧邻 acceptance group 为 `-3/0/0` 且三类趋势全 false。NativeHud hidden/enabled commit p95=`7.3670/7.2699 ms`、相对变化 `-1.3180%`；split surface/request/paint/observer-commit/ULW-only p95=`3.6748/0.0590/2.2765/1.1382/0.2028 ms`；3000 visible 的 request/paint/commit 均为 3000、ULW 失败为 0，3000 idle 均为 0。报告的 `oldFlashHudVisibilityObserved=false` 只表示自动 runner 没有观察旧 HUD，不能代签真实同屏可见性。
...
正式收尾在两个独立空目录各运行一次 C# capture，并与两份 Web capture 分别形成 direct C#↔Web、C#↔Flash 诊断；比较器只输出 RGBA8/固定深色背景指标、overlay/diff/side-by-side 与输入闭包，不设阈值、不计算 Web↔Flash 边、不作传递推断。pre-origin-sync `visual-evidence-premerge-v3.json` 继续作为历史输入保留。最终索引 [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json) 绑定 base commit `0215c03b4594221f02e993bf8d58dd1bc669e815`：C# A/B 各 36 文件、796,806 B，完整 directory closure 均为 `53F019EB…47000`，manifest 均为 162,661 B / `16B78FB3…66855`，35-PNG 子闭包为 634,145 B / `0263EFF7…F3D4`；Web A/B 各 12 文件、120,280 B，完整 closure 均为 `2608142C…D1388`，report 均为 5,485 B / `C6B6C8E7…BD38`；两次 direct-edge 各 66 PNG，排除自引用 report 的输出 closure 均为 1,449,236 B / `87EA77C8…E9668`。两份 comparison report 因如实保留 `-a/-b` 输入路径而分别为 183,816 B / `963A16BD…2D2F6` 与 183,816 B / `23D012E6…3097D`，其余 66 PNG 逐字节一致。该状态仍为 `final_tree_diagnostic_awaiting_human_review`；Flash candidate 仍为未签 oracle，人类还必须查看透明 crop、真实游戏 composite、关键比例/平滑与手工鼠标透传。
...
依赖关系：B0-01A、B0-02、B0-03a 在 B0-00 后可并行；B0-01B 等 B0-01A，但可与 B0-02 并行。B0-04 明确依赖 B0-01A + B0-02 后方可开始；其 `canonical_asset_candidate_validated` 结构入口足以启动 B0-03b，也足以在 B0-03b 后启动 B0-05。B0-01B 的 accepted oracle/人工签收只阻断 B0-04 视觉 Exit、B0-06 parity 结论和最终 B0 接受，不阻断 B0-03b/05 的无关结构施工。B0-06 已闭合 MP fill 独立 mask fragment、HP 固定轮廓/旋转渐变、path glyph atlas、独立 surface、正式 qualification 与双份视觉确定性；其最终退出仍依赖承载最终证据/文档的 closeout containing commit 的双 builder/quorum、B0-04 人工 oracle Exit、真实游戏 composite/手点、Kimi K1 和最终人类视觉/UI签收。
...
| path glyph atlas 的 source SWF、FFDec 导出字节或 generator 身份漂移，机器字体又会引入版本与许可差异 | 已从 source-bound child SWF 经 FFDec 21.1.1 导出 LCD Std/Aero，生成 `0-9 / %MP-` 的最小 path/advance/baseline atlas；Generated.cs 为 10,331 B、SHA-256 `B15CA80E…5D66`，provenance SHA-256 `9486BEEB…DC3D`。runtime 不分发 TTF、不使用系统/GDI font；任何源 SWF/字体导出/generator 改变都必须重生成并重跑 closure Gate |
...
| 当前 computer-use native pipe 不可用，自动化误把无头截图当真实桌面交互 | 无头 Edge 只做 Web/SVG 与 direct-edge 诊断，实际 HWND/ULW 由专用 STA qualification 测；真实游戏 composite、真实手点透传、层级遮挡与审美必须由人类在桌面环境验收，connector 恢复也不能代替签收 |
...
- [ ] 两类正式 builder 可 locked restore，并对同一冻结源形成一致 build identity / payload closure；最终以同一 production promotion 验证链的 `-VerifyOnly` 报告留证，不执行 promotion/deployment，也不把报告回灌为发布输入。
...
- [x] fixture HP/MP 静态视觉矩阵与 41-tick HP full→empty 已由最终实现基线的两份独立 C# capture、两份 Web capture和两份 direct-edge comparison 绑定；deferred effect 有显式清单，代码未隐藏旧 HUD。`visual-evidence.json` 证明 C# 36-file、Web 12-file 与 direct 66-PNG A/B 各自逐字节一致；该勾选只关闭自动确定性，不代表旧 HUD 同屏可见、跨 renderer parity 或人工审美。
...
- [x] `launcher/README.md`、testing guide、runtime build canonical doc、总体迁移 ADR 与本台账已同步最终实现基线结果；当前 evidence/docs-only 候选工作树的 doc governance 与 `git diff --check` 均通过。
- [ ] evidence/docs-only 后继提交已形成，两份最终 JSON 已被 Git 跟踪，且提交级 scope / clean-tree / doc governance / `git diff --check` 已复核。
...
| B0-06 | `qualification_final_tree_passed; automated_visual_frozen; human_exit_pending` | 已实现单表驱动的 11-case `PlayerInfoFixtureInput`、surface 独占的 visual state/41-tick HP 式动画、getter-only widget seam、MP 双 fragment、HP 固定轮廓/旋转渐变、Aero/LCD Std path glyph atlas、typed active/deferred effect policy、窄 composition recipe、恒 false hit-test widget、带可靠 resume pending 的独立 click-through `PlayerInfoSplitSurface`、PanelHost companion 与 Program allowlist opt-in；代码未发起旧 Flash HUD 修改/隐藏。最终 `runtime-qualification.json` 为 47/47，run `c53cd48c…6c85`、647,221 B、SHA `E41F8CFC…FBFAB`；focused 为 134 pass + 3 opt-in skip / 137，全量为 1,735 pass + 3 opt-in skip / 1,738。`visual-evidence.json` 证明 C# 36-file、Web 12-file、comparison 66-PNG A/B 各自逐字节一致；Runtime Lane C 11/11 exit 0、scalar 566，guardrails `scripts=3 / unsafeCandidateCases=3` | 旧 Flash 同屏可见性、B0-04 oracle 人签、真实游戏 composite、真实手点、审美、最终 Kimi K1 与承载最终证据/文档的 closeout containing commit 的双 builder/quorum 尚未完成；promotion/标准入口/部署不属于 B0 Exit，预期仍为 `NOT_DEPLOYED`；不声称 `fixture_static_parity` 或 `fixture_full_parity` | 冻结证据/文档提交后构建该 closeout containing commit 的双 builder 并执行只读 quorum preflight，再做 Kimi k3/max K1，最后交人工 |
| B0-07 | blocked_by_B0-06_exit_and_human_review | §9.5 已完成文档建设 k3/max 审计 | 最终实现包 k3/max 再审、全 Exit Gate、人工接受；computer-use native pipe 当前不健康，不得以无头截图代替桌面人审 | 等 B0-06 最终证据与人类签收 |

**上游整合复核（2026-07-29）**：B0-05 commit `27364dbc9ff579200ae0dbc9d2d5c6c10151c479` 已在安全分支之外的独立 worktree 以 merge（不 rebase）纳入 `origin/main` `ef31b39069272304f5831b9028b786ce333aa62e`。三处文档冲突按“上游正式 runtime 真值优先、PlayerInfo source-ahead/`NOT_DEPLOYED` 边界保留”解决；自动合并的 release policy 同时保留上游 workbench closure 与 PlayerInfo candidate contract。合并树通过 doc governance、release policy `61/61`、release-state `48/0`、runtime consensus `15` assertions、PlayerInfo production contract 与防御矩阵 `8/8`、全量 xUnit `1651 pass + 1 opt-in skip / 1652`，PlayerInfo focused 为 `69 pass + 1 opt-in skip / 70`。这些是合并兼容性证据，不重写 B0-05 报告所绑定的 pre-merge 37-file closure/test DLL，也不把最新正式 Core `DADE2CC2…CCA1` 外推为已包含 PlayerInfo。

**B0-06 收尾整合复核（2026-07-29）**：实现冻结 commit `0215c03b4594221f02e993bf8d58dd1bc669e815` 与安全分支已建立；独立 integration worktree 重新 `fetch` 后确认 `origin/main=ef31b39069272304f5831b9028b786ce333aa62e`、merge-base 即 `origin/main`、ahead/behind=`18/0`，`git merge --ff-only origin/main` 为 no-op。该 clean tree 上完整 Launcher 回归为 1,735 pass + 3 opt-in skip / 1,738，formal qualification 与视觉 A/B 取得上述最终自动证据；一次并发重载下发生的 `EQUIP_UI_SendTrueWaitsForExactPanelRequestAndTimesOut` 计时失败在停止并发后单测连续 10/10、随后全量 1/1 通过，作为并发条件下未复现的时序失败（疑似负载抖动）留痕，未据此修改生产代码。收尾完成前还须再次 fetch/祖先关系/干净树复核，若远端变化则先冻结并集中整合后重跑受影响门。
...
> Oracle 轨保留 B0-01B 第七轮 candidate，等待人类签署来源、状态、层隔离、crop 与审美 review receipt；不得用 FFDec/Web/Flash-Web 指标代签。工程轨的 B0-06 最终实现基线已通过 47/47 formal qualification，并以 `runtime-qualification.json` / `visual-evidence.json` 冻结 C#/Web/comparison A/B 确定性；fixture state/widget、MP fragment、HP 固定轮廓/旋转渐变、path glyph atlas 与独立 click-through surface 绝不注册进现有 NativeHud union，代码也不隐藏/修改旧 Flash HUD。下一步先把证据/文档作为只读后继提交冻结并复核上游，再让两个正式 builder 对同一 full commit 形成 quorum `-VerifyOnly` 报告，完成 Kimi k3/max K1，最后由人类检查旧 HUD 同屏、游戏 composite、手点与审美。B0 不要求 promotion、标准入口或部署；但两正式 builder 一致性和人签未完成前 Exit 仍不勾，任何自动结果也不得冒称 parity 或代替人签。每完成一片都在同提交回写 §12。
