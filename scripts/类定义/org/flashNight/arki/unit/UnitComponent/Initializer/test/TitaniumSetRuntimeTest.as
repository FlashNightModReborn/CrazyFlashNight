...
class org.flashNight.arki.unit.UnitComponent.Initializer.test.TitaniumSetRuntimeTest {
    ...
    private static function testBloodPact():Void {
        ...
        check(near(runtime.getBloodPulsePower(),3037),"战技每段威力取配置指定的完整血剑负担");
        ...
        check(near(runtime.getBloodPulsePower(),3037),"砸地配给后仍保留后续血浪伤害");
    ...
    private static function testBloodAttacks():Void {
        ItemUtil.itemDataDict = weapons;
        var unit:MovieClip = makeUnit(5,0,false,false,false,13);
        var runtime:TitaniumSetRuntime = unit.__titaniumType61;
        unit.攻击模式 = "兵器";
        var props:Object = {};
        runtime.projectBloodAttack(unit,unit.刀,props);
        check(props.血量上限击溃==undefined,"初始化未兑现不冒充战斗破盾");
        advance(1);
        runtime.projectBloodAttack(unit,unit.刀,props);
        check(props.血量上限击溃==undefined,"盾在线时普通血剑不带条件击溃");
        var shield:Shield = Shield(unit.shield.getShieldById(runtime.getShieldId()));
        shield.consumeCapacity(shield.getCapacity());
        runtime.projectBloodAttack(unit,unit.刀,props);
        check(near(props.血量上限击溃,0.09) && props.斩杀==9,"破盾立即投射0.09击溃和9斩杀");
        var snapshot:Object = props;
        props={血量上限击溃:0.2,斩杀:12};runtime.projectBloodAttack(unit,unit.刀,props);
        check(props.血量上限击溃==0.2 && props.斩杀==12,"更高同类贡献取高值");
        props={};runtime.projectBloodAttack({},unit.刀,props);
        check(props.斩杀==undefined,"其他单位不能借用血剑运行态");
        runtime.projectBloodAttack(unit,{name:"血色光剑天秤"},props);
        check(props.斩杀==undefined,"同名但不同装备实例不能借用资格");
        unit.攻击模式="长枪";runtime.projectBloodAttack(unit,unit.刀,props);
        check(props.斩杀==undefined,"破盾不向机枪投射血剑属性");
        unit.攻击模式="兵器";
        org.flashNight.arki.unit.Action.Skill.SkillAttributeCore.transferWeaponAttributes(unit,props);
        check(props.血量上限击溃==0.09 && props.斩杀==9,"兵器技能真实属性入口继承破盾能力");
        var outer:MovieClip=unit.man.createEmptyMovieClip("刀",2);
        var inner:MovieClip=outer.createEmptyMovieClip("刀",1);
        var dressup:MovieClip=inner.createEmptyMovieClip("装扮",1);
        for(var i:Number=1;i<=3;i++) dressup.createEmptyMovieClip("刀口位置"+i,i);
        unit.刀_刀口数=3;
        var template:Object={子弹威力:999};
        org.flashNight.arki.unit.Action.Melee.BladeShootCore.shoot(unit,template);
        check(emitted.血量上限击溃==0.09 && emitted.斩杀==9,"真实刀口发射入口带破盾属性");
        check(template.血量上限击溃==undefined && unit.击溃==undefined,"刀口投射不写入模板或人物全局属性");
        shield.setCapacity(shield.getMaxCapacity());
        org.flashNight.arki.unit.Action.Melee.BladeShootCore.shoot(unit,template);
        check(emitted.斩杀==undefined,"回盾后下一刀立即停止条件贡献");
        check(snapshot.斩杀==9,"回盾不追溯修改旧攻击快照");
        unit.状态="兵器站立";
        check(runtime.commitBloodPact(),"满盾仍可启动完整战技");
        var man:MovieClip=bindBloodAnimation(unit);
        props={霰弹值:5,最小霰弹值:3};
        check(runtime.prepareBloodPactAttack(man,props),"满盾战技不需要破盾快照");
        check(props.hitBehavior.crumble==0.09 && props.hitBehavior.execute==9 && props.子弹威力==3037,"战技自带配置中的终结属性及威力");
        runtime.grantBloodPactBonus(man);
        check(unit.伤害加成>440,"血剑保持献血后的自身固定增伤");
        if (!DamageManagerFactory.Basic) DamageManagerFactory.init();
        BulletInitializer.inheritShooterAttributes(props,unit);
        props.flags=2;props.固伤=0;props.百分比伤害=0;
        BulletHitEffectRegistry.prepare(props,unit,unit);
        check(props.霰弹值==5 && props.击溃==0.09 && props.斩杀==9,"实际命中准备保留联弹段数与战技终结属性");
        var victim:MovieClip=makeUnit(0,0,false,false,false);
        victim.hp=victim.hp满血值=1000000;victim.防御力=1;
        DamageCalculator.calculateDamage(props,unit,victim,1,"");
        check(victim.hp满血值==999100,"五霰弹只结算一次0.09击溃");
        var buffedDamage:Number=1000000-victim.hp;
        victim.hp=victim.hp满血值=1000000;
        BulletHitEffectRegistry.prepare(props,unit,victim);
        DamageCalculator.calculateDamage(props,{伤害加成:0},victim,1,"");
        check(buffedDamage>1000000-victim.hp,"战技联弹真实伤害兑现自身固定增伤");
        runtime.finishBloodPact(man,man.__ti61BloodCastId);
        check(!runtime.prepareBloodPactAttack(man,{}),"取消后旧动作不能再造血浪判定");
        closeUnit(victim);
        shield.consumeCapacity(shield.getCapacity());unit.hp=0;props={};
        runtime.projectBloodAttack(unit,unit.刀,props);
        check(props.斩杀==undefined,"死亡停止新破盾攻击");
        unit.hp=100;unit.头部装备={};runtime.projectBloodAttack(unit,unit.刀,props);
        check(props.斩杀==undefined,"换装立即停止新破盾攻击");
        closeUnit(unit);
        ItemUtil.itemDataDict = {};
    }
