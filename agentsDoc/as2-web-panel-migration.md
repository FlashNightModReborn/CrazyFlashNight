...
**最后核对代码基线**：release source commit `5f5cfce7c162ba616bfc51f3c03f3134e937d36a`（2026-08-27，库存显式批量转移真批量事务及封闭审计映射；deployment `4d5fd254752a149ce07006f8f48391ab26485f61`；`HUMAN_ACCEPTANCE_PASSED / promoted`）。Web 以一次 `autoTransferBatch` 提交最多 50 个有序 lease，Host 严格重建请求与 full/partial 回包，AS2 一次扫描目标、每容器一次 revision/index rebuild、一次窗口组重投影；旧 `autoTransfer` 只保留给单件 `Ctrl+单击`。fresh Shared TestLoader 为 EquipmentInventory `28/28` + Inventory `170/170`、Compiler `0/0`、32K retry `0`；同进程 50 件 fixture 为旧逐项约 `673 ms`、新 batch 约 `20 ms`。维护者随后在 exact isolated candidate 中确认大量物资即时完成转移；Host 日志分别以单一 call 完成 10 件（约 423 ms）和 31 件（约 277 ms），无 reconcile/stale/timeout/rollback/重复派发，退出时 shadow save/confirm 成功且 Flash code 0。候选日志把新命令投影为 `other` 的词表缺项已补为 exact `autoTransferBatch → inventoryAutoTransferBatch`；双故障域共识、39/39 production policy、strict v2 promotion 与正式入口身份/生命周期窄纵切均已通过。正式入口没有重跑库存业务写或写后重启读回，因此不称业务或持久化 `standard_entry_verified`。
