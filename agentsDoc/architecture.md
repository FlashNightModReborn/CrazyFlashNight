...
- **当前失败/重入边界**：历史五次 failed attempt 之后，S6 又有两次 operationally invalid/aborted run，以及 run `82a440cc…9870` 在前十 case/三份 TWS 非静音诊断 capture 后，于 Realtek→TWS default-device switch 触发 `miniaudio.dll` `0xc0000005` 崩溃；S9 `b2370…a108` 又因旧有限 SFX/UTC/stale-counter evidence contract fail-close。九个 runId/capture 均不可复用。R3 修正后的完整 Source matrix 已通过：observer `25/25`、runner `29/29`、assembler `5/5`、C# Diagnostics/Stimulus 各 `16/16`、Launcher `3547 passed + 3 opt-in skipped / 3550`，并闭合 native/runtime、AS2 static、Jukebox 与发布控制面；本文当前版本随 implementation/generated/doc closure 同树落盘的 clean commit 定义新 Source S，commit/tree 仅由提交后的 Git object 解析。该 exact S 的双 clean repro、新 candidate 与新 runId 尚不存在，没有 E1/H2，严格 **H2 blocked / NOT_DEPLOYED**；完整事故证据与台账见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
