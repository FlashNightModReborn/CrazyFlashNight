...
**状态边界**：本列车把斗兽标定从工作簿原始来源摄取、参数闭包、`contamination` Schema/fixture、timeout 口径和真正 Schema 实例校验，收束到可冻结、可撤销、可持久恢复的 Gate F 星期级批处理。tracked 计划仍含 58 个 scheduled candidate，另将 `B12` 隔离为 quarantine，共 198 个 10–25 run shard / 3,255 个机器 run；`C9` 仍保留 `10 + 20 + 25` run 与 side-swap 全量复核。
...
准确状态仅为斗兽 Gate F 所需正式 runtime `promoted`、阵营修复已取得工作树实机诊断：下一 cohort 尚未 freeze/arm。新的 fresh 门除 `node tools/arena-calibration/run-checks.js`、`node tools/test-agent-entry-contract.js`、计划/manifest/decision hash、workbook hash + cell provenance、runtime identity、受保护存档和低注意力 fail-closed 契约外，还必须让已知派生召唤者在 original/side-swap 两方向都实际产生 `phaseSpawnCount > 0`，且全程无 `faction_contamination`。promotion 不代签 Gate F low-touch/星期级通过、数值样本或业务 `standard_entry_verified`；Audio H2 仍保持独立门。
