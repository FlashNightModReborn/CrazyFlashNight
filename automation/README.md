...
**文档角色**：启动与运行自动化入口。  
**最后核对代码基线**：当前材料、商城与战备箱体验优化发布冻结 source commit `7d869c5eadf224953b2c61ec7a6ceee2ca03055f`（tag `runtime-build-v2/20260728-material-shop-storage-workbench-v1`、request `9AC9E3C9A1BC5162F17881DDB37A6F23C8AAAA9830A808CAF4648EDE60753771`），由 commit `40675cae8264a5330904731b9dd8871a2263723a` 记录正式 promotion；exact isolated candidate 与无参 `automation/start.ps1` 正式入口均完成材料档案、战备箱、K 点商城及普通关闭 smoke，严格状态为 `standard_entry_verified`。PlayerInfo historical v1 仍只证明旧字节；首列 main-space v2 q3 因 qualification 漏挂 `PlayerInfoRasterPlan.cs` 使 production policy 仅 22/23 而退役。修复后的 F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` 已由全新 r2 tag/request 完成双 builder、双 faultDomain、两侧 23/23 policy 与不可复用的 `-VerifyOnly` preflight；该 preflight 单独只到 `candidate_built`，随后 exact isolated candidate 已以 `p50` fixture 启动并取得维护者人类接受。PlayerInfo B0 当前为 `b0_accepted`，oracle 为 `oracle_frozen_for_b0`；Launcher 仍严格为 `candidate_executed / source-ahead / NOT_DEPLOYED`，没有真实 `pi_*`、跨 renderer pixel parity、业务 E2E、promotion、标准入口或部署，不能覆盖上述现役正式身份。精确证据见 [runtime v2 深层文档](../docs/runtime-build-reproducibility.md)和[PlayerInfo 工具 README](../tools/player-info-hud/README.md)。
...
PlayerInfo B0 的**历史 v1** source freeze/tag/request、双 builder 与 [`runtime-promotion-preflight.v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight.v2.json) 继续只作旧字节的非部署审计；首列 main-space v2 q3 也因 22/23 policy 门失败而退役。当前 F2/r2 已另行完成双 builder、23/23 policy 与 [`runtime-promotion-preflight-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight-main-space-v2.json) 的 `-VerifyOnly` 闭环，但该报告明确不执行或授权 runtime/release-state mutation、promotion、deployment，也不可复用为 promotion 输入，因此自身仍只关闭 `candidate_built`。随后独立启动的 exact isolated candidate 才把当前状态推进为 `candidate_executed / source-ahead / NOT_DEPLOYED`；精确 source/request/identity/receipt/quorum 与启动证据以 [runtime v2 深层文档](../docs/runtime-build-reproducibility.md) 为准。
...
最后一条命令会从受保护的单路径段 `runtime-build-v2/<release-id>` source tag 触发固定 cloud workflow，并验证 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 都精确绑定请求的 full commit；随后等待精确 run、安全解压并产出 `$cloud.candidateRoot` / `$cloud.proofPath`。unsigned job 交接 artifact 保留 1 天，失败诊断与 signed 结果保留 7 天；超期未 promotion 就重新 dispatch，不把 Actions artifact 当长期档案。request、队列/CAS、双故障域 quorum、receipt 与 `promote-runtime-bundle.ps1` 的完整步骤以 [runtime v2 发布列车](../docs/runtime-build-reproducibility.md) 为准。最近一次完整 Runtime Lane C 仍是 historical v1 clean F `cb38600aae51f5019d09f87c33bd9e67d2b1f511` 的 11/11、scalar 566、831.271 秒；当前 F2/r2 不声称重跑该 Lane，但已独立完成新 tag/request、双 builder、两侧 23/23 policy 与不可复用 `-VerifyOnly` preflight，随后 exact isolated candidate 已启动并由维护者接受真实游戏 composite；oracle 已冻结为 `oracle_frozen_for_b0`，B0 为 `b0_accepted`。现役正式 release 仍是材料/商城/战备箱 request `9AC9E3C9…53771` 经 promotion commit `40675cae…3723a` 达到的 `standard_entry_verified`；PlayerInfo Launcher 生命周期仍为 `candidate_executed / source-ahead / NOT_DEPLOYED`，没有真实 `pi_*`、业务 E2E、promotion、标准入口或部署。功能回归证据统一维护在 [测试指南](../agentsDoc/testing-guide.md)，正式发布记录见 [材料、商城与战备箱关闭证据](../docs/evidence/material-kshop-battlebox-workbench-closure-2026-07-28.md)。任何时候都禁止手工换 manifest、伪造证明，或把单机 candidate 复制进根 runtime。
