...
**文档角色**：启动与运行自动化入口。  
**最后核对代码基线**：当前材料、商城与战备箱体验优化发布冻结 source commit `7d869c5eadf224953b2c61ec7a6ceee2ca03055f`（tag `runtime-build-v2/20260728-material-shop-storage-workbench-v1`、request `9AC9E3C9A1BC5162F17881DDB37A6F23C8AAAA9830A808CAF4648EDE60753771`），由 commit `40675cae8264a5330904731b9dd8871a2263723a` 记录正式 promotion；exact isolated candidate 与无参 `automation/start.ps1` 正式入口均完成材料档案、战备箱、K 点商城及普通关闭 smoke，严格状态为 `standard_entry_verified`。PlayerInfo F `cb38600aae51f5019d09f87c33bd9e67d2b1f511`、其 v1 tag/request、两类 builder 与 promotion `-VerifyOnly` 都是历史 v1 非部署证据，只证明旧 placement 字节。main-space v2 首列 source F=`40853287e7ed04714d68935c0002f8ad6d8aea05`、evidence/docs 后继=`884fbf43…`，Kimi `k3/high` 已 PASS；其受保护 tag `…-b0-v2` 与 q3 本地 X509 票保持审计有效，但 production policy 因 qualification 项目漏挂 `PlayerInfoRasterPlan.cs` 仅 22/23，通过列车已退役且未启动 cloud/quorum/preflight。修复已通过 production contract、renderer 12/12 + 78/78 + 8/8、合同 8/8 与 Launcher 1747+3/1750；新 cloud ref 为 `runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2`，必须在全新 q4/w4 对最终 F2 fresh 重算。总体仍为 `source-ahead / NOT_DEPLOYED`，不得把任何 PlayerInfo source/candidate 覆盖为上述现役正式部署身份。
...
正式 v2 consensus 当前绑定 tag `runtime-build-v2/20260728-material-shop-storage-workbench-v1`、request `9AC9E3C9A1BC5162F17881DDB37A6F23C8AAAA9830A808CAF4648EDE60753771`、source commit `7d869c5eadf224953b2c61ec7a6ceee2ca03055f`、build identity `3F7887ADF9041DFEDA74EEA26040A3628919A725B04D322FCE667791425F7B44` 与 payload closure `A9B142A417CF2AC45EA19845144569391FCDEE8DF4235F5279A1EC5F0F9022F2`；promotion 记录 commit 为 `40675cae8264a5330904731b9dd8871a2263723a`。本次 local signer 位于 `physical-host-a`，不可导出 keyId 为 `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`；GitHub Actions run `30364763726` 的 OIDC identity `1BDD1B0F419E0CA384E59986DBC1C9C9195A01CFEAF0838E116A8AD13B9BCEC3` 提供 `github-hosted-windows` 第二票。production policy `F8C5D87ADA848323D0C35FD6DC0AC895327BC7A382FFBE7997287C7AA646440B` 的 22/22 final receipt SHA-256 为 `E1604B933786DBA76C989A18AFC401AB7F1E925AE677FFCE27F3729C6B2A9A43`；cloud workflow 仍只允许 `Crazyfs` / `Flash-Night` 的固定 actor ID 首次 dispatch。

PlayerInfo B0 的**历史 v1**独立非部署 source-freeze 使用 F commit `cb38600aae51f5019d09f87c33bd9e67d2b1f511`、tree `c74cc66f445921f5fcda1da04a2de0f013fef8ef` 与不可变 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v1`。clean F 的 Runtime Lane C 为 11/11、scalar 566、guardrails `scripts=3 / unsafeCandidateCases=3`、831.271 秒；stdout SHA-256 为 `9BFBDDD521BE54D70E0158CE595C37C54820EE4C66C73C5681AFC56636A524AC`，stderr SHA-256 为 `F7FA16D6B9927C3B36D6A61FCBC49C00874EAAC41EBFC662F863793E442479C6`。request `839C74FD1DF61ACC1DA580041F6FA71CA13A84DF1F43A55237BF9BEEF8648FB2` 的本地 X509 signer `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5` / `physical-host-b` 与 GitHub OIDC signer `FA7BFFEB064C9B263B3A8BB6CFB5A655891CD9A9065B02D3BD6862DBD42C34F4` / `github-hosted-windows`（run `30440903705`）实际产出同一 build identity `7687B56106F0C19EFD4463DCA2B69B3892BD3EE39503A3FD5521ED6DB0257A9F` 与 payload closure `AF2E9E5727E9B6ADE056EBE4CA52BE93AD518116F10FF3307DA4409059CC724B`。[`runtime-promotion-preflight.v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight.v2.json) 为 `preflight-passed`，但明确 runtime/release-state/promotion/deployment 均未改变且 `reusableAsPromotionInput=false`。这些事实不能批准当前 main-space v2。current v2 source F 已为 `40853287…`，最终 Kimi 已 PASS；全新 tag/request/双 builder quorum/preflight 仍待 fresh 生成，PlayerInfo 仍为 `source-ahead / NOT_DEPLOYED`，不能称为当前正式 consensus。
...
最后一条命令会从受保护的单路径段 `runtime-build-v2/<release-id>` source tag 触发固定 cloud workflow，并验证 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 都精确绑定请求的 full commit；随后等待精确 run、安全解压并产出 `$cloud.candidateRoot` / `$cloud.proofPath`。unsigned job 交接 artifact 保留 1 天，失败诊断与 signed 结果保留 7 天；超期未 promotion 就重新 dispatch，不把 Actions artifact 当长期档案。request、队列/CAS、双故障域 quorum、receipt 与 `promote-runtime-bundle.ps1` 的完整步骤以 [runtime v2 发布列车](../docs/runtime-build-reproducibility.md) 为准。最近一次完整 Runtime Lane C 是 historical v1 clean F `cb38600aae51f5019d09f87c33bd9e67d2b1f511` 的 11/11、scalar 566、831.271 秒；该 Lane 仍不能单独替代环境门或双 builder，request `839C74FD…8FB2` 的 X509 + GitHub OIDC 与 promotion `-VerifyOnly` 也只闭合 v1 非部署 quorum。当前 main-space v2 source F=`40853287…` 与 Kimi K1 已冻结/通过，但 fresh Lane/tag/request/quorum/preflight 尚未运行。现役正式 release 仍是材料/商城/战备箱 request `9AC9E3C9…53771` 经 promotion commit `40675cae…3723a` 达到的 `standard_entry_verified`；PlayerInfo 没有 promotion、标准入口或部署。功能回归证据统一维护在 [测试指南](../agentsDoc/testing-guide.md)，正式发布记录见 [材料、商城与战备箱关闭证据](../docs/evidence/material-kshop-battlebox-workbench-closure-2026-07-28.md)。任何时候都禁止手工换 manifest、伪造证明，或把单机 candidate 复制进根 runtime。
