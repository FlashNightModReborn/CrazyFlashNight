...
_root.主动战技函数.长枪.天启大封印.取落成位置 = function(自机:MovieClip):Object {
    ...
    return { x: 自机._x + 接口.法阵兜底距离 * 方向, y: 自机._y };
};
...
_root.主动战技函数.长枪.天启大封印.法阵兜底距离 = 300;
 ...
 * SealDomain 的椭圆判定范围（X 半径 300 × Z 半径 150 → 600 × 300 像素）。
 *
 ...
 *
 * 换算：圆阵素材外圈椭圆实测 940 × 470（宽高比 2.00，与 300/150 一致）。
