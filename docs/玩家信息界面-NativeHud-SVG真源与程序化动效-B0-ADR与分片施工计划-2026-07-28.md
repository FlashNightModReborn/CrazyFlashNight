...
- 已完成：本 ADR、B0 分片边界、依赖准入门、runtime build identity 纳入方案与验收词典；Kimi k3/max 的文档建设审计见 §9.5，v1 实现首轮与 closeout 增量复审仅作为历史证据保留在 §9.6。main-space v2 完整增量仍须在 source/evidence/doc 冻结后以 `kimi-code/k3` 重新执行一轮最高可用思考强度审计（当前 CLI 配置的最高值为 `high`）。
...
- v1 非部署 source-freeze 只作历史：F commit/tag、两类正式 builder、production receipt 与 quorum `-VerifyOnly` 的 33-file payload closure `AF2E9E57…724B` 不批准 v2。main-space v2 source commit `40853287…` 已形成，但新 tag/request、双 builder quorum 与不可复用为 promotion 输入的 preflight仍待完成；本轮仍不 promotion、不部署、不改变现役 runtime。
...
main-space v2 的完整只读审计已于 2026-07-30 完成。首次运行建立 session 后，本机因内存/存储压力重启；恢复时没有新开审计或更换模型，而是续跑**同一 session**直至自然 `end_turn`。最终裁决为 `APPROVE_AUTOMATED_CLOSEOUT / K1_AUTOMATED_GATE=PASS`，P0=0、P1=0；只授权继续全新 v2 tag/request、双 builder/quorum/preflight 与人类终审，不表示 parity、`oracle_frozen`、promotion、部署或 `b0_accepted`。
...
审计对“sourceRef 指向尚不存在的 v2 tag”这一候选 P1 作 `reject`：tag 当前缺失是 §11.3 规定的创建前状态，helper 对 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 三重绑定，缺失或指错均 fail-closed；只有按冻结 F 创建一次后才准 dispatch。
...
- [ ] v1 两类正式 builder/F/quorum 事实仍按历史保留。main-space v2 首列 tag `…-b0-v2` 已冻结 `40853287…`，q3 本地票有效但 production policy 仅 22/23，故该列车已退役且未启动 cloud/quorum/preflight。最终闭环必须使用提交后的 F2、全新 `refs/tags/runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2` 与全新 q4/w4，fresh 取得双 signer/双 faultDomain 一致 identity/payload/Core 和不可复用 promotion preflight；旧 v1 数值及 q3 的 `DAE8A71C…8ED1` / `051FA840…D2C` 均不得代签。
...
- [ ] v2 SVG/JSON 与包图已由首列本地 candidate 证明 payload 可构建，但最终 F2/r2 仍须 fresh 比对 local/cloud identity、payload closure、manifest 与 Core；不得复用 q3 票。
- [ ] qualification 编译闭包修复已通过 targeted contract、12/12 renderer 与 8/8 正负合同；production policy 仍须在 exact F2 的新 local/cloud candidate 上达到 23/23，不能把修复前 22/23 或 focused xUnit 当作通过。
...
- [x] `launcher/README.md`、testing guide、工具 README、总体迁移 ADR 与本台账已同步 clean `40853287…` 的 qualification/full/视觉 closure 及当前 Apps 边界；旧 F/G docs-only 事实继续只作 v1 history，当前 K1 已通过，build/human 仍明确 pending。
- [x] main-space v2 的 evidence/docs-only 后继 commit 已形成：commit `884fbf436f21c5e27ed3370c445cbae941349496` / tree `11d89d25001185aa2eb19d5bfbd36b579b38059d` 以 F=`40853287…` 为唯一父，只修改 5 份文档与 3 份 tracked B0-05/B0-06/visual v2 snapshot；提交前后 clean-tree、doc governance 与 diff check 均通过。旧 `81168e…c549` 只承载 v1。
...
| B0-06 | `main_space_v2_automated_qualification_passed; r2_builder_human_pending` | clean merge `40853287…` 的 formal run `ed61d7275fdd4a96a73f762c5c2a4d71` 为 48/48、**653,904 B**、SHA `182C7DC9…CA8E1`；clean full 为 1747 pass + 3 opt-in skip / 1750。两份 C# 36-file、Web 12-file、direct 66-PNG 与 viewer 自动包均确定；MP 四字段 unique `bestDx=0` 且 anchor edge `dx=0`。§9.7 当前 K1 已 PASS。首列 v2 q3 本地 producer/票有效但 policy 22/23，唯一 qualification compile-closure 缺口已修复并通过 contract/renderer/8-case/full 回归 | F2/r2 的 fresh local+cloud builder、23/23 production policy、quorum/preflight、Computer Use 实际初始化、Flash oracle/真实桌面与全部人工项仍待完成；自动图像/Kimi 只作 diagnostic/audit，不批准 parity/部署 | 在全新 q4/w4 闭合 r2 build；旧 tag/q3/CAS/票/失败 receipt 仅保留审计且不复用，再做 Computer Use/真实游戏/IDE 身份和人签 |
| B0-07 | `main_space_v2_k1_passed; human_pending` | §9.7 同一 Kimi session `session_cbe7e035-bcdd-475a-9147-5f021ff5700d` 在 `k3/high` 下完成；165/165 request、5/5 config update 全部命中，P0/P1=0，裁决 `APPROVE_AUTOMATED_CLOSEOUT`；23 个唯一 PNG 已看，MP 不建议整组平移或改生产 blend mode | 运行时 SWF、new main-RSL oracle、游戏 composite、关键比例/平滑、混合 DPI、真实手点、HP halo/色调、`%` 字形、审美与人类明确接受 | build 自动门闭合后交人类终审；Computer Use/FFDec/Edge/Kimi 均不能代签 |
...
**当前未闭合项（2026-07-30；上述自动报告 UTC 日期为 2026-07-29）**：首列 v2 本地 producer 已运行，但因 production policy 22/23 退役；F2/r2 的 fresh request、本地+GitHub builder、23/23 policy、双 faultDomain quorum 与不可复用 `-VerifyOnly` preflight尚未形成。Kimi Code `kimi-code/k3` + 当前最高可用 `high` 的完整异构对抗已按 §9.7 通过，但不代签桌面/审美。ChatGPT App 当前只确认 Computer Use/Browser skill 已暴露，实际控制会话尚未初始化，不能据 skill 可见性签署桌面结果。最终仍须完成真实 loader/player/child SWF 与 IDE Output 身份、可见 DWM/游戏 composite、z-order/occlusion、鼠标透传、关键比例/平滑、混合 DPI、审美与人类明确接受；B0 不要求 promotion，当前也没有 promotion、release-state 或部署变更。
...
> v1 工程轨曾冻结 F=`cb38600…f511` 与不可变 tag，并由 clean F Lane、两个正式 builder、production receipt、`-VerifyOnly` preflight 和 clean-G K1 证明旧字节；这些事实保持历史有效，但不得外推到 main-space v2，也不得移动/删除 v1 tag。main-space v2 首列 F=`40853287…`、G=`884fbf43…`、B0-05 32/32、B0-06 48/48、full 1747+3/1750、自动视觉包与 Kimi `k3/high` PASS 均已形成。首列 tag `…-b0-v2` 与 q3 本地票有效，但 policy 22/23，因 qualification compile closure 漏项退役；修复回归已通过，恢复时只能从最终 F2 建全新 r2 tag/q4/w4，先取本地 23/23，再启动 cloud 并完成双 faultDomain `-VerifyOnly`，禁止复用 q3。真实 11-case Flash candidate 仍只有 strict tooling identity，human review 四项为空，所有自动图像都只是 diagnostic。随后初始化 Computer Use/Browser 实际控制，并核验旧 Flash 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实桌面手点、运行时 SWF/IDE Output 身份与审美人签。当前总体状态仍为 `visual_correction_in_progress; human_exit_blocked`，不是 `awaiting_human_acceptance`；B0 不要求 promotion、标准入口或部署，build/人签全部完成前不得写成 parity、`oracle_frozen`、`awaiting_human_acceptance` 或 `b0_accepted`。每完成一片都在同一提交回写 §12。
