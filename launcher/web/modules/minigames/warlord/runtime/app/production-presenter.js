...
function nodePauseReasons(state, factionId, nodeId) {
    ...
    if (nodeOccupyingFactions(state, nodeId).some((occupier) => occupier !== factionId))
        return ['敌军占据'];
    if (!isNodeActive(state, nodeId))
        return ['据点尚未激活'];
    if (!hasStableSupplyPath(state, factionId, nodeId))
        return ['补给线中断'];
...
function projectOrder(state, factionId, order, queuePosition) {
...
function projectLane(state, factionId, slot, index) {
    ...
    const laneState = !sourceHead
        ? 'idle'
        : paused ? 'paused' : sourceHead.status;
    const workRounds = slot.orders.reduce((total, order, orderIndex) => {
        if (orderIndex === 0 && order.status === 'building')
            return total + Math.max(0, order.remainingRounds);
        if (orderIndex === 0 && order.status === 'waiting_deployment')
            return total;
...
export function flattenProductionOrders(nodes) {
    return nodes
...
function compareRankedLanes(a, b) {
...
function rankedStableLanes(state, factionId) {
...
function recommendationReason(node, lane) {
    if (lane.queueLength === 0)
        return `空闲槽 · 部署余量 ${node.freeCapacity}/${node.capacity}`;
    if (lane.blocked)
        return `其余槽同样受阻 · 当前 ${lane.queueLength} 单`;
    return `最低负载 · ${lane.workRounds} 回合 / ${lane.queueLength} 单`;
}
export function recommendProductionLane(state, factionId) {
...
function choiceFrom(state, factionId, cardId, mode, nodeId, slotId, reason) {
...
export function resolveProductionChoice(state, factionId, cardId, mode, exactNodeId, exactSlotId) {
    if (mode === 'exact') {
        return choiceFrom(state, factionId, cardId, mode, exactNodeId, exactSlotId, '人工指定槽位');
    }
    const ranked = rankedStableLanes(state, factionId);
    for (const entry of ranked) {
        const choice = choiceFrom(state, factionId, cardId, mode, entry.node.nodeId, entry.lane.slotId, recommendationReason(entry.node, entry.lane));
        if (choice.ok)
            return choice;
    }
    ...
    if (fallback) {
        return choiceFrom(state, factionId, cardId, mode, fallback.node.nodeId, fallback.lane.slotId, recommendationReason(fallback.node, fallback.lane));
    }
    return choiceFrom(state, factionId, cardId, mode, exactNodeId, exactSlotId, '没有可用生产槽');
}
//# sourceMappingURL=production-presenter.js.map