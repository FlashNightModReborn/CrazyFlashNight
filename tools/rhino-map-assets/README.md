...
三处地图共用原生矢量分件：外交基地两辆、前线第三图三辆后景坦克，防御地带首图一辆抬炮坦克。采用固定车号、单帧 Graphic 和既有背景烘焙，没有 AS3 控制器、轮履循环、开炮演出或新增运行时类。具体摆放参数由 [placements.json](placements.json) 维护。
...
| 地点 | 车号 | 地面世界坐标 | 姿态 |
|---|---:|---|---|
| 外交军阀基地帐篷后 | 25 | (600,165) | 左向平射，前排帐篷遮挡；27号位置由磁暴坦克接替 |
| 军阀前线基地第三张战斗图帐篷后 | 22、23、28 | (650,175)、(1060,165)、(1740,175) | 左向平射，前排帐篷遮挡 |
| 防御地带第一图原牵引炮位置 | 29 | (990, 320) | 右向，主炮相对车体抬升15°，整车随坡度旋转-4° |
...
外交基地26号位置现由猛虎41号接替，27号位置由磁暴坦克接替；高地犀牛右移140px，为猛虎替换卡车留出间距。见 [猛虎装配](../tigr-map-assets/README.md)与[磁暴装配](../tesla-map-assets/README.md)。同时同步三种车时，先执行犀牛，再执行猛虎，最后执行磁暴工具及三者检查。

默认命令同步派生分件和摆放；只读检查核对 Include、分件引用、几何一致性、六个互异车号、摆放矩阵、后景层次/色调、火炮替换以及所有坦克元件均单帧 Graphic / 无脚本 / 无位图实例。仅清理工具拥有的 `Codex/犀牛R09/` 中退出目标闭包的派生 XML，不清理其他库项。
