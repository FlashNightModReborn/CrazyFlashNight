...
- **capture / evidence boundary**：loopback capture 必须先由 capture process 在 `IAudioClient.Start` 后写 `ReadySignalPath`，再允许 stimulus；runtime tuple/`f32` 与 endpoint WAV `pcm_s16le` 分别绑定。toolchain writer 用 pinned runtime environment gate 生成 canonical Node/MSVC/.NET/PowerShell/cmd/vcvars 描述，隔离 runner 只接收 hash-bound absolute `CF7_NODE_EXE`，不继承 PATH；materializer、operator/client 和 endpoint lister 只准备输入或执行资格化。assembler 只生成 9 组配置/输入和 `HUMAN_REQUIRED` drafts，固定 `promotionAuthorized=false`，不生成 E1、H2 或 pass。crossfade 自动证据只证明非零 fade request、相关 `started`、source 替换及相邻 ≤500ms 的连续非静音 meter，不证明 dual-slot gain envelope。
- **当前失败/重入边界**：旧 Source S `90985ae…705644` / tree `beaaab2…24a3` 的 candidate `1A675D…6CEB / D5232F…6A1F` 在 runId `cdf7…59f64` 的 case 1 因无 AS2 ingress、source playing 但 cursor/meter 为 0 而静音 fail-close；candidate 已优雅停止，无 E1/H2，runId 作废且始终 `NOT_DEPLOYED`。本段所在版本与完整修复 closure 首次同树落盘的 clean implementation commit 定义为新 S，commit/tree 由提交后的 Git object 解析；该 exact S 仍必须重新双 clean repro、构建新 candidate 并从新 runId 重跑完整 A6。当前 source 状态与 promotion 边界见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)；Launcher 细节以 [launcher/README.md](../launcher/README.md) 为准。H2 尚未取得，因此严格为 **H2 blocked / NOT_DEPLOYED**。
