...
> **PlayerInfo 当前施工边界**：B0-03b 仅达到 `renderer_qualified / candidate_built / NOT_DEPLOYED`，B0-05 达到 `passed / synthetic_fixed_bounds / split_required`。2026-07-29 的 B0-06 已实现 allowlist opt-in 的 fixture-only `PlayerInfoWidget`、程序化 HP/MP 与独立 click-through split surface；它不注册进现有 NativeHud union、不接 UiData/真实 `pi_*`，也不自动隐藏或修改旧 Flash HUD。最终实现基线的真实 HWND/ULW qualification 为 47/47，C# 36-file A/B、Web 12-file A/B 与 comparison 66-PNG A/B 也各自逐字节一致，证据为 [`runtime-qualification.json`](../tools/player-info-hud/evidence/b0-06/runtime-qualification.json) 与 [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json)。computer-use native pipe 当前不健康，Flash candidate 仍未获人签；真实游戏 composite、手工鼠标透传与审美必须由人类签收，所以这些自动结果仍不代表 parity、promotion 或标准入口部署。exact runner 与生命周期边界见 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)和[验证矩阵](../agentsDoc/testing-guide.md)。
...
B0-06 自动入口为 `tools/player-info-hud/run-b0-06-runtime-qualification.ps1` 和 [`tools/player-info-hud/README.md`](../tools/player-info-hud/README.md) 的 C# visual capture / headless Edge direct-edge 流程。qualification 合同固定 3000 visible visual steps、3000 idle ticks，并在同一 STA/owner 上运行 1–5 个 100-cycle excluded warmup group；只有三类 handle 的全部 checkpoint 不高于组起点、endpoint delta 不为正且无正向趋势时才收敛，随后立即运行新的独立 100-cycle acceptance group。五组不收敛必须以 `diagnostic_after_warmup_cap` 失败，不能放宽原门。最终实现基线 run `c53cd48cd1054f6e9b988d11f5f06c85` 为 647,221 B、SHA-256 `E41F8CFC…FBFAB`、47/47；第三组首次收敛，紧邻 acceptance 的 process/GDI/USER delta=`-3/0/0`。NativeHud hidden/enabled commit p95=`7.3670/7.2699 ms`，split surface/observer-commit p95=`3.6748/1.1382 ms`。visual manifest 固定为 `structural_capture_complete / fixture_only`，comparison 固定为 `diagnostic_awaiting_human_review` 且无接受阈值；最终 C#/Web/comparison A/B 已闭合自动确定性，精确闭包见 [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json)，同轮机器资格见 [`runtime-qualification.json`](../tools/player-info-hud/evidence/b0-06/runtime-qualification.json)。在完成人类 Flash oracle、游戏 composite、真实手点和审美验收前，不得写 `fixture_static_parity`、`fixture_full_parity` 或已部署。
