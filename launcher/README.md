...
> **PlayerInfo 当前施工边界**：本工作树的 B0-03b 仅达到 `renderer_qualified / candidate_built / NOT_DEPLOYED`；B0-05 仅取得 `passed / synthetic_fixed_bounds / split_required`。它们没有形成生产 `PlayerInfoWidget`、实际 split surface / `UpdateLayeredWindow` 测量、UiData、真实 `pi_*`、视觉签收、promotion 或标准入口部署；exact candidate、runner 与 lifecycle 约束见 [B0 专项 ADR](../docs/玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)和[验证矩阵](../agentsDoc/testing-guide.md)。
