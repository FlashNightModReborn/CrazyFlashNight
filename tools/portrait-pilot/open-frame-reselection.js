...
async function main() {
  ...
  if (options.help || !options.batch) {
    process.stdout.write("用法：node tools/portrait-pilot/open-frame-reselection.js --batch <frame reselection batch> [--check]\n");
  ...
  if (options.check) {
    process.stdout.write(`${JSON.stringify({ status: "frame_reselection_open_preflight_verified", datasetDigest: loaded.dataset.datasetDigest, rows: loaded.dataset.items.length, candidates: loaded.dataset.counts.candidateCount, artifactCount: loaded.artifactCount })}\n`);
    return;
  }

  const { chromium } = require(PLAYWRIGHT);
  const profilePath = path.join(loaded.batchRoot, "frame-reselection-profile");
  const decisionsPath = path.join(loaded.batchRoot, "portrait-pilot-frame-reselection.json");
  const readyPath = path.join(loaded.batchRoot, "frame-reselection-opened.json");
  if (fs.existsSync(readyPath)) throw new Error("frame-reselection-opened.json 已存在；拒绝覆盖旧启动证据");
  ...
  try {
    ...
    await page.exposeFunction("savePortraitFrameReselection", async (value) => {
      ...
      const archiveRoot = path.join(loaded.batchRoot, "frame-reselection-exports");
      fs.mkdirSync(archiveRoot, { recursive: true });
      const stamp = new Date().toISOString().replaceAll(":", "").replaceAll(".", "");
      const archivePath = path.join(archiveRoot, `portrait-pilot-frame-reselection-${stamp}.json`);
    ...
    page.on("download", async (download) => {
      ...
      await page.evaluate((savedPath) => {
        window.dispatchEvent(new CustomEvent("frame-reselection-export-saved", { detail: savedPath }));
    ...
    writeReadyMarker(readyPath, {
      schema: "cf7.portrait-pilot-frame-reselection-opened.v1",
    ...
    process.stdout.write(`${JSON.stringify({ status: "frame_reselection_opened", readyPath, datasetDigest: loaded.dataset.datasetDigest })}\n`);
