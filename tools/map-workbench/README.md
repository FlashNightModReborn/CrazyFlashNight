...
最后核对代码基线：commit `7187d793b3`。维护者已确认第一阶段维护与预览可用，并授权发布；发布状态以[两阶段施工](../../docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)为准。

游戏入口为 **其他 → 工具 → 地图工作台**。开发验收双击根目录 `地图工作台测试启动.cmd`，使用当前源码身份的隔离候选。范围与发布节奏见[两阶段施工](../../docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)。
...
“保存到项目”仅更新 `data/map/map_definition.json`；**使用根目录地图工作台测试启动.cmd 重启候选后**完整地图与 NativeHud 使用新定义。草稿预览独立，不修改当前游戏目录或存档。“撤回所选修改”恢复原字节，配置后来改变时拒绝覆盖。结果未知时点击“重新读取 / 核对结果”。草稿支持 JSON 导入/导出，导入必须匹配配置摘要并通过同一 C# 校验。
