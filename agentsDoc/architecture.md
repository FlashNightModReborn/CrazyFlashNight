...
- loot feed（左下物品获得/击杀播报）：AS2 `_root.发布战利品消息`（`scripts/引擎/引擎_鸡蛋_lsy_物品系统.as`，埋点 `PickUpManager` 拾取、通关奖励界面、`FinishTask` 任务交付、`LootClaimCommitCoordinator` 战利品箱提交）经 `{"task":"loot","payload":{kind,name,count,source,icon,eliteLevel?,doll?}}` fire-and-forget 到 `LootFeedTask` → NativeHud `LootFeedWidget`；native 单渲染端，无 web fallback，与地图战利品箱的 `loot_response` 回包是两个域。`eliteLevel` 只承载 `UnitUtil.getEliteLevel` 的 `0/1/2` 击杀事实，Host 校验后再映射调度和视觉，AS2 不拥有最终优先级。`launcher/web/icons/manifest.json` 自此同时是 NativeHud 图标源（`LootIconCatalog` 为其首个运行时消费者）。击杀播报与 killStats 共用 `enemyKilled` 事件链收口（`BulletQueueProcessor` 子弹/攻击体致死发布 → `EnemyKilledEventComponent.onEnemyKilled` 更新击杀统计并调 `_root.发布击杀播报`，语义为“我方（主角/佣兵）击杀”），按 `getUnitTypeKey` 发射 `kind:"kill"`，显示名取 `_root.敌人属性表` displayname，头像取 `launcher/web/assets/enemy-portraits` manifest（键=`敌人-*` 类型键，214/214 覆盖），默认 variant SVG 经 Svg.Skia 光栅化（SKSvg 生命周期与安全配置对齐 PlayerInfoStrictSvg；`<filter>` 元素解析前剥离）；后续关卡结算击杀数展示消费同一 `killStats`/事件链。
- loot feed 调度/渲染：`LootFeedModel` 是五个稳定可见槽 + recoverable pending queue。任务/通关奖励、战利品箱与 Boss 为 guaranteed 最高级，精英与拾取为 exact-aggregate 次级，普通击杀最低且仅身份溢出可压缩；合并键必须含 `source + eliteLevel`，pending 不计时，高级替换复用 victim slot。`LootFeedWidget` 使用 26px 直角卡、96–220px/8px 内容微量化宽度和按当前计数位数稳定的可选右列；调度重要性不直接等同 Boss 视觉。入场、退场与数字替换统一使用 SmoothStep；计数反馈只绘制数字列的短色洗/底沿与 1–2px 交叉位移，击杀增量只在有界等级内提高强度。事件 burst 先单飞 drain，计数视觉 8Hz、签名 32ms 采样并受 overlay 33ms 合成上限约束、图标首轮最多 450ms，静态 hold 零 repaint；bounds 只随可见卡/等待行或计数位数桶变化。
- loot 图标缓存与纸娃娃回包：`LootIconCatalog` 以有界 `BitmapLruCache` 作为 64px 位图唯一所有者，帧集只在一次查询/绘制周期内组合，不能跨淘汰保留位图引用；整组动画超过预算时回退静态首帧。纸娃娃缺图由 Host 发 `{type:"dollBake",key,requestId,tuple}`，Web 通过 allowlisted `doll_bake_result` 回 `{key,requestId,pngBase64|error}`；Host 仅接受当前 key 的 exact 在飞 requestId，并在 completing 状态保持同键单飞，foreign/duplicate/timeout 后迟到回包不能写盘或清除新请求。Web 快照显式使用 `pixelRatio:1`、`animate:false`，因此 API 的 256 尺寸不受 WebView DPI 放大；只有完整解码且 exact 256×256 的 PNG 才可原子落到 `launcher/data/doll-portraits/<hex>.png`，拒绝时记录 key/requestId/reason。历史缓存首次复用也须通过同一验证，旧尺寸/损坏文件进入重烘焙且读取端拒绝消费，成功后 `PortraitReady` 驱动占位卡重探。常驻 UI 路线只保留 NativeHud；`useNativeHud=false` 是待移除 legacy 装配开关，不定义 loot fallback。
- 集成测试入口：`--bus-only`
