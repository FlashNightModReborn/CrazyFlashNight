...
export interface BattleVisualUnit {
  snapshot: BattleRecord['attackerSnapshots'][number];
  hp: number;
  dead: boolean;
...
export interface MapProjectionFrame {
  centerX: number;
  centerY: number;
  worldUnitsPerMapX: number;
  worldUnitsPerMapY: number;
}
...
export function mapProjectionFrame(state: GameState): MapProjectionFrame {
...
export function nextPromotionFor(
  game: GameState,
  factionId: FactionId,
  cardId: CardId,
): PromotionId | null {
...
function initialAttackRound(behaviorId: BattleVisualUnit['snapshot']['behaviorId']): number {
  return behaviorId === 'ammo' ? 2 : 1;
}

function attackInterval(behaviorId: BattleVisualUnit['snapshot']['behaviorId']): number {
  return behaviorId === 'sniper' || behaviorId === 'ammo' ? 2 : 1;
}

export function projectBattleVisual(record: BattleRecord, eventCount: number): Map<string, BattleVisualUnit> {
  const units = new Map<string, BattleVisualUnit>();
  ...
  if (eventCount >= record.result.eventLog.length) {
    for (const result of record.result.pieceResults) {
      ...
      if (result.dead) unit.lastStatus = '阵亡';
    }
  }
  return units;
}

export function stateProjectionDigest(state: GameState): string {
