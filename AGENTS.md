...
**最后核对代码基线**：release source commit `1e24fca52165e909fd53743f30105487a89708a4`（2026-09-08；tag `runtime-build-v2/20260908-panel-handoff-return-retry-v1`）。选关关闭前台交接、返回失败后的原生重试与统一开发入口已完成双 signer / 双故障域共识、40/40 production policy 和原子 promotion，当前为 `HUMAN_ACCEPTANCE_PASSED / promoted`（限已实测两次选关、非零奖励领取后再次入场及正常退出；失败重试由故障注入回归覆盖）。完整身份见 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)，现场边界见[焦点诊断 §9.13](docs/焦点管理-诊断与卡顿排查-2026-05-24.md#913-2026-09-08选关关闭前的前台交接)与[关卡结果 ADR §0D](docs/关卡结果与基地结算-CSharp-Web-ADR-2026-08-27.md#0d-2026-09-08-返回失败后的原生重试入口)，不外推完整焦点根治或业务 `standard_entry_verified`。地图中途撤退的既有验收与地图工作台第二阶段继续有效，分别见上述 ADR §0C 与[两阶段记录](docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)。
