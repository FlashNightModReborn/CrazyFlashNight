...
│
├── contracts/
│   └── panel-contracts.v1.json            NPC/KShop/Crafting 跨层命令、数值权威与共享边界向量登记表
│
├── src/
...
| Policy | `tools/validate-launcher-release-policy.ps1` | Web/data/native cursor/优化程序集等只读产品审计；其中 `panel-cross-layer-contracts` 门调用 `node tools/validate-panel-contracts.js` 核对 Panel 命令、数值权威、exact response handler 与剥离注释后的 Host/AS2 可执行锚点；最后验证 tree 前后不变并签发绑定 tree/identity 的 production receipt |
