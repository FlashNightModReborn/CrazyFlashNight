...
- 2026-07-11 补充（历史）：刘海“战备箱”由 Flash `warehouse` 切到独立 Web `workbench`（背包—战备箱）时曾保留诊断回退；该回退、环境开关与 Flash `warehouse` 命令已于 2026-07-30 删除，现为 Web-only fail-closed。路由、40 槽/页与 0..6 页权限窗口由 Launcher xUnit、Web harness、Flash fresh trace 与主文件旧 UI 可达性门共同守门。
