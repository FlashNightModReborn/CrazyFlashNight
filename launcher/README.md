...
当前 `Launcher.Tests.csproj` 采用 SDK glob 自动纳入测试源码；2026-07-11 全量执行为 **753/753** 通过，其中 `ShopTaskTests` 覆盖 KShop Host 写 gate、成功载荷校验、对账与防重放，`InventoryTaskTests` 追加 lease-bound `tooltip` 白名单、同容器 source/target 透传、`sortAndMerge` action/容器窗口/八策略白名单与路由形状。`SaveMigratorTests` 继续使用代码内 helper 数据；外部 fixture 目前主要集中在 `Fixtures/MapHud/`。
...
| K点商城 / 物品工作台 (旧商城界面 MC 已退役) | `game-ui-behavior.*` + `workbench.js` + `kshop-runtime.js` + `inventory-runtime.js` + `kshop.js` | `SHOP` → `kshop`；Gate A1/A2 已验收，Gate A3 candidate 支持采购态、背包内/仓库当前页内/背包↔仓库转移、1200 槽按 50 分页（24 页矩阵直跳/键盘跳页/会话位置记忆）、displaySort、权威 sortAndMerge、discard 与 claim→背包刷新；AS2 权威 + slot lease/containerEpoch |
...
**面板类型**：
- **kshop**（K 点商城 + Gate A3 完整仓库）: 唯一支持入口为 Launcher `SHOP` → Web Panel；旧 Flash `shopMainMC` 已退役。Gate A1/A2 已由用户验收，Shell/ViewHost/Grid/pointer 与 InteractionBroker/owned transfer 分别标 `gate-a1-accepted` / `gate-a2-accepted`；完整仓库为 `gate-a3-candidate`。顶栏切换采购态与库存态；库存态保留背包 50 槽，仓库按 50 槽窗口覆盖 1200 容量。分页保留前后页，并把当前页改为 4×6 的 24 页直接跳转矩阵；工具条显示当前物理槽位范围，支持 `PageUp/PageDown`、`Ctrl+Home/End`，模块会话内关闭再打开保持最后 offset。以上全部复用 `setWindow`，不新增协议、不全量加载 1200 槽。仓库另支持只改 DOM 且保留 physicalSlot 的 displaySort、确认后的 AS2 权威 sortAndMerge，以及背包内/仓库当前页内/跨容器的 whole-slot move/merge/swap、discard、claim 刷新。物品格标题优先使用玩家可见 `displayname`，仅缺失时回退内部 `name`，且不显示机器分类 `stack/equipment`；非装备数量与装备强化度使用图标角标，大数量按中文 `万/亿` 紧凑显示且完整值保留在可访问文本，装备达到动态最大强化等级时只给图标加金框。升阶状态改为图标左上角独立菱形，名称下方只保留三个插件槽；槽体统一为中性深灰金属凹槽，已装插件只在内部角色符号和短辉光上使用低级 `#006600`、中等 `#996600`、高等 `#0099FF`、特殊 `#FFFF00`，不再整格填色。`ui_presentation.xml` 以受控 `形状-solid|outline` token 编码角色：▲火力、△精准操控，稳定防护/续航/结构功能使用线框方/圆/菱形，特殊机制使用实心星。档级由材料子文件前缀派生，特殊档现有错色图标不参与运行时取色。occupied 槽静止悬浮会按 `{containerId,slot,expectedLease}` 读取真实物品实例富注释；点击选取 source 或开始 pointer 拖拽即隐藏并持续抑制，释放后须重新进入槽位才显示，避免注释与抓取镜像碰撞。同 physicalSlot 自拖拽由 Web 零写入、AS2 `same_slot` 双层保护；同容器第二槽提交失败完整回滚。每次打开先清空跨 generation 的旧商城投影并保持 `shopReady=false`，只有新的完整 `bulkQuery` 成功后才开放购物车编辑、结账和领取。`InventoryTask` 白名单现含 `inventoryTooltip/inventorySortAndMerge`；AS2 在隔离副本计划整理，数值型 `消耗品/收集品`（正式材料 schema=`type=收集品,use=材料`）参与堆叠合并，经 item mass 守恒后单次提交，成功清仓库 lease、`containerEpoch+1`、dirtyMark，未知历史物品若会被过滤则 `sort_failed` 且不写。Overlay 同步加载 `game-ui-behavior.css/js`，统一抑制原生文本选取/拖影/菜单并放行编辑控件。确定性 gate：`validate-equipment-mod-ui.js` 当前覆盖 102 插件/35 tag/6 role；Edge harness 40/40；Launcher xUnit 753/753；Flash `InventoryPanelServiceTest` 50/50（fresh trace 以本轮验证记录为准）；视觉用 `--visual=inventory|inventory-pages`。生产验收仍须真实存档覆盖真实装备 displayname/满级/升阶/插件档级与角色投影及实例注释、同容器/跨容器转移、24 页矩阵与键盘跳页、真实材料/装备整理、InputShield、未知插件物品保护、断线重开与重启回读；完成前不进入 NPC 金钱买卖/装备槽或 Step 6。
