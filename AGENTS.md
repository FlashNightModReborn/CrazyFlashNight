...
**最后核对代码基线**：release source commit `1eaa51d2afec10c8c6a9bf5abe1ff20343208e19`（2026-08-26；implementation `064f462b6e927bc56bae08e342e8be21aa5d17cf`、tag `runtime-build-v2/20260826-hit-numbers-csharp-v1`、tree `9e30c52740f2c18e4fc3c732a571aa314e2fced3`、request `99FA28B523CB89C3F68B14DEBE2DEC4A1C6B6E2A966DBEB665F605753290890A`、deployment `cef2c8d227049248d5767a06e0bf57700931e0b2`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `8654E63F0679502148828E5E6D0489BDEDE88312050D076F47F736ADA81EA69F` / `github-hosted-windows`（cloud run `32923040870`）已对 identity `3B075D1FD1E429137253315E5B2095B4D3C942FE42D039E43D55C93625FDA187`、closure `8E25D01458A05334FF1CFF44CB9A1F0E8B1BCE6420A4DE21D41B4D648479CF1D` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `5D160A15ECD569D3582E8BD41768A33C99BE92910E283276C514C68DABABC4BB`，39/39 production receipt SHA-256 为 `096080A9620608D6CA1824D5E0D5DA09011A7E32D6204596D1F53A6A16098432`；deployment commit `cef2c8d227049248d5767a06e0bf57700931e0b2` 与首次 post-promotion audit run `32923796401` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车把打击伤害数字表现彻底收敛到 C#：AS2 只发送逐段结算事实，`HitNumberRuntime` / `HitNumberOverlay` 独占短寿命 reducer、布局与绘制，V8/Flash renderer 及旧 `_root` 开关/上限不再提供 fallback。Host 机器全局偏好提供 `off / balanced / total / classic / detail`、全局世界行上限与显式无限制；屏外余量剔除始终保留，精确历史独立进入固定 32,768 段环形账本，并只在暂停态 Web 设置按需分页物化。
fresh 门包括 Hit Number focused `97/97`、Settings Host `20/20`、Settings Web `18/18`、Launcher 全量 `4213 pass + 3 explicit opt-in skip / 4216 total`、视觉 harness 26 张图 + 2 段视频（平均 `0.910 ms/frame`，GDI `3→3`），以及随上游合入后 TimePool XML `configured=3 / pools=3 / refs=9`、AS2 focused `46/46`（8 cases）与 Compiler `0/0`。production policy `39/39`、worktree/index 33-file bundle、signed consensus、正式根 bootstrap `--verify-only`、远端 Audit 与无 candidate selector 的正式入口身份/生命周期 smoke 均通过。
维护者已确认与正式 Core 逐字节相同的 D8 隔离候选有效，覆盖高频命中、四种可视模式、颜色语义、逐发长数字排布与伤害对账；正式入口随后确认 exact identity/closure、bus ready、正常 Guardian shutdown 与零子进程残留，但没有从正式入口重跑战斗。因此准确状态为打击数字 `HUMAN_ACCEPTANCE_PASSED / promoted`，正式入口身份/生命周期窄纵切达到 `standard_entry_verified`，不称打击数字业务 `standard_entry_verified`；Audio H2 仍保持独立门。
