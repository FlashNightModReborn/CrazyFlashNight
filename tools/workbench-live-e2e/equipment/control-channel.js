...
const NATIVE_INPUT_EVENT_KEYS = Object.freeze(["schema", "runId", "requestId", "step",
  "observedAt", "receivedAt", "eventType", "isTrusted", "selector", "tagName",
  "visible", "enabled", "viewport", "rect", "clientPoint", "hitTargetMatches", "key",
  "button", "repeat", "eventSha256"]);
...
const REQUIRED_CONTROL_STEPS = Object.freeze([
  ...
  "exit_confirm",
  "restart_open_tuning",
  "restart_select_source",
  "restart_close_tuning",
]);
...
function validateInputEvidence(value, request) {
  ...
  const expectedNativeArtifact = "control/native-input-events/" + request.requestId + ".json";
  const validNativeEventRef = native && SharedEvidence.isPlainObject(eventRef)
    && SharedEvidence.canonicalJson(Object.keys(eventRef).sort())
      === SharedEvidence.canonicalJson(["artifact", "eventSha256", "sha256"])
    && eventRef.artifact === expectedNativeArtifact
    && /^[a-f0-9]{64}$/.test(String(eventRef.eventSha256 || ""))
    && /^[a-f0-9]{64}$/.test(String(eventRef.sha256 || ""));
...
function expectedProviderOperationId(receipt) {
  ...
  return "eqop-" + SharedEvidence.sha256Text(SharedEvidence.canonicalJson(projection));
}
...
function expectedProviderCaptureEventId(event) {
  ...
  return "eqcap-" + SharedEvidence.sha256Text(SharedEvidence.canonicalJson(projection));
}
...
function validateProviderReceipt(receipt, request, transport, result, completedAt, binding) {
  ...
  if (receipt.schema !== PROVIDER_RECEIPT_SCHEMA || receipt.runId !== request.runId
    ...
    fail("provider_receipt_invalid", "control",
      "control acknowledgement lacks one exact request-to-start-to-input-to-capture-to-completion receipt", {
        step: request.step,
      });
  }
...
class ControlChannel {
  constructor(root, runDir) {
    ...
    this.nativeInputEventsDir = SharedEvidence.ensureExactChildDirectory(
      this.controlDir, "native-input-events", "control");
  ...
  issue(step, options) {
    ...
    if (!REQUIRED_CONTROL_STEPS.includes(step)) {
      fail("control_step_invalid", "control", "control step is outside the exact Equipment journey", { step });
    }
...
function nativeInputEventRelativePath(requestId) {
  return "control/native-input-events/" + requestId + ".json";
}

function readNativeInputEvent(root, runDir, request, inputEvidence) {
  if (!inputEvidence || inputEvidence.kind !== "native_input") return null;
  const exactRun = SharedEvidence.assertOwnedRunDirectory(path.resolve(root), runDir,
    OWNED_BASE_RELATIVE, "native_input_event");
  const reference = inputEvidence.eventRef;
  const expectedRelative = nativeInputEventRelativePath(request.requestId);
  if (!SharedEvidence.isPlainObject(reference) || reference.artifact !== expectedRelative) {
    fail("provider_native_input_binding_invalid", "control",
      "native input event reference is not the exact provider-owned step artifact", {
        step: request.step,
      });
  }
  const filePath = path.resolve(exactRun, expectedRelative.replace(/\//g, path.sep));
  if (!SharedEvidence.pathInside(exactRun, filePath)) {
    fail("provider_native_input_binding_invalid", "control",
      "native input event artifact escaped the owned run directory", { step: request.step });
  }
  const file = SharedEvidence.readExactRegularFile(filePath, {
    phase: "native_input_event", maximumBytes: 64 * 1024,
  });
  if (file.sha256 !== reference.sha256) {
    fail("provider_native_input_binding_invalid", "control",
      "native input event reference does not bind the exact file bytes", { step: request.step });
  }
  let value;
  try { value = JSON.parse(file.bytes.toString("utf8")); }
  catch (error) {
    fail("provider_native_input_binding_invalid", "control",
      "native input event artifact is not valid JSON", { step: request.step, message: error.message });
  }
  const unsigned = Object.assign({}, value);
  delete unsigned.eventSha256;
  const observed = Date.parse(value && value.observedAt);
  const received = Date.parse(value && value.receivedAt);
  const mirrored = ["eventType", "isTrusted", "selector", "tagName", "visible", "enabled",
    "viewport", "rect", "clientPoint", "hitTargetMatches", "key", "button", "repeat"];
  if (!SharedEvidence.isPlainObject(value)
      || SharedEvidence.canonicalJson(Object.keys(value).sort())
        !== SharedEvidence.canonicalJson(NATIVE_INPUT_EVENT_KEYS.slice().sort())
      || value.schema !== NATIVE_INPUT_EVENT_SCHEMA || value.runId !== request.runId
      || value.requestId !== request.requestId || value.step !== request.step
      || !Number.isFinite(observed) || new Date(observed).toISOString() !== value.observedAt
      || !Number.isFinite(received) || new Date(received).toISOString() !== value.receivedAt
      || observed > received
      || inputEvidence.observedAt !== value.receivedAt
      || value.eventSha256 !== reference.eventSha256
      || value.eventSha256 !== SharedEvidence.sha256Text(SharedEvidence.canonicalJson(unsigned))
      || mirrored.some((field) => SharedEvidence.canonicalJson(value[field])
        !== SharedEvidence.canonicalJson(inputEvidence[field]))) {
    fail("provider_native_input_binding_invalid", "control",
      "native input event bytes, digest, time, or target differ from provider input evidence", {
        step: request.step,
      });
  }
  return { relativePath: expectedRelative, file, value, eventTime: value.receivedAt };
}
...
function verifyProviderReceiptReference(root, runDir, request, ack) {
  ...
  provider.nativeInputEvent = readNativeInputEvent(root, runDir, request,
    provider.receipt.inputEvidence);
  return provider;
}
...
function writeAck(root, runDir, requestId, rawAck) {
  ...
  const capture = { relativePath: provider.capture.relativePath,
    sha256: provider.capture.sha256, bytes: provider.capture.bytes,
    mediaType: provider.capture.mediaType };
