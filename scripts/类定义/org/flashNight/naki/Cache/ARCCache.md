...
**ARCCache v3.2** 是一个基于 **自适应替换缓存（Adaptive Replacement Cache，ARC）** 算法（Megiddo & Modha, 2003）的完整实现，旨在提供高效的缓存策略，适应多种访问模式，提高缓存命中率。该实现使用 **ActionScript 2（AS2）**，采用哨兵节点、原始键语义、节点池化等优化，适用于对性能和资源敏感的环境。
...
2. **完全新 key（标准 ARC Case IV，v3.2 由 `_evictForInsert()` 统一处理）**：
   - **Case A**（|L1| >= c，ROBUST-3）：如果 |T1| < c，删除 B1 尾部最老幽灵（入池）+ REPLACE；否则直接从 T1 删除（入池，不进 B1）。
...
| 优先级 | 编号 | 变更 | 效果 |
|--------|------|------|------|
| P0 | OPT-C | `get()` 变量整合（20→13 var）+ B1/B2 幽灵路径合并 + localT1 延迟 | 省 7 次 DefineLocal2 + T2 HIT 省 1 属性读取 |
| P0 | OPT-D | `get()` MISS 检查后置至 T1/T2 之后 | AS2 AVM1 对 undefined.prop 返回 undefined，T2/T1 HIT 各省 1 次 StrictEquals |
...
3. **内部工具**：
   ...
   - `_drainToPool()` — 将队列节点回收入池并清空队列（v3.2 OPT-A）
   - `_clear()` — 重置所有状态（重用哨兵 OPT-8 + 回收节点入池 OPT-A）
   - `_doReplace()` — 标准 ARC REPLACE 淘汰 + 空队列回退（ROBUST-1）
   - `_evictForInsert()` — Case IV 淘汰统一入口（v3.2 OPT-B，ROBUST-3 防御性 >=）

4. **`get` 方法**：检索数据，设置 `_hitType`/`_lastNode`。v3.2 优化：变量整合（20→13 var，OPT-C）、MISS 延迟检查（OPT-D）、B1/B2 路径合并。
...
**ARCCache v3.2** 在 v3.1 基础上进行了系统性代码审阅优化：

- **热路径优化**：`get()` 变量整合（20→13 var，OPT-C）+ MISS 延迟检查（OPT-D）+ B1/B2 路径合并，T2 HIT 路径省去 1 次 `this.T1` 读取 + 1 次 StrictEquals 比较 + 7 次 DefineLocal2。
- **代码克隆消除**：提取 `_evictForInsert()` 统一 Case IV 淘汰逻辑（OPT-B），消除 put()/_putNew() 间 ~80 行同步维护风险。
- **防御性改进**：Case IV `L1==c` → `L1>=c`（ROBUST-3）；putNoEvict ghost 路径增加容量守卫（ROBUST-4）。
- **GC 优化**：`_clear()` 使用 `_drainToPool()` 回收节点入池（OPT-A），reset 后立即可复用，避免重新分配。
- **put() 优化**：B1/B2 路径合并 + 变量整合（~35→16 var，OPT-E）。
...
Performed 10000 cache operations in 103 ms.
Cache Operations per Second: 97087.3786407767
