...
对于 `bullet` 等字符串属性，merge 支持联弹格式（`{联弹类型}-{子弹类型}`）的智能拼接：

| 场景 | 原值 | 合并值 | 结果 |
|------|------|--------|------|
| 保留联弹前缀 | `横向联弹-普通子弹` | `次级穿刺子弹` | `横向联弹-次级穿刺子弹` |
| 完整格式覆盖 | `横向联弹-普通子弹` | `纵向联弹-穿甲子弹` | `纵向联弹-穿甲子弹` |
| 无前缀直接替换 | `普通子弹` | `次级穿刺子弹` | `次级穿刺子弹` |

**拼接规则：**
1. 如果新值包含 `-`：视为完整联弹格式，直接覆盖
2. 如果原值包含 `-`：保留原值的联弹前缀，替换子弹后缀
3. 否则：直接使用新值
