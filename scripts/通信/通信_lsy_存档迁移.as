...
_root.存档系统.convertInventory = function(data){
    ...
    for(var i=0; i < _root.物品栏总数; i++){
        var 旧物品 = 旧背包[i];
        var oldItemName = 旧物品[0];
        var oldValue = 旧物品[1];
        if(!旧物品 || oldItemName == "空") continue;
        if(ItemUtil.isMaterial(oldItemName)){
            收集品栏.材料.add(oldItemName, oldValue);
            continue;
        }else if(ItemUtil.isInformation(oldItemName)){
            收集品栏.情报.add(oldItemName, oldValue);
            continue;
        }
        var 新物品 = BaseItem.create(oldItemName,oldValue);
        if(新物品 == null){
            if(oldItemName.indexOf("阶") == 1) 新物品 = new BaseItem(oldItemName, {level:oldValue});
            else if(oldItemName.indexOf("制作图纸") == -1 && (oldItemName.indexOf("墨冰") > -1 || oldItemName.indexOf("狱火") > -1)) 新物品 = new BaseItem(oldItemName, {level:oldValue});
            else continue;
        }
        if(旧物品[2] == 1 && ItemUtil.isEquipment(oldItemName)){
            var use = _root.getItemData(oldItemName).use;
    ...
    data.inventory = {
        背包:  新物品栏.背包.toObject(),
        装备栏:新物品栏.装备栏.toObject(),
        药剂栏:新物品栏.药剂栏.toObject(),
        仓库:  新物品栏.仓库.toObject(),
        战备箱:新物品栏.战备箱.toObject()
    };
    data.collection = {
        材料:收集品栏.材料.toObject(),
        情报:收集品栏.情报.toObject()
...
_root.存档系统.convert_material_info = function(data){
    ...
    data.collection.情报 = 情报.toObject();
}
