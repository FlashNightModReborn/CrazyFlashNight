...
> **2026-08-16 tooltip 收尾/最小化恢复黑屏修复发布列车当前态**：source `29ab59fd7832d10d54badc3132a5153646c1fa3d` / tree `978eaa7e75708b82fd8d57b9f8c63b11f5e689cb` 已由 tag `runtime-build-v2/20260815-overlay-compositor-restore-v2`、request `2EE9A1E09A07A93EE2B7D49D74BCCDFE71A6953E290AB7C8D6A58BC460DAC4A0`、本地 X509 `builder-local-b` / `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `31892519833`）完成双 signer / 双 faultDomain、production policy、strict v2 verifier 与原子 promotion/rollback；正式 identity 为 `74D14EE3A17CD12AE5F74CB6779CF702339E4D22E3F6C6523F1E9C72EFA04CA3`，closure 为 `2F6B950B64441E1378A687549638464D420C255251D517EA935D866021D6D1CC`，deployment commit 为 `a469f487f065abea7558a89a13f0345ab468d4a6`，post-promotion audit run `31897530643` 已通过。通用 promotion 不承载 Audio H2 或 emergency bypass；Audio H2 仍为专项 `pending`。正式根入口 `--verify-only` 为 `exit 0`，但本轮未执行面板业务专项正式旅程，因此 runtime/DLL 状态为 `PROMOTED`，不冒称业务 `standard_entry_verified`。
...
> **最后核对架构与工作流**：current formal release source commit `29ab59fd7832d10d54badc3132a5153646c1fa3d`、release tree `978eaa7e75708b82fd8d57b9f8c63b11f5e689cb`、tag `runtime-build-v2/20260815-overlay-compositor-restore-v2`、request `2EE9A1E09A07A93EE2B7D49D74BCCDFE71A6953E290AB7C8D6A58BC460DAC4A0`、deployment commit `a469f487f065abea7558a89a13f0345ab468d4a6`；双 signer / 双 faultDomain v2 promotion 已绑定 identity `74D14EE3A17CD12AE5F74CB6779CF702339E4D22E3F6C6523F1E9C72EFA04CA3` 与 closure `2F6B950B64441E1378A687549638464D420C255251D517EA935D866021D6D1CC`，cloud build run `31892519833`、post-promotion audit run `31897530643` 均通过。Audio H2 仍为专项 `pending`，但不再是通用 supply-chain promotion 的门或 emergency bypass 来源；正式根入口 `--verify-only` 已通过，业务专项正式旅程未执行。此前发布列车只保留为历史分层。通用判定见 [验证矩阵](../agentsDoc/testing-guide.md)。
...
> **上一冻结发布证据（历史）**：source `f01f4b121a4ceebd7dae051f14bb511c5ae3f1cb` 的正式 runtime 曾绑定 Core SHA-256 `565C1F9710421E6B6CC5CB6DDA05DE36B8F1B22B3D7A7CA19617F9786C7D8A4B`、build identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465` 与 payload closure `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF`；immutable tag `runtime-build-v2/20260730-workbench-no-as2-fallback-v1`、request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B` 和 GitHub run `30538330223` 形成双故障域共识。无参标准入口 attempt `8baf52bbcceb452da32da641e58d2922` 取得 fresh handoff、真实 title receipt、单次 enter 与同实例首个权威调制 snapshot 后 supported shutdown；未发送 preview/commit，也未验证普通 panel close、持久化或重启回读。
> **CF7 Agent Runtime / Wings F7 历史 source freeze（2026-07-31）**：一期实现与协议源曾冻结在 C1 `dd84230a1d262c6478591cae2d11051b7a8aa7b1`，当时 `1.0` 尚无 promoted consumer；ADR 文件名保留首次冻结日 `2026-07-30` 以避免 canonical 路径/链接漂移。documentation-only D1 只引用其父 C1，不能在自身内容中自引用 D1 commit hash。F8 已成为首个 promoted v1 consumer，今后破坏 wire 的变更必须新增 revision/version 与迁移，并连同 schema、生成物、实现、fixture、harness 与文档原子升级，不能在现役 v1 内静默替换。exact C1 candidate 已通过 production policy **26/26**，只达到 `candidate_built / NOT_DEPLOYED`；当时无前台会话的严格入口按凭据门失败关闭且无 completion evidence，故未达到 `candidate_executed`、`e2e_verified` 或 `promoted`。这组记录只保留为 F7 历史证据。
> **CF7 Agent Runtime / Wings F8 正式发布与验收（历史，2026-07-31）**：implementation source `53caabc90941826ddacf626f536b0f473adbf049` 的 isolated candidate 先达到 `e2e_verified / NOT_DEPLOYED`；随后 release source `6f3d50a52413c747b05b74be88d6ee46650f4597` / tree `253e57f6d20a90fef6addfa744d0487d88f00dfb` 在相同 build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C`、payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0` 与 Core SHA-256 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD` 上取得双故障域共识并 promotion。无 candidate id 的 operator 只经项目 Agent Runtime MCP 完成正式可见旅程；报告 `tmp/manual-agent-acceptance/formal-f8/agent-runtime-help-20260731T040942Z.json`、transcript 同目录 `.jsonl` 与 `formal-residue-comparison.json` 共同证明 `runtimeMode=formal_runtime`、可信 shutdown、存档不变和无新增残留差量，当时严格状态为 `standard_entry_verified`。完整边界见 [一期范围冻结 ADR](../docs/CF7-Agent-Runtime与Wings-Network一期-范围冻结-ADR-2026-07-30.md)。
> **前一正式 runtime 边界（历史）**：审阅/F0 锚点 `c96f4c3d750561022b706c72a4d53050431e627d` 之后的 A2b、A3、B1–B7、C1–C3、E、G1–G5、H1–H6、双宿主浏览器策略、Native 右侧条件槽，以及旧 AS2 工作台 fallback / Skill 无 token 兼容 / main legacy UI 可达边的退役，曾包含在 `f01f4b121a4c…` 冻结 tree 与 F8 runtime；这些能力现继续包含在 `bf9be8c43b…` current release。自动门和总体 promotion 不等于所有功能都完成新 release identity 下的专项实机验收，本轮标准入口只覆盖上方列出的 Help 窄纵切。
...
│   │   ├── PreparedPanelReplace.cs        Host-owned panel exact replacement：prepare/commit/abort 与 capability transfer
│   │   ├── MaterialShopNavigationCoordinator.cs 材料档案↔NPCShop 单 deadline、settlement witness 与 exact replace 协调器
...
│   │   ├── ShopTask.cs                    K 点商城桥接 + command-specific normalize / Host owner 重建 / checkout identity-preview-postcondition / 写对账
│   │   ├── InventoryTask.cs               inventory-domain v1 strict owner/instance 桥 + 三态未知写门 / affected-container reconcile / 递归回包重建
│   │   ├── NpcShopTask.cs                 NPC 金币商店桥 + owner-local catalog/preview authority / identity-price-postcondition / 写对账
│   │   ├── CraftingTask.cs                合成桥 + command-specific v1/v2 normalize / materials snapshot session / strict response validator / owner-local single-use preview proof
│   │   ├── MaterialShopAccessTask.cs       材料商店 Host→AS2 dedicated fid/correlation 桥（不持 transition timer）
│   │   ├── MaterialShopSettlementWitness.cs Crafting/NpcShop/Inventory owner generation 与 settled lease 见证
...
│   │   ├── EquipmentTuningTask.cs         equipment_tuning 严格 envelope / identity triple / 实例租约 / write epoch 与精确对账水位
...
│   │   ├── ArenaTask.cs / ArenaAuthorityCatalog.cs  竞技场双层 callId 桥 + XML/JSON 权威 session 目录（arena_response）
...
│   │   ├── bg/                            背景图层资源
│   │   ├── pm19/                          PM19 种子库（seed-bank.json + 8×1460B .bin，vendored 自 tools/pm19-prime-magic/runtime/web/assets）
...
│   │   ├── stage-select/                  选关背景与 hover 预览（previews/derived/ 为 P4-b 派生的关卡背景预览，由 tools/derive-stage-select-previews.js 生成，勿手改）
...
│       ├── lazy-loader.js                 串行保序按需注入 <script>（URL promise 去重；失败项驱逐后可重试，后置脚本不预注入）
...
│       ├── tooltip.js                     Tooltip（三 profile：短提示 / 密集检视 / 固定检视栏）
...
│       ├── npcshop-material-navigation.js 材料来源定位、一次性返回与四类普通关闭的 feature-local 状态机
│       ├── npcshop-secondary-pages.js     NPCShop 二级页共享装配/清理 primitive
...
│       ├── arena-panel.js                 竞技场薄 facade（Panels.register + QA 调试面）
│       ├── arena/                         竞技场生产实现六模块（core → shell → challenge-browser → preview-authority → custom-editor → result）+ dev harness
│       ├── arena-custom-parameters.js / arena-custom-param-editor.js / arena-custom-undo.js / arena-custom-polling.js / arena-custom-result-view.js / arena-factions.js / arena-meta-rosters.js 竞技场功能/派生支撑模块（后两者均禁止手改）
...
│       ├── stage-select-data.js / stage-select-fixtures.js / stage-select-panel.js 选关界面 Stage 2 runtime panel（Panels.register('stage-select')；P1-B 起 manifest 与 fixtures 分层，fixtures 兼作 runtime bridge 失败兜底；P4-a 起 panel 为薄 facade）
│       ├── stage-select/                  选关面板生产实现五模块（P4-a 四层拆分：core 状态容器+工具 → view-model 纯数据层（无 DOM，P5 三维插座）→ renderer 空间 DOM+生命周期 → inspector 决策检查器+提交 intent → bridge 请求信封+回包守卫）+ dev harness/qa-suite/golden
...
│       │       ├── stage-select-golden.js 选关 inventory 计数基线单一真值（audit / qa-suite 共同消费）
...
输入由 [`runtime-inputs.v2.json`](../config/build/runtime-inputs.v2.json) 分为 `artifactSourceHash`、`producerRecipeHash`、`toolchainLockHash`、`policyHash` 四个互斥域；build identity 只含前三域。位于 native 源码前缀内、但不产生 DLL 字节的 [`AgentRuntime/Contracts/README.md`](src/AgentRuntime/Contracts/README.md) 以单文件形式归入 `policyHash`，因此参与 request/receipt，又不会冒充 artifact source 漂移。payload closure 排除 manifest，所以政策/manifest 变化不会冒充二进制漂移。candidate 位于 `tmp/runtime-candidates/v2/c-<identity-prefix>-<builder-hash>-<run-token>/`，完整身份保留在 metadata/证明；短目录与构建前后的 MAX_PATH 门兼容 native bootstrap 的 legacy 缓冲。producer 使用独立 native/Cargo/MSBuild/temp 输出，不写正式 runtime、不签名、不跑政策门。
...
上一 F8 consensus 采用本地 X509 keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3` / `physical-host-a` 与 GitHub OIDC builder identity `2E05B5577CC8D2895DC56FC8778A05D09C440302832A05DD4AE551589C9AE918` / `github-hosted-windows`（run `30602046108`）；tracked registry 仍只含公钥。正式 bootstrap/runtime/manifest/consensus 后续已由唯一 promotion 入口切换到本页顶部记录的 P4/P5 release；该 F8 request `A9B33601805709DBB5EAE6DAF312C2B7B0B502096FDD3BDCEA9CBE26D8B1299C`、build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C` 与 payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0` 只保留为历史记录。一次性 migration fuse 不得复用，后续发布不得降级 v1。
...
2. fail-closed 解析 source `xunit.runner.json`，严格要求冻结的 v2.8 schema、`diagnosticMessages` 为布尔 `true`、`parallelizeTestCollections` 为布尔 `false`
3. 通过 `resolve-dotnet.ps1` 探测用户级与系统级 host，并由 `global.json` 强制选择精确 10.0.300 SDK；即使只差一个 patch 的 10.0.301 也拒绝
4. echo `dotnet --version` 作为日志证据
5. `Push-Location $projectRoot` 保证 dotnet host 找到 repo root 的 `global.json`（SDK pin 10.0.300 + `rollForward: disable`）
6. `dotnet test Launcher.Tests.csproj -c Release --logger "console;verbosity=normal"` —— Microsoft.NET.Test.Sdk + xunit.runner.visualstudio 自动 discover + run，连带编译主工程的 `ProjectReference`，并保留 effective runner policy 的诊断证据

`xunit.runner.json` 是 Launcher 测试程序集的 canonical execution policy；`Launcher.Tests.csproj` 只用 SDK 隐式项的 `None Update + CopyToOutputDirectory=PreserveNewest` 把它复制到输出目录，不重复声明 `Content Include`。`XunitRunnerPolicyTests` 从 `AppContext.BaseDirectory` 读取实际复制件，严格断言只有冻结的三项治理键及其类型/值：v2.8 schema、`diagnosticMessages=true`、`parallelizeTestCollections=false`；runner 还会在 PowerShell 5.1 下暂时用非终止模式把完整 testhost 诊断流保存在内存，立即保存退出码、在 `finally` 恢复 fail-fast，再要求恰好一条 `parallel test collections = off` marker。source preflight、构建复制与 testhost 消费三层证据缺一不可，单个 policy fact 不能代签实际调度策略。全量套件包含真实 `Task.Run`、Timer、匿名管道与短 wall-clock deadline；默认按 12 个逻辑处理器并行测试集合会产生线程池饥饿和漂移红项，所以 canonical runner 固定集合串行，不用逐项放宽业务 deadline，也不得用一次重跑绿覆盖并行失败证据。

2026-08-03 A3 fresh canonical 证据连续两轮均为 **3149 passed + 3 explicit opt-in skipped / 3152 total，0 fail**，且 effective-policy marker 各恰好一次、stderr 均为 0 bytes；stdout SHA-256 依次为 `E5B41B76D569E301A263C6F958AABA014C429E7F537755A59FA17957620D835B`、`D7E94C803216026C58FE72AA7E8BE525938C3B623A3235A494448F97F0542E3B`。这只闭合 Launcher 自动测试与 runner policy，不代签真实游戏旅程、candidate、promotion 或 standard entry。
...
装备调制接入增加 `EquipmentTuningTaskTests`、AgentControl 安全门和 Router/WebOverlay 实例路由回归。它固定七键 envelope `{type,panel,domain,cmd,callId,panelInstanceId,payload}`，覆盖 snapshot/preview/commit/tooltip/detach、七类 Web operation 白名单、一次性 token、write epoch、发送失败/timeout/迟到、断连与切离 session 撤销、强制关闭后的 exact reconcile hint，以及 battlebox、Character Build 嵌入态、agent source 和 warehouse 隔离。回归必须负向断言 `source=legacy_equipment_tuning`、Router/TaskRegistry 专项分支、`executeLegacy` 与 tooltip 合并 `html` 字段均不存在；七类 Web operation 继续走同一正式协议，不得恢复迁移暂停或 AS2 UI 旁路。协议没有 `tuningAvailable` feature flag；UI 固定“强化度 / 交换 / 进阶 / 配件”、永久 `+13`、按 source `majorType+use` 的只读交换候选、安全/快速边界、分段富注释和 `48/40/4px` 紧凑格。一个持久化完整/紧凑控件在 storage、构筑候选与 embedded tuning 标题间迁移；调制 compact 只作用材料、配件、转换候选等浏览型卡片，operation tabs 与主提交语义保持完整，切换零业务流量。commit 的 busy/write capability 必须覆盖 inventory refresh 整段，并等待新 lease 后才开放下一交互。当前自动命令和证据口径统一见 [验证矩阵](../agentsDoc/testing-guide.md)；本文不再复制已失效的 per-viewport case 数、Host 全量数或旧 asLoader 哈希。

A3 物品身份治理把 Equipment Tuning、KShop、Crafting 与 NPC Shop 的共同叶语义固定为 `name/itemName=内部规则身份`、`displayName=玩家文案`、`icon=资产键`，但不建立通用领域状态机。AS2 可以在 authority projection 内为旧数据补齐缺少的展示/图标字段；越过该边界后 Host 必须按 cmd exact normalize、重建与 sanitize，Web 再独立 strict proof，不能用内部名二次猜 display/icon。KShop catalog 的 `item/displayname/icon`、KShop/NPC 的领域 tooltip 旧 profile 与 Crafting tooltip 的一次 `displayname→displayName` 都是命令特定 adapter，不是共享 alias；KShop current contract mode 为 `normalized-domainless-owner-rebuilt`。**2026-08-03 历史状态（Superseded / Reopened）为 A3 Awaiting live E2E / Automated + semantic GO / NOT CLOSED**：此前 Equipment canonical before/after/materials 与 fresh snapshot 深绑定、KShop authoritative cart、KShop delivered catalog 三元组/unitPrice 对账、NPC ordinary `sell` 退役四项 High 已在 current tree 闭合；普通出售的现役精确写面只保留 `tradePreview→tradeCommit`。共享 fixture/validator 只证明三元组叶节点和 19/13/3 数据分歧，不接管 selector、token、价格、容量或 reconcile。fresh `PG-IDENTITY-TRIPLE -CompileAs2`、连续两轮串行 Launcher 全量与独立 runner 审阅继续只作历史证据。**2026-08-04 历史重开状态（Superseded / historical）为 A3 Audit Reopened / CHANGES_REQUIRED / LIVE_BLOCKED / NOT_DEPLOYED**：Host 只在 exact active name/instance 匹配且 `DoClose()` 返回后写 `event=panel_exact_close_completed panel=<known> panelInstanceId=<exact>`；Equipment、KShop、Crafting、NPC 当时必须先各自从 canonical bootstrap 封闭 actual-loaded production closure、完整 Host 记录集合、输入/退出与重启读回，并由不同作者终审全部 GO，之后才准构建同一 current-tree isolated candidate。四条同候选 live receipt 与终局独立复核完成前，当时不得改为 Closed 或准入 A4；唯一入口与通过标准见 [验证矩阵](../agentsDoc/testing-guide.md) 的 `PG-IDENTITY-TRIPLE`。该中间阻断已由验证矩阵顶部的 2026-08-05 current closure 覆盖，不得再据此把当前 A3 标为 `LIVE_BLOCKED` 或阻断 A4–A6。

2026-08-05 Crafting v8 作者侧进一步把“产物身份”扩为完整物理产物合同：AS2 preview 的 `acceptedPlan.outputPrototype` 由真实 `BaseItem` 严格投影，commit 在写前重算计划、扣料后核对真实投递，并以 `outputReceipt` 返回完整 Inventory item/confirm；Host 与 Web 复用 Inventory 的严格叶 validator，离线 verifier 只由 prototype + receipt 建立预期，不再拿 after-readback 自证。该轮 fresh canonical 为 `265/265`，相邻门为 AS2 structure `384/384`、Inventory Web `70/70`、Crafting Web `20/20`、Host focused `286/286`，三视口 harness 各为 `120/120 + 15/15 + 8/8 + 10/10`。随后共享 `InventoryRuntime` 补齐完整物理 surface 刷新后的当前可见页投影，定向回归为 `71/71`；KShop current canonical 为 `241/241`、journal `79ba38f9…`，browser `146/146`、Host Shop/Inventory focused `399/399`。由于共享生产字节变化，既有 Equipment independent GO 以及 NPC/Crafting/KShop 的旧作者闭包在该时点均只保留为 `Superseded / Reopened` 历史，四面必须在同一当前树重跑并重新终审；当时 A3 继续 `CHANGES_REQUIRED / LIVE_BLOCKED / NOT_DEPLOYED`，不得准入 candidate/live 或 A4。四面后续已按同一 current tree 重跑并由验证矩阵顶部的 2026-08-05 current closure 关闭，本段不再恢复当前阻断。
...
B5 Materials exact handoff 的 fresh 自动门为：Launcher 定向 `249/249`、全量 `1707/1707`；tracked item-panel run `bec9c608004d48e0bcb18a7f1c0f5708` 为 EquipmentInventory `28/28`、NPC `46/46`、Inventory `144/144`、Crafting `36/36`，Compiler Errors/Warnings `0/0`、32K retry `0`。direct HUD 与 Character settled 都先建立独立 material wait，再发送 Host opaque `openRequestId`；只消费 AS2 顶层 exact echo 与 Host admission。合法无 nonce ordinary Web materials envelope 仅在没有 armed intent/material wait 时准入；armed/wait 存在时 missing nonce 拒绝但保留 wait；显式畸形 token 在 AS2 零发送，nonce-bearing wrong/near-match 终止当前目标，迟到/重复零打开；Character 退场后失败至多一次 rollback。本批只刷新 TestLoader，未 publish `scripts/asLoader.swf`，也未构建 candidate 或部署。
...
9118 冻结树的 B7 当时把 `PreparationNavigationV1` 的代码默认与随仓配置切为 `true`，并以同一值接线 Native/legacy HUD、Router、WebOverlay recovery 与 Build Web initData。on 时 Build exact-set 为 `preparation-menu | stats | help | close`，Skills return 与三目标 rollback 聚焦 `preparation-menu`；当时显式 `false`（或配置项存在但值非法）会整体恢复旧 HUD/header 与 `skills` focus。当前 cut 仍保留这个默认-on gate 与成套 off 路径；显式 `false` 或配置项存在但值非法时恢复旧导航 toolbar/header/focus presentation。Web 只接受严格 optional bool 与固定六项映射，错误类型或 gate/focus 不配对直接拒绝，gate 不参与 route authorization。带 reason 的 Build busy header action 保持 `aria-disabled` 与真实激活解释路径，保留基线 accessible name、把生产 capture 先读取的 cue 切为 `error`，显示对应动作 reason（Close 明示完成后才能关闭工作台）后阻止原 handler并在恢复时还原；无 reason 才使用原生 `disabled`。standalone tuning 的兄弟切换统一写“战备箱与背包”，不臆造“返回收纳”历史。fresh 自动门为 preparation leaf `10/10`、inventory modules `27/27`、Character production 三视口合计 `831/831` + hidden/lifecycle `12/12` + normal/reduced 菜单矩阵 `18/18`、Tuning 三视口 `96/96`、KShop `134/134`、Launcher focused `237/237` 与全量 `1720/1720`；busy action 使用真实 Edge 内 DOM `.click()`，不冒充 trusted pointer/Enter/Space。这是 B7 当批 promotion 前的历史快照：当批未修改 `.as`/Flash/SWF，也未构建 candidate、执行游戏 E2E、promotion、部署或标准入口验收；实现字节后来随 `9118eb…` 正式部署，但本轮 formal smoke 仍未执行 B7/Character 游戏旅程；现役 off 路径仍由 AppConfig、PreparationNavigationCatalog 与 Router 回归守住，但只恢复旧导航呈现，不恢复任何退役 AS2 全屏 UI。

合成工作台生产接入为独立 `crafting` domain：配方使用 `snapshot/preview/tooltip/commit`，材料目录使用只读 `materials/materialDetail`，Flash 统一回 `crafting_response`。`LauncherCommandRouter` 对配方只接受 12 个已知分类并重建 `{mode:"runtime",category,source,debug:false}`；Native HUD `MATERIALS` 只在 exact correlated admission 后重建 `{mode:"runtime",view:"materials",source:"nativehud_materials",debug:false}`，不再打开旧 material-only 页面且没有 fallback。`CraftingTask` 只接受分类、recipeIndex、严格整数 `craftCount=1..99`、itemName 或 opaque `expectedCraftToken`，不透传 Web 价格/材料/产物。snapshot 为每条配方执行一次 `craftCount=1` 权威计划，返回严格配对的 `canCraftOne/availability`，不运行 `maxCraftCount` 二分；Web 用它显示卡片状态、可合成计数和本地“只看可合成”，但提交仍只信 preview/token。Flash 只对堆叠产物且无装备素材的配方开放批量，返回 `batchEligible/maxCraftCount`，并把份数、总材料、总产出和双货币总价绑定进一次性 token；装备产物/装备素材仍固定单份。材料目录由 `crafting-materials.js` 消费 `ItemUtil.materialDict`、材料注释、`ItemObtainIndex` 与 `SynthesisIndex` 的 AS2 投影，展示持有量、怪物/关卡来源和配方/装备用途，Web 不扫描 XML 或推断掉率；视图固定为 `44:56` 真双栏档案，目录和详情独立滚动，选择按稳定材料 key 原位更新。v2 默认按 authored `archiveOrder`，以 `ItemFilter.buildMany + FilterNavigator` 让同一材料同时进入“类型 / 用途”多路径：装备插件在类型下展开档级/适用范围/定位三轴，用途下展开配方分类/受控直接用途/兜底；敌人/关卡按稳定来源身份分组，但 `variants[]` 的全部 occurrence 必须按 `occurrenceIndex` 展示。v1 success 只进入 `legacy_limited` 并隐藏 taxonomy/breadcrumb，不冒充 authored 顺序、分类或完整 occurrence。Web 将每次成功 preview 保存为同 category/recipeIndex/craftCount 的 checkpoint；普通读 timeout/disconnected/畸形恢复 checkpoint，权威分歧刷新 snapshot。只有 AS2 明示 reconcile，或已投递 commit 的超时、断线、drop/畸形结果进入 `needs_reconcile`；同步未投递失败保持可操作，此后受门控写入返回 `reconcile_required`；只有同配方结构完整的成功 preview 能解除写门，snapshot 不能。配方入口为 `modules/crafting.js`，复用 `Workbench.DualPaneShell` 与共享 `ItemFilter catalog` 类别/套装视觉契约，生产 `#panel-content` 使用全 anchor、1024×576 scale-shell 等比铺满，内部以约 60:40 双栏和窄轨原生滚动呈现；材料不足时先 snapshot 撤销 token，再本地切入 `workbench profile=battlebox`，返回仅保留 category/recipeIndex/craftCount 及展示筛选意图并强制重新核算。材料档案另以中性 `WorkbenchComponents.Dropdown` 提供“档案顺序 / 持有数 / 名称 / 用途数”，fresh open 恢复 authored 顺序，v1 隐藏排序；壳层固定显示不随本地约束变化的“持有种类 x / y”，加载/失败为 `— / —`。档案摘要与已发现来源分区，目录/详情错误互不连坐，真空目录与筛选空分别给出重试/清除动作；网格卡继续经 production scoped `PanelTooltip` 使用同材料 key 的 basic→rich 缓存。无头验证入口为 `node tools/run-crafting-harness.js`，当前三视口均为 baseline **150/150** + `PG-CRAFT-CURRENT-COVERAGE` **15/15** + `PG-INVENTORY-FAULT` **8/8** + `PG-CRAFTING-IDENTITY` **10/10** + `PG-MATERIALS-V1` **6/6** + `PG-MATERIALS-SESSION-LOCK` **9/9** + `PG-MATERIAL-RECIPE-JUMP` **26/26** + `PG-MATERIAL-SHOP-NAVIGATION` **12/12** + `PG-MATERIAL-INFRASTRUCTURE` **9/9**，并覆盖材料直达/目录/详情/来源用途、224 条 fixture 的目录可达性、排序/metric/空态/错误分域/详情重试/tooltip cache/0.5 PanelScale 几何、exact 配方/装备动作、重复重建、跨面板路由后 `ResizeObserver` 全量断连与检视器 RAF 生命周期、普通武器动效、双刀/疾影 strict 2/2 holder 组合、battle rig 漂移整组回退，以及纸娃娃图片加载失败图标回退，并固定证明共享模态先按焦点栈处理，只有 `reason="escape"` 继续消费材料排序菜单/搜索/树层级；`reason="backdrop"|"toggle"` 不伪装成 Esc，最终与普通 `×` 一样只关闭 exact crafting owner，缺失 reason 才按 legacy `escape` 兼容。

材料→NPCShop 使用 domain mux 外的 dedicated flat `open_npc_shop`，返回使用 `return_crafting_materials`；Web 的 material/shop/index/参考价仅是 hint。`MaterialShopNavigationCoordinator` 是唯一 transition deadline、settlement witness 与 Crafting/NpcShop/Inventory lease owner，`MaterialShopAccessTask` 仅维持 Host→AS2 fid/correlation；AS2 每次点击以 current material snapshot、exact ItemObtainIndex occurrence 与 raw/live NPC catalog 三重重授权。成功才由 `PreparedPanelReplace/TryReplacePanelExact` 原子提交目标 admission、源退休与一次性返回 capability；pre-commit 失败保留源，post-commit lazy/register/mount 失败只 exact close 目标且不复活源。NPCShop 以 index+name 双键定位，navigation highlight 与购买选择分离，打开/聚焦零自动预览、加购或提交；locked/达到上限仍可导航，但 fresh NPC authority 继续阻止购买。普通 `button|escape|backdrop|toggle` 关闭不消费为返回，Web 也不把 `postMessage` 成功视为 close commit。当前 scoped 证据为 Crafting runtime **30/30**、Panel runtime **40/40**、NPCShop **129/129 + 21/21 + 2/2**；这仍不等于真实 WebView2→Flash、candidate、promotion 或部署。

WebOverlay 的 `MoveFocus(Programmatic)` 只把焦点交给 WebView，并不冻结材料页里的 exact DOM active target。材料 v2 的首次非空成功 snapshot 因此在完成默认选择/详情渲染后，仅当 `activeElement` 仍为空、`body` 或 `documentElement` 时，用现有 `FilterNavigator.focusPath([])` 聚焦 exact “全部材料”根按钮；已有 DOM 焦点、v1 或空目录均不抢焦点。A5 current Agent Runtime 控制计划为 v4/24 步：首个 structured `panel.open materials` 后以 `input.press_key` 覆盖默认档案/排序/树/网格/详情，exact 跳到“军用帆布”→“属性武器” index 0 “二阶复合防御组件”，抓取可见终态后 Escape、重开材料，再继续唯一“食用油” q1 购买。current evidence v9 必须把 16 个 key intent 机械绑定 trusted action receipt、同 session ledger 及 exact recipe capture；`purchase-authorization.v2` 同时绑定 exact runId、applicabilitySha256 与 `{厨师,24,食用油}`，generic/冻结 t1903 才允许 v1。Escape 与 ordinary close 不能由 dispatch receipt 单独代签：两者必须在 first session 紧邻后继 structured Materials open，并绑定该 open 后首个 visible WebOverlay observation、content.read 与 successor PNG；Host stable-idle admission证明旧 tracked visual 已退出。独立 PNG reviewer 只签可见终态，不能代签键盘、焦点、关闭或重开 provenance。
...
- **双栏工作台治理门**：`node tools/audit-workbench-ui.js --strict-warnings` 做结构化静态审计；`node tools/run-workbench-visual-atlas.js --strict-warnings [--shot-dir=tmp/workbench-visual-atlas]` 跑现役 66 场景（48 个 shared synthetic 笛卡尔积 + 18 个 Arena 生产闭包挑战→结算两阶段）；懒加载中途失败还须跑 `node tools/test-panel-lazy-loader-browser.js`。视觉与工程规则见 [workbench-ui-system.md](../agentsDoc/workbench-ui-system.md)，它不替代各 feature harness 或游戏内手测。当前 fresh 基线：atlas `66/66`、物品格矩阵 `20/20`、LazyLoader 真实 Edge 恢复 `17/17`、静态审计 `0 error / 0 warning`、ratchet `66/66`。这些仍只是当前源码树证据，不外推为 candidate、promotion 或业务 E2E。
...
- **Stage Select manifest / audit / harness**：`node tools/export-stage-select-manifest.js --summary`、`node tools/audit-stage-select-layout.js --json`、`node tools/audit-diplomacy-stage-select-links.js --json`、browser harness `web/modules/stage-select/dev/harness.html?qa=1`（in-page `dev/qa-suite.js`，46 用例），也可用 `node tools/run-stage-select-harness.js --browser edge` 跑 headless QA
  - 从 `flashswf/UI/选关界面/LIBRARY/选关界面UI/选关界面 1024&#042576.xml` 导出 `StageSelectData` 所用 manifest；Stage 2 通过 `StageSelectTask` / `StageSelectPanelService` 接入真实解锁 snapshot、`StageInfoDict` 关卡简介/限制词条/任务提示数据、普通难度进关、外交地图直达、副本任务直达重定向 Web `tasks` 副本 tab、runtime 页内 frame 同步与关闭语义。关卡预览链为四级：外部 PNG → `Symbol 3274` 内部命名帧 → P4-b 派生帧（`previewSource=derived`，`node tools/derive-stage-select-previews.js` 按 关卡名→StageInfo→单关 XML 首个 Background SWF→FFDec 主视觉帧→异常帧拒收→细节滑窗裁缩 161×69 生成，宁可保留 default 不出假预览）→ 默认预览帧兜底，layout audit 要求 `previewMissing=0`。P2 起普通关卡节点支持点击 / Enter/Space 选中并打开 pinned 决策检查器（复用 hover 卡数据链、键盘难度按钮 ≥40px、锁定原因可读、Esc 分层「菜单 > 检查器 > 面板」、方向键几何最近邻导航、选中态跨 snapshot 重建按 stage id 恢复；2026-08-16 打磨批起检查器内 ←/→ 在同排难度按钮间循环移焦（焦点即选中高亮，各难度自身色系发光）、Enter/Space 提交当前焦点难度、↑/↓ 无操作、检查器开态时节点方向键截停并回引检查器，检查器皮肤与选中光环与 hover 卡 / 任务闪光标记环同族化），hover 卡一点即进的鼠标快捷路径保持不变；2026-08-16 起 snapshot `stageDetails[]` 新增恒在 `lockReason` 字段（AS2 `StageSelectPanelService.buildLockReason` 按 `UnlockCondition` 主路径 + `AltUnlockCondition` 替代路径生成逐关中文原因，仅锁定时非空，C# 回包整体透传零改动），锁定节点检查器显示具体原因、空串回退通用文案
...
- **Icon bake offline（FFDec）**：`python tools/bake-icons-offline.py --scope all --dry-run --report tmp/icon-bake-offline-dry-run-report.json`
  ...
  - 默认目标目录为 `launcher/web/icons`；默认开启既有图标布局保护：已有图标文件出现非微小差异时记录 `layoutProtected` 并保留旧文件。静态 f1 现在可以写入 `bakeProvenance`；只有上次权威渲染配方与当前一致、盘上可见 RGBA（完全透明像素的隐藏 RGB 统一归零）仍精确匹配该 provenance，且源 SWF digest 与归一化渲染 digest 同时变化时，才记录 `sourceAwareRefreshed` 并自动更新。`sourceArtifactMtimeNs` 只作审计线索，不单独授权覆盖；首次建立基线、渲染配方变化、手工改过输出或缺 provenance 仍保守拦截，需针对候选显式 `--force-overwrite-existing`；显式强制或 source-aware 授权会连微差一并真实落盘，避免报告已更新但 provenance 仍对应候选而非产物；可用 `--no-source-aware-refresh` 完全关闭这条自动路径。默认 `--image-format webp`；PNG 仅用于 FFDec 中间帧、`--image-format png` legacy 路径或少量 layered/nested 兼容产物。只审 XML/source map 覆盖率时可用 `--resolve-only`，它不启动 FFDec，只刷新 `unresolvedSummary` 与冲突候选 `conflictSources`。报告必须关注 `unresolved`（含 `reason=conflict|missing_asset`）、`export_errors`、`missing_frame`、`layoutProtected`、`sourceAwareRefreshed`、`f1Profile`。FFDec 导出后还会扫描 symbol 实际 PNG 帧并写 `animationAudit`：`sourceFrameCount/uniqueFrameImages/duplicateFrameRefs/timelineFrameEntries/timelineCompressedFrameRefs/longestHoldFrames/animatedCandidate`，用于区分真动态图标、多帧静态 hold 和后续可写 `timelineFrames` 的空间收益；同轮还用 `swf2xml` + `script:as` 建 sprite 图谱，补充 `nestedAnimatedDescendantCount/maxNestedDescendantFrameCount/sampleNestedDescendants` 与 `nestedStoppedDescendantCount`，用于发现父级单帧但内部子 MovieClip 自播放的图标。全量清理只允许 `--scope all` 且无 `--limit/--name` 时加 `--purge`。
  ...
  - 装备调制插件不得默认“内部 mod 名 = 显示名 = 图标名”；AS2 snapshot 显式投影 `itemName/displayName/icon`，Web 只用 `itemName` 做交易键，候选格、已安装槽与富 tooltip 都使用独立图标键。`node tools/audit-web-item-icon-closure.js` 已进入 release policy，会沿 `equipment_mods/list.xml -> data/items -> icons/manifest.json -> 图片文件` 检查全量闭包。
...
- **Dressup paper-doll offline（data + XFL rig）**：`python tools/bake-dressup-offline.py --export-assets`
  ...
  - `report.missingSourceAudit` 是剩余缺口的来源审计：按当前 `asset_source_map.xml` 与同名 XFL/XML 扫描分类缺失 skin key，并在 `entries[*].references` 反查产生该 key 的 item、性别字段、数据文件、佣兵装备引用数量和样本。现役离线策略明确禁用 `auto_opposite_gender`：只存在异性素材的 key 保持 `covered=false`，renderer 由当前性别 holder 的 `basic` fallback 承接，不把异性身体/肢体伪装成精确覆盖；经人工确认可共用的 alias 仍可写入 `COMPAT_DRESSUP_ALIASES`，但每项必须保留显式 `reason`。上行 2026-06-19 数字只作历史快照，现役 manifest/report 基线为 items `1292`、skinKeys `2853`、covered/export `2712`、missing `141`、manual alias `9`、auto alias `0`、battle states `7`、battle audit error `0`；4 个仅服务 `data/units/units.json#235` 的远古诛神 virtual item 精确引入 10 个已登记 source-map skinKey，定向 FFDec 导出 10 个 PNG（总计 `2,536,431` bytes），最终 exported frames `2985`、unique frame images `2822`，不得映射到普通诛神素材；missing 分类为 `opposite_gender_only=135`、`exact_xml_without_as_linkage=3`、`no_matching_source=3`。全量佣兵闭包为 resolved parts `3253`、当前性别 holder basic fallback `12`、failures `0`。
...
# 性能收益：panel 打开期 α blend 成本下降；panel 关闭后 DoFullIdleSuspend 会 SW_HIDE + 冻结 timer/UI 派发
# 运行态 WebView2（TrySuspendAsync 已退役——实测 suspend 恢复后可能残留 hidden 小视口黑底）。
useNativeHud = true
...
- 所有 panel 打开统一进 [PanelHostController.cs](src/Guardian/PanelHostController.cs) 的 command queue：[FlashSnapshot.cs](src/Guardian/FlashSnapshot.cs).Capture → ComposeBackdrop → [NativePanelBackdrop.cs](src/Guardian/NativePanelBackdrop.cs) 显示 → [NativeHudOverlay.cs](src/Guardian/NativeHudOverlay.cs)（含 ToastWidget+NotchWidget）整层 Suspend → WebOverlayForm.ResumeForPanel（去 `WS_EX_LAYERED|WS_EX_TRANSPARENT`、`TransparencyKey=Empty`、`DefaultBackgroundColor=Black`、SetWindowPos `HWND_TOP|SWP_FRAMECHANGED` 至 [PanelLayoutCatalog.cs](src/Guardian/PanelLayoutCatalog.cs) 决定的矩形；show 后先做 `KickWebViewCompositor` 1px bounds 抖动——同值 sync 在 WebView2 内部被合并，无法唤醒 SW_HIDE/最小化恢复后 wedge 的 DirectComposition 合成链，抖动保证至少一次真实 commit——再正常 sync 回目标几何）→ PostToWeb `panel_viewport_set` → InputShield 进 telemetry → 顶置 HitNumber/Cursor → 启用 ESC
- panel 关闭（useNativeHud=true 路径）：WebOverlayForm.ForceIdleState 走 `DoFullIdleSuspend` —— `SuspendWebTimers` 停 fps/audio/position-settle/reload timer + `_frozenForIdle=true` 冻结 HandleUiData 仅缓存不 ExecScript + `ShowWindow SW_HIDE` + 恢复 `WS_EX_LAYERED|WS_EX_TRANSPARENT` + `HWND_NOTOPMOST` 防御 + `TransparencyKey/transparent BG` 复位；`CoreWebView2.TrySuspendAsync` 已退役（实测恢复后可能残留 hidden 小视口黑底），SW_HIDE + timer freeze 已足够移除 idle 合成成本；NativeHud Resume 重新评估 widget union（toast/notch 一并复显）。下次 `ResumeForPanel` 仍会调一次 `CoreWebView2.Resume()`（未 suspend 时是无害 no-op）。useNativeHud=false 仍走 `DoSoftIdleRestore` 仅恢复样式拉回 anchor 矩形（保留 web HUD 显示）
- 最小化恢复补偿：owned overlay 随 GuardianForm 最小化被 OS 一并隐藏/重显，这一路径不产生任何 WM_SIZE 视口刷新以外的 WebView2 动作，而恢复序列（bounds/zoom/IsVisible sync）全是值幂等的——合成链一旦 wedge 就表现为 panel 只剩 `DefaultBackgroundColor=Black` 黑底但 JS 正常。两层修复：GuardianForm 的 WM_SIZE 在 `SIZE_MINIMIZED→SIZE_RESTORED` 时也 `ScheduleViewportRefresh("guardian_wm_size_restored_from_minimized")`（与 maximize→restore 对称，重测 Flash 与 overlay）；`ResumeForPanel` 的 `KickWebViewCompositor` 1px 抖动保证每次 panel 打开至少产生一次真实 composition commit，对所有 wedge 成因（SW_HIDE / 最小化 / 其他 OS 级隐藏）自愈。
...
- **Native HUD 默认组成**：[NotchWidget](src/Guardian/Hud/NotchWidget.cs) 接管 web `#notch`。`PreparationNavigationV1` 默认 `true`（配置缺失也取默认）时，Native 为“游戏 / 整备 / 辅助 / 系统”四行，legacy launcher 至少为“游戏 / 整备”两行；整备 frozen tuple 固定 `装备 / 战备箱 / 装备调制 / 技能 / 材料 / 情报`，battlebox/tuning 未解锁时仍结构可见并显示统一 reason。显式 `false` 或配置项存在但非法时，Native/legacy launcher 必须成套恢复旧 toolbar、Build header 与 `skills` focus；这只是 navigation presentation 回退，所有生产 UI 入口仍 Web-only fail-closed，绝不恢复退役 AS2 全屏 UI；各目标的 Host/AS2 授权协议保持不变。[AudioHudState](src/Guardian/Hud/AudioHudState.cs) 以最高 10Hz 采样 BGM L/R peak；“系统 → 其他”使用“控制 / 测试 / 工具”分类页并随刘海收起。[ToastWidget](src/Guardian/Hud/ToastWidget.cs) 承载 toast，[RightContextWidget](src/Guardian/Hud/RightContextWidget.cs) 承载 252px 六入口行、条件槽和可选地图预览，[SafeExitPanelWidget](src/Guardian/Hud/SafeExitPanelWidget.cs) 只消费 exact transaction owner，[ComboWidget](src/Guardian/Hud/ComboWidget.cs) 承载搓招输入与命中动画。地图 shared renderer 复用 [MapHudWidget](src/Guardian/Hud/MapHudWidget.cs) 的 WebP alpha silhouette/blocks fallback，并只异步预热当前 outline 工作集。
...
- 性能收益：panel 打开期 α blend 成本下降（panel 矩形小 + opaque）；idle 期 `DoFullIdleSuspend` 整个 SW_HIDE WebView2 + 冻结 timer/派发 → 拿回 ~15pp DWM α 地板（所有常驻 HUD 已迁到 C# widget，玩家在 panel 关闭期间仍能看到 notch / toast / 货币 / combo / RightContext 右侧 cluster）
...
`useNativeHud=true` 的视觉/功能 gate 以当前收敛布局为准。`PreparationNavigationV1` 默认 `true`（缺失取默认）时，刘海与右侧动作行共享居中 `32px` 顶行，展开的“游戏 / 整备 / 辅助 / 系统”四行不得超过安全宽度；显式 `false` 或非法配置的负门必须成套恢复旧三行、七项游戏行、Build header 与 `skills` focus，同时继续证明各生产入口 Web-only fail-closed、零退役 AS2 UI 回退。所有绘制硬裁切在刘海矩形内，“系统 → 其他”的分类页随刘海收起，不能残留独立长菜单。常驻组件使用黑底、直角发丝外框、内侧明暗压边和技能槽式角标；hover/pressed/active/danger 主要通过底色、文字和角标表达，不得回退为粗白框、圆角/切角外壳或大圆角半透明卡片。展开/收起动画的 HWND placement 与 bitmap resize 只允许端点级变化，透明 CompositeBounds 必须 click-through。

右侧动作行固定为 `right:48px / width:252px / 3×50px+3×34px`。第二行只在 exact `contextHint / actionableNotice / transactionDecision` owner 存在时占用；无 owner 时不保留空框，地图直接贴在动作行下方。SafeExit Done 态无操作 5 秒自动收起，悬停确认按钮时暂停，自动收起不得触发退出；Failed 态只允许取消/重试，raw 或重放 `EXIT_CONFIRM` 不得触发普通退出。地图 `compact=64px / expanded=112px`；卡片 `＋/－` 只切 `compact ↔ expanded`，刘海“辅助 → 地图开关”只切显示/关闭，恢复 `auto` 走设置/偏好写入。人工覆盖仍包括 1024×576、1600×900、1920×1080、4:3 letterbox 与 100%–175% DPI，以及刘海/右槽/SafeExit/map/combo/toast/panel 生命周期；当前默认-on GUI 旅程已由人工验收通过，但这仍不能替代新的 candidate、promotion 或标准入口验收；显式 off/非法配置的旧导航 presentation 仍属于自动准入，不得把它误解为恢复任何 AS2 全屏 UI。
...
### PM19 质数幻方背景层（modules/bg-gl/）

欢迎页背景层栈在 `.bg-photo` 与 `.bg-tint` 之间多了一层 `<canvas id="bg-gl">`：19×19 完整质数幻方矩阵（每行/列/双对角线和恒为 190000361）作电子战氛围层。它是 web 侧**首个 ESM 模块**（`type="module"`，自包含、无构建步骤、无第三方依赖），相位由真实启动事件驱动而非定时演示：

| 事件 | 相位 |
|------|------|
| 页面加载 | SCRAMBLE 频谱扰动（环境底态，8–12Hz 换合法棋盘） |
| 首个 `list_resp` | ROW ACQUISITION → COLUMN COHERENCE → 环境态（每 6–10s 插播行/列扫描） |
| 点「确认」/ `Spawning`…`Embedding`/`WaitingGameReady` | DIAGONAL LOCK 双对角锁定（扫满 1.8s 后进保持期行波循环，**任何事件不截断扫描**） |
| `Ready` / `flash_ready` | 扫描在播则 `syncPending` 挂起等播完 → MATRIX SYNCHRONIZED（保持到 panel swap）；否则直接同步 |
...
对齐游戏内 web 栈设计语言的一轮翻新（完整设计与施工记录见 `docs/启动引导-PM19质数幻方背景-设计与施工-2026-08-05.md`）：

- **Token 体系统一**：head 直引 `css/workbench/tokens.css`，welcome.css/bootstrap.css 全部 transition 走 `--wb-motion-*` / `--wb-ease-*` token；`@media (prefers-reduced-motion: reduce)` 兜底（JS 侧视图切换同步硬切）；`--dls` 族与 `--dls-crystal` 族数值同源不改名，`--rust`/`--bone` 作欢迎页风味色保留（注释标注游戏内语义对应）。
...
**Flash reveal watchdog**：生产默认 deadline 为从 `GameLaunchFlow` 进入 Ready 起算的有界 **45,000ms**。原 20,000ms 已覆盖保留日志中约 15.68s 的样本，但 A1 精确隔离候选冷启动直到约 30.85s 才收到真实 title receipt，并被 20s watchdog 正确 fail-closed；45s 为该实测保留有界余量。内部构造仅允许测试注入更短 deadline。watchdog 仍只负责旧 SWF/Flash hang 时的玩家可见降级 reveal，**永远不是** `bootstrap_reveal_ready` 成功回执：watchdog 先到的 attempt 必须 fail-closed，迟到 title 继续记为 `not waiting, ignored`，不得续用该 attempt 进档。Reset 在入场时同步清除 Flash wait flag 并取消 timer，使已排队 callback 也由现有 guard 丢弃。
...
### CF7 Agent Runtime / Wings Network（F7 历史 + F8 历史正式发布；当前由 2026-08-06 release 继承）

Agent Runtime 的 wire 真源是 `launcher/contracts/agent-runtime/v1/`，C# 对照与测试入口见 [AgentRuntime/Contracts](src/AgentRuntime/Contracts/README.md)。身份与授权固定为显式 `consumerType`、`agentId`、`principal`、`connectionId`、Core `sessionId/lifecycleId` 与最小 capability/grant 交集；Windows `CurrentUserOnly` 只证明当前用户，不能替代同 Windows session/elevation 的独立 OS peer token 校验。所有 selector 与操作还绑定 exact process incarnation（可执行路径、PID、进程启动时间及 HWND/owner 关系），`activePanel` 固定为 `{name,instanceId,targetId}`。

`LeaseDescriptor.purpose` 是必填字段，`renewAfter` 可选；shutdown descriptor 必须省略 `renewAfter`。每个 action 的唯一绝对 deadline 从完整 CF7A request frame 收到时开始，覆盖 UTF-8/JSON/参数解析、capability/admission、scheduler、performer、response writer lock，以及 JSON 与可选 binary 等全部 response-frame `WriteAsync`；任何阶段不得重置预算。

Web 导航一开始就递增 document generation，旧 generation 的 observation/input/action receipt 一律失效；`window.state` 与 `window.activate` 在 type-level wire registry 中仍是不同操作，但 applicability 条目只是潜在能力，不能替代当前 session capability 与 surface mode 的实际交集。F8 production 不授予、也不在 MCP `tools/list` 中列出 `window.activate`，production activator map 精确为空；客户端不得把仍存在的 schema/method 名解释成可调用入口。`business_modal` selector 在 production 恒为空；内部 `BusinessModal` 是 human-only security surface，既不可发现也不可输入。

F8 production surface composition 固定为：嵌入的 Flash `observationModes=[]` 且 `inputModes=[]`，只暴露身份、几何与可见性 metadata；即使 principal 拥有 `pixels` grant，对该 Flash 调用 `observation.capture` 也必须精确失败为 `unsupported_for_surface`，不得回退 snapshot keyframe、父窗口裁剪或 native input。Launcher、WebOverlay、NativeHud 的 production 像素观察模式只有 `window_graphics_capture`（WGC）；其中 WebOverlay / NativeHud 的其他 type-level input mode 不能外推为本次验收或 Flash 的授权。

F8 `panel.open` 是独立的 Host-owned structured action，不是 GUI/native input。lease acquire 必须使用 `kind:"structured_action"`，scope 精确解析一个当前 `RuntimeOwned` Launcher target，capability/operation 仅 `panel.open`、TTL≤30 秒、`maximumActions:1`、no renew；Host 只允许 `help`、`map`、`tasks`、`team`、`jukebox`、`materials`。`materials` 不直开 Web panel，它复用固定 `nativehud_materials` 路由发送 AS2 `openMaterialUI` 并等待 exact nonce/tuple。`input_dispatched / broker_dispatch` receipt 只证明命令交付或同步 exact admission，面板可见仍须 fresh WebOverlay grant + WGC observation 独立确认。整个路径不绑定 `NativeInputGuard`、不发送 mouse/keyboard packet，也不允许 `gui_input` downgrade。PanelHost、Loot 专用 coordinator 与 Router fallback 等全部 production panel instance producer 都使用 CSPRNG 生成至少 144-bit entropy 的 opaque ID；prefix 只用于诊断，不承载权限或顺序语义。

`trace.export` 仅供显式 enrolled developer，在 `DeveloperInteractive` 会话同时满足 `trace.export + observation.export + consentPurpose`、授权 grant 的 `data.export/allowExport` 与 consent receipt 时开放。导出只含 Runtime-owned 数据，Runtime 以 8 MiB 上限、owner staging → 共同 process-incarnation pending marker 与同目录 atomic move 写 JSONL；可控失败只清理 owned files，删除受阻则保留 marker，由后续调用在 owner process incarnation 已确定死亡或同进程 transaction 退出后重试。marker 删除是发布线性化点，单文件 move 不提供 audit/filesystem 跨资源的“任一失败零残留”保证；marker 删除前写入的单条 `trace_export_completed` 只是 publish-prepared audit，不是 artifact 已发布证明，若随后收尾失败由同 artifact 的 failed fact 补偿。wire 只返回 artifact 名称/ID，不暴露任意路径。

Hair 外观写采用 `expectedCurrentHair` CAS、focused runner、零 replay 与关闭/重开隔离。同一 Core session/lifecycle 内的 service restart 只能从 durable service/store 重建事务状态并继续 reconcile；它不能恢复或重新生成旧事务实例的原始 token，新 service/transaction 实例必须 fail closed。Core restart 还会改变 lifecycle 并拒绝旧事务。unknown receipt 不携带原始 token：token 只保存在同一个 `HairAppearanceModifierTransaction` 实例的 lifecycle-local escrow；Gateway 必须用 exact `connectionId + principal + session/lifecycle + target` 对 preview store 验证，由 reconcile authority 证明已提交后，才按 `transactionId/previewHash/expiry` 单次消费。同一实例的 terminal known-committed receipt 可以返回 token；不同连接、新 transaction 或 Core restart 永不交付。恢复时先对账 durable state，确认已提交就收敛为 `Committed`；无法确认时保留 commit receipt，且绝不重发 commit。

`session.shutdown` 不是普通 GUI input lease：它只允许 `DeveloperInteractive` / `UnattendedTest`；请求的 exact target scope 必须恰含当前一个 `RuntimeOwned` Launcher target，这是本次 scope 的 cardinality，不能据此推导整个 session 全局只有一个 Launcher surface。专用 shutdown lease 的唯一 capability/operation 为 `session.shutdown`，TTL≤30 秒、one action、no renew。`PlayerAssist` 只有语法有效、已认证且通过 connection/principal/session/method capability/target authority 等先行门、实际到达 issuance policy 的 acquire 才返回 `consent_required`；畸形、越权或直接 action 可更早失败。前序 mutation/restore-token receipt 未收束时 shutdown 不能越过。

WriteLeaseBroker 的 live table 只保存 active 或仍在 action execution / response delivery 的 lease；terminal tombstone 按 FIFO 最多 256 条，committed shutdown session 使用独立 64 条 latch，容量溢出后转为全局 fail-closed。tombstone eviction 永远不能遗忘 committed shutdown 并重新开放 writer。renew/release 的失败 cleanup 只允许 exact active owner；attacker、已 consumed 或仍 pending 的 lease 都不能借失败路径 untrack/cancel/release 他人资源。同 session execution reservation 只归成功 consume 的 action owner，并保持到 JSON 与可选 binary 等全部 CF7A response frames 完成或显式 abort；失败 consume 从未拥有 reservation。abort callback 返回 false 或抛异常时 reservation 必须保持并标记 continuity lost；完整 frames 已写出后 commit callback 返回 false 或抛异常时不能回滚字节，只能标记 continuity lost，且 SafeExit continue 不再有保证。

SafeExit 只先 arm。Gateway 在首个成功 response 字节前先 claim exact audit response identity，再 claim shutdown lease delivery-write ownership 与 human-input sequence fence。在 ownership 建立前，external input 会撤销全部 active / execution-pending / delivery-pending / queued 且尚未 delivery-write-owned 的工作；第二道失败时保持零成功字节、用专用路径收束 `action_response_unknown` 并同步 abort。一旦 ownership 成功 claim，随后 ordinary revoke/human override 不得回滚它，terminal 收束只归 response-completion state machine。

`action_response_written` 与 `action_response_unknown` 是 reserved audit facts，generic append 必须以 `audit_event_reserved` 拒绝；只有绑定 exact pending terminal audit sequence/entry hash、connection/principal/session/lifecycle/action/payload 的专用 claim/complete 路径可以追加。DeliveryUnknown contract receipt 必须使用 `EvidenceKind.ReconciliationRequired`。ActionIdempotencyLedger 对 terminal replay 原样返回 retained `ContractReceipt`，包括 Unknown；不得 re-dispatch，也不得从简化字段二次合成。全部 required frame 的 `WriteAsync` 完成只表示 server-side delivery disposition，不是 peer acknowledgement；正常路径追加唯一 `action_response_written` 并保留原始 `shutdown_requested` receipt。后置 Flush、post-write audit append 或 commit callback 失败均不回滚已写字节；post-write audit 失败只允许 continuity lost、pending removal 与 `truncated` segment，后续 dispose 不得再合成 Unknown。

最终无人值守安全平面由受信 Core 二进制 runner 提供：`automation/start.ps1 -UnattendedAdapter jsonl|mcp -UnattendedSlot <slot>` 先用 v2 strict verifier 校验 manifest、完整 inventory、selected payload、build identity 与 payload closure，再执行同一已验证 `Core.exe --agent-unattended-runner`。allowlist 只含 `cf7_agent_equipment_tuning`、`cf7_agent_arena_calibration`、`cf7_agent_character_build`、`cf7_agent_loot_target_full_v1`、`cf7_agent_a5_material_shop_run`；A5 槽只接受两种 runtime binding：物化 `resources` 根以 `--candidate-id a5` 绑定 exact `resources/tmp/runtime-candidates/v2/a5`，或由 canonical root 在不携 candidate selector 时进入 `formal_runtime`；普通 `c-*`、其他短叶与 unqualified runtime 一律拒绝，旧槽仍只接受 formal 或原有 immutable `c-*` candidate。这里新增的 formal binding 只开放受信入口形状；本批不随源码发布材料专项 formal admission/runner；通用 runtime promotion 与正式入口 smoke 均不能代签材料 24-step 旅程或关闭 `PG-MAT-RELEASE-01`。Node/PowerShell 只提供便利封装，不是安全身份、授权或协议裁决者。Host 在每次完整 surface refresh（含周期 refresh）重试 credential publication，以 single-flight 锁串行，并在 teardown 先停止 admission/周期 refresh、再越过 in-flight publication barrier 后 dispose bootstrap/authenticator。A5 是唯一增加 credential 前置进档栅栏的槽：同 attempt 的真实 `bootstrap_reveal_ready`（watchdog 不算）到达后，Host 才可发送一次固定、无客户端参数的 `_root.agentEnterResolvedSave()`；只有同档位/attempt 的 `s:1|ga` 与 `agent_runtime_status.loaded=true` 同时成立才发布 credential。JSONL/MCP 客户端既不能选择 console 文本，也不能跳过该栅栏；其他无人值守槽行为不变。受信 Core 内部按 exact ordinal slot 选择 caller 不可覆盖的单调 credential-acquisition 上限：四个历史槽仍为 30 秒，仅 A5 为 60 秒，以容纳真实标题→固定进档→同 attempt runtime ready；该上限独立于最长 10 分钟的 bootstrap request/session lifetime，watchdog 仍不能签标题或发布凭据。


A5 的凭据发布还绑定一次 Host-owned rendezvous 换票：在 credential 文件原子可见之前，只有当前 exact `pipeId/PID/startTime/lifecycleId/runtimeMode` owner 可以原子生成一张新的随机票，TTL 仍不超过 30 秒。过期旧票不会被接受或消费；换票失败回滚 proof 且不写凭据，旧四槽不走该分支。

stdin 正常结束走受支持的 `session.shutdown → shutdown_requested`；退出 observation 固定 `allowValidatedFlashKeyframeFallback=false`，只接受 exact target 的 `SourceLayer.Launcher` frame。shutdown capture 只对结构化错误精确为 `capture_unavailable / retryable=true / reconcileKind=none` 做至多四次总尝试（间隔 750/1250/2000 ms），shutdown lease acquire 只对 `input_not_quiescent / true / none` 做至多四次总尝试（间隔 100/200/400 ms）；重试沿用同一 grant/session/target/params 或同一 lease scope，仅 RPC request ID 更新。其他错误立即失败，最终 `session.shutdown` action 永不重试。shutdown lease 必须严格匹配 active `UnattendedTest`、purpose `Shutdown`、无 `renewAfter`、exact owner/principal/session/attempt/target、singleton capability/operation、one action 与 issuer receipt；terminal receipt 必须逐字段匹配同 action/target/before observation，且精确为 `terminal=true`、`outcome=input_dispatched`、`evidenceKind=broker_dispatch`、`reasonCode=shutdown_requested`、`reconcileKind=none`、`retryable=false`、`focusVerified=false`、`leaseState=consumed`。

每个进入 Core handler 的 JSONL call 有 30 秒截止；每个 MCP `tools/call` 从 handler 启动到 buffered response copy/flush 共用一个绝对 30 秒预算，active 并发输出复用其剩余时间，idle lifecycle/tools-list/error 输出各有独立 30 秒预算。A5 隔离旅程的外层 JSONL harness 只给首个 `session.status` 90 秒启动预算，用于覆盖 wrapper 校验、Guardian 构造和受信 credential gate；后续 RPC/写动作仍为 30 秒。完整 shutdown transcript 另限 30 秒。timeout 取消调用、异步关闭认证 pipe 且不生成伪 response，再走 bounded exact-child recovery，Kill 后 5 秒仍未观察到退出会显式失败。trusted stdout 始终只承载 JSONL/MCP protocol，不输出 credential、secret 或完成证据；只有 adapter exit code 0、上述 strict receipt、同一 exact owned child exit code 0 且未发生 forced recovery 时，stderr 才输出单行、不超过 16 KiB 的非秘密 completion evidence，字段限于 schema、runtime mode、process path、Core SHA-256、build identity、payload closure、guardian PID 与 terminal receipt。

F7 历史 source evidence 为 Launcher 全树 **2678 passed + 3 explicit opt-in skipped / 2681 total，0 fail**、SDK resolver **7/7**（精确 `.NET SDK 10.0.300`）、Node client **37/37**、TrustedRunner 过滤 **48/48**。exact C1 tree `7362881e96d8ed0f9c20ccae580426c522f14946` 的 candidate identity 为 `F67F1054E7DD19600138C3196D0798CFA487701CB7143C4DDFD2DC426D26E372`、closure 为 `3C2CA3E6E935BF23A061228ED3D9BDA3823E81186057E8C86118FAD5C7CEBF0D`、Core EXE SHA-256 为 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD`，production policy **26/26**，状态为 `candidate_built / NOT_DEPLOYED`。当时无真实前台的严格入口返回 `trusted_runner_credential_timeout` 且未生成 completion evidence；该负向证据不构成 `candidate_executed`、`e2e_verified` 或 `promoted`。

F8 current fresh gates 为 SDK resolver **7/7**（精确 `.NET SDK 10.0.300`）、Launcher **2724 passed + 3 explicit opt-in skipped / 2727 total，0 fail**、Node client **37/37**、TrustedRunner **57/57**、production policy **26/26**。implementation source `53caabc90941826ddacf626f536b0f473adbf049` 的 isolated candidate 证据仍固定为 `e2e_verified / NOT_DEPLOYED`；formal release 则绑定 source `6f3d50a52413c747b05b74be88d6ee46650f4597`、tree `253e57f6d20a90fef6addfa744d0487d88f00dfb`、tag `runtime-build-v2/20260731-agent-runtime-wings-f8-v1`、request `A9B33601805709DBB5EAE6DAF312C2B7B0B502096FDD3BDCEA9CBE26D8B1299C`、build identity `0F4C92F237ABD7785C957F3CD135ABF2EFB1EB5D9AB5671B869F39D00970675C`、payload closure `54FBCCBA7C90ACF407B09E38FFB874C13DE3CDFB80CF62D0F8D4E239A42962F0`、Core EXE `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD` 与 Core DLL `0CEA0C64C037090ADAB4E9C38294075E58F1D298615DD447677D0D6725A9271E`。本地 X509 与 GitHub Hosted run `30602046108` 达成双故障域共识并 promotion；无 candidate id 的 pure-MCP report `tmp/manual-agent-acceptance/formal-f8/agent-runtime-help-20260731T040942Z.json` 与 residue comparison 把该同一身份窄纵切推进到 `standard_entry_verified`。operator 未调用 Computer Use/browser/legacy privileged HTTP/`input.*`，也未持久化像素；单屏、Flash metadata-only 与未覆盖产品边界继续显式保留。

### Legacy Agent control（HTTP 无人值守外层控制）

`agent_control` 是 legacy HTTP `/task` 专用的本地自动化控制面，不是 CF7 Agent Runtime principal、Bootstrap WebView2 `cmd`，也不是任意 GUI/DOM 遥控器。它用于旧无人值守任务的外层编排：查询启动状态、选择专用存档进入游戏、等待 Flash/socket/运行态存档 ack、调用领域专用的固定入口，必要时取消启动或关闭 launcher。它保持快速返回；长轮询、日志水位、恢复与报告生成放在各领域 runner 中。斗兽标定使用 `tools/arena-calibration/run-unattended.js`，装备调制入口门使用 `tools/equipment-tuning/run-unattended.js`；新 Agent 调用应走上节受信 Core runner。
...
- WASAPI 默认设备/当前默认 endpoint inactive 时，vendored miniaudio 在 `noAutoStreamRouting` 下只上报 rerouted；bridge 固定启用该开关，COM callback 与 audio write thread 禁止 stop/reinit/start 或 uninit graph。CF7 native callback 只写 armed atomic recovery flag 并唤醒 owner，设备图 mutation 仍由 native/managed owner 串行完成。
- 首次 Initialize 的 `device_unavailable|device_lost` 计 attempt 1，attempt 2–5 沿用同一 ready epoch；default-device notification、普通 BGM `device_lost` 与 hot catalog refresh 启动 active single-flight episode，recovery replay 的 `not_ready` 沿用同一已消耗预算。最多 5 次 initialize；现役 200 ms poll 按 `1/2/4/8` ticks 退避，即 `200/400/800/1600 ms`、总等待 3000 ms。qualification pending 暂停，completion 总先 strict inspect；只有 exact valid tuple 的 `Recovering` 或 `Ready + deviceGeneration drift` 继续，`QueryRuntime` exception、null/invalid ABI、tuple drift 与其他 non-ready 立即 terminal。latest successfully committed BGM intent 的 replay 留在 `Recovering`，成功后才唯一发布 `Ready`，失败无 transient Ready；SFX 不 replay，shutdown fence 未决 backoff/late work。

**production backend、格式与构建**：
- production 只按 `WASAPI → DirectSound → WinMM` 尝试输出，**不允许 Null backend**。三者均失败时发布 `audio_unavailable`，音频进入 no-output 降级；这不是 Launcher/游戏进程 fatal，其他业务继续运行。engine 建立后 BGM/SFX group 必须分别显式进入 `started`；任一 start 失败即让初始化失败，不能留下“source playing、group 停止、cursor/meter 为 0”的伪成功图。native runtime contract 必须播放非静音 fixture，并在有界窗口同时证明 `started`、`playing=true`、`cursorFrames>0`、`lengthFrames>0`、meter frame 推进及非零 peak/RMS；这仍不等于 endpoint 可听。
...
**A6 exact-candidate qualification surfaces**：
...
- pipe 名固定为 `cf7-audio-v2-qualification-{pid}-{runId}`，OS 名称没有随 schema 升级而改变；使用 `PipeOptions.CurrentUserOnly`、单一同时客户端及有界顺序重连。qualification request/response、candidate-journal carrier 与 live-observation schema 为 v2；stimulus protocol/response 保持 v1。请求是最多 64 KiB 的 compact canonical UTF-8 NDJSON，绑定 exact runId/requestId；响应再次绑定 Core 的 PID/start time、实际模块路径/哈希、build identity 与 payload closure。跨 C#/Node 的 meter 与 BGM request 小数只接受有限非负且 `<1e21`，统一量化到 `1e-6`、零值写 `0`、其余写 `0.######` 非指数 JSON number；事件 hash 与最终响应均保留同一 raw decimal lexeme。客户端仍须用 OS `GetNamedPipeServerProcessId` 独立核对 exact Core PID，不能信任 caller 自报 telemetry。
- `begin_case` / `end_case` 只给冻结的 14 个 endpoint case 写顺序唯一 marker，`snapshot` 必须携带并匹配当前 active `caseId`，`journal` 返回连续 sequence、逐事件 hash chain 与整表 SHA-256。每个事件新增 hash-bound `workingStateElapsed100ns`，唯一调用实际可用 API-set `api-ms-win-core-realtime-l1-1-1.dll!QueryUnbiasedInterruptTimePrecise`，以 Host 启动读数为 origin、100ns 单位，允许相等但禁止回退，排除 sleep/hibernate 且没有 fallback；`monotonicTicks` 继续严格排序。仅对 `sleep_resume` 恢复时长/SLA，UTC 字段保留格式/hash、允许墙钟回拨且只作审计；seek、crossfade 与格式 case 的既有 UTC window 不变。`bgm_crossfade` marker 内由 Host 以 100ms cadence 运行 single-flight 只读 snapshot sampler，避免逐次启动 PowerShell 客户端破坏时序；它最多取 150 点且最多运行 15 秒，任一先到即停止，case end/dispose 也会取消并 fence late callback。crossfade raw snapshot 必须全部非静音且 frameCount 不回退；短暂同 frame 的 cached read 合法，但首尾 source 必须替换、总 frame 必须前进、至少有 3 个 distinct frame sample，且从每个 distinct sample 到下一次推进以及最后一个 distinct sample 到 trailing duplicate 的连续 no-progress wall-clock 窗口均须 `<=500ms`。因此不再声称每次读取都推进；该门仍不证明双 slot 同时活跃或各 slot 增益包络正确。唯一外部 annotation 是物理蓝牙/HDMI case 的 `routeKind=bluetooth|hdmi`；它不承载设备 telemetry 或 verdict。
- 该 pipe **不是音频控制面**：它只写 marker、在 coordinator owner 队列读取 snapshot、读取 journal，绝不直接驱动 native。BGM/SFX 动作仍经生产 AS2 → `AudioTask` → facade/coordinator 链；journal 将 AS2 ingress、coordinator completion/recovery 与 qualification snapshot 分源记录。snapshot 只暴露 backend、设备 digest/name、session/ready/device generation、BGM source、BGM/SFX raw meter 和全部 SFX counters，不暴露 raw endpoint ID；engine session sample format（当前 `f32`）也不等同于 endpoint capture 文件格式（例如 `pcm_s16le`）。`sleep_resume` 必须至少有 1 个 recovery episode；marker 是 endpoint transition boundary，首个 `Recovering` 可已暴露新 endpoint digest。每 episode 保持同 session、`audioReadyGeneration` 相对当前 `Ready` baseline 精确 `+1`，episode 内 `deviceGeneration` 不回退；retry 可迁移 endpoint，closing `Ready` physical tuple 必须对齐末个 `Recovering`、`deviceGeneration` 相对该 baseline 前进，并成为下一 baseline。每段 working-state 差值 `<=150000000` 命中目标，`150000000 < delta <= 300000000` 记录 `targetMiss=true` 且继续，超过 `300000000` 才 fail-closed。恢复后的 frame counter 可按 graph generation 重置；最终必须取得末 closing `Ready` 后两份同 session、同 audio/device generation、同 physical tuple 的显式 snapshot，并由同一路 BGM 或 SFX meter 同时证明第二份 frame 前进且 `peakAbsPcm16>=64`。有限 SFX 与 stale case 的既有语义不变；内部 meter 非零仍不能代签物理端点可听。
- stimulus 是与 observer 分离的 CurrentUserOnly pipe：`cf7-audio-v2-qualification-stimulus-{pid}-{runId}`。Host 只把 strict grammar 的命令投递为 `audioV2QualificationStimulus`；每条 dispatch 都在同一 ready generation 内串行、幂等重发 `arm` 后才发送命令，避免进程启动时的首次 arm 早于 AS2 handler 而丢失，generation 在 rearm 与命令之间变化则 fail-closed。Flash 端仍通过生产 `AudioQualificationStimulus → AudioBridge → XMLSocket → AudioTask → AudioCoordinator/native` 执行；响应中的 `sent=true` 只证明 exact-generation socket delivery，不证明 AS2 ingress、native started 或端点可听。前 10 case 顺序固定为 BGM 播放/seek/crossfade、Vorbis/AAC-MP4/Opus、SFX、dense overlap、BGM+SFX mix、gain zero/default-max；`sfx_playback`、dense 与 mix 都使用 plan 中同一份 strict-EOF hash/duration-bound `qualifiedLongSfx`，避免短音效在 post-stimulus snapshot 前已经结束。dense case 的唯一合法 grammar 是在同一 immediate S2 batch 内把该 full-filename linkage ID 重复 6 次；observer 静态绑定 per-entry voice cap `4`，并验证 `1 <= played <= 4`、`throttled >= 1`、`played + throttled = 6` 与其他 outcome 零增长，不把 H1 擅自收紧为 exact `1/5`。dense 后恢复 BGM gain，并等待选中源 floor duration +250ms（当前 `3813+250=4063ms`）再进入 mix；mix initial SFX meter 必须 quiet，随后锁定同一 playing request、codec/container/decoder、非空 length 与合法 cursor。循环 cursor 可以 wrap 或整圈后相等，BGM/SFX 贡献分别由各自非静音且推进的 meter window 证明，禁止再用 raw cursor 严格递增代签。Opus 后在 case 外把 BGM gain 置 0，末尾再次恢复为 1，避免 SFX capture 被循环 BGM 污染并保证 mix 前状态确定。
- `write-qualification-toolchain.ps1` 先复用 pinned runtime build environment gate，生成 assembler 接受的 canonical toolchain JSON，精确绑定 Node/MSVC/.NET/PowerShell/cmd/vcvars 路径与 SHA；显式 `CF7_NODE_EXE` 无效时不得回退 PATH。`qualification-operator.js prepare` 只接受全新 runId：在 materialize 前独占创建 `tmp/audio-v2-qualification/<runId>/` 与空 `captures/`；run root 已存在时无论 captures 缺失、为空或非空都 fail-closed，且不触碰旧 run。`materialize-qualification-fixtures.js` 只把 tracked、hash-bound 输入复制到该 ignored run root；SFX `linkageId` 必须逐字等于 source path 的 full basename（包含 `.wav`），逐项重读 source 并核对 bytes/hash，再按连续 MPEG Layer III frame 严格解析到 exact EOF，锁 version/sample rate/frame/sample/duration。只接纳 `sourceDurationMs >= 3000`，按 duration、sourceBytes 降序与 linkageId 升序写入 `qualifiedLongSfx`；当前确定性选择为 `04_and_df1-22.wav`，`60882` B、146 frames、`168192` samples、`44100` Hz、floor `3813ms`，SHA-256 `D06403877D9328656F5EA4B92D168274DAEBC2F8ABE43D24697C2DB286AB3C42`。operator 组合 observer、stimulus client 与 capture tool，自动执行前 10 case。capture tool 在 `IAudioClient.Start` 后原子写 `ReadySignalPath`，operator 必须等到该 token 才发送刺激；observer/stimulus pipe 使用无空白 compact canonical JSON，而 capture configuration 必须精确是 recursive key-sort、two-space indent、terminal LF、UTF-8 no-BOM tracked artifact，operator 用独立 comparator 接受该格式并拒绝 compact wire、CRLF、乱序或 runtime tuple tamper。configuration 绑定 endpoint、backend/device digest 与 runtime tuple，runtime `f32` 和输出 WAV `pcm_s16le` 是两个不同合同，不能互相代证。`list-playback-endpoints.ps1` 只读枚举当前 render endpoint，operator 不替人切换默认设备、路由或睡眠机器。
- `run-automated --capture-output-root` 不能绕开 prepare：它必须精确等于 `<projectRoot>/tmp/audio-v2-qualification/<runId>/captures`，该真实目录及到 project root 的 ancestry 均不得包含 symlink/junction/reparse，lane 启动时必须为空；operator 不创建或清理它，并在每次 production capture 前重验 exact real path/ancestry（后续允许本 run 已发布的 capture pair 已存在）。当前 long SFX 的 strict parser 结果是 `146` 个连续 MP3 frame、`168192` samples、`44100` Hz、floor duration `3813ms`（约 `3.813877s`）；`minimumDurationMs=3000`，duration 与 source SHA/bytes 一并绑定并由 operator 复算。
- 人类接手顺序固定：先在端点 A 完成前 10 case、三份自动 capture 与 gain restore；再于 marker 外切 A→临时端点 B并等待 ready，开始 `default_device_switch` 后切 B→A并等待恢复，随后取得 `device_recovery` capture；之后在 A 上完成蓝牙/HDMI、睡眠恢复和 recovery 后无 stale SFX。后 4 个端点动作与 10 项听感矩阵必须由人类亲自执行，自动 meter、loopback 或 endpoint 枚举均不能替代。
- `assemble-a6-evidence.js` 只准备/复核 9 组 runner configuration/input manifest 和 `HUMAN_REQUIRED` drafts，并始终写 `promotionAuthorized=false`；它绝不生成 E1、H2、正式 schema 的 pass 或任何 promotion 授权。最终 source commit 的双 clean-producer 身份/闭包一致、9 reports、44 frozen checks、4 captures、10 listening verdicts 与 H2 的精确恢复卡仍以 ADR 为准。
- **失败 attempt 台账**：历史五次 failed attempt 之后，S6 又产生 run `fe73…3766`（TWS 电量耗尽）与 `8320…d1c6`（自动 lane 早于存档选择/Flash prewarm exit）两次 operationally invalid/aborted；run `82a4…9870` 的前十 case 与三份 TWS 非静音诊断 capture 已完成，但 Realtek→TWS 切换令 Core 在 `miniaudio.dll` 以 `0xc0000005` 崩溃，整个 run/capture 作废；S9 `b2370…a108` 因旧有限 SFX/UTC/stale-counter evidence contract fail-close。S10 run `b0672e56…59a03` 的 production/player 在 `sleep_resume` 两个 bounded episode 完成 TWS→Realtek→TWS 并回原 pre digest，但 frozen observer 误要求每 episode digest 不变，A6 在 case 13 diagnostic invalidated；case 14/E1/H2 不存在。合计十个 runId/capture 均不可复用、始终 `NOT_DEPLOYED`；S11 双 clean repro、新 candidate 与全新完整 A6 均待完成。

**A5 source inventory**：生成器的 shipped-audio 清单共 827 项，精确分为 795 个 `tracked` + 32 个 `ignored_source`；manifest 为 `843455` B / SHA-256 `60CA5E3E6B42842965FBD5A2A8313827D0C2E7AB0CAE4E16D9E8E3B0986B6A9C`。qualification dependency 为 `568` entries / closure `78F24B46977C3D370FFAA41D657B184B8617D21C9DAB02EE115F6B90724A9764` / manifest `142054` B / SHA-256 `F38A7E83AFF8070D7B82E97BF905471C8CBF4D3C56C858199E8CDA25CCD5ACF1`。content sniff 确认的 11 个 AAC 资产已从错误的 `.wav` 名称重命名为 `.m4a`；这些清单是 source truth，不把“被扫描到”偷换成“已可播放”。详细投放说明见 [`../sounds/README.md`](../sounds/README.md)。
...
- **Panel pending 清理**：NpcShop/Crafting/Hairdresser 共用 `PanelPendingCallTracker<TContext>`，只收敛机械 correlation、Timer、去重与单终态；业务 write/reconcile 仍由各 Task 裁决，early / bus-only / normal 三条退出路径均显式 `Dispose()` 三个 Task。`MaterialShopAccessTask` 是 dedicated Host→AS2 fid/correlation bridge，不是第四个 shared tracker consumer，也不持 transition timer；材料商店唯一 deadline 与 lease 归 `MaterialShopNavigationCoordinator`。Hairdresser 的生产 close observer 还会 `ClearPending()`；若关闭时已有 commit，领域 `needs_reconcile` 与期望发型保留，重开只以 fresh snapshot 收敛，迟到 commit 不复活
...
| Native HUD 功能入口 | 右侧六路动作 + 默认“游戏 / 整备 / 辅助 / 系统”四行；整备为 frozen 六项目标 | `PreparationNavigationV1` 默认 `true` 时原子接通两套 launcher HUD、Build menu 与 Host focus；显式 `false` 或非法配置成套恢复旧导航 presentation/header/focus；`WAREHOUSE` 固定打开 Web 背包—战备箱，`EQUIP_UI` 固定打开 Web Character Build，`MATERIALS` 走 Host-owned nonce + AS2 exact echo 直达 Web crafting 材料视图，`EQUIPMENT_TUNING` 走独立 exact nonce opener，Intelligence 走 Host 固定同步 admission。发送、准入或挂载失败只作可见 fail-closed，不识别已删除的环境变量回滚，也不派发旧 `warehouse/openEquipUI` 或任何退役 AS2 全屏 UI |
...
| NPC 物品商店（Web-only） | `npcshop-runtime.js` + `npcshop.js` + 共享 `workbench.js`/图标/tooltip | 场景 NPC 对话与平板联系人 `openNpcShop` → `npcshop`；主页面选择待购/待售，右栏顶层切背包/收集品，二级结算页调整数量并以 AS2 opaque token 原子提交整单；Web 请求失败只给出可见错误，不调用旧 `刷新商店图标` renderer |
| 合成工作台（Web-only） | `crafting-runtime.js` + `crafting.js` + `crafting-inspector.js` 薄适配 + 共享 `equipment-inspector.js`/`dressup-doll-renderer.js`/`item-filter.js`/工作台壳 | 12 类旧合成入口 → `crafting`；Flash 权威 preview/一次性 commit，并以权威 `actionType`/`gender` 驱动普通武器完整 skin、双刀/疾影双 holder、防具性别聚焦或其他当前图标的只读特写 modal；支持批量份数上限与共享树筛选。“整理背包”只是 exact crafting instance 内的纯 Web 子路由，Host owner/instance/pause 不变；仅显式返回恢复合成，普通关闭或子路由加载/挂载失败都关闭 crafting；入口失败只提示，不打开旧 `物品改装界面` |
| 技能管理（教师 Web-only） | `skills-runtime.js` + `skills.js` + 独立 `skills` domain | 刘海屏与 Character Build 的「技能」入口都以 `nativehud/manage` 打开自身技能管理，不含教师 session/返回研习能力；只有 Character 来源的 exact manage instance 获得展示位 `canReturnCharacterBuild` 与显式“← 返回构筑”，真正一次性能力仍只在 Host。刘海直开、`×`、Esc、native backdrop 都普通关闭，不走返回。世界教师独立以 `world_skill_trainer/trainer` 打开，同一 active Skills 内才可通过 exact rebind 暂时切到管理页并返回教师；请求失败只提示，不打开旧 `学习技能界面`/物品栏技能页。浏览、初学、升级、12 槽装备/卸下/排序和纯被动启用均由 AS2 权威服务裁决，展示复用完整/紧凑、三组首击生效且可组合的 Skill facet 和共享 pointer drag，但不进入 inventory/equipment domain |
| 战队界面 | Panel 系统 `team/team-panel.js`（佣兵 / 伙伴 / 战宠 / 机械；宠物管理/领养/进阶 + 佣兵管理/雇佣/培养） | `TEAM` → `mercPanelOpen` + `team`；子控制器继续使用 `pets` / `mercs` Task 协议 |
| 竞技场 (DEATH MATCH) | `arena-panel.js` 薄 facade + `arena/{arena-core,arena-shell,arena-challenge-browser,arena-preview-authority,arena-custom-editor,arena-result}.js`（挑战双栏 / 定制赛 / 独立结算页） | `arena`，ArenaTask 双层 callId + `ArenaAuthorityCatalog`；P5 以 XML/JSON 生成 session card，Web 只提交 cardId/roster，C# 重建且 AS2 复算经济 |
...
装备调制不设独立玩家放量开关，也不下发可制造调试/游玩差异的 `tuningAvailable` capability。Character Build 可从已穿戴装备或候选进入嵌入式调制，并在同一 Character session 内以 external-write/reconcile 门收束；`profile=battlebox view=storage` 仍提供工作台调制入口，宿舍 `profile=warehouse` 不提供。旧调制 renderer 与 `source=legacy_equipment_tuning` fallback 已退役；现役 Web 路径仍调用同一 AS2 `EquipmentTuningService`，但入口发送/准入失败只能提示并 fail-closed。`EQUIP_UI` 固定打开 Character Build，不再读取 `CF7_WEB_INVENTORY_WORKBENCH`，也不再调用完整旧 Hub。agent source 走同一正式 workbench/profile 语义，但另受专用槽、attempt、runtime-ready 与正式 opener 约束。

主文件旧 UI 可达性由 `node tools/audit-main-legacy-ui-reachability.js` 守门：发布前用 `--source-only` 检查 XFL Include/递归放置，CS6 重新发布 main 后用 `--require-swf` 通过 FFDec 检查实际 ImportAssets、linkage 与 PlaceObject；单测入口是 `node tools/test-audit-main-legacy-ui-reachability.js`。旧 `物品与技能相关界面.swf` / `物品改装界面.swf`、旧仓库/商店/物品栏/教师/资源箱 symbol 不得重新进入 main 闭包。该 source/Flash 变化必须先完成隔离候选 GUI 人工验收；只有验收通过后才冻结 immutable request、构建本地 X509 + GitHub Hosted OIDC/Sigstore 的双 signer/双 faultDomain 共识并正式 promotion DLL。
...
JS Panels.open(id) → 创建/显示面板 DOM → 遮罩 + ESC 支持
    ↓ (ESC)
C# KeyboardHook / native backdrop / same-active opener → `panel_esc`，分别携 `reason:"escape"|"backdrop"|"toggle"`；缺失 reason 只按 legacy `escape`
    ↓ (关闭)
...
> **Inventory owner 与未知写边界**：通用 inventory 请求只允许 `workbench / kshop / npcshop / crafting` 四个生产 owner，并固定使用 exact 七键 envelope；`panel` 与 `panelInstanceId` 必须等于 Host 当前 active tuple，response 还要匹配同一 `cmd/callId`。Loot organizer 与 Character candidate tooltip 保留专用 validator，不能进入通用 owner gate。四个 owner 共用 `InventoryTask` 的进程内 `idle → write_pending → needs_reconcile` 水位：每次 mutation 冻结受影响容器集合；send-false/throw、timeout、断线/导航、accepted owner close、畸形结果与 `commit_failed` 都按未知写处理，后续 mutation 返回 `reconcile_required` 且零 replay。只有未知态后新发起并覆盖全部受影响容器的完整合法 snapshot 才解锁；旧、部分或畸形 snapshot 不得清门。Host 对 snapshot、稀疏槽、item/confirm、mod、递归 facets/filterSpec 与可选 balance summary 逐层白名单重建，Web 独立 strict proof 后 deep-copy 并整批原子应用。该水位不跨 Launcher Host 进程重启持久化，不能据此宣称 crash/restart reconcile 已解决。

> Workbench、KShop、NPCShop 与 crafting organizer 的 inventory mux 都必须携各自 exact owner/instance；crafting 子路由始终保留原 crafting owner、instance 与 pause witness。producer 必须 `return Bridge.send(...)`，使同步 false/throw 沿原 owner 立即失败；不得吞掉返回值后等待伪 timeout。KShop 在 cart 已保存而 owner close transport 同步失败时保持页面并重新 `open()` 本地 write coordinator，使原 exact instance 可重试且只关闭一次；不能遗留永久 `closing` 或重放购物车写。ordinary/force close、lazy reject 或 mount throw 只清理 incoming exact owner，只有显式“返回合成”恢复 crafting。

> `PanelRequestOwnerLifecycle` 是 Host-owned Inventory 四-owner family 的唯一 admission primitive：`Advance()` 只在权威 `{panelName,panelInstanceId}` 变化时退休旧 owner并接纳新 owner；accepted close 在异步视觉退休前调用 `SealExact()`，所以 same tuple 的后续重观察不能重开 admission。只有不同 tuple 才创建 fresh owner；同名新实例也必须使用新 ID。PanelHost 的 deferred open 必须先完成旧 visual retire，再 bind 新 tuple，不能用“面板名相同”跳过生命周期边界。

> KShop 的 domain-less 旧 AS2 wire 当前采用 `normalized-domainless-owner-rebuilt`，不是宽松 passthrough。Web 请求仍必须绑定 exact `panel=kshop/panelInstanceId`，且 `domain` 必须完全不存在；显式 `null`、空串或其他值都 fail closed。`ShopTask` 把 owner tuple 冻结在 pending entry，按命令 exact normalize 业务 payload，转发 Flash 前移除 `panelInstanceId/domain`，回包后只用该 pending entry 重建 Web owner；旧 AS2 不能观察或决定 Host capability。该模式由 `panel-contracts.v2.json`、validator/selftests 与 Host/Web 回归共同守门；A2 文档中出现的 `passthrough-owner-sanitized` 只保留为当时历史记录，不再代表 current contract。
...
> **2026-08-02 A2 当前验证边界**：精确命令与全部 current 计数只以 [testing-guide](../agentsDoc/testing-guide.md) 的 `PG-INVENTORY-FAULT` 收口段为真源；该门已独立从零通过，Launcher frozen Release DLL 全量为 `2998 pass + 3 explicit opt-in skip / 3001 total`、0 fail。A2 最终 live journey 绑定 isolated candidate `c-acb7d341bf25-08846e81b3-20260802t112923788z-0de05d30`、identity `ACB7D341BF2524AD9FA4B2FBBBF48DC3B48020FEE799899C70FAA0FD7DA4C090`、closure `DE073A8934696B62DF95741779EA2B99AE27F5E1C294C74687AAA53D4711EFA3`：生产 workbench owner 的 `autoTransfer` 经 Host/InventoryTask/AS2 完成 `背包/0 → 战备箱/9`，安全存档后同一 clone 未重播种重启并由 fresh Inventory snapshots 与 JSON 精确读回，独立终局审阅为 `GO / 0 Blocker / 0 High / 0 Medium / 0 Low`，严格达到 `e2e_verified / NOT_DEPLOYED`。Agent Runtime 的 `panel.open` allowlist 不含 Equipment Tuning，Flash descriptor 也没有 pixels/input，因此按维护者授权回退 Codex computer-use；最终动作由生产 DOM synthetic Ctrl+click 触发，`isTrusted=false`。这不签 physical hit-test/trusted input、真实断线/response-lost、A4/A5、promotion 或标准入口。UI 路径只触发安全保存/返回地图，最终进程退出使用 supported `agent_control shutdown`。

- **kshop**（K 点商城 + 战备箱库存态）: 唯一支持入口为 Launcher `SHOP` → Web Panel；旧 Flash `shopMainMC` 已退役。采购态继续使用 catalog/cart 恢复影子与可靠写 gate；`kshop-views.js` 承载视图 DOM；`kshop-cart-controller.js` 与 catalog/owned/tooltip presenter 把本地意图、投影和注释从 facade 拆开，`kshop.js` 只保留领域编排。新结算先发只读 `checkoutPreview`，由 AS2 重算目录、等级、K 点与 `ItemUtil.require` 容量并铸造单次 `checkoutToken`；`checkoutCommit` 复核后以 `ItemUtil.acquire` 整单直接交付到背包/材料/情报/药剂栏等实际落点，再扣 K 点、清空购物车并强制存盘。余额或容量不足整单不扣款，恰好等额允许购买。新购买不再进入 `_root.商城已购买物品`；旧存档队列继续以 purchased token + claim 单飞自然清空。旧五元数组的价格/数量仅在 AS2 权威边界接受有限数字或非空数字字符串，NaN、Infinity、负数、空串、对象及非整数数量均 fail closed；兼容层构造全新 canonical snapshot，`bulkQuery`、preview 和失败 claim 都不得改写原数组。真实存档只读复验入口为 [`tools/workbench-live-e2e/kshop-legacy-readback.js`](../tools/workbench-live-e2e/kshop-legacy-readback.js)：逐字节克隆到专用槽，跨同进程重开与新进程读回固定核对 227 商品、K 点、待领取投影及原档 SHA-256；关闭证明接受可见 header 的 `isTrusted` 左键，或 exact close request 后紧邻、哈希连续且已隐藏的 Host `panel_esc`，后者明确不冒充浏览器 DOM 物理点击。成功 checkout/claim 回包必须携写后 `catalog`，Web 立即采用动态 `maxQuantity` 并再次清理购物车；首次 bulkQuery 清理出的失效/超限购物车也会经 saveCart 写回恢复影子。commit 未知结果只做 `bulkQuery + inventory snapshot` 对账。套装数据以 `data/items/item_sets.xml` 的 `id/name/order` 为元数据单一权威，物品 XML 只声明 `setId`；`ItemUtil` 在启动加载期补齐既有 `setName/setOrder` 投影，故 AS2 → Host → Web 协议不变。`item-filter.js` 为 KShop/NPC/库存共用分类模型：商品目录显示“类别 / 套装 / 专柜”三个一级入口；类别沿 `majorType → use → actionType/weaponType` 下钻，套装沿注入后的 `setId/setName/setOrder` 聚合并按中心 `order` 排序，专柜沿 KShop JSON `type` 展示 13 个策划来源分组，三条维度互不覆盖。背包使用 AS2 权威 `setFacets` 显示“类别 / 套装”，其中 `{branch:"set",setId}` 始终扫描完整容器而非当前物理页；native select 仅作旧 AS2 无 facets 时的兼容回退。分类轨道固定占位并取消按钮过渡动画，切层不再推动下方物品格。owned 库存的纯 Web 展示排序已退役，只保留会明确修改存档的权威整理。共享 GridRenderer 按稳定 key 复用引用未变化的节点；同一窗口的选择、数量与权威刷新保留滚动及实体/子控件焦点，分类/分页/重开显式归顶。共享 `PanelTooltip` 在商品格采用 `dense-inspect`：浮层不参与命中，正文确实溢出时稳定停留 1 秒才进入 owner-wheel 检视，快速换格立即由新 owner 抢占；点击或键盘激活商品则打开右侧 `pinned-inspector`，rich 回包原位更新，普通 hover 不覆盖，并由关闭按钮、Esc 或 outside click 退出。公共视觉门固定 **8 个正交样本 / 20 项 conformance**。自动门：Edge harness **150/150**（1024×576、1366×768、1920×1080）、NPC harness **129/129** + material navigation **21/21** + reduced **2/2**、item-filter model 22/22、视觉矩阵 20/20；Launcher xUnit F8 current fresh 全树 **2724 passed + 3 explicit opt-in skipped / 2727 total，0 fail**；fresh Output Panel 为 `InventoryPanelServiceTest` 144/144、`NpcShopPanelServiceTest` 46/46、`CraftingPanelServiceTest` 36/36；真机门见验证矩阵。
...
- **workbench**（独立物品工作台与角色构筑）: `web/modules/inventory-workbench.js` 是唯一 `Panels.register("workbench")` facade，只拥有公共 shell/header/close 与 `storage/tuning/build` dispatch；storage/tuning 业务已迁入 `inventory-storage-workbench.js`，角色构筑由 session/view/controller 加 mutation/action-view/tuning/pose 模块组合，controller 均不得反向注册 Panel。Host 固化严格 profile：`battlebox`=背包 50 + 战备箱 40（Native HUD `WAREHOUSE` 默认），`warehouse`=背包 50 + 真实仓库 50（宿舍 `openInventoryWorkbench` → `panel_request` 专用，24 页）；`build ↔ storage` 只在同一个 `panelInstanceId`/pause lease 内切换。两种 storage profile 均不请求商城目录、不触发 `shopPanelOpen/Close`；图标 required-assets 门、全屏 anchor、tooltip、类别/套装权威筛选、权威整理、lease 与事务实现完全共享。两种 profile 都提供“批量存入/批量取出”：入口、模式、计数、退出与唯一提交位于双栏下方的正文命令栏，不进入全局顶栏或覆盖格子；普通点击暂存多选、重复点击取消，显式“执行转移”后按序严格单飞调用既有 `inventoryAutoTransfer`；`Ctrl+单击` 保持单件立即转移，任一 stale/timeout/未知写停止余项并对账。最低画布固定验证 `0/1/5/50` 件零溢出/零重叠；独立战备箱紧凑态使用 `6×7` 容纳 40 槽并保持 48px 卡高。AS2 落位仍只执行 merge-first→首空位。顶栏只有一个共享 `HelpAction`，按 storage/tuning/build 动态解释当前视图；战备箱帮助必须明确精确放置、拖拽、`Ctrl+单击` 与批量暂存/取消/执行。所有提供完整/紧凑切换的生产界面首次进入默认紧凑，已有显式本地 `full|compact` 偏好优先；storage/build/tuning 共用 `cf7.itemgrid.mode.workbench`。模块行数 ratchet 由 `tools/audit-workbench-ui.js` 单源维护，文档不冻结易漂移的当前计数；不得提高阈值或压行换余量。自动门不能替代宿舍、现役英雄和真实存档回读。
  ...
  2026-07-30 收敛后，standalone storage 的初始 lazy registry 固定为 **16 个脚本**：从旧 35 脚本闭包移除 20 个 tuning/build 重模块，只新增一个 2.9KB 的 view feature loader，净减少 19 个脚本、约 388.7 KiB 当前源码载荷。tuning/build 只能按显式 view 加载各自封闭依赖集，缺 loader、非 thenable、依赖未初始化或 mount 拒绝均 fail-closed；不得恢复“storage 先载入所有未来视图”的保险式聚合。`DualPaneShell` 只接受六个封闭 layout profile，宏观 grid 统一由 `core.css` 投影，领域 CSS 不再各自覆写列模板。crafting organizer 是 crafting-owned local child，不注册/打开第二个 workbench panel，也不需要 `returnTo`、无实例或 Host-owner 特判。
- **character build**（角色构筑，Web-only）: `EQUIP_UI` 经 AS2 opener preflight 打开 `panel=workbench, profile=battlebox, view=build`；preflight、发送或挂载失败只给出可见错误，不读取 `CF7_WEB_INVENTORY_WORKBENCH`，不回退完整旧装备 Hub。独立 `domain=loadout` 提供 `snapshot/candidates/tooltip/flushLive/statsSnapshot/finalize` 与 `equipEquipment/unequipEquipment/equipDrug/unequipDrug`；snapshot 固定投影 11 个装备槽、4 个药剂槽、三条 revision/dirty、权威性别/portrait 与结构化 stats。已持有装备的 candidate、facet 与 mutation 资格统一由 AS2 当前实例 `BaseItem.getData()` 的有效数据裁决，必须包含强化、进阶/战术涂层和插件修正；基础 catalog 与实例 value 中的影子等级都不能替代该事实，无法取得有效数据时 fail closed，Web 不解析或复制物品规则。

  已装备装备/药剂槽的完整注释使用上述 loadout-domain `tooltip`，不借用只接纳背包 source 的 `character_build_candidate` 分流。Web 只可发送当前 `sessionGeneration`、`expectedLoadoutRevision`、`expectedDrugRevision`，并且在 exact `slotKey` 与零基 `drugSlot` 中二选一；Host 与 AS2 都拒绝多余键、错目标、旧 revision、空槽和畸形 rich payload。该命令是读，因此第一次写入前的整数 `writeEpoch=0` 合法；负数、字符串、缺字段或多 selector 仍 fail closed。AS2 复用 `InventoryPanelService.buildTooltipProjection()` / `TooltipComposer`，跨 composer 返回后再次核对双 revision 与原对象引用；Host 只投影规范 `displayName`。Web 对 pointer/focus 共用 basic→rich，单一 pending owner 与 cache 都随 snapshot、写入、supersede、离开 `idle`、rebind/close 失效，迟到回包不得重现已隐藏注释。失败回包必须在 owner/key/visibility 仍匹配时把“读取中”替换成明确可重试终态；移开再悬停才重试，旧 owner 的迟到失败不得覆盖新 owner。该命令只读，不调用 `_applyCommon`、不推进写状态，也不拥有选择、装备、调制或 reconcile authority。
  ...
  B4 将这条 post-close 事务收敛为固定 `Skills | Materials | Intelligence` 目标 enum 和小型 Host intent；intent 只持有目标、exact Build instance、`armed | rollback_after_settle` phase、generation、lifecycle epoch 与阶段 timer，不提供 registry、DSL 或任意 destination。strict close parser 只接受 `navigate_skills | navigate_materials | navigate_intelligence`；B4 当批只启用 Skills，Materials/Intelligence 在任何 normal close、binding 消费或目标 open 前明确拒绝。arm→coordinator-settled timeout 与 settled→Skills 的既有 `SkillOpenTimeoutMs` 严格分离：settled transition 在同一 lifecycle 锁内销毁前一 timer 并进入后一 wait。pre-destructive 失败恢复同一 exact Build DOM，退场后失败至多一次 native Character rollback；competition、stale、navigation/热重载、socket、shutdown 和 epoch 变化均单次取消，迟到 callback 不得重开或重复回滚。Web 的 `returnFocusAction` 仅解析 `skills | preparation-menu`；B4–B6 当批 Host 仍只发送 `skills`，现役默认-on 投影为 `preparation-menu`；显式 off 或非法配置仍成套恢复 `skills` focus，只改变 navigation presentation。

  B5 已在该固定 enum 内启用 Materials：Native HUD direct 与 Character settled 各自建立独立 material wait，Host 生成 `material.open.*` opaque requestId，再发送 `openMaterialUI({openRequestId})`；AS2 只对合法字符串原样回显，Host 只消费 exact `{task:"panel_request",panel:"crafting",source:"nativehud_materials",initData:{view:"materials"},openRequestId}` 与对应 baseline/admission。合法省略 nonce 时 AS2 发送 ordinary Web materials envelope，它只在没有 armed intent/material wait 时准入；存在 wait 时 missing nonce 拒绝并保留当前 wait；显式畸形 token 在 AS2 零发送，携带 nonce 的 wrong/near-match 则结束当前目标。send-false/throw、`MaterialPanelOpenTimeoutMs`、competition、navigation/热重载、socket、shutdown、admission、迟到/重复都零旁路；Character 已退场时失败至多一次复用原生 Build rollback。该路径不创建 registry、bus 或通用返回栈。
  ...
  前向 Character 已退场后，如果 Skill preflight send-false/timeout、请求被拒或 `OpenPanel("skills")` 返回 false，Router 只允许一次 `origin=skill_open_rollback` 的 Character Build preflight；成功后用 fresh session 重建，失败则停止、不循环并 toast 引导从“装备”重开。反向 Skills 已退场后若 Character preflight 失败，不自动复活 Skills，只提示重试。所有 open/rebind false 都记 failed/rejected，不能继续记录 opened。跨 panel 不保存旧 DOM opener；`PreparationNavigationV1` 默认 on 时，`skills_return` 与 `skill_open_rollback` 给新 workbench 下发无权限的 `navigationOrigin/returnFocusAction:"preparation-menu"`，首屏稳定后聚焦“整备”；显式 off 或非法配置时则成套恢复 `returnFocusAction:"skills"` 与旧“技能配置”导航 presentation。配置缺失取默认 on；两种 presentation 都不得恢复退役 AS2 技能 UI。
...
- **npcshop**（NPC 金币物品商店）: `web/modules/npcshop-runtime.js` + `web/modules/npcshop-secondary-pages.js` + `web/modules/npcshop.js`；入口与交易权威边界保持不变。三视图只在 Web 组合：背包唯一来自 `domain=inventory` 的 `InventoryCoordinator`，NPC snapshot 只拥有材料/情报，Host 不再要求重复 `views.bag`。Host 对购买数量只执行 `1..999999` 传输技术护栏；`purchaseLimit/maxPurchasable` 等有效上限由 AS2 按价格、容量与策划配置动态裁决，Host 不再复制固定 100 业务上限。`purchaseLimit` 是合法 preview 输入硬上限，`maxPurchasable` 则是当前可直接提交上限和“可用”按钮/轨道标记目标；`+ / +5` 可以构造暂时超出余额或容量的计划以追加待售项或进入整理空间，但权威 preview 必须同步阻塞原因、禁用提交并保持加减/返回/可用恢复可操作。re-preview 在途时数量行会与 handler gate 同步可见锁定，权威回包后再重开，避免按钮看似可点却静默吞操作。`tradePreview` 成功回包在进入 Web 前还要严格校验 token、总额、余额/容量/commit-state 自洽、slot/same_name 计数、行 schema，以及买入/卖出行与归一化请求的物品身份、数量和 scope 一致性。inventory slot lease 与 collection lease 在资源未变化时跨纯读稳定，容器写版本/集合数量变化后才返回 `stale_state`；一次性 `tradeToken` 仍在提交尝试后消费。Web 以 preview epoch 屏蔽关闭/重开前的迟到回包，并把每次成功 preview 的精确请求与 settlement 固化为 checkpoint：一般只读 timeout、断线或畸形回包恢复最近成功 checkpoint 并重新开放加减/返回；`stale_state/shop_not_found/item_not_found/locked/invalid_price/invalid_quantity/insufficient_quantity/nothing_to_sell/sell_forbidden` 改取 fresh authority snapshot；只有已投递写入的 timeout/send failure、`reconcile_required` 或畸形 commit 才进入 reconcile，且绝不重放 token；commit 在途同时锁住返回、数量调整、批售、移除与重复提交。公共 `ItemFilter` 从 `majorType/use/actionType/weaponType` 建立类别树，并从运行时补齐的 `setId/setName/setOrder` 建立并列套装树；配置了人工 sections 时再保留“专柜”入口。目录与背包统一为行内单层 drilldown，材料/情报保持 NPC 商店领域投影。目录/背包选择按稳定 key 原位更新，结算数量重算保留滚动，筛选换窗归顶；整理空间返回结算前另取一次不改变当前筛选 UI 的完整 `filterKey=all` 背包快照，用于重绑跨筛选累计的待售项。容量分析即使收到重复情报行也会先按名称聚合，超出收集栏剩余量稳定归类 `destination_full`。数值 fixture、NPC 双上限 `interactionPolicy` 与 C# 定向测试共享 `contracts/panel-contracts.v2.json`；契约变异门当前 **62/62**，另覆盖 capability/owner、nullable Flash handler binding、全局 action/response-handler 唯一、HandleWebRequest 内实际 resolver 与 fail-closed domain guard、Host case 覆写/歧义 return、未登记 Host command、AS2 action→cmd/receiver/source 精确绑定、静态 action/response-task assignment、handler alias 及字符串伪证据。领域冻结时自动门：套装数据 66 套/327 件、NPC 数据 35 NPC/834 商品、browser harness **86/86**、视觉矩阵 17/17；Launcher xUnit **1257/1257**；Flash `NpcShopPanelServiceTest` 46/46 与 `InventoryPanelServiceTest` 131/131 fresh Output Panel。
  当前整合树的现役 browser 基线为 **129/129**，材料来源导航专项为 **21/21**，reduced SecondaryPage 独立门为 **2/2**；除严格整数输入、线性/对数 range、真实数量键盘步进、`A=purchaseLimit` / `E=maxPurchasable`、“可用”标记、首个 Esc 撤销草稿、Inventory exact owner/instance 与 preview 后稳定行/焦点/滚动外，生产 NPC 首次打开及成功提交后刷新均实际调用共享 `readPhysicalInventorySurface`：固定首批背包 `0/50` + 战备箱 `0/100`，按 `A` 补页，消费 exact 十键 response，并在 UI 只显示背包 50 格/战备箱前 40 格时仍保留完整 raw-window 与 merged `50+A` receipt。材料来源进入时只用 Host 下发的 index+name 定位并投影独立 navigation highlight；定位本身零 preview/加购/提交，随后玩家显式激活未锁定商品仍走现役购买 authority。locked/达到上限仍可定位检视但购买继续阻断；一次性“返回材料档案”与 `button|escape|backdrop|toggle` 普通关闭严格分离。该本地离线门不替代 stable canonical rerun、同候选 live journey、实机存档重启或部署证据。上一段 **86/86** 仅保留为增量前记录。
  **2026-08-16 NPC snapshot 预防性加固与诊断契约**：材料/情报集合的现役数量只允许正的 JSON-safe 整数（最大 `9007199254740991`）。旧档中仍为正数但属于小数、非有限数或越过安全整数上限的条目由 `DictCollection` 隔离：不进入 `getValue()/getItems()` 与 Web snapshot，但 `toObject()` 原样保留，避免加载时猜测性取整、截断或删除；后续同名合法整数获得会显式修复该键。`buildCollectionView` 在协议边界再次执行同一不变量，并且 AS2 只记录 `collection_quarantine_state` 的材料/情报加载隔离及运行期边界过滤条目计数，不记录名称或数值。Web 的 best-effort 诊断只允许 exact `{type,scope,event,outcome,cmd,webCallId,panelInstanceId,generation,error}`，覆盖 snapshot 发出、Host 回包、采用、拒绝、过期与本地 timeout/send failure；诊断发送失败不得改变业务回包采用。Host 将其投影成闭集元数据，同时为每个 snapshot 记录 `npcshop_response_validation`；拒绝时只写 validation stage、JSON 字段路径、预期类型和结构哈希 `shapeRef`，不写物品名、原始值或原始 payload。当前实现封闭了已证明可构造的“非法 collection 数量导致 Host 严格拒绝、Web 留在 reconcile”污染族，并为下次复现布设关联证据；它不把 2026-08-12 的间歇事故冒称为已经唯一归因。分层门为 Panel runtime **41/41**、NPC runtime **7/7**、browser **129/129 + 21/21 + 2/2**、Host 定向 xUnit **147/147**、fresh CS6 `NpcShopPanelServiceTest` **53/53** 与 Compiler **0/0**。同轮只刷新逻辑注入层 `scripts/asLoader.swf`（1,084,669 bytes，SHA-256 `46C59B92C871DE43A7531ADBE14EE56509273CF73BD19A47C600ED9D1C7EBA91`，9,871 functions，最大 46,025B）；这些仍不是隔离测试机上的真实存档 E2E、Launcher promotion 或标准入口验证。
- **crafting**（合成工作台）: `web/modules/crafting-runtime.js` + `web/modules/crafting.js` + `web/modules/crafting-inspector.js` + `web/modules/dressup-doll-renderer.js`；12 个旧分类入口由 Host 严格重建 category 后进入独立 domain。snapshot 目录投影带权威 `gender`、`batchEligible`、`canCraftOne/availability`、互不混用的 `name/displayName/icon`、`majorType/use/actionType/weaponType` 与 `setId/setName/setOrder`；Flash 每配方只做一次单份计划且不探测最大份数，Web 复用公共 `ItemFilter catalog` 视觉契约做本地类别/套装树筛选、“只看可合成”、卡片状态和计数，但保留稳定 recipeIndex 且不把目录提示当提交权威。详情原图标点击打开只读检视器：普通武器绘制完整装备 skin，不带人物；双刀/疾影按权威 `actionType` 使用 battle `兵器站立` 的两个真实 holder，分别绘制主刀+副刀或刀身+刀鞘；复合渲染必须 strict 2/2 且无 missing/failed image，任一必需部件缺失或 rig 漂移即整组回退图标；防具按当前权威 gender 仅绘制装备聚焦 fields，且不借异性分支；其他、缺失素材或纸娃娃图片加载失败也回退当前图标，图标再缺失时显式缺图。窗口默认 185%，支持拖拽/滚轮/键盘/按钮、全貌/重置与纸娃娃/图标动效暂停继续，24fps 重绘，窗口缩放后重采样 backing，关闭时销毁唯一 live Canvas/RAF/监听器。生产挂载与商城/工作台一致使用全 anchor + 1024×576 scale-shell，内部双栏为同向约 60:40；preview 的 `craftCount=1..99` 只对堆叠产物且无装备素材的配方生效，Flash 返回 `maxCraftCount`、总材料/产物/双货币价格并把份数绑定一次性 token，双货币先按每份 `Math.floor(basePrice × 铁匠倍率)` 再乘份数，commit 原子复核。Web 保存同 category/recipeIndex/craftCount 的成功 preview checkpoint；普通读 timeout/disconnected/畸形回包恢复并继续可操作，权威分歧才刷新 snapshot，只有明示 reconcile 或已投递 commit 的未知结果进入写对账。材料不足时“背包 / 战备箱”先 snapshot 撤销当前 token，再在同一 Overlay 本地打开 `workbench profile=battlebox`；“返回合成”只恢复 category/recipeIndex/craftCount 与展示筛选意图并强制重新 snapshot/preview，不共享 lease 或写 token。目录/详情保留浏览器原生滚动语义，提交后同配方 snapshot 重绘保留当前目录位置，自动回退其他配方时目录/详情归顶，数量预览跨占位重绘保留详情位置，以 CSS 窄轨统一外观；数量控件使用自绘按钮而非原生 number spinner。自动门：`test-crafting-inspector.js` 男女全量 560/560（每性别 71 武器、153 防具、56 图标；另固定全 XML 9 双刀+9 疾影、合成子集 3+4、同 skin 双槽、缺件/rig 漂移回退与非双刀例外）、browser harness 三视口均 76/76、Launcher xUnit 1257/1257（该领域冻结时历史树）；Flash 必须取得 `CraftingPanelServiceTest` 27/27 fresh Output Panel 后才能称 TestLoader 通过。
  Native HUD 材料直达现已进入同一 domain 的 `materials/materialDetail` 两个只读命令，由 `crafting-materials.js` 以 `44:56` 双栏显示持有量、怪物/关卡来源和配方/装备用途且不保留旧页 fallback。v2 目录默认按 authored `archiveOrder`，以共享 `ItemFilter.buildMany + FilterNavigator` 让同一材料进入“类型 / 用途”多路径；装备插件在类型下并列档级/适用范围/定位三轴，用途下并列配方/受控直接用途/兜底。敌人/关卡按稳定 source identity 分组，但每个 `variants[]` occurrence 都按 `occurrenceIndex` 显示；v1 success 只标记 `legacy_limited` 并隐藏 taxonomy/breadcrumb/sort。v2 用途 article 本身保持只读且不伪装按钮，每条配方提供独立真按钮“查看配方”，装备产物另有“查看装备”；两个动作都先在材料页请求 fresh `snapshot {v:1,category}`，只接受唯一 exact `recipeIndex` 且 `output.name===productName`。配方成功才在同一 owner 采用 exact snapshot、重置配方查询/树筛选/只看可合成、聚焦 exact 卡后 preview；`panelInstanceId` 与 CharacterBuild 返回能力不变，不提供返回材料。装备检视只消费 fresh output/gender 并留在 materials；任何漂移、超时、发送失败、rebind/close/切材料或迟到回包均 fail closed 为可重试 inline error，不回落过滤后第一条。v1 用途继续只读且不生成动作按钮；此流程复用现役 snapshot wire，没有新增 Host/AS2 命令。现役 browser 基线为三视口各 baseline **150/150** + current **15/15** + owner **8/8** + identity **10/10** + v1 **6/6** + session-lock **9/9** + recipe-jump **21/21**，Crafting runtime 为 **27/27**；tracked TestLoader 契约现为 `CraftingPanelServiceTest 115/115 + SynthesisIndexTest 13/13`，在原入口 wire 上增量覆盖 drop occurrence 分组、概率/数量/等级边界、整来源失败关闭、任务双次存档重建、static source identity、v1 首档兼容和 exact reverse-use；必须取得本轮 fresh Output Panel/trace 后才能声称 Flash 套件通过。上述 recipe-jump 只属 mock-browser/Node current-source 证据，不是 WebView2→Flash、游戏内 E2E、candidate 或部署。上一段 **76/76、27/27** 与既有 **36/36、49/49** 都是本次权威计划扩展前记录。
  材料来源商店跳转只对 v2 `full + indexed_live_match` 暴露 CTA：Web 发送 exact 10-key `open_npc_shop`，Host 取得 Crafting/Inventory settled witness 后，用独立 fid 请求 AS2 对 current material snapshot、exact source occurrence 与 live NPC catalog 做三重授权；目标 admission、源退休及 return capability 由 `PreparedPanelReplace` 在同一 commit 点转移。反向只发送 exact 5-key `return_crafting_materials`，Host 私有保存稳定 materialName，目标 Web 不持有返回 payload。所有 pre-commit failure 保留当前页面并允许解释性重试；commit 后目标 lazy/register/mount 失败只关闭目标且永久退休 displaced source。现役自动门为 Panel runtime **40/40**、Crafting runtime **30/30**、Crafting 三视口各 **150+15+8+10+6+9+21+11**、NPCShop **129+21+2**；AS2 full item-panel 为 **28/48/147/128/13**，Host 全量为 **3422 passed + 3 explicit opt-in skipped / 3425 total、0 fail**。fresh `scripts/asLoader.swf` 为 **1,071,546 bytes**、SHA-256 `127BB7F1AB5DCAB77C66917E0C58C074E5A82B252F8B3B02984D06A902B7E887`；目标服务锚闭合，但一个无关 JSON decompile 报 `Index -2`，故不把它表述为整 SWF 反编译全绿。以上仍是 current-source/focused closure，不外推为 candidate、真实游戏 E2E、promotion 或部署。

  材料档案 v2 继续复用业务 `payload.v`，仅 `materials/materialDetail` 接受 `1|2`：`materials` 精确为 `{v}`，v2 detail 精确为 `{v,itemName,snapshotId}`；snapshot/preview/tooltip/commit 仍 v1-only。Host 原样转发 Web opt-in 版本，以当前 exact crafting instance 的首个合法 materials success 锁定实际 session 版本；初始 v2 请求只允许完整 v1 success 这一种兼容降级，随后不得摇摆。v2 目录通过严格 taxonomy/catalog schema 建立 `snapshotId` 与 immutable catalog proof，detail 必须回显同 ID，并与目录的材料 identity、持有量、purpose/count 及 source/use occurrence 逐项闭合；新目录淘汰旧 ID。failure 保持 versionless exact `{success:false,error}`，`stale_snapshot` 只对 v2 materialDetail 合法。source union、sourceKey/order、drop variants、数值/字符串/集合上限均由 Host 重建验证。Crafting owner 退休、断线或 `ClearPending()` 同时销毁版本与 snapshot session，不跨实例复用。`data/dictionaries/material_catalog.xml` 是 authored SOT；日常/发布只运行 `python tools/derive-material-catalog.py --check`，只有维护者明确接受 generated diff 时才显式执行 `derive`，禁止发布流程先写后验。 Crafting 的真实 Boot S9 还必须等待 `craft/material/item/enemy/legacy-dictionary/equipment-mod/shop/kshop` 八组 readiness；金币商店与 KShop catalog 必须非空，任一 producer 失败、空 manifest/catalog 或解析异常都以对应 `*_failed` 原因停止，sticky failure 与 lifecycle fence 禁止迟到 success 进入 index build/rehydrate。
  材料来源头像只消费已经进入 detail 的结构化、已发现来源行：enemy 以 exact `enemyType` 调共享 `EnemyPortraits.mount()`；shop 以 exact `shopId` 调独立 `ShopPortraits.mount()`。同一 enemy/shop 来源卡只挂一张 48px 装饰头像，卡内多个 occurrence 不重复挂载；摘要文本、未发现来源和其他 source kind 都不解析、不预取。Crafting lazy closure 固定先加载 `portrait-resolver.js`、再加载 `shop-portrait-resolver.js`，最后才加载 `crafting-materials.js`，且不得为头像引入 DialogueView / MapPanel runtime。shop runtime manifest 为 exact `cf7-shop-portraits-v1`，顶层仅 `schema/geometry/entries`，entry 直接映射内容寻址 PNG、尺寸、alpha bounds 与 digest；无 alias、trim/case 猜测或 `undefined` sentinel 兼容。manifest/entry/transport/解码失败保留固定占位，`img alt=""`，resolver request token/disconnected fence 与 consumer detail-render epoch 共同阻断切材料、刷新、错误、关闭和销毁后的迟到写，不改变来源文字、档位、计数或发现制。
...
- **stage-select**（选关界面 Stage 2 runtime）: `web/modules/stage-select-panel.js` + `web/modules/stage-select-data.js`（.fla 退役后的手写 SOT；P1-B 起 mixed/allUnlocked/challenge 三段 fixture 拆至 `web/modules/stage-select-fixtures.js`，兼作 runtime bridge 失败兜底，样式自 features.css 拆至 `css/panels/stage-select.css` 并 `--ss-*` token 化 + 面板根 `data-workbench-skin="stage-select"`）；可通过 Native HUD “系统 → 其他 → 测试 → 选关测试” 的 `STAGE_SELECT_TEST` 打开，也可由 AS2 场景门 `openWebStageSelect` → `panel_request stage-select` 正式打开。支持 16 个 frame label、182 个源 XML 入口实例、166 个 Web 运行时渲染实例（含 14 个 `entryKind=map/task` 直达入口；计数基线单一真值为 `web/modules/stage-select/dev/stage-select-golden.js`）、fixture 锁定/任务/挑战模式、按外部 PNG / 内部命名帧 / 默认帧回退的 hover 预览、browser harness 和 FFDec/Web 视觉对照审计；runtime 下使用 `stageSelectSnapshot` 读取真实解锁/挑战状态，普通难度按钮通过 `stageSelectEnter entryKind=difficulty` 进入已解锁关卡，外交地图按原版绿色点直达、从源符号内部 `shape/外交地图点` / `DOMDynamicText` 矩阵复原点和文字位置、通过 `entryKind=map` 走 AS2 淡出跳转且不显示二次选择，旧外交地图 SWF 内仍指向 Flash `关卡地图` 的门由公共 `切换场景` 捕获后打开 Web 选关，`地图-*` frameLabel 会按 `StageInfoDict.RootFadeTransitionFrame` 反查回选关页签，魔神/副本任务区域把 `Symbol 3325 -> Symbol 3323 -> bitmap3321` 导出的法阵底图放在装饰层，文字按钮仍按 XFL 源矢量 CSS 复刻，`entryKind=task` 改发 `openWebDungeon` 跳转 Web `tasks` 面板副本 tab（旧 Flash `委托任务界面` 已退役删除；`StageSelectPanelService.handleEnter` 走 closePanel:false 重定向），`localFrame` 通过 `jump_frame` / `stageSelectJumpFrame` 同步 Web 当前选关页但不改 `_root.关卡地图帧值`，return 类 nav 通过独立 `returnFrameLabel` + `return_frame` / `stageSelectReturnFrame` 复刻原版 `_root.淡出动画.淡出跳转帧(_root.关卡地图帧值)`，同场景返回会直接关闭 Web panel、跨场景返回仍淡出跳转，避免旧外交地图底层场景泄露。runtime 布局隐藏测试标题、fixture/dev 控件与右侧空栏，frame tab 默认收纳到可展开区域菜单，旧 Flash `关卡地图` 保留为 fallback。P3 起协议加固（只加守卫、AS2 零改动）：请求携带 `panelInstanceId`+`sessionGeneration`（snapshot 另带 manifest `catalogVersion`/`catalogSchema`），C# `StageSelectTask` 按 Host 活跃实例 exact 绑定（SkillTask 同模式 `BindPanelInstance`/精确关闭幂等，路由层 `panel_not_active`/`panel_instance_expired` 双校验）并代封回显会话字段（守卫键剥除、不进 AS2 面），snapshot 回包附单调 `stateRevision`；Web 分发校验 instance+session、按 stateRevision 水位拒 stale 快照，关闭/换绑即清 pending 令在途回包失效（迟到/跨会话回包记 dev log）。P4-a 起控制器按四层拆分为薄 facade（`stage-select-panel.js` = Panels.register + `_debug*` QA 接口）+ `web/modules/stage-select/` 五模块（core 状态容器+跨模块工具 → view-model 纯数据层（frame 路由/fixture+snapshot 状态合并/选中态/挑战模式/任务统计/导航打分，无 DOM 无 document/window 引用、节点身份只用 stageButton.id 挂接，为 P5 三维 renderer 插座）→ renderer 空间 DOM 舞台/帧菜单/缩放与生命周期编排 → inspector pinned 决策检查器+难度提交 intent → bridge 请求信封/pending 表/回包守卫）；行为/协议/视觉零变化，qa 46/46 不倒退
...
- **team**（战队）: `web/modules/team/team-panel.js` 是唯一生产 Panel，固定标签顺序为佣兵 / 伙伴 / 战宠 / 机械；首次进入伙伴，同一 WebView 会话记忆末次标签，顶层切换会把目标子视图复位到管理列表，写操作 busy 时禁止切换和关闭。`pet-panel.js` 与 `merc-panel.js` 是可嵌入子控制器，不再独立注册 Panel；它们继续发送 `panel:"pets"` / `panel:"mercs"`，复用现有 `PetTask` / `MercTask` 与 AS2 写操作。统一 close 为纯 Web no-op，不调用有旧 Flash UI 重排副作用的 `petPanelClose`。旧命令 `PETS` / `MERCS` 仅作为隐藏兼容入口打开 `team` 的伙伴 / 佣兵标签。**壳层形态**：team-panel 是薄协调器，不渲染独立顶栏/画布（外套顶栏会把子面板压进 1024×518 触发二次缩放与黑边）——唯一一条 tab 条（`.team-tabs`）经 `initData.tabNav` 传给当前激活控制器，作为 header 第一个 action 注入：佣兵/伙伴/战宠/机械四 tab 已全部迁双栏工作台（`merc-panel.js` 与 `pet-panel.js` 各以 `catalog-decision` profile 字面量重建 Shell；pet 视图 = roster 名册 / store 领养 CommitBar / advance SecondaryPage 培养页，merc 视图 = roster 名册 / hire 雇佣市场 CommitBar / detail SecondaryPage 培养页；tab 激活态 = `.team-tab-active` + `aria-current` 双投影，关闭路径统一经 `TeamPanelHost.requestClose` 收口）。
...
- 佣兵子视图已迁入双栏工作台（Phase C，样式归并 `web/css/workbench/team.css`，旧 `merc_panel.css` 已删）：`DualPaneShell`（`catalog-decision`）+ `PanelScale`（1024×576 逻辑画布），tab 条经 `initData.tabNav` 作 header 第一个 action，header 另有名册密度切换（`GridDensityController` panelId `team-merc`，默认 compact）、HelpAction、关闭钮，metrics = 金币 / K点 / 复活币，副题「出战 n · 佣兵栏 n/n」。**roster 名册**：左栏 EntityTile 卡片网格（小纸娃娃头像 + 名称/等级 + 出战/阵亡徽标；世界内雇佣候选置顶带「候选」标识），右栏决策面（中号纸娃娃 + 性格特质要点/技能数/装备概要 + 战斗技能图标流），动作 = 出战·休息（阵亡→复活 · 复活币×1，币不足 blocked 直写原因）+「培养 →」（解雇只在培养页）。**hire 雇佣市场**：左栏等级定位 chips（全部/Lv.20+/40+/60+/80+）+ 无缝下滑佣兵池（`hire_list` 分页协议不变，触底加载 + 哨兵行 + 首屏未撑满自动续载守卫；`minLevel` 仅随 reset 请求发送，AS2 跳页后 Web 端页内锚定滚动；回包 `maxLevel` 禁用超池 chip），右栏候选预览（纸娃娃 + 技能/特质摘要 + 价格）+ CommitBar「确认雇佣」（busy 到回包；成功刷新 snapshot + toast + 回名册；失败/超时 status error + 重拉对账，绝不自动重放；天价/满员等 blocked 候选仍可预览，由 CommitBar blocked 投影）。**detail 培养页**（SecondaryPage 覆盖 body）：造型预览大号 dressup Canvas（渲染链保留：manifest 缓存 / 性别外观归一 / cacheKey / alpha 测量；卡小图 / 右栏中图 / 培养页大图三档）、性格六维条（主导标记）、技能全量行（skillTipHtml hover 协议保留）、装备调配只读 11 槽（「更换」禁用占位）、出战/复活 + 解雇（共享 modal danger 确认，自绘确认层已删）。装备 rich tooltip 协议（`equip_tooltip` 消息格式 + `_ttCache` + hover 先 basic 后 rich 原位升级）原样保留，生命周期改由 `PanelTooltip.createScope('team-merc')` 的 `bindAsync` 托管。佣兵摘要 `gender` 判定按字符串「男/女」（源 `mercenaries.json` 的 gender 字段；旧实现按 1/"1" 判男导致全员显示女，已修）。`mercSnapshot` / `mercHireList` 佣兵摘要新增 `skills`（name/level/type/trait/cooldown/cost/unlock）与 `personality`（勇气/技术/经验/反应/智力/谋略六维有序数组）——由 `MercPanelService` 按 `单位函数_fs_aka_玩家模板迁移.as` 的确定性算法（`初始化可用技能` LCG / `生成随机人格` aiSeed）重算：命中 `_root.技能缓存` 时直接采用游戏内结果，未命中时本地重算且**不回写缓存**（仅展示，不构成战斗权威）；旧 asLoader.swf 下两字段缺失，Web 端兜底显示「技能/性格情报暂不可用」。
- **共享佣兵 / 主角纸娃娃预览（2026-08-09）**：`mercSnapshot` / `mercHireList` 与 Arena preview 的佣兵摘要下发 `id/gender/height/face/hair/equips`；`merc-portrait-renderer.js` 暴露共享 `MercPortraits`，用 `assets/dressup/manifest.json` 的 battle rig `空手站立` 重建 Team 卡片/右栏和 Arena 卡片/对手行胸像。Arena 全单位目录 v2 还会把 `units.json` 中 `主角-*` 单位的 `gender/face/hairstyle` 与装备字段派生成只读 dressup actor，使 Pig/Boy/King 等按各自纸娃娃外观显示，不再把 3 个主角模板身份误记为怪物头像缺口。u235「远古流年」的远古诛神头/胸/腿/鞋不借普通诛神假素材，而由 dressup 生成器中的 unit-scoped virtual item 精确绑定 10 个真实 skinKey；手套只通过其 canonical item owner `黄金骑士牙狼手套` 绑定两个真实远古手部 skinKey。`arena-meta-rosters.js` 的 199 个公开佣兵投影保留源 `face/hair/equipment` tuple 与 canonical `portrait.actor`，hidden 条目不进入 Web；标准 mixed 由 mercId 复建同一 actor，无佣兵 meta-team/synthetic 回退按 `ArenaUnitCatalog.portrait.actor` hydrate 并显式声明 `portraitKind="dressup"`。若旧链路或开发夹具下发原始编号，先用 manifest 的 `appearance.faceById/hairById` 归一化；缺失/未知 gender 与 AS2 一致归女，缺 `face/hair` 时降级为性别默认脸型与空发型。快照使用规范化外观/渲染 key；LRU 默认受 96 条与估算 12MiB 双上限约束，同 key 并发合并但逐 mount 跟踪订阅，最后订阅者离开会取消 renderer。任一异步 tick 异常都销毁实例并释放 4 槽队列，`clear` 先作废 token/订阅再回退，迟到结果不能复活旧卡。Arena 长目录再用可见区 + 144px 邻域挂载，不能随目录条目数一次创建全部 Canvas；Team/Arena 卡片统一消费 `card + 112px + cover` 胸像口径。头像说明必须绑定 `PanelTooltip` scope 并在 DOM 替换前释放，禁止用原生 `title` 白色气泡绕过项目样式和生命周期。这条只读展示链不回写佣兵、单位数据或战斗权威。Team 培养页仍只保留一个 live canvas，绘制身体外观并与性格特质左右分栏；manifest item 的 `helmet` 继续控制头盔压发。
...
- **arena**（竞技场 DEATH MATCH）: `web/modules/arena-panel.js` + `web/modules/arena-custom-parameters.js` + `web/modules/arena-custom-param-editor.js` + `web/modules/arena-custom-undo.js` + `web/modules/arena-custom-polling.js` + `web/modules/arena-custom-result-view.js`；`Panels.register('arena')`。P5 后标准/隐藏档位以 `data/arena/arena_config.xml` 为运行时真源；Launcher 启动期由 `ArenaAuthorityCatalog` 严格解析 XML、`meta_teams.json` 与 `arena_factions.json`，fresh snapshot 按 `countMin/countMax` 生成带 source digest 的 session cards。下发到 Web 的 `cardId` 是 `<随机 sessionId>:<canonical authorityId>` 形态的会话 capability，不是跨打开稳定业务 ID。Web 只渲染权威卡并提交 `cardId/cardIndex/roster`，不提交 `expr/deposit/reward`；C# 按同一 session 反查并重建完整经济/表达式/爬升池，AS2 `ArenaPanelService` 在任何扣款、预载或跳关前再按公开公式复算并精确比较。未知/过期 cardId、损坏真源、字符串数值或不一致公式均 fail-closed，不回退 Web 硬编码。
  P4 起 `arena-panel.js` 只是 98 行注册/QA facade；生产逻辑按固定依赖序进入 `web/modules/arena/arena-core.js`、`arena-shell.js`、`arena-challenge-browser.js`、`arena-preview-authority.js`、`arena-custom-editor.js`、`arena-result.js`。P4 拆分本身不改协议；P5 随后把 snapshot 扩为 `arenaAuthority` 并把 preview/enter 请求收窄为 card intent。
  头像消费者覆盖自定义目录/已选阵容、标准/堕落/爬升卡和右栏对手：怪物以 roster `type/spritename` 进入 `EnemyPortraits`，佣兵以只读外观投影进入共享 `MercPortraits`。隐藏警报卡不挂载/预取身份头像；完整密度固定 2 列识别卡，并按阵容顺序展示最多 4 个单位小头像（余量 `+N`），紧凑密度固定 3 列速览卡且只显示同组首项。密度切换不重新抽取阵容或请求 Host，两态保留对手数、等级和经济语义。AS2 `buildOpponentSummary` 只增加 `id/gender/height/face/hair` 展示字段，card capability、经济复算、扣款与入场权威不变。当前消费者无关 production manifest 闭合 222 个真人接受 variant；Arena 217 个 exact identity 中怪物 214/214、主角纸娃娃 3/3，合计 217/217 ready、0 受控回退锁定图。`敌人-锡蒙利范围光环发生器` 只通过签名 alias 复用 `敌人-锡蒙利::default`，不复制主体资产。
  C# 根据 `arenaSnapshot.playerLevel` 额外生成高 1/2 档的 2 张死线警报隐藏卡（超出最高档时封顶到 60-100），死线警报 I 配置范围 1-3 人但混编实际至少 2 人，死线警报 II 固定 4 人，即使同档也用人数区分压力；隐藏混编按等效人数均衡配比：2 人 = 1 佣兵 + 1 怪物，3 人 = 2 佣兵 + 1 怪物，4 人 = 2 佣兵 + 2 怪物；隐藏卡只显示经济与倍率，不显示等级 / 人数 / 对手配置。
  标准 mixed / 死线警报混编的人形侧优先从 `data/merc/mercenaries.json` 派生的普通佣兵池抽取（排除 `hidden:true`），避免剧情 NPC / 彩蛋兵种模板进入常规经济刷取；Web roster 下发 `{kind:"merc", mercId, level}`，C# 只接受 meta-team 中唯一且等级精确匹配的 mercId（重复 ID 视为歧义并拒绝），AS2 入场端再按本地 `mercs_list` 重建佣兵 tuple。怪物侧优先从 `ArenaMetaRosters.teams` 的关卡拆解真实小队抽取并按 `members[].count` 展开，纯怪物卡同样优先走非人形真实小队；仅当当前等级带没有“玩家已击杀过全部 spritename”的可用小队时，才退回 `factions` 聚合兵种池单体抽样。C# 对怪物 type、等级带和 Parameters 变体做 exact source 校验，对隐藏 mixed 强制同时含人形/非人形，对堕落卡强制势力白名单与精确人数；仅当当前等级带没有可用普通佣兵时，才回退到 `ArenaMetaRosters.teams` 中 `humanoid=true` / `spritename` 含 `主角` 的关卡人形模板。真实刷怪数可高于卡面佣兵等效人数，Web 端单个怪物组展开上限为 12 体，AS2 入场后按 12 活体上限分批补刷更大 roster；UI 对 roster 卡显式区分“等效”“实体”“怪物组”，详情行用 `怪物组 i/n` 标注组内成员，避免把等效人数误读成实际刷怪体数。
  `ArenaTask` 双层 callId；`arenaSnapshot` 读取 AS2 `_root.等级` 与 `_root.killStats.byType` 已击杀 `spritename`，Host 据此建立一次性 `_authoritySession` 并附加 `arenaAuthority`。堕落/爬升卡与 canonical pool 只包含已知敌人（人形模板例外）；fresh snapshot 或 transport clear 会替换 session，精确接受的 arena close 会先 `ClearPending()` 再执行 close effect，因此关闭重开后旧 session card 必须在 Flash dispatch 前以 `stale_authority` 拒绝。AS2 `arenaEnter` 仍按 `兵种库[type].兵种名` 再校验，避免未见敌人提前出现。`定制赛` tab 走 `custom_start/status/abort`，赛程代码仍是配置真源但控件已迁入三层编辑页，入口卡只显示当前对阵、费用、状态和确认入口，配置总览负责整局预设、赛程代码、红蓝交换和双方配置卡，单方阵容页负责全量单位目录、关卡参数预设与 roster 编辑；`arena-unit-catalog.js`（`tools/derive-arena-unit-catalog.js` 由 `data/units/units.json` 全量派生，`is_hostile` 只作 UI 标签，`faction` 只作浏览器分组）可手动添加蓝/红 roster，`arena-unit-param-presets.js`（`tools/derive-arena-unit-param-presets.js` 从 `data/stages/**` 的 `Enemy.Parameters` 派生）提供关卡参数预设；inline single-case 委托给 `ArenaCalibrationTask.startSingle`，定制赛结果写 `logs/arena-custom/`，批量标定仍写 `logs/arena-calibration/`；`custom_start` 成功后关闭 Web panel，后台终态以 `custom_result` 回开独立结算页，确认按钮、全局 `×`、ESC / backdrop 关闭入口都发送 `returnBase:true`；browser harness 为 `web/modules/arena/dev/harness.html`，无头入口为 `node tools/run-arena-harness.js --browser edge [--case <id>]`
  - P4 源码门包含 `node tools/test-panel-lazy-loader-browser.js`（真实 Edge 中对中间依赖注入一次 503，验证首轮后置 facade 零请求/零缓存，重试后只注册一次）与 66 场景 atlas（48 shared synthetic + 18 Arena 生产闭包两阶段）；这仍只作为 P4 历史分层证据。P5 的专项门另含 C# authority catalog/forged-payload tests、fresh `ArenaPanelAuthorityTest`、真实 WebView2→AS2 入场和同一正式身份复验，不能由 P4 harness 代签。无人值守真机只允许当前 attempt + 专用 `cf7_agent_*` clone 调固定 `agent_control openArena`；Host 只能下发无参数 `openArenaForAgent`，AS2 固定复用 `stage_select_arena_redirect` / `difficulty=冒险` 的生产 `panel_request`。正向关闭、选择与确认由绑定 candidate PID/页面的 CDP Input 真实浏览器点击完成并要求 DOM `isTrusted=true`；关闭只命中生产 selector `.arena-panel .workbench-close-btn`。被动观察器允许重开时 `snapshot` 请求先于对应 `panel_cmd open` 入账，但只接受晚于本次 opener 且响应晚于该 open 的同 callId exchange；成功 enter 的 close 可先于 response 入账，但必须晚于 enter request 且携 `dismissReturnStack:true`；Host→Flash 增量与重建命令只读取 `/logs` 规范化 `records[].line`。save-universe 继续覆盖全部非目标玩家 JSON 与 owned SOL，只显式排除每次启动按设计更新 `writtenAt` 的内部 `.launcher-version-marker.json`。旧 cardId 只允许用一次无写 `preview` 负向探针，必须在 Flash 前拒绝，不能作为正向业务入口。标准页合法同时展示 10 张 `mode=standard` 卡与 2 张隐藏挑战卡，实跑器只把 10 张标准卡子集与 Host 标准权威逐项对齐；卡片奖金/押金使用上下两行，并由三视口几何断言和真实 1067×600 / DPR 1.5 截图锁定零重叠、零溢出。执行入口为 `node tools/workbench-live-e2e/arena-live-e2e.self-test.js` 后运行 `node tools/workbench-live-e2e/arena-live-e2e.js --candidate-root <candidate> --allow-read-only-live-seed`；该输入证据明确是 candidate-page trusted browser input，`physicalInputAttestation=false`。
  - 2026-08-08 正式证据：同一 promoted identity 的 Arena 报告 `tmp/workbench-live-e2e/arena/20260808T081239008Z-p5-cf7_agent_p5_arena_authority/report.json` 为 `passed`，完成真实 opener、受信关闭/重开、旧 session card 的 `stale_authority`/零 Flash dispatch 负例及标准卡 `arena-1` 的真实选择/确认；Host 与 AS2 一致接受 `#0@1-5%1 / 500 / 1000 / 1`，截图 SHA-256 `7DAFDBF7AFFC2E8B914B35ED8656614FBC733FAE7D229BE7684FE0EA7ED4FD85` 已视觉检查。商城旧档报告 `tmp/workbench-live-e2e/kshop/20260808T081422808Z-legacy-cf7_agent_p5_kshop_legacy/report.json` 三次一致读回 227 商品、K 点 1,838,471、购物车 1 项与待领取“远古碎片”29 件，并证明 seed 字节及 clone `shop` 不变。配对 v2 promotion 与无 candidate id pure-MCP 正式入口已完成，故本列车严格达到 `standard_entry_verified`；商城 `panel_esc` 与 Arena candidate-page 输入均不冒充物理设备证明。
  - `arena-custom-presets.js` 是 `tools/derive-arena-custom-presets.js` 从 `data/arena/meta_teams.json` 派生的待标定组合池：蓝方固定为 `thief-lv30x4` 对照阵容，红方为关卡拆解元战队；`tools/derive-arena-meta-teams.js` 对关卡 Enemy 中出现的兵种全量保留，`is_hostile` 只作为标签，不再过滤 `teams[]` / 定制预设，人形模板仅从自动势力采样池剔除；`Enemy.Parameters` 会进入 `meta_teams.json`、`arena-meta-rosters.js`、定制赛程码、C# 标定命令与 AS2 刷怪 init，但 AS2 最后覆盖 `是否为敌人`、`产生源`、`掉落物=[]`、`不掉钱` 和 `计算经验值`。release prepare 现按顺序重派生 meta teams/rosters、factions Web 投影、custom presets、unit catalog 与 parameter presets；`derive-arena-meta-teams.js --check` 和 `derive-arena-factions.js --check` 都做 tracked 字节精确比较，release policy 同时要求三份真源/生成器/输出进入 runtime closure。生成物禁止手改；触及 `data/stages/**` 或 faction JSON 后必须重跑生成器并审核差量。
...
**A3 Crafting v6 历史权威与生产证据边界（Superseded / historical）**：AS2 preview 按真实 `ItemUtil.contain/require/singleRequire` 投影每项材料 `storageKind` 与产物 `outputDelivery`，仅可提交预览携 exact `acceptedPlan`；Host/Web 严格校验 route、能力、成本及 plan 深一致，commit 成功必须回显同一 plan，Web 发现漂移即对账。生产 journey 在发出任何 commit 控制请求或点击前，用 fresh Crafting + 完整 90 槽 Inventory 调用与 postcondition 相同的 planner；该版证据未读取药剂栏/collection，因此这些 route 以及 `bag_and_drug/unavailable` 都在 precommit 零写 fail-closed。该版离线生产证据 Gate 为 **256/256（19 正例 + 237 负例）**、source closure **143 roles**，另闭合 provider receipt v5/capture-event、CDP script/context 原始 aux 双真源及 `ItemUtil.as`/`ArrayInventory.as` 固定算法锚；当时状态仅 `AUTHOR OFFLINE_VERIFIED / LIVE_BLOCKED / NOT_DEPLOYED`，不同作者终审前不得启动 live/candidate 或外推部署。该中间状态已由验证矩阵顶部的 2026-08-05 current closure 覆盖。

**通用模块**：
- `panels.js`: 面板注册/生命周期管理 (register/registerLazy/open/close/force_close)
- `panels-lazy-registry.js` + `lazy-loader.js`: 面板懒注册表（id → deps[]）+ 逐 URL 串行保序 `<script>` 注入，首次 `Panels.open(id)` 才加载对应模块（kshop/workbench/npcshop/crafting/hairdresser/skills/help/jukebox/dressup/map/stage-select/intelligence/arena/team/lockbox/pinalign/gobang/tasks/cutscene-test）。成功 URL 按 promise 去重；失败 URL 驱逐缓存以便重试，且该项之后的脚本在失败轮不得被创建/缓存。真实浏览器恢复门为 `node tools/test-panel-lazy-loader-browser.js`。
- `tooltip.js`: 保留 AS2 HTML 转换与 mouse/anchored 几何，但交互按 `simple-tooltip / dense-inspect / pinned-inspector` 三种 profile 收口。密集格浮层不参与 hit-test；仅正文溢出时稳定停留 1 秒进入检视，滚轮仍落在原 owner，移动/换格/离开立即退出；显式激活的固定检视栏可独立滚动、Esc/outside-click/关闭按钮退出，且不会被 hover 覆盖。KShop 商品激活使用右侧固定栏，角色构筑使用区域锚定 dense rail；商城、仓库、NPC、合成、Loot、佣兵装备、技能、情报、任务和竞技场装备统一迁入 dense，确认为短文的帮助/技能标签保留 simple。权威输入与布局门见 `launcher/perf/tooltip-audit/README.md`，轨迹状态门见 `launcher/perf/tooltip-interaction/README.md`；图标继续通过 `PanelTooltip.dynamicIconHtml` 接入动态图/分层图播放链
- `workbench.js` + `item-filter.js`: 共享工作台标题提供筛选路径挂载点；目录、背包、仓库与战备箱的树筛选在标题同一行渲染可点击祖先路径，宽度不足时隐藏中段并保留根级和最近两级
- `workbench-components.js`: 共享 `SecondaryPage`、`Dropdown`、`QuantityControl` 与 `HelpAction`；Dropdown 只收口单值 listbox 的键盘、outside-close 与销毁，不拥有领域排序；非平凡双栏工作台复用唯一 `?` 入口和 shell modal，不在领域内复制 help button/focus/inert 生命周期
- `asset-timeline.js`: 图标与纸娃娃共享的烘焙时间线选择器，统一解释 `timelineFrames[]` / `durationFrames` / nested layer 独立周期
- `portrait-resolver.js` + `shop-portrait-resolver.js`: 前者按结构化 enemy `portraitRef` 共享 Team/Arena/材料的 accepted subject 与旧图 fallback；后者只按 exact `shopId` 读取 `assets/shop-portraits/manifest.json`，失败保持无障碍占位，不依赖 Dialogue/Map 模块或模糊 identity
...
**2026-08-14 材料/配方人类验收纠正（历史）**：`a5-0814-h3` 虽然形成了机器 acceptance，但真人在配方页完成合成后证明无“返回材料”路径，关闭后游戏 UI 也失去交互。因此 h3 只保留为历史机器证据，不再是 current `e2e_verified`。随后 source 在同 crafting owner 内补上一跳“← 返回材料”，以 stable `materialName` 重读并重选，并修复 telemetry 后迟到的全屏 `interactiveRect` 在透明 InputShield 留 mask 的问题；该时点曾保持 `A5_REOPENED / NOT_DEPLOYED / RELEASE_HOLD`。这段历史不再阻断 2026-08-15 的发布授权，也不能被恢复为 material formal journey 证据。

该历史修复阶段的 fresh Flash run `66a55e40661a4294aaeb4f2c117c860c` 为 Crafting `128/128`、Synthesis `13/13`、compiler `0/0`；旧 publish 产物为 `1,071,740` bytes / SHA-256 `5D4A7170E3B85E7AF96BB9ED845E3C9D89D6844FE81D93BEC58EAA2C65AAAC03`，已被下段 2026-08-15 合并树证据取代。

**2026-08-15 材料 current truth**：材料 v2 的 `navigationAccess` 由 AS2 现役基建投影并在执行时重验；自行车及以上解锁“前往商店”，摩托车及以上解锁“前往合成”，越野车兼容两者。`infrastructure.xml` 的需求继续投影为只读逐项目/逐等级缺口卡，总持有量只在详情标题显示一次，无“前往基建”按钮或新导航协议。合并树 fresh Flash 为 material loader `224/58/2/27`、Boot `86/86 + 12/12`、Equipment `28/28`、NPC `48/48`、Inventory `147/147`、Crafting `133/133`、Synthesis `13/13`、Audio `185/185`，全部 Compiler `0/0`；publish-only `scripts/asLoader.swf` 为 `1,082,305` bytes / SHA-256 `B9EFB7B4AE615E548868BD8D3CE7C9B9AA13009688B6BF2DE4C75BCD94C68F2A`。Web 为 Crafting runtime `32/32`，基建/配方/商店三视口各 `9/9`、`26/26`、`12/12`；Host material-focused `119/119`，Launcher 全量 `3723 pass + 3 explicit opt-in skip`。source `b298138fbac1b17ceaab2540729735f2d64d81d1` / tree `87cf3bb16250f02c9a2b396c1184a75064089e80` 已由 tag `runtime-build-v2/20260815-material-archive-v1`、request `8D87E3CE74FEBB33F739F486BB18AEC3C4DB81E92416D61B991402E8AB69AEFD`、双 signer / 双 faultDomain、production policy、strict v2 verifier 和原子 promotion/rollback 写入正式 runtime/DLL；identity `6437070BD78C64713FCBC1CDC681EAC581636BDA07372DDE78299B798515A113`、closure `B61AE1F3A3F3B7A3041D7ED7798471A8B9F363224B3CC213EADE7C72EBA9F6FE`、deployment commit `539fa306181da40d092c03508afdbddef18eff8d`，cloud build `31854335888` 与 audit `31855597166` 均通过。owner non-H2 明示 bypass Audio E1/H2/E3，Audio H2 仍未满足。generic no-candidate formal smoke 在 reveal stable 后可信 shutdown，`exit 0` 且 slot/SOL 不变；immediate EOF before reveal 的 pre-reveal exit 竞态保留为已知边界。状态为 `SOURCE_FIX_ACCEPTED_FOR_RELEASE / RELEASE_AUTHORIZED / RUNTIME_DLL_PROMOTED / MATERIAL_FORMAL_JOURNEY_OPEN / PG-MAT-RELEASE-01_OPEN`；本批没有材料专项 formal admission/runner，generic smoke 不代签材料 24-step journey，也不构成材料专项 `standard_entry_verified`。
