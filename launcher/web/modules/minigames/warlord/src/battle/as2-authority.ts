...
export const AS2_BATTLE_RECEIPT_SCHEMA = 'warlord.as2-battle-receipt.v1' as const;
...
interface NormalizedUnitReceipt {
  pieceId: string;
  factionId: FactionId;
  petId: number;
  identifier: string;
...
function applyAcceptedReceipt(
  ...
  const defenderFactionId: FactionId = command.factionId === 'red' ? 'blue' : 'red';
