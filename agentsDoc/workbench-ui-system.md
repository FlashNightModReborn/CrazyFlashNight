...
地图内容工作台的帮助亦复用同一 `HelpAction`／`SecondaryPage`，正文来自 `launcher/web/help/map-workbench.md`；提供零写入入门练习、十个可搜索主题及按当前对象定位的说明。顶栏保持唯一 `?`，左侧入门／对象说明按钮进入同一帮助页；返回保留草稿、方案、选择和画布，二级页右上关闭仍是面板级关闭。帮助只读，不新增 Host 命令或自动执行教程；A／B 模拟分别有直接编辑入口，不需要临时交换选择器来编辑 B。

地图草稿与当前地图／任务摘要冲突时，生产预览区必须显示暂停原因、保留数量和显式导出／放弃入口，不能留下空白画布与等待文案；编辑、导入、应用和剧情预览暂不可用。未知写入先查询原操作，不提供直接丢弃；关闭重开仍保留旧草稿。放弃仅清除未应用草稿，再通过原读取链恢复当前预览，不新增 Host 命令或覆盖磁盘内容。最低画布的按钮几何与行为由 `tools/map-workbench/test-help.js` 的独立副本覆盖。

重开恢复顺序中，已保存但未确认的操作必须优先查询，再判断是否属于外部内容冲突；不能把该操作自己造成的摘要变化当成旧草稿冲突。确认已应用后采用已保存内容；未知／未完成仍保留，不自动重放。旧面板／旧文档的迟到响应不得覆盖重开后新建的草稿，时序由同目录 `test-ui-mutations.js` 验证。
...
其隔离子文档默认“创作视图”：隐藏页／分层仍可选，隐藏对象以文字和虚线保留编辑外观；“玩家预览”严格消费同一快照可见性且画布只读，整页隐藏必须显示页面名称、原因和返回入口。该开关只改变呈现，不能改写 C# snapshot／导航许可／事实或形成全解锁模拟。明确点击子页签才改变外层选页，旧重绘回报不得覆盖编辑目标。基础图／占位不代表剧情外观已确定；提示层在隔离文档复用全局终端 token，不依赖跨文档 CSS 继承。视图往返与滚动／多视口验证见 `test-view-modes.js`。
...
| `stage-focus` | 关卡聚焦详情 | 最低 1024×576；左侧约 60% 地点/特写，右侧至少 360px 简报与固定出战区，无 FlowRail。二维定位与三维特写共用信息区；真实 fixture 为 `tools/test-stage-select-focus.js` |
...
`character-build` 的最低画布为 1024×576；“当前装备/候选预览”只改变同一个 Canvas 的合成输入，不生成第二个并排 Canvas。角色构筑、建角和医务室整形统一通过 `character-appearance-preview.js` 创建人物预览；其内部每次从当前纸娃娃输入重新测量 `空手/长枪/手枪/手枪2/双枪/兵器/手雷` 七种 battle pose，合并一个带留白的结构骨架 envelope：`身体/脸型/发型/面具/屁股/大腿/小腿/脚` 参与取景，横向变化最大的手臂、手与武器只绘制、不参与缩小人物。`手雷站立` 已由 battle rig 的真实 `手雷_装扮` holder 承载，不再是映射到空手姿态的兼容缺口。武器装扮字段按槽位隔离合并（对齐 AS2 `DressupInitializer` 逐槽赋值、空槽不挂载）：`手枪` 槽只贡献 `手枪_装扮`、`手枪2` 槽只贡献 `手枪2_装扮`，空槽不产出任何武器层；烘焙层对手枪类物品固定写 `手枪_装扮+手枪2_装扮` 双字段，那是槽位无关数据，不构成双持或腿侧枪套幻影的依据。刀同槽多字段（刀身/刀鞘/复合部件）为 AS2 `dressup/dressup2/dressup3` 的合法语义，不在隔离之列。相同身体投影下，切槽、武器候选、嵌入/放大迁移和 resize 使用同一几何结果；装备或外观权威变化必须重新测量，禁止复用只按 `panelInstanceId + gender` 建立的陈旧缓存。嵌入态允许姿态末端受控裁切，完整武器/动作查看由放大页的平移和缩放承担。主体取景字段、绘制字段与七站姿范围由共享预览组件独占，`character-build-pose.js` 只保留装备槽对应的姿势选择，消费者不得复制取景常量。共享组件内部使用 renderer 的 opt-in 输入；未接入该组件的对话、佣兵与独立 dressup 页面仍保持逐次内容自适应。轻量门 `node tools/test-dressup-stable-fit.js` 覆盖超长武器光效、男女七姿势、真实武器与换装；加载顺序由 `node tools/test-inventory-workbench-lazy-closure.js` 覆盖建角、整形和角色构筑。需要更大构图时，`character-build-doll-preview.js` 只把现有 `.character-build-doll-stage` 与 exact Canvas 临时迁入全 body SecondaryPage；不得复制 renderer、Canvas 或 current/preview state。打开时底层 body/header inert，Esc 后按原顺序放回 stage 并恢复 opener。放大页复用 `workbench-inspection-viewport.js` 的瞬态相机：滚轮或 `+/-` 缩放、主键拖拽或方向键平移、“全貌”/`Home` 复位；transform 只作用 exact Canvas，不作用 stage、候选覆盖层或 renderer 输入。关闭必须清零缩放/位移；嵌入态停用相机且不得吞掉 pointer、wheel 或键盘输入。该 profile 必须覆盖满 11+8（药剂两排四列）、空/blocked、长中文、未知性别/缺素材、stats SecondaryPage 与完整键盘路径。stats 在最低画布下不得靠缩小中文字号硬塞：标题/footer 固定，正文使用单一、可聚焦的纵向滚动区，保留可见滚动提示、滚轮与键盘滚动，并在关闭后把焦点还给入口。任何新 profile 都必须先进入本文，说明适用角色、最小画布和验证样本；禁止只在单个 feature CSS 中创造隐式结构。

当前只读 dressup manifest/report 基线固定为：items `1292`、skinKeys `2853`、covered/export `2712`、missing `141`、manual compatibility alias `9`、`auto_opposite_gender` alias `0`、battle states `7`、battle audit error `0`；missing 分类为 `opposite_gender_only=135`、`exact_xml_without_as_linkage=3`、`no_matching_source=3`，佣兵闭包为 resolved parts `3253`、当前性别 holder basic fallback parts `12`、failures `0`。自动异性 alias 必须持续为零；仅有异性素材的 key 保持 uncovered，并由当前性别 holder 的 basic fallback 承接，不能把异性身体部件伪装成精确覆盖。本文只同步已生成报告的数字，不以文档修改或测试运行重生成 manifest/report。
...
| Crafting `inventory organizer`（exact crafting owner 内的本地整理子路由） | `transfer-pair` |
...
- 复用格子时也必须保留其注释绑定。共享 `GridRenderer` 负责清理被替换或删除的格子；收纳来源切换只清理已退出的旧视图，不能在快照投影前对整棵活动视图调用 `releaseTree` 后又复用原节点。来源往返回归必须先悬停验证，再做筛选或翻页，避免重建格子掩盖绑定丢失。
...
材料档案 v2 的目录默认按 authored `archiveOrder`，不得退回 AS2 对象枚举或本地化名称排序。共享 `FilterNavigator` 通过 `ItemFilter.buildMany` 为同一材料建立多条可命中路径，但目录仍只渲染一张稳定卡：一级并列“类型 / 用途”；装备插件在类型下并列档级、适用范围、定位三轴，其他材料进入 base type；用途下并列配方分类、受控直接系统用途和无用途兜底。v1 success 只能进入 `legacy_limited`，隐藏 taxonomy navigator 与 breadcrumb，不能把不完整旧投影包装成 authored 分类或顺序。结构化敌人/关卡来源按稳定 source identity 合并为一张来源卡，但必须把每条掉落 occurrence 全部保留在 `variants[]` 并按 `occurrenceIndex` 展示；不得只留第一条、最后一条或把同敌人的多条掉落记录折叠丢失，手写摘要文本也不得反向解析成结构化来源。

材料目录的本地排序固定为“档案顺序 / 持有数 / 名称 / 用途数”：默认和每次 fresh open 都回到档案顺序，后三种只改变当前成功 snapshot 的展示顺序，并以 `archiveOrder` 作最终稳定 tie-break；名称比较固定 `Intl.Collator('zh-CN',{numeric:true,sensitivity:'base'})`。`legacy_limited` 隐藏并禁用排序。壳层 metric 独立显示“持有种类 x / y”，不随搜索、分类树、快捷筛选或排序变化；目录读取中/失败都保留槽位但显示 `— / —`，不得保留旧数字。右栏把“档案摘要”和“已发现来源”分成独立有标题 section；无结构化来源只表述“尚未发现”，不能推断为不存在。目录错误与详情错误分域：目录失败清除旧目录/metric，普通 transient 详情失败只替换右栏、保留左栏 snapshot，并提供绑定当前 `selectedName` 的单飞重试；重试仍失败时焦点回到新 retry button。`stale_snapshot` 或 detail/catalog identity drift 必须改走目录重同步，禁止在必然过期的同一 snapshot 上显示详情 retry。真空目录、筛选空、加载与错误分别呈现；筛选空的“清除筛选”只重置 query、quick filter 与树路径，保留 sort/density，恢复结果后聚焦首项但不自动选择。结果计数使用单一 debounce 后的 `aria-live="polite"`，metric 本身不承担 live announcement。

材料来源中的“前往商店”只在 v2 source 同时给出 `full + indexed_live_match` 时出现；参考价格只作导航提示并通过 `aria-describedby` 关联，不能成为购买 authority。点击后材料页进入单一 pending，冻结重复导航与冲突动作；目标 NPCShop 的定位高亮必须与购买选择/数量意图分离，打开与聚焦本身不得自动预览、加购或提交。锁定与达到持有上限的商品仍可导航检视，但购买继续服从 NPCShop 的 fresh authority。显式“返回材料档案”只消费 Host 下发给 exact NPCShop instance 的一次性 capability；`× / escape / backdrop / toggle` 是普通关闭，不自动返回。Web 发送 close/transition intent 后必须等待 Host exact commit 才退休 owner，不能把 `postMessage` 成功当作业务接受；目标在 commit 后 lazy load、注册或 mount 失败时只关闭目标并永久退休被替换的源，不回滚或复活旧页面。

Character Build 的 density 只作用右侧候选网格：full 沿用 68px owned card，compact 沿用 48px 格、40px 图标、4px 间距；左侧纸娃娃与固定 11+8 槽、PaneChrome 内联候选操作组和调制提交区均不压缩。compact 隐藏的中性 preview badge 不得承载独占业务信息，选中、blocked、数量等仍由角标、ARIA、当前浏览摘要或共享 tooltip 表达。同一个持久化 density toggle DOM 在 storage chrome 与 Character Build 候选 PaneChrome 之间迁移；切换不发送业务消息，也不创建第二份偏好。full → compact → full 必须保持候选稳定顺序、选择和焦点。

Character Build 初始页的 11+8 稳定槽节点各自承载一个只读候选数 data/CSS slot。计数只消费 Host 已严格验证的可选 `candidateFacets`，不扫描名称、不发额外业务请求，也不把完整背包的 `filterItemCount` 当成候选总数；装备按 exact `use` 映射，`手枪2` 是唯一固定兼容映射并合并 `手枪2 + 手枪`，药剂槽读取 `药剂` leaf。权威 `0` 必须显示数字 `0`，旧 payload 省略或整组投影失效必须显示 `—` 并以“暂不可用”进入 ARIA，二者不得用“无角标”混淆。badge 只是既有 slot 的 leaf decoration：选择、roving key、DOM identity、焦点摘要和滚动仍由 Character Build View 持有；snapshot 或写后 refresh 只更新投影，不创建第二套组件生命周期。
...
Character Build 已装备装备与药剂槽使用另一条窄 `loadout/tooltip` 只读检查合同，不能把背包 candidate source 或本地 projection 当作完整注释。请求必须带 current `sessionGeneration`、`expectedLoadoutRevision`、`expectedDrugRevision`，以及 exact `slotKey` / 零基 `drugSlot` 二选一；Host/AS2 严格校验封闭 shape，AS2 复用 `InventoryPanelService.buildTooltipProjection()` / `TooltipComposer`，composer 返回后再核双 revision 与对象引用。Web 同样用共享 `PanelTooltip` 做 pointer/focus basic→rich，但 loadout tooltip 具有独立单飞 owner/cache，不调用 common snapshot apply。supersede、snapshot、写入、离开 `idle`、rebind/close 均显式取消 pending、隐藏现有浮层并拒绝迟到结果；它只解释当前占用格，不拥有选择、装备、调制或对账能力。
...
背包—战备箱与背包—仓库的普通点击在批量模式下只暂存选择：再次点击同一格取消，玩家显式点击“执行转移”后才把最多 50 个有序、同源、lease-bound 来源作为**一次** `autoTransferBatch(mergeThenEmpty)` 权威写提交；AS2 仍按选择顺序执行“已有堆优先、否则首空位”，目标不足只提交失败项之前的前缀，不跳项。任一 stale、timeout、断线、`commit_failed` 或畸形回包立即清空本地选择并进入既有对账，绝不把未确认项拆成单件写或重放不确定写入。`Ctrl+单击` 保留为旧 `autoTransfer` 单件立即转移，不进入批量暂存。批量入口、模式、计数、退出与唯一提交按钮属于双栏下方的正文命令栏，不得塞回全局顶栏或覆盖任一物品格；空闲态保持低高度，进入模式后扩展为可读操作栏。最低画布必须分别验证暂存 `0/1/5/50` 件时栏体位于 body 内、零横纵溢出、零子控件重叠。批量模式和密度都不得改变 AS2 落位策略、目标容器可访问容量或 slot lease 复核。
...
物品数据本身也保持三种正交身份：`name/itemName` 是内部规则键，`displayName` 是玩家文案，`icon` 是资产键。稳定 DOM key、selector、request 与 mutation 可以使用内部规则键，但可见标题、实体名、当前浏览摘要、tooltip 标题、`aria-label`/accessible name 及文本搜索只能使用 `displayName`；图标渲染只能把 `icon` 交给 `Icons.html` / `PanelTooltip.dynamicIconHtml`。Web 不得用 `displayName || name`、`icon || name`、`iconName || itemName` 等 fallback 掩盖坏投影，也不得把内部名加入搜索索引或泄露到缺省 ARIA。严格响应缺少任一非空三元组时应由领域 validator 拒绝整份投影；确需玩家可读降级时使用明确的通用缺失文案/缺图态，不能显示内部规则键。旧 `displayname`、KShop `item` 等拼写只允许在 migration canonical 登记的领域边界转换，presentation primitive 不提供通用别名适配。
...
- 面板级 `×` 的位置恒定在最右端：主壳 header 的 `×` 位于 header actions 条末尾；二级页投影的 `×` 必须是其页头的最右端动作（与壳 `×` 同一角落），不得塞进 `‹ 返回`/帮助所在的左侧导航条，也不得夹在页头中部。语义契约的可发现性依赖位置一致性：用户在全工作台范围学到的“`×` 在右上”不能逐页重学；且 `‹ 返回`（退一级）与 `×`（全关）语义差异大、误触代价不对称，必须分端表达。位置契约的锚点是 `[data-secondary-action="close"]` 元素在页头 flex 序中最右，harness 以几何断言固定。transient 覆盖层的自身关闭控件不受本条约束。
- 真·确认/丢弃 modal：Esc 与自身关闭控件都只解散 modal 并把焦点还给 opener，不穿透到面板关闭。

正确基线：KShop 结算页 `×`（关结算 + 关面板）、Loot 整理页 `×`（重同步后直接关面板）、合成战备箱整理 `×`（直接 completeClose）。NPC 商店二级页、战队佣兵/战宠培养页、Arena 定制赛 header `×` 的历史逐层语义按本节修订对齐；各二级页 `×` 的位置曾左右两派并存（NPC/KShop/Loot 在左条、战队培养页在右端），统一收敛为页头最右端；Esc 域内顺序仍以 §6 焦点合同为准。
...
- `workbench-components.js`：SecondaryPage、ChoiceGroup、Dropdown、CommitBar、HelpAction、OwnedInventoryPane、QuantityControl；所有 open/close/destroy 回调都必须容忍重入和异常，并保持 DOM、focus stack 与业务 active 状态一致。Dropdown 只拥有单值 listbox 的 expanded/selected、Arrow/Enter/Space/Esc/Tab、outside-close 与 document listener 清理，不拥有领域排序或筛选；Esc 还焦 trigger，Tab 关闭但不抢焦。HelpAction 只拥有标准顶栏入口、更新和 listener/DOM 清理，并把领域文案交给 shell modal；并列 SecondaryPage 按打开顺序形成模态栈，只允许顶层页进入可访问树；关闭顶层恢复下层，关闭被覆盖下层不得在后续 unwind 中复活，焦点须沿 opener 链跳过已关闭页。QuantityControl 只同步数字输入、range、步进/预设按钮、`A/E` 标记及其生命周期；领域必须分别提供合法上限与当前可直接提交上限，并按 `number/range/increment/maximum` 原因解释意图，组件本身不计算价格、容量或可提交性。
...
- `equipment-tuning-write-lifecycle.js`：只拥有插件即时意图、inventory-source write、refresh/retry 与 quick-commit 阶段；`equipment-tuning-loadout-lifecycle.js` 只拥有 loadout-source commit、完整构筑刷新及 known/unknown 对账屏障。两者都不拥有 renderer、候选 taxonomy、Host/AS2 authority 或第二套 snapshot 状态机；`equipment-tuning-view.js` 继续编排 session/read/detach，render 只消费阶段投影。
- Character Build 的拆分边界固定为：`character-build-drug-layout.js` 只做严格 `drugLayout.v=2`、8 行/成对冷却验证与药剂视图投影，`character-build-session-contract.js` 只做其余 wire shape，`character-build-transport.js` 只做 loadout mux framing，`character-build-candidate-channel.js` 只做单条权威只读缓存、候选读取及 stale recovery，`character-build-loadout-presenter.js` 只做 11+8 槽 tooltip/角标回注与已装备注释（槽位网格渲染已迁 `loadout-picker-slot-grid.js`），候选范围/选择/恢复/pointer drag 交互层已抽为共享 `loadout-picker/`（pane 状态机 `loadout-picker-candidate-pane.js`、五态 `loadout-picker-candidate-state.js`、拖拽接线 `loadout-picker-candidate-drag.js`、落点策略 `loadout-picker-drop-policy.js`、操作栏动词注入 `loadout-picker-action-view.js`；ports 默认值保留 character-build-* 类名与文案，佣兵换装二期经 `merc/merc-loadout-*.js` 适配层消费同一 picker，会话/写通道/revision 仍各面板私有），`character-build-candidate-eligibility.js` 只做 universal 背包行的纯资格重投影。facade/session/view 继续拥有组合与生命周期，不得把这些叶模块扩成第二套领域状态机或多 key 缓存框架。
...
- `dense-inspect`：用于物品格、装备栏、技能格等密集相邻目标。基础内容立即出现、异步 rich 回包在同一 owner 内连续替换；浮层始终 `pointer-events:none`，允许视觉覆盖相邻格，却不得改变底层 hit-test 或迫使鼠标寻找空地。只有正文实际溢出时才启动稳定停留门：默认 1000ms，明显快速移动会重置进度；owner 和浮层同步播报 pending/inspect 状态，浮层状态位于内容流顶部的预留模式条，禁止绝对定位压住正文；模式条宽度跟随文案而非撑满双栏总宽，进入 inspect 1.2s 后折叠为 2px accent 细条把纵向空间还给正文（reduced-motion 下保持完整静态条），pending 态不做折叠。进入 inspect 时 owner 以一次外扩确认和稳定外轮廓提供就近反馈，轮廓不得被领域 hover 样式吞掉。进入 inspect 后仍由原 owner 接收滚轮并滚动正文，同一 wheel gesture 即使到达边界也不泄漏给宿主滚动区；键盘 focus 可用 `PageUp/PageDown` 或 `Alt+↑/↓` 立即进入。正文滚动条保留 WebView2 原生滚动语义但使用 6px 无箭头皮肤，pending/inspect 分别以琥珀/绿色滑块表达状态；由于浮层不命中，它只表示阅读位置，不提供 hover 或可拖拽暗示。Esc 分两层：指针仍停留在 owner 上时（document capture 拦截）只退出检视回到 scan、浮层继续跟随指针，不冒泡给面板层；键盘 focus 路径由 owner 节点自身处理，直接关闭整个 tooltip。离开 owner、切到新 owner、拖拽/选择抑制、Esc、整树替换、rebind 或 close 都退出；玩家不需要、也不应尝试把鼠标移入浮层。
- `pinned-inspector`：只由显式激活打开，作为稳定侧栏独立滚动；普通 hover 不得覆盖。它的独立滚动 body 使用 7px 无箭头滑块，并提供 hover/active 反馈，明确区别于 dense 的只读位置指示器。它以关闭按钮、Esc、配置允许的 outside click 或下一次显式检视退出，若加入操作控件则按 popover/dialog 的焦点合同设计。header 通过 `showPinned` 的 `opts.title` 常驻展示实体 displayName（缺省“物品检视”），Esc 提示降级为独立 chip，不再用静态状态文案占据标题位；desc 在壳内锁 `width:100%`，JS 不做内容估算写宽。外壳与内容面板同属 AS2 Symbol 274 灰族主题（`--tt-shell-bg` token，cyberpunk 主题经同一 token 切换），阅读排版不受 AS2 复刻约束（行距 1.4/1.45、面板内边距 8px）；外壳 max-height 按 `--cf7-overlay-scale` 折算，保证 scale>1 视口下物理像素不越界。KShop 商品激活使用右侧固定检视栏；Character 已装备区与候选区继续使用区域级锚点的 dense rail，因此“把装备区视作整体”是正式空间分区策略，不是失败避让的临时补丁。

几何仍由 Web 浮层系统负责：按 left/right/top/bottom 做视口碰撞选择并保留至少 `10px` anchor gap，最后夹紧 `8px` viewport inset；同一次展示侧向锁定，basic→rich、字体或图片迟到只按原侧重测，原侧主轴确实放不下才重选。区域级 `anchor/placement` 用于稳定一组目标的阅读位置，而不是追着鼠标左右闪烁。并排双栏超出视口时转为纵向；覆盖相邻格对 dense profile 是允许的视觉结果，视口越界、源格命中丢失、profile 漂移才是失败。`buildItemRichHtml` 的 `suffix` 统一包进 `.flash-tt-suffix` 底条（自带渐变底色、横跨双栏总宽、向右渐隐），rich 模式外层透明，不允许裸文字后缀直接浮在背后面板内容上。

真实输入约束由自动语料而非手写样例决定。`scripts/run-tooltip-corpus-audit.ps1` 事务化加载正式 `EquipmentConfig`、`ItemData`、105 个插件定义与 `TooltipComposer`；当前工作树得到 3652 条变体（1600 base、574 tier、1098 单插件宿主形态、279 三插件形态、101 条补充安装路径），105/105 个插件均至少有一条实际合法安装语料且 compose failure=0，正文字符 p50/p90/p99/max 为 102/208/369/692。这里不声称穷举插件排列的笛卡尔积：全部作者文本由基础物品覆盖，全部插件另有合法安装态，所有装备再有 1/3 插件堆叠形态；布局安全不依赖命中某个最长排列。`node launcher/perf/tooltip-audit/runner.js` 再以 3 视口 × 2 密度 × 4 个生产后缀测 87648 个 dense 物品实例和 396 个技能实例；当前越界、热点失守与 hit-test/profile 错误均为 0，106 个实例需要滚动，78286 次相邻格视觉覆盖由 dense 的非命中合同吸收。同一入口另以 3 个视口测 10956 个全语料 pinned 实例，15 次需要检视器自身滚动（外壳 max-height 按 overlay scale 折算后，小视口可用布局高度更大），越界与交互 profile 错误均为 0。交互门 `node launcher/perf/tooltip-interaction/runner.js` 固定覆盖 1 秒分界、快速移动重置、快速/慢速横纵斜轨迹、异步替换连续性、owner 抢占、滚轮边界、指针 Esc 只退出检视、键盘、固定检视器 chrome（默认标题 / Esc chip / desc 撑满壳宽）和 scope 销毁。
...
| `Dropdown` | 单值 listbox 的键盘、outside-close、焦点归还与销毁 | 领域 comparator、持久偏好、权威筛选与数据排序 |
...
`f01f4b121a4c…` 退役 cut 的最终 fresh 自动证据为 Launcher `1896 pass + 3 explicit opt-in skip / 1899`、0 fail；Crafting 三视口各 `120/120`、Tuning 各 `115/115`、Character standalone 各 `218/218`、workbench `867/867 + 12/12 + 18/18`、inventory modules `30/30`、dressup `222/222`、Loot `77/77 + 6/6`、KShop `135/135`、NPC `106/106 + 2/2`、Skills `150/150`、focus `15/15`、lazy closure `12/12`、atlas `48/48`、ratchet `66/66`，strict UI audit `0 error / 0 warning`。Flash focused runs 为 Tuning `023878476c2b40148a31e8f895635a15`、Item Panels `eb0e77b5337243f3976548fa7f3a8c37`、Skills `57e94a734d264d66b6860a870fc2db4b`、Character `d742450109cf4036882282c3ff4e61c8`，均 fresh trace/output/errors、Compiler `0/0`、32K retry `0`；main、PlayerInfo、基地与 `scripts/asLoader.swf` 均已精确 publish/verify，GUI 人工验收通过。当前 formal release 已到 `standard_entry_verified`，但标准入口实机结论仍只限 Equipment Tuning opener 与首个权威 snapshot。

整备 IA 的 B1–B7 已完成源码原子切换。C# frozen tuple 固定为 `equipment / battlebox / tuning / skills / materials / intelligence`；`PreparationNavigationV1` 的代码默认和随仓配置均为 `true`，显式 `false`（以及配置项存在但不是合法 bool）会原子恢复旧导航 presentation：原 toolbar/header exact-set、Build `storage | stats | skills | help | close` 与 `returnFocusAction:"skills"`。on 时 Native/legacy launcher toolbar 使用“游戏 / 整备”分组，Build exact-set 为 `preparation-menu | stats | help | close`；Host 只给 Build `initData` 投影严格布尔 `preparationNavigationV1:true`，off 时省略，非 Build 永不携带。Skills 返回、Skills/Materials/Intelligence 退场后失败回滚统一在 on 时投影 `returnFocusAction:"preparation-menu"`，off 时仍为 `skills`；该 focus key 只是无权限 presentation enum，不是 selector。这个 gate 从不恢复任何已退役 AS2 全屏 UI；仓库、构筑、调制、技能教师、合成等生产入口在两种 presentation 下都保持 Web-only fail-closed。
...
当前 `css/panels.css` 是纯 `@import` facade；XML 生成的 `../generated/font-catalog.css` 固定为第一项，历史样式再按原顺序保存在 `panels/foundation-top.css`、`panels/foundation-rest.css`、`panels/features.css`，工作台样式物理拆为 `workbench/{tokens,core,profiles,arena,inventory,skins,entities,crafting,equipment-inspector,skills,equipment-tuning,components,character-build,character-build-stats,team,states,motion,utilities}.css`（`arena.css` 紧随 features.css 导入——P1c 自 features.css 尾段拆出，级联位置原位保持；`team.css` 为战队双栏工作台基座）。`profiles.css` 紧随 core 导入并独占壳级 profile tracks，`utilities.css` 保持最后一个 unlayered compatibility import。`tools/lib/read-css-bundle.js` 以与浏览器相同的顺序递归解析，拒绝循环、越根和丢失的外部 import；字体生成物使 bundle 安全根提升为 `launcher/web/`，仍不得越出 Web 根。静态审计必须扫描聚合结果而不是只扫 facade。`launcher/build.ps1` 必须调用 `tools/check-workbench-css-bundle.js`，因此 transitive import 与本地 `url()` 闭包也是 candidate build 的 fail-fast 门；checker 与 reader 本身属于 runtime build recipe 身份输入。
...
> **现役门（ADR E 已完成）**：`tools/workbench-ui-ratchet-baseline.json` 通过 `explicit-e-batch-handshake` 显式启用 profile gate。`WB128` 要求门握手完整；扫描器计入每个直接 `new ...DualPaneShell(...)`，只接受没有 spread/computed key/重复 `profile` 的封闭 options object literal，其中 profile 是合法字符串字面量，或所有结果叶子均为合法字符串字面量的条件表达式；非对象实参、shorthand、逻辑 fallback、arrow value、覆盖键与非法叶子都 fail closed。结构探针刻意只守可稳定证明的 literal 子语法：源码只有一个 `DualPaneShell` 声明，构造器顶层只有一个不可变 `const` 直接调用 `WorkbenchShellProfile.requireProfile(options.profile)`；验证前不得出现明确的 `this._root =`、构造器 `return` 或直接 `throw`，validated binding 不得被重赋值；随后只能有一次顶层直接 `this._root.setAttribute("data-profile", validated)` literal 写入。它不是通用 JavaScript CFG 或动态键/API 分类器；真实时序与次数由既有 Focus/Fake DOM 行为门证明：缺失/空 options 都以 `TypeError` 在零 DOM、零 attribute mutation 前失败，合法构造只对 exact root 写一次已验证值，非法 `setProfile` 零写、合法转换恰好一写。生产 consumer/count 清单与直接引用方式一并冻结，别名、解构、bracket/Reflect 间接引用默认失败；合法 profile 列表由运行时 `workbench-profile.js` 以 frozen `validProfiles` 单源导出，ratchet 与 profile 纯 Node 叶门直接消费，禁止维护副本。`WB129` 拒绝 feature 壳级 `grid-template-columns/rows` 覆盖以及无理由/无退出条件的例外。当前 profile/structure 的 65 项与 Focus/Fake DOM `15/15` 均通过；ratchet 总计为 `65/66`，唯一失败是 release-tree strict 必须零 warning 的元断言。default/release-tree 审计均为 `0 error / 4 WB112 warning`：`equipment-tuning-view.js` `1396/1200`、`character-build-session.js` `704/700`、`character-build-view.js` `1185/760`、`character-build.js` `647/550`。9 个生产构造调用缺失/非法/间接引用、feature 壳级覆盖与登记例外仍均为 0；WB040 的声明扫描同时覆盖 spaced `! important`、末声明省略分号及任意 `all:* !important` reset。四项 WB112 是同步前 A1–A5 已存在的 feature-local 模块拆分债务，不重开 A6 功能闭环；但在完成按职责拆分，或维护者显式调整 WB112 strict policy 前，formal strict/release-tree Gate 仍未通过。不得静默抬阈值或压行伪造通过。只有 ledger 登记的 exact `display:none!important` 语义隐藏例外可存在，任何可见 display priority 必须失败，不能再用计数守恒替换复活 `[hidden]`。

> **2026-08-06 WB112 关闭（取代上一段中的 current-tree 数量结论）**：五个超标文件均按职责拆分，未抬高原文件阈值，也未压行规避扫描。release-tree/default `--strict-warnings` 均为 `0 error / 0 warning`，ratchet 为 `66/66`。当前审计行数为 `equipment-tuning-view.js` `1161/1200`、`character-build-session.js` `698/700`、`character-build-view.js` `541/760`、`character-build.js` `511/550`、`character-build-projection.js` `148/160`；新增叶模块分别受 `300/220/120/260/600/180/300` 上限约束，最大 `character-build-candidate-pane.js` 为 `569/600`。上一段 `65/66 + 4 WB112 warning` 只保留为拆分前失败列车证据，不再表示现役门状态。
> **2026-08-27 双药剂组 WB112 续检**：新增 `character-build-drug-layout.js` 后仍不抬高旧阈值：`character-build-session-contract.js` 为 `183/220`、`character-build-projection.js` 为 `150/160`，新叶模块为 `152/170`；release-tree/default `--strict-warnings` 均为 `0 error / 0 warning`，ratchet 为 `67/67`。上一段 2026-08-06 的行数与 `66/66` 仅保留当时拆分证据，不覆盖本轮现役结果。
...
共享验收至少要求同一源码树的 atlas、物品格矩阵与 `--strict-warnings` 静态审计全部通过；当前 fresh `66/66`、`20/20` 和 `0 error / 0 warning` 是对应门的完整成功结果，不是可从历史输出继承的永久状态。角色构筑另有 workbench、领域控制器和 dressup combination 三条 browser runner；它们必须覆盖三个视口、`55:45` 且右栏不少于 `360px`、1024 下左内层 Canvas 至少 `300px`、槽区按内容收缩到约 `204px` 并贴右、三组槽位末格与左 pane 右缘对齐、药剂两排四列（2 rowgroup / 2 row / 8 gridcell）且内层零横溢出、左 PaneChrome 的“当前构筑 + 浏览摘要”在左且放大入口在右、无总槽位/分组重复计数、空候选 overlay 隐藏、routine 底部提示折叠、候选 action group 最多三项（装备/调制/卸下，药剂为装入）且无通用详情/pin、完整 `aria-label`、1024 单行无溢出、无独立动作行和底部重复 rail、单一 density toggle DOM、候选 full/compact 往返零业务流量、stats 3×3 与抗性 4×2/2×4、power `data-scale=log10` / resistance linear、八 SVG `aria-hidden`、负值/缺值/全零降级、同一 Canvas/renderer identity、`空手/长枪/手枪/手枪2/双枪/兵器/手雷` 七种 battle pose 的当前状态结构骨架 fit envelope，并证明 `手雷站立` 真实消费 `手雷_装扮` 而非空手兼容、单手枪槽装备下另一手枪装扮字段零产出（无跨槽幻影）、放大页 Canvas-only wheel/`+/-`/drag/arrows/full-view reset、关闭复位、嵌入态不吞输入、inert/focus/Esc、11+8 槽位及上下同列共享四条 lane 冷却、无 pin 且随 operation/focus 更新的“调制说明”、检视 modal/内联说明逐层 Esc 与权威写后刷新。纯几何 envelope 回归另由 `node tools/test-dressup-stable-fit.js` 固定，未传 envelope 的 renderer 默认路径也必须覆盖。case 总数随契约增加而变化，本文不冻结最终数字；验收只引用同一源码树的完整 runner 输出。Browser harness 仍须按受影响领域分别执行，不能用 atlas 数量替代。
...
### 身份编辑表单的共享边界（2026-09-08）

建角和医务室整形是固定 1024×576 的身份表单，使用 `CharacterIdentityControls` 与 `CharacterAppearancePreview` 组合；它们不创建 `DualPaneShell` 或新的库存 layout profile。Bootstrap 保留三步流程，整形为单页预览与字段编辑。输入、IME、键盘、校验、比例预览共用；场景、付费、保存和关闭归各自宿主。整形的主要操作和错误原因在最低画布保持可见，预览缺件必须标明降级；具体布局样本由 `tools/run-plastic-surgery-harness.js` 验证。
