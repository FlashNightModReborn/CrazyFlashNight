...
# Phase 4 部分（当前）：默认 false。设 true 启用 Panel-Only + NotchOverlay/ToastOverlay + 已迁 widget：
...
#   - 注入 CSS 隐藏 web 端 #notch / #toast-container / #top-right-tools 避免双重 UI
#   - 已迁 widget（NativeHudOverlay 容器内）：TopRightToolsWidget（GAMESETTINGS/SETTINGS/PAUSE/HELP/SAFEEXIT），
#     NotchToolbarWidget（7 键 PETS/MERCS/TABLET/WAREHOUSE/SHOP/EQUIP_UI/TASK_UI，覆盖 kshop + 装备/任务 入口），
#     CurrencyWidget（金币 + KP，含 600ms ease-out 滚动 + 1200ms delta 浮动），
#     SafeExitPanelWidget（UiData "sv" 驱动的存盘中→取消/退出确认面板），
#     QuestNoticeWidget（td/tdh/tdn/mm 持久态 + task/announce 一次性事件 flash + 长文本 marquee），
#     ComboWidget（combo|cmdName|typed|hints 输入态 + N combo 命中态：DFA 金/Sync 青；HIT_MS=1200ms 后回落），
#     JukeboxTitlebarWidget（标题栏 + mini wave + pause/expand：UiData "bgm" 推送曲名，AudioEngine peak/is_playing 直拉，
#       pause 走 webOverlay.ToggleBgmPause 共享 _bgmPaused 镜像；展开按钮调 router JUKEBOX_EXPAND → OpenPanel("jukebox") （Phase 5 jukebox-panel.js 已注册）），
#     MapHudWidget（mm/mh UiData → MapHudDataCatalog 查 outline → fallback 圆角矩形 + currentRect beacon；click → router TASK_MAP）
#   - 注入 CSS 隐藏 web 端 #safe-exit-panel / #quest-notice-bar / #combo-status / #jukebox-panel / #map-hud 避免双重渲染
...
`useNativeHud=true`（或 `CF7_NATIVE_HUD=1`）开启 Panel-Only 架构 + NotchOverlay 接管 HUD + 已迁 NativeHud widget。Phase 4 部分范围（当前）：
...
- WebOverlay 注入 CSS 隐藏 web 端 `#notch` / `#toast-container` / `#top-right-tools` / `#safe-exit-panel` / `#quest-notice-bar` / `#combo-status` / `#jukebox-panel` / `#map-hud` / **`#context-panel`**（整个容器，含 `#quest-row > #map-hud-toggle / EQUIP_UI / TASK_UI` 按钮）避免与 C# 渲染重叠；notch/toast 消息（AddNotice/SetStatusItem/AddMessage）始终走 fallback (NotchOverlay/ToastOverlay) 而不是 web ExecScript。装备/任务入口已迁到 NotchToolbarWidget 第 6/7 键（EQUIP_UI/TASK_UI），通过 [LauncherCommandRouter.cs](src/Guardian/LauncherCommandRouter.cs) 直接 `SendGameCommand("openEquipUI"/"openTaskUI")`，与原 web 路径等价
- **NativeHud widget 已迁**（[Hud/](src/Guardian/Hud/)）：[TopRightToolsWidget](src/Guardian/Hud/TopRightToolsWidget.cs)（5 键：GAMESETTINGS/SETTINGS/PAUSE/HELP/SAFEEXIT，监听 UiData "p"/"s"），[NotchToolbarWidget](src/Guardian/Hud/NotchToolbarWidget.cs)（7 键：PETS/MERCS/TABLET/WAREHOUSE/SHOP/EQUIP_UI/TASK_UI，WAREHOUSE 跟 q>13 条件可见，覆盖 kshop + 装备 + 任务 入口，弥补 #context-panel 被 CSS 隐藏后丢失的入口），[CurrencyWidget](src/Guardian/Hud/CurrencyWidget.cs)（金币 + KP，含 600ms ease-out 滚动 + 1200ms delta 浮动），[SafeExitPanelWidget](src/Guardian/Hud/SafeExitPanelWidget.cs)（**必须由 SAFEEXIT click → router → widget.Arm() 显式开启**才显示——`sv` 是通用存盘事件，自动存盘/商店关闭/升级路径都会推 sv:1/2，仅 sv 不能决定可见性；Arm 后 sv:1 显示「存盘中…」状态条，sv:2 切到 取消/退出 按钮；取消纯本地 disarm 不通知 AS2，退出走 router `EXIT_CONFIRM` → ForceExit；s:0 完整复位），[QuestNoticeWidget](src/Guardian/Hud/QuestNoticeWidget.cs)（td/tdh/tdn/mm 持久任务态 + task/announce 一次性事件 flash 入队，NOTICE_MS=5000ms 退出回落；canDeliver 与 web `notch.js canDeliverNow` 同优先级——td=1 && tdn=1 && tdh!="" && mm!="3" && 非 flash 中 才走 TASK_DELIVER，否则 TASK_UI；长文本启 GDI+ phase marquee；s=0 清队列复位），[ComboWidget](src/Guardian/Hud/ComboWidget.cs)（搓招进度 + 命中可视化：legacy `combo|cmdName|typed|hints` 输入态显示 typed+各分支 remain，N `combo|color|"DFA 招式名"`/`"Sync 招式名"` 触发 1200ms 命中态——DFA 路径金色、Sync 路径青色；typed 来源 pendingTyped→knownPatterns→name fallback；命中期间 input 帧静默；s=0 整体复位），[JukeboxTitlebarWidget](src/Guardian/Hud/JukeboxTitlebarWidget.cs)（标题栏 + mini wave + pause/expand：UiData "bgm" 推曲名 / "pl" 决定是否关 visualizer，AudioEngine `ma_bridge_bgm_get_peak`/`ma_bridge_bgm_is_playing` 直拉播放态，100ms 节流采样推 100 点 ring buffer；pause 翻转本地镜像并调 `webOverlay.ToggleBgmPause` 共享 `_bgmPaused` 单一权威源避免与 HandleAudioTrackState 冲突；expand 调 router `JUKEBOX_EXPAND` → `OpenPanel("jukebox")`，Phase 5 [jukebox-panel.js](web/modules/panels/jukebox-panel.js) 注册 `Panels.register('jukebox')` —— createDOM 构建展开态 DOM（jbp-* 前缀新 id）+ `audio`/`catalog`/`catalogUpdate`/`helpText` Bridge 订阅 + `bgm`/`jbo`/`jbr`/`jbm`/`vg`/`vb` UiData 订阅，onClose 全部解订阅；旧 jukebox.js + overlay.html 旧 #jukebox-panel DOM 仍在但已 CSS 隐藏并随 Phase 7 删除），[MapHudWidget](src/Guardian/Hud/MapHudWidget.cs)（位置 HUD 缩略图：UiData "mm:N" 控可见性 mode 1=base/2=外部、"mh:hotspotId" 决定显示哪个区域；启动期 [MapHudDataCatalog](src/Guardian/Hud/MapHudDataCatalog.cs) 加载 [launcher/data/map_hud_data.json](data/map_hud_data.json)（由 [tools/export-maphud-data.js](../tools/export-maphud-data.js) 从 [map-panel-data.js](web/modules/map-panel-data.js) 构建期导出，map-panel-data 改动需重跑 export 同步 JSON；build.ps1 Step 6b 强制校验 JSON 在位）；MVP 渲染 fallback 圆角矩形 blocks + currentRect beacon，按 `meta.group` 主题染色（base/warlord/rock/blackiron/fallen/defense/restricted/schoolOutside/schoolInside），不渲染 PNG silhouette mask；卡片右上 × 按钮 → 折叠成 28px pin badge（与 web `MapHud.toggleCollapsed` 镜像，pin 仍 Visible 作为重新展开入口），click pin → 展开，click 卡片主体 → router `TASK_MAP`）
...
- NativeHudOverlay 鼠标管线：拦 `WM_MOUSEACTIVATE` 返 `MA_NOACTIVATE` 防 Owner 被点击 deactivate；OnMouseDown/Up/Move 派发屏幕坐标到 widget；hit testing 走 `widget.TryHitTest(screenPt)`
...
- 性能收益：panel 打开期 α blend 成本下降（panel 矩形小 + opaque）；idle 期 `DoFullIdleSuspend` 整个 SW_HIDE WebView2 + TrySuspendAsync → 拿回 ~15pp DWM α 地板（所有常驻 HUD 已迁到 C# widget，玩家在 panel 关闭期间仍能看到 notch / toast / 5 键工具栏 / 货币 / 任务通知 / combo / jukebox 标题 / 地图 HUD）
