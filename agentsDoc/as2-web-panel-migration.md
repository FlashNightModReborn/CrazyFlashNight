...
2026-07-16 快捷槽互拖的首次真机失败表明 Host 运行时产物也是协议闭环的一部分：日志若停在 Web `moveSlot` 已路由、但没有 `skillMoveSlot` Flash dispatch/response，应先核对游戏实际加载的 `runtime/CRAZYFLASHER7MercenaryEmpire.Core.dll`，不能仅凭源码和 xUnit 判断协议已部署。凡修改 C# 命令白名单或映射，`launcher/build.ps1` 只会生成隔离 candidate；必须启动脚本返回的精确 Core，记录实际进程路径、build identity 与 payload closure 后完成领域 E2E。若要声明正式部署，还必须经 promotion 原子写入完整闭包，再从 `automation/start.ps1` / 根 bootstrap 标准入口验证同一身份；不得手工复制或单独恢复任一二进制。
