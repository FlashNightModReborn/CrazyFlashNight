...
**实现/构建基线分层**：B0-06 v1 实现与首轮 Kimi 问题修复冻结于 commit `bf8dd2c410267855c8ea12f25a594042b3158479`（2026-07-29），两份 v1 机器/视觉报告由 evidence/docs-only commit `81168ece8cd295d8e15b8f6baed46a0da7f9c549` 承载并继续精确绑定该旧实现闭包。v1 source-freeze F 为 commit `cb38600aae51f5019d09f87c33bd9e67d2b1f511`、tree `c74cc66f445921f5fcda1da04a2de0f013fef8ef`、parent `2882456c8777246fba8d2f6942ef1027caafc650`；不可变 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v1` 只永久指向 F。F/G 的 builder、quorum 与 K1 事实均只证明 v1 字节，不能批准本轮 main-space v2 纠正。v2 runtime/visual implementation source 已冻结到 commit `40853287e7ed04714d68935c0002f8ad6d8aea05` / tree `9c0909d7260d741fde34ac29c31a668babb330d1`，current machine/visual snapshot 已重新生成；v2 K1 已对 evidence/docs 后继 `884fbf436f21c5e27ed3370c445cbae941349496` 通过。最终 r2 build-train source 为 F2 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / tree `5b65f9fde417f35ce34012edf68fd9c899cb1813`；它只补 qualification 编译闭包，不把 `40853287…` 的 B0-05/06 报告冒写成 F2 重跑结果，也不能用“当前 HEAD”或旧 F/G 代替精确身份。

**当前裁决状态**：B0 已达到 `b0_accepted`。main-space v2 在 clean merge `40853287…` 通过 B0-05 32/32、B0-06 48/48 与完整 Launcher 1747+3/1750；tracked `visual-evidence.v2` 冻结 C# A/B `36 files / 35 PNG / 1,261,624 B / DD58CA7C…6661E`、Web A/B `12 files / 11 PNG / 221,781 B / 9D21D823…17A7B`、direct A/B `66 PNG / 3,114,450 B / F05AE7F7…50ABF` 及 viewer `651,451 B / 2F71A3ED…01BC2`，MP 四字段均为 unique `bestDx=0` 且对应 anchor edge `dx=0`。child authoring wrapper 已排除，Flash/C#/Web 均以 1024×576 main content viewport 为基准；MP current 右锚 `74.55`、maximum authored `80.65` + C# local phase `0.30`、HP Glow/横线均进入 clean closure。真实 Flash v2 capture `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 的 strict tooling/source/placement 身份与 22-PNG closure 已冻结；其 ignored manifest 仍保留 `candidate` 原始字节，不以手工改写冒充签收。最终人类 receipt 通过 exact F2 `p50` 的 actual main runtime 可见 composite、旧 HUD 同屏、Ready/reveal/player_info_split、来源路径及 source-bound main-RSL-equivalent capture 联合形成 `oracle_frozen_for_b0`，明确不声明 cross-renderer pixel parity。`kimi-code/k3 + high` 已对 clean evidence/docs commit `884fbf43…` 完成同一会话完整审计，165/165 请求与 5/5 config update 全部命中，结论 `APPROVE_AUTOMATED_CLOSEOUT / K1_AUTOMATED_GATE=PASS`、P0/P1=0；F2/r2 的全新 request、local X509 + GitHub OIDC 双 builder、两份 23/23 receipt、双 signer/双 faultDomain quorum 与不可复用 `-VerifyOnly` preflight 同样闭合。exact F2 隔离候选以 `p50` fixture 达到 `candidate_executed / source-ahead / NOT_DEPLOYED`；人类明确接受游戏 composite、z-order/occlusion、球体保留、HP Glow、MP 横向/基线/蓝条、crop、鼠标透传、时序观感、144 DPI/dpr 1.5 缩放外观与整体审美。这是 F2/r2 自身的历史列车状态。PlayerInfo 实现字节随后被较新的 `9118eb5097…` 正式 source/runtime 包含，但该 release 的 `standard_entry_verified` 只执行 Equipment Tuning opener 与同实例首个权威 snapshot，没有启用 PlayerInfo fixture、观察真实 `pi_*` 或做 PlayerInfo-specific E2E；B0 与 oracle 状态均不因此升级。

**2026-07-30 首列 v2 build-train 退役（历史）**：受保护 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v2` 已永久指向 `40853287…`；q3 request `AC3917AB…EC4A` 的本地 X509 producer/票有效，状态 `1/2`，build identity `DAE8A71C…8ED1`、payload closure `051FA840…D2C`。随后 production policy 为 **22/23**：唯一失败 `candidate-player-info-svg-contract` 发现隔离 `RendererQualification.csproj` 漏挂 `PlayerInfoStrictSvg.cs` 已使用的 `PlayerInfoRasterPlan.cs`，失败 receipt **12,141 B** / SHA-256 `3B812E9576164D8472D16020AD0D88824127CB43F7D07B067C0BBAC108293315`。因此旧列车准确状态是“因 production policy 门失败而退役”，不是 producer/request failed；没有启动 GitHub builder，没有 quorum/preflight，也没有 runtime/release-state/promotion/deployment 变更。旧 tag/q3/CAS/票/receipt 只保留审计，绝不移动、删除、supersede 或复用。

**2026-07-30 F2/r2 非部署自动闭环（覆盖上段旧时点）**：修复只给 qualification 项目显式链接 `PlayerInfoRasterPlan.cs` 并把 cloud ref 切到全新一次性 tag；F2 为 `891d9b08dbd826d8b2624c6bdc59082b3db57ecd`、tree `5b65f9fde417f35ce34012edf68fd9c899cb1813`、parent `b2124efc63e8b8da90d2c07561a97a06f4087ee2`。受保护 tag `runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2` 精确指向 F2；全新 q4 request `6B5E9BDD5393553871C646952B1266E877D8B8941D50AB510B5387FC1261460D` 绑定 build identity `DAE8A71C0AE04AF1BD89B0076DB1EF6EF5DFEEC51C32843D300D868581568ED1` 与 33-file payload closure `051FA840019E24763724B5DA0868486AF94322F3BB990B1354D181989AA94D2C`。`builder-local-b` 的 X509 signer 位于 `physical-host-b`；GitHub run `30502544175` 的 OIDC signer 位于 `github-hosted-windows`，两端 Core `6BBE712CB0F97AC127FA3DF18158CA3976D7A03A64DC8121794E7C7C1E18C6CC` 与 manifest `53F44B8188A425728975197CD8B38FF5CE523B0306CA2EB8EE1ECBD76FF77261` 一致。本地与 cloud production policy 均为 **23/23**；最终 [`runtime-promotion-preflight-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight-main-space-v2.json) 为 **15,128 B** / SHA-256 `2D0A2A862ADFFF4254FB8AFB910B1529568DABD33350AE8E735AC20F52B47356`、`preflight-passed`、`proofCount=2`，并明确 runtime/release-state/promotion/deployment/reusable flags 全为 false。build identity/closure 与首列相同是确定性结果；q4 的 request、本地票、cloud proof 与 receipts 均针对 F2/tag fresh 生成，不复用 q3。该 build/quorum/VerifyOnly 闭包本身只证明 `candidate_built / NOT_DEPLOYED`；后续隔离候选执行是独立证据，不回写 preflight 的能力边界。

**2026-07-30 F2 隔离候选现场启动**：在 clean detached human-live worktree `C:\cf7-integration\player-info-b0-human-live-20260729\resources` 精确切到 F2 后，先构建、再以 `CF7_PLAYER_INFO_FIXTURE_CASE=p50` 运行 `automation/dev.ps1 -ReuseOnly`，复用候选 `c-dae8a71c0ae0-08846e81b3-20260730t004223766z-e8ce2e3c`。启动结果为 `cf7-local-dev-runtime-result.v1 / candidate_executed / NOT_DEPLOYED`，Core、build identity 与 payload closure 分别仍为 `6BBE712C…C6CC`、`DAE8A71C…8ED1`、`051FA840…D2C`；`logs/launcher.log` 明确记录 `[PlayerInfoSplitSurface] fixture-only surface enabled; case=p50; old Flash HUD remains untouched`。进程核验时 Guardian PID 21628 精确来自上述 candidate；用户明确点击启动后，成功的子 Flash PID 20644 以 human-live worktree 的 `Adobe Flash Player 20.exe` 精确加载同目录 `CRAZYFLASHER7MercenaryEmpire.swf`，日志随后命中 `WaitingGameReady -> Ready`、`bootstrap_reveal_ready: Flash reveal cleared`、`performing reveal (panel swap)` 与 `[RevealProbe] setready.player_info_split 0.3ms`。更早 PID 12196 因未在 45 秒预热窗内收到 `start_game` 而正常 reset，只作启动页等待诊断，不是最终可见 attempt。本切片只改/执行 C# NativeHud fixture，未编辑或发布 FLA/XFL/SWF，因此不存在“NativeHud 源码因未重启 XFL 而不生效”的链路；若后续另改 Flash 资产，仍必须精确 publish 对应 XFL 并重启 player。该证据关闭 candidate execution、实际 Ready/reveal 与运行时 SWF 路径疑问，但不代签 DWM/game composite 观感、鼠标透传、IDE Output/来源内容一致性、业务 E2E、视觉审美、人类接受、promotion 或部署。

上述进程、active selection、成功/废弃 Flash attempt、日志 marker 与最终人签引用已冻结到 [`runtime-candidate-execution-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-candidate-execution-main-space-v2.json)：**3,960 B** / SHA-256 `F7A7B03F0333C61B467B50B1F7003D77764A130C7FE745098B9B0DFE09928E50`。人类接受本体由 [`human-acceptance-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/human-acceptance-main-space-v2.json) 持有：**3,509 B** / SHA-256 `FB2ED0D6550F51A715E21FB21D110B7EF5B1B04BD46FBC3D5FEA69653C105831`。两份文件与 current-truth 文档同提交后共同构成 tracked execution/human evidence。

**2026-07-29 RSL placement 纠错（覆盖本文更早的视觉收尾表述）**：B0-01A r4 已证明独立 child 文档与主游戏 RSL 是两条不同的根放置路径。直接加载 `玩家信息界面.swf` 会执行 child `DOMDocument` 中 `ty=3` 的包装 placement，使 HP/MP 合成为 `ty=5.65/-1.3`；主游戏则从 RSL 直接实例化 linkage-exported `玩家信息界面`，相对 main instance 的导出符号根为 identity（`ty=0`），HP/MP 运行时真值是库符号 local `ty=2.65/-4.3`。人类同屏复核已明确拒绝 v1：整体约下偏 3 logical px，且 HP 文字 Glow 偏弱；这不是“待签”状态。因此第七轮 TestLoader 截图及其 Flash/Web、C#/Flash 比较只保留为 `standalone_child_document_wrapper` 历史诊断，不能再通过补一张人工签名升级为 main-runtime `oracle_frozen`。v2 TestLoader 从精确绑定的 child SWF 中隔离 `loaded_child_exported_symbol_instance`，排除 document wrapper，并按 main placement `ty=512` 画入 1024×576 main-stage 画布；捕获 profile 为 `main_rsl_exported_symbol_equivalent`，参考 profile 为 `main_rsl_exported_symbol`，main SWF 只绑定身份链而不执行。真实 v2 candidate 已完成 11 case 捕获且工具身份通过 strict 检查；生成时四个 manifest human 字段为空，因此原 manifest 继续按不可变诊断输入保留 `candidate` 状态。随后人类对 exact F2 actual main runtime 的可见 `p50` composite 明确接受，并由 tracked human receipt 将该 live main 身份与上述 source-bound capture/closure 联合冻结为 `oracle_frozen_for_b0`；这不追溯改写旧 manifest，也不批准 v1 wrapper 或 cross-renderer pixel parity。
...
- 已完成：本 ADR、B0 分片边界、依赖准入门、runtime build identity 纳入方案与验收词典；Kimi k3/max 的文档建设审计见 §9.5，v1 实现首轮与 closeout 增量复审仅作为历史证据保留在 §9.6。main-space v2 已按 §9.7 由 `kimi-code/k3` 以当前最高可用思考强度 `high` 完成同一 session 的完整只读审计：165/165 request、5/5 config update 全部命中，P0/P1=0，`K1_AUTOMATED_GATE=PASS`。
...
- 已完成历史诊断、v2 capture 与 accepted main oracle：B0-01B 第七轮 v1 candidate 绑定 fresh TestLoader、实际 Debug Player、精确 child、11-case raw/crop 与安全恢复，但因包含 `ty=3` wrapper 永久降为 `standalone_child_diagnostic_captured`。v2 tooling 以 `main_rsl_exported_symbol_equivalent` / wrapper-excluded / 1024×576 合同完成真实 11-case capture，tool strict identity 为 `head_index_clean_filter_equal`，且 clean `40853287…` 的 direct A/B 已重新消费该 candidate。生成态 manifest 的四个人工字段仍保持原始空值；最终 tracked human receipt 通过 exact F2 actual main runtime 的可见接受与该 source-bound capture 联合形成 `oracle_frozen_for_b0`，不手改 manifest、不接受旧 wrapper、不声明像素 parity。
- 已完成自动结构、视觉诊断与人类接受：B0-04 的 8 个 canonical SVG、runtime manifest、repo-only provenance、strict validator 与 Web、FFDec/Web 诊断已闭合自动结构门；current asset 为 99,576 B / revision `d67ab427…92ba1` / manifest SHA `6F0AAA45…FEF826`。tracked `visual-evidence.v2` 冻结 placement-correct C#/Web/direct A/B；Web 仍只捕获 canonical static SVG，不含 C# 程序化文字/Glow，自动指标仍没有 parity verdict。一次性 converter 原型按 stop-loss 已移出权威工具树且不进入 build；最终 actual game composite、crop 与审美由人类 receipt 接受。
...
- v2 B0-05 自动资格已完成：clean `40853287…` 的 run `c42d755d…` 为 32/32，继续证明 typed runtime manifest、viewport-height/576、8 logical layer / 10 owned PArgb payload、单 worker latest-wins、16 MiB whole-batch LRU、100 churn/3000 current-hit、固定 16 轮 excluded warmup与 `split_required`；raw canonical envelope `285×81@(-3,474)`、main-visible envelope `282×81@(0,474)` 保持。
- v2 B0-06 自动资格、视觉诊断与现场验收已完成：clean `40853287…` 的 run `ed61d727…` 为 48/48，完整 Launcher 为 1747+3/1750，tracked visual index 已冻结两份 C#/Web/direct/viewer。独立 split surface、fixture widget、程序化 HP/MP、MP 双 fragment、path glyph atlas、prepared DIB 与 PanelHost 生命周期均进入 exact closure；最终 Kimi 按 §9.7 PASS，exact F2 `p50` 候选达到 Ready/reveal，且人类已接受可见 composite、鼠标透传、缩放外观与审美。
- v1 与 F2/r2 非部署 source-freeze 只作历史：F2/r2 的全新 tag/request、本地 X509 + GitHub OIDC 双 builder、两份 23/23 receipt、双 faultDomain quorum、不可复用 `-VerifyOnly` preflight 与 `p50` candidate execution 都不冒写为该列车已 promotion。PlayerInfo 实现字节随后由较新的 9118 formal release 带入正式 runtime；该总体 release 只做 Equipment Tuning 首 snapshot 纵切，不补签 PlayerInfo 专项门。
- 未改变：AS2 游戏状态/输入/冷却权威、`FrameBroadcaster` 协议、旧 HUD 可见性与旧 XFL/SWF、药剂拖放命中壳。正式 runtime 字节已随 9118 更新，但 standard-entry 没有启用 fixture surface 或真实 `pi_*`；fixture surface 仍是显式 opt-in 的并列诊断面，不会自动隐藏旧 Flash HUD。
- 总体视觉交付严格状态仍为 `b0_accepted`；资产/实现子状态为 `main_space_v2_automated_closeout_passed; human_accepted`，oracle 子状态为 `standalone_child_diagnostic_captured; main_runtime_oracle_frozen_for_b0`。`b0_accepted` 由自动门、Kimi 与 tracked human receipt 联合得出，不把 FFDec/Web 指标、错 placement 的 Flash candidate 或 9118 总体 promotion 冒写为真实 UiData、跨 renderer parity 或 PlayerInfo 业务 E2E。
...
本文严格区分四种帧口径：`DOM max index` 是 XFL XML 中最大的零基 `DOMFrame@index`；实际 timeline span 要按每层 `max(index + duration)` 取值；`display frame/state` 是 Flash 一基播放头可达状态；`ratio step` 是显示公式的比例格数。四者不得互换。HP/MP/韧性的公式步数分别是 128/100/30，不能由此反推每个 symbol 的 DOM 最大 index 或 timeline span。
...
**Oracle 轨**：第七轮 Flash candidate 仅保留为 standalone child 历史诊断，不能进入 accepted review。v2 TestLoader/runner 从精确 child 中隔离 exported-symbol instance，排除 `ty=3` wrapper，以 identity root + main `ty=512` 生成 1024×576 `main_rsl_exported_symbol_equivalent` capture；manifest/receipt 均为 v2，历史 v1 fail-closed。真实 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 完成 11 case、通过 strict tooling identity，并被 clean `40853287…` 的两份 direct comparator 重新消费。其生成态 manifest 继续保持 `candidate` 和空 human 字段；最终 human receipt 通过 exact F2 actual main runtime 的可见 `p50` composite、来源/进程/Ready/reveal 身份及该 11-case capture closure 联合形成 `main_runtime_oracle_frozen_for_b0`，不改写 ignored manifest，也不声明 cross-renderer pixel parity。

**工程轨**：B0-05 的 raster/cache/PArgb 与 `split_required` 架构裁决仍有效。main-space v2 已修正 child-stage 非裁剪语义、HP/MP placement、MP 文字锚点/底字、HP Glow 与横线效果，并把 C#/Web/Flash capture/comparator 统一到 main content viewport。clean `40853287…` 的 B0-05 32/32、B0-06 48/48、full 1747+3/1750、两份独立 C#/Web/direct 与诊断 viewer 已形成 final source-bound automatic evidence；最终 `kimi-code/k3 + high`、F2/r2 的双 builder/quorum/不可复用 preflight、exact isolated candidate Ready/reveal 和人类签收均已闭合。实现字节后来进入 9118 formal runtime，但本次只读 Tuning snapshot gate 不构成 PlayerInfo-specific E2E；B0 到此停止，B1 只能按既定 state-first 边界接真实 UiData。
...
player-info-hud converter ── validate/fail-closed
            ...
            └─ repo-only provenance/oracle     │
                ...
                Svg.Skia parse to SKPicture
                         │  只在首次尺寸/DPI key
                         ▼
    8 logical-layer / 10 owned-PArgb-payload batch cache
                          │
...
launcher/src/Guardian/Hud/PlayerInfo/
  ...
  PlayerInfoLayeredDibSurface.cs
...
tools/player-info-hud/
  ...
  evidence/
    ...
    b0-06/
      ...
      runtime-qualification.json          # current clean main-space v2 qualification
      visual-evidence.json                # frozen v2 automated index; its pending field is historical, later receipt closes current review
...
当前 v2 工具先以 `-ValidateOnly` 闭合 11 case、manifest/receipt v2、1024×576 canvas、`CLEAR`/`PART` round-trip、历史 v1 拒绝、false runtime claim 负例与 SWF 恢复正/负例，随后两份 tracked capture tooling 在 commit `fe56b2b9…` 取得 `head_index_clean_filter_equal` strict identity，并生成真实 candidate `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c`。该 candidate 是 source-bound 真实 Flash 捕获，不是 ValidateOnly synthetic artifact；它早于本轮整包 source freeze，故 `40853287…` 的 direct comparator 重新消费并复核了 tooling/source/placement closure。生成态 manifest 的四个人工字段继续为空且不手工改写；最终 `human-acceptance-main-space-v2.json` 通过 exact F2 actual main runtime 接受与该精确 capture closure 联合形成 B0 oracle，若 tooling/source closure 未来变化仍必须重采。
...
MP 的 shape tween 不保存 101 份路径。早期“无法稳定插值时先退为单 mask”只是施工备选，已被 B0-06 的两个独立 immutable PArgb fragment 与逐虚拟帧 mask correspondence 取代；当前实现不得再描述为单 mask。该实现本身只取得自动结构/确定性证据；v1 Flash candidate 已确认为 standalone child wrapper profile，不能据此写 oracle parity。其后的 exact F2 actual main runtime 人签与 source-bound main-RSL-equivalent capture 已联合关闭 B0 oracle，但仍不产生 cross-renderer pixel parity。
...
- B0-05 的 PlayerInfo 专用管线只缓存原子 whole-batch：8 个 logical layer 共同拥有 10 个 PArgb payload，其中 `mp.fill` 除 canonical payload 外还拥有左右两个 clip fragment；active+inactive 合计 16 MiB，单 worker latest-wins，旧 batch 只在新 batch 完整成功后换入。parse/raster 计数单位是成功完成且归属到 logical layer 的 owned payload，不能把 8 层误写成每批只有 8 次 parse/raster。它沿用 byte-budget/LRU 范式，不创建无预算全局字典，也不冒充通用 `BitmapLruCache` 实例。

### 7.4 B0-06 已实现边界

- `CF7_PLAYER_INFO_FIXTURE_CASE=<allowlisted-case>` 是唯一进程入口；空值保持完全不创建，`--bus-only` 或非 exact allowlist 值均 fail-closed。现役 11 case 是 `empty/min_step/p25/p50/p75/p99/full/mp_vf34/mp_vf35/mp_vf70/mp_vf91`，只供 fixture/取证，不是用户配置或生产数据源选择。
...
- MP fill 在 bake 时按 manifest 的两个 clip binding 生成独立、不可变、由 batch 所有的 PArgb fragment；合成器按虚拟帧建立 mask。HP 把固定覆盖轮廓与 `hp-fill-gradient-0003` 坐标旋转分开，不能旋转轮廓。数字使用由 XFL/SWF 来源与生成器身份约束的 Aero/LCD Std path glyph atlas，不读取机器字体，不新增运行时字体依赖；focused Gate 要求 UTF-8 无 BOM，把 checkout CRLF clean 为 canonical LF、拒绝 lone CR，再按 path/canonical byte length/SHA-256 校验 `PlayerInfoPathGlyphAtlas.Generated.cs` 与 provenance，任一漂移都 fail-closed。effect policy 的当前顺序是两个 `ImplementedActive`：`hp-mp-dynamic-text-and-glow` → `hp-horizontal-line-glow`；唯一 `DeferredB3` 是保留 authored `blendMode=overlay` 元数据的 `hp-light-overlay`。生产动态文字与横线均使用 ordinary source-over，Difference/Invert 只允许出现在诊断 viewer。`PlayerInfoCompositionRecipe` 精确冻结 9 个文字布局：6 个 MP、3 个 HP。MP current 保留 source-derived gauge-local `anchor=(74.55,17.85), right`；maximum 的 XFL authored 左锚仍是 `80.65`，但 C# path rasterizer 为匹配 Flash standard-text 的主画布 coverage 对其施加 `+0.30` local phase correction，故运行时 recipe 为 `anchor=(80.95,17.85), left`，不是改写 XFL 真值。两层装饰底字分别与前景保持同一 X 锚，即 `(74.55,18.05), right` 与 `(80.95,18.05), left`，内容固定为 `00000`；focused test 显式断言两对锚点相等。direct comparator 对四字段都要求 unique `bestDx=0`，并按真实对齐语义要求 label/maximum/percent 的 leading edge 与 current 的 trailing edge 均为 `dx=0`；cyan-weighted centroid 只作抗锯齿诊断，不设接受阈值。不能把旧 `78.55` current anchor、未校正的 maximum runtime anchor 或 `99999` 底字带回。HP 的 3 个布局保留 source red、blur 2、strength 1.5 语义；Skia 以 `sigma=1` 的两次 source-over blur（alpha 255 + 128）近似 strength 1.5，再绘制白色文字 core，不再使用 alpha 220 single-pass。横线从 `hp.rim` 中被静态隐藏的 exact source path 提取，在文字后绘制 red blur + white core；只有 `hp-light-overlay` 延后到 B3。compositor 对 disposition/order/ID、布局与 source path 漂移 fail-closed，不建立通用效果图。
...
- 2026-07-29 较早会话中，Computer Use 初始化曾在 native transport 报 `os error 2`，Browser runtime 当时返回 `agent.browsers.list()=[]`；这些都只是切换 ChatGPT App 与授予“完全访问”前的历史诊断，不能继续称为当前能力结论。切换后 workspace/terminal 为 unrestricted，Computer Use 与 Browser skill 均已重新暴露；本轮仍须实际初始化并把可见桌面/浏览器结果回写后，才可声明控制能力恢复。独立只读 `Get-Process` 快照只确认 `Flash.exe` 的 `MainWindowTitle=TestLoader.xfl`，且未见可识别的独立 Flash Player/Test Movie 进程或有标题窗口；它不能确认活动文档内容、Output 面板、publish freshness 或运行时实际加载的 SWF。仓内 FFDec 与 headless Edge/Playwright harness 仍可做确定性诊断，但即使 Apps 后端恢复，也不能“补算”真实手点、游戏 composite、遮挡层级或审美接受。
...
在登记的 Windows x64 基线机上。下列先保留 **v1 historical qualification** 与中间诊断，随后给出 current v2；历史值继续证明旧实现，但不得作为当前 v2 Gate。tracked `tools/player-info-hud/evidence/b0-05/runtime-qualification.json` 与 `evidence/b0-06/runtime-qualification.json` 现已由 current v2 snapshot 取代，旧报告只能从 Git history 读取。B0-05 的测量种类始终严格为 `synthetic_fixed_bounds`：
...
- 本轮 v2 在正式重跑前先修正 B0-05 测量合同：固定执行 16 轮 round-robin excluded warmup，共 4 viewport × 16 = 64 个完整样本；它与 acceptance 使用同一 `Task.Run`、round-robin、fresh SVG parse+raster+PArgb `Bake` 路径，不允许自适应提前停或改变进程优先级。runner 在启动任何 dotnet host 前拒绝会改变 JIT/GC 路径的已登记 `DOTNET_*` / `COMPlus_*` 覆盖，并由报告与脚本双向核验 Normal priority、继承的 affinity mask、server-GC 状态；当前合同共 32 Gate。随后仍为每 viewport 20 个独立 acceptance 样本、nearest-rank p95 `≤100 ms`，所有 64 个 excluded 样本也写入报告供 runner 独立重算。parse/raster 计数是完成的 StrictSvg/Skia payload-slot 操作总数，不宣称 PArgb copy completion 或逐 logical-layer 归因；独立 PArgb copy 诊断只为每个逻辑层复制一个代表性 payload，`mp.fill` 两个 fragment 不在该诊断中。中间 clean F=`48b1f524…` 的报告 run `db0527d4…c7fb` 为 739,608 B、SHA-256 `924BBBA1C6AB00A81F3AA66461165C208AC605BC8960595F73989A01F2A672F2`，32/32、`repositoryDirty=false`、`closureDirty=false`、`recordedDecision=split_required`；其后 prepared-DIB 修改触及该 37-file closure 内的 `LayeredWindowCommit.cs`、`OverlayBase.cs` 与 `LayeredWindowCommitTests.cs`，所以该中间报告已被 clean `40853287…` 的 current run supersede，最终同源结果见 §12“main-space v2 final source 与自动证据”，不得再把中间 F 写作 current qualification。
...
- B0-06 的 PlayerInfo split surface 单独记录 `repaintRequest / paint / commit / UpdateLayeredWindow` count 与 p50/p95/p99/max、tight surface 像素/字节；每个可见状态步最多 1 次 repaint、1 次 paint、1 次 commit，UI 同步工作 p95 `≤4 ms`，实际 split commit p95 必须 `<33 ms`。surface 为当前 tight physical bounds 持有一个可复用 top-down PArgb DIB + memory DC，尺寸变化时才重建；因此 `LayeredWindowCommitResult` 只计实际 prepared-DC `UpdateLayeredWindow` transaction，不能继续写成每帧 GDI setup/cleanup。DIB/DC setup+cleanup 摊入 surface 生命周期，仍由 lifecycle GDI/USER/process 零增长与趋势 Gate 约束。相对门与绝对门是独立拒绝条件，不得沉默放宽。
...
- B0-06 专用 runner 已实现 exact SDK 10.0.300、locked restore、单一 opt-in STA HWND/ULW test、fresh runId、canonical LF/无 BOM、报告源/二进制/renderer closure 自身份和 runner 侧重算；在启动任何 `dotnet` host 前，它复用 B0-05 的 exact 12 项 JIT/GC override 拒绝表，要求 runner priority=`Normal` 并冻结非零 affinity mask。testhost 报告原始 override map、实际/期望 priority 与 affinity、server GC，runner 再独立重建并要求 JSON 布尔字段类型严格、override 全空、priority=`Normal`、affinity 精确继承、server GC=false。该 execution-environment 合同直接决定 qualification status/failures，不新增 Gate ID，总数仍为 48；现有 `surface_lifecycle_warmup_converged` 有意从 v1 的单组收敛收紧为连续两组收敛，原 GDI/USER/process handle 零增长与趋势数值门不变。测试合同固定 3000 visible visual steps、3000 idle ticks和独立 100 次真实 HWND/surface acceptance lifecycle。lifecycle acceptance 前先在同一 STA/owner 上运行最多 5 个完整 100-cycle excluded warmup group；只有 GDI/USER/process handle 的全部 checkpoint 均不高于各自组起点、端点 delta 均不为正且三类都无正向单调趋势时，该组才严格收敛。必须连续两组各自满足该规则，首个合格 pair 后才立即运行一个新的、相邻的 100-cycle acceptance group；若五组内仍没有合格 pair，后续只能标 `diagnostic_after_warmup_cap`、`acceptanceMeasurementEligible=false`，既有 `surface_lifecycle_warmup_converged` Gate 必须失败。runner 还必须从每组 checkpoint/delta/trend 独立重建布尔序列与首个相邻 pair，不能只信报告自报索引。
- prepared-DIB dirty worktree 诊断报告 `tmp/player-info-hud-b0-06-runtime-qualification-v2-prepared-dib-dirty-20260729.json` 为 661,643 B、SHA-256 `253E3F4E34134AEF0F9289A9FBE0DE8AFC92DDAFF92699B3481CBBDCFCE568A0`，run `3b6e9aa8d0574a80a1f91bce82941513`、48/48、0 failure；33-file sourceTrace 精确包含新 helper，environment 为 Normal/精确继承 affinity/server-GC=false，连续收敛 pair 结束于第 4 组，surface/observer-commit/ULW-only p95=`3.3409/0.3052/0.3036 ms`。它只作冻结前历史诊断，已被下述 clean `40853287…` 报告 supersede。
- 两个被拒的 pre-merge run 已由 `runtime-qualification-attempts.json` 固化：v1 run `12f54a0…a3069cb6` 为 622,378 B / SHA-256 `01E68211…D867F`，USER `+2`、process `+35` 且 USER 呈正向趋势，仅 43/46；v2 run `4c3101b…43ae` 为 629,595 B / SHA-256 `C3FB74E7…2011`，固定一组 100-cycle warmup 后 acceptance 仍为 USER `+1`、process `+3` 且二者均呈正向趋势，也仅 43/46。两份早期合同当时总数就是 46；随后新增独立 `surface_lifecycle_warmup_converged` 作为第 47 个 Gate，才形成当时 v1 最终报告的 47/47，因此 43/46 与 47/47 是合同演进，不是分母误记。没有为通过而放宽原始零增长或趋势门。
...
pre-origin-sync `visual-evidence-premerge-v3.json` 与旧版 tracked index 继续作为 **v1 historical input** 保留；旧 tracked index 的 7,783 B / SHA-256 `6ED4A8662A4426C0C1EFD7B236406332D526991C6D3730E98546814F4B0EE9D6`、base commit `bf8dd2c410267855c8ea12f25a594042b3158479` 只能从 Git history 取回，不再是 current tracked path。当前 [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json) 已升级为 schema `cf7.player-info-hud.b0-06-visual-evidence-index.v2`，绑定 clean `40853287…` 并把 `finalTreeEvidencePending` 固定为 false：C# A/B 各 36 files / 35 PNG / 1,261,624 B / `DD58CA7C…6661E`，Web A/B 各 12 files / 11 PNG / 221,781 B / `9D21D823…17A7B`，direct A/B 各以 report-excluded 66 PNG / 3,114,450 B / `F05AE7F7…50ABF` 冻结，viewer 为 651,451 B / `2F71A3ED…01BC2`。direct 报告仍明确 `metricsOnly=true`、`parityClaimed=false`、`flashOracleAccepted=false`。

FFDec binary reference 另冻结 23 files / 22 PNG：PNG-only 为 245,587 B，30,433 B report 计入后完整目录为 276,020 B / closure `AEC84002…E59`，reference revision `sha256:a8369211…79c0`；FFDec↔Web A/B 的 33-PNG 子闭包均为 698,799 B / `BC2A83D1…9E08A`，standalone-child Flash-candidate↔Web A/B 的 33-PNG 子闭包均为 661,944 B / `6141D387…C185`。这些都是 placement 纠错前的 supplemental history，各 A/B report 因输入路径不同而保留不同哈希，不能写成“34/34 全文件相同”。current v2 index 顶层 `final_tree_diagnostic_awaiting_human_review` 与 comparator 的 `diagnostic_awaiting_human_review` 是自动报告自身的冻结状态；它们没有接受阈值，也不自行表示 Flash oracle 被接受。FFDec、headless Web、C# offscreen capture 与 placement-correct Flash candidate 仍只作机器诊断；其后的 actual main runtime 透明/游戏 composite、关键比例/平滑、旧 HUD 同屏、可见 DWM/z-order/occlusion、144 DPI 缩放外观与手工鼠标透传已由 tracked human receipt 接受。原始图片继续留在 ignored `tmp/` 供追溯，最终签收不回写自动 report 或宣称像素 parity。
...
B0 完成前必须使用本机 Kimi Code 的 `kimi-code/k3`，以其当前配置支持的最高思考强度做至少一轮**完整、只读**异构对抗；当前 CLI `0.29.1` 中该最高值为 `high`（旧 `max` 配置会迁移为 `high`）。不得因运行慢而终止思考、降 effort、换模型或把局部 smoke 冒充完整审计。审计 prompt 必须同时提供本 ADR、总体迁移 ADR、代码/资产 diff、结构化测试与视觉报告，并逐项挑战：
...
证据至少记录：CLI 版本、model、effort、session ID、prompt SHA-256、输入 commit/worktree digest、开始/结束时间、原始输出 SHA-256、逐条 `accept/reject/defer` 处置与复核测试。Kimi 配额不足、服务不可用或无法配置 `k3 + highest-available-thinking` 是明确 `blocked`，不能用同构 subagent 替代。

Kimi 通过也不代替人类。B0 最终视觉/UI包必须交由人类查看透明 crop、游戏 composite、关键比例和平滑录制，并取得明确接受；在此之前台账最多为 `awaiting_human_acceptance`，§11.3 不得全勾、目标不得报完成。当前 exact F2 可见候选已取得明确接受并由 tracked receipt 持有，所以本门已关闭；未来人类提出新的审美/交互修改仍须回到对应切片复测，再重新出终审包。
...
v1 实现包完整只读审计已于 2026-07-29 运行；首轮结论是 `REVISE_BEFORE_CLOSEOUT / K1_AUTOMATED_GATE=FAIL`，修复后的 closeout 增量随后由同一 `k3 + max` 配置复审到 PASS。该执行事实只满足 v1 当时的异构门；main-space v2 修改了 placement、资产、compositor、Flash/C#/Web capture 与 comparison tooling，故必须在新 source/evidence/doc 冻结后以当前 CLI 的 `k3 + highest-available-thinking` 重新执行完整审计，不能把本节 PASS 外推为当前 K1。
...
Kimi 另称 `PlayerInfoRasterLayer` 的便捷重载“无调用”，该事实不成立：`PlayerInfoRasterPipelineTests.cs:551-556` 有直接调用，故 reject；`SplitSurfaceSnapshot`/`CounterSnapshot` 合并与 `CombinedText` 只属弱评估项，没有足够锚点，不登记为已验证债务。v1 K1 PASS 当时只允许推进到 `awaiting_human_acceptance`；main-space v2 出现后状态曾退回 `visual_correction_in_progress; human_exit_blocked`。§9.7 的新 K1 与 F2/r2 全部自动门随后通过，exact F2 可见候选又取得人类接受，因此当前最终状态为 `b0_accepted`。

### 9.7 B0-07 main-space v2 最终完整异构审计

main-space v2 的完整只读审计已于 2026-07-30 完成。首次运行建立 session 后，本机因内存/存储压力重启；恢复时没有新开审计或更换模型，而是续跑**同一 session**直至自然 `end_turn`。最终裁决为 `APPROVE_AUTOMATED_CLOSEOUT / K1_AUTOMATED_GATE=PASS`，P0=0、P1=0；当时只授权继续全新 v2 tag/request、双 builder/quorum/preflight 与人类终审。F2/r2 随后已消费该授权并闭合自动 build Gate；两者都不表示 parity、`oracle_frozen`、promotion、部署或 `b0_accepted`。

| 字段 | 证据 |
|---|---|
| CLI / binary / session | Kimi Code CLI `0.29.1`；binary 129,918,464 B / SHA-256 `D8F142445DAB79F4D98DD924FF53B684C252664AD7A3F4A7A91D58AE87F611BC`；`session_cbe7e035-bcdd-475a-9147-5f021ff5700d` |
| 模型 / thinking | `kimi-code/k3`、当前 CLI 最高可用 `high`；session archive 中 165/165 个 `llm.request` 与 5/5 个 `config.update` 均命中，0 个 bad request/update；没有降 effort、切模型或因耗时终止 |
| 时间 / 中断恢复 | 首次 runner 于 2026-07-30 03:00:08 +08 启动；主机重启中断后台进程后，于 06:50:57 +08 续跑同一 session，07:12:14 +08 exit 0。中断只延长墙钟时间，不把前半程冒充独立审计 |
| 输入身份 / 只读 | HEAD `884fbf436f21c5e27ed3370c445cbae941349496`、tree `11d89d25001185aa2eb19d5bfbd36b579b38059d`、source parent F `40853287e7ed04714d68935c0002f8ad6d8aea05`；审计前后 `git status --porcelain` 均为 0。输入简报误写的 F 第二父 full hash已按仓库事实纠正为 `3df3bf3dd9d69c5e929e6f1eb856ed6c7fd17e6f` |
| Prompt / runner | 完整 prompt 13,036 B / SHA-256 `E0774D43FCCDCCA1C92D823436087A743517EB5402B389C37A56A97185E32FC4`；恢复 prompt 2,049 B / `0EBD2B630D332EF3397BF97BBC7368EA7F22C927B528BF9BD68B6451B6A3CAD4`；两 runner 分别为 8,300 B / `4F67DF77C4EC84A59D09707E0979E6CC09FDD99D30B0930580AD1733565F53BF` 与 7,645 B / `DBDB05B909533B6F0B7ADA3AD5D2D4DBCB3730677E0DDBE8894F9B5CF2381E71` |
| 原始输出 / archive | 最终 stdout 29,044 B / SHA-256 `CD69D86B00C619483E3CC67D3A434F21F0E63D9B84AF098389B9FCF2F5876287`；stderr 325,662 B / `5D794550CB0027D702AECE7409F3857BE618136F8F5DD3ADB761AAB78B5A7777`；ignored archive `tmp/kimi-b0-final-v2-mainspace/session_cbe7e035-bcdd-475a-9147-5f021ff5700d.zip`，1,804,810 B / `1A4356DA8F4B3D988CBEF60D70B783BA61A1F3C54EA1ADAB8C9E1955ED63B09D` |
| 视觉覆盖 | wire 中 67 次 media-read 记录；最终覆盖 23 个唯一 PNG，包含 C# case/HP 41-tick/contact sheet、两个 viewport/DPI、placement-correct Flash crop、C#↔Flash side-by-side/overlay/diff 与 Web 代表图；两次猜错路径均在列目录后重读，0 个最终未解决图像读取 |
| 最终裁决 | `APPROVE_AUTOMATED_CLOSEOUT`；`K1_AUTOMATED_GATE=PASS`；P0=0、P1=0。未重跑 build/test，只独立核验 tracked 报告字节、内部一致性、可复算字段与可视原件 |

审计对“sourceRef 指向尚不存在的 v2 tag”这一候选 P1 作 `reject`：审计当时 tag 尚未创建，正是 §11.3 规定的创建前状态；helper 对 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 三重绑定，缺失或指错均 fail-closed。随后 r2 tag 只在 F2 冻结后创建一次并精确指向 F2，GitHub run `30502544175` 已完成该三重绑定验证。

P2 逐条处置如下；没有一项授权在 builder 前“顺手”改 executable closure：

| P2 | 裁决 | 处置 |
|---|---|---|
| 1 B0-04 tracked validation report 仍是未显式标版的 99,564 B v1 资产快照 | accept | 本轮把工具 README 明确改为 historical v1 snapshot；current 99,576 B 字节由 B0-05/06、full 与 visual-evidence.v2 承载。若要刷新该报告，另开可复算 evidence 片，不覆盖历史含义 |
| 2 `visual-evidence.json` 没有 tracked 生成器/校验器 | accept | 当前由两次独立 A/B closure、Kimi 逐字段/字节复算补足可信度；后续给索引补 read-only verifier，不在不可变 F 前扩工具 |
| 3 MP `allFieldsAligned` 只写诊断，未被 gate fail-closed 消费 | accept | 当前两份报告已独立确认四字段 unique `bestDx=0` + anchor edge `dx=0`；下一次 comparator 工具片增加显式 require 开关并重跑，不因此移动当前锚点 |
| 4 split surface metrics `List<double>` 长会话无界增长 | defer | fixture-only 影响有限；B1 改环形缓冲或聚合，并补长会话验证 |
| 5 两种 split-surface snapshot record 字段重复 | defer | B1 代码卫生片评估合并；不为结构简化重开 F |
| 6 `gaugeLayerOrder` / `opacity` 校验弱且 runtime 未消费 | accept，defer | B1 明确为预留字段或改成逐值锚定；当前 manifest 与硬编码一致，不影响本包 |
| 7 PArgb bridge 的 negative-stride 寻址不完整 | defer | 引入新 bitmap source 前硬化；现役 top-down PArgb 路径不触发 |
| 8 `NativeHudTopologyProbe` 驻留生产程序集、仅测试消费 | defer | B1 评估移入测试程序集 |
| 9 `_currentLayerHitCount` 实为 layer 数，不是 request 数 | reject Kimi 的重命名建议 | 名称本就表示命中的 layer 数，并与 `CacheLayerHitCount` 对称；focused test 明确断言 `3000 × ExpectedAssetCount = 24,000`。当前语义与 Gate 一致，不登记债务 |
| 10 B0-05 默认增量 build、B0-06 强制 `--no-incremental` | accept | 下一次 runner 改动时统一；当前 B0-05 与随后 B0-06 记录的 Core/Test DLL SHA 完全相同，source closure hash 也已兜底，未发现 stale binary |
| 11 NativeHud A/B 固定 hidden-first，JIT 可能使回退门略宽 | accept | 下次重跑增加双侧预热或交错采样；当前原始 p95 为 `8.0754/7.598 ms`，独立丢弃 hidden 前 160 样本后为 `8.104 ms`、回退 `-6.244%`，未观察到 cold-JIT 制造通过 |
| 12 非首帧 `SetWindowPos` 失败只记录日志 | defer | B1 生命周期硬化为 dismiss 或置 dirty；当前下一帧自愈且无自动失败证据 |
| 13 `THIRD-PARTY-NOTICES` 只覆盖 PlayerInfo renderer，不是 Launcher 全量 notice | defer | 文件头已准确限定范围；既存全量分发治理另开片 |
| 14 ParseMatrix 重复、字面量/未用字段/全局 DTD 开关等代码卫生 | defer | 合并为 B1 卫生片；任何触及 `launcher/src` 的实施都须重跑 B0-05/06 |
| 15 MP percent 的 Jaccard 仅 0.4286，Flash bounds 比 C# 宽高各 1 px | accept | 几何锚点仍为唯一 `dx=0`，列入真实游戏人眼专项；若仍不适只做局部 glyph phase/outline，不平移整组 |

过度设计审计结论是保留 strict SVG 双层 fail-closed、8-field subpixel key、whole-batch atomic LRU、single-worker latest-wins、独立 split surface、path glyph atlas、双层 B0-05/06 gate 与 embedded-resource closure；它们各有当前风险/测量锚。通用 XFL→SVG converter、通用效果图和独立 schema 目录已正确停止扩张。视觉上不建议把生产叠层改为 Difference/Invert：这些只适合 viewer 诊断，游戏仍保持 source-over。自动图只排除了 MP 整体几何错位；HP halo 强度/色调、`%` 字形、真实 DWM composite、z-order/occlusion、鼠标透传、关键比例/平滑、缩放外观、运行时 SWF 身份和整体审美随后均由 exact F2 可见候选与人类 receipt 关闭；本轮未改 Flash 资产，因此不要求另一次 XFL publish/IDE 编译作为 NativeHud 生效条件。
...
- `planned`：只有 ADR/计划。
- `visual_correction_in_progress`：纠错实现或 source freeze 后正式 qualification/视觉/builder/Kimi 自动门仍至少一项未闭合；不能与“只剩人签”等同。
- `human_exit_blocked`：人类视觉/UI明确接受是 B0 的必要 Exit Gate；该词可以与未完成自动门并存，只声明人签不可省略，不声明它是当前唯一 blocker。
...
| HP 边框 placement 使用 `exportAsBitmap=true` + Bevel，Flash oracle 含派生 raster/filter 语义 | 不把它误判为源 bitmap；B0-04 candidate 已在 `hp.rim` 的 `hp-rim-static-bevel-expanded` 组展开为矢量渐变，但不声称 Adobe filter 像素精确；透明边/审美已由 exact F2 actual main runtime 人签关闭，仍不外推为像素 parity |
...
| 左下 PlayerInfo 与顶部/侧边 widget 被单一外接 union 合成，HP/MP 过渡时近全屏提交 | main-space v2 的 tight envelope 为 `282×81@(0,474)`；与右上块合成并按 6 px inflation/clipping 后仍占 viewport 约 93.40%、放大约 17.83×，继续接受 `split_required`。B0-06 用独立 click-through surface，不得回并现有 union；clean `40853287…` 的 B0-05 32/32 与 B0-06 48/48 已复核该拓扑/生命周期合同 |
...
| Computer Use/Browser 控制后端不可用，或把 authoring 窗口标题/历史截图误当成已运行 SWF | workspace/terminal full access 与 Apps 控制面分开记录；`os error 2` / Browser `[]` 只作切换 ChatGPT App 前的历史诊断。截至 2026-07-29，ChatGPT App 已暴露 Computer Use/Browser skill，但本轮尚未实际初始化，不能据“skill 可见”推断控制后端可用。`Get-Process` 只作进程/标题诊断；FFDec/无头 Edge 只作静态或 Web/SVG 诊断，实际 HWND/ULW 由专用 STA qualification 测，运行身份仍以 loader/player/SWF trace 为准，真实 composite、手点、遮挡与审美必须人签 |
...
- 环境配置经安全排查后仍不可完成，或 Kimi `k3 + highest-available-thinking` 因配额/服务不可用无法完成强制审计：状态记为 `blocked`，不得以降级模型或弱化 Gate 绕过。
- 自动 Gate 和 Kimi 均完成但尚无人类最终验收：状态记为 `awaiting_human_acceptance`；人类提出修改就退回对应切片重测，不得由自动指标或 Kimi 代签。
- 自动 Gate、Kimi 与 exact candidate 现场均完成且人类明确接受：记录 tracked human receipt 后状态记为 `b0_accepted`；该词只关闭 B0，不外推为真实 UiData、业务 E2E、promotion、标准入口或部署。

### 11.3 B0 Exit Gate

- [x] HP/MP active placement closure 已持久化并独立复核，达到 `placement_closure_frozen`；unused library 未混入，整座 HUD 的 BitmapFill 未误算进本纵切。
- [x] §6.4 oracle provenance、actual main runtime 与 main-RSL-equivalent capture 已联合冻结：exact F2 `p50` 可见 composite 由人类接受，capture 同时满足 `capturedPlacementProfile=main_rsl_exported_symbol_equivalent`、`referencePlacementProfile=main_rsl_exported_symbol`、wrapper=false/extraction/matrix 全合同；tracked [`human-acceptance-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/human-acceptance-main-space-v2.json) 将 source/runtime/capture 身份绑定为 `oracle_frozen_for_b0`。旧 `standalone_child_document_wrapper` candidate 与 ValidateOnly synthetic artifact 未补签冒充。
...
- [x] v1 两类正式 builder/F/quorum 事实仍按历史保留。main-space v2 首列 tag `…-b0-v2` 已冻结 `40853287…`，q3 本地票有效但 production policy 仅 22/23，故该列车已退役且未启动 cloud/quorum/preflight。最终 F2 `891d9b08…57ecd` 已由全新 `refs/tags/runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2` 与全新 q4/w4 fresh 取得 X509 + GitHub OIDC 双 signer、`physical-host-b` + `github-hosted-windows` 双 faultDomain、一致 identity/payload/Core 和不可复用 `preflight-passed`；旧 v1 与 q3 票均未代签或复用。
- [x] main-space v2 canonical SVG/manifest exact closure 已随 `40853287…` 冻结并由 strict/full 与 Web A/B 重验：8 SVG 合计 **99,576 B**，asset-set revision `sha256:d67ab427748f5256bd18ea5669c9f1d045a6adaac30dce96f9660ea20c892ba1`，manifest exact SHA-256 `6F0AAA452343319F861EBA8ED678ADE24E8B2B87CCD7CD01795D3D6B45FEF826`。旧 **99,564 B**、`sha256:c8f58cb7…93871`、`1006f90e…5630` 全部只属 v1 historical；本项仍不表示视觉被接受。
- [x] v2 SVG/JSON 与包图已由 F2/r2 fresh local/cloud 构建比对：33 个 payload 文件、payload closure `051FA840…D2C`、manifest `53F44B81…7261` 与 Core `6BBE712C…C6CC` 一致；manifest 独立计项，不冒称“34 个 payload 文件”，且未复用 q3 票。
- [x] qualification 编译闭包修复已在 exact F2 的新 local/cloud candidate 上分别取得 23/23 production policy：本地 receipt **12,144 B** / SHA-256 `91BBF333D4EC65A387D9A7CF94A73F8B43F54A77E05F7C684821F767FAF630D5`，cloud receipt **11,884 B** / SHA-256 `454EA66A721BC59E63E81169F5E4832490788FDC21CC2C37925E9D0B76FB2C9F`；修复前 22/23 与 focused xUnit 仍不代签。
- [x] B0-05 main-space v2 专用 qualification 已在 clean `40853287…` 上以 32/32 重新证明 8-field batch key（含 `sourceToBitmapIdentity`）、8 logical layer / 10 owned PArgb payload、固定 16 轮同路径 excluded warmup、3000 current-key 的 parse/raster/publish delta 与 exact executable source closure；run `c42d755dfa294ea5898366c7870e6ab8`，报告 **738,872 B** / SHA-256 `3C738AC233136B98978285F8BC8069A3890511742123F2BA58E8AC74D7B1C514`。
- [x] 同一 B0-05 clean 报告已闭合 Flash content viewport height / 576、monitor DPI telemetry-only、PArgb/透明边、四 viewport/letterbox、16 MiB cache、100 churn、dispose 与 `synthetic_fixed_bounds` bake 门；结论仅为自动 qualification，不是可见桌面或视觉验收。
- [x] `split_required` 决策由上述 32/32 structured report 重采确认，raw envelope `285×81@(-3,474)` / visible tight `282×81@(0,474)` 保持不变；B0-06 仍不得把 PlayerInfo 注册进现有 `NativeHudOverlay` union。
- [x] B0-06 main-space v2 正式 qualification 已在 clean `40853287…` 上通过 48/48：run `ed61d7275fdd4a96a73f762c5c2a4d71`，报告 **653,904 B** / SHA-256 `182C7DC9DE106B41AAF8073399DD390CA03A857347EA69A958E707CE771CA8E1`，覆盖 3000 visible + 3000 idle + 独立 100 lifecycle acceptance、完整 NativeHud union/独立 split surface、真实 ULW、handle/长循环与 exact source/binary/renderer closure。旧 47/47 仍只绑定 v1。
- [x] exact F2 的隔离候选已在 human-live worktree 以 `p50` fixture 进入 `candidate_executed / NOT_DEPLOYED`；启动输出重验 Core/build identity/payload closure，成功 Flash attempt 达到 Ready/reveal/player_info_split set-ready，命令行精确加载该 worktree 的 `CRAZYFLASHER7MercenaryEmpire.swf`，结构化证据见 [`runtime-candidate-execution-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-candidate-execution-main-space-v2.json)。本项只证明 candidate execution 与可见 surface 启动路径，不证明业务 E2E、视觉接受、鼠标透传、IDE Output、promotion 或部署。
- [x] fixture HP/MP 的 11-case main-space v2 自动视觉包已形成两份独立 C#/Web/direct A/B 与 viewer；两份 direct report 的 MP label/current/maximum/percent 都是 unique `bestDx=0`，且各自 leading/trailing anchor edge `dx=0`。其精确 closure 见 §12 当前证据段；报告仍为 `diagnostic_awaiting_human_review`，不产生 parity、`oracle_frozen` 或审美结论。
- [x] `PlayerInfoWidget.TryHitTest` 恒 false 的 focused test、surface `HTTRANSPARENT` 自动合同与 exact F2 可见候选的真实桌面手点均通过；人类接受回复关闭 `human_desktop_clickthrough`，自动项未代签手点。
- [x] `launcher/README.md`、testing guide、工具 README、runtime deep doc、automation README、总体迁移 ADR 与本台账已同步 clean `40853287…` 的 qualification/full/视觉 closure、F2/r2 自动闭环、exact candidate execution 与最终人签；旧 F/G docs-only 事实继续只作 v1 history，B0 当前为 `b0_accepted`。
- [x] main-space v2 的首轮 evidence/docs-only 后继 commit 已形成：commit `884fbf436f21c5e27ed3370c445cbae941349496` / tree `11d89d25001185aa2eb19d5bfbd36b579b38059d` 以 F=`40853287…` 为唯一父，只修改 5 份文档与 3 份 tracked B0-05/B0-06/visual v2 snapshot；提交前后 clean-tree、doc governance 与 diff check 均通过。它承载 K1 输入快照，不承载后来的 F2/r2 preflight；旧 `81168e…c549` 只承载 v1。
- [x] F2/r2 的 current [`runtime-promotion-preflight-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight-main-space-v2.json) 与本轮 current-truth 文档由同一收尾提交承载；报告 **15,128 B** / SHA-256 `2D0A2A862ADFFF4254FB8AFB910B1529568DABD33350AE8E735AC20F52B47356`，明确 `proofCount=2` 且五个 mutation/promotion/deployment/reuse 标志全为 false。
- [x] exact candidate execution 与人签由本次最终 closeout commit 同时承载：[`runtime-candidate-execution-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-candidate-execution-main-space-v2.json) 为 **3,960 B** / `F7A7B03F…928E50`，[`human-acceptance-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/human-acceptance-main-space-v2.json) 为 **3,509 B** / `FB2ED0D6…105831`；两者保持 `NOT_DEPLOYED`、无业务 E2E/promotion/standard-entry 边界。
...
- [x] §9.5 文档级 K0 与 §9.6 v1 K1 事实保留；main-space v2 已按 §9.7 以 `kimi-code/k3`、当前最高可用思考强度 `high` 对 clean `884fbf43…` 完成同一 session 的完整只读审计。165/165 request 与 5/5 config update 均命中，最终 `K1_AUTOMATED_GATE=PASS`、P0/P1=0；逐条 disposition 已记录，未因重启/耗时降级或切换模型。
- [x] 人类已明确回复“可接受，可继续推进”，接受最终透明 crop、游戏 composite、关键比例、平滑、144 DPI/dpr 1.5 缩放外观、UI 交互与整体审美；签收由 tracked human receipt 持有，自动指标与 Kimi 未代签。
...
| B0-01A | completed (`placement_closure_frozen`) | r4：16-file exact closure、HP/MP/shared=10/5/2、17 authored/18 expanded edge、14 timeline、0 BitmapFill/DOMBitmapInstance；Git-canonical digest `6f4bf9…4368`；index/clean-filter/anchor/source-binary verifier 全绿；另独立证明 standalone root `ty=3`/HP-MP `5.65/-1.3` 与 main RSL root `ty=0`/HP-MP `2.65/-4.3` | B0 内无未验证项；旧 wrapper 仍只作历史诊断 | closure/source/placement 改变时重跑，不外推到 B1 |
| B0-01B | completed (`main_runtime_oracle_frozen_for_b0`) | 第七轮 v1 candidate 保留为 wrapper 错位历史。v2 run `20260729T143326Z-32f8ec2142ec47e29c5b191cf2ee339c` 完成 11 case、wrapper=false、main `ty=512`、`head_index_clean_filter_equal` strict identity，且 `40853287…` direct comparator 已重新消费。exact F2 actual main runtime `p50` 达到 Ready/reveal，tracked human receipt 接受 live composite 并绑定 candidate manifest/PNG closure | ignored manifest 保持原始 `candidate`/空 human 字段；不声明旧 wrapper 或 cross-renderer pixel parity | B0 oracle 已关闭；capture/tool/source 漂移时重新取证 |
...
| B0-04 | completed (`canonical_asset_human_accepted`) | `40853287…` 冻结 8 SVG / **99,576 B**，revision `d67ab427…92ba1`、manifest SHA `6F0AAA45…FEF826`；strict/full、Web A/B、placement-correct C#/direct 自动链通过，actual main runtime 的透明 crop、HP Glow/gradient、MP variants 与整体审美由人类接受。旧 **99,564 B** / revision/hash 与 standalone Flash/Web closure 只作 v1 history | 不声明 cross-renderer pixel parity | 保留 exact closure；资产/manifest 改变时重跑 B0-04/05/06 相关门 |
...
| B0-05 | `main_space_v2_qualified; split_required` | clean merge `40853287…` 的专用报告 run `c42d755dfa294ea5898366c7870e6ab8` 为 32/32、**738,872 B**、SHA `3C738AC2…1C514`；8 logical layer / 10 owned payload、8-field key、16 轮 excluded warmup、四 viewport、PArgb/cache/churn/dispose/topology 与 exact executable closure 全部通过，`split_required` 保持。v1、被 supersede 的中间 clean F 与 dirty 诊断只作历史 | 这是 `synthetic_fixed_bounds` 自动资格，不含可见桌面/游戏 composite/人签 | 保持独立 split surface，不回并现有 NativeHud union；后续 source/asset/executable closure 变化必须重跑 |
| B0-06 | completed (`main_space_v2_human_accepted; candidate_executed`) | clean merge `40853287…` formal run `ed61d727…` 为 48/48、**653,904 B**、SHA `182C7DC9…CA8E1`；clean full 1747+3/1750。C#/Web/direct/viewer 自动包确定，MP 四字段 unique `bestDx=0`/anchor `dx=0`，§9.7 K1 PASS。F2/r2 fresh tag/request、local/cloud 23/23、identity `DAE8A71C…8ED1`、33-file closure `051FA840…D2C`、双 faultDomain 与 `preflight-passed` 一致；exact F2 `p50` 候选达到 `candidate_executed / NOT_DEPLOYED`、Ready/reveal/player_info_split，旧 Flash HUD 不变，人类 receipt 接受 composite/click-through/缩放/审美 | 真实 UiData、业务 E2E、promotion、标准入口与部署不属于 B0 且仍未完成；不声明 parity | B0 停止；B1 仅按 state-first 草案接真实数据，旧 q3 证据不复用 |
| B0-07 | completed (`k1_passed; human_accepted`) | §9.7 同一 Kimi session `session_cbe7e035-bcdd-475a-9147-5f021ff5700d` 在 `k3/high` 下完成；165/165 request、5/5 config update 命中，P0/P1=0，裁决 `APPROVE_AUTOMATED_CLOSEOUT`；最终 human receipt 记录“可接受，可继续推进” | 无 B0 未验证项；Kimi/自动图均未代签人类 | B0 状态转为 `b0_accepted`；后续变化按对应 slice 重新开门 |

表中 B0-03b 行的“两正式 builder 未验证”仅描述其早期 renderer-qualified candidate `c-c571…` 自身，不能外推到后续 F；v1 source-freeze 的正式 builder/quorum 曾由旧 B0-06 行与 preflight 报告闭合。反过来，F/F2 的 proof 也不重写早期 candidate 的 payload/Core 身份。F2/r2 proof 关闭当前 main-space v2 builder/quorum Exit，actual main runtime + human receipt 另行关闭 oracle/桌面/视觉 UI Exit；两者都不表示 promotion 或部署。
...
**main-space v2 runtime/visual implementation source 与自动证据（2026-07-30；报告 UTC 日期为 2026-07-29）**：实现冻结先形成 commit `7e718a1fad83f2bc4ce92605d60d2afbf7e86e1a`，随后 merge commit `40853287e7ed04714d68935c0002f8ad6d8aea05`（tree `9c0909d7260d741fde34ac29c31a668babb330d1`；parents=`7e718a1…`、`3df3bf3…`）集中纳入当时的 `origin/main`。第二父提交只落在武器输入/`things0`/`asLoader.swf` 相关路径，与 PlayerInfo source、canonical asset、capture/comparator 输入无交集；下述 fresh 报告都直接绑定 clean `40853287…`，不把 merge 前报告外推到 merge 后，也不冒写为后续 F2 重跑结果。B0-05 run `c42d755dfa294ea5898366c7870e6ab8` 为 32/32，报告 `tmp/player-info-hud-b0-05-runtime-qualification-v2-f40853287-20260729.json` 为 **738,872 B** / SHA-256 `3C738AC233136B98978285F8BC8069A3890511742123F2BA58E8AC74D7B1C514`；B0-06 run `ed61d7275fdd4a96a73f762c5c2a4d71` 为 48/48，报告 `tmp/player-info-hud-b0-06-runtime-qualification-v2-f40853287-20260729.json` 为 **653,904 B** / SHA-256 `182C7DC9DE106B41AAF8073399DD390CA03A857347EA69A958E707CE771CA8E1`。同一 clean source 的完整 Launcher 回归为 **1,747 pass + 3 opt-in skip / 1,750 total**。

**main-space v2 final visual A/B（2026-07-30；报告 UTC 日期为 2026-07-29）**：正式 closure 一律以 tracked `visual-evidence.v2` 的 canonical directory contract 为准。两份 C# capture 各为 **36 files / 35 PNG / 1,261,624 B**，`completeDirectory` SHA-256 均为 `DD58CA7C84D70ADB1890A75AD133755BC9A7BA1051072DA282A8AF1B4956661E`；两份 Web capture 各为 **12 files / 11 PNG / 221,781 B**，`completeDirectory` SHA-256 均为 `9D21D823AD4B905011AF0C253A5456C04CE0224AD92296686C13E2A77E817A7B`；两份 direct comparator 各输出 **66 PNG / 3,114,450 B**，report-excluded output closure 均为 `F05AE7F77CA5034D3F645480FCE110D745913F6539FBB767682AFC4AA8B50ABF`。A/B direct JSON 各 **236,862 B**，SHA-256 分别为 `B2FF695A54CA872AA35F143AF72C6DBC03B9C0AD6C5379D68ED3F84C7C017095` 与 `F5C57984D7E617CBC300D9AC9344114A707BB4E6C1F438FDB57B175BA244F0C5`；viewer `tmp/player-info-hud-b006-viewer-v2-f40853287-a.html` 为 **651,451 B** / SHA-256 `2F71A3ED50F2626E497999A88E6F0F6E35EDD0ACD593411B5794921E64501BC2`。两份 direct report 都对 MP label/current/maximum/percent 得到唯一 `bestDx=0`，且 label/maximum/percent 的 leading edge、current 的 trailing edge 都为 `anchorEdgeDx=0`。这些 closure 只证明独立捕获与自动诊断确定性；direct report 内的 `parityClaimed=false`、`flashOracleAccepted=false` 继续保持真实。最终 `oracle_frozen_for_b0` 来自其后的 actual main runtime + human receipt 联合决策，不回写自动报告，也不声明像素 parity。

**F2/r2 build-train 自动闭环（2026-07-30）**：最终 build source F2=`891d9b08dbd826d8b2624c6bdc59082b3db57ecd` / tree=`5b65f9fde417f35ce34012edf68fd9c899cb1813`；tag=`runtime-build-v2/20260730-player-info-nativehud-b0-v2-r2`；request=`6B5E9BDD5393553871C646952B1266E877D8B8941D50AB510B5387FC1261460D`。artifact/recipe/toolchain/policy/build identity 五域分别为 `6AAF891B6A27681B7493782FF9D85B3527DBD4B25AB086AABEDEEC02A062A4E0`、`B97998EA7246D6AB667902BCBBD7994DFA5F658A37CFA427D2EEEABA6924DE28`、`7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD`、`4F3F7749564CD82272B014C5EFF7A962FB63A533185AD3B27612C79CFF12744B`、`DAE8A71C0AE04AF1BD89B0076DB1EF6EF5DFEEC51C32843D300D868581568ED1`。本地 X509 `EB5D32E04B6EE8697850314E19698DE1A3FACFFCCC6418A12CF7FEDE6033CDA5` / `physical-host-b` 与 GitHub OIDC `1B2F3B53CC50695D620A90CE423BEF3FC55C0866AEE958E7A8042BE682185630` / `github-hosted-windows`（run `30502544175`）对同一 33-file payload closure `051FA840019E24763724B5DA0868486AF94322F3BB990B1354D181989AA94D2C`、Core `6BBE712CB0F97AC127FA3DF18158CA3976D7A03A64DC8121794E7C7C1E18C6CC`、manifest `53F44B8188A425728975197CD8B38FF5CE523B0306CA2EB8EE1ECBD76FF77261` 达成共识。local/cloud receipts 分别 23/23；[`runtime-promotion-preflight-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight-main-space-v2.json) 为 `preflight-passed`、**15,128 B**、SHA-256 `2D0A2A862ADFFF4254FB8AFB910B1529568DABD33350AE8E735AC20F52B47356`、`proofCount=2`，runtime/release-state/promotion/deployment/reusable flags 全为 false。该段的 VerifyOnly 结论严格只到 `candidate_built / NOT_DEPLOYED`；它没有执行 candidate、E2E、promotion、标准入口或部署。

**F2 隔离候选执行（2026-07-30）**：随后在 clean detached human-live worktree 以 `CF7_PLAYER_INFO_FIXTURE_CASE=p50` 运行 `automation/dev.ps1 -ReuseOnly`，精确复用 candidate `c-dae8a71c0ae0-08846e81b3-20260730t004223766z-e8ce2e3c`；启动输出为 `cf7-local-dev-runtime-result.v1 / candidate_executed / NOT_DEPLOYED`，并重验 Core `6BBE712C…C6CC`、build identity `DAE8A71C…8ED1` 与 payload closure `051FA840…D2C`。`logs/launcher.log` 记录 fixture-only surface `case=p50` 且旧 Flash HUD 不变；用户点击启动后，exact candidate Guardian 的成功子 Flash PID 20644 从 human-live worktree 加载 `CRAZYFLASHER7MercenaryEmpire.swf`，并依次命中 `Ready`、`bootstrap_reveal_ready`、panel swap 与 `setready.player_info_split`。F2/r2 该专项列车的严格生命周期因此停在 `candidate_executed / source-ahead / NOT_DEPLOYED`；它没有业务 `e2e_verified`、promotion、标准入口或部署。较新的 `9118eb…` release 包含其实现字节，但没有运行 PlayerInfo-specific standard-entry E2E，不会反向升级该专项列车结论。

本次执行快照由 [`runtime-candidate-execution-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-candidate-execution-main-space-v2.json) 持有，大小 **3,960 B**、SHA-256 `F7A7B03F0333C61B467B50B1F7003D77764A130C7FE745098B9B0DFE09928E50`。其 `humanAcceptance` 指向 [`human-acceptance-main-space-v2.json`](../tools/player-info-hud/evidence/b0-06/human-acceptance-main-space-v2.json)，后者为 **3,509 B** / SHA-256 `FB2ED0D6550F51A715E21FB21D110B7EF5B1B04BD46FBC3D5FEA69653C105831`，记录 `ACCEPTED_FOR_B0`、原文“可接受，可继续推进”、actual runtime/capture/oracle 绑定及全部发布负面边界。

**B0 最终关闭（2026-07-30）**：Kimi Code `kimi-code/k3` + 当前最高可用 `high` 的完整异构对抗按 §9.7 通过；exact F2 candidate 的 process/SWF/Ready/reveal/player_info_split 身份、144 DPI/dpr 1.5 可见 game composite 与手点随后由人类明确接受。Computer Use native pipe/Browser backend 不可用只保留为历史环境诊断，没有代签任何桌面结论。由于本片只改 C# NativeHud、未改 FLA/XFL/SWF，进程命令行与旧 HUD untouched marker 已关闭“加载错 SWF/未重启 XFL”疑问，本片不要求另一次 Flash publish。全部 B0 Exit Gate 已闭合，状态为 `b0_accepted`；F2/r2 该专项列车自身没有真实 UiData、业务 E2E、promotion、release-state、标准入口或部署。实现字节后来随 `9118eb…` 总体 runtime 部署，但本轮没有 PlayerInfo-specific standard-entry E2E，不能据此升级 B0/oracle 结论。
...
> B0 已完成并记为 `b0_accepted`，oracle 为 `oracle_frozen_for_b0`。F2=`891d9b08…57ecd` 的 r2 build train 仍按其历史事实停在 `candidate_executed / source-ahead / NOT_DEPLOYED`；其实现字节随后被 `9118eb5097…` 正式 source/runtime 包含。9118 release 已 promotion 并达到总体 `standard_entry_verified`，但正式入口只执行 Equipment Tuning opener 与同实例首个权威 snapshot，没有启用 PlayerInfo fixture、观察真实 `pi_*` 或形成 PlayerInfo-specific E2E；不能从总体部署升级 B0/oracle 结论。后续若进入 B1，严格按 §7.5 state-first 草案另开实施/验证，不复用 B0 fixture 或 9118 Tuning smoke 冒充运行态接入。
