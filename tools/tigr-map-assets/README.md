...
| 地点 | 猛虎车号 | 世界接地点 | 显示倍率 |
|---|---|---|---|
| 外交军阀基地中间后景，替换26号犀牛 | 41 | (1010,185) | 23% |
| 防御地带高地，替换解放卡车 | 42 | (400,340) | 45%，右向，顺坡-2° |
| 军阀前线基地第二张图，三组帐篷后 | 43、44、45 | (250,200)、(860,205)、(1650,205) | 24%、25%、24% |

外交基地保留25号犀牛，原27号位置由磁暴坦克接替；前线第三张图22/23/28号犀牛的位置和遮挡维持既有结果。高地29号犀牛向右移至(990,320)，给猛虎留出间距。所有车辆仍位于行走区后方，不增加碰撞或改变关卡脚本。
...
先同步犀牛，再同步猛虎，最后同步磁暴坦克；外交三种车辆的图层相邻。猛虎工具复用犀牛工具的 XML/闭包/摆放函数，分别限定命名空间。车号41–45是矢量笔画，右向包装局部反射，整车镜像后仍正读。日常修改基础分件后同步，不要重复 `--import-xfl` 覆盖已有改稿；该开关只用于首次读取外部原生 XFL。
