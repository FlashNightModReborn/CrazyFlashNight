...
**最后核对代码基线**：commit `2d19cd6681a0219749a58501271b6f1bd23cc28f`（HEAD / `origin/main`，2026-07-21 断电恢复后复核）+ 当前尚未晋级的本地 S0/S1/S2 合并工作树；主线允许 native 源码 fast-forward 直推并由事后 Audit 报告 `source-ahead`，但这不构成正式 Core 部署证据。Skill、合成、双栏工作台与 runtime v2 的细分门以对应专题 canonical doc 为准。
...
当前真人业务证据已覆盖 attempt `62f04…1754` 的正常 NativeHud 入口、同实例 organizer、真实一次点击 modal、discard、fresh ACTIVE 返回、继续领取与 terminal close/unpause 主链，以及普通主页单次 X→`LOOT_SUSPENDED`→同锚点内容不变重开→最终领取；历史 `b9bb…2748` 与冷启动 `7560…add7` 仍只负责旧流程经济写 / 跨进程持久化旁证。当前 `c-2a0cddb0…f32a40a3` 的单 canary 资源箱切片为 **e2e_verified / NOT_DEPLOYED**，候选已优雅退出且 Flash CS6 保留；地图临时夹具也已恢复并以 stage **41/41**、map **243/243** 复验。完整 S1/S2 资产与 S0/全局 arbiter 人工门仍独立保留，正式 promotion 与 standard-entry 继续受各自门禁约束。
