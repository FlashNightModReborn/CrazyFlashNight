...
_root.主角函数.重量速度关系 = function(重量:Number, 等级值:Number):Number {
    ...
    if (重量 < 轻甲下限)
        return 1 + _root.超重惩罚;
    if (重量 < 轻甲阈值)
        return 1 + (轻甲阈值 - 重量) / (轻甲阈值 - 轻甲下限) * _root.超重惩罚;
    if (重量 <= 重甲阈值)
        return 1;
    if (重量 < 重甲上限)
        return 1 - (重量 - 重甲阈值) / (重甲上限 - 重甲阈值) * _root.超重惩罚;
    return 1 - _root.超重惩罚;
};
...
人形怪技能表.push({技能名: "气动波", 点数: 15, 冷却: 5, 消耗: 20, 限制: 1, 距离min: 0, 距离max: 400, 类型: "格斗", 功能: "远程输出"});
...
_root.主角函数.初始化可用技能 = function() {
    ...
    for (var i:Number = 0; i < 技能表.length; i++) {
        if (技能表[i].限制 <= 等级) { //检测装备，只有装备刀才能学刀技技能，只有装备枪才能学火器技能
            if (!((持刀检测 && 技能表[i].类型 == "刀技") || (持枪检测 && 技能表[i].类型 == "火器"))) { // 存储符合条件的技能索引
