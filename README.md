...
- **适用场景**：
  ...
  - 单个角色或物品素材
  - 独立的背景图片和简单动画
- **限制**：无法进行版本控制差异比较，不便于团队协作

**XFL格式（eXtensible Flash format）**
- **文件结构**：解压缩的文件夹，包含入口xfl文件，DOMDocument.xml主文件和LIBRARY、META-INF等子文件夹
- **特点**：XML文本格式，便于版本控制和团队协作
- **适用场景**：
  - 复杂界面系统（如任务栏界面、地图界面、系统设置UI）
  - 需要团队协作的大型素材库（如统合美术资产、素材库-地图元件）
  - 动画效果丰富的界面（如物品与技能相关界面）
- **优势**：可以用文本编辑器查看部分内容，支持Git等版本控制系统

**使用建议**：
- 团队开发优先使用XFL格式，便于版本控制和多人协作
- 简单素材可使用FLA格式，减少文件夹复杂度
- 大型项目建议统一使用XFL格式，便于资源管理和维护
...
**A:** ActionScript需要重新编译，具体步骤取决于源文件格式：

**对于FLA文件（.fla）：**
1. **在Flash CS6中打开对应FLA文件**
   ```
   找到 flashswf/arts/ 或相关目录下的 .fla 文件
   双击打开，或在Flash CS6中选择"文件" → "打开"
   ```
...
**对于XFL文件（.xfl文件夹）：**
1. **在Flash CS6中打开XFL项目**
   ```
   找到 flashswf/UI/ 或相关目录下的 同名 文件夹
   选择文件夹内的 xfl 文件打开
   ```
...
**注意事项：**
- XFL格式的项目会在文件夹内的bin子目录生成SWF文件
- 确保发布设置中的ActionScript版本设置为2.0
- 如果是团队开发，推荐使用XFL格式便于版本控制
