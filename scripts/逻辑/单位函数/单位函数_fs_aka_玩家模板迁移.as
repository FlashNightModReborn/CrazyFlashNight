...
_root.刷新人物装扮 = function(目标)
{
	...
	else
	{
		...
		if(目标人物.装备强化度){
			// _root.发布消息(目标人物.装备强化度.头部装备 + typeof 目标人物.装备强化度.头部装备);
			头部强化等级 = 目标人物.装备强化度.头部装备;
			上装强化等级 = 目标人物.装备强化度.上装装备;
			手部强化等级 = 目标人物.装备强化度.手部装备;
			下装强化等级 = 目标人物.装备强化度.下装装备;
			脚部强化等级 = 目标人物.装备强化度.脚部装备;
			颈部强化等级 = 目标人物.装备强化度.颈部装备;
			长枪强化等级 = 目标人物.装备强化度.长枪;
			手枪强化等级 = 目标人物.装备强化度.手枪;
			手枪2强化等级 = 目标人物.装备强化度.手枪2;
			刀强化等级 = 目标人物.装备强化度.刀;
		}else{
			头部强化等级 = 强化等级;
			上装强化等级 = 强化等级;
	...
	if(攻击模式 != "空手" && 目标人物.状态.indexOf(旧攻击模式)!= -1){
		...
		if(旧攻击模式 == "手枪" && !手枪) 是否卸下装备 = true;
		if(旧攻击模式 == "手枪2" && !手枪2) 是否卸下装备 = true;
		if(旧攻击模式 == "双枪" && !双枪) 是否卸下装备 = true;
		if(旧攻击模式 == "刀" && !刀) 是否卸下装备 = true;
		//
		if(是否卸下装备){
			目标人物.状态 = "空手站立";
			目标人物.攻击模式 = "空手";
...
_root.主角函数.初始化可用技能 = function()
{
	...
	技能表.push({技能名:"兴奋剂", 点数:10, 冷却:120, 消耗:16, 限制:1, 距离min:200, 距离max:400, 类型:"增益", 功能:"速度"});
	技能表.push({技能名:"小跳", 点数:10, 冷却:1, 消耗:5, 限制:1, 距离min:0, 距离max:100, 类型:"躲避", 功能:"高频位移"});
	技能表.push({技能名:"铁布衫", 点数:20, 冷却:120, 消耗:16, 限制:10, 距离min:200, 距离max:400, 类型:"增益", 功能:"防护"});
	技能表.push({技能名:"闪现", 点数:30, 冷却:2, 消耗:10, 限制:25, 距离min:0, 距离max:100, 类型:"躲避", 功能:"长距位移"});
	技能表.push({技能名:"霸体", 点数:10, 冷却:20, 消耗:40, 限制:15, 距离min:0, 距离max:400, 类型:"增益", 功能:"解围霸体"});
	技能表.push({技能名:"觉醒霸体", 点数:150, 冷却:120, 消耗:60, 限制:30, 距离min:0, 距离max:400, 类型:"增益", 功能:"解围霸体"});
	技能表.push({技能名:"寸拳", 点数:10, 冷却:5, 消耗:20, 限制:1, 距离min:30, 距离max:100, 类型:"格斗", 功能:"爆发输出"});
	技能表.push({技能名:"气功波", 点数:15, 冷却:5, 消耗:20, 限制:1, 距离min:0, 距离max:400, 类型:"格斗", 功能:"远程输出"});
	...
	技能表.push({技能名:"组合拳", 点数:5, 冷却:5, 消耗:15, 限制:10, 距离min:30, 距离max:60, 类型:"格斗", 功能:"贴身持续蓄力输出"});
	...
	技能表.push({技能名:"瞬步斩", 点数:20, 冷却:5, 消耗:20, 限制:10, 距离min:50, 距离max:250, 类型:"刀技", 功能:"位移持续输出"});
	...
	技能表.push({技能名:"能量盾", 点数:20, 冷却:60, 消耗:50, 限制:10, 距离min:100, 距离max:400, 类型:"增益", 功能:"防护"});
	技能表.push({技能名:"翻滚换弹", 点数:15, 冷却:15, 消耗:50, 限制:15, 距离min:200, 距离max:500, 类型:"火器", 功能:"换弹"});
