...
**最后核对代码基线**：release source commit `a4a85dbdcb266f66677eef28875e0862892e48ad`（2026-08-25；implementation `920f2c7d8b30cf4ec19204f2490fc3be44b69c3b`、发布输入/EOL 修正 `4c2426f7b4df18ab1ceaa75daf4413b2cc90a6e5`、tag `runtime-build-v2/20260825-stage-time-pools-v3`、tree `0b4778aa346a6acce276052debf79a93af7a39a1`、request `ADDBA21EF66AA9429D00D349E0ACD33F27BE55641CB3B9F541ACB6BEAC47D043`、deployment `9f68a3ee5fbd6db9447118da12fa0fa0a00d1829`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `B4A625D76B6E132856557C169BA3C9FA63C2B3147ABCF8AB0B905FF354C8DF40` / `github-hosted-windows`（cloud run `32837546069`）已对 identity `50ED16457B8C82787A495F957259A9544AD96C819E8D1EF11087D5AF06E0BFB0`、closure `60981913A1D18682C06B6ABF2CC6DB7EC0F57345BAF8B1D372E67D7EC3ADE5EB` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `1AA2774BFD49A579EDE75E1593CD83DF3832A9A8693E9D2C629ACDB1DC32900A`，39/39 production receipt SHA-256 为 `F69E8A0569838920B86A42F3B704EABFEEF5AE4E6193A6389FB97055DDDAD9B4`；deployment commit `9f68a3ee5fbd6db9447118da12fa0fa0a00d1829` 与首次 post-promotion audit run `32838658629` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车把会话级 `GameStage.TimePools`、任意 `SubStage` 的零到多池引用及 keyed NativeHud 倒计时接入正式 runtime；同一池可跨不连续子图累计，同一子图也可同时属于多个池。AS2 `StageTimePoolController` 独占倒计时与失败裁决，Launcher 的 `T` 快车道只展示；JK 救援 300 秒、核电站四图 600 秒、断壁残垣前两图 600 秒为首批试点。
fresh 门包括 XML validator `configured=3 / pools=3 / refs=7`、AS2 focused `46/46`（8 cases）与 Compiler `0/0`、Launcher focused `16/16`、Launcher 全量 `4115 pass + 3 explicit opt-in skip / 4118 total`、runtime v2 `181/181`；重新发布的 `scripts/asLoader.swf` 为 1,147,468 bytes / SHA-256 `3BCC14D3A74FB71E366605A50520ACFDB88BDB5F5DD6CB4573055DD40E93545C`。production policy `39/39`、worktree/index 33-file bundle、signed consensus、正式根 bootstrap `--verify-only`、远端 Audit 与无 candidate selector 的正式入口身份/生命周期 smoke 均通过。
维护者已确认限时关卡行为有效；部署后的 formal run `eb0d6743820512eccccfc6138cdf5605` 覆盖 fresh reveal、两次同 lifecycle `session.status=verified`、精确 identity/closure、strict shutdown、Flash/Guardian exit 0、玩家调制档哈希不变与零残留，但没有重跑 JK、核电站或断壁残垣业务。因此准确状态为限时关卡 `HUMAN_ACCEPTANCE_PASSED / promoted`，正式入口身份/生命周期窄纵切达到 `standard_entry_verified`，不称限时关卡业务 `standard_entry_verified`；Audio H2 仍保持独立门。
