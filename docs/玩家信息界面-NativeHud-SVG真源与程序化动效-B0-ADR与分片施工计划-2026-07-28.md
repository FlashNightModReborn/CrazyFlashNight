...
**最后核对代码基线**：B0-06 实现与首轮 Kimi 问题修复冻结于 commit `bf8dd2c410267855c8ea12f25a594042b3158479`（2026-07-29），两份最终机器/视觉报告由 evidence/docs-only commit `81168ece8cd295d8e15b8f6baed46a0da7f9c549` 承载并继续精确绑定该实现闭包。最终 source-freeze F 为 commit `cb38600aae51f5019d09f87c33bd9e67d2b1f511`、tree `c74cc66f445921f5fcda1da04a2de0f013fef8ef`、parent `2882456c8777246fba8d2f6942ef1027caafc650`；在 F push、tag 冻结与 builder dispatch 时，远端 `main` 和不可变 tag `runtime-build-v2/20260729-player-info-nativehud-b0-v1` 均精确解析到 F，后续只要求该 tag 永久保持指向 F，不把会继续前进的 `main` 写成持久等式。F 相对 parent 只修改 6 个文件，其中非文档字节仅为目标 `sourceRef` 与 prepare 捕获的 `arena-unit-param-presets.js` 机械派生差量；artifact source、producer recipe、toolchain lock 与 build identity 不变，policy 为 `0E52F22FD3A42C5770B5B0C7C58E48662097C01CACA8306595991AD61E4DF105`。F 的外部 builder/quorum 事实由 G commit `6bd9e83583c84f47faed285e9a0957198f46121a`（tree `44b087945e972d75e5cced1d84aaabd7670898df`、唯一 parent=F）承载；最终 K1 只读审计也以 clean G 为输入。本 docs-only 后继只记录该审计及其处置，不把 G 或自身冒充为被构建源。

**当前裁决状态**：`Svg.Skia 5.1.1`、8 个 canonical SVG、strict facade、raster/cache、fixture-only PlayerInfo split surface 与 47/47 formal qualification 均已闭合；B0-03b/B0-06 仍为 source-ahead、`NOT_DEPLOYED`，真实 `pi_*` 未接入。F 的 request `839C74FD1DF61ACC1DA580041F6FA71CA13A84DF1F43A55237BF9BEEF8648FB2` 已由本地 X509 / `physical-host-b` 与 GitHub OIDC / `github-hosted-windows` 对同一 build identity `7687B561…7A9F`、payload closure `AF2E9E57…724B` 形成双 signer / 双 faultDomain quorum；[`runtime-promotion-preflight.v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight.v2.json) 为 `preflight-passed`，并明确零 runtime/release-state/promotion/deployment 变更且不可复用为 promotion 输入。完整实现包 Kimi `k3 + max` 首轮的 3 项 P1 与内部复核的 2 项 fail-open/false-green 已修复；最终增量复审已对 clean G 给出 `APPROVE_AUTOMATED_CLOSEOUT / K1_AUTOMATED_GATE=PASS`，P0=0、P1=0。该 PASS 只关闭自动门：B0-01B/B0-04 子状态仍为 `awaiting_human_review`，旧 Flash 同屏、真实游戏 composite、真实手点、关键比例/平滑、混合 DPI、运行时 SWF 身份与审美仍待人类接受。因此 B0 总体状态现在最多为 `awaiting_human_acceptance`，不是 parity、`oracle_frozen`、`b0_accepted`、promotion 或部署。
...
- 已完成：本 ADR、B0 分片边界、依赖准入门、runtime build identity 纳入方案与验收词典；Kimi k3/max 的文档建设审计见 §9.5，最终实现首轮与 closeout 增量复审见 §9.6。
...
- 已完成自动资格、尚未人工退出：B0-06 独立 split surface、fixture widget、程序化 HP/MP、MP 双 fragment、HP 固定轮廓/旋转渐变、path glyph atlas、PanelHost 生命周期与 deterministic C# capture/三方 direct-edge 诊断工具。clean `bf8dd2c…8479` 的 formal qualification 已通过 47/47，C#/Web A/B 与 comparison PNG 闭包也由 [`runtime-qualification.json`](../tools/player-info-hud/evidence/b0-06/runtime-qualification.json) / [`visual-evidence.json`](../tools/player-info-hud/evidence/b0-06/visual-evidence.json) 冻结；完整实现包 Kimi 审计的问题已修复，最终增量复审为 K1 PASS、P0/P1=0。它们都不声称 parity、真实 UiData、游戏 composite、手点或人工视觉结论。
- 已完成非部署 source-freeze：F commit/tag、clean-tree 11-output prepare、锁定环境、Runtime Lane 11/11、本地 X509 与 GitHub OIDC 两类正式 builder、production receipt 及 quorum `-VerifyOnly` 均通过；33-file payload closure 为 `AF2E9E57…724B`。报告明确 `promotionPerformed=false`、`deploymentPerformed=false`、`reusableAsPromotionInput=false`，所以该完成项既不是 promotion，也不改变现役正式 runtime。
...
- 总体视觉交付严格状态：`awaiting_human_acceptance`；其中资产/oracle 子状态继续是 `canonical_asset_candidate_validated; awaiting_human_review`。不得把结构化验证、FFDec/Web 指标、Kimi PASS 或未签 Flash candidate 写成 `fixture_static_parity`、`fixture_full_parity`、`oracle_frozen`、`runtime_integrated`，更不得写成已部署。
...
MP 的 shape tween 不保存 101 份路径。早期“无法稳定插值时先退为单 mask”只是施工备选，已被 B0-06 的两个独立 immutable PArgb fragment 与逐虚拟帧 mask correspondence 取代；当前实现不得再描述为单 mask。该实现只取得自动结构/确定性证据，accepted Flash oracle 与人类视觉接受仍未闭合，不能据此写 oracle parity。
...
依赖关系：B0-01A、B0-02、B0-03a 在 B0-00 后可并行；B0-01B 等 B0-01A，但可与 B0-02 并行。B0-04 明确依赖 B0-01A + B0-02 后方可开始；其 `canonical_asset_candidate_validated` 结构入口足以启动 B0-03b，也足以在 B0-03b 后启动 B0-05。B0-01B 的 accepted oracle/人工签收只阻断 B0-04 视觉 Exit、B0-06 parity 结论和最终 B0 接受，不阻断 B0-03b/05 的无关结构施工。B0-06 已闭合 MP fill 独立 mask fragment、HP 固定轮廓/旋转渐变、path glyph atlas、独立 surface、正式 qualification、双份视觉确定性、F source-freeze/tag 与双 builder 非部署 quorum；Kimi K1 也已明确 PASS，最终退出只剩 B0-04 人工 oracle Exit、真实游戏 composite/手点、关键比例/平滑/混合 DPI/运行时 SWF 身份/审美与最终人类视觉/UI签收。
...
### 9.6 B0-07 最终实现包完整异构审计（K1 首轮 + closeout 增量复审）

最终实现包完整只读审计已于 2026-07-29 运行，满足“必须有一轮覆盖代码、资产、结构化测试与视觉报告的异构对抗”的执行要求，但首轮结论是 `REVISE_BEFORE_CLOSEOUT / K1_AUTOMATED_GATE=FAIL`，当时不能据此勾选最终 K1；修复后的 closeout 增量随后已由同一 `k3 + max` 配置复审到 PASS，首轮失败历史与最终通过证据均保留如下。
...
修复冻结于 `bf8dd2c410267855c8ea12f25a594042b3158479`；随后 focused 为 136 pass + 3 opt-in skip / 139，clean official full 为 1,737 pass + 3 opt-in skip / 1,740，formal qualification 47/47，Runtime Lane C 11/11。dirty/负载态曾暴露多个既有 wall-clock flaky，隔离复跑仍可复现；未发现指向 PlayerInfo 的定向证据，故不据此修改 PlayerInfo，根因与长期稳定性仍未裁决，不能据 clean full 反推“时序稳定性已修复”。最终增量复审覆盖了本节 disposition、两份最终 JSON、evidence/docs-only containing commit、F 配置差量/不可变 tag、双 builder/quorum 的结构化 `-VerifyOnly` 报告、首次本地 worker UTF-8 fail-closed、Windows EOL tooling debt、Arena meta-team 传递派生债务、Computer Use/Browser 控制后端不可用与 FFDec/Edge fallback 边界，以及同步后的 canonical docs；其证据如下。
...
| 最终裁决 | `APPROVE_AUTOMATED_CLOSEOUT`；`K1_AUTOMATED_GATE=PASS`；P0=0、P1=0；没有 `REVISE_BEFORE_CLOSEOUT`、`BLOCKED` 或 `K1_AUTOMATED_GATE=FAIL` |
...
| 8 文档卫生 | accept，本轮关闭 | §7.1 单-mask 备选标为 superseded；§8.3 解释 46→47 Gate 演进；§7.4 点名两个 deferred 层；总体 ADR 明确 161 总命中/约 157 可执行，§8.5 解释两个 diagnostic 状态的作用域映射 |
...
Kimi 另称 `PlayerInfoRasterLayer` 的便捷重载“无调用”，该事实不成立：`PlayerInfoRasterPipelineTests.cs:551-556` 有直接调用，故 reject；`SplitSurfaceSnapshot`/`CounterSnapshot` 合并与 `CombinedText` 只属弱评估项，没有足够锚点，不登记为已验证债务。K1 PASS 后只允许把总体状态推进到 `awaiting_human_acceptance`；它不代签 §11.3 的人类项。
...
- [x] 两类正式 builder 已对同一 F commit/tag 形成一致 build identity `7687B561…7A9F`、payload closure `AF2E9E57…724B` 与 Core `A45471CF…B652`：本地 X509 signer `EB5D32E…CDA5` / `physical-host-b`，GitHub OIDC signer `FA7BFFEB…34F4` / `github-hosted-windows`（run `30440903705`）。production receipt 与同一 promotion 验证链的 [`runtime-promotion-preflight.v2.json`](../tools/player-info-hud/evidence/b0-06/runtime-promotion-preflight.v2.json) 均通过；报告 `proofCount=2`，并明确 `runtimeMutationPerformed=false`、`releaseStateMutationPerformed=false`、`promotionPerformed=false`、`deploymentPerformed=false`、`reusableAsPromotionInput=false`。该勾选只关闭非部署 reproducibility/quorum Gate，不代表 candidate E2E、promotion 或标准入口。
...
- [x] `launcher/README.md`、testing guide、runtime build/automation canonical docs、工具 README、总体迁移 ADR 与本台账已同步最终实现、F source-freeze、非部署 quorum、最终 K1 PASS 与当前 Apps 能力边界；G 及本 docs-only 回写均位于 F 之后，未反写或冒充 builder source。
...
- [x] §9.5 文档级 K0 的问题已有逐条 disposition；§9.6 的完整实现包 K1 首轮已以 `kimi-code/k3`、`max` 可复核运行并完成问题修复，closeout 增量复审又在 clean G 上取得 `K1_AUTOMATED_GATE=PASS`、P0=0、P1=0；审计身份、wire/tool/image 复核、原始输出哈希与 12 组 P2 disposition 已记录。
...
| B0-06 | `qualification_final_tree_passed; source_freeze_quorum_verified; automated_visual_frozen; human_exit_pending` | 已实现 11-case fixture、程序化 HP/MP、MP 双 fragment、HP 固定轮廓/旋转渐变、source-bound glyph atlas、source-red HP Glow、恒 false hit-test 与独立 click-through split surface；clean `bf8dd2c…8479` 的 qualification 为 47/47，focused 为 136 pass + 3 opt-in skip / 139，clean full 为 1,737 pass + 3 opt-in skip / 1,740，`visual-evidence.json` 冻结 C#/Web/direct/supplemental 双跑。F=`cb38600…f511` / tree `c74cc66f…f8ef` / immutable tag；clean F Lane 11/11、scalar 566、guardrails `3/3`、831.271 秒。request `839C74FD…8FB2` 的本地 X509 与 GitHub run `30440903705` 产出同一 identity/payload/Core；`runtime-promotion-preflight.v2.json` 为双 signer/双 faultDomain、`preflight-passed`、零 deployment mutation；最终 K1 为 PASS、P0/P1=0 | 旧 Flash 同屏、B0-04 oracle 人签、真实游戏 composite、真实手点、关键比例/平滑、混合 DPI、运行时 SWF 身份与审美尚未完成；promotion/标准入口/部署不属于 B0 Exit，仍为 `NOT_DEPLOYED`；不声称 parity | 交付冻结视觉包并取得人类明确终审 |
| B0-07 | `k1_automated_gate_passed; awaiting_human_acceptance` | §9.5 已完成文档建设 k3/max 审计；§9.6 的首轮实现审计后，3 项 Kimi P1 与 2 项独立复核问题已修复于 `bf8dd2c…8479`；closeout 增量又在 clean G 以 113/113 个 k3/max request 完成，结论 `APPROVE_AUTOMATED_CLOSEOUT / K1_AUTOMATED_GATE=PASS`、P0/P1=0，12 组 P2 已逐项 disposition。较早 Computer Use 截图仅保留为历史观察；当前 Computer Use/Browser 控制后端不可用，只读进程枚举仅见 `Flash.exe` 标题 `TestLoader.xfl`，无独立 player 身份结论 | 运行时 SWF 身份复核、Flash oracle、旧 HUD 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实手点、审美与人类明确接受；自动桌面/FFDec/Edge/Kimi 均不能代签 | 只剩人工终审；有修改则退回对应切片复测并重新出包 |

表中 B0-03b 行的“两正式 builder 未验证”仅描述其早期 renderer-qualified candidate `c-c571…` 自身，不能外推到后续 F；B0 最终 source-freeze 的正式 builder/quorum 已由 B0-06 行与 preflight 报告闭合。反过来，F 的 proof 也不重写早期 candidate 的 payload/Core 身份。
...
> 工程轨已经冻结 F=`cb38600…f511` 与不可变 tag，并由 clean F Lane、两个正式 builder、production receipt 和 `-VerifyOnly` preflight 关闭非部署 reproducibility/quorum Gate；G=`6bd9e835…121a` 只记录 F 的外部事实，最终 K1 又在 clean G 上取得 `K1_AUTOMATED_GATE=PASS`、P0/P1=0。本 docs-only 后继只同步 K1 与 disposition，不冒充 builder source；恢复后先 fetch 并确认 `origin/main` 包含该后继、不可变 tag 仍精确指向 F、工作树干净。总体状态为 `awaiting_human_acceptance`：按人工包逐项签署 B0-01B 第七轮 candidate 的来源/状态/层隔离/crop/审美，以及旧 Flash 同屏、游戏 composite、关键比例/平滑、混合 DPI、真实桌面手点和运行时 SWF/IDE Output 身份；有修改即退回对应切片重测并重新出包。当前 workspace/terminal 为 unrestricted，但 Computer Use native pipe 仍报 `os error 2`、Browser 后端列表为空；只读 `Flash.exe` 标题、FFDec 与 headless Edge/Playwright 都只能辅助诊断，不能确认运行 SWF 身份或代替人类。B0 不要求 promotion、标准入口或部署；人签完成前不得写成 parity 或 `b0_accepted`。每完成一片都在同一提交回写 §12。
