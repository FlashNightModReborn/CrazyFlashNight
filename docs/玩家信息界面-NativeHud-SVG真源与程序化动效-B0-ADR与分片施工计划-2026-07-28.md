...
| B0-05 | `v1_historical_qualified; main_space_v2_requalification_pending; split_required` | raster/cache/PArgb/LRU 架构与 split 裁决保留；v1 31/31 报告继续作历史。v2 focused 已固定 raw envelope `285×81@(-3,474)`、visible tight `282×81@(0,474)` 与四 viewport bounds；runner 已纠正为 8 logical layer / 10 owned payload、8-field key、固定 16 轮同路径 excluded warmup与 32 Gate，并冻结 JIT/GC override、Normal priority、继承 affinity/server-GC；parse/raster 只作 aggregate payload-slot 计数，代表性 PArgb copy 不含两个 `mp.fill` fragment。dirty-tree 诊断通过但不构成 formal evidence | 新 source closure 下的专用 qualification、性能/cache/dispose/topology structured report；旧 run/hash/timing 与 dirty 诊断都不批准 v2 | source-freeze 后运行 `run-b0-05-runtime-qualification.ps1`，回写新 run/hash/计数；不回并现有 NativeHud union |
| B0-06 | `v1_historical_qualified; main_space_v2_preflight_passed; formal_requalification_pending` | v1 的 47/47、旧视觉 A/B、F/quorum/preflight 继续证明旧字节。v2 已完成 placement、MP current 右锚、maximum `+0.30` local renderer phase correction、前景/装饰锚点关系、`00000`、HP two-pass Glow/横线 active，以及按真实 anchor edge 收紧的 main-space C#/Web/Flash tooling；真实 Flash v2 candidate 已捕获但待人审；当前完整 PlayerInfo/PanelHost/union/topology focused 为 175+3，compositor 6/6 定向复核通过，不可变 v1 tag 永不移动 | fresh full/47-Gate、两份 C#/Web/direct/viewer、新 source freeze/tag/request/双 builder quorum/preflight、既有 Flash v2 candidate 的最终身份复验，以及全部人工桌面项；旧结果不能批准新字节 | 先 source-freeze 并复验 Flash candidate/tooling identity，再依次重跑 qualification/视觉/full/builder；新版本不得复用或移动 v1 tag |
