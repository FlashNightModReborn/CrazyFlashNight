...
- stimulus 是与 observer 分离的 CurrentUserOnly pipe：`cf7-audio-v2-qualification-stimulus-{pid}-{runId}`。Host 只把 strict grammar 的命令投递为 `audioV2QualificationStimulus`，Flash 端仍通过生产 `AudioQualificationStimulus → AudioBridge → XMLSocket → AudioTask → AudioCoordinator/native` 执行；响应中的 `sent=true` 只证明 exact-generation socket delivery，不证明 AS2 已执行、native 已开始或端点可听。前 10 case 顺序固定为 BGM 播放/seek/crossfade、Vorbis/AAC-MP4/Opus、SFX、dense overlap、BGM+SFX mix、gain zero/default-max；Opus 后在 case 外把 BGM gain 置 0，dense overlap 后在 case 外恢复为 1，末尾再次恢复为 1，避免 SFX capture 被循环 BGM 污染并保证 mix 前状态确定。
