...
function freezeParamValue(value: unknown): unknown {
  if (Array.isArray(value)) return Object.freeze(value.map(freezeParamValue));
  if (isRecord(value)) {
    const frozenEntries = Object.entries(value)
      .map(([key, entry]) => [key, freezeParamValue(entry)] as const);
...
function addIssue(
  ...
  const frozenParams = Object.freeze(Object.fromEntries(
    Object.entries(params).map(([key, value]) => [key, freezeParamValue(value)]),
  ));
...
function collectComponents(
  ...
  edges: readonly { readonly a: string; readonly b: string }[],
...
function validateEncounterBinding(
  binding: EncounterDefinitionBindingV1 | undefined,
  issues: StrategyValidationIssue[],
): EncounterDefinitionBindingV1 | null {
  if (!isRecord(binding)) {
  ...
  if (!definitionResult.ok) {
    for (const issue of definitionResult.issues) {
      addIssue(
        issues,
        issue.reasonCode,
        `$.encounterDefinition${issue.path === '$' ? '' : issue.path.slice(1)}`,
        { ...issue.params },
      );
    }
  }
...
export function validateMapDefinition(
  input: unknown,
  encounterBinding?: EncounterDefinitionBindingV1,
): MapValidationResult {
  ...
  const encounterDefinitionRef = readOpaqueId(
    normalized.encounterDefinitionRef,
    '$.encounterDefinitionRef',
    'encounter-definition-reference',
    issues,
  );
  ...
  const encounterProfileById = new Map(
    resolvedEncounter?.definition.profiles.map((profile) => [profile.id as string, profile]) ?? [],
  );
  const nodeDefinitions: MapNodeDefinition[] = [];
  const uniqueNodeIds: string[] = [];
  const nodeFirstPath = new Map<string, string>();
  ...
  for (let index = 0; index < normalized.nodes.length; index += 1) {
    ...
    const garrisonCapacity = readNumber(
      rawNode.garrisonCapacity,
      `${path}.garrisonCapacity`,
      issues,
      { integer: true, minimum: 1 },
    );
    const attackWidth = readNumber(
      rawNode.attackWidth,
      `${path}.attackWidth`,
      issues,
      { integer: true, minimum: 1 },
    );
    const defenseBonus = readNumber(
      rawNode.defenseBonus,
      `${path}.defenseBonus`,
      issues,
    ...
    const encounterProfileRef = readOpaqueId(
      rawNode.encounterProfileRef,
      `${path}.encounterProfileRef`,
      'encounter-profile-reference',
      issues,
    );
    ...
    if (
      ...
      nodeDefinitions.push(Object.freeze({
        ...
        defenseBonus,
        nodeAPBonus,
        encounterProfileRef: encounterProfileRef as EncounterProfileId,
  ...
  const edgeDefinitions: MapEdgeDefinition[] = [];
  const edgeFirstPath = new Map<string, string>();
  const pairFirstEdge = new Map<string, { readonly id: string | null; readonly path: string }>();
  const connectivityEdges: { readonly a: string; readonly b: string }[] = [];
  ...
  const definition: MapDefinition = Object.freeze({
    schemaVersion: 1,
    id: mapId as MapId,
    rulesVersion,
    encounterDefinitionRef: encounterDefinitionRef as EncounterDefinitionId,
