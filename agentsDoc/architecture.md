...
- 关键职责：
  ...
  - Native HUD 组合：`NativeHudOverlay` 是右侧条件槽的唯一 owner，按 `stageDecision > transactionDecision > actionableNotice > contextHint > hidden` 计算一次并把同一 `RightContextSlotOwner` 投影给 RightContext 与 SafeExit；透明 `CompositeBounds` 只负责合成范围，不拥有命中
  - 启动前存档决议与 Protocol 2
- 当前存档 authority 边界：
  - Launcher 拥有启动期 SOL / shadow 快照选择权、文件保管、修复与外部编辑入口；这不是运行期领域事务权威
  - 启动前门进一步拆分物理 `slotKey`、玩家可改且允许重名的 `displayName` 与 AS2 存档内 `characterName`；Host 只保管前两者的启动期元数据，不把显示名当文件路径。建角的 `displayNameCustomized=false` 与空 rename 都删除 override、恢复长期跟随角色名
  - 新建 / 重建的 Web 草稿由 AS2 按实时理发店及服装目录复核；`SharedObject.flush() === true` 是 durable 唯一本地存档终点，Launcher shadow 和重建备份都不代签它
  - 重建只保留每槽一份经规范化与读回校验的轮换备份；tombstone-only 可重建而不伪造备份，tombstone 与有效 shadow 并存时先备份，畸形 shadow fail closed。旧 SOL 不预删除，也不引入自动恢复状态机；普通 delete 保留 tombstone/catalog 身份，彻底 reset 必须连 catalog 一并清除
...
- `warlord` 现役 Phase C 已作为 `1024×576` Three ESM 正式 Panel 接入 lazy registry、全锚 Web 布局、Host 生命周期与 exact close；当前 `productionWrites=false`，已部署旧 Phase C 仍通过 Arena 兼容路径运行。vNext 折叠后的场景链固定为：`StageManager` 将 encounter 物化为临时普通 `StageInfo`，两次只提交标准 `wuxianguotu_1` 淡出跳帧，并在 frame 209 复用普通 world 初始化/返回；`SceneManager` 只做物理 `init/remove`，不拥有 Warlord generation/lease/receipt；`StageRunSession` 只拥有父 GameStage；`WarlordSubStageRunner` 只拥有 outer binding/result；`WarlordActionEncounterService` 只拥有战斗事实。四个逻辑 owner 是后四项中的 StageManager、StageRunSession、runner 与 service；SceneManager 只是物理机制。Host 仅校验、传输、关联和幂等，Web 是唯一战略提交者。复杂度预算为 `owners=4 / clocks=1 / leases=0 / terminal rewrites=0`。标准淡出仍可调用 `SceneManager.removeGameWorld()`；禁止的是任何 Warlord/Host/Web 旁路形成第二套生命周期。
- vNext 外层是 `StageSelectPanelService → _root.载入关卡数据 → StageManager → WarlordSubStageRunner → warlord_stage_start → WarlordStageTask → tracked Warlord stage-v1 → stage_terminal → exact close → warlord_stage_result → StageRunSession`；单 Warlord GameStage 可用 exact `scenarioRef` 选择 Demo 1 或 Demo 2，无 `driver` 的 SubStage 仍走 `StageManager` 普通 Action，混合/未知/multi-Warlord fail closed。Web 内层由受校验的 Map/Scenario/Visual、Organization 与 Encounter sidecar 驱动通用规则、AI、presenter、Three、冻结 battle request、Host 复验、AS2 Action terminal 与 replay。
- Slice 4 已在机器实现中移除 `red/blue` 专用 `GameState` 分支：阵营使用 opaque ID，关系矩阵固定对称，行动按确定性 faction block 推进；VictoryGroup、指挥所失败、投降清理和指挥官状态均进入规则、命令、AI、回放、结果与 UI。Demo 2 固定四阵营和三个胜利组：玩家使用真实主角，`boss-pact-a = 吴豫 / Itinerant / cardId 111`，`boss-independent = 袁望 / Surveyor / cardId 113`，`boss-pact-b = 阎凝儿 / Gazer / cardId 112`。吴豫与阎凝儿互为同盟；袁望政治独立，但本局与其余三方关系均为 hostile。
- Slice 6 已在机器实现中加入 `demo2-thick-x-80` 的 80 节点“厚 ×”：四角各 14 个本土节点、16 个争夺臂节点和中央 8 节点高价值工业环。大图消费六节点虚拟导航窗口、全域分页、中文检索/告警、三级相机 LOD、路线批处理与 overview instancing；战区工具默认收起，由节点导航栏按需展开，`Esc` 收起后恢复按钮焦点；底部 8 张普通兵牌可由 `120px` 折为 `32px`，向沙盘返还 `88px`。玩家和 AI 共用受预算约束的局部命令枚举器。AI 行动镜头追随是纯表现适配器：连续非玩家移动共用一个 camera token 和批次前原视窗，逐次移动不归位，只在批次结束、交还玩家或结算时恢复一次；批次期间拖拽、滚轮、方向键/WASD、全图/定位和缩放按钮均锁定，`prefers-reduced-motion` 下禁用，且不得进入 state、replay、digest 或 receipt。Host 只接受 exact `warlord_demo_02_v1 / wargame-demo-v0.1.1 / demo2-thick-x-80`，固化阵营关系并按全部 80 个目标节点重算 `near/180`、`medium/360`、`far/650`；尚无稳定摘要的是 Demo 2 Scenario/Map 战略 authoring，不能外推到已稳定的 Organization/Encounter digest，也不能信任浏览器自报摘要。Demo 1/2 的独立 GameStage XML 只由 `其他 → 测试 → 军阀演习测试` 的白名单 Stage Select 目录暴露，默认生产目录不含军阀条目；正式可见入口留给 Slice 7。
- Demo 1 的编组、Formation 与三档距离已经真人确认有效；Slice 4/6 仍只有工作树机器证据。初始至 r13 的场景交接/呈现候选均已根据真人阻断固定为 `FAILED / SUPERSEDED / NOT_DEPLOYED`：r9 暴露批次运镜、主角结算、普通演员视觉门与 outer owner 泄漏，r10 暴露玩家初始化期间控制身份过早切换，r11 在连续完成 25 次 Action 后于第 26 次 exact close 与 AS2 admission 之间断链；r12 已完成战斗与 terminal，但 7/7 阵容把结算 footer 裁出固定高度窗口，播放期间又持续唤醒 modal 下方沙盘；r13 只暂停帧链而保留 WebGL context、自动战报 tick 与模糊合成，真人首战结算仍卡死、Intel iGPU 持续高负载并随后非正常重启。2026-09-02 外部审查对 SHA-256 `1BA3367F9C5E883F903FFAA2E4336E81C703C1C91CC6EBA6320B82648C6EDEA5` 的求助包给出 `APPROVE_COLLAPSE / PREFER_B`；该结论是架构输入，不是测试。r14 在 Action 交接、battle modal 与 panel-close intent 时即时销毁沙盘 WebGL 资源/canvas，resume 直接呈现最终静态结算，game reveal 后单向退休 Bootstrap WebView2；移动与返回镜头的 rAF/timeout 共用 identity-fenced exact-once 收尾。fresh Node `201/201`、Edge/CDP `29/29`、Launcher `4628 + 3 skip / 4631` 后，根入口已把隔离 candidate `warlord-s6-gpu-r14` 从合法 JSON snapshot 恢复到同 attempt game scene 并进入首场 Action，状态为 `R14_SOURCE_MACHINE_VERIFIED / R14_CANDIDATE_EXECUTED / HUMAN_ACCEPTANCE_IN_PROGRESS / NOT_DEPLOYED`；正式 runtime、formal closure 与 deployment 均未改变。
- r12 的 Web→Host→AS2 handoff 只传当前战斗所需的有界投影：清空 `battles/commandHistory`，保留 `eventLog` 尾 32 条、未结算伤亡及单调身份计数，并按真实 UTF-8 字节设置 `384 KiB` 软门。Host 在 exact close 后捕获一次 ready socket generation，以单一 AS2 命令授权 pause release + Action start；本地 `Write + Flush` 不是接纳，只有 exact `warlord.action-encounter-admission.v1` 才停止同 binding、同 generation、1 秒间隔、总计最多 4 次的有界发送。AS2 fresh/duplicate 分别返回 accepted/duplicate，已有 terminal 时 terminal 优先重投；若新 B 已 active 而旧 A 晚到，则 `encounter_active` 只标记 stale 并丢弃，不释放 pause、不生成 A terminal，也不污染 B。Program 对 socket disconnect 的同步订阅是 Stage/Battle owner 唯一断线围栏，WebOverlay 的 generation 回调只做视觉清理，防止延迟 UI callback 误杀 replacement generation。
- 整局恢复旁路已经删除，不改通用 `stage_outcome v1`。outer `not_started` 是吸收性父 GameStage 启动失败；Suspended/Unknown 只冻结诊断且不可复活。父 GameStage clear、返回基地、restart、reinitialize 或 setup/admission/calibration failure 时，`WarlordSubStageRunner` 唯一发送 `warlord_stage_outer_cancelled`，payload 为 `warlord.stage-outer-cancellation.v1` + exact 六字段 binding；Host 只幂等退休当前 outer owner。内层 Action definitively `not_started` 仍允许玩家重新选兵，AI 只收束一次。Action actor readiness 只属于单个 fresh world：普通演员持有 AI hold，必须满足 exact world parent、初始化版本、dispatcher、collider、shield、正尺寸及普通单位 `unitAI`；全员在 90 帧内就绪且淡出到 runtime frame 36 后稳定 6 帧，才统一 release hold/prime，scene reveal 总上限 180 帧。结算通过 `encounterProjectionKind=player_avatar` 保持真实主角/纸娃娃身份。
...
- 关卡结果沿 `StageRunSession → StageOutcomeTask → StageOutcomeState → RightContextWidget` 投影：AS2 独占 outcome/life、30 FPS 未暂停帧时、击杀与物资获得/消耗、复活币事务、返回基地和奖励一次性随机化；C# 只校验快照并复用右上常驻 HUD 的 32px 条件状态槽，不创建独立 HWND。respawn 在同一生命周期恢复 HP/MP/可见性后还须显式清 `倒地/_killed`、死亡诊断 latch 并 reset WatchDog，不能依赖旧死亡动画 unload 或换图重建来恢复普通技能门。`stageDecision` 压过安全退出、任务播报和 hover 提示；胜负条不暂停且保持常驻，玩家忽略即继续探索，无可交付任务时只保留“回基地”，AS2 `tdr` 证明存在返回后可路由的已完成任务时追加“前往交付”。该动作仍先完成基地奖励结算，并只在终态、Web exact close 与 pause lease 释放后由 AS2 重解析目的地并导航。NativeHud 保持既有 `cursor > hit numbers > NativeHud / Flash` 层级，并随普通 Panel 整体 suspend。基地人物场景 ready 后，奖励切入 `loot sourceKind=stage_settlement`；`LootContainerService` 继续独占 inventory/claim/revision/lease/悬挂/放弃。Web 左栏把击杀与物资 gain/loss 合并进同一滚屏，右栏切换待领奖励与材料只读存量，一个默认紧凑的完整/紧凑控制器同步驱动三者；库存 organizer 恢复 K 点商城同源原始 inventory skin。`claimBatch` 一次跨层冻结最多 50 个 exact lease，在 AS2 内顺序提交且批尾一次重投影，部分写异常由 Host/Web exact query 对账。完整合同见[关卡结果与基地结算 C# / Web 分层 ADR](../docs/关卡结果与基地结算-CSharp-Web-ADR-2026-08-27.md)。
- 关卡导航同样由 `StageRunSession` 独占生命周期：所有选关、地图/任务/竞技场/外交直达、legacy 与新建角色入口先取得 exact start reservation；异步 loader 只可 pending → exact commit，失败/替换/迟到回调不能消费新 token。loader 只把奖励表与场景掉落写入 reservation staging，首次 gameplay init 先提交 drop index、后替换奖励数组；失败/abort/supersede 均保留旧场景权威。跨淡出的进入/返回回包保存在 class-level exact envelope，Host 只接纳正整数 `callId`；Stage Select 本地关闭必须先得到同步 transport acceptance，发送 false/throw 时保留面板并可重试。战斗或结算期间地图仍可读，但 Web `navigate/open_stage_select` 由 AS2 lifecycle lock 拦截。返回基地在淡出成功后立即清 manager/battle；淡出异常回滚转场 globals 并保持可重试。NativeHud 手势冻结 down 时 action/revision 并在 suspend/hide/capture loss/外放时取消；Panel close 仅在 close 起点和实际恢复前两次 live foreground 都属于 CF7 时恢复 Flash 焦点；完整隐藏前若前台仍属于当前面板，则先向 Flash 根窗口同步交接，见[焦点诊断 §9.13](../docs/焦点管理-诊断与卡顿排查-2026-05-24.md#913-2026-09-08选关关闭前的前台交接)。零奖励 suspend 例外只属于带有效冻结报告的 `stage_settlement`，不扩到 `map_chest`。
...
- **Launcher frontdoor lease**：`FrontdoorBgmLease` 是窄 Host 生命周期，不是通用 owner/priority 路由器。它只在 `Ready + SourceRequestId 为空` 时以稳定的 `host.frontdoor.*` requestId 获取固定 `sounds/PTXOA馆长/主菜单.mp3`（loop、gain `0.4`）；自身 source 为 no-op，任何异源 source 都不抢权。无 OP 的 Bootstrap 读档/角色创建/提交后加载保持前门曲；OP 与 legacy 非 Bootstrap 在 admission 让出，Bootstrap 在 actual reveal 紧邻 `ReleaseBootstrapBgmGate` exact-CAS revoke。`SceneReady` 仅关闭场景门，reveal 前 reset/error 仍可恢复；actual reveal 才永久交给 AS2。revoke 必须只命中自身 latest/pending recovery intent；AS2 已 supersede 时不得 stop，已 revoke 的意图也不得被设备恢复复活。Web Audio、槽位音量偏好和通用音频仲裁不参与此链。
...
### 输入守护与诊断补强（2026-09-13）

最后核对代码基线：release source commit `fbbc47a8c0829019bec7b53b3cf173d948f9bba5`。`KeyboardHook` 专用线程与 `HotkeyGuard` 独立进程并存，后者由同一 Core apphost 子模式启动，父 PID/可执行路径/MVID 校验后才安装 hook；历史根 `hotkey_guard.exe` 不再消费。两侧 Ctrl 独立校准与 NativeHud 队列时间围栏不依赖诊断开关。白名单键盘决策、鼠标队列状态、AS2 采样/移动门与文件归属边界统一见[焦点诊断 §9.17/§9.18](../docs/焦点管理-诊断与卡顿排查-2026-05-24.md#918-2026-09-13输入边沿防御与分层观察)。
