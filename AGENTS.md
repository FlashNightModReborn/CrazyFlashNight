...
**最后核对代码基线**：release source commit `cda82e6e4128faf8664235e48b3ed0d84c14a553`（2026-08-29；implementation `ec32a3406a1792925af91b623c225e3480862302`、tag `runtime-build-v2/20260829-equipment-tuning-final-state-v2`、tree `f52fe54c5e292856280343c74a0dd56cdbd2b3c8`、request `3A7030BF2E3177039DD012ED5B852AFD01171EA7690C22572DD7956115B6D426`、deployment `43c8ad6ea23f677bcb2dd7d406e8024b831d93dc`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `ABE14CF3360B1AAC057E3A08DCFBC199E37915309B95E2E43DBBC9BC7C4D319D` / `github-hosted-windows`（cloud run `33227123193`）已对 identity `BEA826A1DD33CAB9548B19F328C8C16264EFDD27F2FAA407689B00EF679774B3`、closure `4860E38FD8DA98B7B9A7080D51E986EE1BC9CEB8FC4BB2EEE28B7D6647881556` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `AD1C53C8550AB21520CE5685ABCB3D189371EBD13B65F6987D2304F340DB4129`，39/39 production receipt SHA-256 为 `46ED9AF8CA18F6472705BBD9B5C66D08447E7D2930F21ED43C57FE5AA3516CF6`；deployment commit `43c8ad6ea23f677bcb2dd7d406e8024b831d93dc` 与首次 post-promotion audit run `33227550372` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车封闭了已穿戴装备经进阶或插件变换突破玩家等级，以及 `replace_mod` 使用旧容量形成最终超槽的正常流程；同时隐藏不可用进阶、把失败原因留在 Web、固定四入口同排，并消除强化度交换完成后的迟到候选误报。非法或手改存档继续原样保留。维护者已确认修正版有效；production policy `39/39`、worktree/index 33-file bundle、signed consensus、原子 promotion、正式根 bootstrap `--verify-only` 与 post-promotion Audit 均通过。
准确状态为装备调制最终态修复 `HUMAN_ACCEPTANCE_PASSED / promoted`。部署后只执行了根 bootstrap 完整性验证，没有从无 candidate 的正式入口重跑装备进阶、插件变换、强化度交换或存档旅程，因此不称本功能业务 `standard_entry_verified`。斗兽 Gate F campaign 与 Audio H2 继续保持各自独立门，不能由本次通用 runtime promotion 代签。
