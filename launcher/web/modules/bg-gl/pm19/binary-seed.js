...
export function decodePrimeMagicBinary(data, canonicalChecksum) {
    const bytes = asByteView(data);
    if (bytes.byteLength < HEADER_BYTES)
        throw new RangeError("truncated PM19 header");
    ...
    if (version !== FORMAT_VERSION)
        throw new RangeError(`unsupported PM19 version ${version}`);
    const size = view.getUint16(6, true);
    if (size < 1)
        throw new RangeError("PM19 size must be positive");
...
export async function loadPrimeMagicSeed(entry) {
    const response = await fetch(entry.url, { cache: "force-cache" });
    if (!response.ok)
        throw new Error(`seed fetch failed: HTTP ${response.status}`);
    const data = await response.arrayBuffer();
    if (data.byteLength !== entry.binaryBytes)
        throw new RangeError("seed binary byte length mismatch");
...
export async function loadPrimeMagicSeedBank(manifestUrl) {
    const response = await fetch(manifestUrl, { cache: "force-cache" });
    if (!response.ok)
        throw new Error(`seed bank fetch failed: HTTP ${response.status}`);
    const manifest = await response.json();
    if (manifest.format !== "prime-magic-runtime-bank/v1" ||
...
//# sourceMappingURL=binary-seed.js.map