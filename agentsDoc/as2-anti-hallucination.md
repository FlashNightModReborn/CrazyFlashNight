...
### 调试
- **`trace()`**：Flash CS6 发布设置中勾选「省略 trace 语句」后，编译产物会**完全剔除所有 trace 调用**（零运行时开销）。因此 **可在热路径中自由使用** trace 进行调试，无需担心性能影响
  - **前提**：非调试用途的正式编译（即最终发布到玩家手中的 SWF）**必须确保「省略 trace 语句」选项已开启**，否则 trace 会产生运行时字符串拼接和输出开销
  - 设置路径：Flash CS6 → 文件 → 发布设置 → Flash 选项卡 → 高级 → ☑ 省略 trace 语句
