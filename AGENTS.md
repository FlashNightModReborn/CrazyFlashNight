...
- **Launcher 二进制发布**：只准从通过 `tools/check-runtime-build-env.ps1` 的机器运行 `launcher/build.ps1 -BuilderId <id>` 生成隔离 candidate；至少两台不同 builder 对同一 source/toolchain/recipe 产出的闭包 SHA 全等后，才准用 `tools/promote-runtime-bundle.ps1` 原子写入根 bootstrap、`runtime/` Core/native DLL、manifest 与 release-consensus 记录。提交前同时跑 `tools/verify-runtime-bundle.ps1 -Staged`、`tools/verify-runtime-consensus.ps1 -Staged`；禁止把 candidate 单机复制到 runtime，基线与换机流程见 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)
