...
**最后核对代码基线**：材料、商城与战备箱体验优化正式发布冻结 source commit `7d869c5eadf224953b2c61ec7a6ceee2ca03055f`（tag `runtime-build-v2/20260728-material-shop-storage-workbench-v1`）、release tree `6dcd424849964d5b174c559d350a9693b59e56f8` 与 request `9AC9E3C9A1BC5162F17881DDB37A6F23C8AAAA9830A808CAF4648EDE60753771`，并由 promotion commit `40675cae8264a5330904731b9dd8871a2263723a` 记录正式部署闭包。exact isolated candidate 与无参 `automation/start.ps1` 正式入口均完成材料档案、战备箱和 K 点商城真实 GUI、运行态数据与普通关闭 smoke，故当前严格状态为 `standard_entry_verified`。完整证据见 [材料、商城与战备箱正式关闭记录](evidence/material-kshop-battlebox-workbench-closure-2026-07-28.md)。

## 当前迁移状态

runtime v2 的工具、schema、队列、本地 X509 证明、GitHub OIDC/Sigstore 证明、promotion 与 CI 状态机已经完成正式闭环；**仓库当前受控部署已是 manifest/consensus v2**。当前 consensus 的 artifact source `F773F7A6F6BA565CABAC14587C2BF7EFCA41AD5F9FFF1F2B89C75E4250F7CAFC`、producer recipe `DE5E1C0263A803CC416598BFDB074C20E5D22C594DF601EC231BC702793BFE7D` 与 toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD` 形成 build identity `3F7887ADF9041DFEDA74EEA26040A3628919A725B04D322FCE667791425F7B44`；payload closure 为 `A9B142A417CF2AC45EA19845144569391FCDEE8DF4235F5279A1EC5F0F9022F2`，manifest SHA-256 为 `236462357B4E13B7F2E5EFE2C14F0433CBAA75EAACEDEAB7D81EA17183325513`。production policy `F8C5D87ADA848323D0C35FD6DC0AC895327BC7A382FFBE7997287C7AA646440B` 的 22/22 receipt SHA-256 为 `E1604B933786DBA76C989A18AFC401AB7F1E925AE677FFCE27F3729C6B2A9A43`；本地 X509 signer / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC builder `1BDD1B0F419E0CA384E59986DBC1C9C9195A01CFEAF0838E116A8AD13B9BCEC3` / `github-hosted-windows`（run `30364763726`）的双 signer、双 faultDomain、production receipt 与 v2 strict/full-install verifier 全部满足后，于 `2026-07-28T14:26:12.9287283Z` 完成 promotion。
...
PlayerInfo B0-03b～B0-06 是当前 `source-ahead`：生产项目已固定 `Svg.Skia 5.1.1` / `SkiaSharp 3.119.4`，显式嵌入 8 个 canonical SVG + runtime manifest，并把 `launcher/THIRD-PARTY-NOTICES.txt`、窄资产树、共享 strict facade/qualification 与 candidate production contract 纳入四域和政策门。B0-03b 的 fresh v2 candidate 已通过 actual renderer/deps exact closure、production policy、全量回归与 1-byte identity/Core/payload sensitivity，达到 `renderer_qualified / candidate_built / NOT_DEPLOYED`；B0-05/06 后续在实现基线 `bf8dd2c410267855c8ea12f25a594042b3158479` 完成 raster/cache、`split_required`、fixture-only split surface、47/47 机器资格与自动视觉闭包。最终 formal run `a6aadeb6e3264577bb9da8fe90857a7b` 的 canonical 报告为 647,234 B、SHA-256 `656286825CE9717D0DC31BDF9DFE00F3ECB57AB515C5C130601BDF23D319CD74`，第 3 个 warmup group 首次收敛，紧邻独立 acceptance 的 process/GDI/USER delta=`-3/0/0`。收尾 merge 为 `8c703c6811ef264da36eb482db524a16a4e64958`；两份最终 JSON 与同步 canonical docs 已由 evidence/docs-only commit `81168ece8cd295d8e15b8f6baed46a0da7f9c549`（tree `496faa9afc7de21c082c7440375b4c6b31139d9f`）承载。该提交只修改 7 份文档与 2 份报告，无代码、配置或资产变更。最终 source-freeze 前再次 fetch 发现 `origin/main` 前进到 `232d25ddf96ba39147f3335162d8a2d033c7822f`；merge `2882456c8777246fba8d2f6942ef1027caafc650`（tree `1bf4e2f657aaefe71c2267ac3fcd48ce8ba4554b`）已无冲突纳入该远端。新增的 700-file 上游直接差量只涉及贫民窟数据与 `flashswf/arts/things1` XFL/SWF，与 PlayerInfo、当时两份 source-freeze docs、runtime 配置/工具及四域输入交集为 0；合并后 `origin/main` 为祖先，ahead/behind=`23/0`。但 final release prepare 随即 fail-closed 检出该 stage 数据所派生的 `launcher/web/modules/arena-unit-param-presets.js` 已陈旧；重生成后 metadata 从 840/271 变为 848/272，并只新增 `param-b6f7e75f4f3b`（裸体兽化僵尸、`登场动画=坠落`、count=8、来源 `基地门口/贫民窟.xml`）。当前 source-freeze 切片因此包含两项非文档字节变化：把 `config/build/runtime-github-builder.v2.json` 的 `sourceRef` 更新为 `refs/tags/runtime-build-v2/20260729-player-info-nativehud-b0-v1`，以及纳入 prepare 枚举的 11 项输出中唯一不一致的该 registry；另同步专项 ADR、runtime deep doc、Launcher README 与测试指南。两者共同使 policy hash、release tree 与 request 身份改变；当前 Index policy 为 `0E52F22FD3A42C5770B5B0C7C58E48662097C01CACA8306595991AD61E4DF105`，artifact source/producer recipe/toolchain lock/build identity 不变。提交后仍须以 clean-tree 四域复算、policy/request/admission 与 Runtime Lane 实证，不能以 staged 分类代替最终 commit 证据。两类正式 builder 必须对该 source-freeze commit/tag 重新产出 candidate/proof 并形成同一 production 验证链的 quorum `-VerifyOnly` 报告；报告另由后继提交持久化，该后继提交不冒充被两类 builder 构建的源，也不得把报告回灌为 promotion 输入。B0-03b candidate exact identity、B0-06 实现/视觉证据和最终双 builder 源冻结是三个不同作用域，分别以 [PlayerInfo B0 专项 ADR §12](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md) 的对应台账为准。后续实现没有修改根 EXE、正式 `runtime/**`、manifest 或 consensus；当前尚无本 source-freeze commit/tag 的双 faultDomain candidate/quorum，也没有 promotion / 标准入口结论。formal owner 位于屏外，机器资格与图像闭包不证明可见桌面 DWM composition、游戏 z-order/occlusion、实际鼠标透传、审美或人类验收；不得把这些源码证据、早期单机 candidate 或旧正式 runtime 三者混写。

merge 前/后、prepare 前的 source-freeze staged input tree 分别为 `5b242c72bcb9a9b521f0f5fa2d24aa9519167e87` 与 `19c931f6406ff6fd2fba66ea1485c0583be8424e`；同步起初两份 source-freeze 文档后的 tree 为 `29745d38674e55c6ac6710008876484f16896532`。对该首次候选执行 prepare 时，11 个 tracked outputs 中唯有 `launcher/web/modules/arena-unit-param-presets.js` 不一致，门以 exit 1 拒绝；其差量只新增上述一个 preset，无删除或既有 preset 改写。纳入派生输出后的 staged tree 为 `81493965fada78b904fc1c857146dd53e1cbdd62`；同步同事实到专项 ADR/runtime deep doc 后的 `f326e6c255fbc12bd386353707ac443f7bbd81d6` 已通过 11-output prepare 且零未暂存残留。随后因识别 meta-team 盲点又只更新 Launcher README、测试指南及两份 source-freeze docs，不再改变配置、派生 registry 或任一身份域；完成这四份 canonical docs 后的 staged snapshot `a7d36f4d0808ffb59e5227aca05265abcc6897d8` 已通过 doc governance、`git diff --cached --check`、11-output release prepare、锁定环境门与 Runtime Lane C 11/11（scalar 567；guardrails 另列 `scripts=3 / unsafeCandidateCases=3`；总耗时 863.010 秒），真实远端 admission audit 也再次通过 `Active`、`mainDirectPush=true`、tag creators=`Crazyfs,Flash-Night`。相对 `81168ece…c549`，artifact source `A7190F548851A505F9C6ACCE9F132E1741BAB4B47597473801C79DE32589DBC8`、producer recipe `B97998EA7246D6AB667902BCBBD7994DFA5F658A37CFA427D2EEEABA6924DE28`、toolchain lock `7B83229BE93F8244810CDD23DAFD97875B23857E547DE520035FE23B453CB3CD` 与 build identity `7687B56106F0C19EFD4463DCA2B69B3892BD3EE39503A3FD5521ED6DB0257A9F` 不变，policy 从 `405CE99C7362514BD3C964403560F7B253F7C7F95AF0E6BD7AB06BD88729BDE2` 旋转为 `0E52F22FD3A42C5770B5B0C7C58E48662097C01CACA8306595991AD61E4DF105`；中间值 `3CA1C204…A90EDBF` 只对应缺少 prepare-governed 差量的 snapshot，不得作为最终 F 身份。本段随后只以 docs-only 证据措辞补记 `a7d36f4d…97d8` 的结果；它证明该精确 staged snapshot 的前置门，不冒充尚未产生的 F commit。commit/full SHA、clean-tree 复核、远端 tag 解析、新 request、两类 candidate/proof、payload 一致与 `-VerifyOnly` quorum 必须在后续步骤以精确产物补证。

另有一条不并入 PlayerInfo F 的 Arena 传递派生债务：`tools/derive-arena-meta-teams.js` 同样读取 `data/stages/**`，但当前不在 release prepare 中，其 `--check` 只解析/打印、不比较 tracked 字节。只读内存复算得到 `data/arena/meta_teams.json` tracked 3,470,547 B / `635FB515…98F72`、现源应为 3,367,986 B / `26099187…47D3`；`launcher/web/modules/arena-meta-rosters.js` 为 1,291,607 B / `2F677C5F…9453` 对 1,292,790 B / `32F1D6B9…FD65`；以新 meta 继续派生时，policy fixed file `arena-custom-presets.js` 也会从 1,186,819 B / `26A77128…51A5` 变为 1,189,068 B / `21F4020F…68B6`。这条约 5.8 MiB 漂移包含既存 Arena 变化，不应冒充 232d 的单一 PlayerInfo 邻接修复。本 F 只闭合 prepare 当前枚举的字节，并不声称全仓 Web 派生语义新鲜；未来 Arena 专项须让 meta sync check fail-closed、纳入 prepare/policy、完整刷新三文件并跑 custom-match 与标准 Arena harness。B0 的双 builder/`-VerifyOnly` 只证明 source-freeze 的 native identity、payload 与当前 policy 字节；由于不 promotion/部署且 PlayerInfo native payload 不消费该链，该债务不改写 PlayerInfo B0 Exit，但必须交给最终 Kimi delta 审计。
