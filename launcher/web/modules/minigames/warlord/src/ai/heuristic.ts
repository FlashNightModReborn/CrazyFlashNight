...
export function generateNextAiAction(
  state: GameState,
  factionId: FactionId,
...
export function runAiActionPhase(state: GameState, factionId: FactionId, maxCommands = 64): AiRunResult {
  let current = state;
  ...
  for (let i = 0; i < maxCommands; i += 1) {
    if (current.activeFactionId !== factionId || current.phase === 'GAME_OVER') break;
    ...
    for (const pieceId of command.pieceIds) {
      ...
      if (from && to && from !== to) seenTransitions.add(`${pieceId}:${from}->${to}`);
    }
  }

  if (current.activeFactionId === factionId && current.phase !== 'GAME_OVER') {
    const end: GameCommand = { type: 'END_ACTION', factionId };
...
function xpToReachLevel(cardId: CardId, currentLevel: number, xpIntoLevel: number, targetLevel: number): number {
  let needed = -xpIntoLevel;
  for (let level = currentLevel; level < targetLevel; level += 1) needed += needXp(cardId, level);
...
function chooseXpCard(state: GameState, factionId: FactionId): CardId {
  ...
  const locked = CARD_IDS
    ...
    .map((cardId) => ({
      cardId,
      needed: xpToReachLevel(
        cardId,
        faction.cards[cardId].level,
        faction.cards[cardId].xpIntoLevel,
        getCardDefinition(cardId).deploymentLevel,
      ),
    }))
...
function roleCounts(state: GameState, factionId: FactionId): Record<string, number> {
  const counts: Record<string, number> = { assault: 0, sniper: 0, ammo: 0, heavy: 0 };
...
function tierRank(cardId: CardId): number {
...
function chooseProductionCard(state: GameState, factionId: FactionId): CardId | null {
  ...
  const roleOrder = ['assault', 'sniper', 'ammo', 'heavy'] as const;
  const missingRole = roleOrder.find((role) => (
    (counts[role] ?? 0) === 0
  ...
  if (unlocked.some((cardId) => tierRank(cardId) >= 2)) return null;
...
export function generateAiPlanningCommands(state: GameState, factionId: FactionId): GameCommand[] {
  ...
  const applyIfLegal = (command: GameCommand): boolean => {
    if (!validateCommand(shadow, command).ok) return false;
  ...
  for (const nodeId of productionNodes) {
    ...
    for (const slot of slots) {
      ...
      if (!cardId) continue;
      const command: EnqueueProductionCommand = { type: 'ENQUEUE_PRODUCTION', factionId, nodeId, slotId: slot.slotId, cardId };
...
export function runAiPlanning(state: GameState, factionId: FactionId): AiRunResult {
  let current = state;
