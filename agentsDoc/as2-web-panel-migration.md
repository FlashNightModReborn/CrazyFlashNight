...
**最后核对代码基线**：commit `f01f4b121a4ceebd7dae051f14bb511c5ae3f1cb` 已由 tag `runtime-build-v2/20260730-workbench-no-as2-fallback-v1`、release tree `3dfe2dcdb25b26f22a2c36b400524a3b6aca0e28`、request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B` 完成 v2 promotion；正式 runtime 绑定 identity `58F1C3F3B128B22CEA4EDAEF74B402976D13ACB09D4A934240FCCFF3FA7C0465`、closure `B529199F6CC00BC0687E8EED6950C6490F239C56E2A2D4AC27E90366CE2C8CAF` 与 Core `565C1F9710421E6B6CC5CB6DDA05DE36B8F1B22B3D7A7CA19617F9786C7D8A4B`。标准入口 attempt `8baf52bbcceb452da32da641e58d2922` 达到 `standard_entry_verified`，范围仅为 production `EQUIPMENT_TUNING` opener → exact workbench instance → 同实例首个权威 snapshot → supported application shutdown；此前 `c4faf14460…` 的双向 GUI 与普通关闭证据仍是独立历史身份，不能拼接扩张本次验收范围。

2026-07-29 的 B7 施工从 commit `c96f4c3d750561022b706c72a4d53050431e627d` 起步；2026-07-30 的当前 cut 又删除仓库、装备、NPC 商店、合成与技能教师的 legacy renderer/fallback，并收口 main XFL 可达闭包。fresh 自动门与 GUI 人工验收通过后，该 cut 已由 immutable tag `runtime-build-v2/20260730-workbench-no-as2-fallback-v1`、request `3BEBE136773D2C09022F01E5B3C176A788FE3D84E453F6500C6C560F03184C7B`、本地 X509 + GitHub OIDC/Sigstore 双 signer / 双 faultDomain quorum、production receipt、promotion 与同身份无参标准入口复验闭环，严格状态为 `standard_entry_verified`。首轮标准入口 attempt `bfb6bfe515f14af58f9b4096c8c539dd` 因 reveal watchdog 比真实 title receipt 早约 8 ms 而 fail-closed，随后 clean attempt `8baf52bbcceb452da32da641e58d2922` 成功；两份证据同时保留。该纵切仍未执行 Character、Materials、Intelligence、PlayerInfo、业务 preview/commit、普通 panel close 或持久化专项旅程。
...
`candidates` 请求现役 exact payload 必须携 `candidateScope:"compatible"|"backpack"`，响应按同一请求精确回显；缺失、非法或错回显都拒绝整份结果。`compatible` 保持原槽位候选语义；`backpack` 只扩张读投影，返回全部合法占用背包行。装备行还必须携权威 `equipmentEligibility:{slots:[...],blockedReason:""|"level_locked"}`：`slots` 只能是 canonical 装备槽的有序子集，AS2 按实例有效 `use/level` 生成；Host 独立重算槽位集合与顺序，并校验 reason 只取封闭枚举且与当前 target 的 `disabled/blockedReason` 精确一致；Web 再做封闭 shape 校验并只据该字段重投影当前槽。未知/空 `use` 但实例 value 仍呈装备形状的损坏行必须进入 diagnostics 并跳过；真正的非装备行才允许 `slots:[]`。因此背包响应中与当前槽不兼容的行仍可检视，但必须是 `disabled=true / blockedReason="incompatible_item"`，不得进入装备写。

Web 的乐观通道严格限定为**权威盖章后的只读重投影**：只保留一个最近成功条目；`candidateScope=backpack` 的装备响应统一存为 `equipment:backpack`，可在全部装备槽间同步本地重投影；`compatible` 仍只把 exact `手枪/手枪2` 规范为 `equipment:手枪`，其他装备族不缓存；药剂因冷却随时间变化而不缓存，也不扩张成多 key LRU。cache key 必须绑定 exact `panelInstanceId`、session generation、loadout/drug revision、candidate scope 与上述 family。新的 candidates/snapshot authority request 在发送前即重置缓存；revision/generation/实例变化、mutation/write、flush/finalize、调制切入、suspend/rebind/close、degraded/失败也都会清除条目。loadout tooltip 等其他只读请求不误伤这份缓存。Host 同时只保留最近一次已验证 universal response 的 source 检视能力。缓存命中不发送新 `candidates`、不生成新 lease，也绝不乐观写入。两种模式最终装备仍复用现役 `equipEquipment/equipDrug`，携当前 exact target、原 source lease 与 revision，并由 Host/AS2 mutation 复验 effective data；不得新增“按物品自动猜槽”或 Web 侧资格规则。
