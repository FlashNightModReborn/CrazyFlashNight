...
**2026-08-15 材料档案发布列车当前态**：source `b298138fbac1b17ceaab2540729735f2d64d81d1` / tree `87cf3bb16250f02c9a2b396c1184a75064089e80` 已由 immutable tag `runtime-build-v2/20260815-material-archive-v1`、request `8D87E3CE74FEBB33F739F486BB18AEC3C4DB81E92416D61B991402E8AB69AEFD`、本地 X509 `builder-local-a` / `physical-host-a` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `31854335888`）完成双 signer / 双 faultDomain、production policy、strict v2 verifier、原子 promotion 与可回滚部署；正式 identity 为 `6437070BD78C64713FCBC1CDC681EAC581636BDA07372DDE78299B798515A113`，closure 为 `B61AE1F3A3F3B7A3041D7ED7798471A8B9F363224B3CC213EADE7C72EBA9F6FE`，deployment commit 为 `539fa306181da40d092c03508afdbddef18eff8d`，post-promotion audit run `31855597166` 已通过。owner non-H2 路径明确 bypass Audio E1/H2/E3，Audio H2 仍未满足且未伪造。无 candidate id 的 generic formal smoke 在 reveal stable 后可信 shutdown，进程 `exit 0` 且 slot/SOL 不变；更早的 immediate EOF before reveal 暴露 pre-reveal exit 竞态，作为已知边界保留。runtime/DLL 已 `PROMOTED`，但材料 24-step formal journey 与 `PG-MAT-RELEASE-01` 仍为 `OPEN`，不得冒称材料专项 `standard_entry_verified`。

**Audio Platform v2 R4 H2 合规路径**：P4 `755c88eef2f0fb92f247d3a381dc91638cdf8e77` 与 H4 `0792adf201802f1369a91b2fd3e4561a25c279b9` 的 A1–A6 合同继续有效；sleep recovery 的 15 秒结构化质量目标、30 秒 working-state 硬上限及末个 closing `Ready` 后同 generation / physical tuple 双 snapshot PCM 证明均未被紧急发布改写。正常路径仍须以 E2/H2 和 E3 `h2-request-link.json` 闭合，当前 owner-emergency promotion 不是 H2 合规证据，也不能用于补签这些门。

**文档角色**：系统拓扑 canonical doc。  
**最后核对代码基线**：current formal release source commit `b298138fbac1b17ceaab2540729735f2d64d81d1`、release tree `87cf3bb16250f02c9a2b396c1184a75064089e80`、tag `runtime-build-v2/20260815-material-archive-v1`、request `8D87E3CE74FEBB33F739F486BB18AEC3C4DB81E92416D61B991402E8AB69AEFD`、deployment commit `539fa306181da40d092c03508afdbddef18eff8d`（2026-08-15）；本地 X509 `builder-local-a` / `physical-host-a` 与 GitHub OIDC/Sigstore `github-hosted-windows` 已完成双 signer / 双 faultDomain v2 promotion，正式 runtime 绑定 identity `6437070BD78C64713FCBC1CDC681EAC581636BDA07372DDE78299B798515A113`、closure `B61AE1F3A3F3B7A3041D7ED7798471A8B9F363224B3CC213EADE7C72EBA9F6FE`。cloud build run `31854335888` 与 post-promotion audit run `31855597166` 已通过。owner non-H2 明示 bypass Audio E1/H2/E3，Audio H2 仍未满足；generic formal smoke 只证明 reveal-stable 启动、可信 shutdown、`exit 0` 与 slot/SOL 不变，且保留 immediate EOF before reveal 的 pre-reveal exit 竞态。材料 runtime/DLL 已 `PROMOTED`，材料 24-step formal journey 与 `PG-MAT-RELEASE-01` 仍 `OPEN`；此前发布列车只保留为历史分层。
