...
本节只描述 H1 activation commit `c5ca5dfa9718e8a7714038e929a64081b7fd0026` 之后的 current source。前三次失败仍分别是 S/run `cdf7…59f64` 静音、S2/run `5cf1…f7ec3` artifact/wire canonical mismatch、S3/run `a481…c2c7` extensionless SFX unknown-ID。full-basename/dense 修复冻结为 S4 `2f28e48d6cb260447c5e268b4e6a45af166893f9` / tree `d4f286a043e15a3d6449d75766dcb5ca67479781`；A/B/live producer 一致得到 identity `E2CF5CF86AB5390B215DF94DE2F077C9678B72923636E32D22718D55DDD6B686`、closure `75D305734DDA5428B76FABD7C2E13B7E5FD1FA6CD0B8ADB184B46357870B4F11`。S4 run `65b2…d8d98` 在 begin 后因 capture root 缺失失败；run `5059…e30bc` 被旧 crossfade replay 对 3.26ms cached read 假红，且诊断同时暴露短 SFX snapshot peak=0 与 loop cursor wrap 误判。五个 runId/capture 全部作废。当前版本加入 exclusive/canonical capture-root、strict-EOF hash/duration-bound long SFX、4063ms drain/quiet mix baseline、loop-aware mix 与 bounded cached-read crossfade；最终 Node focused matrix 为 runner/materializer/operator/observer/assembler/updater `28/8/12/23/5/5`，dependency manifest 为 `568` entries / `142053` B / closure `4FC516AA6807EA30F5B94BCE9445698E2A4692B7E62C7A1FA1AAE02525D802DD` / SHA-256 `40E51C802005A8D08D9FEB0048887A37CE2E4426BC261D01F3CEA85A8A741A86`。本节所在版本与最终 dependency/doc closure 首次同树落盘的 clean implementation commit 定义为下一 Source S，commit/tree 不在自身文档 blob 中预填，而由提交后的 Git object 解析；该 exact S 仍须重新双 clean repro、新 candidate、全新 runId 并从 case 1 重跑 A6。当前继续为 **H2 blocked / NOT_DEPLOYED**，未进入 runtime promotion；不能把源码、单元测试、失败 run capture 或旧 candidate 外推为正式入口已升级。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
- **失败 attempt 台账**：前三次仍分别是 S `90985ae…705644` / run `cdf7…59f64` 的静音、S2 `0360972…2f950` / run `5cf1…f7ec3` 的 artifact/wire canonical mismatch，以及 S3 `176b2a0…c800a` / run `a481…c2c7` 的 extensionless SFX unknown-ID；其既有诊断 capture/存档边界保持 ADR 记录。full-basename/dense 修复冻结为 S4 `2f28e48d6cb260447c5e268b4e6a45af166893f9` / tree `d4f286a043e15a3d6449d75766dcb5ca67479781`，clean A/B/live producer 一致得到 identity `E2CF5CF86AB5390B215DF94DE2F077C9678B72923636E32D22718D55DDD6B686`、closure `75D305734DDA5428B76FABD7C2E13B7E5FD1FA6CD0B8ADB184B46357870B4F11` 与 runtime manifest SHA-256 `CBC3786872891F8A3AED1188F8129CF4F16FD8B7CE8DA7B80B23212FBAB9C32A`。S4 run `65b22a3382076ef59fcaa70e5f9d8d98` 在写入 `bgm_playback` begin 后因 `prepare` 未创建 capture root 而失败；run `50598a6f408e793b325ded41380e30bc` 的自动 lane/三份 capture 仅作诊断，旧 replay 把 seq `68→70` 相隔约 `3.26ms` 的同 frame 非静音 cached read 当作 crossfade 失败。该 journal 还显示 `sfx_playback` 与 mix 的 post-stimulus SFX peak 为 0，以及循环 Opus cursor `18240→14880/24000` 被 strict-increasing 误判。五个 runId 与全部 capture 永久作废，均无 E1/H2 且始终 `NOT_DEPLOYED`。后续必须把 exclusive prepare、qualified long SFX、loop-aware mix 与 bounded cached-read crossfade 连同最终 dependency/docs closure 冻结为下一 Source S，再做双 clean repro、新 candidate 与新 runId。
