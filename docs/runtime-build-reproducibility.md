...
**最后核对代码基线**：commit `711c469036ad6b1226833faf255499abb1ebf2ed`（2026-07-18）+ 当前 runtime v2 发布列车工作树。

## 当前迁移状态

runtime v2 的工具、schema、队列、本地 X509 证明、GitHub OIDC/Sigstore 证明、promotion 与 CI 状态机已经接线；**仓库当前受控部署仍是 manifest/consensus v1**。`builder-local-a` 已以 `physical-host-a` enrollment：CurrentUser 不可导出私钥已实签/验签，tracked registry 只保存公钥证书；GitHub cloud 票、双故障域 quorum 与正式 v2 promotion 仍未完成，因此不得声称“正式 runtime 已迁移 v2”。

v1 只保留迁移兼容：开发分支可在部署集合未变化时进入 `source-ahead`；受保护分支通常仍要求 legacy strict bundle + consensus。为解决“cloud workflow 必须先进入 default branch，才能为同一 tree 取得第二票”的 bootstrap 悖论，本轮只允许一次 `migration-bootstrap`：`config/build/runtime-v2-migration-bootstrap.json` 精确绑定 base `711c469036ad6b1226833faf255499abb1ebf2ed`、旧 artifact closure 与目标 builder registry 字节哈希，且根 EXE、`runtime/**`、legacy consensus 必须零变化。marker 是永久 fuse；它已出现在 base 后，任何后续 v1 head 都失败，下一提交只能走完整 v2 promotion。首次合法 v2 promotion 后，CI 也永久拒绝 v2 → v1 降级。
