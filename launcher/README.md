...
> **CF7 Agent Runtime / Wings F7 source freeze（2026-07-31）**：一期实现与协议源冻结在最终 C1 `dd84230a1d262c6478591cae2d11051b7a8aa7b1`，仍是 `1.0` 预发布合同；ADR 文件名保留首次冻结日 `2026-07-30` 以避免 canonical 路径/链接漂移。当前 documentation-only D1 只引用其父 C1，不能在自身内容中记录尚未知的 D1 commit hash。破坏 wire 的变更必须连同 schema、生成物、实现、fixture、harness 与文档原子升级。exact C1 candidate 已通过 production policy **26/26**，只达到 `candidate_built / NOT_DEPLOYED`；当前无前台会话的严格入口按凭据门失败关闭且无 completion evidence，故未达到 `candidate_executed`、`e2e_verified` 或 `promoted`，正式 runtime 与上面的已 promotion 身份保持不变；完整边界见 [一期范围冻结 ADR](../docs/CF7-Agent-Runtime与Wings-Network一期-范围冻结-ADR-2026-07-30.md)。
...
### CF7 Agent Runtime / Wings Network（F7 一期冻结）
...
Web 导航一开始就递增 document generation，旧 generation 的 observation/input/action receipt 一律失效；`window.state` 与 `window.activate` 是不同操作。production `window.activate` 只接受当前 session/lifecycle、attempt、target、HWND/owner、surface/coordinate 全绑定并在 dispatch 后复核的命令，歧义返回 unknown。`business_modal` selector 在 production 恒为空；内部 `BusinessModal` 是 human-only security surface，既不可发现也不可输入。
...
F7 source evidence 为 Launcher 全树 **2678 passed + 3 explicit opt-in skipped / 2681 total，0 fail**、SDK resolver **7/7**（精确 `.NET SDK 10.0.300`）、Node client **37/37**、TrustedRunner 过滤 **48/48**。exact C1 tree `7362881e96d8ed0f9c20ccae580426c522f14946` 的 candidate identity 为 `F67F1054E7DD19600138C3196D0798CFA487701CB7143C4DDFD2DC426D26E372`、closure 为 `3C2CA3E6E935BF23A061228ED3D9BDA3823E81186057E8C86118FAD5C7CEBF0D`、Core EXE SHA-256 为 `86DF1F5DC611037DB3A85FD9BA0D43490394F232D25A4F62B59AA4F2B4B6E4FD`，production policy **26/26**，状态为 `candidate_built / NOT_DEPLOYED`。当前执行会话没有真实前台，严格 `automation/start.ps1 -CandidateRoot` 入口返回 `trusted_runner_credential_timeout` 且未生成 completion evidence；post-check 无候选/Flash/热键进程、端口文件或 live bootstrap/credential/rendezvous 残留，因此该负向证据不构成 `candidate_executed`、`e2e_verified` 或 `promoted`，正式 runtime 保持不变。
