...
本节只描述 H1 activation commit `c5ca5dfa9718e8a7714038e929a64081b7fd0026` 之后的 current source。第一份 S `90985aecc9a3ce9a0bcd1ad11dde2b7158705644` / tree `beaaab2622bb9d3bfaa44485d63e274de14224a3` 的静音 A6 attempt 已失败；修复后的 S2 `036097289bac2b62304b0cfc23605246d4c2f950` / tree `d6052412c3ddcfdaefe7825b9b0775c5fd1ab12c` 又在真实非静音 capture 后暴露 operator artifact/wire canonical mismatch。本节所在版本与 comparator/test/dependency/doc closure 首次同树落盘的 clean implementation commit 定义为新的 Source S，commit/tree 不在自身文档 blob 中预填，而由提交后的 Git object 解析；operator 当前 `9/9`，dependency 为 `568` 项 / `142053` bytes / closure `7BB1F7E5…860B` / manifest `D9361D6C…34B7`。该 exact S 仍须重新执行双 clean repro、构建新 candidate 并从 case 1 重跑 A6。当前继续为 **H2 blocked / NOT_DEPLOYED**，未进入 runtime promotion；不能把源码、单元测试、本地 DLL 或旧 candidate 外推为玩家正式入口已经升级。权威合同见 [Audio Platform v2 ADR](../docs/原生音频平台-v2-格式能力桥接契约与可观测性-ADR-2026-08-09.md)。
...
- **失败 attempt 台账**：第一份 S 的 candidate identity `1A675D3F200557E41E2821E74CE31309107051FE0EAEC79D9A402E7DFA146CEB` / closure `D5232F5688598118196EE09945E6FE2C4AD2C739AFB247A985BD6C920DA96A1F` 以 runId `cdf7c0efbd6745d29ddb467808259f64` 进入 case 1 `bgm_playback`，因无 AS2 ingress、source `playing=true` 但 cursor/meter 为 0 而静音 fail-close。S2 的 A/B/live producer 则一致得到 identity `0044438A14045F53DA88588DA96BCFF3E90AE740BA3A728234A2FE5403FAF2FA`、closure `32FD1E5B1035A369416F6A2AA837DEA455A884B80DC3C02A3107373D367A71EF` 与 runtime manifest SHA-256 `ED2DFB302DD7A659D9636530D3D92C3CDB857210AD1D9C9F49B7D356A49BEC98`；runId `5cf1375c53f5de5123d16b1b750f7ec3` 生成 384044 B / SHA-256 `00606ADAF21B326A4AFA8B9741CF1AA80F5D55A1F3FF5EEB9A79CB2AFD884ECA` 的真实非静音 WASAPI WAV 和 944 B / SHA-256 `9EF166E3DBDE8685AF4535E3964F028F44F581F4D0A0E0ED23DAD5411F8359CC` 的 configuration，却因 operator 混用 artifact/wire canonical 格式而 fail-close。两份 candidate 均已优雅停止，始终 `NOT_DEPLOYED`；没有 E1/H2，两个 runId 与已有 capture 永久作废。后续验证必须绑定本文所定义新 S 的双 clean repro、新 candidate 与新 runId。
