...
首份请求 `A550C3EF53B7725D136C40EA3EA36C10F86F565BC2BB67D9D4351872F95A3EBE` 在本地完整政策门为 36/40，未创建远端标签或调用云端。发布准备进一步刷新材料 sidecar 的装备来源摘要，字典正文和 224 项材料不变；新增撤退按钮的两处颜色字面量改用既有主题变量，WB130 限额未放宽。商店头像按当前彩蛋地图真实 SWF/XFL 重烘焙，唯一 linkage 链为 `464→270→268 / frame 1`，校验器同步钉死这个已验证的新地图根编号，未放宽身份、帧数、配饰或几何检查。34 项来源/清单/图片闭包通过；27 张图片字节不变，7 张各有 5–300 个可见像素的细微差异，尺寸和 alpha 边界一致，抽查最大差异样本外观一致。上述修正只改变政策域及完整源码树，重新建请求并 supersede 首份请求；本地签名产物仍按同一 build identity 复用，最终完整政策收据和云端证明必须绑定最终 source。
