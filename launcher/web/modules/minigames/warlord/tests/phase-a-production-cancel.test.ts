...
import { applyOk, faction, makeState, setPlanning } from './helpers.js';

test('PHASE-A-CANCEL 未开工订单可全额撤销并释放预留人口', () => {
  let state = makeState('cancel-unstarted');
  setPlanning(state);
  faction(state, 'red').gold = 100;
  const populationBefore = faction(state, 'red').populationReserved;
  state = applyOk(state, {
    type: 'ENQUEUE_PRODUCTION', factionId: 'red', nodeId: 'R-Supply', slotId: 'R-Supply:1', cardId: 14,
  });
  const order = faction(state, 'red').productionQueues['R-Supply']?.[0]?.orders[0];
  assert.ok(order);
  assert.equal(faction(state, 'red').gold, 100 - order.goldCost);
  assert.equal(faction(state, 'red').populationReserved, populationBefore + order.populationCost);

  state = applyOk(state, {
    type: 'CANCEL_PRODUCTION', factionId: 'red', nodeId: 'R-Supply', slotId: 'R-Supply:1', orderId: order.orderId,
  });
  assert.equal(faction(state, 'red').gold, 100);
  assert.equal(faction(state, 'red').populationReserved, populationBefore);
...
test('PHASE-A-CANCEL 已获得生产进度的订单保持锁定', () => {
  ...
  setPlanning(state);
  faction(state, 'red').gold = 100;
  faction(state, 'red').cards[15].level = 10;
  state = applyOk(state, {
    type: 'ENQUEUE_PRODUCTION', factionId: 'red', nodeId: 'R-HQ', slotId: 'R-HQ:1', cardId: 15,
  });
  const order = faction(state, 'red').productionQueues['R-HQ']?.[0]?.orders[0];
  assert.ok(order);
...
test('PHASE-A-CANCEL 可精确撤销待开工队尾而不改变队首', () => {
  ...
  assert.ok(head && tail);
  const goldBeforeCancel = faction(state, 'red').gold;
  const reservedBeforeCancel = faction(state, 'red').populationReserved;

  state = applyOk(state, {
    type: 'CANCEL_PRODUCTION', factionId: 'red', nodeId: 'R-HQ', slotId: 'R-HQ:1', orderId: tail.orderId,
  });
  const remaining = faction(state, 'red').productionQueues['R-HQ']?.[0]?.orders;
  assert.deepEqual(remaining?.map((order) => order.orderId), [head.orderId]);
  assert.equal(faction(state, 'red').gold, goldBeforeCancel + tail.goldCost);
  assert.equal(faction(state, 'red').populationReserved, reservedBeforeCancel - tail.populationCost);
});
...
test('PHASE-A-CANCEL 仅在本阵营尚未提交的结算规划中开放', () => {
  ...
  const orderId = faction(state, 'red').productionQueues['R-Supply']?.[0]?.orders[0]?.orderId;
  assert.ok(orderId);
  const command = {
    type: 'CANCEL_PRODUCTION' as const,
    factionId: 'red' as const,
  ...
  faction(state, 'red').planningCommitted = true;
  assert.equal(validateCommand(state, command).error, '该阵营已经提交规划。');
  faction(state, 'red').planningCommitted = false;
...
test('PHASE-A-CANCEL 撤销命令进入录像并可逐字段重放', () => {
  ...
  const orderSlot = faction(state, 'red').productionQueues['R-Supply']?.[0];
  assert.ok(orderSlot);
  state = applyOk(state, {
