...
**文档角色**：启动与运行自动化入口。  
**最后核对代码基线**：当前角色构筑 ↔ 玩家自身技能双向导航发布冻结 source commit `c4faf14460238c7ea3e85983f31dee8be1b79afa`（tag `runtime-build-v2/20260728-character-build-skills-bidirectional-v1`、request `BBCC17C7E46AC852BA0F1D12AA9434188C7775BEA8F593C13B13710783183548`），由 commit `45b9748baa68786a52557239d5bd7c52869970f7` 记录正式 promotion；exact isolated candidate 与无参 `automation/start.ps1` 正式入口均完成原生装备 → Character Build → Skills manage → 返回构筑及普通关闭 smoke，严格状态为 `standard_entry_verified`。
...
正式 v2 consensus 当前绑定 tag `runtime-build-v2/20260728-character-build-skills-bidirectional-v1`、request `BBCC17C7E46AC852BA0F1D12AA9434188C7775BEA8F593C13B13710783183548`、source commit `c4faf14460238c7ea3e85983f31dee8be1b79afa`、build identity `4E5EEE4AB5BE0CC8D084254C54F23AC4D6269C11CFED987409EA6BBE171CE191` 与 payload closure `7460D8D4FC4416EDBDDFE7577403CBA7233ECC347BDF2D605BC5D2252AF39507`；promotion 记录 commit 为 `45b9748baa68786a52557239d5bd7c52869970f7`。本次 local signer 是 `builder-local-a` / `physical-host-a`，不可导出 keyId 为 `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`；GitHub Actions run `30333045634` 的 OIDC identity `3B32F64D834C2A9961A7C4EA57AD998E0A463360F703904601C1A33823F675BC` 提供 `github-hosted-windows` 第二票。production policy `89F35ED0F8618346614642B0D913443041345433CDDA3536046F8F1E5C6F36BE` 的 22/22 final receipt SHA-256 为 `C5E683CDD3958B9CF7503DBD08C309CC6EA6E6EFF92F3203B78682AF9C1BE9C3`；cloud workflow 仍只允许 `Crazyfs` / `Flash-Night` 的固定 actor ID 首次 dispatch。
