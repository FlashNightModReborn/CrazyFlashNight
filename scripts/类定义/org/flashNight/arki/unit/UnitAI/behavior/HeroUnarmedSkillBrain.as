...
//   3. 平A保底：射程内先掷骰子（参数 平A保底概率，默认 15%），命中即出平A 且不进技能裁决；
//      平A 期间置 自机.虎妙平A中，禁止 _tryInterrupt 换招（否则平A 会被 tier0 保命技顶掉）
//   4. 打断与 commit（D10）：
...
class org.flashNight.arki.unit.UnitAI.behavior.HeroUnarmedSkillBrain {
    ...
    // 击倒脱困边缘检测：上一 tick 是否处于击倒（用于"新一次击倒"判定，配合 自机.虎妙击倒已脱困）
    ...
    public function HeroUnarmedSkillBrain(_data:UnitAIData, params:Object) {
        ...
        this.p = params;
        // 战技组：优先用生命周期写入的 自机.虎妙战技组（技能组配置机制）；
        // 每次裁决时实时读 self（见 _decide），重初始化改配置可即时生效，这里只做兜底副本。
        this.battleSkills = (this.self.虎妙战技组 instanceof Array)
            ? this.self.虎妙战技组
            : HeroUnarmedSkillTable.getBattleSkillCopy();
    }

    /** 战技组实时读取：自机.虎妙战技组 优先（生命周期每次初始化重建），兜底构造时副本 */
    private function _battleSkillList():Array {
        return (self.虎妙战技组 instanceof Array) ? self.虎妙战技组 : battleSkills;
    }
    ...
    public function tick(frame:Number):Void {
        ...
        if (_skillStartFrame >= 0 && _isNoInterruptSkill()
            && frame - _skillStartFrame < 30 && self.虎妙已用永久技能 != null) {
            // 用 =false 而非 delete：AS2 的 delete 对动态属性行为不稳定，判定口径是 == true
            self.虎妙已用永久技能[_currentSkillName] = false;
        }
        _skillStartFrame = -1;
        _currentSkillTier = -1;
        ...
        _currentSkill = null;
        _lastUnitState = st;
        self.虎妙平A中 = false; // 平A 已收招，解除打断保护
        ...
        if (isKnockdown) {
            if (!_wasKnockdown) self.虎妙击倒已脱困 = false; // 新一次击倒，重置成功标记
            _wasKnockdown = true;
            // 每次击倒只成功脱困一次：小跳/闪现在 CD 时下 tick 继续重试（保证"立刻"），
            // 一旦放出去就打标记，避免击倒持续期间反复小跳（此前的"无限高频小跳"）。
            if (self.虎妙击倒已脱困 != true) {
                if (_knockdownEscapeTick(frame)) self.虎妙击倒已脱困 = true;
            }
            return;
        }
        _wasKnockdown = false;
        ...
        // ── G. 平A保底：射程内先掷骰子，命中即走平A（不进技能裁决）──
        // 必须排在技能裁决之前，否则技能几乎每 tick 都能命中候选，平A永远轮不到。
        // 标记 虎妙平A中：平A 期间禁止 _tryInterrupt 换招（平A 不设 tier，
     ...
     *
     * @return true = 本次成功放出脱困技（调用方据此打 虎妙击倒已脱困 标记，
    ...
    private function _decide(frame:Number, maxTier:Number):Object {
        ...
        for (var li:Number = 0; li < lists.length; li++) {
            ...
            for (var i:Number = 0; i < skills.length; i++) {
                ...
                if (data.absdiff_z > (_isMobileSkill(sk) ? zLoose : zLimit)) continue;
    ...
    private function _passSpecialRules(sk:Object):Boolean {
        ...
        if (sk.单图一次 == true) {
            if (self.虎妙已用永久技能 != null && self.虎妙已用永久技能[sk.技能名] == true) return false;
        }
        return true;
    }
    ...
    private function _release(sk:Object, frame:Number):Void {
        ...
        if (sk.技能名 == "小跳" || sk.技能名 == "闪现") {
            ...
            self.虎妙方向锁 = {上行: up, 下行: down, 左行: lf, 右行: rt,
        ...
        // 单图一次标记
        if (sk.单图一次 == true) {
            if (self.虎妙已用永久技能 == null) self.虎妙已用永久技能 = {};
            self.虎妙已用永久技能[sk.技能名] = true;
        }
    ...
    private function _isCurrentAirSkill():Boolean {
        ...
        var airNames:Array = (self.虎妙空中技能组 instanceof Array)
            ? self.虎妙空中技能组
    ...
    private function _isNoInterruptSkill():Boolean {
        ...
        return (_currentSkill.不可打断 == true);
    }
