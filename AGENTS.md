...
**最后核对代码基线**：NPC 商店 snapshot 数量污染防护与跨层诊断加固 source commit `9df1fb8fdadf1df10c6bcc9b0552ebcfcf16a834`（tag `runtime-build-v2/20260816-npcshop-snapshot-quarantine-v2`）、release tree `72656dde8da92ee5863f3fc760db8e9f20e07ead`、request `13EB5AC5F2413D7C1547D5E0486C5ACF8183C0145DE02785F8B1932FE5FE2FEC` 已由本地 X509 `physical-host-b` 与 GitHub OIDC/Sigstore `github-hosted-windows`（cloud run `31928663322`）完成 v2 双 signer/双故障域 promotion；正式 runtime 绑定 identity `C23D09677F191D6CCF5666E64E64F8CE664738E6F85AB46FDE916CED4F667708`、closure `FF6CD703624ECA341C8B52AAB726DA7DEDF3914FBD2306F0712A3BE5D3183C6E`、Core SHA-256 `6543352D859EFCB4361EB5C90AC8D61AA3A9955D770D958C4596745073287165`，promotion 提交 `72f988d4e6aca7936166f13a6e751707a5492c63` 与 post-promotion audit run `31930140127`（`state=promoted`）均已推送/通过。该列车把材料/情报 collection 中正数但不符合安全整数契约的旧档项隔离出业务投影、保留原始值供存档无损回写，并在 AS2/Host/Web 布设只含计数、生命周期键与枚举原因的关联脱敏诊断；首个 v1 request `E674F0E13CC8DC19C65F7F6B3A50307CD753124C82E0BAA1BB87D9E40B6717B4` 因真实 `WB112`（`npcshop.js` 1047/1040）在本地 policy 阶段失败，未 dispatch 云构建、未 promotion，随后由 v2 原子 supersede。正式根入口 `--verify-only` 为 `exit 0`；污染夹具、严格协议、Web/Host/AS2 门与发布链已通过，但测试员机器物理隔离且原存档已丢失，故不能声称原事故唯一归因、真实存档 E2E 或 `standard_entry_verified`。此前 Web UI 音效语义/audioPrefs `b8565b3681e59dd977d71fb80b1493ffae8fac92`、tooltip/合成链修复 `29ab59fd7832d10d54badc3132a5153646c1fa3d`、面板回包最小门 `704d836f42b70f788db8b5b2151c8ab3dcf77792`、Audio Platform v2 emergency `48c312234ab189b16f9edd2f5f7b0b2e78fc1407`、装备调制属性对比 `167d929a4bc3a85d6d21cc7faa220644eefe01d7`、跨层回包键 `a87be26f1d6e80c0ea8883bbc10e46c895c35799`、竞技场 P4/P5、商城旧档、双栏工作台、F8 与 F7 C1 均只保留为历史分层证据；Audio H2 继续作为专项 `pending`，不进入通用 runtime promotion 门。
