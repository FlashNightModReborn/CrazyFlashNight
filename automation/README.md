...
当前证据分为自动门、精确隔离 candidate E2E 与正式标准入口三层。`node tools/test-agent-entry-contract.js` 静态锁定 notifier guard、`s:1|ga:<attemptId>` 同包与 `gotoAndStop("读盘")` 顺序；装备与斗兽 runner 的 `--check` 覆盖 post-watermark 真实 title-frame marker、watchdog 拒绝、`title_frame_not_observed`、fresh handoff、exact slot/attempt、single enter 和 attempt-bound `gameEnteredObserved`。Launcher source-tree 全量 **1220/1220**，其中 AgentControl 定向 **29/29**；装备 runner `--check` 为 **34 checks**。隔离 candidate `tmp/runtime-candidates/v2/c-2a0cddb077b7-08846e81b3-20260721t100014612z-f32a40a3`（build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107`、payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C`、Core SHA-256 `0F58BF864B8DE9C7FCEA098D7E1EEA1996BDE38D85D87E844B047B53F5247232`）曾在 attempt `62f04fa249c44d94ba3171f619e61754` 完成 corrected Agent entry、正常 NativeHud、同实例 organizer、普通 suspend/same-anchor 内容不变 reopen/final claim，因此当时达到 `e2e_verified / NOT_DEPLOYED`；其 25-file native bundle 不含 `launcher/web`，外部 Web 字节仍须由 [测试指南](../agentsDoc/testing-guide.md) 的源码哈希与 WebView2 实机日志独立绑定。随后同一 build identity / payload closure 经 immutable request、production receipt、`builder-local-b` + GitHub OIDC quorum 与 strict verifier 完成 promotion；prewarm-held formal attempt `4eae1360cedd413fa3175db6a8997158` 取得 fresh title marker、exact attempt receipt，并完成真实 loot 两次 claim、terminal close/unpause 与存盘，故唯一 canary 已达到 `standard_entry_verified`。此前一次 formal 冷启动因 reveal watchdog 先于真实 title receipt 而返回 `title_frame_not_observed`，仅保留为换机 portability 历史观察，不覆盖后续成功结论。
...
最后一条命令会从受保护的单路径段 `runtime-build-v2/<release-id>` source tag 触发固定 cloud workflow，并验证 API-resolved tag、`GITHUB_REF/GITHUB_SHA` 与 run `headSha` 都精确绑定请求的 full commit；随后等待精确 run、安全解压并产出 `$cloud.candidateRoot` / `$cloud.proofPath`。unsigned job 交接 artifact 保留 1 天，失败诊断与 signed 结果保留 7 天；超期未 promotion 就重新 dispatch，不把 Actions artifact 当长期档案。request、队列/CAS、双故障域 quorum、receipt 与 `promote-runtime-bundle.ps1` 的完整步骤以 [runtime v2 发布列车](../docs/runtime-build-reproducibility.md) 为准。最近一次完整 Runtime Lane C 复跑为 **11/11 个入口 exit 0**；十个 scalar 套件合计 **409**，guardrails 另报告 `scripts=3 / unsafeCandidateCases=3`。bootstrap / build environment 均 exit **0**；这些只证明 runtime/admission 守门回归，本身不产生 candidate identity、runtime proof 或 promotion。FCA19/B2AF/231388 candidate 及 `c-2a0cddb077b7-08846e81b3-20260721t100014612z-f32a40a3` 的 E2E 均保留为 promotion 前隔离证据，后者当时是 `e2e_verified / NOT_DEPLOYED`，且其 native bundle 不绑定外部 `launcher/web` 字节。本轮相同 build identity `2A0CDDB077B760328B3141EFFFEEE3996841FA1CE49AD09E5D7339417F60A107` / payload closure `3B837DCDBC69AA47074E635DACACAE3B80263023E6032AC1FDF209768B1C150C` 随后已完成正式 quorum、promotion 与 attempt `4eae1360cedd413fa3175db6a8997158` 的标准入口领域 smoke，因此唯一 canary 当前为 `standard_entry_verified`。功能回归证据统一维护在 [测试指南](../agentsDoc/testing-guide.md)，不在 runtime 发布段重复复制。任何时候都禁止手工换 manifest、伪造证明，或把单机 candidate 复制进根 runtime。
