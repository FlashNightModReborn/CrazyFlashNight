...
**最后核对代码基线**：release source commit `34a944055880f5e150cdf25053fffc4fd238cce7`（2026-09-08；tag `runtime-build-v2/20260908-map-retreat-v1`）。地图主动撤退已完成双 signer / 双故障域共识、40/40 production policy 与原子 promotion，当前为 `HUMAN_ACCEPTANCE_PASSED / promoted`（限已实测地图中途撤退与再次入场）。完整身份见 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)，功能边界见[地图主动撤退](docs/关卡结果与基地结算-CSharp-Web-ADR-2026-08-27.md#0c-2026-09-08-地图主动撤退入口)；不外推新地图胜利/死亡入口、非零奖励或业务 `standard_entry_verified`。地图创作工作台第二阶段及上游焦点/撤退结算修复保持现役，见[两阶段记录](docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)。
