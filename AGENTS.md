...
**最后核对代码基线**：CF7 Agent Runtime / Wings F7 C1 source commit `dd84230a1d262c6478591cae2d11051b7a8aa7b1`（2026-07-31）。当前 documentation-only D1 只引用其父 C1；D1 不能在自身内容中记录尚未知的自身 commit hash。
...
- **Launcher 验收术语**：统一使用 `compiled → candidate_built → candidate_executed → e2e_verified → promoted → standard_entry_verified`；`launcher/build.ps1` 最多到 `candidate_built`，候选执行 / E2E 必须绑定实际进程路径、build identity 与 payload closure。F7 C1 `dd84230a1d262c6478591cae2d11051b7a8aa7b1` 已取得 exact `candidate_built`（identity `F67F1054E7DD19600138C3196D0798CFA487701CB7143C4DDFD2DC426D26E372` / closure `3C2CA3E6E935BF23A061228ED3D9BDA3823E81186057E8C86118FAD5C7CEBF0D`），但未达到 `candidate_executed`、`e2e_verified` 或 `promoted`，严格状态仍为 `NOT_DEPLOYED`，正式 runtime 保持不变。只有 `promoted` 后再从 `automation/start.ps1` / 根 bootstrap 标准入口验证到同一正式身份，才可称“已部署 / 正式验收”；完整证据契约见 [testing-guide.md](agentsDoc/testing-guide.md)
