...
- SAFEEXIT 二次确认：router 路由到 PostToWeb `safe_exit_show`（web 端 [notch.js](web/modules/notch.js) Bridge.on 仅展示面板）+ `SendGameCommand("safeExit")` 触发存盘；存盘成功后 web 现有 sv 状态机驱动 取消/退出游戏 按钮
...
- 完整 idle SW_HIDE 推迟到 Phase 4 收尾：当前 web 端仍承载 combo / quest-notice / jukebox-titlebar / map-hud / safe-exit-panel 等常驻 HUD（其他已迁）。`DoFullIdleSuspend` 代码已就位，等剩余 widget 都迁完后启用
