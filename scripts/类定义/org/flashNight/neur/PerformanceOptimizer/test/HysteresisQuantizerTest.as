...
class org.flashNight.neur.PerformanceOptimizer.test.HysteresisQuantizerTest {
    ...
    private static function test_downgrade_twoStep():String {
        ...
        out += line(r2.levelChanged == true && r2.newLevel == 1,
            "降级第2次：达到阈值，切换到1");
        ...
        out += line(r3.levelChanged == false && !q.isAwaitingConfirmation(),
            "候选等于当前：确认状态清空");
    ...
    private static function test_upgrade_threeStep():String {
        ...
        var r1:Object = q.process(0.8, 2);
        out += line(r1.levelChanged == false && q.getConfirmCount() == 1,
            "升级第1次：不切换，计数=1");

        var r2:Object = q.process(0.8, 2);
        out += line(r2.levelChanged == false && q.getConfirmCount() == 2,
            "升级第2次：不切换（需3次），计数=2");
        out += line(q.getPendingDirection() == -1,
            "方向=升级(-1)");

        var r3:Object = q.process(0.8, 2);
        out += line(r3.levelChanged == true && r3.newLevel == 1,
            "升级第3次：达到阈值，切换到1");
    ...
    private static function test_directionReversal():String {
        ...
        // 方向反转到降级
        var r:Object = q.process(2.6, 2);
        out += line(r.levelChanged == false && q.getConfirmCount() == 1 && q.getPendingDirection() == 1,
            "方向反转：计数重置为1，方向=降级(+1)");

        // 继续降级方向，达到阈值
        var r2:Object = q.process(2.6, 2);
        out += line(r2.levelChanged == true && r2.newLevel == 3,
    ...
    private static function test_clampByMinLevel():String {
        ...
        var r3:Object = q.process(0, 3);
        out += line(r3.levelChanged == true && r3.newLevel == 2,
            "升级第3次：达到阈值，切换到2");
    ...
    private static function test_customThresholds():String {
        ...
        // 降级：1次即切换
        var r1:Object = q.process(1.2, 0);
        out += line(r1.levelChanged == true && r1.newLevel == 1,
            "降级阈值=1：首次即切换");
        ...
        var r2:Object = q.process(0.4, 2); // count=4
        out += line(r2.levelChanged == true && r2.newLevel == 0,
            "升级阈值=4：第4次切换");
