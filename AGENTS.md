...
**最后核对代码基线**：release source commit `1d03e77cb2666a4686e41ab4a647e180d2e5d0bc`（2026-09-09；tag `runtime-build-v2/20260909-hairdresser-grid-v1`；部署与合并 head `c595bc2cd261342d37cd461cf5ff08ee62b9c76a`）。理发店目录图标网格 / 性别页签 / 全身预览切换、服务面板共享动效、整形反馈补强与 hairdresser snapshot 可选 `portrait` 投影已完成双 signer / 双故障域共识、40/40 production policy 和原子 promotion，当前为 `promoted / FIELD_REVALIDATION_PENDING`：部署闭包完整性已经 post-promotion Audit 确认，理发店网格手感与 tooltip 落点、全身预览真机画面、整形音效红行与基础斩杀 AI 行为仍待测试员正式入口回归，不称业务 `standard_entry_verified`。完整身份见 [runtime-build-reproducibility.md](docs/runtime-build-reproducibility.md)，功能与验收边界见[理发店网格说明](docs/理发店目录图标网格与共享外观服务动效-2026-09-08.md)。上一发布（医务室整形 Web 迁移与共享外观预览，`c59b9b4cdb9319c04503a703019a9dff30bc04a1`）保持 `HUMAN_ACCEPTANCE_PASSED / promoted` 原范围，功能边界见[整形说明](docs/医务室整形-Web面板与外观共享-2026-09-08.md)。选关前台交接、返回重试与地图中途撤退的既有结论保持原范围，见[焦点诊断 §9.13](docs/焦点管理-诊断与卡顿排查-2026-05-24.md#913-2026-09-08选关关闭前的前台交接)、[关卡结果 ADR §0C/§0D](docs/关卡结果与基地结算-CSharp-Web-ADR-2026-08-27.md)；地图工作台第二阶段见[两阶段记录](docs/地图工作台与CSharp收束-两阶段施工-2026-09-06.md)。

焦点诊断默认关闭；测试员设置根 `config.toml` 的 `diagFocusTrace = true` 后重启即可持续录制并在正常退出时自动打包，专用 CMD 保留为临时启动/途中采集入口。边界见 [焦点诊断 §9.12](docs/焦点管理-诊断与卡顿排查-2026-05-24.md#912-2026-09-07配置化持续录制与固定容量保留)。启停录制复验不证明原焦点故障已根治，真实关卡失灵仍为 `FIELD_REVALIDATION_PENDING`。 本分支另有[消息/技能取证独立发布](docs/runtime-build-reproducibility.md)：source `ac33374c4cc6255a81ebd65280d1eb0801a177b5` 已完成双构建共识、40/40 与独立工作树 promotion，按维护者要求保留分支、暂不合流原安装目录；新协议与 22 MiB 实时容量按[焦点诊断 §9.14](docs/焦点管理-诊断与卡顿排查-2026-05-24.md#914-2026-09-09消息取出边界与技能输入分层取证)。前一 R1 列车仍待合并后的正式入口业务复验，边界见 [R1 收尾记录](docs/R1存盘API迁移收尾-2026-09-05.md)，不称 R1 专项 `standard_entry_verified`。
