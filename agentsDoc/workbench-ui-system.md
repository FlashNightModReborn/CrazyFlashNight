...
- Character Build 的拆分边界固定为：`character-build-session-contract.js` 只做 wire shape，`character-build-transport.js` 只做 loadout mux framing，`character-build-candidate-channel.js` 只做单条权威只读缓存、候选读取及 stale recovery，`character-build-loadout-presenter.js` 只做 11+4 槽与已装备注释，`character-build-candidate-pane.js` 只做候选范围/选择/恢复/pointer drag，`character-build-candidate-eligibility.js` 只做 universal 背包行的纯资格重投影。facade/session/view 继续拥有组合与生命周期，不得把这些叶模块扩成第二套领域状态机或多 key 缓存框架。
