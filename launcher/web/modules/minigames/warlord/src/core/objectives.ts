...
import type { FactionId, GameResult, GameState, NodeId } from './types.js';

function commanderSnapshot(state: GameState): GameResult['commanderStates'] {
...
function activeVictoryGroupIds(state: GameState): string[] {
...
function representativeFaction(state: GameState, victoryGroupId: string): FactionId | 'draw' {
  const group = state.victoryGroups[victoryGroupId];
  if (!group) return 'draw';
  return group.factionIds.find((factionId) => requireFaction(state, factionId).defeatedAtRound === null)
    ?? group.factionIds[0]
    ?? 'draw';
}

export function applyGameResultInPlace(state: GameState, result: GameResult): void {
...
export function evaluateVictoryGroupsInPlace(
  state: GameState,
  reasonCode: GameResult['reasonCode'] = 'AllHostileVictoryGroupsEliminated',
  reason: GameResult['reason'] = 'elimination',
): boolean {
...
export function captureCommandPostInPlace(
  state: GameState,
  capturingFactionId: FactionId,
  nodeId: NodeId,
): boolean {
  const defeated = Object.values(state.factions).find((faction) => (
    faction.commandPostNodeId === nodeId
