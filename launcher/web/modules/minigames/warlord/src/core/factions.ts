...
export function factionIds(state: GameState): FactionId[] {
  return [...state.turnOrder];
}

export function requireFaction(state: GameState, factionId: FactionId): FactionState {
  const faction = state.factions[factionId];
  if (!faction) throw new Error(`Unknown runtime faction ${factionId}.`);
  return faction;
}

export function createDefaultRelations(ids: readonly FactionId[]): Record<FactionId, Record<FactionId, StrategicRelation>> {
  const matrix: Record<string, Record<string, StrategicRelation>> = Object.create(null) as Record<string, Record<string, StrategicRelation>>;
  for (const left of ids) {
    const row: Record<string, StrategicRelation> = Object.create(null) as Record<string, StrategicRelation>;
    for (const right of ids) row[right] = left === right ? 'allied' : 'hostile';
    matrix[left] = row;
  }
  return matrix;
}

export function createDefaultVictoryGroups(
  ids: readonly FactionId[],
  displayNameByFaction: Readonly<Record<string, string>>,
): Record<string, VictoryGroupState> {
...
export function areHostile(state: GameState, left: FactionId, right: FactionId): boolean {
  return relationBetween(state, left, right) === 'hostile';
}

export function areAllied(state: GameState, left: FactionId, right: FactionId): boolean {
  return relationBetween(state, left, right) === 'allied';
}

export function isFactionDefeated(state: GameState, factionId: FactionId): boolean {
  return requireFaction(state, factionId).defeatedAtRound !== null;
}

export function activeFactionIds(state: GameState): FactionId[] {
  return factionIds(state).filter((factionId) => !isFactionDefeated(state, factionId));
}

export function refreshFactionActionPoints(faction: FactionState): void {
...
export function spendFactionActionPointsInPlace(faction: FactionState, amount: number): void {
...
export function clearUnspentFieldActionPointsInPlace(faction: FactionState): void {
  faction.apLedger.fieldRemaining = 0;
  refreshFactionActionPoints(faction);
}

export function defeatFactionInPlace(
  state: GameState,
  factionId: FactionId,
  reason: NonNullable<FactionState['defeatReason']>,
): void {
  const faction = requireFaction(state, factionId);
  if (faction.defeatedAtRound !== null) return;
  ...
  for (const pieceId of removedPieceIds) {
    ...
    if (node) node.pieceIds = node.pieceIds.filter((candidate) => candidate !== pieceId);
    ...
    if (elementId) {
      ...
      if (element) {
        ...
        if (element.memberIds.length === 0) delete state.organization.commandElements[elementId];
      }
  ...
  for (const commander of Object.values(state.commanders)) {
    if (commander.factionId !== factionId) continue;
...
export function nextActiveTurnIndex(state: GameState, afterIndex: number): number | null {
  for (let index = afterIndex + 1; index < state.turnOrder.length; index += 1) {
    const factionId = state.turnOrder[index];
    if (factionId !== undefined && !isFactionDefeated(state, factionId)) return index;
  }
  return null;
}
