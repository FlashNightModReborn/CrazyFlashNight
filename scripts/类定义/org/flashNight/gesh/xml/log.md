...
--- 分相分解（50 项） ---
  ...
  50 项 | 6007 字符
    全流水线:      2.156 ms/次 | 64 次/轮 | 中位总 138.00 ms | 2.66 MB/s | 波动 1.10x
    原生 parseXML: 0.143 ms/次 | 512 次/轮 | 中位总 73.00 ms | 40.18 MB/s | 波动 1.12x
    parseXMLNode:  1.583 ms/次 | 96 次/轮 | 中位总 152.00 ms | 3.62 MB/s | 波动 1.03x
    --
    原生占全流水线: 7% (0.143 / 2.156 ms)
    parseXMLNode 占全流水线: 73% (1.583 / 2.156 ms)
    分相加总 vs 全流水线偏差: 20% (正常应 <10%)
...
--- parseXMLNode 多规模基准（变体冷路径 + 基线扣除） ---
  ...
  小(10项) | 1283 字符 | 64 变体
    全流水线(冷):   0.454 ms/次 | 24 批/轮 x32 = 768 次 | 中位总 349.00 ms | 2.69 MB/s | 波动 1.05x
    parseXMLNode(冷): 0.337 ms/次 | 24 批/轮 x32 = 768 次 | 中位总 259.00 ms | 3.63 MB/s | 波动 1.05x
    parseXMLNode(热): 0.334 ms/次 | 512 次/轮 | 中位总 171.00 ms | 3.66 MB/s | 波动 1.02x
    --
    parseXMLNode 占全流水线: 74% (0.337 / 0.454 ms)
    parseXMLNode 冷/热 = 1.01x

  中(50项) | 6007 字符 | 64 变体
    全流水线(冷):   2.164 ms/次 | 4 批/轮 x32 = 128 次 | 中位总 277.00 ms | 2.65 MB/s | 波动 1.03x
    parseXMLNode(冷): 1.691 ms/次 | 8 批/轮 x32 = 256 次 | 中位总 433.00 ms | 3.39 MB/s | 波动 1.01x
    parseXMLNode(热): 1.663 ms/次 | 80 次/轮 | 中位总 133.00 ms | 3.45 MB/s | 波动 1.04x
    --
    parseXMLNode 占全流水线: 78% (1.691 / 2.164 ms)
    parseXMLNode 冷/热 = 1.02x

  大(200项) | 24176 字符 | 32 变体
    全流水线(冷):   8.844 ms/次 | 2 批/轮 x16 = 32 次 | 中位总 283.00 ms | 2.61 MB/s | 波动 1.07x
    parseXMLNode(冷): 6.906 ms/次 | 4 批/轮 x16 = 64 次 | 中位总 442.00 ms | 3.34 MB/s | 波动 1.01x
    parseXMLNode(热): 6.792 ms/次 | 24 次/轮 | 中位总 163.00 ms | 3.39 MB/s | 波动 1.07x
    --
    parseXMLNode 占全流水线: 78% (6.906 / 8.844 ms)
    parseXMLNode 冷/热 = 1.02x
...
--- 热点剖析（微基准） ---
  ...
  属性迭代（50 项 XML 的 item 节点，每节点 4 属性）
    item 节点数: 50
    属性迭代:         13.65 us/次 | 192 批/轮 x50 = 9600 次 | 中位总 131.00 ms | 波动 1.03x

  同名节点数组提升（展平全树碰撞模式）
    模式: 根层 50 item 碰撞 + 50 item 各含 tags/Description +
          50 tags 各含 2 tag 碰撞。展平为单次循环测量总工作量。
    pairs 数: 212
    数组提升:         0.185 ms/次 | 1024 次/轮 | 中位总 189.00 ms | 波动 1.04x

  Description 单次解码路径（密集模式：每项都有 Description）
    当前路径: getInnerTextDecoded(node) 拼接子文本 + 单次 decodeHTML。
    Phase 1 已修复双重解码问题。
    Description 节点数: 50
    Description(当前): 31.67 us/次 | 96 批/轮 x50 = 4800 次 | 中位总 152.00 ms | 波动 1.03x
    decodeHTML(DOM文本):29.38 us/次 | 96 批/轮 x50 = 4800 次 | 中位总 141.00 ms | 波动 1.07x
    注意: DOM 文本不含实体，decodeHTML 近似 no-op；仅反映函数调用开销。

  decodeHTML 真实实体对比（含 &lt;/&gt;/&amp; 的字符串 x50）
    对比 decodeHTMLFast（单次扫描 O(L)）与旧 unescapeHTML（O(L×30)）。
    decodeHTMLFast:   60.94 us/次 | 64 批/轮 x50 = 3200 次 | 中位总 195.00 ms | 波动 1.06x
    unescapeHTML(旧):  0.116 ms/次 | 24 批/轮 x50 = 1200 次 | 中位总 139.00 ms | 波动 1.02x
    旧 / 新 = 1.90x

  convertDataType（240 值轮转）
    convertDataType:  2.84 us/次 | 768 批/轮 x60 = 46080 次 | 中位总 131.00 ms | 波动 1.05x

  --- 各热点占 parseXMLNode 总耗时占比（当前实现） ---
    parseXMLNode(参照): 1.563 ms/次 | 96 次/轮 | 中位总 150.00 ms | 波动 1.04x
    属性迭代:          1% (0.014 / 1.563 ms)
    数组提升:          12% (0.185 / 1.563 ms)
    Description(当前): 2% (0.032 / 1.563 ms)
    convertDataType:   0% (0.003 / 1.563 ms)
  ...
  isValidXML 累积成本（历史参考，50 项）
    ...
    旧 isValidXML 占当前 parseXMLNode: 134% (2.094 / 1.563 ms)

  Description 双重解码路径（历史参考）
    旧行为: getInnerText(node) 内调 decodeHTML → 外层再调 decodeHTML。
    当前: getInnerTextDecoded 单次解码。
    Description(旧双重): 60.94 us/次 | 64 批/轮 x50 = 3200 次 | 中位总 195.00 ms | 波动 1.03x
    旧双重 / 当前单次 = 1.92x
...
--- 解析器 CPU 基准（启动语料分布） ---
  ...
  极小(2项) | 1215 字符 | 模拟 7 个文件
    全流水线:  0.352 ms/次 | 256 次/轮 | 中位总 90.00 ms | 3.30 MB/s | 波动 1.02x

  小(10项) | 6037 字符 | 模拟 17 个文件
    全流水线:  1.738 ms/次 | 80 次/轮 | 中位总 139.00 ms | 3.31 MB/s | 波动 1.04x

  中(22项) | 13348 字符 | 模拟 15 个文件
    全流水线:  3.781 ms/次 | 32 次/轮 | 中位总 121.00 ms | 3.37 MB/s | 波动 1.03x

  大(48项) | 29232 字符 | 模拟 5 个文件
    全流水线:  8.063 ms/次 | 16 次/轮 | 中位总 129.00 ms | 3.46 MB/s | 波动 1.03x

  超大(96项,武器类) | 58567 字符 | 模拟 2 个文件
    全流水线:  16.250 ms/次 | 8 次/轮 | 中位总 130.00 ms | 3.44 MB/s | 波动 1.05x

  超大(96项,防具类) | 47640 字符 | 模拟 2 个文件
    全流水线:  15.100 ms/次 | 10 次/轮 | 中位总 151.00 ms | 3.01 MB/s | 波动 1.05x

  巨型(217项,防具类) | 109308 字符 | 模拟 2 个文件
    全流水线:  35.500 ms/次 | 4 次/轮 | 中位总 142.00 ms | 2.94 MB/s | 波动 1.03x

  Array.concat 累积合并（50 文件, 1572 总物品）
    concat累积: 2.792 ms/次 | 48 次/轮 | 中位总 134.00 ms | 波动 1.06x

  --- 解析器 CPU 时间估算 ---
    注意: 仅含 parseXML + parseXMLNode，不含 IO/PathManager/日志/回调开销。
    真实初始化耗时 = 本值 + IO等待 + BaseXMLLoader开销 + ItemDataLoader串行调度。
    极小(2项) x7: 0.35 ms/文件 x7 = 2.46 ms
    小(10项) x17: 1.74 ms/文件 x17 = 29.54 ms
    中(22项) x15: 3.78 ms/文件 x15 = 56.72 ms
    大(48项) x5: 8.06 ms/文件 x5 = 40.31 ms
    超大(96项,武器类) x2: 16.25 ms/文件 x2 = 32.50 ms
    超大(96项,防具类) x2: 15.10 ms/文件 x2 = 30.20 ms
    巨型(217项,防具类) x2: 35.50 ms/文件 x2 = 71.00 ms
    concat: 2.79 ms
    --
    解析器纯 CPU 合计: 265.52 ms（不含 IO 及框架开销）
