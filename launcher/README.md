...
> **最后核对代码基线**：commit `36666b3cf1`（2026-07-15）；Skill 独立 Web 面板 S0–S5 代码纵切、四个实际 Flash 目标发布、FFDec/compiled dictionary 证据、NativeHud `SKILLS` 真实 manage 点击、真实 NPC trainer 与此前 trainer→manage rebind 另核对本轮工作树。当前 UX 已收口为共享安全技能注释、无常驻详情栏、全宽技能库、1—12 单排技能带、方形紧凑瓦片、共享拖拽、形态/配置或学习/流派三组 direct facet，以及仅教师来源 manage 可返回研习的 Host-only capability；新布局/往返尚只有自动几何证据。旧技能页 live 指纹已确认 main `DefineSprite 53`。S4C 尚缺 legacy fallback 事务等价/重启回读，S5 尚缺 legacy Notch/fallback、pending-write/断线真机 Gate，S6 观察未开始。
...
| Native HUD 图像 | **SkiaSharp 3.119.4**（MIT；解码共享地图 WebP）→ `System.Drawing.Bitmap` 既有 tint/cache/ULW 管线 |
...
- **Native HUD 默认组成**：[NotchWidget](src/Guardian/Hud/NotchWidget.cs) 接管 web `#notch`（金币/KP、FPS/光照、时钟、游戏/辅助/系统三行 hover toolbar、通知栈和展开图表；[AudioHudState](src/Guardian/Hud/AudioHudState.cs) 以最高 10Hz 采样 BGM L/R peak，并把低透明度包络画在 FPS 折线背景；辅助行含点歌机/地图开关/修改器/帮助）。“系统 → 其他”不再渲染 16 项长列，而以“控制 / 测试 / 工具”三级分类页显示当前分类；任一叶子命令执行或刘海开始收起时同时关闭子页。[ToastWidget](src/Guardian/Hud/ToastWidget.cs) 承载 toast，[RightContextWidget](src/Guardian/Hud/RightContextWidget.cs) 承载 252px 六入口行、32px 条件状态槽和可选 `compact/expanded` 地图预览，[SafeExitPanelWidget](src/Guardian/Hud/SafeExitPanelWidget.cs) 以同锚点单行覆盖条件状态槽并取得最高绘制/命中优先级，[ComboWidget](src/Guardian/Hud/ComboWidget.cs) 承载搓招输入与命中动画。地图 shared renderer 仍复用 [MapHudWidget](src/Guardian/Hud/MapHudWidget.cs) 的 PNG alpha silhouette/blocks fallback。
