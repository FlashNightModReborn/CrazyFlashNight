...
_root.主动战技函数.长枪.旋转抡枪 = {初始化: null,
        释放许可判定: function(自机) {
            return 自机.攻击模式 === "长枪" && (!自机.倒地 && 自机.状态 != "击倒" && 自机.状态 != "技能" && 自机.状态 != "技能容器");
        },
        释放: function(自机) {
 ...
 * 天秤之力 - 光剑天秤主动技能
 *
 * 【玩法核心】伤害公式：耗蓝量 × 伤害系数 × 天秤切换次数
 * - 天秤切换次数：来自装备生命周期 ref.天秤切换次数，每次切换形态+1
 * - 释放后重置为1，鼓励玩家在积累足够切换次数后释放
 *
 * 数据配置: data/items/武器_刀.xml -> <skill> 节点
 */
_root.主动战技函数.兵器.天秤之力 = {
    初始化: function(自机) {
        var skill:Object = 自机.刀数据.skill;
        自机.天秤之力耗蓝比例 = skill.mpcost > 0 ? Number(skill.mpcost) : 5;
        自机.天秤之力伤害系数 = skill.power > 0 ? Number(skill.power) : 12;
        自机.天秤之力Z轴范围 = skill.range > 0 ? Number(skill.range) : 72;
        自机.天秤之力击退速度 = skill.knockback > 0 ? Number(skill.knockback) : 18;
    },
    释放许可判定: function(自机) {
        if (自机.攻击模式 !== "兵器") return false;
        var 耗蓝量:Number = Math.floor(自机.mp满血值 / 100 * 自机.天秤之力耗蓝比例);
        return 自机.mp >= 耗蓝量;
    },
    释放: function(自机) {
        var 耗蓝量:Number = Math.floor(自机.mp满血值 / 100 * 自机.天秤之力耗蓝比例);
        ...
        // 来源: 装备生命周期函数/光剑天秤.as -> 光剑天秤切换到形态() 中累加
        // ─────────────────────────────────────────────────────────────────────
        var ref:Object = 自机.装备生命周期反射对象.刀;
        var 切换次数:Number = ref && ref.天秤切换次数 ? ref.天秤切换次数 : 1;

        _root.发布消息("共转换过" + 切换次数 + "次天秤，星盘转动的力量因此得到了强化……");

        var 子弹威力:Number = 耗蓝量 * 自机.天秤之力伤害系数 * 切换次数;
