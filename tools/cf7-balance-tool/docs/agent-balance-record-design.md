...
首件确认案例见 [QJZ171](../../../docs/QJZ171-独立物品与Flash资产流程标定-2026-09-05.md)：37 级、888 威力、独立价格层 1、金币 460000；当前属性和历次调整结果见案例末节。内建暴击的效果期望与 formula 1 的基础残差分列，不把补充估算写成公式本身的输出。其中 impact 传 XML 原值 4；玩家面板由 `floor(500 / impact)` 显示 125。
...
钛合金61式原型入口为[models/ti61](../models/ti61/README.md)，强度锚点与结论见[建模记录](../../../docs/钛合金61式套装-原型建模-2026-09-08.md)。该模型读取当前装备、所选插件、生命周期与发射源，显式区分维护者转述的玩家实测、设计参数、源码投影和假设接触率；输出资源守恒、压力脚本与条件伤害敏感性，不修改本契约的八个武器输入、公式系数或审计状态。

使用`python -X utf8 -B tools/cf7-balance-tool/models/ti61/test_model.py`、`model.py`与`model.py --check`复算。历史候选和模拟策略维护于该目录`parameters.json`；当前运行原型数值从胸甲与钛合金P90的生产XML读取，报告默认写入`tmp/ti61-model/`。由模型试算出的171基础威力不等于已批准改值；本轮171的套装条件机制同步追加到现有unresolved台账说明，保留四层基础输入；P90的额外发电在套装ADR分列记录，完整武器层数仍待S3量化，不通过修改公式系数伪造confirmed。模型的守恒回归不能代替Flash编译和实际战斗标定。
