...
在 detached clean source-freeze F `cb38600aae51f5019d09f87c33bd9e67d2b1f511`、Windows PowerShell 5.1 上，最近一次完整 Runtime Lane C 复跑为 **11/11 个入口 exit 0**：其中十个会输出 scalar 计数的套件合计 **566** 项，`test-runtime-entry-guardrails.ps1` 另报告 `scripts=3 / unsafeCandidateCases=3`，总耗时 **831.271 秒**。完整 stdout 为 18,488 B / SHA-256 `9BFBDDD521BE54D70E0158CE595C37C54820EE4C66C73C5681AFC56636A524AC`，stderr 为 3,280 B / SHA-256 `F7FA16D6B9927C3B36D6A61FCBC49C00874EAAC41EBFC662F863793E442479C6`。`test-resolve-runtime-trusted-base.ps1` 是单列 supplemental，最近独立历史基线为 **9/9**，未在本次 11 项执行中复跑，也不计入 11 个入口或 566。Lane 自身不含环境 bootstrap，也不能单独替代真实双 builder；本轮另由 F request `839C74FD1DF61ACC1DA580041F6FA71CA13A84DF1F43A55237BF9BEEF8648FB2` 的 X509 + GitHub OIDC proof 与 promotion `-VerifyOnly` preflight 闭合非部署 quorum。该结论仍不产生 promotion、正式 runtime 变更或标准入口验收。
