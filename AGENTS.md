...
合并前 bundle `sha256:fbda67edc9378bc77348900d4b097f614d59f4cb0ccb775bdf5b49c62686d3b5` 已获人类批准并在旧 revision 上取得 `APPLIED_VERIFIED`；本次上游整合会改变 Git revision、`ArenaTask.cs` 与 release-policy closure，因此该回执只作历史输入，最终上线前必须在合并后真源和 fresh `asLoader.swf` 上重新生成 exact bundle、再次批准并复验。当前不称本增量已推送、promoted 或业务 `standard_entry_verified`。
