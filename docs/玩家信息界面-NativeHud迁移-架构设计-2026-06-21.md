...
**最后核对代码基线**：B0-06 当前实现冻结 commit `bf8dd2c410267855c8ea12f25a594042b3158479`（2026-07-29）。收尾整合后的主工作树 `HEAD=8c703c6811ef264da36eb482db524a16a4e64958`，其双亲为该实现基线与 `origin/main=c96f4c3d750561022b706c72a4d53050431e627d`；远端是当前 HEAD 祖先，ahead/behind=`21/0`。上游唯一增量只涉及双栏工作台文档，不改变 PlayerInfo 源码、配置或下述报告闭包。B0-04 的资产/证据身份、B0-03b 的新鲜 candidate/production Gate、B0-05 的 raster/topology 与 B0-06 的最终自动资格统一以 [专项 ADR](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md) §12 为稳定锚，不以聊天或过期临时产物为事实来源。当前 `runtime-qualification.json` 为 run `a6aadeb6e3264577bb9da8fe90857a7b`、647,234 B、SHA-256 `656286825CE9717D0DC31BDF9DFE00F3ECB57AB515C5C130601BDF23D319CD74`、47/47；focused 为 136 pass + 3 opt-in skip / 139，全量为 1,737 pass + 3 opt-in skip / 1,740。C#/Web A/B 与 comparison PNG 闭包由 `visual-evidence.json` 确定性冻结；两份报告仍精确绑定上述实现基线和明确 sourceTrace/input closure。真实 `pi_*`、accepted Flash oracle、可见桌面 DWM/游戏 z-order 与 occlusion、真实手点、审美、最终 Kimi K1、后续 source-freeze commit/tag 的双正式 builder/quorum 和人签尚未闭合。上游已完成的角色构筑/Skills 与材料工作台正式验收不得外推为本 HUD 纵切。本文 §2/§3 的初始断言来自 13-agent 代码级盘点 + 4 视角对抗审计 workflow，后续施工记录按实际 `.as`/`.xml` 持续回写。
...
- **建议**：HP/MP fixture-only B0 的 formal qualification 与双份 visual closure 已形成；下一步按专项 ADR 先冻结 evidence/docs-only containing commit，再用独立最小配置切片形成最终 source-freeze commit/tag，完成双正式 builder/quorum 与最终 Kimi k3/max K1，并由人类签收 Flash oracle、可见 DWM/游戏 composite、真实手点与审美。它们全部通过也只证明视觉基座。B0 accepted 后再沿阶段2→3建立真实 state/UiData，并做 HP/MP 双轨对比，不能把 fixture 结论外推为运行态接入。
...
**文档治理**：本文为玩家信息 HUD 迁移的纲领 canonical doc；SVG/B0 专项文档只承载视觉渲染子域，避免形成第二份业务权威。B0-04 已同步 strict canonical validator 与 Web/FFDec/Flash diagnostic 入口；B0-03b 已同步生产包、embedded resource、runtime identity/policy/notice 与 `renderer_qualified / NOT_DEPLOYED` 边界；B0-05 已同步 raster/cache、`synthetic_fixed_bounds` 与 `split_required`。B0-06 已按实现基线 `bf8dd2c…8479` 的 fixture visual-state/widget/compositor/path glyph atlas、HP source-red Glow/MP no-Glow、surface 单一 model ownership、可靠 Resume pending、typed active/deferred effect policy、独立 split surface、PanelHost 生命周期、strict convergence qualification、最终机器报告与 A/B capture 索引同步 `launcher/README.md`、testing guide 与本路线；Web static-only 与 direct metrics 包含 scope 差的边界也已显式回写。本轮仍未声称 B0-04 人工视觉、真实 `pi_*`、可见 DWM/游戏交互、parity、promotion 或部署。双 builder/quorum、最终 K1 与人签完成后必须再次同提交同步并运行 doc governance；后续按实际改动回写本文 §5 路线与 §6 风险，行号锚点随重构刷新。
