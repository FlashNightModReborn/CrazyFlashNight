...
**最后核对代码基线**：release source commit `732898b8aa1308cf820976324f47bba97f654e41`（2026-08-27；implementation `32ae67a1c57e10002ed00354d172ae88207a24fa`、tag `runtime-build-v2/20260827-stage-outcome-settlement-v2`、tree `d498236a3ac06294f0c871dba73302bb24bc398e`、request `CB8268D83DB35DF3BC7D22587822F279C29A93CD12A15B1F10DF14759B000991`、deployment `339b15694d631d483736880c0dfd44429f6926a3`）。
本地 X509 `builder-local-a` / `physical-host-a`（keyId `28DBEAF3761CCF3177FE396596A2557D8A6C9393371CD41DC893FF75A02723B3`）与 GitHub OIDC/Sigstore builder `8D01D752BC01F072456E3A39D97A4CCDC7674C98552EF44EDEDA2C6C352D32C4` / `github-hosted-windows`（cloud run `33070890481`）已对 identity `03F9825C2A2A248C73A7FA6FFB4E6C4305D49C4F97BD9E6042747B9BC5A0F5AA`、closure `5DA2B3053BF70EFCDCE74A503C33098774F9026AAD6E2D1F555C1A9EC5181D07` 达成 v2 双 signer / 双故障域共识；正式 Core DLL SHA-256 为 `5B010C6040A5E80E1E8923FD02D6B632AB2D309511F8C6A54BA7D78B7516314F`，39/39 production receipt SHA-256 为 `977B2B2A7466C307198438C1F8362CE9ED06F3415977F664CB4E402B4192A181`；deployment commit `339b15694d631d483736880c0dfd44429f6926a3` 与首次 post-promotion audit run `33071572650` 已推送/通过，审计明确输出 `state=promoted`、`deploymentChanged=true`。

**状态边界**：本列车把关卡内死亡/胜负决策投影进 C# `StageOutcomeTask → RightContextWidget` 既有 32px 条件状态槽，把返回基地后的行动报告、奖励领取、材料存量与库存整理交给 Web `sourceKind=stage_settlement` 工作台。AS2 `StageRunSession` 与 `LootContainerService` 继续独占胜负/生命、30 FPS 帧时、复活币、转场、奖励随机化与资产写；设置页和原生状态槽共用同一幂等复活事务，批量领取一次最多 50 个 exact lease。
fresh 门包括 AS2 `301/301`（StageRunSession `88/88`）、Settings `42/42`、Host focused `37/37`、Launcher 全量 `4290 pass + 3 explicit opt-in skip / 4293 total`、Web `98/98、6/6、57/57、9/9、28/28、17/17、68/68、53/53、63/63、18/18`、Settings visual `116/116`、Workbench strict audit `0/0`、UI ratchet `67/67`、visual atlas `66/66`，以及 fresh Compiler `0/0`、FFDec 与 `scripts/asLoader.swf` SHA-256 `BB7823F9FF61F1B390810A8AD46913159DDED0FEE2926186451B255B01CFDB97`。production policy `39/39`、worktree/index 33-file bundle、signed consensus、原子 promotion 与正式根 bootstrap `--verify-only` 均通过。
维护者已确认与正式 Core 逐字节相同的 A3 隔离候选有效。无 candidate selector 的正式入口随后确认 `formal_runtime`、exact identity/closure、bus ready、正常 Guardian 关闭与零新增 Flash/WebView2/ports 残留；因为没有选择存档，预热在 deadline 后以 Flash code 0 安全回收，没有 fresh `bootstrap_reveal_ready`，也没有重跑关卡业务。因此准确状态为关卡结果与基地结算 `HUMAN_ACCEPTANCE_PASSED / promoted`；本轮正式入口只提供身份、总线和正常退出窄证据，不称业务或完整入口 `standard_entry_verified`。Audio H2 仍保持独立门。
