...
**文档角色**：以 2026-09-12 盘点为起点持续维护的完整 HUD 施工路线，记录剩余工作、复用范围、风险和下一步；不是已实现协议或验收报告。业务与有限重设计边界归[玩家信息 HUD 架构设计](玩家信息界面-NativeHud迁移-架构设计-2026-06-21.md)，HP/MP 历史视觉证据归 [B0 专项 ADR](玩家信息界面-NativeHud-SVG真源与程序化动效-B0-ADR与分片施工计划-2026-07-28.md)。

**最后核对代码基线**：commit `29567cb6c2f330410ac28250cabaa34ffc3eef60`（2026-09-19，限定 `defa9530f3` 切组图标收尾、资源区规划与 C# fixture 接入状态）。全域业务、资产计数和共享 Host 盘点仍对应 `cde09af93526dbe894aac90709d51ef978ac4c05`（2026-09-12）；其中 NPC/原生注释的在途描述保留当时含义，接线前重新读取其最终状态。

**本次修订范围**：读取提交差异、现役 XFL、C# 入口与 #71 维护者回执，同步设计范围、施工顺序和验收口径；未执行游戏、Flash 编译、Launcher 构建或部署，未重新测量性能或完成新布局人验。旧自动证据仍绑定各自基线。
...
**下一次可直接领取的施工内容**：从 B 开始前完成 A 的短核对，建立覆盖 §3 全区域的 AS2 投影结构和只读 C# 状态，再把已有 B0 surface 从 fixture-only 扩展为可选择真实状态的候选入口。首个可见里程碑必须同时显示 HP/MP、护盾、韧性、经验姓名、模式弹药、12 技能、活动药剂组和 Buff；新布局尚未完成的区域标记临时，不算完整迁移或新设计验收。
...
**本轮文档收尾后的交付状态**：现役点阵与资源区规划已区分，接数、视觉收束和验收口径已同步；资源区改版未实施，C# 完整生产迁移尚未完成。文档检查不构成源码测试、候选执行、游戏验收或发布证据。
