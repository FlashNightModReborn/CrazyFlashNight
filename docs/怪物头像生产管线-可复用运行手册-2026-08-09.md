...
**当前状态**：`OPERATIONAL_RUNBOOK / AUTOMATED_CLOSURE_VERIFIED / ARENA_EXACT_IDENTITY_CLOSED / NOT_GAME_E2E_VERIFIED`。2026-08-09 快照为 226 identity / 227 variant / 222 human-accepted variant / 2 pending / 1 excluded / 2 aliases，生产 manifest digest `EFDBD928…06E5`、receipt digest `17FC0D9B…0EDF`、supplement closure digest `C70B03A8…8AD3`、base manifest digest `EED4D8DC…01D0`；Arena 目录为 217/217 ready、0 locked fallback。这些数量是一次已验证快照，不是永久硬编码。
...
| 层 | 权威与职责 | 禁止事项 |
|---|---|---|
| 身份层 | `data/enemy_properties` 的敌人节点名默认就是稳定 `portraitRef`；只有共享身份时才写显式 `<portraitRef>` 覆盖，必要 variant 由稳定运行时状态选择 | 用文件名猜身份；要求每个 XML 都重复默认指针；把近似换皮自动合并 |
...
所有 worker 只写独立 shard；只有单一 controller 可以合并、生成回执和执行受控 promotion，运行时权威只由 manifest-last 的单文件替换切换。`candidate_proposed`、`automated_checked`、模型 A/B 一致、进程退出 0 和 mock-browser 全绿都不是 `human_reviewed` 或真实游戏验收。
...
flowchart LR
  ...
  E --> F["高分辨率确定性渲染"]
  F --> G["人类审阅"]
  G -->|pass| H["Manifest-last Promotion"]
  G -->|框选或方向明确| F
  G -->|wrong pose| D
...
6. 透明主体同时派生 SVG 与 512px PNG 回退；promotion 重扫 SVG 的嵌入栅格与字节比，纯矢量使用 SVG-first，任何含嵌入栅格的包装 SVG 一律 PNG-first，`isRasterHeavy` 只记录风险。边框、底色和氛围属于消费者展示层。
...
全量正式写入由 `promote-enemy-portraits-v1.py promote --replace-existing` 执行；已冻结基础包上的有限 Arena 尾批可由 `promote-arena-portrait-supplement-v1.py preflight/promote/check` 执行，但必须精确绑定基础 manifest digest、全部人审/方向/alias 回执。两条入口都先在 staging 完整预验，再依次发布内容寻址 subjects、版本化 backup、receipt，最后以单文件替换切换 manifest 这一唯一运行时权威，随后清理 stale subjects 并执行 shared manifest 复验。任一中断点都必须让当前 manifest 的引用保持完整，重跑须收敛 exact fileset 与 receipt；这不是整目录或多文件事务原子性证明。失败会保留完整旧闭包、恢复旧闭包，或保留引用完整的新闭包供同一命令重跑收敛，不做半包修补。当前直接回滚点为：
...
该目录在 .gitignore 覆盖下，只作本机运维恢复，不得写成 clean-checkout 权威证据；clean-checkout 的权威证据是随仓 evidence pack 与 receipt 闭包。
...
```text
python tools/portrait-pilot/build-enemy-portrait-evidence-pack-v1.py check
python tools/portrait-pilot/test-evidence-pack-v1.py
...
CF7_PORTRAIT_EVIDENCE_ONLY=1 python tools/portrait-pilot/promote-arena-portrait-supplement-v1.py check
CF7_PORTRAIT_EVIDENCE_ONLY=1 python tools/portrait-pilot/promote-enemy-portraits-v1.py check
python tools/portrait-pilot/test-evidence-only-full-build-v1.py
python tools/portrait-pilot/preserve-orientation-source-assets-v1.py check
python tools/portrait-pilot/audit-portrait-orientation-propagation-v1.py check --output tmp/portrait-pilot/orientation-audit-r221-full222-evidence-closure-20260809T220500Z --current
...
`CF7_PORTRAIT_EVIDENCE_ONLY=1` 两门强制在没有 `tmp/portrait-pilot` provenance 的 clean-checkout 语义下只使用随仓 immutable evidence，防止用现存 tmp 文件假绿；`test-evidence-only-full-build-v1.py` 只证明进程内 base→supplement assembly 564/564 全消费且不触达 live tmp（显式 stub 四个 historical verifier 子进程），不外推为子进程 clean-checkout 证明。

已冻结 internal-subject r180 批次的控制器/审阅器源码闭包由批次内 `internal-subject-source-supersession-v1.json` 收条承担后续正当漂移（见 tools/portrait-pilot/README.md「冻结批次的源码漂移超越收条」）；两个浏览器 harness `node tools/portrait-pilot/test-internal-subject-review-v1.js <r180 批次>` 与 `node tools/portrait-pilot/test-internal-subject-reconfirm-v1.js <r180 批次> '敌人-闪流步兵::default'` 须在收条下保持绿色。

涉及方向时，另外复验人工回执、当前 propagation audit，以及带显式 supersession receipt 的历史视觉审计。当前自动基线为：

- production：222 human-accepted；
- propagation：当前 r221 digest `DDC843A4…0576` 为 222/222、0 action/SVG/PNG mismatch、0 legacy visual audit required；来源分为基础 178 model keep + 39 human audit，以及 supplement 4 direct human pass + 1 explicit `flip_x`；r210/r220 只保留为旧 controller/manifest 的历史快照；
...
1. 更新 consumer inventory，确认敌人节点名默认身份、可选显式 `portraitRef` 覆盖与是否确有运行时 variant；不要因 XML 中没有冗余 `<portraitRef>` 子节点就把默认身份误报为缺失。
...
7. fresh 预演后执行 subjects-first / manifest-last promotion；重跑全量方向传播和消费者覆盖。
...
- Arena 原 6 项缺口已由 r219 增量包关闭：`敌人-红水晶 / 敌人-唐头肌肉男 / 敌人-锯片陷阱 / 敌人-旧型号机器人改 / 敌人-家用机器人` 绑定真人 4 pass + 1 orientation-only adjustment，家用机器人使用经校验的 `flip_x` 产物；`敌人-锡蒙利范围光环发生器` 的内部唯一帧只有 57×3、6 个可见黑像素，属于不可识别的退化逻辑图形，经人类明确批准后以 identity alias 复用 `敌人-锡蒙利::default`，不复制主体资产。alias receipt digest 为 `AF29A2B5…9ED`，当前 supplement closure digest 为 `C70B03A8…8AD3`。生产 checker 还要求 immutable evidence 精确闭合 353 条显式 records + 211 条 digest-bound selected-master 派生记录 = 564 条（560 条 `tmp/portrait-pilot` 生产证据 + 4 条 tracked controller provenance），并对 25 项 raw sidecar blob 逐条校验。runtime preserved evidence 精确为 12 条 orientation-only + 5 条 SVG reconstruction basis；后 5 条同时属于最终 runtime，而 evidence pack 的 `preservedSubjects` 只含前 12 条。故 442 个唯一 runtime 引用与 preserved 的去重并集、disk、tracked 均为 454，extra/missing 为 0。`CF7_PORTRAIT_EVIDENCE_ONLY=1` 是无 tmp provenance 的两级 clean-checkout checker 门；`test-evidence-only-full-build-v1.py` 只证明进程内 promotion assembly，显式不启动父进程 audit hook 无法覆盖的四个历史 verifier 子进程，后者由真实 normal supplement promotion 与 standalone checks 覆盖。
